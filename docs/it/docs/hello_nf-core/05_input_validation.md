...
In questa quinta parte del corso di formazione Hello nf-core, vi mostriamo come utilizzare il plugin nf-schema per validare gli input e i parametri della pipeline.
...
??? info "Come iniziare da questa sezione"
    ...
    Se non avete completato la Parte 4 o volete ripartire da zero per questa parte, potete utilizzare la soluzione `core-hello-part4` come punto di partenza.
...
Immaginate di eseguire la vostra pipeline per due ore, solo per vederla crashare perché un utente ha fornito un file con l'estensione sbagliata. O di passare ore a fare debug di errori criptici, solo per scoprire che un parametro era scritto male. Senza validazione degli input, questi scenari sono comuni.
...
La pipeline ha accettato input non validi ed è stata eseguita per ore prima di fallire. Con una validazione appropriata:
...
La pipeline fallisce immediatamente con messaggi di errore chiari e utilizzabili. Questo fa risparmiare tempo, risorse di calcolo e frustrazione.
...
Sebbene nf-schema funzioni con qualsiasi flusso di lavoro Nextflow, è la soluzione di validazione standard per tutte le pipeline nf-core.
...
- **Validazione del foglio campioni**: Valida i file di input rispetto a `assets/schema_input.json`
- **Conversione in canali**: Converte i fogli campioni validati in canali Nextflow
...
### 0.3. Due file di schema per due tipi di validazione

Una pipeline nf-core utilizzerà due file di schema separati, che corrispondono a due tipi di validazione:

| File di Schema             | Scopo                     | Valida                                               |
| -------------------------- | ------------------------- | ---------------------------------------------------- |
| `nextflow_schema.json`     | Validazione dei parametri | Flag da riga di comando: `--input`, `--outdir`, `--batch` |
| `assets/schema_input.json` | Validazione dei dati di input | Contenuto dei fogli campioni e dei file di input |

Entrambi gli schemi utilizzano il formato JSON Schema, uno standard ampiamente adottato per descrivere e validare strutture dati.

**La validazione dei parametri** valida i parametri da riga di comando (flag come `--outdir`, `--batch`, `--input`):

- Controlla tipi, intervalli e formati dei parametri
- Assicura che i parametri richiesti siano forniti
- Valida che i percorsi dei file esistano
- Definita in `nextflow_schema.json`

**La validazione dei dati di input** valida la struttura dei fogli campioni e dei file manifest (file CSV/TSV che descrivono i vostri dati):

- Controlla la struttura delle colonne e i tipi di dati
- Valida che i percorsi dei file referenziati nel foglio campioni esistano
- Assicura che i campi richiesti siano presenti
- Definita in `assets/schema_input.json`

!!! warning "Cosa NON fa la validazione dei dati di input"

    La validazione dei dati di input controlla la struttura dei *file manifest* (fogli campioni, file CSV), NON il contenuto dei vostri file di dati effettivi (FASTQ, BAM, VCF, ecc.).

    Per dati su larga scala, la validazione del contenuto dei file (come il controllo dell'integrità BAM) dovrebbe avvenire nei processi della pipeline in esecuzione sui nodi worker, non durante la fase di validazione sulla macchina di orchestrazione.
...
La validazione dovrebbe avvenire **prima** che qualsiasi processo della pipeline venga eseguito, per fornire un feedback rapido e prevenire spreco di tempo di calcolo.
...
### 1.1. Configurare la validazione per saltare la validazione dei file di input
...
- La validazione viene eseguita dal subflusso di lavoro `UTILS_NFSCHEMA_PLUGIN` durante l'inizializzazione della pipeline

Il comportamento della validazione è controllato tramite lo scope `validation{}` in `nextflow.config`.

Poiché lavoreremo prima sulla validazione dei parametri (questa sezione) e non configureremo lo schema dei dati di input fino alla sezione 2, dobbiamo temporaneamente dire a nf-schema di saltare la validazione del contenuto del file del parametro `input`.
...
- **`monochromeLogs`**: Disabilitare l'output colorato nei messaggi di validazione quando impostato a `true` (controllato da `params.monochrome_logs`)
...
!!! note "Perché ignorare il parametro input?"
    ...
    Rimuoveremo questa impostazione nella sezione 2 dopo aver configurato lo schema dei dati di input.

### 1.2. Esaminare lo schema dei parametri

Diamo un'occhiata a una sezione del file `nextflow_schema.json` fornito con il nostro template di pipeline:
...
- **`type`**: Tipo di dati (string, integer, boolean, number)
- **`format`**: Formati speciali come `file-path` o `directory-path`
- **`exists`**: Per i percorsi dei file, controlla se il file esiste
...
Se avete occhio attento, potreste notare che il parametro di input `batch` che abbiamo utilizzato non è ancora definito nello schema.
...
??? info "Da dove provengono i parametri dello schema?"
    ...
    I parametri dichiarati altrove negli script del vostro flusso di lavoro (come in `main.nf` o nei file dei moduli) **non** vengono automaticamente rilevati dal validatore dello schema.
...
Sebbene lo schema sia un file JSON che può essere modificato manualmente, **la modifica manuale è soggetta a errori e non è raccomandata**.
...
Il vostro browser si aprirà mostrando il costruttore di schema dei parametri:

![Interfaccia del costruttore di schema](./img/schema_build.png)

Per aggiungere il parametro `batch`:

1. Cliccate sul pulsante **"Add parameter"** in alto
2. Utilizzate la maniglia di trascinamento (⋮⋮) per spostare il nuovo parametro nel gruppo "Input/output options", sotto il parametro `input`
3. Compilate i dettagli del parametro:
   - **ID**: `batch`
   - **Description**: `Name for this batch of greetings`
   - **Type**: `string`
   - **Required**: spuntate la casella
   - Opzionalmente, selezionate un'icona dal selettore di icone (es. `fas fa-layer-group`)
...
Premete `Ctrl+C` per uscire dal costruttore di schema.
...
Prima, provate a eseguire senza il parametro richiesto `input`:
...
Perfetto! La validazione cattura il parametro richiesto mancante prima che la pipeline venga eseguita.
...
L'interfaccia web gestisce tutta la sintassi JSON Schema per voi, rendendo facile gestire schemi di parametri complessi senza modifiche manuali del JSON soggette a errori.

### Cosa c'è dopo?

Ora che la validazione dei parametri funziona, aggiungiamo la validazione per il contenuto del file di dati di input.

---

## 2. Validazione dei dati di input (schema_input.json)

Aggiungeremo la validazione per il contenuto del nostro file CSV di input.
Mentre la validazione dei parametri controlla i flag da riga di comando, la validazione dei dati di input assicura che i dati all'interno del file CSV siano strutturati correttamente.

### 2.1. Comprendere il formato di greetings.csv
...
- Tre colonne (senza intestazione)
...
3. Assicurare che i saluti non siano vuoti e non inizino con spazi bianchi
4. Assicurare che il campo lingua corrisponda a uno dei codici lingua supportati (en, fr, es, it, de)
5. Assicurare che il campo punteggio sia un intero con un valore tra 0 e 100

Struttureremo questo come un array di oggetti, dove ogni oggetto ha almeno un campo `greeting`.

### 2.3. Aggiornare il file di schema

Il template della pipeline nf-core include un `assets/schema_input.json` predefinito progettato per dati di sequenziamento paired-end.
...
- **`properties`**: Sostituite `sample`, `fastq_1` e `fastq_2` con `greeting`, `language` e `score`
  ...
  - **`"minimum": 0` e `"maximum": 100`**: Il valore del punteggio deve essere tra 0 e 100
...
Quando nf-schema legge un file CSV, si aspetta che la prima riga contenga intestazioni di colonna che corrispondano ai nomi dei campi nello schema.
...
Il passo finale è implementare la validazione nel codice della pipeline utilizzando `samplesheetToList`.

### 2.5. Implementare la validazione nella pipeline

Ora dobbiamo sostituire il nostro semplice parsing CSV con la funzione `samplesheetToList` di nf-schema, che validerà e analizzerà il foglio campioni.

La funzione `samplesheetToList`:

1. Legge il foglio campioni di input (CSV, TSV, JSON o YAML)
...
3. Estrarre solo le stringhe di saluto per il nostro flusso di lavoro
...
Questo completa l'implementazione della validazione dei dati di input utilizzando `samplesheetToList` e schemi JSON.

Ora che abbiamo configurato lo schema dei dati di input, possiamo rimuovere l'impostazione di ignore temporanea che abbiamo aggiunto in precedenza.
...
Verifichiamo che la nostra validazione funzioni correttamente testando sia input validi che non validi.

#### 2.7.1. Testare con input valido

Prima, confermiamo che la pipeline venga eseguita con successo con input valido.
...
#### 2.7.2. Testare con input non valido

Superare la validazione è sempre una bella sensazione, ma assicuriamoci che la validazione catturi effettivamente gli errori.
...
Perfetto! La validazione ha catturato l'errore e ha fornito un messaggio di errore chiaro e utile che indica:
...
Avete implementato e testato sia la validazione dei parametri che la validazione dei dati di input. La vostra pipeline ora valida gli input prima dell'esecuzione, fornendo feedback rapido e messaggi di errore chiari.

!!! tip "Ulteriori letture"

    Per saperne di più su funzionalità e pattern di validazione avanzati, consultate la [documentazione di nf-schema](https://nextflow-io.github.io/nf-schema/latest/). Il comando `nf-core pipelines schema build` fornisce una GUI interattiva per gestire schemi complessi.
