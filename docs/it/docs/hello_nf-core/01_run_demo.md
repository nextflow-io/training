...
In questa prima parte del corso di formazione Hello nf-core, vi mostriamo come trovare e provare una pipeline nf-core, comprendere come è organizzato il codice e riconoscere come differisce dal codice Nextflow base mostrato in [Hello Nextflow](../hello_nextflow/index.md).
...
Assicuratevi che la vostra directory di lavoro sia impostata su `hello-nf-core/` come indicato nella pagina [Per iniziare](./00_orientation.md).

---

## 1. Trovare e recuperare la pipeline nf-core/demo

Iniziamo localizzando la pipeline nf-core/demo sul sito web del progetto all'indirizzo [nf-co.re](https://nf-co.re), che centralizza tutte le informazioni come: documentazione generale e articoli di aiuto, documentazione per ciascuna delle pipeline, post del blog, annunci di eventi e così via.
...
Ogni volta che state considerando di adottare una nuova pipeline, dovreste leggere attentamente la documentazione della pipeline prima per capire cosa fa e come dovrebbe essere configurata prima di tentare di eseguirla.
...
- Quali input e parametri accetta o richiede la pipeline (Controllate la scheda: `Parameters`)
...
Noterete che il comando di esempio NON specifica un file di flusso di lavoro, solo il riferimento al repository della pipeline, `nf-core/demo`.
...
Nextflow esegue un `pull` del codice della pipeline, il che significa che scarica l'intero repository sul vostro disco locale.

Per essere chiari, potete farlo con qualsiasi pipeline Nextflow che sia configurata appropriatamente in GitHub, non solo con le pipeline nf-core.
Tuttavia nf-core è la più grande collezione open-source di pipeline Nextflow.

Potete far sì che Nextflow vi fornisca un elenco delle pipeline che avete recuperato in questo modo:
...
Nextflow mantiene il codice sorgente scaricato intenzionalmente 'fuori mano' sul principio che queste pipeline dovrebbero essere utilizzate più come librerie che come codice con cui interagire direttamente.

Tuttavia, ai fini di questa formazione, vogliamo essere in grado di curiosare e vedere cosa c'è dentro.
Quindi per rendere questo più facile, creiamo un collegamento simbolico a quella posizione dalla nostra directory di lavoro corrente.

```bash
ln -s $NXF_HOME/assets pipelines
```

Questo crea una scorciatoia che rende più facile esplorare il codice che abbiamo appena scaricato.
...
Ora possiamo più facilmente dare un'occhiata al codice sorgente secondo necessità.
...
Questo è un insieme minimo di impostazioni di configurazione per la pipeline da eseguire utilizzando un piccolo dataset di test ospitato nel repository [nf-core/test-datasets](https://github.com/nf-core/test-datasets).
È un ottimo modo per provare rapidamente una pipeline su piccola scala.

!!! note

    Il sistema di profili di configurazione di Nextflow vi consente di passare facilmente tra diversi motori di container o ambienti di esecuzione.
...
È buona pratica controllare cosa specifica il profilo di test di una pipeline prima di eseguirla.
Il profilo `test` per `nf-core/demo` si trova nel file di configurazione `conf/test.config` ed è mostrato di seguito.
...
Come promemoria, `<docker/singularity>` si riferisce alla scelta del sistema di container. Tutte le pipeline nf-core sono progettate per essere utilizzabili con container (Docker, Singularity, ecc.) per garantire riproducibilità ed eliminare problemi di installazione del software.
Quindi dovremo specificare se vogliamo usare Docker o Singularity per testare la pipeline.

La parte `--outdir <OUTDIR>` si riferisce alla directory dove Nextflow scriverà gli output della pipeline.
...
Passando alla sezione dopo il blocco di commenti, il profilo di test ci mostra cosa è stato preconfigurato per il test: in particolare, il parametro `input` è già impostato per puntare a un dataset di test, quindi non abbiamo bisogno di fornire i nostri dati.
...
Noterete che c'è molto più output sulla console rispetto a quando eseguite una pipeline Nextflow base.
...
Questi includono i nomi dei loro flussi di lavoro genitori e riflettono la modularità del codice della pipeline.
...
Per saperne di più sugli output della pipeline `nf-core/demo`, consultate la sua [pagina di documentazione](https://nf-co.re/demo/1.0.2/docs/output/).
...
!!! note
    ...
    Per vederle eseguire in parallelo, provate ad aumentare l'allocazione di CPU del vostro codespace e i limiti di risorse nella configurazione di test.
...
Ora che abbiamo eseguito con successo la pipeline come utenti, cambiamo prospettiva per guardare come sono strutturate internamente le pipeline nf-core.

Il progetto nf-core applica linee guida rigorose su come sono strutturate le pipeline, e su come il codice è organizzato, configurato e documentato.
Comprendere come è tutto organizzato è il primo passo verso lo sviluppo delle vostre pipeline compatibili con nf-core, che affronteremo nella Parte 2 di questo corso.

Diamo un'occhiata a come è organizzato il codice della pipeline nel repository `nf-core/demo`, utilizzando il symlink `pipelines` che abbiamo creato in precedenza.
...
C'è molto in corso lì dentro, quindi affronteremo questo passo dopo passo.

Prima di tutto, notiamo che al livello superiore, potete trovare un file README con informazioni di riepilogo, così come file accessori che riassumono informazioni sul progetto come licenza, linee guida per i contributi, citazioni e codice di condotta.
...
L'organizzazione standard del codice della pipeline nf-core segue una struttura modulare progettata per massimizzare il riutilizzo del codice, come introdotto in [Hello Modules](../hello_nextflow/04_hello_modules.md), Parte 4 del corso [Hello Nextflow](../hello_nextflow/index.md), anche se in vero stile nf-core, questo è implementato con un po' di complessità aggiuntiva.
Nello specifico, le pipeline nf-core fanno abbondante uso di subworkflow, cioè script di flusso di lavoro che sono importati da un flusso di lavoro genitore.

Potrebbe sembrare un po' astratto, quindi diamo un'occhiata a come questo viene utilizzato in pratica nella pipeline `nf-core/demo`.

!!! note

    Non esamineremo il codice effettivo per _come_ questi componenti modulari sono connessi, perché c'è una complessità aggiuntiva associata all'uso dei subworkflow che può essere confusa, e comprendere ciò non è necessario in questa fase della formazione.
...
C'è un cosiddetto script _entrypoint_ chiamato `main.nf`, che funge da wrapper per due tipi di flussi di lavoro annidati: il flusso di lavoro contenente la logica di analisi effettiva, situato sotto `workflows/` e chiamato `demo.nf`, e un insieme di flussi di lavoro di gestione situati sotto `subworkflows/`.
Il flusso di lavoro `demo.nf` richiama **moduli** situati sotto `modules/`; questi contengono i **processi** che eseguiranno i passaggi di analisi effettivi.

!!! note

    I subworkflow non sono limitati alle funzioni di gestione, e possono utilizzare moduli di processo.
...
L'utilizzo di uno script entrypoint rende facile eseguire subworkflow di 'gestione' standardizzati prima e dopo l'esecuzione dello script di analisi effettivo.
Esamineremo quelli dopo aver esaminato il flusso di lavoro di analisi effettivo e i suoi moduli.
...
È strutturato in modo molto simile a un normale flusso di lavoro Nextflow, tranne per il fatto che è progettato per essere chiamato da un flusso di lavoro genitore, il che richiede alcune funzionalità extra.
...
Il flusso di lavoro `demo.nf` richiama **moduli** situati sotto `modules/`, che esamineremo successivamente.

!!! note

    Alcuni flussi di lavoro di analisi nf-core mostrano livelli aggiuntivi di annidamento richiamando subworkflow di livello inferiore.
    Questo è utilizzato principalmente per avvolgere due o più moduli che sono comunemente utilizzati insieme in segmenti di pipeline facilmente riutilizzabili.
    Potete vedere alcuni esempi sfogliando i [subworkflow nf-core](https://nf-co.re/subworkflows/) disponibili sul sito web nf-core.
...
Nel progetto nf-core, i moduli sono organizzati utilizzando una struttura annidata multi-livello che riflette sia la loro origine che il loro contenuto.
Al livello superiore, i moduli sono differenziati come `nf-core` o `local` (non parte del progetto nf-core), e poi ulteriormente collocati in una directory denominata secondo lo/gli strumento/i che avvolgono.
Se lo strumento appartiene a un toolkit (cioè un pacchetto contenente più strumenti) allora c'è un livello di directory intermedio denominato secondo il toolkit.
...
Il file di codice del modulo che descrive il processo è sempre chiamato `main.nf`, ed è accompagnato da test e file `.yml` che ignoreremo per ora.

Presi insieme, il flusso di lavoro entrypoint, il flusso di lavoro di analisi e i moduli sono sufficienti per eseguire le parti 'interessanti' della pipeline.
...
Come notato sopra, la pipeline `nf-core/demo` non include alcun subworkflow specifico per l'analisi, quindi tutti i subworkflow che vediamo qui sono cosiddetti flussi di lavoro di 'gestione' o 'utilità', come indicato dal prefisso `utils_` nei loro nomi.
...
!!! tip
    ...
    Oltre al loro schema di denominazione, un'altra indicazione che questi subworkflow non eseguono alcuna funzione veramente correlata all'analisi è che non chiamano alcun processo.

Questo completa il riepilogo dei componenti di codice principali che costituiscono la pipeline `nf-core/demo`.
Ora diamo un'occhiata agli elementi rimanenti di cui dovreste sapere un po' prima di immergervi nello sviluppo: configurazione della pipeline e validazione dell'input.

### 3.2. Configurazione della pipeline

Avete appreso in precedenza che Nextflow offre molte opzioni per configurare l'esecuzione della pipeline, sia in termini di input e parametri, risorse di calcolo e altri aspetti dell'orchestrazione.
Il progetto nf-core applica linee guida altamente standardizzate per la configurazione della pipeline che mirano a costruire sulle opzioni di personalizzazione flessibili di Nextflow in un modo che fornisce maggiore coerenza e manutenibilità tra le pipeline.

Il file di configurazione centrale `nextflow.config` viene utilizzato per impostare valori predefiniti per parametri e altre opzioni di configurazione.
La maggior parte di queste opzioni di configurazione sono applicate per impostazione predefinita mentre altre (ad es., profili di dipendenze software) sono incluse come profili opzionali.
...
- `base.config`: Un file di configurazione 'tabula rasa', appropriato per l'uso generale sulla maggior parte degli ambienti di calcolo ad alte prestazioni. Questo definisce ampi contenitori di utilizzo delle risorse, per esempio, che sono convenienti da applicare ai moduli.
- `modules.config`: Direttive e argomenti aggiuntivi del modulo.
- `test.config`: Un profilo per eseguire la pipeline con dati di test minimi, che abbiamo utilizzato quando abbiamo eseguito la pipeline demo.
- `test_full.config`: Un profilo per eseguire la pipeline con un dataset di test di dimensioni complete.
...
Un samplesheet di esempio è anche fornito sotto la directory `assets`, anche se i percorsi in questo non sono reali.
...
Sfortunatamente, poiché questi file possono essere difficili da controllare a occhio, la formattazione impropria dei dati di input è una fonte molto comune di fallimenti della pipeline.
Un problema correlato è quando i parametri sono forniti in modo errato.

La soluzione a questi problemi è eseguire controlli di validazione automatizzati su tutti i file di input per garantire che contengano i tipi di informazioni attesi, formattati correttamente, e sui parametri per garantire che siano del tipo atteso.
...
Per ora, siate solo consapevoli che ci sono due file JSON forniti per quello scopo, `nextflow_schema.json` e `assets/schema_input.json`.

Il `nextflow_schema.json` è un file utilizzato per memorizzare informazioni sui parametri della pipeline inclusi tipo, descrizione e testo di aiuto in un formato leggibile dalla macchina.
Questo viene utilizzato per vari scopi, inclusa la validazione automatizzata dei parametri, la generazione di testo di aiuto e il rendering di moduli di parametri interattivi nelle interfacce UI.

Il `schema_input.json` è un file utilizzato per definire la struttura del samplesheet di input.
Ogni colonna può avere un tipo, pattern, descrizione e testo di aiuto in un formato leggibile dalla macchina.
Lo schema viene utilizzato per vari scopi, inclusa la validazione automatizzata e la fornitura di messaggi di errore utili.

### Takeaway

Sapete quali sono i componenti principali di una pipeline nf-core e come è organizzato il codice; dove si trovano gli elementi principali della configurazione; e siete consapevoli di cosa serve la validazione dell'input.

### Cosa c'è dopo?

Prendetevi una pausa! È stato molto. Quando vi sentite riposati e pronti, passate alla sezione successiva per applicare ciò che avete imparato per scrivere una pipeline compatibile con nf-core.

!!! tip

    Se desiderate imparare come comporre flussi di lavoro con subworkflow prima di passare alla parte successiva, consultate la [Workflows of Workflows](../side_quests/workflows_of_workflows.md) Side Quest.
