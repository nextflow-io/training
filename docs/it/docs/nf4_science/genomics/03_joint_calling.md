...
Questa parte si basa direttamente sul workflow prodotto dalla Parte 2.
...
??? info "Come iniziare da questa sezione"
    ...
    Questo vi fornisce un workflow completo di variant calling per-campione.
...
- Aggiornare il percorso del file di output per usare l'estensione `.g.vcf` corrispondente, secondo la convenzione GATK.
...
Dobbiamo anche aggiornare la configurazione di publish e output del workflow per riflettere i nuovi output GVCF.

### 1.3. Aggiornare i target di publish per i nuovi output GVCF

Poiché ora stiamo producendo GVCF invece di VCF, dovremmo aggiornare la sezione `publish:` del workflow per usare nomi più descrittivi.
...
Eseguite il workflow per verificare che le modifiche funzionino.
...
Possiamo raccogliere i GVCF e i loro file indice separatamente (invece di tenerli insieme in tuple) perché Nextflow metterà in staging tutti i file di input insieme per l'esecuzione, quindi i file indice saranno presenti accanto ai GVCF.
...
### 2.2. Scrivere il processo di joint genotyping e chiamarlo nel workflow

Seguendo lo stesso schema che abbiamo usato nella Parte 2, scriveremo la definizione del processo in un file modulo, lo importeremo nel workflow e lo chiameremo sugli input che abbiamo appena preparato.
...
Importante, dobbiamo costruire questa stringa dinamicamente da qualsiasi file sia nel canale raccolto.
...
Aprite `modules/gatk_jointgenotyping.nf` ed esaminate lo schema della definizione del processo.
...
Elencarli assicura che Nextflow metta in staging questi file nella directory di lavoro accanto ai file che appaiono nei comandi, che GATK si aspetta di trovare in base alle convenzioni di denominazione.
...
Per usarlo nel workflow, dovrete importare il modulo e aggiungere una chiamata di processo.
...
Aggiungete la chiamata a `GATK_JOINTGENOTYPING` nel corpo del workflow, dopo le righe `collect()`:
...
Aggiungete il VCF joint e il suo indice alla sezione `publish:` del workflow:
...
Con il processo, i target di publish e il blocco output tutti a posto, possiamo testare il workflow completo.

### 2.4. Eseguire il workflow

Eseguite il workflow per verificare che tutto funzioni.
...
Se aprite il file VCF joint, potete verificare che il workflow abbia prodotto le chiamate di varianti attese.
...
Ora avete un workflow di joint variant calling automatizzato e completamente riproducibile!
...
Sapete anche come costruire una riga di comando usando le closure Groovy, e come eseguire più comandi in un singolo processo.
