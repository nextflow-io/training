...
Qui utilizzeremo strumenti e metodi progettati per chiamare varianti germinali brevi, _cioè_ SNP e indel, in dati di sequenziamento dell'intero genoma.

![GATK pipeline](img/gatk-pipeline.png)

Una pipeline completa di variant calling coinvolge tipicamente molti passaggi, inclusi il mapping al riferimento (talvolta chiamato allineamento genomico) e il filtraggio e la prioritizzazione delle varianti.
...
Prima inizieremo con un semplice **approccio per-campione** che chiama le varianti indipendentemente da ciascun campione.
...
Prima di addentrarci nella scrittura di qualsiasi codice di workflow per entrambi gli approcci, proveremo i comandi manualmente su alcuni dati di test.
...
- **Una lista di intervalli genomici**, cioè coordinate sul genoma dove i nostri campioni hanno dati adatti per chiamare varianti, fornita in formato BED.
...
Questi strumenti non sono installati nell'ambiente GitHub Codespaces, quindi li utilizzeremo tramite container (vedi [Hello Containers](../../hello_nextflow/05_hello_containers.md)).

!!! note

     Assicuratevi di essere nella directory `nf4-science/genomics` in modo che l'ultima parte del percorso mostrato quando digitate `pwd` sia `genomics`.
...
In questa sezione testiamo i due comandi che compongono l'approccio di variant calling per-campione: indicizzare un file BAM con Samtools e chiamare varianti con GATK HaplotypeCaller.
...
2. Eseguire GATK HaplotypeCaller sul file BAM indicizzato per generare chiamate di varianti per-campione in VCF (Variant Call Format)
...
Il vostro prompt cambia in qualcosa come `(base) root@a1b2c3d4e5f6:/tmp#`, indicando che ora siete all'interno del container.
I file di dati sono accessibili sotto `/data`.
...
### 1.2. Chiamare varianti con GATK HaplotypeCaller
...
Questo dovrebbe essere più veloce del primo pull perché le due immagini del container condividono la maggior parte dei loro layer.
...
Il vostro prompt cambia per indicare che ora siete all'interno del container GATK.

#### 1.2.3. Eseguire il comando di variant calling

La [documentazione di GATK](https://gatk.broadinstitute.org/hc/en-us/articles/21905025322523-HaplotypeCaller) ci fornisce la riga di comando da eseguire per effettuare il variant calling su un file BAM.
...
Tuttavia, non dobbiamo specificare il percorso del file indice; lo strumento lo cercherà automaticamente nella stessa directory, basandosi sulla convenzione di denominazione e co-locazione stabilita.
...
??? success "Output del comando"
    ...
    Lo strumento produce output di logging dettagliato. Le righe evidenziate confermano il completamento con successo.
...
Il file di output `reads_mother.vcf` viene creato all'interno della vostra directory di lavoro nel container, quindi non lo vedrete nell'esploratore file di VS Code a meno che non cambiate il percorso del file di output.
Tuttavia, è un piccolo file di test, quindi potete usare `cat` per aprirlo e visualizzarne i contenuti.
Se scorrete fino all'inizio del file, troverete un'intestazione composta da molte righe di metadati, seguita da una lista di chiamate di varianti, una per riga.
...
Ogni riga descrive una possibile variante identificata nei dati di sequenziamento del campione. Per una guida sull'interpretazione del formato VCF, consultate [questo articolo utile](https://www.ebi.ac.uk/training/online/courses/human-genetic-variation-introduction/variant-identification-and-analysis/understanding-vcf-format/).
...
L'approccio di variant calling che abbiamo appena utilizzato genera chiamate di varianti per-campione.
Va bene per esaminare le varianti di ciascun campione in isolamento, ma fornisce informazioni limitate.
È spesso più interessante vedere come le chiamate di varianti differiscono tra più campioni.
...
Ciò che è speciale nel GVCF di un campione è che contiene record che riassumono le statistiche dei dati di sequenza su tutte le posizioni nell'area mirata del genoma, non solo le posizioni in cui il programma ha trovato evidenza di variazione.
...
La combinazione dei GVCF viene effettuata con GATK GenomicsDBImport, che combina le chiamate per-campione in un data store (analogo a un database).
L'analisi di 'joint genotyping' vera e propria viene quindi effettuata con GATK GenotypeGVCFs.

Qui testiamo i comandi necessari per generare GVCF ed eseguire il joint genotyping.
...
Ora dobbiamo testare tutti questi comandi, iniziando con l'indicizzazione di tutti e tre i file BAM.
...
### 2.2. Generare GVCF per tutti e tre i campioni

Per eseguire il passaggio di joint genotyping, abbiamo bisogno di GVCF per tutti e tre i campioni.
...
Per produrre un genomic VCF (GVCF), aggiungiamo l'opzione `-ERC GVCF` al comando base, che attiva la modalità GVCF di HaplotypeCaller.
...
Se usate `cat` per visualizzarne i contenuti, vedrete che è molto più lungo del VCF equivalente che abbiamo generato nella sezione 1. Non potete nemmeno scorrere fino all'inizio del file, e la maggior parte delle righe appare abbastanza diversa da ciò che abbiamo visto nel VCF.
...
Queste rappresentano regioni non-varianti dove il variant caller non ha trovato evidenza di variazione, quindi ha catturato alcune statistiche che descrivono il suo livello di confidenza nell'assenza di variazione.
Questo rende possibile distinguere tra due casi molto diversi: (1) ci sono dati di buona qualità che mostrano che il campione è omozigote-riferimento, e (2) non ci sono abbastanza dati di buona qualità disponibili per fare una determinazione in un modo o nell'altro.
...
Provate a eseguire `head -176` sul GVCF per caricare solo le prime 176 righe del file per trovare una chiamata di variante effettiva.
...
Proprio come il VCF originale, anche il file GVCF di output è accompagnato da un file indice, chiamato `reads_mother.g.vcf.idx`.

#### 2.2.3. Ripetere il processo sugli altri due campioni

Generate GVCF per i restanti due campioni eseguendo i comandi seguenti, uno dopo l'altro.
...
Utilizzeremo lo stesso container nel prossimo passaggio.

### 2.3. Eseguire il joint genotyping

Ora che abbiamo tutti i GVCF, possiamo provare l'approccio di joint genotyping per generare chiamate di varianti per una coorte di campioni.
È un metodo in due passaggi che consiste nel combinare i dati di tutti i GVCF in un data store, quindi eseguire l'analisi di joint genotyping vera e propria per generare il VCF finale di varianti chiamate congiuntamente.
...
L'output di questo passaggio è effettivamente una directory contenente un insieme di directory ulteriormente annidate che contengono i dati di varianti combinati sotto forma di più file diversi.
Potete esplorarla ma vedrete rapidamente che questo formato di data store non è destinato a essere letto direttamente dagli esseri umani.

!!! note

    GATK include strumenti che rendono possibile ispezionare ed estrarre dati di chiamate di varianti dal data store secondo necessità.

#### 2.3.2. Eseguire l'analisi di joint genotyping vera e propria

Questo secondo passaggio utilizza un altro strumento GATK, chiamato GenotypeGVCFs, per ricalcolare le statistiche delle varianti e i genotipi individuali alla luce dei dati disponibili su tutti i campioni nella coorte.
...
È un altro file ragionevolmente piccolo quindi potete usare `cat` su questo file per visualizzarne i contenuti, e scorrere verso l'alto per trovare le prime righe di varianti.
...
Se guardiamo i genotipi chiamati per il nostro trio familiare di test per la primissima variante, vediamo che il padre è eterozigote-variante (`0/1`), e la madre e il figlio sono entrambi omozigoti-variante (`1/1`).
...
Sapete come testare i comandi di indicizzazione Samtools e variant calling GATK nei rispettivi container, incluso come generare GVCF ed eseguire il joint genotyping su più campioni.
