...
Nella Parte 1, avete testato i comandi Samtools e GATK manualmente nei rispettivi container.
...
1. Generare un file di indice per ogni file BAM di input utilizzando [Samtools](https://www.htslib.org/)
...
Come punto di partenza, vi forniamo un file di flusso di lavoro, `genomics.nf`, che delinea le parti principali del flusso di lavoro, insieme a due file modulo, samtools_index.nf e gatk_haplotypecaller.nf, che delineano la struttura dei moduli.
...
Per rendere il processo di sviluppo più educativo, abbiamo suddiviso il lavoro in quattro passaggi:
...
4. **Fare in modo che il flusso di lavoro accetti un file di testo contenente un batch di file di input.**
...
Ricordate il comando `samtools index` dalla [Parte 1](01_method.md):

```bash
samtools index '<input_bam>'
```

Il comando prende un file BAM come input e produce un file di indice `.bai` accanto ad esso.
...
Ci sono diverse opzioni per fornire un valore predefinito; qui utilizziamo un profilo di test.
...
Aggiungete un blocco `profiles` a `nextflow.config` con un profilo `test` che imposta il parametro `reads_bam` su uno dei file BAM di test.
...
Nel blocco workflow, create un canale di input dal valore del parametro utilizzando la fabbrica di canali `.fromPath` (come usato in [Hello Channels](../../hello_nextflow/02_hello_channels.md)).
...
Aprite `modules/samtools_index.nf` ed esaminate la struttura della definizione del processo.
...
Il blocco `output {}` si trova fuori dal flusso di lavoro e specifica dove viene pubblicato ogni target denominato.
...
!!! note
    ...
    I link simbolici si romperanno quando pulite la vostra directory `work`, quindi per flussi di lavoro in produzione potreste voler sovrascrivere la modalità di pubblicazione predefinita con `'copy'`.
...
Ricordate il comando `gatk HaplotypeCaller` dalla [Parte 1](01_method.md):
...
Ora dobbiamo creare variabili che caricano questi percorsi di file per l'uso nel flusso di lavoro.
...
GATK sa cercare il file di indice BAM e i file accessori del genoma di riferimento in base alle convenzioni di denominazione, ma Nextflow è domain-agnostic e non conosce queste convenzioni.
Dobbiamo elencarli esplicitamente in modo che Nextflow li prepari nella directory di lavoro al momento dell'esecuzione; altrimenti GATK genererà un errore sui file mancanti.

Allo stesso modo, elenchiamo esplicitamente il file di indice del VCF di output (`"${input_bam}.vcf.idx"`) in modo che Nextflow ne tenga traccia per i passaggi successivi.
...
Dovreste riconoscere la sintassi `*.out` dalla serie di formazione Hello Nextflow; stiamo dicendo a Nextflow di prendere il canale emesso da `SAMTOOLS_INDEX` e collegarlo alla chiamata del processo `GATK_HAPLOTYPECALLER`.
...
!!! note
    ...
    In Nextflow, gli input sono posizionali, il che significa che _dovete_ seguire lo stesso ordine; e ovviamente deve esserci lo stesso numero di elementi.
...
Il primo processo è stato saltato grazie alla cache, come previsto, mentre il secondo processo è stato eseguito poiché è nuovo di zecca.
...
Fare in modo che il flusso di lavoro gestisca più campioni in blocco.

---

## 3. Adattare il flusso di lavoro per eseguirlo su un batch di campioni

Va benissimo avere un flusso di lavoro che può automatizzare l'elaborazione su un singolo campione, ma cosa succede se avete 1000 campioni?
Dovete scrivere uno script bash che cicla attraverso tutti i vostri campioni?
...
Provate a eseguire il flusso di lavoro ora che il collegamento è configurato per eseguire su tutti e tre i campioni di test.

```bash
nextflow run genomics.nf -profile test -resume
```

Cosa curiosa: questo _potrebbe funzionare_, OPPURE _potrebbe fallire_. Ad esempio, ecco un'esecuzione che ha avuto successo:
...
Beh, è strano, considerando che abbiamo esplicitamente indicizzato i file BAM nel primo passaggio del flusso di lavoro. Potrebbe esserci qualcosa di sbagliato nel collegamento?
...
Prestate particolare attenzione ai nomi del file BAM e dell'indice BAM elencati in questa directory: `reads_son.bam` e `reads_father.bam.bai`.

Ma che diavolo? Nextflow ha preparato un file di indice nella directory di lavoro di questa chiamata al processo, ma è quello sbagliato. Come può essere successo?
...
Aggiungete queste due righe nel corpo del workflow prima della chiamata al processo `GATK_HAPLOTYPECALLER` per visualizzare i contenuti del canale:
...
Potete vedere che i file BAM e i file di indice per i tre campioni non sono elencati nello stesso ordine!
...
Come attualmente scritto, il nostro script del flusso di lavoro presume che i file di indice usciranno dal passaggio di indicizzazione elencati nello stesso ordine madre/padre/figlio in cui sono stati forniti gli input.
...
!!! tip
    ...
    Tuttavia ingombreranno l'output della console, quindi raccomandiamo di rimuoverle quando avete finito di risolvere il problema.

### 3.4. Aggiornare il flusso di lavoro per gestire correttamente i file di indice

La soluzione è raggruppare ogni file BAM con il suo indice in una tupla, poi aggiornare il processo downstream e il collegamento del flusso di lavoro per corrispondere.

#### 3.4.1. Cambiare l'output del modulo SAMTOOLS_INDEX in una tupla

Il modo più semplice per garantire che un file BAM e il suo indice rimangano strettamente associati è impacchettarli insieme in una tupla che esce dall'attività di indice.
...
Sapete come fare in modo che il vostro flusso di lavoro funzioni su più campioni (indipendentemente).
...
!!! note
    ...
    I file che stiamo usando qui sono solo sul filesystem locale del vostro GitHub Codespaces, ma potremmo anche puntare a file nello storage cloud.
...
Cambiate il parametro `reads_bam` per puntare al file `sample_bams.txt` invece di elencare i singoli campioni.
...
### 4.3. Aggiornare la fabbrica di canali per leggere le righe da un file

Attualmente, la nostra fabbrica di canali di input tratta qualsiasi file che le diamo come gli input di dati che vogliamo fornire al processo di indicizzazione.
Poiché ora le stiamo dando un file che elenca i percorsi dei file di input, dobbiamo cambiare il suo comportamento per analizzare il file e trattare i percorsi dei file che contiene come gli input di dati.

Possiamo farlo usando lo stesso pattern che abbiamo usato nella [Parte 2 di Hello Nextflow](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file): applicando l'operatore [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) per analizzare il file, poi un'operazione `map` per selezionare il primo campo di ogni riga.
...
Tuttavia, usando l'operatore più versatile `splitCsv` (integrato da `map`), possiamo rendere il nostro flusso di lavoro a prova di futuro nel caso decidessimo di aggiungere metadati al file contenente i percorsi dei file.

!!! tip

    Se non siete sicuri di aver capito cosa stanno facendo gli operatori qui, questa è un'altra grande opportunità per usare l'operatore `.view()` per vedere come appaiono i contenuti del canale prima e dopo averli applicati.

### 4.4. Eseguire il flusso di lavoro

Eseguite il flusso di lavoro un'ultima volta. Questo dovrebbe produrre lo stesso risultato di prima, giusto?
...
E questo è tutto! Il nostro semplice flusso di lavoro di variant calling ha tutte le funzionalità di base che volevamo.
