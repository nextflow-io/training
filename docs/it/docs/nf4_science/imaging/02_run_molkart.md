...
Nella Parte 1, abbiamo eseguito un semplice flusso di lavoro Hello World per comprendere le basi dell'esecuzione di Nextflow.
Ora eseguiremo una pipeline di bioimaging reale: **nf-core/molkart**.

Questa pipeline elabora dati di trascrittomica spaziale Molecular Cartography di Resolve Bioscience.
Tuttavia, i pattern di Nextflow che imparerete qui si applicano a qualsiasi pipeline nf-core o flusso di lavoro di produzione.

## 1. Comprendere le pipeline nf-core

Prima di eseguire la pipeline, capiamo cos'è nf-core e perché è importante per l'esecuzione dei flussi di lavoro.
...
- **Dati di test integrati**: Ogni pipeline include profili di test per una rapida validazione
- **Documentazione completa**: Istruzioni d'uso dettagliate e descrizioni dei parametri
- **Controllo qualità**: Report QC automatizzati utilizzando MultiQC
- **Supporto container**: Container pre-costruiti per la riproducibilità

!!! tip "Volete saperne di più su nf-core?"
...
3. **Assegnazione degli spot**: Assegna gli spot di trascritto alle cellule segmentate
4. **Controllo qualità**: Genera report QC completi
...
!!! note "Perché stiamo clonando localmente?"
    ...
    Nextflow scarica automaticamente la versione richiesta della pipeline per voi in `$HOME/.nextflow/assets/nf-core/molkart` e la esegue da lì.
...
Prima di eseguire la pipeline completa, impariamo perché i container sono essenziali per le pipeline nf-core.
...
L'errore `command not found` (exit status 127) significa che Nextflow ha provato a eseguire `duplicate_finder.py` ma non è riuscito a trovarlo sul vostro sistema.
Questo perché:

1. La pipeline si aspetta che sia installato software bioinformatico specializzato
2. Questi strumenti (come `duplicate_finder.py`, `apply_clahe.dask.py`, ecc.) non fanno parte delle distribuzioni Linux standard
3. Senza container, Nextflow prova a eseguire i comandi direttamente sulla vostra macchina locale

**Da dove dovrebbero provenire questi strumenti?**

Ispezioniamo uno dei moduli di processo per vedere come dichiara i suoi requisiti software.
...
Guardate la riga 5 - vedrete:

```groovy
container 'ghcr.io/schapirolabor/molkart-local:v0.0.4'
```

Questa riga dice a Nextflow: "Per eseguire questo processo, usa l'immagine Docker `ghcr.io/schapirolabor/molkart-local:v0.0.4`, che contiene tutto il software richiesto."

Ogni processo dichiara quale immagine container fornisce i suoi strumenti richiesti.
Tuttavia, Nextflow usa questi container solo se glielo dite!
...
3. Eseguirà ogni processo all'interno del suo container specificato
...
!!! Tip "Perché i container sono importanti"
    ...
    - Usano software bioinformatico specializzato non disponibile negli ambienti standard
...
Mentre la pipeline viene eseguita, vedrete un output simile a questo:
...
Notate come questo output sia più dettagliato del nostro esempio Hello World a causa delle convenzioni nf-core che la pipeline segue:

- La pipeline mostra la sua versione e il logo
- I parametri di configurazione vengono visualizzati
- Più processi vengono eseguiti in parallelo (indicato da più righe di processo)
- I nomi dei processi includono il percorso completo del modulo (es. `NFCORE_MOLKART:MOLKART:MINDAGAP_MINDAGAP`)

### 2.4. Comprendere l'esecuzione dei processi
...
Ogni riga di processo mostra:

- **Hash** (`[1a/2b3c4d]`): Identificatore della directory di lavoro (come prima)
- **Nome del processo**: Percorso completo del modulo e nome del processo
...
Quando la pipeline si completa con successo, vedrete un messaggio di completamento e un riepilogo dell'esecuzione.
...
Dovreste vedere diverse sottodirectory:
...
Ogni sottodirectory contiene output da una fase specifica della pipeline:
...
Aprite il report nel browser dei file e poi cliccate il pulsante "Show Preview" per vederlo renderizzato direttamente in VS Code.
...
!!! Tip
    ...
    Forniscono sempre una panoramica di alto livello dell'esecuzione della pipeline e della qualità dei dati.
...
Questa vi dice quanti di ciascun trascritto sono stati rilevati in ciascuna cellula.
...
Controllate la directory pipeline_info:
...
- **execution_timeline.html**: Diagramma di Gantt dell'esecuzione dei processi
- **execution_trace.txt**: Metriche dettagliate di esecuzione delle attività
- **pipeline_dag.html**: Grafo aciclico diretto che mostra la struttura del flusso di lavoro
...
- Quali attività sono state messe in cache vs. eseguite
...
Proprio come con il nostro esempio Hello World, tutto il lavoro effettivo avviene nella directory `work/`.

### 4.1. Comprendere la struttura della directory di lavoro

La directory di lavoro contiene una sottodirectory per ogni attività che è stata eseguita.
Per questa pipeline con 12 attività, ci saranno 12 sottodirectory di lavoro.
...
Scegliete uno degli hash del processo di segmentazione dall'output della console (es. `[3m/4n5o6p]`) e guardate all'interno:
...
La differenza chiave da Hello World:

- Le pipeline reali mettono in stage file di input di grandi dimensioni (immagini, dati di riferimento)
...
!!! Tip
    ...
    Se un processo fallisce, potete navigare alla sua directory di lavoro, esaminare `.command.err` per i messaggi di errore, e persino rieseguire `.command.sh` manualmente per fare debug del problema.

### 4.3. Pulizia della directory di lavoro

La directory di lavoro può diventare piuttosto grande su più esecuzioni della pipeline.
Come abbiamo imparato nella Parte 1, potete usare `nextflow clean` per rimuovere le directory di lavoro dalle vecchie esecuzioni.

Tuttavia, per le pipeline nf-core con file intermedi di grandi dimensioni, è particolarmente importante pulire regolarmente.

### Takeaway

Capite come le pipeline nf-core organizzano le loro directory di lavoro e come ispezionare singole attività per il debug.

### Cosa c'è dopo?

Imparate sulla cache di Nextflow e come riprendere esecuzioni di pipeline fallite.
...
2. Se input, codice e parametri sono identici, riutilizza il risultato in cache
...
Notate `cached: 2` o `cached: 1` per ogni processo - nulla è stato rieseguito!
...
- Dovete modificare processi a valle senza rieseguire i passaggi a monte
...
!!! Warning
    ...
    Resume funziona solo se non avete cambiato i dati di input, il codice della pipeline o i parametri.
    Se cambiate uno qualsiasi di questi, Nextflow rieseguirà correttamente le attività interessate.

### Takeaway

Sapete come usare `-resume` per rieseguire efficientemente le pipeline senza ripetere le attività riuscite.

### Cosa c'è dopo?

Ora che potete eseguire nf-core/molkart con dati di test, siete pronti a imparare come configurarla per i vostri dataset.
