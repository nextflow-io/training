...
Se non l'avete ancora fatto, vi preghiamo di farlo ora, idealmente in una seconda finestra o scheda del browser in modo da poter consultare queste istruzioni.
...
!!!warning "Requisito dimensione macchina"
    ...
    Assicuratevi di selezionare una **macchina a 8 core** quando create il vostro Codespace per questo corso di formazione. I workflow di bioimaging richiedono risorse di calcolo aggiuntive.
...
Tuttavia, è necessario un account GitHub (gratuito) per effettuare l'accesso e, se non avete familiarità con l'interfaccia, dovreste dedicare qualche minuto per familiarizzare con essa completando il mini-corso [GitHub Codespaces Orientation](../../envsetup/index.md).
...
!!!tip
    ...
    Il flag `-stub` consente al pipeline di essere eseguito rapidamente senza elaborare dati reali, il che è perfetto per scaricare le immagini. Potete monitorare il progresso nella scheda del terminale.
