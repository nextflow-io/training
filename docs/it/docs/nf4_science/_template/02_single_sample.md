...
Ora andremo a incorporare quegli stessi comandi in un flusso di lavoro Nextflow.
...
4. **Fare in modo che il flusso di lavoro accetti un foglio campioni contenente un batch di file di input.**
...
Prenderemo queste informazioni e le incorporeremo in Nextflow in tre fasi:
...
Aggiungete un blocco `profiles` a `nextflow.config` con un profilo `test` che imposta il parametro `{PRIMARY_PARAM_NAME}` su uno dei {PRIMARY_INPUT_TYPE} di test.
