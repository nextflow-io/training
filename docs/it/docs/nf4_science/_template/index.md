...
Abbiamo suddiviso il corso in tre parti, ciascuna focalizzata su aspetti specifici dell'applicazione di Nextflow a un caso d'uso in {DOMAIN}.
...
| [Parte 2: Elaborazione singolo campione](./02_single_sample.md) | Costruzione di una pipeline che {PART2_SUMMARY}, quindi scalabilità a più campioni                                        | 60 min         |
| [Parte 3: Aggregazione multi-campione](./03_multi_sample.md)   | Aggiunta di {AGGREGATION_SUMMARY} multi-campione utilizzando operatori di canale per aggregare gli output per campione | 45 min         |

Al termine di questo corso, sarete in grado di applicare concetti e strumenti fondamentali di Nextflow a un caso d'uso tipico in {DOMAIN}.
