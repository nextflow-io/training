...
Congratulazioni per aver completato il corso di formazione Nextflow per {DOMAIN}!

## Il vostro percorso

Avete iniziato eseguendo manualmente gli strumenti {METHOD} nel terminale per comprendere la metodologia.
Poi avete costruito una pipeline Nextflow per un singolo campione per automatizzare il processo, l'avete scalata per gestire più campioni in parallelo e avete aggiunto {AGGREGATION_SUMMARY} multi-campione.

### Cosa avete costruito

- Una pipeline {METHOD} che prende {PRIMARY_INPUT_TYPE} come input e produce {FINAL_OUTPUT_TYPE} come output.
- {N} processi ({PROCESS_LIST}) memorizzati in file modulo separati.
...
- Gestire appropriatamente file accessori come {ACCESSORY_FILES}
...
Ora siete pronti per iniziare ad applicare Nextflow ai flussi di lavoro di analisi {DOMAIN} nel vostro lavoro.
...
Prima di proseguire, dedicate un minuto per completare il sondaggio del corso! Il vostro feedback ci aiuta a migliorare i nostri materiali di formazione per tutti.
