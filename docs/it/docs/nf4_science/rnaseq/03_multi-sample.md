# Parte 3: Implementazione multi-campione paired-end
...
1. Fare in modo che il flusso di lavoro accetti più campioni di input e parallelizzi l'esecuzione
...
Dovremo modificare il modo in cui gestiamo l'input.

### 1.1. Cambiare l'input primario in un CSV di percorsi file invece di un singolo file

Forniamo un file CSV contenente gli ID dei campioni e i percorsi dei file FASTQ nella directory `data/`.
Questo file CSV include una riga di intestazione.
Notate che i percorsi dei file FASTQ sono percorsi assoluti.
...
Rinominiamo il parametro di input primario in `input_csv` e cambiamo il valore predefinito con il percorso del file `single-end.csv`.
...
### 1.2. Aggiornare la fabbrica di canali di input per gestire un CSV come input

Vogliamo caricare il contenuto del file nel canale invece del solo percorso del file, quindi usiamo l'operatore `.splitCsv()` per analizzare il formato CSV, poi l'operatore `.map()` per estrarre l'informazione specifica che vogliamo (il percorso del file FASTQ).
...
Questo è tutto ciò che è servito per far eseguire il flusso di lavoro su più file!
...
Tutto questo produce molti report QC, e non vogliamo dover scavare tra i singoli report.
...
Creiamo un file modulo chiamato `modules/multiqc.nf` per contenere il processo `MULTIQC`:

```bash
touch modules/multiqc.nf
```

Aprite il file nell'editor di codice e copiate il seguente codice al suo interno:
...
Aggiungete l'istruzione `include { MULTIQC } from './modules/multiqc.nf'` al file `rnaseq.nf`:
...
Dobbiamo dare al processo `MULTIQC` tutti gli output relativi al QC dei passaggi precedenti.

Per questo, useremo l'operatore `.mix()`, che aggrega più canali in uno solo.

Se avessimo quattro processi chiamati A, B, C e D con un semplice canale `.out` ciascuno, la sintassi sarebbe così: `A.out.mix( B.out, C.out, D.out )`. Come potete vedere, lo applicate al primo dei canali che volete combinare (non importa quale) e aggiungete semplicemente tutti gli altri, separati da virgole, nelle parentesi che seguono.
...
Ma poiché vogliamo aggregarli su tutti i campioni, dobbiamo aggiungere l'operatore `collect()` per raccogliere i report di tutti i campioni in una singola chiamata a `MULTIQC`.
...
Nel contesto del blocco completo del flusso di lavoro, finisce per apparire così:
...
Quell'ultimo file `all_single-end.html` è il report aggregato completo, comodamente impacchettato in un unico file HTML facile da consultare.

---

## 3. Abilitare l'elaborazione di dati RNAseq paired-end

Al momento il nostro flusso di lavoro può gestire solo dati RNAseq single-end.
...
### 3.1. Fare una copia del flusso di lavoro chiamata `rnaseq_pe.nf`

```bash
cp rnaseq.nf rnaseq_pe.nf
```

### 3.2. Modificare il valore predefinito di `input_csv` per puntare ai dati paired-end

Forniamo un secondo file CSV contenente gli ID dei campioni e i percorsi dei file FASTQ paired nella directory `data/`
...
Cambiamo il valore predefinito di `input_csv` con il percorso del file `paired-end.csv`.
...
### 3.3. Aggiornare la fabbrica di canali

Dobbiamo dire all'operatore `.map()` di estrarre ora entrambi i percorsi dei file FASTQ.
...
Facciamo una copia del modulo in modo da avere entrambe le versioni a portata di mano.

```bash
cp modules/fastqc.nf modules/fastqc_pe.nf
```

Aprite il nuovo file modulo `fastqc_pe.nf` nell'editor di codice e apportate le seguenti modifiche al codice:

- Cambiate `fastqc $reads` in `fastqc ${reads}` nel blocco `script` (riga 17) in modo che l'input `reads` venga spacchettato, poiché ora è una tupla di due percorsi invece di un singolo percorso.
- Sostituite `${reads.simpleName}` con un carattere jolly (`*`) per evitare di dover gestire i file di output individualmente.
...
Infine, aggiornate l'istruzione di importazione del modulo per usare la versione paired-end del modulo.
...
Fate una copia del modulo in modo da avere entrambe le versioni a portata di mano.

```bash
cp modules/trim_galore.nf modules/trim_galore_pe.nf
```

Aprite il nuovo file modulo `trim_galore_pe.nf` nell'editor di codice e apportate le seguenti modifiche al codice:

- Cambiate la dichiarazione di input da `path reads` a `tuple path(read1), path(read2)`
- Aggiornate il comando nel blocco `script`, sostituendo `$reads` con `--paired ${read1} ${read2}`
- Aggiornate le dichiarazioni di output per riflettere i file aggiunti e le diverse convenzioni di denominazione, usando caratteri jolly per evitare di dover elencare tutto.
...
Sostituite `TRIM_GALORE.out.fastqc_reports,` con `TRIM_GALORE.out.fastqc_reports_1,` più `TRIM_GALORE.out.fastqc_reports_2,`:
...
Aprite il nuovo file modulo `hisat2_align_pe.nf` nell'editor di codice e apportate le seguenti modifiche al codice:

- Cambiate la dichiarazione di input da `path reads` a `tuple path(read1), path(read2)`
- Aggiornate il comando nel blocco `script`, sostituendo `-U $reads` con `-1 ${read1} -2 ${read2}`
- Sostituite tutte le istanze di `${reads.simpleName}` con `${read1.simpleName}` nel comando nel blocco `script` così come nelle dichiarazioni di output.
...
Non usiamo `-resume` poiché questo non userebbe la cache, e c'è il doppio dei dati da elaborare rispetto a prima, ma dovrebbe comunque completarsi in meno di un minuto.
...
E questo è tutto! Ora abbiamo due versioni leggermente divergenti del nostro flusso di lavoro, una per dati single-end e una per dati paired-end.
Il prossimo passo logico sarebbe far accettare al flusso di lavoro entrambi i tipi di dati al volo, il che è fuori dall'ambito di questo corso, ma potremmo affrontarlo in un seguito.

---

### Takeaway

Sapete come adattare un flusso di lavoro a campione singolo per parallelizzare l'elaborazione di più campioni, generare un report QC completo e adattare il flusso di lavoro per usare dati paired-end se necessario.

### Cosa c'è dopo?

Congratulazioni, avete completato il mini-corso Nextflow For RNAseq! Celebrate il vostro successo e prendetevi una pausa ben meritata!

Successivamente, vi chiediamo di completare un brevissimo sondaggio sulla vostra esperienza con questo corso di formazione, poi vi porteremo a una pagina con link a ulteriori risorse di formazione e link utili.
