...
Esistono diversi metodi validi per processare e analizzare dati bulk RNAseq.
Per questo corso, seguiamo il metodo descritto [qui](https://www.bioinformatics.babraham.ac.uk/training/RNASeq_Course/Analysing%20RNA-Seq%20data%20Exercise.pdf) dai Dott. Simon Andrews e Laura Biggins del [Babraham Institute](https://www.babraham.ac.uk/).

Il nostro obiettivo è sviluppare un flusso di lavoro che implementi i seguenti passaggi di elaborazione: eseguire il controllo qualità iniziale sulle read in un campione bulk RNAseq, rimuovere le sequenze adattatrici dalle read, allineare le read a un genoma di riferimento e produrre un report completo di controllo qualità (QC).

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/rnaseq/img/preprocess.svg"
</figure>

- **FASTQC:** Eseguire il QC sui dati delle read prima del trimming usando FastQC
- **TRIM_GALORE:** Rimuovere le sequenze adattatrici ed eseguire il QC dopo il trimming usando Trim Galore (che include Cutadapt e FastQC)
- **HISAT2_ALIGN:** Allineare le read al genoma di riferimento usando Hisat2
- **MULTIQC:** Generare un report QC completo usando MultiQC

Tuttavia, prima di addentrarci nella scrittura del codice del flusso di lavoro, proveremo i comandi manualmente su alcuni dati di test.
Gli strumenti di cui abbiamo bisogno non sono installati nell'ambiente GitHub Codespaces, quindi li useremo tramite container (vedi [Hello Containers](../../hello_nextflow/05_hello_containers.md)).

!!! note "Nota"

     Assicuratevi di essere nella directory `nf4-science/rnaseq`. L'ultima parte del percorso mostrato quando digitate `pwd` dovrebbe essere `rnaseq`.

---

## 1. QC iniziale e rimozione degli adattatori

Scaricheremo un'immagine container che ha sia `fastqc` che `trim_galore` installati, la avvieremo in modo interattivo ed eseguiremo i comandi di trimming e QC su uno dei file di dati di esempio.

### 1.1. Scaricare il container

```bash
docker pull community.wave.seqera.io/library/trim-galore:0.6.10--1bf8ca4e1967cd18
```

Questo produce il seguente output nella console mentre il sistema scarica l'immagine:
...
Il vostro prompt cambierà in qualcosa come `(base) root@b645838b3314:/tmp#`, che indica che siete ora all'interno del container.

La parte `-v ./data:/data` del comando ci permetterà di accedere ai contenuti della directory `data/` dall'interno del container.
...
Eseguiamo `fastqc` per raccogliere metriche di controllo qualità sui dati delle read.
...
### 1.4. Rimuovere le sequenze adattatrici con `trim_galore`

Ora eseguiamo `trim_galore`, che include Cutadapt e FastQC, per rimuovere le sequenze adattatrici e raccogliere metriche QC post-trimming.

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

Il flag `--fastqc` fa sì che il comando esegua automaticamente un passaggio di raccolta QC dopo il completamento del trimming.

_L'output è molto dettagliato quindi quello che segue è abbreviato._
...
### 1.5. Spostare i file di output nel filesystem fuori dal container
...
Scaricheremo un'immagine container che ha `hisat2` installato, la avvieremo in modo interattivo ed eseguiremo il comando di allineamento per allineare i dati RNAseq a un genoma di riferimento.

### 2.1. Scaricare il container `hisat2`
...
Il comando è lo stesso di prima, con l'URI del container rilevante sostituito.

### 2.3. Creare i file di indice del genoma per Hisat2

Hisat2 richiede che il riferimento del genoma sia fornito in un formato molto specifico e non può semplicemente utilizzare il file FASTA `genome.fa` che forniamo, quindi cogliamo questa opportunità per creare le risorse rilevanti.

```bash
hisat2-build /data/genome.fa genome_index
```

L'output è molto dettagliato quindi quello che segue è abbreviato:
...
Questo crea diversi file di indice del genoma, che potete trovare nella directory di lavoro.
...
Li useremo tra un momento, ma prima generiamo un tarball compresso con questi file di indice del genoma; ne avremo bisogno più tardi e generarli non è tipicamente qualcosa che vogliamo fare come parte di un flusso di lavoro.

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

Questo memorizza un tarball `genome_index.tar.gz` contenente i file di indice del genoma nella directory `data/` sul nostro filesystem, che tornerà utile nella Parte 2 di questo corso.

### 2.4. Eseguire il comando `hisat2`

Ora possiamo eseguire il comando di allineamento, che esegue il passaggio di allineamento con `hisat2` e poi invia l'output a `samtools` per scrivere l'output come file BAM.

L'input dei dati delle read è il file `/data/trimmed/ENCSR000COQ1_1_trimmed.fq.gz` che abbiamo generato con `trim_galore` nel passaggio precedente.
...
Su scala reale questo potrebbe richiedere molto più tempo.
...
### 2.5. Spostare i file di output nel filesystem fuori dal container
...
Scaricheremo un'immagine container che ha `multiqc` installato, la avvieremo in modo interattivo ed eseguiremo un comando di generazione report sui file di report FastQC prima/dopo.

### 3.1. Scaricare il container `multiqc`
...
MultiQC è in grado di cercare nelle directory i report QC compatibili e aggregherà tutto ciò che trova.

Qui vediamo che lo strumento ha trovato tutti e tre i report QC che abbiamo generato: il QC iniziale che abbiamo fatto con `fastqc`, il report post-trimming da `cutadapt` (fatto tramite `trim_galore`) e il QC post-allineamento prodotto da `hisat2`.
...
### 3.4. Spostare i file di output nel filesystem fuori dal container
...
Avete testato tutti i singoli comandi in modo interattivo nei container rilevanti.

### Cosa c'è dopo?

Imparate come racchiudere quegli stessi comandi in un flusso di lavoro multi-step che usa container per eseguire il lavoro.
