...
Questa è una convenzione comune nell'ecosistema Nextflow (vedere [Hello Config](../../hello_nextflow/06_hello_config.md) per maggiori dettagli).
...
Usiamo la sintassi `emit:` per assegnare nomi a ciascun canale di output, che sarà utile per collegare gli output nel blocco publish.
