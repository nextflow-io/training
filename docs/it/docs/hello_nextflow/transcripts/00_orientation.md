...
Questo corso è un'introduzione pratica alla creazione di flussi di lavoro con Nextflow. È progettato per persone completamente nuove a Nextflow che vogliono sviluppare le proprie pipeline.
...
Una volta acquisite le basi qui, abbiamo anche corsi di approfondimento che applicano questi concetti ad analisi scientifiche reali. Vi insegneremo come utilizzare le pipeline e le best practice della comunità nf-core.
...
Questa è la homepage come appare oggi mentre registro. Potete vedere che c'è una panoramica delle diverse cose, il background e i diversi corsi che abbiamo, e l'elenco è in continua crescita.
...
Un paio di altri pulsanti se vi piace avere la modalità chiara. Invece di quella modalità, potete seguire il sito web in modalità chiara qui in alto.

E poi anche tutto ciò che guardiamo è in un singolo repository GitHub, che è open source, chiamato nextflow-io/training. E se cliccate questo pulsante in qualsiasi momento, andrà al repository GitHub. Torneremo su questo tra un minuto.

## Configurazione di GitHub Codespaces

Okay, quindi ora avete questo aperto nella scheda del browser. Andiamo su Hello Nextflow e clicchiamo. Potete vedere nella pagina introduttiva, ci dice alcuni dei requisiti, la panoramica e il piano della lezione di cosa affronteremo approssimativamente, e poi ci addentreremo nell'iniziare.

Ci sono diversi modi per fare questo tutorial interattivo. Se vi fa piacere, siete i benvenuti a farlo localmente sul vostro computer con la vostra installazione di Nextflow. Se clicchiamo su Environment Options, potete vedere che ci sono più dettagli su come farlo usando Devcontainer locali o potete anche semplicemente installare tutto il software localmente, con installazione manuale.

Stiamo lavorando per far funzionare bene questo con Seqera Studios, quindi questa è un'altra opzione. Ma la più comune al momento è usare GitHub Codespaces.
...
Ecco come appare. Possiamo vedere che abbiamo impostato tutte le opzioni qui per voi. Se volete, potete cliccare change options. Alcune cose che potete fare qui. Potete dare un'istanza macchina più grande, per esempio, se scoprite che si blocca perché finisce la memoria o qualcosa del genere. O impostare versioni specifiche del materiale di formazione. Ma di solito potete semplicemente andare con quello che abbiamo configurato qui e potete vederlo. In questo caso sta usando la release 3.0.
...
Velocemente mentre aspettiamo che carichi, andrò anche su github.com/codespaces e mostrerò solo questa è la pagina panoramica dove potete vedere tutti i diversi Codespace che avete attualmente in esecuzione.
...
Infine, un altro modo per entrare in questo. Se andiamo al repository GitHub. E questo funziona per qualsiasi repository GitHub. Cliccate code. Potete avere comandi per clonare il repository localmente. E c'è una scheda chiamata Codespaces. E di nuovo, potete crearne uno nuovo, e potete vedere quelli che sono già in esecuzione.
...
Altre cose sono, c'è una finestra principale di modifica del codice qui in alto, che ha appena caricato un'anteprima del readme, che è per il materiale di formazione. Quindi in questo caso sta visualizzando markdown, ma normalmente questo sarà un editor di codice.
...
E ora potete vedere che ho qualcosa in più sul lato sinistro. Sono un po' ingrandito qui, ma se riduco lo zoom potete vedere che uno dei pulsanti dice Nextflow con l'icona Nextflow. E ha alcune cose carine qui dentro per esplorare il progetto e cose del genere, a cui torneremo più tardi.
...
Userò questi parecchio perché sono molto ingrandito, quindi cercherò di aiutarvi a vedere tutto il testo sul mio schermo, ed è utile poter andare a schermo intero con il terminale e poi nasconderlo quando guardiamo il codice. Ma la maggior parte del tempo potete semplicemente avere tutte queste cose aperte allo stesso tempo.
...
Ci sono altre scorciatoie da tastiera che userò, giusto nel caso siate curiosi. Per esempio, proprio allora ho premuto cmd+K sul mio Mac e questo ha pulito il terminale, per sbarazzarsi di tutto l'output precedente. Quindi è bello per mantenere le cose pulite. Se mi vedete farlo è così che lo faccio.

E anche se siete nuovi al terminale, ricordate che potete usare tab per completare automaticamente, cosa che farò molto per completare automaticamente i percorsi.
...
Okay. Questo è fantastico. C'è un sacco di roba in questo repository però.
...
Potete impostare questo come vostro spazio di lavoro con un po' di clic qui intorno e impostare una directory di progetto e roba del genere. Ma il modo più semplice è digitare code, che è il comando CLI per lanciare VS Code, e poi "hello-nextflow".
...
Due cose di cui avete bisogno per questo. Avete bisogno di una versione aggiornata di Nextflow e dovete assicurarvi che sia abilitata.

Se faccio "nextflow -version" di nuovo, vedrete che Codespaces sta eseguendo con 25.10.2 e 25.10 è la versione minima per poter usare questa roba.

Se state usando 26.04, che per me non è ancora uscita, ma lo farà presto. Allora questo eseguirà il nuovo parser di sintassi per impostazione predefinita, e non dovete fare nient'altro.

Ma se state eseguendo 25.10, dovete abilitare lo strict syntax parser, come viene chiamato, o v2 syntax parser.
...
Se avete qualche dubbio su qualsiasi cosa riguardo alla versione Nextflow e al syntax parser, innanzitutto, ricordate, non dovete preoccuparvi se siete in Codespaces. Tutto dovrebbe essere configurato correttamente. Ma in secondo luogo, se andate sul materiale di formazione Nextflow, se scendete, parlate dei requisiti di versione, c'è un link qui che vi porta alla pagina di aiuto sulle versioni di esplorazione, e questo spiega tutto in dettaglio.
...
Okay, quindi ora si spera che tutto sia in esecuzione. Il vostro schermo sembra uguale al mio e capite come accedere a tutto e quali sono tutti i diversi file.

Se scorrete fino in fondo alla pagina su getting started, piccola casella di controllo dovreste dire che capisco cosa sto facendo. Il mio ambiente è attivo e funzionante e siete impostati, la vostra directory di lavoro correttamente sulla cartella "Hello Nextflow".
