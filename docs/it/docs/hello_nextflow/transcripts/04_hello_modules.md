...
Ora, come ho detto, non scriveremo o modificheremo molto codice. Sposteremo semplicemente il codice che abbiamo già. I file dei moduli Nextflow tipicamente hanno un singolo processo al loro interno, e per convenzione normalmente li conserviamo in una directory chiamata modules. Ma potete chiamarla come volete. Io però manterrò una directory modules nel mio repository qui, e poi creerò un file per ogni processo. Quindi dirò new file, sayHello.nf.
...
Quindi scrivo include e alcune parentesi graffe, e poi prendo il nome del processo. E dico from, e poi fornisco un percorso relativo del file. Quindi dice, inizia con ./ perché è relativo da dove questo script è salvato. Quindi è modules sayHello.nf.
...
Possiamo eseguire di nuovo il flusso di lavoro Nextflow, funzionerà esattamente nello stesso modo. E questo è fondamentalmente il resto di questo capitolo del corso, semplicemente spostare questi tre processi nei loro file.
...
Un sacco di taglia, copia e incolla.
...
E potete vedere come il progetto ora inizia a costruirsi con i nostri diversi file. Possiamo immergerci nei dettagli nei punti che vogliamo. Navigare per trovare passaggi specifici nella pipeline molto più facilmente, e ottenere una panoramica di ciò che la pipeline sta facendo rapidamente.

## Navigare i moduli con VS Code

Ora, ovviamente, lo svantaggio di fare questo è che se avete una grande pipeline, avrete molti file di moduli e potrebbero essere organizzati in più sottodirectory o ogni genere di cose. Ora, ancora, un piccolo suggerimento qui. L'estensione VS Code è piuttosto brava a navigare la vostra base di codice per voi e anche a dirvi informazioni sul codice lì.

Potete vedere che VS Code capisce cos'è questo processo e mi dà una piccola panoramica quando passo sopra con il mouse, così posso vedere senza dover andare a trovare il codice sorgente, quali sono gli input e gli output, che è tipicamente la cosa più importante quando lo uso in un flusso di lavoro.
...
Okay, controlliamo solo che la pipeline funzioni ancora come ci aspettiamo. Quindi apro il terminale. Facciamo "nextflow run hello modules", e vediamo se si esegue senza problemi.
...
Bene, questo è tutto per i moduli. Vi avevo detto che era una sezione breve del corso. Date un'occhiata al quiz, assicuratevi di aver capito e assicuratevi che tutto funzioni ancora correttamente. E ci vediamo nel prossimo video, che è tutto sui container software. Grazie mille.
