...
Nextflow è particolarmente bravo in due cose: riproducibilità e portabilità. I file di configurazione sono dove vediamo brillare davvero la seconda di queste caratteristiche. La capacità di configurare una pipeline Nextflow per funzionare in modi diversi e su sistemi diversi, senza dover modificare il codice della pipeline sottostante.

Questo superpotere permette alle pipeline Nextflow di essere riutilizzate da altre persone in luoghi diversi, o attraverso diverse infrastrutture a cui potreste avere accesso voi stessi.
...
In questa sezione, affronteremo alcuni argomenti. Inizieremo con come Nextflow gestisce i file di configurazione, da dove li carica, come si scrivono e come si strutturano, e quella separazione tra la pipeline stessa e cosa dovrebbe andare in un file di configurazione.

Poi passeremo ad alcuni casi d'uso comuni come cambiare dove vengono memorizzati i file di output, e anche come far funzionare la pipeline su diverse infrastrutture, sia usando diversi tipi di packaging software sia inviando job a diverse infrastrutture.
...
Potete fornire file di configurazione in tutti questi luoghi se volete, e saranno caricati iterativamente, ciascuno sovrascrivendo il precedente solo negli scope di configurazione dove si scontrano.
...
Okay, chiudiamo questo e saltiamo nei nostri Codespaces e iniziamo. Come prima ho fatto pulizia qui, ho rimosso le mie precedenti directory results, le mie directory Nextflow e work e così via. Non preoccupatevi se avete ancora quei file in giro. È solo perché sono molto ingrandito e quindi le cose diventano disordinate molto rapidamente altrimenti.
...
Abbiamo anche già un file "nextflow.config" dal capitolo precedente, dove abilitiamo Docker, e oggi costruiremo su questo file.
...
Tuttavia, lascio i nomi dei parametri e i loro tipi, così Nextflow conosce quelle informazioni e può ancora fare tutta la sicurezza dei tipi e tutto il resto.
...
È un po' lungo da digitare. Potete anche chiamarlo semplicemente "-o" se volete. Quindi se torno indietro. Posso semplicemente abbreviarlo in "-o", che è un po' più semplice.

Okay. Eseguiamo quello. Non abbiamo cambiato nulla nella nostra pipeline o anche nella nostra configurazione a questo punto, e dovrebbe si spera salvare tutti i nostri risultati in una directory di livello superiore diversa. E potete immaginare di poterlo impostare praticamente su qualsiasi percorso vogliate.

È appena arrivato in alto. Abbiamo un custom-outdir-cli, e tutti i file sono organizzati lì esattamente nello stesso modo, con le stesse sottodirectory e nomi di file. Quindi questo è un modo davvero facile per cambiare dove la pipeline pubblica i suoi risultati, senza pensare troppo a come quei risultati sono organizzati.
...
Quindi quello che possiamo fare è prendere quel percorso dinamico fuori dalla configurazione outputDir, che verrebbe cancellato, e metterlo nel percorso di output, che non viene cancellato.
...
Posso farlo molto semplicemente. Se vado a code, "nextflow.config". Se ricordate in alto, abbiamo definito docker.enabled in precedenza, e nell'ultimo capitolo così potevamo usare il container con cowpy dentro.
...
Oppure è buona pratica fare quello che dice il sito web del materiale di formazione, che è definire uno specifico canale conda con la sua notazione a doppio due punti, e sicuramente definire una versione specifica del software così ogni persona che esegue la pipeline otterrà la stessa versione.

Notate che i container sono un po' superiori in questo senso, perché quando installate qualcosa con Conda, sta ancora per capire tutte le dipendenze per quel pacchetto, e possono cambiare nel tempo. Chiamato dependency drift.
...
Okay, ottimo, è stato eseguito. Possiamo solo ricordarci cosa la pipeline è attualmente configurata per usare. Se guardiamo nel file di configurazione, era "custom-outdir-config" in questo momento per me. Vediamo se vado su a quella directory base. E ho fatto --batch conda. C'è la nostra sottodirectory conda. Quindi ha funzionato e c'è il nostro output cowpy.
...
E questo è davvero più interessante per le persone che sono abituate a eseguire su SLURM e conoscono come appaiono gli header SLURM. Ma se faccio Nextflow run, hello config. Fallirà perché proverà a inviare job a un cluster che non esiste. Quindi otterremo qualche tipo di errore su sbatch che non è disponibile.
...
Cos'altro è diverso tra diverse infrastrutture di calcolo? Un'altra cosa è quante risorse disponibili avete, e infatti, in molti ambienti di calcolo, è un requisito che dovete specificare quante CPU e quanta memoria un job necessita.

Di nuovo, Nextflow astrae questo per noi, così non è più specifico per un singolo tipo di ambiente di calcolo, e possiamo digitare nello scope a livello di processo qui. CPUs equals one, memory equals two gigabytes. La nostra pipeline non è molto esigente, quindi dovrebbe andare bene.

Ora, ho solo indovinato questi numeri qui, ma come fate a sapere qual è una quantità sensata di risorse da usare? È un lavoro piuttosto difficile andare a scavare attraverso tutti questi diversi processi di una grande pipeline di molti campioni e capire quale fosse l'utilizzo delle risorse.

Quindi un buon approccio per questo è impostare questi valori a numeri alti per iniziare, solo così la vostra pipeline viene eseguita senza errori, e poi chiedere a Nextflow di generare un report di utilizzo per voi.

Questo è super facile da fare, quindi tornerò a un terminale. Oh, devo ricordarmi di reimpostare quello su local così la mia pipeline viene effettivamente eseguita. E dirò Nextflow run, e userò un flag della riga di comando -with-report.

E posso lasciarlo vuoto e darà un nome file predefinito, ma gli darò un nome file specifico così viene salvato in un posto specifico.
...
Ora posso eseguire la pipeline con l'opzione CLI -profile, dire quale profilo voglio usare. Quindi userò my_laptop, e Nextflow applicherà tutta la configurazione all'interno di quello scope del profilo. Quindi posso provarlo ora. È lo stesso comando di prima. Nextflow run hello config, e faccio dash profile, singolo dash perché è l'opzione core di Nextflow, dash profile my_laptop.
...
E questo è fantastico perché non devo necessariamente andare a specificare tutte queste cose, che potrebbero essere parametri richiesti. Altrimenti posso solo dire dash profile test e verrà eseguito subito.

Ora qualcosa da notare è che i profili possono anche essere combinati più di uno. Quindi posso fare profile my_laptop qui, e poi aggiungere anche test. Non faccio profile due volte. Faccio solo una lista separata da virgole qui senza spazi. E applicherà questi profili in ordine. Quindi prenderà la configurazione dal profilo my_laptop, e poi applicherà la configurazione test sopra.
...
Si spera che vi abbia convinto che la risoluzione della configurazione di Nextflow è potente, ma non vi biasimerei se state diventando un po' strabici a questo punto dopo che ho detto circa 20 modi diversi per fornire configurazione e dare tutti questi diversi strati come una buccia di cipolla.

Quindi se mai vi sentite insicuri su quale sia la configurazione risolta finale per Nextflow, sappiate che c'è un comando chiamato "nextflow config", e possiamo eseguirlo e ci dirà quale è la configurazione risolta nella nostra posizione attuale.
