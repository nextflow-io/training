...
Quindi, si spera che ora siate configurati in Codespaces o in un ambiente equivalente con VS Code in esecuzione, e abbiate la cartella Hello Nextflow nello spazio di lavoro nell'Explorer con tutti questi diversi file qui.
...
Quindi iniziamo davvero in modo semplice. Iniziamo solo con "echo", per stampare qualcosa in un terminale. "Hello World". Premo invio e questo va al terminale. Hello World. Spero che questo non sia una sorpresa per nessuno che guarda questo corso.

Ok, facciamo qualcosa con questo. Invece di stamparlo solo sul terminale, scriviamolo in un file. Premerò il tasto freccia su della tastiera, che scorre attraverso la cronologia di Bash, quindi mi dà il mio ultimo comando, e aggiungerò alla fine di esso, un piccolo simbolo maggiore di, che reindirizza l'output da questo comando a un file, e lo chiamerò output.txt.
...
Come ho detto, tutti i diversi capitoli in questo corso iniziano con uno script e questo si chiama Hello World. Quindi troverò Hello World. Lo visualizza in anteprima se ci faccio un singolo clic, farò doppio clic per aprirlo nell'editor qui. E rimuoverò rapidamente il terminale.

Ora questo è uno script molto semplice, quindi il più semplice possibile. È lungo solo 22 righe e fa sostanzialmente la stessa cosa. In effetti, parte di questo dovrebbe sembrare familiare. È quello che abbiamo appena digitato. Possiamo vedere il nostro comando bash che reindirizza a un file lì.

Ok. Cos'altro? Inoltre, in questo file, possiamo iniziare a vedere alcuni dei concetti fondamentali di Nextflow. Abbiamo un **process** in rosso qui e un **workflow**. Queste sono le parole chiave speciali e la terminologia speciale in Nextflow.
...
Il nome del processo segue questa parola chiave qui, e questo può essere praticamente qualsiasi cosa. E poi i contenuti del processo sono all'interno di queste parentesi graffe.
...
Ok. E poi in basso abbiamo un workflow qui, e di nuovo, abbiamo una dichiarazione. Questo si chiama Main. Questo è l'equivalente del workflow di un blocco script, se volete. È la parte del workflow che fa qualcosa. E in questo caso, stiamo dicendo, chiama il processo chiamato sayHello.
...
Posso anche cliccare su preview DAG qui in VS Code. Il DAG è una rappresentazione di una struttura di flusso di dati nella pipeline, e possiamo vederlo renderizzato sul lato come un diagramma mermaid. In questo caso è molto semplice. C'è una scatola, che è il workflow e un processo, che si chiama sayHello, ma potrebbe sembrare più interessante man mano che andiamo avanti.

## 1.2. Eseguiamo il workflow

Ok, proviamo a eseguire questo workflow e vediamo cosa succede.
...
Se il vostro assomiglia a questo con un piccolo segno di spunta verde, allora ben fatto. Avete appena eseguito la vostra prima pipeline. Fantastico.
...
## 1.2.2. Troviamo l'output e i log nella directory di lavoro

Ora quando eseguite una pipeline Nextflow, ognuno di quei processi viene cucito insieme, e ogni processo, come ho detto prima, può generare attività una o più. Quindi in questo caso, avevamo una singola attività da questo processo. È stato eseguito solo una volta e questo è stato fatto sotto questo _hash_ dell'attività.
...
Quando elenco quei file, potete vedere che il file output.txt è stato generato. Potete vederlo anche qui. E c'è un gruppo di file nascosti, che non vengono mostrati con il mio "ls" normale.

Se clicco su output.txt, sicuramente, abbiamo il nostro output. Fantastico. Quindi la pipeline ha funzionato.

Potrebbe sembrare un sacco di boilerplate per eseguire quello che era essenzialmente uno script bash di una riga, ma avrà più senso man mano che i nostri processi diventano più complicati. E questa directory work con Nextflow e questi file, che vengono creati è davvero la spina dorsale di ciò che rende Nextflow così potente.
...
Diamo una rapida occhiata a questi altri file nella directory work. Dall'alto verso il basso, abbiamo un file chiamato _.command.begin_. Questo è vuoto. È solo quello che viene chiamato un file sentinella, creato da Nextflow dicendo, ok, sto iniziando l'attività. Niente di interessante lì.
...
Ok, quelli erano tutti vuoti anche per questo, quindi non molto interessante, ma le cose diventano più interessanti quando arrivate a _.command.run_.
...
Generalmente, non avete davvero bisogno di guardare mai in questo file, però. È autogenerato da Nextflow e non c'è niente di veramente particolarmente unico per la vostra pipeline, che è in esso. Ma questo è davvero il nucleo di ciò che sta eseguendo.
...
Questo è davvero utile, che è quello che di solito finite per guardare di più quando state cercando di fare debug di qualcosa e controllare che la logica della vostra pipeline Nextflow stia facendo quello che vi aspettate che faccia.

Infine, abbiamo un file chiamato _.exitcode_, e questo cattura solo il codice di uscita da un'attività, che in questo caso ha avuto successo. Quindi il codice di uscita era zero.
...
## 1.3. Eseguiamo di nuovo il workflow

Un'altra cosa da capire sulle directory work è che se continuo a eseguire questa pipeline ripetutamente, quindi se faccio _"nextflow run hello-world.nf"_, farà esattamente la stessa cosa, ma questa volta avrà un nuovo id attività. Potete vedere che questo hash qui è diverso, e ora se guardo in work, ci sono due directory hash. E queste sono, di nuovo, separate l'una dall'altra.

Quindi ogni volta che eseguite un workflow Nextflow, a meno che non usiate il resume, che usa la cache, toccheremo più tardi, rieseguirà quei processi in nuove directory work, che sono separate l'una dall'altra. Non avrete collisioni di nomi di file, non avrete problemi del genere. Tutto è isolato e pulito.
...
Ma non è super utile se siete una persona che cerca di esplorare i vostri risultati. Non volete davvero scavare attraverso migliaia e migliaia di diverse directory work cercando di trovare i vostri file di risultato. E non siete davvero destinati a farlo. Le directory work non sono destinate ad essere lo stato finale di dove vengono creati i vostri file.
...
Ho fatto accidentalmente un errore di battitura lì, ma questa è una buona opportunità anche per sottolineare alcune delle caratteristiche dell'estensione Nextflow VS Code. Potete vedere che subito mi ha dato una piccola linea ondulata rossa sotto questo dicendo che qualcosa non va. E se ci passo sopra, mi dirà che questa variabile non è definita. Non so cosa sia.
...
Dove diventa importante è quando facciamo il nostro nuovo blocco qui, e questo è sotto il blocco workflow ora, non siamo più all'interno del workflow. Parentesi graffe di nuovo. E questo è dove diciamo solo a Nextflow dove mettere tutti i file, che vengono creati dal workflow.

Ora prenderò questo nome di variabile, che ho creato qui, e lo metterò lì e metterò alcune parentesi graffe per questo. E dirò a Nextflow di usare un path. Ops. Path, tra virgolette. E userò il punto. Questo dice solo a Nextflow di mettere il file nella radice della directory results. Quindi nessuna sottodirectory o altro.

Proviamo a eseguire di nuovo il nostro workflow. Se faccio _"nextflow run hello-world.nf"_, allora si spera che dovrebbe sembrare praticamente esattamente lo stesso. Niente è davvero cambiato con Nextflow qui. Sta eseguendo le stesse cose. Le sta solo facendo di nuovo nelle directory work.

Ma ora se faccio _"ls results/"_, vedrete che c'è una nuova directory qui che è stata creata chiamata results, che è la directory base predefinita per la pubblicazione del workflow. E lì c'è un file chiamato _output.txt_.

Se faccio _"ls -l results"_, vedrete che questo è in realtà soft linkato alla directory work. Quindi questo non è un file reale, è collegato alla directory work e ha raccolto tutti i file lì per noi.

## 2.2. Impostiamo una posizione personalizzata

"Results" è il nome predefinito per questo path. Se eseguo di nuovo il workflow, e questa volta faccio _dash_ singolo trattino, questo è, perché è un'opzione core di Nextflow. _"-output-dir **my**results"._ Potrei anche fare solo _"-o"_ per abbreviare. Quindi imposterà una directory base diversa per dove vengono archiviati i file e ancora una volta, qui in _myresults/_, ora abbiamo un _output.txt_.
...
Ok, ho menzionato che questi file sono soft link, quindi se faccio _"ls -l results/hello_world/"_, potete vedere che sta facendo soft linking alla directory work. Questo è generalmente una buona cosa se state lavorando su qualcosa come HPC, e questi sono file davvero enormi e non volete duplicarli, perché significa che i file sono archiviati solo una volta sul file system.

Tuttavia, significa che se cancellate la directory work: se faccio _"rm -r work"_ e cancello tutti quei file intermedi che sono stati creati. Ora, se provo a leggere questo file _"results/hello_world/"_. Starà puntando come un soft link a un file che non esiste più e i dati sono andati persi per sempre e sono irrecuperabili, il che forse non è fantastico.

Quindi generalmente, dico che è buona pratica copiare i file invece di fare soft linking se potete, perché è più sicuro. Siate solo consapevoli che userà il doppio dello spazio su disco a meno che non cancelliate quelle directory work.

Per farlo con il blocco output, andrò al primo output qui. Ho impostato il path prima e ora imposterò la modalità e potete vedere mentre digito, l'estensione VS code sta suggerendo cose che sa che è una direttiva di output qui. E dirò copy. Premo salva.

Rieseguiamo il workflow. Creerà di nuovo i file, nuova directory work.

Ora, se vado a _"ls -l results/hello_world/"_ potete vedere che questo è un file reale e non è più un soft link, e Nextflow lo ha copiato. Buono a sapersi. Quindi path e mode sono cose che vi troverete a scrivere abbastanza spesso.
...
Tuttavia, poiché siamo in questa fase di transizione in questo momento, potreste benissimo vedere pipeline in circolazione, che usate qualcosa chiamato publishDir, che è il vecchio modo di farlo, e questo è definito non a livello di workflow e output, ma questo è definito a livello di processo.
...
Ma sarebbe più interessante se non fosse tutto hardcoded. Quindi poi, pensiamo a come dire a Nextflow che questo processo può prendere un input variabile, che è qualcosa che possiamo controllare a runtime quando lanciamo un workflow.

Dobbiamo fare alcune cose diverse per far sì che questo accada.

Prima di tutto, dobbiamo dire a questo processo che può accettare una variabile di input e digitiamo _input_ qui come un nuovo blocco di dichiarazione. E lo chiameremo _"val greeting"_.
...
Lo farò rapidamente senza parentesi graffe solo per mostrarvi che questo è consentito, e questo è il vecchio modo di farlo. Ma ora con la nuova sintassi, raccomandiamo davvero di metterlo all'interno di parentesi graffe come questa, e rende davvero chiaro che questo viene interpolato da Nextflow qui.

Fantastico. Quindi _"input greeting"_ va in _$\{greeting\}._ L'ultima cosa è che dobbiamo dire a Nextflow a livello di workflow che questo processo ora prende un input. E per farlo, gli daremo sostanzialmente una variabile.
...
Quindi abbiamo passato dinamicamente un input da una CLI a un parametro. Abbiamo passato quello come un input al processo e il processo lo ha interpretato e lo ha messo in un blocco script, che ha poi cambiato dinamicamente l'output di quel risultato dello script. Abbastanza bello.

Logica abbastanza complessa con pochissima sintassi qui. E potete si spera vedere come questo ora inizia a scalare. E questo è come costruiamo davvero la logica e la personalizzabilità delle nostre pipeline nello script Nextflow.

## 3.4. Usiamo valori predefiniti per i parametri da riga di comando

Ok, questo è fantastico. Il problema però ora è che ogni singola volta che eseguo questa pipeline, devo fare dash, input perché venga eseguita.

Se provo a eseguire senza questo parametro, ora Nextflow lancerà un errore dicendo che aveva bisogno di questo parametro e non era impostato. e quindi non sapeva cosa fare.

Questa è una cosa nuova interessante, tra l'altro. In passato, Nextflow sarebbe stato eseguito solo con una stringa vuota, e avreste avuto tutti i tipi di errori strani, che sarebbero stati difficili da capire. Ma nel nuovo parser di sintassi Nextflow, è un po' più attento e ve lo dice subito.
...
Noterete che quando abbiamo scritto questo, abbiamo messo _params.input_ direttamente dove lo stiamo usando. Quindi la soluzione ovvia è che definiamo un valore predefinito, e lo facciamo in alto nello script qui in un blocco params speciale nel workflow. Questo è nello script workflow qui.
...
Giusto salverò quello script, tornerò indietro, eseguirò di nuovo lo script senza _--input_. E questa volta dovrebbe essere eseguito e creerà il nostro nuovo file in _results_. E in questo file ora dice _"Holà mundo!"_.

Questo è solo un valore predefinito però, quindi non significa che non possiamo ancora fare la stessa cosa di prima. Se torno indietro e trovo il mio vecchio script qui, _"Hej Världen"_, perché faccio _--input_ sulla riga di comando, questo sovrascriverà quel valore predefinito e userà quello di nuovo nel file output.txt.

Quindi questo nello script è solo il valore predefinito che sto impostando.

Man mano che costruiamo il nostro workflow per essere più complesso e includere più parametri, questo blocco params in alto nello script inizierà a raccoglierli tutti in un unico posto.

E finite con questa simmetria abbastanza carina nel vostro script, dove avete effettivamente tutti i vostri input del workflow qui e i vostri output del workflow in basso. Ed è molto chiaro quale sia l'interfaccia del vostro workflow verso il mondo esterno. Quindi potete prendere una nuova pipeline molto rapidamente con la nuova sintassi e capire come usarla.

Un'ultima cosa interessante. Non dobbiamo impostare un valore predefinito con questo. Se facciamo params input ma non impostiamo un valore predefinito, allora dice a Nextflow che questo parametro è richiesto, e di nuovo, la pipeline fallirà nell'esecuzione senza di esso, ma vi darà un messaggio di errore più utile piuttosto che qualcosa sul fatto che sia null.
...
Ok, quindi si spera che ora sia chiaro come impostare la vostra pipeline Nextflow con input variabili e parametri, come impostare il valore predefinito, impostare i tipi, potrebbe essere un flag booleano vero falso o un intero o diversi tipi qui. Come passarli nel vostro workflow, dove va attraverso, e poi interpola nel vostro processo. E poi sapete anche come personalizzarli sulla riga di comando quando lanciate Nextflow. Questo sta iniziando a sembrare più interessante del nostro semplice comando bash.

## 4. Gestiamo le esecuzioni del workflow

Ok. Cosa c'è dopo? Per la parte finale di questo capitolo, parleremo un po' di come gestire tutte le diverse esecuzioni del workflow. Se guardate nella mia barra laterale qui e nell'Explorer sotto work, vedrete che ho eseguito un gruppo di diverse pipeline e queste directory work stanno diventando abbastanza lunghe, ce ne sono molte.

E l'altra cosa è, come ho detto prima, ogni volta che rieseguo questa pipeline, sta creando un nuovo set di directory work, e sta rieseguendo tutti i processi da zero, il che è una buona cosa. Questo è il comportamento previsto. È riproducibile e sta rigenerando tutto fresco. Ma ovviamente, se state eseguendo processi molto lunghi, è fastidioso dover sempre iniziare la vostra pipeline dall'inizio se si è bloccata a metà, o se cambiate qualcosa alla fine della pipeline.

## 4.1. Rilanciamo un workflow con -resume

Fortunatamente, Nextflow è davvero bravo a sapere cosa è stato precedentemente eseguito e cosa è disponibile, e riutilizzare quei vecchi risultati è molto semplice. Aggiungiamo solo un nuovo flag alla fine del comando _"-resume"_.
...
Ok, quindi ora faccio _-resume_ e eseguo esattamente lo stesso workflow di nuovo. E questa volta dovrebbe sembrare praticamente esattamente lo stesso con una differenza chiave.

Nell'output qui, potete vedere che i risultati erano in cache. E infatti, questo hash dell'attività qui è esattamente lo stesso dell'esecuzione precedente, e ha solo riutilizzato quella directory work nella sua interezza. Gli input e gli output e lo script erano tutti non modificati. E quindi prende solo quel file da quello e se ci sono passaggi a valle nel processo, li passerebbe al passaggio successivo nella pipeline.

Quindi sta ancora eseguendo l'intera pipeline dall'inizio alla fine, ma sta usando risultati in cache per ognuna di quelle attività, dove può.

Ora, quando fate _-resume_, riprende solo l'ultima esecuzione della pipeline nella vostra directory di lavoro, qualunque essa fosse. Ma potete effettivamente riprendere da qualsiasi esecuzione precedente che avete fatto lì. E ne abbiamo fatte parecchie ora.

## 4.2. Ispezioniamo il log delle esecuzioni passate

Per guardarle tutte, possiamo fare _"nextflow log"_ invece di _"nextflow run"_, e questo ci darà un bell'output che mostra tutte queste diverse.. Ho bisogno di rendere il mio schermo un po' più piccolo così possiamo vederlo, tutte queste diverse esecuzioni quando le abbiamo fatte, l'id della sessione, il comando e tutto.

E possiamo guardare qui e possiamo prendere il nome dell'esecuzione di una qualsiasi di queste e poi riprendere una di quelle specifiche. Quindi posso tornare indietro e posso riprendere quella chiamata _hungry_ekeblad_. E la metto solo dopo il _resume_.

Se siete curiosi, tra l'altro, tutti questi aggettivi e nomi di scienziati sono nel codice sorgente di Nextflow. È un modo davvero buono per ottenere la vostra prima pull request a Nextflow andando e trovandolo e aggiungendo il vostro scienziato preferito.

E comunque, quindi ho fatto quello e è tornato indietro e ha guardato i risultati in cache da questa esecuzione del workflow, ha realizzato che poteva ancora riutilizzarli, e lo ha fatto. Quindi ho ottenuto di nuovo i risultati in cache.

## 4.3. Cancelliamo le vecchie directory work

È fantastico. E se voglio pulire queste directory work? Ce ne sono carichi qui. Ci sono carichi di file. Forse so per certo che voglio riprendere dalle ultime due esecuzioni della pipeline, ma non mi interessano tutte quelle prima.

Allora posso sceglierne una qui e posso usare un altro comando Nextflow, che è _"nextflow clean"_, e posso fare _"nextflow clean"_, farò _"-before"_, e il particolare nome dell'esecuzione, che in questo caso era _reverent_pike_ e farò _"-n"_, che dice a Nextflow solo di fare un dry run. Quindi mi dice solo cosa cancellerà. Senza effettivamente fare nulla, quindi rimuoverebbe queste directory work.

Sembra sensato. Quindi farò lo stesso comando di nuovo, ma invece di _"-n"_ farò _"-f"_ per effettivamente fare la pulizia. E questa volta ha effettivamente rimosso tutte queste directory. E se entro e guardo le directory work, ora sembra molto più leggera. Fantastico.

Quindi questo è come pulire tutte le vostre directory work locali in un modo abbastanza sicuro senza distruggere completamente la cache. Quindi potete ancora riprendere se volete.
...
Ok, questa è la fine della parte uno di Hello Nextflow. È un inizio abbastanza intenso del corso, ma si spera che ora abbiate una comprensione abbastanza buona di come appare uno script Nextflow; con diverse parti chiave, i processi, i workflow, gli output e i parametri. Sapete come configurarli con override di base dalla riga di comando, come creare un blocco di input dinamico con uno script dinamico e sapete come gestire tutte le vostre esecuzioni del carico di lavoro: vedere cosa avete già eseguito, riprendere, pulire. C'è un sacco di roba. Avete fatto molta strada. Quindi se volete fare una pausa e fare una breve passeggiata e una tazza di tè, ora è probabilmente un buon momento. Ve la siete meritata.

Da qui in poi, stiamo sostanzialmente costruendo su questa base. Come possiamo rendere questo più complesso, più potente? Come possiamo renderlo più flessibile? Fare le cose che vogliamo fare la nostra analisi su scala.

## Quiz

Ora se scorrete verso il basso fino alla parte uno, hello world, sulla pagina web vedrete un piccolo quiz e questa è una cosa nuova che abbiamo fatto per questa versione della formazione Nextflow. E potete passare attraverso e mettervi alla prova per controllare che abbiate capito tutto il materiale che abbiamo fatto in questo capitolo.

Questo non viene inviato a noi o altro, è solo archiviato nel vostro browser. Quindi non sappiamo quali sono le vostre risposte, ma è solo un piccolo auto-controllo per assicurarvi di non aver perso nulla o frainteso nulla. E potete provarlo quante volte volete.
