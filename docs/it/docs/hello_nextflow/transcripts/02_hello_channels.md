...
C'è un'altra parte dei canali che sono gli operatori. Questi sono fondamentalmente funzioni che possiamo usare sui canali per modificarne i contenuti. Addentriamoci in VS Code e vediamo a che punto siamo.

Sono molto ingrandito su questo VS Code, quindi per mantenere le cose pulite e ordinate, ho rimosso tutti i file _.nextflow\*_ e la directory _work/_ e _results/_ e tutto dal Capitolo Uno. E sto semplicemente ricominciando da capo qui. Ma non preoccupatevi troppo di questo. Se non volete, potete lasciare quei file lì. Non causeranno problemi.

Inizieremo a lavorare su _hello-channels.nf_ per questo capitolo, e se apro questo, dovrebbe sembrare molto simile al file su cui stavamo lavorando in precedenza. Potrebbe essere che parti diverse siano in punti diversi dello script, ma tutto dovrebbe essere fondamentalmente lo stesso.
...
I canali sono un concetto un po' unico per molte persone che si avvicinano a Nextflow. Deriva da questi concetti di programmazione funzionale, e può richiedere un po' di tempo per capirlo, ma una volta che fa click, sbloccano davvero il potere di Nextflow ed è la chiave di come scrivete i vostri flussi di lavoro.

## 1.1. Creare un canale di input

Iniziamo prendendo questo script e facendolo usare un _canale_ invece di solo un _parametro_.

Andiamo giù al flusso di lavoro, che è dove c'è tutta la nostra logica del flusso di lavoro per mettere insieme le cose. E andrò qui e creerò un nuovo canale.
...
Ci sono molte fabbriche di canali diverse. Se faccio solo "." qui, potete vedere che VS Code ne sta suggerendo un sacco, ma _".of"_ è il più semplice e prende solo un input qui.

Quindi posso fare delle parentesi e dirò _"Hello Channels!"_.

Ottimo. Ho un canale. Fantastico. Posso premere salva, potrei eseguirlo di nuovo, ma non succederà nulla di interessante. VS Code mi ha dato una linea di avviso arancione qui sotto e mi ha detto che questo è impostato: l'hai creato, ma non l'hai mai effettivamente usato per nulla. Questo canale non viene consumato.
...
Sicuramente, potete vedere che è stato stampato nel terminale qui. _"Hello Channels!"_.
...
Potreste pensare che abbiamo finito a questo punto. Ma in realtà c'è un piccolo tranello qui, che ci farà inciampare. Se guardiamo il nostro file di output qui. Potete vedere che ha _"Hello"_ dentro, ma non ha nessuno degli altri output. Infatti, è solo questo.
...
Ci sono alcune cose che possiamo fare per capire meglio questo. Possiamo guardare nella directory work stessa e potete vedere tutte le diverse directory work lì, ma è un po' confuso perché sarà mescolato con diverse esecuzioni di Nextflow.

Oppure possiamo dire a Nextflow di non usare i codici di escape ANSI.

Quindi se eseguo di nuovo il comando, ma questa volta dico _"-ansi-log false"_ per disattivarlo, potrei anche usare le variabili d'ambiente _$NO_COLOR_ o _"$NXF_ANSI_LOG=false"_. Quindi usa il tipo di stile di logging di Nextflow più vecchio stile senza nessuno di questi codici di escape. Stampa semplicemente direttamente su un terminale senza aggiornamenti intelligenti.
...
Quindi questo ha un po' più senso ora. Si spera che capiate che Nextflow stava facendo questo, stava solo essendo un po' intelligente con quello che vi mostrava nel terminale con quelle directory work.
...
Questa è una configurazione un po' banale che abbiamo fatto qui, ma sottolinea alcuni dei concetti chiave che dovete capire su come funziona la pubblicazione dei file, e alcune delle cose in cui potreste cadere come trappole. Quindi si spera che possiate evitarlo nei vostri flussi di lavoro.
...
Non abbiamo più un bel segno di spunta verde qui. Abbiamo una croce rossa, e se rendo questo un po' più largo così è più facile da leggere, Nextflow ci sta dicendo cosa è andato storto.
...
In questo caso, useremo qualcosa chiamato _flatten_. Che passiamo alla fine del canale qui. Quindi creiamo il canale e poi eseguiamo _flatten_. E di nuovo, se ci passiamo sopra, ci mostra la documentazione per questo comando direttamente in VS Code, il che è molto utile. Potete anche trovare tutti questi documenti sul sito web di Nextflow, la documentazione.
...
Qui, questo sta dicendo prendi il saluto, che è l'input dell'operatore view, e questo è qui. Potrei chiamarlo come volevo, potrei chiamarlo _"foo"_ e devo solo riferirmi ad esso come _"foo"_ dopo. E poi dico con questo, restituisci questo.

E poi imposto la restituzione di una stringa che dice prima del flatten per una variabile. molto semplice.
...
E poi questo canale greeting viene ancora creato, quindi verrà ancora passato al processo. E si spera che ora il flusso di lavoro venga eseguito. Proviamolo.

Ottimo. Quindi prima di tutto la pipeline non è andata in crash questa volta. Abbiamo avuto tre processi che sono stati eseguiti correttamente e abbiamo un piccolo segno di spunta. E poi possiamo vedere che le nostre istruzioni view hanno funzionato.
...
Molto rapidamente, vorrei solo menzionare che quando stavo creando fabbriche di canali prima, ho fatto _"."_, e poi abbiamo visto che c'erano molti modi diversi per creare canali, e uno di questi si chiama "_fromList"_. E questo è effettivamente specificamente progettato per fare questa stessa operazione. Quindi avremmo potuto semplicemente fare fromList greetings array, e funzionerà. È una sintassi leggermente più pulita e carina. Ma per gli scopi di questa dimostrazione, volevamo renderlo un po' più passo dopo passo così potevate vedere come il canale viene manipolato e come diversi operatori possono cambiare cosa c'è nel contenuto di un canale.
...
E poi possiamo impostare il default se vogliamo, di nuovo, su un Path. E se guardo in esplora a sinistra, potete vedere in questo repository, in questa directory di lavoro, ho una directory chiamata data. Ho un file lì chiamato _"greetings.csv"._
...
Quindi dobbiamo analizzare questo file CSV. Dobbiamo scompattarlo, arrivare ai contenuti del file CSV, e poi passare i contenuti all'interno del canale al processo.
...
Quindi questa volta eseguiremo, abbiamo analizzato il CSV in tre oggetti array, e poi l'abbiamo appiattito. E questa volta è passato. E la pipeline Nextflow è stata eseguita. Tuttavia potete vedere che _flatten_ va davvero a fondo e appiattisce tutto. E quindi otteniamo tre voci array indipendenti per ogni riga. E quindi ha eseguito il processo tre volte ogni riga di un CSV. E ora abbiamo un mucchio di file di risultati, e 123, 456, e tutti i tipi di cose, non solo quella prima colonna del CSV, che è quello che volevamo davvero.
...
Gli array in Nextflow sono basati su zero, quindi diremo solo il primo elemento, che è riga zero. Se volessimo la seconda colonna, potrei essere uno o la terza colonna essere due, e così via. Possiamo restituire quello che vogliamo qui, ma restituirò solo il primo valore.
...
I canali, come ho detto all'inizio di questo video, sono la colla di Nextflow. E potete vedere qui che possiamo prendere input diversi e manipolarli e prendere quei dati e poi passarli alla logica del flusso di lavoro a valle.
