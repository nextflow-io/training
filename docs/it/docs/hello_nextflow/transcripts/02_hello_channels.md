...
I canali sono un concetto un po' unico per molte persone che iniziano con Nextflow. Deriva da questi concetti di programmazione funzionale, e può richiedere un po' di tempo per capirlo, ma una volta che scatta, sbloccano davvero la potenza di Nextflow ed è fondamentale per come scrivete i vostri flussi di lavoro.

## 1.1. Creare un canale di input

Iniziamo prendendo questo script e facendolo usare un _canale_ invece di solo un _param_.

Andiamo giù al workflow, che è dove c'è tutta la nostra logica del flusso di lavoro per mettere insieme le cose. E andrò qui e creerò un nuovo canale.
...
Quindi in pipeline Nextflow più mature e reali, passerete spesso in giro un oggetto meta con tutti i metadati associati a un dato campione. Potete quindi creare nomi di file dinamici basati su quello, che è molto più pratico.
...
Qui, questo sta dicendo prendi il saluto, che è l'input dell'operatore view, e questo è qui. Potrei chiamarlo come voglio, potrei chiamarlo _"foo"_ e devo solo riferirmi ad esso come _"foo"_ dopo. E poi dico con questo, restituisci questo.
...
Molto rapidamente, vorrei solo menzionare che quando stavo creando fabbriche di canali prima, ho fatto _"."_, e poi abbiamo visto che c'erano molti modi diversi per creare canali, e uno di questi si chiama "_fromList"_. E questo è effettivamente specificamente progettato per fare questa stessa operazione. Quindi avremmo potuto semplicemente fare from list greetings away, e funzionerà. È una sintassi leggermente più pulita e carina. Ma per gli scopi di questa dimostrazione, volevamo renderlo un po' più passo dopo passo così potevate vedere come il canale viene manipolato e come diversi operatori possono cambiare cosa c'è nel contenuto di un canale.
...
Quindi sbarazziamoci del nostro greetings away. E ora cambieremo di nuovo questa fabbrica di canali. Ho appena detto che ce ne sono un mucchio tra cui scegliere e ce n'è una chiamata _".fromPath"_. E gli dirò di, in questo caso, prendere _params.input_, che sta tornando al nostro input che stavamo usando prima.
...
E poi possiamo impostare il default se vogliamo, di nuovo, su un Path. E se guardo in explore a sinistra, potete vedere in questo repository, in questa directory di lavoro, ho una directory chiamata data. Ho un file lì chiamato _"greetings.csv"._
...
Con il nostro logging dall'operatore view, possiamo vedere prima che _splitCsv_ fosse il path. E sicuramente, dopo _splitCsv_, abbiamo tre output diversi e ognuno di quegli output assomiglia molto a ciascuna delle righe del file _greetings.csv_, il che ha senso.
...
Abbiamo finito il nostro tipo di debugging e questo è il codice con cui finiamo. Prendendo il nostro parametro CLI chiamato _input_, che è classificato come un _Path_. Nextflow trova il path, lo carica e comprende il file CSV. Restituisce tutte le diverse righe. E poi mappiamo solo il primo elemento di quella riga nel canale che ci dà i contenuti del canale, che viene passato al processo.
...
Questo non è lo stesso della programmazione regolare in Python o altri linguaggi. Non stiamo usando istruzioni _if_ qui, questa è programmazione funzionale di flusso usando canali e operatori. Quindi è un po' diverso, ma è anche super potente.
