...
Metteremo a buon uso quel canale di output tra poco.
...
Per dimostrare, aggiungeremo un nuovo passaggio al nostro flusso di lavoro che raccoglie tutti i saluti maiuscoli prodotti dal processo `convertToUpper` e li scrive in un singolo file.
...
Quando aggiungeremo un altro passaggio al nostro flusso di lavoro nella Parte 5 (Containers), saremo in grado di riferirci facilmente a `collectGreetings.out.outfile` e passarlo al nuovo processo (spoiler: il nuovo processo si chiama `cowpy`).
...
Per approfondire: [3. Passare più di un input a un processo](#3-passare-parametri-aggiuntivi-a-un-processo)
</quiz>
