# Parte 3: Creare un modulo nf-core

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } Traduzione assistita da IA - [scopri di più e suggerisci miglioramenti](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>


In questa terza parte del corso di formazione Build with nf-core, vi mostreremo come creare un modulo nf-core applicando le convenzioni chiave che rendono i moduli portabili e manutenibili.

Il progetto nf-core fornisce un comando (`nf-core modules create`) che genera automaticamente template di moduli correttamente strutturati, simile a quello che abbiamo utilizzato per il workflow nella Parte 1.
...
??? info "Come iniziare da questa sezione"
    ...
    Questa sezione presuppone che abbiate completato la [Parte 2: Utilizzare un modulo nf-core](./02_use_module.md) e abbiate integrato il modulo `FIND_CONCATENATE` nel vostro pipeline.

    Se non avete completato la Parte 2 o desiderate iniziare da zero per questa parte, potete utilizzare la soluzione `core-hello-part2` come punto di partenza.
    Eseguite questi comandi dall'interno della directory `nfcore-build/`:
...
// Genera arte ASCII con cowpy (https://github.com/jeffbuttars/cowpy)
process cowpy {
...
Come potete vedere, abbiamo modificato sia l'**input principale** che l'**output** in una tupla che segue il pattern `tuple val(meta), path(input_file)` introdotto nella Parte 2 di questa formazione.
...
"Ehi," potreste dire, "`FIND_CONCATENATE` sta inviando i suoi output a `--outdir`. Forse dovremmo copiare la sua direttiva `publishDir`?"

Sì, è un'ottima idea.

Tranne che non ha una direttiva `publishDir`. (Andate pure a guardare il codice del modulo.)
...
Vediamo cosa succede se eseguiamo il pipeline ora.
...
Detto questo, potreste decidere di voler organizzare i vostri output in modo diverso, e la buona notizia è che è facile farlo.
...
### 1.6. Acquisizione delle versioni con i topic channel

La riproducibilità è un principio fondamentale di nf-core: ogni esecuzione del pipeline registra esattamente quali versioni software sono state utilizzate, in modo che i risultati possano essere riprodotti e confrontati tra esecuzioni diverse.
Il report `hello_software_versions.yml` in `pipeline_info/` è il modo in cui i pipeline nf-core soddisfano questo requisito.

Il meccanismo alla base è quello dei **topic channel**: una funzionalità di broadcast di Nextflow in cui qualsiasi processo può pubblicare dati su un topic nominato, e qualsiasi sottoscrittore riceve tutto ciò che è stato pubblicato su quel topic — indipendentemente da dove proviene nel pipeline.
nf-core sta introducendo questa funzionalità in tutti i pipeline nel 2026.
I moduli pubblicano la loro versione come tupla direttamente nel blocco output usando la parola chiave `topic:`, e il workflow si sottoscrive una volta sola per raccoglierle tutte.

#### 1.6.1. Aggiungere il reporting della versione a `COWPY`

Aprite il file del modulo `cowpy.nf` e aggiungete una riga di output per la versione come mostrato di seguito.
...
La nuova riga pubblica una tupla `[process_name, tool_name, version]` sul canale topic `"versions"`.
Non sono necessarie modifiche al blocco script — la versione è dichiarata staticamente nel blocco output.

#### 1.6.2. Eseguire il pipeline e ispezionare il report delle versioni
...
La raccolta lato workflow — il blocco `channel.topic("versions")` che avete visto nel workflow segnaposto nella Parte 1 — si sottoscrive al topic e scrive automaticamente questo report combinato.

!!! info "Compatibilità con le versioni precedenti"

    Il ramo `versions_file` nel blocco del topic channel del workflow esiste per gestire i moduli che non sono ancora stati aggiornati per usare `topic: versions` e scrivono ancora un file `versions.yml` nel blocco script con `emit: versions`.
    Entrambi gli stili sono supportati simultaneamente durante la transizione.
...
- Capire come i topic channel raccolgono automaticamente le versioni software da tutti i moduli.

Per ulteriori approfondimenti sulle convenzioni dei moduli, consultate le [specifiche dei moduli nf-core](https://nf-co.re/docs/guidelines/components/modules).
...
!!! tip "Suggerimento"
    ...
    Il file di test generato utilizza nf-test, un framework di testing per pipeline e moduli Nextflow. Per imparare come scrivere ed eseguire questi test, consultate la [side quest nf-test](../side_quests/nf_test/index.md).
...
- **Output `topic: versions`**: Reporting della versione tramite topic channel, come trattato nella sezione 1.6
...
- **Blocco `script:`**: Vuoto — aggiungeremo qui il comando `cowpy`
- **Blocco `stub:`**: Lo aggiorneremo per produrre gli output corretti
...
Per l'ambiente Conda, il codice del modulo specifica `#!groovy conda "${moduleDir}/environment.yml"`, il che significa che deve essere configurato nel file `environment.yml`.
...
=== "Prima"
    ...
    channels:
      ...
      - bioconda
    dependencies:
      # TODO nf-core: Elenca i pacchetti Conda richiesti.
      #               Il software DEVE essere fissato al canale (es. "bioconda"), versione (es. "1.10").
      #               Per Conda, la build (es. "h9402c20_2") deve essere ESCLUSA per supportare l'installazione su diversi sistemi operativi.
      - "YOUR-TOOL-HERE"
    ```
...
- Una stringa di versione statica (`#!groovy val("1.1.5")`) al posto del `#!groovy eval("cowpy --version")` del template, in linea con il modulo manuale della sezione 1.6 (lo strumento `cowpy` non espone un flag `--version`)

Se state usando il language server di Nextflow per validare la sintassi, la parte `#!groovy ${prefix}` sarà segnalata come errore in questa fase perché non l'abbiamo ancora aggiunta al blocco script.
Passiamo a quello adesso.

#### 2.3.2. Il blocco script

Il template genera un blocco script vuoto che è necessario riempire con il comando effettivo dello strumento.
...
- Le **tuple di metadati** propagano i metadati attraverso il workflow
...
Quando siete pronti, continuate con la [Parte 4: Validazione dell'input](./04_input_validation.md) per imparare come aggiungere la validazione dell'input basata su schema alla vostra pipeline.
