...
Poiché i corsi ora utilizzano input tipizzati a livello di workflow e direttive di output a livello di workflow, richiedono l'uso del parser di sintassi V2.
Se prevedete di utilizzare l'ambiente che forniamo tramite [Github Codespaces](../envsetup/01_setup.md) o [devcontainer locali](../envsetup/03_devcontainer.md), non dovete fare nulla a meno che non sia specificatamente indicato nelle istruzioni del corso.
...
Potete accedere alle versioni precedenti dei materiali di formazione che funzionano con versioni di Nextflow **precedenti alla 25.10.2** tramite il menu a tendina in alto su ogni pagina che mostra la versione numerata dei materiali di formazione.
...
DSL1 era la sintassi originale in cui i processi erano collegati implicitamente tramite canali.
...
**Parser di sintassi v1 vs v2** si riferisce a parser diversi che entrambi funzionano con codice DSL2.
...
Il parser v2 fornisce anche messaggi di errore migliori e rileva più errori al momento del parsing piuttosto che a runtime.
...
- [Migrazione agli output di workflow](https://www.nextflow.io/docs/latest/tutorials/workflow-outputs.html)
