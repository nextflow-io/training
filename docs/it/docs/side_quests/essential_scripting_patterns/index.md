...
Nextflow è un linguaggio di programmazione che gira sulla Java Virtual Machine. Sebbene Nextflow sia costruito su [Groovy](http://groovy-lang.org/) e ne condivida gran parte della sintassi, Nextflow è molto più di un semplice "Groovy con estensioni" -- è un linguaggio autonomo con una [sintassi](https://nextflow.io/docs/latest/reference/syntax.html) e una [libreria standard](https://nextflow.io/docs/latest/reference/stdlib.html) completamente specificate.
...
Questa side quest vi porta in un percorso pratico dai concetti di base ai pattern pronti per la produzione.
Trasformeremo un semplice flusso di lavoro di lettura CSV in una sofisticata pipeline bioinformatica, facendola evolvere passo dopo passo attraverso sfide realistiche:
...
Questo tutorial spiegherà i concetti di programmazione man mano che li incontriamo, quindi non è necessaria una vasta esperienza di programmazione.
Inizieremo con i concetti fondamentali e arriveremo ai pattern avanzati.
...
!!! tip "Suggerimento: Riepilogo delle Operazioni sulle Map"
...
Gli [operatori di dataflow](https://www.nextflow.io/docs/latest/reference/operator.html) di Nextflow accettano closure valutate a runtime, consentendo alla logica condizionale di guidare le decisioni del flusso di lavoro in base al contenuto del canale.
...
In questo caso tutti e tre i campioni soddisfano il filtro, quindi ogni campione continua lungo la pipeline.
Una soglia più restrittiva escluderebbe i campioni a bassa profondità e ridurrebbe il numero di attività eseguite.
...
!!! note "Nota: La Truthiness in Pratica"
...
!!! tip "Suggerimento: Combinare Navigazione Sicura ed Elvis"
...
Nel corso di questa side quest, avete costruito una pipeline completa di elaborazione dei campioni che si è evoluta dalla gestione di base dei metadati a un flusso di lavoro sofisticato e pronto per la produzione.
Ogni sezione ha costruito sulla precedente, dimostrando come i costrutti di programmazione trasformino semplici flussi di lavoro in potenti sistemi di elaborazione dei dati, con i seguenti vantaggi:
...
Questa progressione rispecchia l'evoluzione nel mondo reale delle pipelinebioinformatiche, dai prototipi di ricerca che gestiscono pochi campioni ai sistemi di produzione che elaborano migliaia di campioni in laboratori e istituzioni.
Ogni sfida che avete risolto e ogni pattern che avete imparato riflette problemi reali che gli sviluppatori affrontano quando scalano i flussi di lavoro Nextflow.
...
- [Riferimento al Linguaggio Nextflow](https://nextflow.io/docs/latest/reference/syntax.html)
