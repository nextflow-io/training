...
Ogni file viene fornito con le proprie informazioni aggiuntive: cosa è, da dove proviene e cosa lo rende speciale.
Queste informazioni extra sono ciò che chiamiamo metadata.

I metadata sono dati che descrivono altri dati.
I metadata tengono traccia di dettagli importanti sui file e sulle condizioni sperimentali, e aiutano ad adattare le analisi alle caratteristiche uniche di ciascun dataset.

Pensatelo come un catalogo di biblioteca: mentre i libri contengono il contenuto effettivo (dati grezzi), le schede del catalogo forniscono informazioni essenziali su ciascun libro—quando è stato pubblicato, chi lo ha scritto, dove trovarlo (metadata).
Nei pipeline Nextflow, i metadata possono essere utilizzati per:

- Tracciare informazioni specifiche dei file durante tutto il flusso di lavoro
- Configurare i processi in base alle caratteristiche dei file
- Raggruppare file correlati per analisi congiunte

### Obiettivi di apprendimento

In questa side quest, esploreremo come gestire i metadata nei flussi di lavoro.
...
- Leggere e analizzare i metadata dei file da file CSV
- Creare e manipolare meta map
- Aggiungere nuovi campi di metadata durante l'esecuzione del flusso di lavoro
- Utilizzare i metadata per personalizzare il comportamento dei processi
...
Troverete un file di flusso di lavoro principale e una directory `data` contenente un foglio dati e una manciata di file di dati.
...
Il foglio dati elenca i percorsi ai file di dati e alcuni metadata associati, organizzati in 3 colonne:
...
- `data`: percorsi a file `.txt` che contengono saluti in diverse lingue
...
3. **Personalizzi** l'elaborazione per ciascun file in base alla sua lingua e ai metadata
4. **Organizzi** gli output per gruppo linguistico

Questo rappresenta un tipico pattern di flusso di lavoro in cui i metadata specifici dei file guidano le decisioni di elaborazione; esattamente il tipo di problema che le meta map risolvono elegantemente.
...
## 1. Caricare i metadata da un foglio dati

Aprite il file di flusso di lavoro `main.nf` per esaminare lo stub del flusso di lavoro che vi forniamo come punto di partenza.
...
Potete vedere che abbiamo impostato una fabbrica di canali di base per caricare il foglio dati di esempio come file, ma questo non leggerà ancora il contenuto del file.
...
Poiché il nostro foglio dati è in formato CSV, questo è un lavoro per l'operatore [`splitCsv`](https://www.nextflow.io/docs/latest/reference/operator.html#splitcsv), che carica ogni riga nel file come elemento nel canale.
...
Notate che stiamo usando l'opzione `header: true` per dire a Nextflow di leggere la prima riga del file CSV come riga di intestazione.
...
Possiamo vedere che l'operatore ha costruito una map di coppie chiave-valore per ogni riga nel file CSV, con le intestazioni delle colonne come chiavi per i valori corrispondenti.

Ogni voce della map corrisponde a una colonna nel nostro foglio dati:
...
Per esempio, potremmo accedere all'ID del file con `id` o al percorso del file txt con `recording`.
...
??? info "(Opzionale) Maggiori informazioni sulle map"
    ...
    Ecco uno script eseguibile che mostra come potete definire una map e accedere ai suoi contenuti in pratica:
    ...
    Anche se non ha un blocco `workflow` appropriato, Nextflow può eseguirlo come se fosse un flusso di lavoro:
...
Diciamo che vogliamo accedere alla colonna `character` dal foglio dati e stamparla.
...
Successo! Abbiamo sfruttato la struttura map derivata dal nostro foglio dati per accedere ai valori dalle singole colonne per ogni riga.
...
### 1.3. Organizzare i metadata in una 'meta map'

Nello stato attuale del flusso di lavoro, i file di input (sotto la chiave `recording`) e i metadata associati (`id`, `character`) sono tutti sullo stesso piano, come se fossero tutti in un grande sacco.
...
Per evitare questo problema, dobbiamo trovare un modo per mantenere la struttura del canale coerente indipendentemente da quante colonne contiene il foglio dati.

Possiamo farlo raccogliendo tutti i metadata in un elemento all'interno della tupla, che chiameremo meta map, o più semplicemente 'meta map'.
...
Di conseguenza, aggiungere più colonne nel foglio dati renderà disponibili più metadata nella map `meta`, ma non cambierà la forma del canale.
Questo ci consente di scrivere processi che consumano il canale senza dover codificare gli elementi dei metadata nella specifica di input:
...
Questa è una convenzione ampiamente utilizzata per organizzare i metadata nei flussi di lavoro Nextflow.

### Takeaway

In questa sezione, avete imparato:

- **Perché i metadata sono importanti:** Mantenere i metadata con i vostri dati preserva informazioni importanti sui file durante tutto il flusso di lavoro.
...
- **Come creare una meta map:** Separare i metadata dai dati dei file usando la struttura tupla `[ [id:value, ...], file ]`

---

## 2. Manipolare i metadata

Ora che abbiamo caricato i nostri metadata, facciamo qualcosa con essi!

Useremo uno strumento chiamato [`langid`](https://github.com/saffsd/langid.py) per identificare la lingua contenuta nel file di registrazione di ciascuna creatura.
Lo strumento viene pre-addestrato su un set di lingue e, dato un frammento di testo, produrrà una previsione della lingua e un punteggio di probabilità associato, entrambi su `stdout`.

### 2.1. Importare il processo ed esaminare il codice

Vi forniamo un modulo di processo pre-scritto chiamato `IDENTIFY_LANGUAGE` che avvolge lo strumento `langid`, quindi dovete solo aggiungere un'istruzione include prima del blocco workflow.
...
La definizione di output è strutturata come una tupla con una struttura simile a quella dell'input, tranne che contiene anche `stdout` come terzo elemento.
Questo pattern `tuple val(meta), path(file), <output>` mantiene i metadata associati sia ai dati di input che agli output mentre fluisce attraverso il pipeline.

Notate che stiamo usando il qualificatore di output [`stdout`](https://www.nextflow.io/docs/latest/process.html#outputs) di Nextflow qui perché lo strumento stampa il suo output direttamente sulla console piuttosto che scrivere un file; e usiamo `sed` nella riga di comando per rimuovere il punteggio di probabilità, pulire la stringa rimuovendo i caratteri di nuova riga e restituire solo la previsione della lingua.
...
Possiamo ora eseguire il flusso di lavoro:
...
Eccellente! Ora abbiamo una previsione per quale lingua parla ciascun personaggio.
...
Questo si rivelerà utile nel prossimo passo.
...
!!! note
    ...
    Invece, dovete usare chiavi per associare correttamente i dati, e le meta map forniscono una struttura ideale per questo scopo.

    Esploriamo questo caso d'uso in dettaglio nella side quest [Splitting & Grouping](./splitting_and_grouping.md).

### 2.3. Arricchire i metadata con gli output dei processi

Dato che i risultati che abbiamo appena prodotto sono di per sé una forma di metadata sul contenuto dei file, sarebbe utile aggiungerli alla nostra meta map.
...
Quindi invece, creeremo una nuova meta map contenente il contenuto della meta map esistente più una nuova coppia chiave-valore `lang: lang_id` che contiene le nuove informazioni, usando l'operatore `+` (una caratteristica di Groovy).
...
Se non avete ancora familiarità con l'operatore `+`, o se questo sembra confuso, prendetevi qualche minuto per esaminare la spiegazione dettagliata qui sotto.
...
??? info "Creazione della nuova meta map usando l'operatore `+`"
    ...
    **Ma cosa succede se dovete aggiungere un campo che non fa già parte di una map?**
    ...
    Diciamo che ripartite da `map1`, ma la previsione della lingua non è nella sua map (non c'è `map2`).
    Invece, è contenuta in una variabile chiamata `lang_id`, e sapete che volete memorizzare il suo valore (`'fr'`) con la chiave `lang`.
    ...
    Qui, `[lang: new_info]` crea una nuova map senza nome al volo, e `map1 + ` unisce `map1` con la nuova map senza nome, producendo lo stesso contenuto di `new_map` di prima.

    Carino, vero?

    **Ora trasponiamo questo nel contesto di un'operazione Nextflow `channel.map()`.**
    ...
    L'output è una singola map senza nome con lo stesso contenuto di `new_map` nel nostro esempio sopra.
    ...
    Speriamo possiate vedere che se cambiamo `map1` in `meta`, è fondamentalmente tutto ciò di cui abbiamo bisogno per aggiungere la previsione della lingua alla nostra meta map nel nostro flusso di lavoro.
    ...
    Se avete difficoltà a capire perché il `file` sembra muoversi nell'operazione `map`, immaginate che invece di `[meta + [lang: lang_id], file]`, quella riga legga `[new_map, file]`.
    Questo dovrebbe rendere più chiaro che stiamo semplicemente lasciando il `file` nella sua posizione originale in seconda posizione nella tupla. Abbiamo solo preso il valore `new_info` e lo abbiamo incorporato nella map che è in prima posizione.
...
Sì, funziona!
Abbiamo riorganizzato ordinatamente l'output del processo da `meta, file, lang_id` in modo che `lang_id` sia ora una delle chiavi nella meta map, e le tuple del canale si adattino di nuovo al modello `meta, file`.

<!-- TODO (future) Should we also show how to remove a key using subMap?! Or note where to find that. -->

### 2.4. Assegnare un gruppo linguistico usando condizionali
...
Potrebbe essere utile avere quella classificazione prontamente disponibile da qualche parte più avanti nel pipeline, quindi aggiungiamo quell'informazione nella meta map.

E, buone notizie, questo è ancora un altro caso che si presta perfettamente all'uso dell'operatore `map`!

Nello specifico, definiremo una variabile chiamata `lang_group`, useremo una semplice logica condizionale per determinare quale valore assegnare a `lang_group` per ciascun pezzo di dati.
...
Potete vedere che è molto simile all'operazione di unione map al volo che abbiamo usato nel passo precedente.
...
- Altrimenti se `lang` è incluso in una lista contenente francese (`'fr'`), spagnolo (`'es'`) e italiano (`'it'`), cambiare `lang_group` in `romance`.
...
- Usiamo una struttura `if {} else if {}` per la logica condizionale, con test `.equals()` alternativi per le due lingue germaniche, e un test per l'esistenza in una lista per le tre lingue romanze.
...
- **Applicare i metadata di input ai canali di output**: Copiare i metadata in questo modo ci consente di associare i risultati successivamente in base al contenuto dei metadata.
- **Creare chiavi personalizzate**: Avete creato due nuove chiavi nella vostra meta map, unendole con `meta + [new_key:value]` nella meta map esistente. Una basata su un valore calcolato da un processo, e una basata su una condizione che avete impostato nell'operatore `map`.

Questi vi consentono di associare metadata nuovi ed esistenti con i file mentre progredite attraverso il vostro pipeline.
Anche se non state usando i metadata come parte di un processo, mantenere la meta map associata ai dati in questo modo rende facile tenere insieme tutte le informazioni rilevanti.

---

## 3. Usare le informazioni della meta map in un processo

Ora che sapete come creare e aggiornare la meta map, possiamo arrivare alla parte davvero divertente: usare effettivamente i metadata in un processo.

Più specificamente, aggiungeremo un secondo passo al nostro flusso di lavoro per disegnare ciascun animale come arte ASCII e fargli dire il testo registrato in un fumetto.
...
??? info "Cosa fa `cowpy`?"
    ...
    Opzionalmente, potete selezionare un personaggio (o 'cowacter') da usare invece della mucca predefinita.
...
In caso contrario, non preoccupatevi; copriremo tutto ciò che dovete sapere mentre procediamo.

### 3.1. Importare il processo ed esaminare il codice

Vi forniamo un modulo di processo pre-scritto chiamato `COWPY` che avvolge lo strumento `cowpy`, quindi dovete solo aggiungere un'istruzione include prima del blocco workflow.
...
Come potete vedere, questo processo è attualmente progettato per prendere un file di input (contenente il testo da visualizzare) e un valore che specifica il personaggio che dovrebbe essere disegnato in arte ASCII, solitamente fornito a livello di flusso di lavoro da un parametro da riga di comando.
...
Aveva senso, perché stavamo generando solo un'immagine per esecuzione del pipeline.

Tuttavia, in questo tutorial, vogliamo generare un'immagine appropriata per ciascun soggetto che stiamo elaborando, quindi usare un parametro da riga di comando sarebbe troppo limitante.

Buone notizie: abbiamo una colonna `character` nel nostro foglio dati e quindi, nella nostra meta map.
Usiamola per impostare il personaggio che il processo dovrebbe usare per ciascuna voce.
...
Per alimentare il contenuto di quel canale al processo successivo (e farlo in modo chiaro e facile da leggere) vogliamo dargli un proprio nome, `ch_languages`.

Possiamo farlo usando l'operatore [`set`](https://www.nextflow.io/docs/latest/reference/operator.html#set).

Nel flusso di lavoro principale, sostituite l'operatore `.view()` con `.set { ch_languages }`, e aggiungete una riga per testare che possiamo riferirci al canale per nome.
...
#### 3.2.2. Accedere al file e ai metadata del personaggio
...
Per scrivere la chiamata al processo `COWPY`, dobbiamo solo sapere come estrarre l'oggetto file corrispondente e i metadata da ciascun elemento nel canale.

Come spesso accade, il modo più semplice per farlo è usare un'operazione `map`.

Il nostro canale contiene tuple strutturate come `[meta, file]`, quindi possiamo accedere all'oggetto `file` direttamente, e possiamo accedere al valore `character` memorizzato all'interno della meta map riferendoci ad esso come `meta.character`.
...
Questo conferma che siamo in grado di accedere al file e al personaggio per ciascun elemento nel canale.
...
Vedete che semplicemente copiamo le due operazioni map (meno le istruzioni `.view()`) come input alla chiamata del processo.
...
Se guardate nella directory results, dovreste vedere i singoli file contenenti l'arte ASCII di ciascun saluto pronunciato dal personaggio corrispondente.

??? abstract "Directory e contenuto del file di esempio"
...
Questo mostra che siamo stati in grado di usare le informazioni nella meta map per parametrizzare il comando nel secondo passo del pipeline.

Tuttavia, come notato sopra, parte del codice coinvolto era un po' goffo, poiché abbiamo dovuto spacchettare i metadata mentre eravamo ancora nel contesto del corpo del flusso di lavoro.
Quell'approccio funziona bene per usare un piccolo numero di campi dalla meta map, ma non si adatterebbe bene se volessimo usarne molti di più.

C'è un altro operatore chiamato `multiMap()` che ci consente di semplificare questo un po', ma anche in quel caso non è ideale.

??? info "(Opzionale) Versione alternativa con `multiMap()`"

    Nel caso vi stiate chiedendo, non potevamo semplicemente scrivere una singola operazione `map()` che produce sia il `file` che il `character`, perché questo li restituirebbe come una tupla.
    ...
    Tecnicamente c'è un altro modo per farlo attraverso una singola operazione di mappatura, usando l'operatore `multiMap()`, che è in grado di emettere più canali.
    Per esempio, potreste sostituire la chiamata a `COWPY` sopra con il seguente codice:
...
In entrambi i casi, è scomodo dover fare un po' di spacchettamento a livello di flusso di lavoro.
...
Il punto della meta map è dopo tutto passare tutti i metadata insieme come un pacchetto.
...
3. Aggiornare la chiamata del processo nel corpo del flusso di lavoro
...
Questo ci consente di usare la struttura tupla `[meta, file]` che abbiamo trattato in precedenza nel tutorial.

Notate che non abbiamo aggiornato la definizione di output del processo per produrre la meta map, al fine di mantenere il tutorial semplificato, ma sentitevi liberi di farlo voi stessi come esercizio seguendo il modello del processo `IDENTIFY_LANGUAGE`.
...
Ora aggiorniamo di conseguenza la chiamata del processo.

#### 3.3.3. Aggiornare la chiamata del processo ed eseguirla

Il processo ora si aspetta che il suo input usi la struttura tupla `[meta, file]`, che è ciò che il processo precedente produce, quindi possiamo semplicemente passare l'intero canale `ch_languages` al processo `COWPY`.
...
Questo semplifica significativamente la chiamata!

Eliminiamo i risultati dell'esecuzione precedente ed eseguiamola:
...
Se guardate nella directory results, dovreste vedere gli stessi output di prima, _cioè_ singoli file contenenti l'arte ASCII di ciascun saluto pronunciato dal personaggio corrispondente.
...
Quindi questo produce gli stessi risultati di prima con codice più semplice.
...
### 3.4. Risoluzione dei problemi con input richiesti mancanti

Il valore `character` è richiesto affinché il processo `COWPY` venga eseguito con successo.
...
Diciamo che eliminiamo il valore character per una delle voci nel nostro foglio dati per simulare un errore di raccolta dati:
...
Ora diciamo che eliminiamo completamente la colonna `character` dal nostro foglio dati:
...
##### 3.4.2.1. Valore accessibile a livello di flusso di lavoro
...
Non troverà alcun elemento che corrisponda all'istruzione, quindi non eseguirà affatto `COWPY`.
...
##### 3.4.2.2. Valore accessibile a livello di processo
...
Questo accade perché `meta.character` non esiste, quindi il nostro tentativo di accedervi restituisce `null`. Di conseguenza, Nextflow inserisce letteralmente `null` nella riga di comando, che ovviamente non è riconosciuto dallo strumento `cowpy`.
...
1. Implementare la validazione dell'input nel vostro flusso di lavoro per assicurarvi che il foglio dati contenga tutte le informazioni richieste. Potete trovare un'[introduzione alla validazione dell'input](../hello_nf-core/05_input_validation.md) nel corso di formazione Hello nf-core. <!-- TODO (future) pending a proper Validation side quest -->

2. Se volete assicurarvi che chiunque usi il vostro modulo di processo possa identificare immediatamente gli input richiesti, potete anche rendere la proprietà dei metadata richiesta un input esplicito.

Ecco un esempio di come funzionerebbe.

Prima, a livello di processo, aggiornate la definizione di input come segue:
...
Poi, a livello di flusso di lavoro, usate un'operazione di mappatura per estrarre la proprietà `character` dai metadata e renderla un componente esplicito della tupla di input:
...
Questo approccio ha il vantaggio di mostrare esplicitamente che `character` è richiesto, e rende il processo più facile da ridistribuire in altri contesti.

Questo evidenzia un importante principio di progettazione:

**Usate la meta map per informazioni opzionali e descrittive, ma estraete i valori richiesti come input espliciti.**

La meta map è eccellente per mantenere pulite le strutture dei canali e prevenire strutture di canali arbitrarie, ma per elementi obbligatori che sono direttamente referenziati in un processo, estrarli come input espliciti crea codice più robusto e manutenibile.

### Takeaway

In questa sezione, avete imparato come utilizzare i metadata per personalizzare l'esecuzione di un processo, accedendovi sia a livello di flusso di lavoro che a livello di processo.

---

## Esercizio supplementare

Se volete esercitarvi nell'uso delle informazioni della meta map dall'interno di un processo, provate a usare altri pezzi di informazioni dalla meta map come `lang` e `lang_group` per personalizzare come gli output vengono nominati e/o organizzati.

Per esempio, provate a modificare il codice per produrre questo risultato:
...
In questa side quest, avete esplorato come lavorare efficacemente con i metadata nei flussi di lavoro Nextflow.

Questo pattern di mantenere i metadata espliciti e collegati ai dati è una best practice fondamentale in Nextflow, offrendo diversi vantaggi rispetto alla codifica delle informazioni sui file:

- I metadata dei file rimangono associati ai file durante tutto il flusso di lavoro
- Il comportamento dei processi può essere personalizzato per file
- L'organizzazione degli output può riflettere i metadata dei file
- Le informazioni sui file possono essere espanse durante l'esecuzione del pipeline

Applicare questo pattern nel vostro lavoro vi consentirà di costruire flussi di lavoro bioinformatici robusti e manutenibili.

### Pattern chiave

1.  **Lettura e Strutturazione dei Metadata:** Leggere file CSV e creare meta map organizzate che rimangono associate ai vostri file di dati.
...
2.  **Espansione dei Metadata Durante il Flusso di Lavoro** Aggiungere nuove informazioni ai vostri metadata man mano che il vostro pipeline progredisce aggiungendo output dei processi e derivando valori attraverso logica condizionale.
...
3.  **Personalizzazione del Comportamento del Processo:** Usare i metadata all'interno del processo.
