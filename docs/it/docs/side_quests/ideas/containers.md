...
In questa sezione opzionale, vi mostreremo due modi per ottenere un'immagine di container per gli strumenti che desiderate utilizzare nelle vostre pipeline Nextflow: utilizzando Seqera Containers e costruendo l'immagine del container da voi stessi.

Otterrete/costruirete un'immagine di container per il pacchetto pip `quote`, che sarà utilizzato nell'esercizio alla fine di questa sezione.
...
Cliccate su "+Add" e poi su "Get Container" per richiedere un'immagine di container per il pacchetto pip `quote`.
...
Cliccate per copiare l'URI (ad es. `community.wave.seqera.io/library/pip_quote:ae07804021465ee9`) dell'immagine di container che è stata creata per voi.
...
### 1.2. Costruire l'immagine del container da voi stessi
...
Abbiamo aggiunto alcuni commenti esplicativi al Dockerfile qui sotto per aiutarvi a comprendere cosa fa ciascuna parte.
...
Copiate il contenuto di questi file negli stub situati nella directory `containers/build`, quindi eseguite il seguente comando per costruire l'immagine di container da voi stessi.
...
Avete imparato due modi diversi per ottenere un'immagine di container per uno strumento che desiderate utilizzare nelle vostre pipeline Nextflow: utilizzando Seqera Containers e costruendo l'immagine di container da voi stessi.

### Cosa c'è dopo?

Avete tutto ciò di cui avete bisogno per continuare al [prossimo capitolo](./04_hello_genomics.md) di questa serie di formazione.
...
Questa sezione contiene alcuni esercizi avanzati, per praticare ciò che avete imparato finora.
Completare questi esercizi _non è richiesto_ per comprendere le parti successive della formazione, ma fornisce un modo divertente per rafforzare le vostre conoscenze capendo come fare in modo che la mucca citi scienziati famosi.
...
Per l'immagine di container `quote`, potete utilizzare quella che avete costruito da voi stessi nell'esercizio avanzato precedente o utilizzare quella ottenuta da Seqera Containers.
...
### 2.2. Modificare la vostra pipeline Nextflow per permetterle di eseguire nelle modalità `quote` e `sayHello`.

Aggiungete della logica di ramificazione alla vostra pipeline per permetterle di accettare input destinati sia a `quote` che a `sayHello`.
...
Festeggiate, prendetevi una piccola pausa e bevete dell'acqua!

Quando sarete pronti, passate alla Parte 3 di questa serie di formazione per imparare come applicare ciò che avete imparato finora a un caso d'uso di analisi dati più realistico.
