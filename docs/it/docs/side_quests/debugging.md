...
Il debug è una competenza critica che può farvi risparmiare ore di frustrazione e aiutarvi a diventare sviluppatori Nextflow più efficaci. Durante la vostra carriera, specialmente quando state iniziando, incontrerete bug mentre costruite e mantenete i vostri workflow. Imparare approcci sistematici al debug vi aiuterà a identificare e risolvere i problemi rapidamente.

### Obiettivi di apprendimento

In questa side quest, esploreremo **tecniche sistematiche di debug** per i workflow Nextflow:

- **Debug degli errori di sintassi**: Usare efficacemente le funzionalità dell'IDE e i messaggi di errore di Nextflow
- **Debug dei canali**: Diagnosticare problemi di flusso dati e problemi di struttura dei canali
- **Debug dei processi**: Investigare fallimenti di esecuzione e problemi di risorse
- **Strumenti di debug integrati**: Sfruttare la modalità preview di Nextflow, l'esecuzione stub e le directory di lavoro
- **Approcci sistematici**: Una metodologia in quattro fasi per un debug efficiente

Alla fine, avrete una metodologia di debug robusta che trasforma messaggi di errore frustranti in chiare roadmap per le soluzioni.
...
**Opzionale:** Raccomandiamo di completare prima la side quest [IDE Features for Nextflow Development](./ide_features.md).
Quella copre in modo completo le funzionalità dell'IDE che supportano il debug (evidenziazione della sintassi, rilevamento degli errori, ecc.), che useremo intensamente qui.
...
4. **Correggete il bug** e verificate che la vostra soluzione funzioni
...
Le soluzioni sono discusse inline, ma provate a risolvere ciascuno da soli prima di leggere oltre.
...
Gli errori di sintassi sono il tipo più comune di errore che incontrerete quando scrivete codice Nextflow. Si verificano quando il codice non è conforme alle regole di sintassi attese del DSL Nextflow. Questi errori impediscono al vostro workflow di essere eseguito del tutto, quindi è importante imparare a identificarli e correggerli rapidamente.

### 1.1. Parentesi graffe mancanti

Uno degli errori di sintassi più comuni, e a volte uno dei più complessi da debuggare è **parentesi graffe mancanti o non corrispondenti**.
...
- **Indicatore EOF**: Il messaggio `<EOF>` (End Of File) indica che il parser ha raggiunto la fine del file mentre si aspettava ancora più contenuto - un segno classico di parentesi graffe non chiuse

#### Controllate il codice
...
Per lo scopo di questo esempio abbiamo lasciato un commento per mostrarvi dove si trova l'errore. L'estensione VSCode di Nextflow dovrebbe anche darvi alcuni suggerimenti su cosa potrebbe essere sbagliato, mettendo la parentesi graffa non corrispondente in rosso e evidenziando la fine prematura del file:

![Bad syntax](img/bad_syntax.png)

**Strategia di debug per errori di parentesi:**

1. Usate la corrispondenza delle parentesi di VS Code (posizionate il cursore accanto a una parentesi)
2. Controllate il pannello Problems per messaggi relativi alle parentesi
...
Ora eseguite di nuovo il workflow per confermare che funzioni:
...
Guardando la riga 4 nel contesto dell'errore, possiamo individuare il problema: stiamo usando `inputs` invece della corretta direttiva `input`. L'estensione VSCode di Nextflow segnalerà anche questo:
...
I nomi delle variabili che usate nei vostri blocchi script devono essere validi, derivati dagli input o dal codice groovy inserito prima dello script. Ma quando state gestendo la complessità all'inizio dello sviluppo della pipeline, è facile fare errori nella denominazione delle variabili, e Nextflow ve lo farà sapere rapidamente.
...
L'errore viene catturato al momento della compilazione e punta direttamente alla variabile non definita alla riga 17, con un accento circonflesso che indica esattamente dove si trova il problema.
...
Il messaggio di errore indica che la variabile non è riconosciuta nel template dello script, ed eccola lì - dovreste essere in grado di vedere `${undefined_var}` usata nel blocco script, ma non definita altrove.
...
All'inizio con Nextflow, può essere difficile capire la differenza tra le variabili Nextflow (Groovy) e Bash. Questo può generare un'altra forma dell'errore di variabile errata che appare quando si cerca di usare variabili nel contenuto Bash del blocco script.
...
Se volete usare una variabile Bash, dovete fare l'escape del simbolo del dollaro così:
...
Questo dice a Nextflow di interpretare questa come una variabile Bash.
...
!!! tip "Variabili Groovy vs Bash"
    ...
    Questo approccio evita la necessità di fare l'escape dei simboli del dollaro e rende il codice più facile da leggere e mantenere.

### 1.5. Istruzioni fuori dal blocco workflow

L'estensione VSCode di Nextflow evidenzia problemi con la struttura del codice che causeranno errori. Un esempio comune è definire canali fuori dal blocco `workflow {}` - questo è ora applicato come errore di sintassi.
...
Il messaggio di errore indica chiaramente il problema: le istruzioni (come le definizioni di canali) non possono essere mescolate con le dichiarazioni di script fuori da un blocco workflow o process.
...
Eseguite di nuovo il workflow per confermare che la correzione funzioni:
...
Gli errori di struttura dei canali sono più sottili degli errori di sintassi perché il codice è sintatticamente corretto, ma le forme dei dati non corrispondono a ciò che i processi si aspettano. Nextflow proverà a eseguire la pipeline, ma potrebbe scoprire che il numero di input non corrisponde a ciò che si aspetta e fallire. Questi errori tipicamente appaiono solo al runtime e richiedono una comprensione dei dati che fluiscono attraverso il vostro workflow.

!!! tip "Debug dei Canali con `.view()`"

    In tutta questa sezione, ricordate che potete usare l'operatore `.view()` per ispezionare il contenuto del canale in qualsiasi punto del vostro workflow. Questo è uno degli strumenti di debug più potenti per capire i problemi di struttura dei canali. Esploreremo questa tecnica in dettaglio nella sezione 2.4, ma sentitevi liberi di usarla mentre lavorate attraverso gli esempi.
...
Dovreste vedere la chiamata `PROCESS_FILES` non corrispondente, che fornisce più canali di input quando il processo ne definisce solo uno. L'estensione VSCode sottolineerà anche la chiamata al processo in rosso e fornirà un messaggio diagnostico quando passate il mouse sopra:
...
Più comunemente di questo esempio, potreste aggiungere input aggiuntivi a un processo e dimenticare di aggiornare di conseguenza la chiamata al workflow, il che può portare a questo tipo di errore. Fortunatamente, questo è uno degli errori più facili da capire e correggere, poiché il messaggio di errore è abbastanza chiaro sulla mancata corrispondenza.

### 2.2. Esaurimento del canale (il processo viene eseguito meno volte del previsto)

Alcuni errori di struttura dei canali sono molto più sottili e non producono errori affatto. Probabilmente il più comune di questi riflette una sfida che i nuovi utenti di Nextflow affrontano nel capire che i canali di coda possono esaurirsi e rimanere senza elementi, il che significa che il workflow finisce prematuramente.
...
Il processo viene eseguito solo una volta invece di tre volte perché il canale `reference_ch` è un canale di coda che si esaurisce dopo la prima esecuzione del processo. Quando un canale si esaurisce, l'intero processo si ferma, anche se altri canali hanno ancora elementi.

Questo è un pattern comune dove avete un singolo file di riferimento che deve essere riutilizzato su più campioni. La soluzione è convertire il canale di riferimento in un canale di valore che può essere riutilizzato indefinitamente.

#### Correggete il codice

Ci sono un paio di modi per affrontare questo a seconda di quanti file sono interessati.

**Opzione 1**: Avete un singolo file di riferimento che state riutilizzando molto. Potete semplicemente creare un tipo di canale di valore, che può essere usato più e più volte. Ci sono tre modi per farlo:
...
**1b** Usate l'[operatore](https://www.nextflow.io/docs/latest/reference/operator.html#first) `first()`:
...
**1c.** Usate l'[operatore](https://www.nextflow.io/docs/latest/reference/operator.html#collect) `collect()`:
...
L'operatore `.combine()` genera un prodotto cartesiano dei due canali, quindi ogni elemento in `reference_ch` sarà accoppiato con ogni elemento in `input_ch`. Questo permette al processo di essere eseguito per ogni campione mentre usa ancora il riferimento.
...
Dovreste ora vedere tutti e tre i campioni essere elaborati invece di uno solo.

### 2.3. Struttura del contenuto del canale errata

Quando i workflow raggiungono un certo livello di complessità, può essere un po' difficile tenere traccia delle strutture interne di ogni canale, e le persone generano comunemente mancate corrispondenze tra ciò che il processo si aspetta e ciò che il canale contiene effettivamente. Questo è più sottile del problema che abbiamo discusso prima, dove il numero di canali era errato. In questo caso, potete avere il numero corretto di canali di input, ma la struttura interna di uno o più di quei canali non corrisponde a ciò che il processo si aspetta.
...
=== "Opzione 2: Estrarre il primo elemento"
...
Scegliete una delle soluzioni ed eseguite di nuovo il workflow:
...
Per evitare di usare operazioni `.view()` eccessivamente in futuro per capire il contenuto del canale, è consigliabile aggiungere alcuni commenti per aiutare:
...
Questo diventerà più importante man mano che i vostri workflow crescono in complessità e la struttura del canale diventa più opaca.
...
Dovreste vedere che c'è una mancata corrispondenza tra il nome del file di output nel blocco `output:` e quello usato nello script. Questa mancata corrispondenza causa il fallimento del processo. Se incontrate questo tipo di errore, tornate indietro e controllate che gli output corrispondano tra la vostra definizione di processo e il vostro blocco output.
...
Il processo non ha accesso al comando che stiamo specificando. A volte questo è perché uno script è presente nella directory `bin` del workflow, ma non è stato reso eseguibile. Altre volte è perché il software non è installato nel container o nell'ambiente dove il workflow è in esecuzione.
...
Nell'uso in produzione, configurerete le risorse sui vostri processi. Per esempio `memory` definisce la quantità massima di memoria disponibile per il vostro processo, e se il processo la supera, il vostro scheduler tipicamente ucciderà il processo e restituirà un codice di uscita di `137`. Non possiamo dimostrarlo qui perché stiamo usando l'executor `local`, ma possiamo mostrare qualcosa di simile con `time`.

#### Eseguite la pipeline

`bad_resources.nf` ha una configurazione di processo con un limite di tempo irrealistico di 1 millisecondo:
...
Sappiamo che il processo impiegherà più di un secondo (abbiamo aggiunto uno sleep lì per assicurarci), ma il processo è impostato per scadere dopo 1 millisecondo. Qualcuno è stato un po' irrealistico con la loro configurazione!
...
Se vi assicurate di leggere i vostri messaggi di errore, fallimenti come questo non dovrebbero confondervi per troppo tempo. Ma assicuratevi di capire i requisiti di risorse dei comandi che state eseguendo in modo da poter configurare le vostre direttive di risorse appropriatamente.

### 3.4. Tecniche di debug dei processi

Quando i processi falliscono o si comportano in modo inaspettato, avete bisogno di tecniche sistematiche per investigare cosa è andato storto. La directory di lavoro contiene tutte le informazioni di cui avete bisogno per debuggare l'esecuzione del processo.

#### Uso dell'ispezione della directory di lavoro

Lo strumento di debug più potente per i processi è esaminare la directory di lavoro. Quando un processo fallisce, Nextflow crea una directory di lavoro per quella specifica esecuzione del processo contenente tutti i file necessari per capire cosa è successo.

#### Eseguite la pipeline

Usiamo l'esempio `missing_output.nf` di prima per dimostrare l'ispezione della directory di lavoro (rigenerate una mancata corrispondenza di denominazione dell'output se necessario):
...
#### Controllate la directory di lavoro
...
- **Percorsi dei file**: Se i file di input sono stati localizzati correttamente
...
- **Percorsi dei file errati**: File di input che non esistono o sono in posizioni errate
...
- **Errori di permessi**: Problemi di accesso ai file
...
- **Output atteso**: Se il comando ha prodotto i risultati giusti
...
- **Codice di uscita 127**: Comando non trovato - controllate l'installazione del software
- **Codice di uscita 137**: Processo ucciso - controllate i limiti di memoria/tempo
...
- **Mancate corrispondenze di denominazione dei file**: File di output con nomi diversi da quelli attesi
- **Problemi di permessi**: File che non potevano essere creati
- **Problemi di percorso**: File creati in directory errate

Nel nostro esempio precedente, questo ci ha confermato che mentre il nostro `sample3.txt` atteso non era presente, `sample3_output.txt` lo era:
...
Il debug dei processi richiede l'esame delle directory di lavoro per capire cosa è andato storto. I file chiave includono `.command.sh` (lo script eseguito), `.command.err` (messaggi di errore) e `.command.out` (output standard). I codici di uscita come 127 (comando non trovato) e 137 (processo ucciso) forniscono indizi diagnostici immediati sul tipo di fallimento.

### Cosa c'è dopo?

Imparate sugli strumenti di debug integrati di Nextflow e approcci sistematici alla risoluzione dei problemi.

---

## 4. Strumenti di Debug Integrati e Tecniche Avanzate

Nextflow fornisce diversi potenti strumenti integrati per il debug e l'analisi dell'esecuzione del workflow. Questi strumenti vi aiutano a capire cosa è andato storto, dove è andato storto e come risolverlo efficientemente.

### 4.1. Output del processo in tempo reale

A volte avete bisogno di vedere cosa sta succedendo all'interno dei processi in esecuzione. Potete abilitare l'output del processo in tempo reale, che vi mostra esattamente cosa sta facendo ogni attività mentre viene eseguita.

#### Eseguite la pipeline

`bad_channel_shape_viewed.nf` dai nostri esempi precedenti ha stampato il contenuto del canale usando `.view()`, ma possiamo anche usare la direttiva `debug` per fare echo delle variabili dall'interno del processo stesso, che dimostriamo in `bad_channel_shape_viewed_debug.nf`. Eseguite il workflow:
...
La modalità preview vi permette di testare la logica del workflow senza eseguire comandi. Questo può essere abbastanza utile per controllare rapidamente la struttura del vostro workflow e assicurarsi che i processi siano connessi correttamente senza eseguire alcun comando effettivo.
...
La modalità preview è particolarmente utile per catturare errori di sintassi presto senza eseguire alcun processo. Valida la struttura del workflow e le connessioni dei processi prima dell'esecuzione.

### 4.3. Esecuzione Stub per il test della logica

A volte gli errori sono difficili da debuggare perché i comandi richiedono troppo tempo, richiedono software speciale o falliscono per ragioni complesse. L'esecuzione stub vi permette di testare la logica del workflow senza eseguire i comandi effettivi.

#### Eseguite la pipeline

Quando state sviluppando un processo Nextflow, potete usare la direttiva `stub` per definire comandi 'dummy' che generano output della forma corretta senza eseguire il comando reale. Questo approccio è particolarmente prezioso quando volete verificare che la logica del vostro workflow sia corretta prima di affrontare le complessità del software effettivo.

Per esempio, ricordate il nostro `missing_software.nf` di prima? Quello dove avevamo software mancante che impediva l'esecuzione del workflow finché non abbiamo aggiunto `-profile docker`? `missing_software_with_stub.nf` è un workflow molto simile. Se lo eseguiamo nello stesso modo, genereremo lo stesso errore:
...
Rispetto a `missing_software.nf`, questo processo ha una direttiva `stub:` che specifica un comando da usare invece di quello specificato in `script:`, nel caso in cui Nextflow venga eseguito in modalità stub.

Il comando `touch` che stiamo usando qui non dipende da alcun software o input appropriati, e verrà eseguito in tutte le situazioni, permettendoci di debuggare la logica del workflow senza preoccuparci degli interni del processo.

**L'esecuzione stub aiuta a debuggare:**

- Struttura del canale e flusso dei dati
- Connessioni e dipendenze dei processi
- Propagazione dei parametri
- Logica del workflow senza dipendenze dal software

### 4.4. Approccio sistematico al debug

Ora che avete imparato le tecniche di debug individuali - dai file di trace e directory di lavoro alla modalità preview, esecuzione stub e monitoraggio delle risorse - leghiamole insieme in una metodologia sistematica. Avere un approccio strutturato vi impedisce di essere sopraffatti da errori complessi e assicura che non perdiate indizi importanti.
...
1. Controllate le sottolineature rosse in VSCode o nel vostro IDE
2. Eseguite `nextflow run workflow.nf -preview` per identificare problemi di sintassi
3. Correggete tutti gli errori di sintassi (parentesi graffe mancanti, virgole finali, ecc.)
4. Assicuratevi che il workflow venga analizzato con successo prima di procedere

**Fase 2: Valutazione rapida (5 minuti)**

1. Leggete attentamente i messaggi di errore di runtime
2. Controllate se è un errore di runtime, logica o risorse
3. Usate la modalità preview per testare la logica base del workflow

**Fase 3: Investigazione dettagliata (15-30 minuti)**
...
1. Fate correzioni mirate minime
2. Testate con resume: `nextflow run workflow.nf -resume`
3. Verificate l'esecuzione completa del workflow

!!! tip "Uso di Resume per un debug efficiente"

    Una volta identificato un problema, avete bisogno di un modo efficiente per testare le vostre correzioni senza sprecare tempo rieseguendo parti di successo del vostro workflow. La funzionalità `-resume` di Nextflow è inestimabile per il debug.

    Avrete incontrato `-resume` se avete lavorato attraverso [Hello Nextflow](../hello_nextflow/), ed è importante che ne facciate buon uso quando debuggate per risparmiare tempo di attesa mentre i processi prima del vostro processo problematico vengono eseguiti.
    ...
    **Strategia di debug con resume:**
    ...
    4. Fate resume per testare solo la correzione
...
Poi potete eseguire la pipeline con questo profilo abilitato:
...
!!! exercise
    ...
    Usate l'approccio sistematico al debug per identificare e correggere tutti gli errori in `buggy_workflow.nf`. Questo workflow tenta di elaborare dati di campioni da un file CSV ma contiene più bug intenzionali che rappresentano scenari di debug comuni.
    ...
    ??? failure "Command output"
        ...
        Questo errore criptico indica un problema di parsing intorno alla riga 11-12 nel blocco `params{}`. Il parser v2 cattura problemi strutturali presto.

    Applicate il metodo di debug in quattro fasi che avete imparato:

    **Fase 1: Risoluzione degli errori di sintassi**
    - Controllate le sottolineature rosse in VSCode o nel vostro IDE
    - Eseguite `nextflow run workflow.nf -preview` per identificare problemi di sintassi
    - Correggete tutti gli errori di sintassi (parentesi graffe mancanti, virgole finali, ecc.)
    - Assicuratevi che il workflow venga analizzato con successo prima di procedere

    **Fase 2: Valutazione rapida**
    - Leggete attentamente i messaggi di errore di runtime
    - Identificate se gli errori sono relativi a runtime, logica o risorse
    - Usate la modalità `-preview` per testare la logica base del workflow

    **Fase 3: Investigazione dettagliata**
    ...
    **Fase 4: Correzione e validazione**
    - Fate correzioni mirate
    - Usate `-resume` per testare le correzioni efficientemente
    ...
    **Strumenti di debug a vostra disposizione:**
    ...
    ??? solution
        Il `buggy_workflow.nf` contiene 9 o 10 errori distinti (a seconda di come li contate) che coprono tutte le principali categorie di debug. Ecco una ripartizione sistematica di ogni errore e come correggerlo
        ...
        **Errore 3: Errore di nome variabile**
        ```groovy linenums="26"
        ...
        **Correzione:** Usate il canale corretto ed estraete gli ID dei campioni
        ...
        A questo punto il workflow verrà eseguito, ma riceveremo ancora errori (es. `Path value cannot be null` in `processFiles`), causati da una struttura del canale errata.
        ...
        **Correzione:** Restituite la struttura della tupla che processFiles si aspetta
        ...
        Ma questo romperà la nostra correzione per l'esecuzione di `heavyProcess()` sopra, quindi dovremo usare una map per passare solo gli ID dei campioni a quel processo:
        ...
        **Errore 9: Mancata corrispondenza del nome del file di output**
        ...
        ```
        **Correzione:** Fate corrispondere la dichiarazione di output
        ```groovy linenums="49"
...
**Categorie di errori coperte:**

- **Errori di sintassi**: Parentesi graffe mancanti, virgole finali, variabili non definite
- **Errori di struttura dei canali**: Forme di dati errate, canali non definiti
- **Errori di processo**: Mancate corrispondenze dei file di output, escape delle variabili
...
3. **Capite il flusso dei dati** - gli errori di struttura dei canali sono spesso i più sottili
...
Applicare queste tecniche nel vostro lavoro vi permetterà di passare meno tempo a combattere con il vostro computer, risolvere i problemi più velocemente e proteggervi da problemi futuri.

### Pattern chiave

**1. Come identificare e correggere errori di sintassi**:
...
- Usare le funzionalità dell'estensione VS Code per il rilevamento precoce degli errori
...
**2. Come debuggare problemi di struttura dei canali**:

- Capire la cardinalità dei canali e i problemi di esaurimento
- Debuggare mancate corrispondenze della struttura del contenuto dei canali
- Usare operatori `.view()` per l'ispezione dei canali
- Riconoscere pattern di errore come parentesi quadre nelle strutture di tuple inaspettate
...
**3. Come risolvere problemi di esecuzione dei processi**:

- Diagnosticare errori di file di output mancanti
- Capire i codici di uscita (127 per software mancante, 137 per problemi di memoria)
- Investigare le directory di lavoro e i file di comando
- Configurare le risorse appropriatamente
...
- Implementare l'esecuzione stub per il test della logica
...
!!! tip "Riferimento rapido per il debug"
    ...
    **Problemi di canali?** → Usate `.view()` per ispezionare il contenuto: `my_channel.view()`

    **Fallimenti di processi?** → Controllate i file della directory di lavoro:
    ...
    **Fatto correzioni?** → Usate `-resume` per risparmiare tempo nei test: `nextflow run workflow.nf -resume`
...
- [Capire i canali Nextflow](https://www.nextflow.io/docs/latest/channel.html): Approfondimento sui tipi e comportamenti dei canali
- [Riferimento delle direttive di processo](https://www.nextflow.io/docs/latest/process.html#directives): Tutte le opzioni di configurazione dei processi disponibili
- [nf-test](https://www.nf-test.com/): Framework di test per le pipeline Nextflow
- [Comunità Slack di Nextflow](https://www.nextflow.io/slack-invite.html): Ottenete aiuto dalla comunità

Per i workflow in produzione, considerate:

- Impostare [Seqera Platform](https://seqera.io/platform/) per il monitoraggio e il debug su larga scala
