...
- **Manipolazione dei dati:** Estrarre, trasformare e suddividere mappe e collezioni usando operatori potenti
...
- **Routing condizionale:** Instradare intelligentemente i campioni attraverso diversi processi in base alle loro caratteristiche di metadata
...
- **Handler basati su configurazione:** Usare handler di eventi del workflow per logging, notifiche e gestione del ciclo di vita
...
- Essere a vostro agio nell'uso di concetti e meccanismi di base di Nextflow (processi, canali, operatori, lavorare con file, metadata)
...
Troverete un file di workflow principale e una directory `data` contenente file di dati di esempio.
...
Iniziamo con un semplice workflow che legge solo il file CSV (l'abbiamo già fatto per voi in `main.nf`):
...
Ora aggiungeremo dello scripting per trasformare i dati, usando l'operatore `.map()` che probabilmente conoscete già. Questo operatore prende una 'closure' dove possiamo scrivere codice per trasformare ogni elemento.
...
Abbiamo aggiunto con successo logica condizionale per arricchire i nostri metadata con un livello di priorità basato sui punteggi di qualità.

#### 1.1.5. Suddividere Mappe con `.subMap()`
...
Aggiungiamo una riga per creare una versione semplificata dei nostri metadata che contiene solo campi di identificazione:
...
Questo mostra sia i metadata completi visualizzati dall'operazione `view()` sia il sottoinsieme estratto che abbiamo stampato con `println`.
...
Questo è particolarmente utile quando dovete creare diverse versioni di metadata per processi diversi - alcuni potrebbero aver bisogno di metadata completi mentre altri necessitano solo di campi di identificazione minimi.

Ora rimuovete quelle istruzioni println per ripristinare il vostro workflow allo stato precedente, poiché non ne abbiamo bisogno andando avanti.
...
Finora, abbiamo restituito solo quella che la comunità Nextflow chiama la 'meta map', e abbiamo ignorato i file a cui quei metadata si riferiscono. Ma se state scrivendo workflow Nextflow, probabilmente volete fare qualcosa con quei file.

Produciamo una struttura di canale composta da una tupla di 2 elementi: la mappa di metadata arricchita e il percorso del file corrispondente. Questo è un pattern comune in Nextflow per passare dati ai processi.
...
Questa struttura di tupla `[meta, file]` è un pattern comune in Nextflow per passare sia metadata che file associati ai processi.

!!! note

    **Mappe e Metadata**: Le mappe sono fondamentali per lavorare con i metadata in Nextflow. Per una spiegazione più dettagliata su come lavorare con mappe di metadata, consultate la side quest [Lavorare con i metadata](./metadata.md).

Il nostro workflow dimostra il pattern fondamentale: le **operazioni di dataflow** (`workflow`, `channel.fromPath()`, `.splitCsv()`, `.map()`, `.view()`) orchestrano come i dati si muovono attraverso la pipeline, mentre lo **scripting** (mappe `[key: value]`, metodi di stringhe, conversioni di tipo, operatori ternari) all'interno della closure `.map()` gestisce la trasformazione dei singoli elementi di dati.
...
- Stampare ogni elemento con `view()` mentre fluisce
...
Comprendere questi confini è essenziale per il debugging, la documentazione e la scrittura di workflow manutenibili.
...
I file bioinformatici hanno spesso convenzioni di denominazione complesse che codificano metadata. Estraiamoli automaticamente usando il pattern matching con espressioni regolari.

Torneremo al nostro workflow `main.nf` e aggiungeremo della logica di pattern matching per estrarre informazioni aggiuntive sui campioni dai nomi dei file. I file FASTQ nel nostro dataset seguono convenzioni di denominazione in stile Illumina con nomi come `SAMPLE_001_S1_L001_R1_001.fastq.gz`. Potrebbero sembrare criptici, ma in realtà codificano metadata utili come ID campione, numero di corsia e direzione di lettura. Useremo le capacità regex per analizzare questi nomi.

Fate la seguente modifica al vostro workflow `main.nf` esistente:
...
Questo analizza le convenzioni di denominazione in stile Illumina per estrarre metadata automaticamente.
...
Questo mostra i metadata arricchiti dai nomi dei file.
...
Prima, includete il modulo alla primissima riga del vostro workflow `main.nf`:
...
- **Espressioni regolari per l'analisi dei file**: Usare l'operatore `=~` e pattern regex (`~/pattern/`) per estrarre metadata da convenzioni di denominazione dei file complesse
...
La logica complessa del workflow inline negli operatori di canale o nelle definizioni dei processi riduce la leggibilità e la manutenibilità. Le **funzioni** vi permettono di estrarre questa logica in componenti con nome e riutilizzabili.
...
Per illustrare come appare con il nostro workflow esistente, fate la modifica qui sotto, usando `def` per definire una funzione riutilizzabile chiamata `separateMetadata`:
...
Questo rende la logica del workflow molto più facile da leggere e comprendere a colpo d'occhio. La funzione `separateMetadata` incapsula tutta la logica complessa per l'analisi e l'arricchimento dei metadata, rendendola riutilizzabile e testabile.
...
L'output dovrebbe mostrare entrambi i processi completati con successo. Il workflow è ora molto più pulito e facile da mantenere, con tutta la logica complessa di elaborazione dei metadata incapsulata nella funzione `separateMetadata`.
...
- **Scope delle funzioni**: Le funzioni definite a livello di script sono accessibili in tutto il vostro workflow Nextflow
...
Potete controllare il comando `docker` esatto che è stato eseguito per vedere l'allocazione di CPU per una data attività:
...
Questo è uno scenario molto comune nei workflow del mondo reale - a volte semplicemente non sapete quanta memoria un'attività avrà bisogno finché non la eseguite.
...
- Combinare più fattori (metadata, numero di tentativi, priorità)
...
Abbiamo fornito un semplice processo Trimgalore in `modules/trimgalore.nf`, date un'occhiata se volete, ma i dettagli non sono importanti per questo esercizio. Il punto chiave è che vogliamo instradare i campioni in base ai loro metadata.
...
... e poi modificate il vostro workflow `main.nf` per ramificare i campioni in base ai loro metadata e instradarli attraverso il processo di trimming appropriato, così:
...
Qui, abbiamo usato espressioni condizionali piccole ma potenti all'interno dell'operatore `.branch{}` per instradare i campioni in base ai loro metadata. I campioni umani con alta copertura passano attraverso `FASTP`, mentre tutti gli altri campioni passano attraverso `TRIMGALORE`.

### 5.2. Usare `.filter()` con Truthiness

Un altro pattern potente per controllare l'esecuzione del workflow è l'operatore `.filter()`, che usa una closure per determinare quali elementi dovrebbero continuare lungo la pipeline. All'interno della closure del filtro, scriverete **espressioni booleane** che decidono quali elementi passano.
...
L'espressione del filtro `meta.id && meta.organism && meta.depth >= 25000000` combina truthiness con confronti espliciti:
...
La nostra pipeline ora instrada intelligentemente i campioni attraverso processi appropriati, ma i workflow di produzione devono gestire con eleganza i dati non validi. Rendiamo il nostro workflow robusto contro valori mancanti o null.
...
Nessun crash! Il workflow ora gestisce con eleganza il campo mancante. Quando `row.run_id` è `null`, l'operatore `?.` previene la chiamata `.toUpperCase()`, e `run_id` diventa `null` invece di causare un'eccezione.
...
A volte dovete fermare immediatamente il workflow se i parametri di input non sono validi. In Nextflow, potete usare funzioni integrate come `error()` e `log.warn`, così come costrutti di programmazione standard come istruzioni `if` e logica booleana, per implementare logica di validazione. Aggiungiamo validazione al nostro workflow.
...
Ora provate ad eseguire senza il file CSV:
...
- **`error()`**: Ferma immediatamente il workflow con un messaggio chiaro
- **`log.warn`**: Emette avvisi senza fermare il workflow
- **Validazione precoce**: Controllare gli input prima dell'elaborazione per fallire velocemente con errori utili
- **Funzioni di validazione**: Creare logica di validazione riutilizzabile che può essere chiamata all'inizio del workflow
...
Fino ad ora, abbiamo scritto codice nei nostri script di workflow e nelle definizioni dei processi. Ma c'è un'altra caratteristica importante che dovreste conoscere: gli handler di eventi del workflow.
...
In questa side quest, avete costruito una pipeline completa di elaborazione campioni che si è evoluta dalla gestione di base dei metadata a un workflow sofisticato e pronto per la produzione.
...
- **Codice più chiaro**: Comprendere dataflow vs scripting vi aiuta a scrivere workflow più organizzati
...
1.  **Dataflow vs Scripting:** Avete imparato a distinguere tra operazioni di dataflow (orchestrazione dei canali) e scripting (codice che manipola dati), incluse le differenze cruciali tra operazioni su tipi diversi come `collect` su Channel vs List.
...
3.  **Creare Funzioni Riutilizzabili**: Avete imparato a estrarre logica complessa in funzioni con nome che possono essere chiamate dagli operatori di canale, rendendo i workflow più leggibili e manutenibili.
...
5.  **Logica Condizionale e Controllo dei Processi**: Avete aggiunto routing intelligente usando gli operatori `.branch()` e `.filter()`, sfruttando la truthiness per espressioni condizionali concise.
    ...
    - Usare `filter()` per suddividere dati con 'truthiness'
...
- Scrivere workflow più puliti con una corretta separazione tra dataflow e scripting
