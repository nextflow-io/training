...
Questa missione secondaria vi accompagna in un percorso pratico dai concetti di base ai pattern pronti per la produzione.
...
Se non lo avete ancora fatto, assicuratevi di aprire l'ambiente di formazione come descritto in [Configurazione Ambiente](../envsetup/index.md).
...
Troverete un file workflow principale e una directory `data` contenente file di dati di esempio.
...
Utilizzeremo questo dataset realistico per esplorare tecniche di programmazione pratiche che incontrerete in workflow bioinformatici reali.
...
Eseguite questo workflow per vedere i dati CSV grezzi:
...
Ora aggiungeremo scripting per trasformare i dati, utilizzando l'operatore `.map()` che probabilmente conoscete già. Questo operatore prende una 'closure' dove possiamo scrivere codice per trasformare ogni elemento.

!!! note

    Una **closure** è un blocco di codice che può essere passato in giro ed eseguito successivamente. Pensate ad essa come a una funzione definita inline. Le closure sono scritte con parentesi graffe `{ }` e possono prendere parametri. Sono fondamentali per come funzionano gli operatori Nextflow e se avete scritto Nextflow per un po', potreste averle già utilizzate senza rendervene conto!
...
Effettuate la seguente modifica:
...
!!! Note
    ...
    Non modificate mai le mappe passate nelle closure - create sempre nuove mappe utilizzando `+` (per esempio). In Nextflow, gli stessi dati spesso fluiscono attraverso più operazioni simultaneamente. Modificare una mappa sul posto può causare effetti collaterali imprevedibili quando altre operazioni fanno riferimento a quello stesso oggetto. Creare nuove mappe assicura che ogni operazione abbia la propria copia pulita.

Eseguite il workflow modificato:
...
Ora rimuovete quelle istruzioni println per ripristinare il workflow allo stato precedente, poiché non ne abbiamo bisogno andando avanti.
...
Finora, abbiamo restituito solo quella che la comunità Nextflow chiama 'meta map', e abbiamo ignorato i file a cui quei metadati si riferiscono. Ma se state scrivendo workflow Nextflow, probabilmente volete fare qualcosa con quei file.
...
!!! note
    ...
    **Mappe e Metadati**: Le mappe sono fondamentali per lavorare con i metadati in Nextflow. Per una spiegazione più dettagliata del lavoro con mappe di metadati, vedete la missione secondaria [Lavorare con i metadati](./metadata.md).
...
Dimostriamo questo con alcuni dati di esempio, iniziando col rinfrescarci su cosa fa l'operatore `collect()` del canale. Date un'occhiata a `collect.nf`:
...
Eseguite il workflow per confermarlo:
...
Eseguite il workflow aggiornato:
...
Effettuate la seguente modifica al workflow `main.nf` esistente:
...
I blocchi script dei processi sono essenzialmente stringhe multi-linea che vengono passate alla shell. Potete usare **logica condizionale** (if/else, operatori ternari) per generare dinamicamente diverse stringhe di script in base alle caratteristiche dell'input. Questo è essenziale per gestire diversi tipi di input—come letture single-end vs paired-end—senza duplicare le definizioni dei processi.

Aggiungiamo un processo al nostro workflow che dimostri questo pattern. Aprite `modules/fastp.nf` e date un'occhiata:
...
Per prima cosa, includete il modulo alla prima riga del workflow `main.nf`:

```groovy title="main.nf" linenums="1"
include { FASTP } from './modules/fastp.nf'
```

Quindi modificate il blocco `workflow` per collegare il canale `ch_samples` al processo `FASTP`:
...
Eseguite questo workflow modificato:
...
Correggete questo aggiungendo logica condizionale al blocco `script:` del processo `FASTP`. Un'istruzione if/else controlla il conteggio dei file di lettura e regola il comando di conseguenza.
...
Date un'occhiata al file modulo `modules/generate_report.nf`:
...
Includete il processo nel vostro `main.nf` e aggiungetelo al workflow:
...
Correggete questo facendo l'escape delle variabili shell e delle sostituzioni di comandi con un backslash (`\`):
...
- **Generazione dinamica di script**: Utilizzare logica condizionale (if/else, operatori ternari) per generare diverse stringhe di script in base alle caratteristiche dell'input
- **Interpolazione di variabili**: Comprendere quando Nextflow interpreta le stringhe vs quando lo fa la shell
  ...
  - `\${var}` - Variabili di ambiente shell (con escape, passate a bash al runtime)
  - `\$(cmd)` - Sostituzione di comandi shell (con escape, eseguita da bash al runtime)

Questi pattern di elaborazione e generazione delle stringhe sono essenziali per gestire i diversi formati di file e convenzioni di denominazione che incontrerete in workflow bioinformatici reali.

---

## 3. Creare Funzioni Riutilizzabili

La logica complessa del workflow inline negli operatori di canale o nelle definizioni di processo riduce la leggibilità e la manutenibilità. Le **funzioni** permettono di estrarre questa logica in componenti nominati e riutilizzabili.
...
Per illustrare come appare con il nostro workflow esistente, effettuate la modifica qui sotto, usando `def` per definire una funzione riutilizzabile chiamata `separateMetadata`:
...
Questo rende la logica del workflow molto più facile da leggere e comprendere a colpo d'occhio. La funzione `separateMetadata` incapsula tutta la logica complessa per analizzare e arricchire i metadati, rendendola riutilizzabile e testabile.

Eseguite il workflow per assicurarvi che funzioni ancora:
...
L'output dovrebbe mostrare entrambi i processi completati con successo. Il workflow è ora molto più pulito e facile da mantenere, con tutta la logica complessa di elaborazione dei metadati incapsulata nella funzione `separateMetadata`.

### Takeaway

In questa sezione, avete imparato la **creazione di funzioni**:

- **Definire funzioni con `def`**: La parola chiave per creare funzioni nominate (come `def` in Python o `function` in JavaScript)
- **Scope delle funzioni**: Le funzioni definite a livello di script sono accessibili in tutto il workflow Nextflow
...
Successivamente, esploreremo come utilizzare le closure nelle direttive di processo per l'allocazione dinamica delle risorse.

---

## 4. Direttive di Risorse Dinamiche con Closure

Finora abbiamo usato lo scripting nel blocco `script` dei processi. Ma le **closure** (introdotte nella Sezione 1.1) sono anche incredibilmente utili nelle direttive di processo, specialmente per l'allocazione dinamica delle risorse. Aggiungiamo direttive di risorse al nostro processo FASTP che si adattano in base alle caratteristiche del campione.
...
Eseguite nuovamente il workflow con l'opzione `-ansi-log false` per rendere più facile vedere gli hash delle attività.
...
In questo esempio abbiamo scelto un esempio che ha richiesto 2 CPU (`--cpu-shares 2048`), perché era un campione ad alta profondità, ma dovreste vedere diverse allocazioni di CPU a seconda della profondità del campione. Provate questo anche per le altre attività.

### 4.2. Strategie di retry

Un altro pattern potente è l'uso di `task.attempt` per strategie di retry. Per mostrare perché questo è utile, inizieremo riducendo l'allocazione di memoria a FASTP a meno di quanto necessario. Cambiate la direttiva `memory` in `modules/fastp.nf` a `1.GB`:
...
... ed eseguite nuovamente il workflow:
...
Questo è uno scenario molto comune in workflow reali - a volte semplicemente non sapete quanta memoria un'attività avrà bisogno finché non la eseguite.

Per rendere il nostro workflow più robusto, possiamo implementare una strategia di retry che aumenta l'allocazione di memoria ad ogni tentativo, ancora una volta usando una closure Groovy. Modificate la direttiva `memory` per moltiplicare la memoria base per `task.attempt`, e aggiungete le direttive `errorStrategy 'retry'` e `maxRetries 2`:
...
Le direttive dinamiche con closure permettono di:
...
- Usare logica condizionale per calcoli di risorse complessi

Questo rende i vostri workflow sia più efficienti (non sovra-allocando) che più robusti (retry automatico con più risorse).

---

## 5. Logica Condizionale e Controllo dei Processi

Precedentemente, abbiamo usato `.map()` con scripting per trasformare i dati del canale. Ora useremo la logica condizionale per controllare quali processi vengono eseguiti in base ai dati—essenziale per workflow flessibili che si adattano a diversi tipi di campioni.

Gli [operatori di dataflow](https://www.nextflow.io/docs/latest/reference/operator.html) di Nextflow prendono closure valutate al runtime, abilitando la logica condizionale per guidare le decisioni del workflow in base al contenuto del canale.
...
... e poi modificate il vostro workflow `main.nf` per ramificare i campioni in base ai loro metadati e instradarli attraverso il processo di trimming appropriato, così:
...
Un altro pattern potente per controllare l'esecuzione del workflow è l'operatore `.filter()`, che usa una closure per determinare quali elementi dovrebbero continuare lungo la pipeline. All'interno della closure filter, scriverete **espressioni booleane** che decidono quali elementi passano.
...
Eseguite nuovamente il workflow:
...
In questa sezione, avete imparato a usare la logica condizionale per controllare l'esecuzione del workflow usando le interfacce closure degli operatori Nextflow come `.branch{}` e `.filter{}`, sfruttando la truthiness per scrivere espressioni condizionali concise.

La nostra pipeline ora instrada intelligentemente i campioni attraverso processi appropriati, ma i workflow di produzione devono gestire i dati non validi con eleganza. Rendiamo il nostro workflow robusto contro valori mancanti o null.
...
Nessun crash! Il workflow ora gestisce il campo mancante con eleganza. Quando `row.run_id` è `null`, l'operatore `?.` previene la chiamata `.toUpperCase()`, e `run_id` diventa `null` invece di causare un'eccezione.

### 6.3. Operatore Elvis (`?:`) per Valori Predefiniti

L'operatore Elvis (`?:`) fornisce valori predefiniti quando il lato sinistro è "falsy" (come spiegato precedentemente). È chiamato così per Elvis Presley perché `?:` assomiglia ai suoi famosi capelli e occhi quando visto di lato!
...
Aggiungete anche un operatore `view()` nel workflow per vedere i risultati:
...
!!! tip "Combinare Navigazione Sicura ed Elvis"
    ...
    Questo pattern gestisce dati mancanti/incompleti con eleganza.

Usate questi operatori in modo coerente in funzioni, closure di operatori (`.map{}`, `.filter{}`), script di processo e file di configurazione. Prevengono crash quando si gestiscono dati del mondo reale.

### Takeaway

- **Navigazione sicura (`?.`)**: Previene crash su valori null - restituisce null invece di lanciare eccezione
- **Operatore Elvis (`?:`)**: Fornisce valori predefiniti - `value ?: 'default'`
- **Combinazione**: `value?.method() ?: 'default'` è il pattern comune

Questi operatori rendono i workflow resilienti a dati incompleti - essenziale per il lavoro nel mondo reale.

---

## 7. Validazione con `error()` e `log.warn`

A volte è necessario fermare immediatamente il workflow se i parametri di input non sono validi. In Nextflow, potete usare funzioni integrate come `error()` e `log.warn`, così come costrutti di programmazione standard come istruzioni `if` e logica booleana, per implementare logica di validazione. Aggiungiamo validazione al nostro workflow.
...
Potete anche aggiungere validazione all'interno della funzione `separateMetadata`. Usiamo il non-fatale `log.warn` per emettere avvisi per campioni con bassa profondità di sequenziamento, ma permettendo comunque al workflow di continuare:
...
Eseguite nuovamente il workflow con il CSV originale:
...
- **`error()`**: Ferma immediatamente il workflow con messaggio chiaro
...
- **Funzioni di validazione**: Crea logica di validazione riutilizzabile che può essere chiamata all'avvio del workflow

Una validazione appropriata rende i workflow più robusti e user-friendly catturando i problemi presto con messaggi di errore chiari.

---

## 8. Handler di Eventi del Workflow

Fino ad ora, abbiamo scritto codice nei nostri script di workflow e definizioni di processo. Ma c'è un'altra caratteristica importante che dovreste conoscere: gli handler di eventi del workflow.

Gli handler di eventi sono closure che vengono eseguite in punti specifici del ciclo di vita del vostro workflow. Sono perfetti per aggiungere logging, notifiche o operazioni di pulizia. Questi handler dovrebbero essere definiti nel vostro script di workflow insieme alla vostra definizione di workflow.

### 8.1. L'Handler `onComplete`

L'handler di eventi più comunemente usato è `onComplete`, che viene eseguito quando il vostro workflow termina (sia che abbia avuto successo o fallito). Aggiungiamone uno per riassumere i risultati della nostra pipeline.
...
Questa closure viene eseguita quando il workflow si completa. All'interno, avete accesso all'oggetto `workflow` che fornisce proprietà utili sull'esecuzione.

Eseguite il vostro workflow e vedrete questo riepilogo apparire alla fine!
...
Potete anche scrivere il riepilogo in un file usando operazioni su file:
...
Oltre a `onComplete`, c'è un altro handler di eventi che potete usare: `onError`, che viene eseguito solo se il workflow fallisce:
...
Potete usare più handler insieme nel vostro script di workflow:
...
- **Closure handler di eventi**: Closure nel vostro script di workflow che vengono eseguite in diversi punti del ciclo di vita
...
Gli handler di eventi mostrano come potete usare la piena potenza del linguaggio Nextflow all'interno dei vostri script di workflow per aggiungere capacità sofisticate di logging e notifica.
...
In questa missione secondaria, avete costruito una pipeline completa di elaborazione campioni che si è evoluta dalla gestione base dei metadati a un workflow sofisticato e pronto per la produzione.
Ogni sezione si è basata sulla precedente, dimostrando come i costrutti di programmazione trasformano semplici workflow in potenti sistemi di elaborazione dati, con i seguenti benefici:

- **Codice più chiaro**: Comprendere dataflow vs scripting aiuta a scrivere workflow più organizzati
- **Gestione robusta**: Navigazione sicura e operatori Elvis rendono i workflow resilienti a dati mancanti
- **Elaborazione flessibile**: La logica condizionale permette ai vostri workflow di elaborare appropriatamente diversi tipi di campioni
- **Risorse adattive**: Le direttive dinamiche ottimizzano l'uso delle risorse in base alle caratteristiche dell'input

Questa progressione rispecchia l'evoluzione reale delle pipeline bioinformatiche, da prototipi di ricerca che gestiscono pochi campioni a sistemi di produzione che elaborano migliaia di campioni attraverso laboratori e istituzioni.
Ogni sfida che avete risolto e pattern che avete imparato riflette problemi reali che gli sviluppatori affrontano quando scalano i workflow Nextflow.

Applicare questi pattern nel vostro lavoro vi permetterà di costruire workflow robusti e pronti per la produzione.

### Pattern chiave

1.  **Dataflow vs Scripting:** Avete imparato a distinguere tra operazioni di dataflow (orchestrazione di canali) e scripting (codice che manipola dati), incluse le differenze cruciali tra operazioni su diversi tipi come `collect` su Channel vs List.
...
2.  **Elaborazione Avanzata delle Stringhe**: Avete padroneggiato le espressioni regolari per analizzare nomi di file, generazione dinamica di script nei processi e interpolazione di variabili (Nextflow vs Bash vs Shell).
...
3.  **Creare Funzioni Riutilizzabili**: Avete imparato a estrarre logica complessa in funzioni nominate che possono essere chiamate dagli operatori di canale, rendendo i workflow più leggibili e manutenibili.

    - Definire una funzione nominata
    ...
    - Chiamare la funzione nominata in un workflow
...
4.  **Direttive di Risorse Dinamiche con Closure**: Avete esplorato l'uso di closure nelle direttive di processo per allocazione adattiva delle risorse basata sulle caratteristiche dell'input.

    - Closure nominate e composizione
...
5.  **Logica Condizionale e Controllo dei Processi**: Avete aggiunto routing intelligente usando operatori `.branch()` e `.filter()`, sfruttando la truthiness per espressioni condizionali concise.

    - Usare `.branch()` per instradare dati attraverso diversi rami del workflow
...
7.  **Validazione con error() e log.warn**: Avete imparato a validare gli input presto e fallire velocemente con messaggi di errore chiari.
...
8.  **Handler di Eventi di Configurazione**: Avete imparato a usare handler di eventi del workflow (`onComplete` e `onError`) per logging, notifiche e gestione del ciclo di vita.
...
Assicuratevi di consultare queste risorse quando avete bisogno di esplorare caratteristiche più avanzate.

Trarrete beneficio dalla pratica e dall'espansione delle vostre competenze per:

- Scrivere workflow più puliti con appropriata separazione tra dataflow e scripting
- Padroneggiare l'interpolazione di variabili per evitare insidie comuni con variabili Nextflow, Bash e shell
- Usare direttive di risorse dinamiche per workflow efficienti e adattivi
- Trasformare collezioni di file in argomenti di riga di comando formattati correttamente
- Gestire con eleganza diverse convenzioni di denominazione dei file e formati di input usando regex ed elaborazione delle stringhe
...
- Aggiungere validazione, gestione degli errori e logging per rendere i vostri workflow pronti per la produzione
- Implementare gestione del ciclo di vita del workflow con handler di eventi
