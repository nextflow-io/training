...
!!! warning "Avviso"
    ...
    Se questo è il vostro caso, potrebbe essere necessario utilizzare un account personale indipendente, oppure optare per un'installazione locale.
...
Una volta effettuato l'accesso a GitHub, aprite questo link nel browser per aprire l'ambiente di formazione Nextflow: <https://codespaces.new/nextflow-io/training?quickstart=1&ref=master>
...
??? info "Opzioni di configurazione"
    ...
    Questo permette di personalizzare la macchina virtuale che utilizzerete per lavorare attraverso la formazione.

    Utilizzare una macchina con più core permette di sfruttare maggiormente la capacità di Nextflow di parallelizzare l'esecuzione del flusso di lavoro.
...
Una volta caricato il vostro GitHub Codespaces, dovreste vedere qualcosa di simile al seguente (che potrebbe aprirsi in modalità chiara a seconda delle preferenze del vostro account):
...
- **L'editor principale** è dove il codice Nextflow e altri file di testo verranno aperti. Qui è dove modificherete il codice. Quando aprite il codespace, questo mostrerà un'anteprima del file `README.md`.
- **Il terminale** sotto l'editor principale permette di eseguire comandi. Qui è dove eseguirete tutte le righe di comando fornite nelle istruzioni del corso.
- **La barra laterale** permette di personalizzare l'ambiente ed eseguire attività di base (copia, incolla, apertura file, ricerca, git, ecc.). Per impostazione predefinita è aperta sull'esplora file, che permette di navigare nei contenuti del repository. Cliccando su un file nell'esplora lo aprirete nella finestra dell'editor principale.

È possibile regolare le proporzioni relative dei riquadri della finestra come preferite.
...
Una volta creato un ambiente, è possibile riprenderlo o riavviarlo facilmente e continuare da dove vi eravate lasciati.
...
Dovreste vedere la sessione precedente, l'opzione predefinita è di riprenderla:
...
È possibile crearli con più risorse (vedere la spiegazione sopra), ma questo consumerà il vostro utilizzo gratuito più velocemente e avrete meno ore di accesso a questo spazio.
Per esempio, se selezionate una macchina a 4 core invece della predefinita a 2 core, la vostra quota si esaurirà in metà tempo.
