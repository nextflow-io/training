...
Así que puedo pegarla aquí, _"$\{greeting\}\_output.txt"_, y luego también necesito pegarla aquí arriba porque ya no estamos creando un archivo llamado _output.txt_. Así que si no actualizo esto, Nextflow fallará con un error diciendo que esperaba un archivo, que nunca se generó.
...
Así que voy a agregar de nuevo un comando view aquí antes de ejecutar _flatten_. Y aquí este tiene estas llaves onduladas, que es el closure dinámico. Y solo hay algo de código arbitrario dentro de aquí que será ejecutado, dentro del contexto de un operador view.
...
Así que lo que esto hace, porque esto se ejecuta en secuencia, vas a ver cómo se ve el canal antes de ejecutar _flatten_, y luego de nuevo después de ejecutar _flatten_.
...
Y el proceso se ejecuta sobre cada elemento en el canal, que son tres. Y ejecuta el proceso tres veces, dándole tres tareas. Y esos resultados luego son publicados desde el workflow, recogidos por la salida del proceso. Publicados desde un workflow y guardados en el bloque output en un subdirectorio llamado _"hello_channels"_.
