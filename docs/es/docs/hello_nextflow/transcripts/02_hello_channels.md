...
Una cosa que es diferente es que la ruta en el bloque output aquí ahora es _hello_channels_ para esta parte, lo que significa que los archivos de resultados se almacenarán en un subdirectorio diferente en tus resultados si aún lo tienes ahí. Así que debería ser un lugar agradable y limpio para comenzar sin confundirse con las salidas.
...
Channel es como el espacio de nombres para todo lo relacionado con canales. "c" minúscula si has estado usando Nextflow antes. Y el _".of"_ es algo llamado una fábrica de canales (Channel factory), que es básicamente una forma de crear un canal.
...
Así que puedo hacer algunos paréntesis y voy a decir _"Hello Channels!"_.
...
Ten en cuenta que he codificado esta cadena por ahora. Esto es un poco un paso atrás después de nuestro bonito parámetro que usamos al final del último capítulo, pero solo mantiene las cosas simples para empezar para que puedas ver la lógica.
...
Esto es análogo al comando _print_ en Python u otros lenguajes que podrías estar acostumbrado a usar, y simplemente vuelca el contenido de este canal a la terminal cuando lo ejecutamos.
...
Efectivamente, puedes ver que se imprimió en la terminal aquí. _"Hello Channels!"_.
...
## 2.1.2. Ejecutar el comando y observar la salida del log
...
Así que si ejecuto el comando de nuevo, pero esta vez digo _"-ansi-log false"_ para apagarlo, también podría usar las variables de entorno _$NO_COLOR_ o _"$NXF_ANSI_LOG=false"_. Entonces usa el estilo más antiguo de registro de Nextflow sin ninguno de estos códigos de escape. Solo imprime directamente a una terminal sin actualizaciones inteligentes.

Y ahora podemos ver los tres procesos que se ejecutaron. Y cada uno de ellos su propio hash de tarea. Y si vamos a estos directorios work, veremos los tres saludos diferentes que especificamos.
...
## 2.2. Asegurar que los nombres de archivos de salida sean únicos

Ahora para entender esto, necesitamos volver a nuestro script de workflow. Estamos generando nuestro canal aquí, lo estamos pasando a nuestro proceso, y si miramos el proceso, estamos escribiendo el saludo a un archivo llamado _"output.txt"_ y pasando ese archivo de salida de vuelta al bloque output aquí abajo, publicándolo.

Sin embargo, cada tres veces que este proceso se ejecuta estas tres tareas diferentes. Todas generan un archivo llamado _"output.txt"_, todos esos archivos de salida se publican en el directorio results, y todos se sobrescriben entre sí. Así que cualquier archivo de resultado que obtengas allí es solo el último que se generó, pero aplastó todos los demás. Eso no es realmente lo que queremos.
...
También vale la pena señalar que lo que hemos hecho aquí es un poco poco práctico en situaciones de la vida real. Hemos tomado algunos datos de entrada y estamos usando esos datos, pero también estamos nombrando el archivo según esos datos, lo cual normalmente no puedes hacer.
...
¿Puedo usar un array en un canal? Intentémoslo. Voy a crear un array, y he copiado esto de la documentación, _"greetings_array"_ y _"Hello", "Bonjour"_ y _"Holà"_. Y luego voy a poner eso aquí en lugar de mis cadenas codificadas. Así que voy a decir "channel.of" _"greetings_array"_, pasando este array a un canal. Intentémoslo.
...
Así que desglosemos esto sección por sección. Dice que el error fue causado por, y luego la razón del error, que son archivos de salida faltantes. Así que básicamente ese bloque output dijo que este archivo debería crearse y no lo fue. A continuación dice que este es el comando que se ejecutó. Así que esto es básicamente el contenido de ese archivo _.command.sh_. Así es como se veía después de que todas esas variables se pusieran.

Y puedes ver aquí que nuestro comando echo en realidad solo se ha ejecutado una vez y ha usado todo el array, pero en una representación de cadena, que no es realmente lo que queríamos.
...
Aquí, esto está diciendo toma el greeting, que es la entrada del operador view, y eso está aquí. Podría llamar a esto como quisiera, podría llamar a esto _"foo"_ y solo necesito referirme a él como _"foo"_ más tarde. Y luego digo con esto, devuelve esto.
...
Y luego este canal greeting todavía se crea, así que todavía se va a pasar al proceso. Y con suerte ahora el workflow se ejecutará. Probémoslo.
...
Así que vamos a deshacernos de nuestro greetings away. Y ahora vamos a cambiar esta fábrica de canales de nuevo. Acabo de decir que había un montón para elegir y hay uno llamado _".fromPath"_. Y voy a decirle que, en este caso, tome _params.input_, que está volviendo a nuestra entrada que estábamos usando antes.
...
Como probablemente puedas decir por el mensaje de log, queremos usar el _splitCsv_, que es otro operador, otro operador de canal. Así que si hago "_dot" "s"_, y luego puedes ver que se sugiere automáticamente. Ups, _splitCsv_ y algunos paréntesis.
...
Bien, entonces la última vez que tuvimos este problema, usamos _flatten_. Hagámoslo muy rápidamente. Probemos flatten y veamos qué pasa.
...
Efectivamente, después de _splitCsv_ tenemos nuestros arrays, y luego después del _map_, tenemos nuestras cadenas limpias y agradables, solo _"Hello", "Bonjour"_ y _"Holà"_. Y el pipeline ahora está haciendo lo que queremos. Fantástico.
...
Bien. Con suerte ahora estás teniendo una idea de qué son los canales y operadores de Nextflow y cómo los operadores trabajan en canales y cómo puedes crearlos.
