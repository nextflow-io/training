...
Esta sección explorará cómo configurar y gestionar la configuración de su pipeline de Nextflow para que pueda personalizar su comportamiento, adaptarlo a diferentes entornos y optimizar el uso de recursos _sin alterar una sola línea del código del workflow en sí_.
...
Repasaremos componentes esenciales de la configuración de Nextflow como directivas de proceso, ejecutores, perfiles y archivos de parámetros.
Al aprender a utilizar estas opciones de configuración de manera efectiva, puede mejorar la flexibilidad, escalabilidad y rendimiento de sus pipelines.

??? info "Cómo comenzar desde esta sección"

    Esta sección del curso asume que ha completado las Partes 1-5 del curso [Hello Nextflow](./index.md) y tiene un pipeline completo y funcional.
    ...
    Si no está familiarizado con el pipeline Hello o necesita un recordatorio, consulte [esta página de información](../info/hello_pipeline.md).

---

## 0. Calentamiento: Ejecute `hello-config.nf`
...
## 1. Gestione los parámetros de entrada del workflow
...
Actualmente, nuestro workflow está configurado para aceptar varios valores de parámetros a través de la línea de comandos, con valores predeterminados establecidos en un bloque `params` en el propio script del workflow.
Sin embargo, es posible que desee anular esos valores predeterminados sin tener que especificar parámetros en la línea de comandos o modificar el archivo de script original.

Hay múltiples formas de hacer eso; vamos a mostrarle tres formas básicas que se usan muy comúnmente.

### 1.1. Mueva los valores predeterminados a `nextflow.config`

Este es el enfoque más simple, aunque posiblemente el menos flexible ya que el archivo principal `nextflow.config` no es algo que desee editar para cada ejecución.
...
#### 1.1.1. Cree un bloque `params` en el archivo de configuración

Realice los siguientes cambios de código en el archivo `nextflow.config`:
...
Tenga en cuenta que no simplemente copiamos el bloque `params` del workflow al archivo de configuración.
La sintaxis es un poco diferente.
En el archivo de workflow, esas son declaraciones tipadas.
En la configuración, esas son asignaciones de valores.

Técnicamente, esto es suficiente para anular los valores predeterminados aún especificados en el archivo de workflow.
Podría modificar el personaje, por ejemplo, y ejecutar el workflow para satisfacerse de que el valor establecido en el archivo de configuración anula el establecido en el archivo de workflow.

Pero en el espíritu de mover la configuración completamente al archivo de configuración, eliminemos esos valores del archivo de workflow por completo.

#### 1.1.2. Elimine los valores del bloque `params` en el archivo de workflow

Realice los siguientes cambios de código en el archivo de workflow `hello-config.nf`:
...
Ahora el archivo de workflow en sí no establece ningún valor predeterminado para estos parámetros.

#### 1.1.3. Ejecute el pipeline
...
### 1.2. Use un archivo de configuración específico para la ejecución

Eso es genial, pero a veces es posible que desee ejecutar algunos experimentos temporales con diferentes valores predeterminados sin modificar el archivo de configuración principal.
...
#### 1.2.1. Cree el directorio de trabajo con una configuración en blanco
...
#### 1.2.2. Configure la configuración experimental

Ahora abra el nuevo archivo y agregue los parámetros que desea personalizar:
...
Tenga en cuenta que la ruta al archivo de entrada debe reflejar la estructura de directorios.

#### 1.2.3. Ejecute el pipeline
...
En esta ejecución, Nextflow combina el `nextflow.config` en nuestro directorio actual con el `nextflow.config` en el directorio raíz del pipeline, y por lo tanto anula el personaje predeterminado (pavo) con el personaje tux.

El archivo de salida final debe contener el personaje tux diciendo los saludos.
...
### 1.3. Use un archivo de parámetros
...
Hay un enfoque más simple para cuando desea ejecutar su pipeline con un conjunto específico de valores, o permitir que otra persona lo haga con un esfuerzo mínimo.

Nextflow nos permite especificar parámetros a través de un [archivo de parámetros](https://nextflow.io/docs/latest/config.html#params-file) en formato YAML o JSON, lo que hace que sea muy conveniente gestionar y distribuir conjuntos alternativos de valores predeterminados, por ejemplo, así como valores de parámetros específicos de ejecución.

#### 1.3.1. Examine el archivo de parámetros de ejemplo
...
Tenga en cuenta el uso de dos puntos (`:`) en lugar de signos de igual (`=`) si compara la sintaxis con el archivo de configuración.
...
El archivo de salida final debe contener el personaje stegosaurus diciendo los saludos.
...
También facilita la distribución de conjuntos de parámetros a colaboradores, o como información de apoyo para una publicación, por ejemplo.
Esto hace que su trabajo sea más reproducible por otros.

### Conclusión

Sabe cómo aprovechar las opciones de configuración clave para gestionar las entradas del workflow.
...
## 2. Gestione las salidas del workflow

Hasta ahora hemos estado codificando todas las rutas para las declaraciones de salida a nivel de workflow, y como notamos cuando comenzamos a agregar múltiples salidas, puede haber un poco de repetición involucrada.

Veamos algunas formas comunes en las que podría configurar esto para que sea más flexible.

### 2.1. Personalice el directorio de salida con `-output-dir`

Cuando controlamos cómo se organizan nuestras salidas 'publicadas' tenemos dos prioridades distintas:
...
#### 2.1.1. Ejecute el pipeline con `-output-dir`

La opción `-output-dir` (abreviatura: `-o`) anula el directorio de salida predeterminado (`results/`) para todas las salidas del workflow.
...
Note que todavía tenemos el subdirectorio `hello_config` de las declaraciones `path` en el bloque de salida.
Limpiemos eso.

#### 2.1.2. Elimine las rutas codificadas del bloque de salida

El prefijo `hello_config/` fue codificado en capítulos anteriores, pero como ahora estamos aprendiendo a configurar rutas de salida de manera flexible, podemos eliminar esta codificación.
Para salidas que no necesitan un subdirectorio podemos establecer la directiva `path` en una cadena vacía, o eliminarla por completo.

Realice los siguientes cambios de código en el archivo de workflow:
...
!!! tip
    ...
    La opción `-output-dir` se usa para controlar _dónde_ van las salidas, mientras que la directiva `path` en el bloque de salida controla la _estructura de subdirectorios_.

### 2.2. Rutas de salida dinámicas

Además de cambiar el directorio de salida a través de la CLI, también podemos establecer un valor predeterminado personalizado en el archivo de configuración usando `outputDir`.
Esto nos permite establecer la ruta del directorio dinámicamente - no solo usando cadenas estáticas.

#### 2.2.1. Establezca `outputDir` en el archivo de configuración
...
Esto establece el directorio de salida en `custom-outdir-config/` más el valor del parámetro `batch` como subdirectorio.
...
!!! note
    ...
    Si está establecida, la opción de configuración será ignorada por completo.

#### 2.2.2. Subdirectorios con nombres de batch y proceso

También podemos establecer declaraciones de `path` de salida de subdirectorio dinámicamente, por salida.

Por ejemplo, podemos organizar nuestras salidas por proceso haciendo referencia a `<process>.name` en la declaración de ruta de salida:
...
En la edición anterior borramos la distinción entre `intermediates` versus salidas finales en el nivel superior.
Pongamos eso de vuelta, y también pongamos los archivos en un subdirectorio `params.batch`.

!!! tip

    Incluir `params.batch` en el `path` del bloque de salida, en lugar del `outputDir` de configuración, significa que no será sobrescrito con `-output-dir` en la CLI.
...
Luego, realice los siguientes cambios en el archivo de workflow:
...
#### 2.2.3. Ejecute el pipeline

Veamos cómo funciona eso en la práctica, estableciendo tanto `-output-dir` (o `-o` para abreviar) en `custom-outdir-config-2` como el nombre del batch en `rep2` desde la línea de comandos:
...
Esto publica las salidas en `custom-outdir-config-2/rep2/`, con la ruta base especificada _y_ el subdirectorio del nombre del batch _y_ resultados agrupados por proceso:
...
### 2.3. Establezca el modo de publicación a nivel de workflow
...
#### 2.3.1. Agregue `workflow.output.mode` al archivo de configuración
...
Establecer `workflow.output.mode` en el archivo de configuración es suficiente para anular lo que está establecido en el archivo de workflow, pero eliminemos el código innecesario de todos modos.

#### 2.3.2. Elimine el modo de salida del archivo de workflow

Realice los siguientes cambios en el archivo de workflow:
...
Esto publica las salidas en `config-output-mode/`, y todas siguen siendo copias adecuadas, no enlaces simbólicos.
...
La razón principal por la que aún podría querer usar la forma por salida de establecer el modo es si desea mezclar y combinar dentro del mismo workflow, _es decir_, tener algunas salidas copiadas y algunas con enlaces simbólicos.

Hay muchas otras opciones que puede personalizar de esta manera, pero con suerte esto le da una idea del rango de opciones y cómo utilizarlas de manera efectiva para adaptarse a sus preferencias.

### Conclusión

Sabe cómo controlar el nombre y la estructura de los directorios donde se publican sus salidas, así como el modo de publicación de salida del workflow.
...
## 3. Seleccione una tecnología de empaquetado de software

Hasta ahora hemos estado viendo elementos de configuración que controlan cómo entran las entradas y dónde salen las salidas. Ahora es momento de enfocarnos más específicamente en adaptar la configuración de su workflow a su entorno de cómputo.
...
### 3.1. Deshabilite Docker y habilite Conda en el archivo de configuración

Supongamos que estamos trabajando en un clúster HPC y el administrador no permite el uso de Docker por razones de seguridad.
Afortunadamente para nosotros, Nextflow admite múltiples otras tecnologías de contenedores como Singularity (que se usa más ampliamente en HPC), y gestores de paquetes de software como Conda.
...
Para hacerlo, cambiemos el valor de `docker.enabled` a `false`, y agreguemos una directiva que habilite el uso de Conda:
...
### 3.2. Especifique un paquete Conda en la definición del proceso
...
### 3.3. Ejecute el workflow para verificar que puede usar Conda
...
Detrás de escena, Nextflow ha recuperado los paquetes Conda y creado el entorno, lo que normalmente requiere un poco de trabajo; ¡así que es bueno que no tengamos que hacer nada de eso nosotros mismos!

!!! note

    Esto se ejecuta rápidamente porque el paquete `cowpy` es bastante pequeño, pero si está trabajando con paquetes grandes, puede tomar un poco más de tiempo de lo habitual la primera vez, y podría ver que la salida de la consola se queda 'atascada' durante un minuto más o menos antes de completarse.
    Esto es normal y se debe al trabajo adicional que Nextflow hace la primera vez que usa un nuevo paquete.
...
??? info "Mezclar y combinar Docker y Conda"
    ...
    Dado que estas directivas se asignan por proceso, es posible 'mezclar y combinar', _es decir_, configurar algunos de los procesos en su workflow para ejecutarse con Docker y otros con Conda, por ejemplo, si la infraestructura de cómputo que está usando admite ambos.
    ...
    Y como se señaló anteriormente, Nextflow admite múltiples otras tecnologías de empaquetado de software y contenedores, por lo que no está limitado solo a esas dos.

### Conclusión

Sabe cómo configurar qué paquete de software debe usar cada proceso, y cómo cambiar entre tecnologías.

### ¿Qué sigue?

Aprenda cómo cambiar la plataforma de ejecución utilizada por Nextflow para hacer realmente el trabajo.

---

## 4. Seleccione una plataforma de ejecución
...
Cuando Nextflow comienza, examina las CPUs y la memoria disponibles.
...
Nextflow admite [muchos ejecutores diferentes](https://nextflow.io/docs/latest/executor.html), incluidos programadores HPC (Slurm, LSF, SGE, PBS, Moab, OAR, Bridge, HTCondor y otros) así como backends de ejecución en la nube (AWS Batch, Google Cloud Batch, Azure Batch, Kubernetes y más).
...
Para establecer el ejecutor para apuntar a un backend diferente, simplemente especificaría el ejecutor que desea usando una sintaxis similar a la descrita anteriormente para asignaciones de recursos (consulte la [documentación del ejecutor](https://nextflow.io/docs/latest/executor.html) para todas las opciones).
...
### 4.2. Manejo de sintaxis específica del backend para parámetros de ejecución
...
Desafortunadamente, cada uno de estos sistemas usa diferentes tecnologías, sintaxis y configuraciones para definir cómo se debe definir y enviar un trabajo al programador relevante.
...
??? abstract "Ejemplos"
    ...
    ```bash title="Configuración para SLURM / enviar usando sbatch"
    ...
    ```bash title="Configuración para PBS / enviar usando qsub"
    ...
    ```bash title="Configuración para SGE / enviar usando qsub"
...
Proporciona una sintaxis estandarizada para que pueda especificar las propiedades relevantes como [`cpus`](https://nextflow.io/docs/latest/reference/process.html#cpus), [`memory`](https://nextflow.io/docs/latest/reference/process.html#memory) y [`queue`](https://nextflow.io/docs/latest/reference/process.html#queue) (consulte [directivas de proceso](https://nextflow.io/docs/latest/reference/process.html#process-directives) para otras propiedades) solo una vez.
...
Ahora sabe cómo cambiar el ejecutor para usar diferentes tipos de infraestructura de cómputo.
...
## 5. Controle las asignaciones de recursos de cómputo
...
Por defecto, Nextflow usará una sola CPU y 2GB de memoria para cada proceso.
...
Puede modificar estos valores, ya sea para todos los procesos o para procesos específicos nombrados, usando directivas de proceso adicionales en su archivo de configuración.
Nextflow los traducirá en las instrucciones apropiadas para el ejecutor elegido.

Pero, ¿cómo sabe qué valores usar?

### 5.1. Ejecute el workflow para generar un informe de utilización de recursos

Si no sabe de antemano cuánta CPU y memoria es probable que necesiten sus procesos, puede hacer un perfilado de recursos, lo que significa que ejecuta el workflow con algunas asignaciones predeterminadas, registra cuánto usó cada proceso y, a partir de ahí, estima cómo ajustar las asignaciones base.

Convenientemente, Nextflow incluye herramientas integradas para hacer esto, y generará con gusto un informe para usted a pedido.
...
El informe es un archivo html, que puede descargar y abrir en su navegador. También puede hacer clic derecho en él en el explorador de archivos de la izquierda y hacer clic en `Show preview` para verlo en el entorno de capacitación.

Tómese unos minutos para revisar el informe y ver si puede identificar algunas oportunidades para ajustar recursos.
Asegúrese de hacer clic en las pestañas que muestran los resultados de utilización como un porcentaje de lo que se asignó.

Consulte [Informes](https://nextflow.io/docs/latest/reports.html) para obtener documentación sobre todas las características disponibles.

### 5.2. Establezca asignaciones de recursos para todos los procesos

El perfilado muestra que los procesos en nuestro workflow de capacitación son muy livianos, así que reduzcamos la asignación de memoria predeterminada a 1GB por proceso.
...
### 5.3. Establezca asignaciones de recursos para un proceso específico
...
Con esta configuración, todos los procesos solicitarán 1GB de memoria y una sola CPU (el predeterminado implícito), excepto el proceso `cowpy`, que solicitará 2GB y 2 CPUs.

!!! tip

    Si tiene una máquina con pocas CPUs y asigna un número alto por proceso, podría ver llamadas de proceso poniéndose en cola una detrás de otra.
    Esto se debe a que Nextflow asegura que no solicitemos más CPUs de las que están disponibles.

### 5.4. Ejecute el workflow con la configuración actualizada
...
Es muy útil cuando sus procesos tienen diferentes requisitos de recursos. Le permite dimensionar correctamente las asignaciones de recursos que configura para cada proceso basándose en datos reales, no en conjeturas.
...
!!! tip
    ...
    Además, la Plataforma Seqera ofrece herramientas impulsadas por IA para optimizar sus asignaciones de recursos automáticamente también.

### 5.5. Agregue límites de recursos

Dependiendo de qué ejecutor de cómputo e infraestructura de cómputo esté usando, puede haber algunas restricciones sobre lo que puede (o debe) asignar.
Por ejemplo, su clúster puede requerir que se mantenga dentro de ciertos límites.

Puede usar la directiva `resourceLimits` para establecer las limitaciones relevantes. La sintaxis se ve así cuando está sola en un bloque de proceso:
...
Nextflow traducirá estos valores en las instrucciones apropiadas dependiendo del ejecutor que especificó.

No vamos a ejecutar esto, ya que no tenemos acceso a infraestructura relevante en el entorno de capacitación.
Sin embargo, si intentara ejecutar el workflow con asignaciones de recursos que excedan estos límites, luego buscar el comando `sbatch` en el archivo de script `.command.run`, vería que las solicitudes que realmente se envían al ejecutor están limitadas a los valores especificados por `resourceLimits`.
...
??? info "Configuraciones de referencia institucionales"
    ...
    Esas configuraciones compartidas son valiosas tanto para las personas que trabajan allí y por lo tanto pueden simplemente utilizar la configuración de su institución de forma inmediata, como modelo para las personas que buscan desarrollar una configuración para su propia infraestructura.

### Conclusión

Sabe cómo generar un informe de perfilado para evaluar la utilización de recursos y cómo modificar las asignaciones de recursos para todos los procesos y/o para procesos individuales, así como establecer limitaciones de recursos para ejecutar en HPC.
...
## 6. Use perfiles para cambiar entre configuraciones preestablecidas

Le hemos mostrado varias formas en las que puede personalizar la configuración de su pipeline dependiendo del proyecto en el que esté trabajando o del entorno de cómputo que esté usando.

Es posible que desee cambiar entre configuraciones alternativas dependiendo de qué infraestructura de cómputo esté usando. Por ejemplo, podría querer desarrollar y ejecutar pruebas a pequeña escala localmente en su computadora portátil, luego ejecutar cargas de trabajo a gran escala en HPC o en la nube.
...
### 6.1. Cree perfiles para cambiar entre desarrollo local y ejecución en HPC

Configuremos dos perfiles alternativos; uno para ejecutar cargas a pequeña escala en una computadora regular, donde usaremos contenedores Docker, y uno para ejecutar en un HPC universitario con un programador Slurm, donde usaremos paquetes Conda.

#### 6.1.1. Configure los perfiles
...
#### 6.1.2. Ejecute el workflow con un perfil
...
### 6.2. Cree un perfil de parámetros de prueba

Los perfiles no son solo para configuración de infraestructura.
También podemos usarlos para establecer valores predeterminados para parámetros de workflow, para facilitar que otros prueben el workflow sin tener que reunir valores de entrada apropiados ellos mismos.
Puede considerar esto como una alternativa al uso de un archivo de parámetros.

#### 6.2.1. Configure el perfil
...
#### 6.2.2. Ejecute el workflow localmente con el perfil de prueba

Convenientemente, los perfiles no son mutuamente excluyentes, por lo que podemos especificar múltiples perfiles en nuestra línea de comandos usando la siguiente sintaxis `-profile <profile1>,<profile2>` (para cualquier número de perfiles).

Si combina perfiles que establecen valores para los mismos elementos de configuración y se describen en el mismo archivo de configuración, Nextflow resolverá el conflicto usando el valor que leyó al final (_es decir_, lo que viene más tarde en el archivo).
Si las configuraciones en conflicto se establecen en diferentes fuentes de configuración, se aplica el [orden de precedencia](https://nextflow.io/docs/latest/config.html) predeterminado.
...
!!! tip
    ...
    Podemos apuntar a URLs para archivos más grandes que se almacenan externamente.
    Nextflow los descargará automáticamente siempre que haya una conexión abierta.

    Para más detalles, consulte la Misión Secundaria [Trabajando con Archivos](../side_quests/working_with_files.md)

### 6.3. Use `nextflow config` para ver la configuración resuelta

Como se señaló anteriormente, a veces el mismo parámetro puede establecerse en diferentes valores en perfiles que desea combinar.
Y más generalmente, hay numerosos lugares donde se pueden almacenar elementos de configuración, y a veces las mismas propiedades pueden establecerse en diferentes valores en diferentes lugares.

Nextflow aplica un [orden de precedencia](https://nextflow.io/docs/latest/config.html) establecido para resolver cualquier conflicto, pero eso puede ser difícil de determinar usted mismo.
...
La herramienta `config` explorará todos los contenidos en su directorio de trabajo actual, recogerá cualquier archivo de configuración y producirá la configuración completamente resuelta que Nextflow usaría para ejecutar el workflow.
Esto le permite averiguar qué configuraciones se usarán sin tener que lanzar nada.

#### 6.3.1. Resuelva la configuración predeterminada
...
#### 6.3.2. Resuelva la configuración con configuraciones específicas activadas

Si proporciona parámetros de línea de comandos, por ejemplo, habilitando uno o más perfiles o cargando un archivo de parámetros, el comando adicionalmente los tomará en cuenta.
...
Sabe cómo usar perfiles para seleccionar una configuración preestablecida en tiempo de ejecución con un mínimo de complicaciones.
...
¡Celebre y déle una gran palmada en la espalda! Ha completado su primer curso de desarrollador de Nextflow.
...
<quiz>
¿Qué tiene precedencia cuando el mismo parámetro se establece tanto en el archivo de configuración como en la línea de comandos?
- [ ] El valor del archivo de configuración
- [x] El valor de la línea de comandos
- [ ] El primer valor encontrado
- [ ] Ninguno; causa un error

Más información: [1.1. Mueva los valores predeterminados a `nextflow.config`](#11-mueva-los-valores-predeterminados-a-nextflowconfig)
</quiz>
...
Más información: [3. Seleccione una tecnología de empaquetado de software](#3-seleccione-una-tecnologia-de-empaquetado-de-software)
</quiz>
...
Más información: [5.3. Establezca asignaciones de recursos para un proceso específico](#53-establezca-asignaciones-de-recursos-para-un-proceso-especifico)
</quiz>

<quiz>
¿Qué opción de línea de comandos genera un informe de utilización de recursos?
- [ ] `-with-metrics`
- [ ] `-with-stats`
- [x] `-with-report`
- [ ] `-with-profile`

Más información: [5.1. Ejecute el workflow para generar un informe de utilización de recursos](#51-ejecute-el-workflow-para-generar-un-informe-de-utilizacion-de-recursos)
</quiz>
...
- [x] Limita los recursos máximos que se pueden solicitar
- [ ] Monitorea el uso de recursos

Más información: [5.5. Agregue límites de recursos](#55-agregue-limites-de-recursos)
</quiz>
...
Más información: [4. Seleccione una plataforma de ejecución](#4-seleccione-una-plataforma-de-ejecucion)
</quiz>
...
Más información: [1.3. Use un archivo de parámetros](#13-use-un-archivo-de-parametros)
</quiz>
...
Más información: [6. Use perfiles para cambiar entre configuraciones preestablecidas](#6-use-perfiles-para-cambiar-entre-configuraciones-preestablecidas)
</quiz>
