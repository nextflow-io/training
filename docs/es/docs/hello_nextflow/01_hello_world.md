...
En esta primera parte del curso de capacitación Hello Nextflow, comenzamos con un ejemplo básico de Hello World independiente del dominio, que desarrollaremos progresivamente para demostrar el uso de la lógica y los componentes fundamentales de Nextflow.

??? info "¿Qué es un ejemplo Hello World?"

    Un "Hello World!" es un ejemplo minimalista diseñado para demostrar la sintaxis y estructura básica de un lenguaje de programación o framework de software.
...
### 0.1. Hacer que la terminal diga hola
...
Esto imprime el texto 'Hello World' directamente en la terminal.

### 0.2. Escribir la salida en un archivo

La ejecución de pipelines implica principalmente leer datos de archivos y escribir resultados en otros archivos, así que modifiquemos el comando para escribir la salida de texto en un archivo para hacer el ejemplo un poco más relevante.
...
Esto no produce ninguna salida en la terminal.
...
Ahora sabe cómo ejecutar un comando simple en la terminal que produce texto, y opcionalmente, cómo hacer que escriba la salida en un archivo.
...
??? full-code "Archivo de código completo"
    ...
    /*
    * Usa echo para imprimir 'Hello World!' en un archivo
    */
    process sayHello {
...
Es importante destacar que la definición de output no _determina_ qué salida se creará.
Simplemente _declara_ cuál es la salida esperada, para que Nextflow pueda buscarla una vez que se complete la ejecución.
Esto es necesario para verificar que el comando se ejecutó correctamente y para pasar la salida a procesos posteriores si es necesario. La salida producida que no coincida con lo declarado en el bloque de output no se pasará a procesos posteriores.
...
#### 1.2.1. Lanzar el workflow y monitorear la ejecución
...
Mirando lo que obtuvimos para la ejecución mostrada arriba, la línea de log de la consola para el proceso sayHello comienza con `[65/7be2fa]`. Eso corresponde a la siguiente ruta de directorio: `work/65/7be2fad5e71e5f49998f795677fd68`
...
En este caso es muy directo porque el comando del proceso estaba codificado, pero más adelante en el curso verá comandos de proceso que involucran alguna interpolación de variables.
...
Sin embargo, eso hace que sea inconveniente recuperar las salidas que nos interesan.
...
Ahora ejecute el script del workflow modificado:
...
Tener una ubicación predeterminada es genial, pero es posible que desee personalizar dónde se guardan los resultados y cómo se organizan.
...
#### 2.2.2. Ejecutar el workflow nuevamente
...
También es posible usar el nombre del proceso u otras variables para nombrar los directorios utilizados para organizar los resultados, y es posible cambiar el nombre predeterminado del directorio de salida de nivel superior (que está controlado por el flag CLI `-o` o la variable de configuración `outputDir`).
...
Esto es genial cuando está tratando con archivos muy grandes, para los cuales no desea almacenar múltiples copias.
...
#### 2.3.1. Agregar la directiva mode
...
#### 2.3.2. Ejecutar el workflow nuevamente
...
Esta vez, si mira los resultados, el archivo es una copia adecuada en lugar de solo un enlace simbólico.
...
#### 3.1.1. Agregar un bloque input a la definición del proceso
...
!!! tip
    ...
    Las llaves (`{ }`) eran técnicamente opcionales en versiones anteriores de Nextflow, por lo que podría ver workflows más antiguos donde esto está escrito como `echo '$greeting' > output.txt`.
...
#### 3.4.2. Ejecutar el workflow nuevamente sin especificar el parámetro
...
## 4. Administrar ejecuciones de workflow

Saber cómo lanzar workflows y recuperar salidas es genial, pero rápidamente encontrará que hay algunos otros aspectos de la administración de workflows que harán su vida más fácil, especialmente si está desarrollando sus propios workflows.
...
### 4.1. Volver a lanzar un workflow con `-resume`
...
- Si está ejecutando un pipeline en producción y algo sale mal, en muchos casos puede corregir el problema y volver a lanzar el pipeline, y se reanudará desde el punto de falla, lo que puede ahorrarle mucho tiempo y cómputo.
...
Su salida tendrá diferentes nombres de directorios de tareas y puede tener un número diferente de líneas, pero debería verse similar al ejemplo.
...
Más información: [1.1.1. La definición del proceso](#111-la-definición-del-process)
</quiz>
...
Más información: [4.1. Volver a lanzar un workflow con `-resume`](#41-volver-a-lanzar-un-workflow-con--resume)
</quiz>
...
Más información: [2.3. Establecer el modo de publicación en copy](#23-establecer-el-modo-de-publicación-en-copy)
</quiz>
...
Más información: [3.2. Configurar un parámetro de línea de comandos para capturar la entrada del usuario](#32-configurar-un-parámetro-de-línea-de-comandos-para-capturar-la-entrada-del-usuario)
</quiz>
