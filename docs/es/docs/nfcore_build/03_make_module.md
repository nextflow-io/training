# Parte 3: Crear un módulo nf-core

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } Traducción asistida por IA - [más información y sugerencias](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>


En esta tercera parte del curso de capacitación Build with nf-core, le mostramos cómo crear un módulo nf-core aplicando las convenciones clave que hacen que los módulos sean portables y mantenibles.

El proyecto nf-core proporciona un comando (`nf-core modules create`) que genera plantillas de módulos estructuradas correctamente de forma automática, similar a lo que usamos para el workflow en la Parte 1.
...
??? info "Cómo comenzar desde esta sección"
    ...
    Esta sección asume que ha completado la [Parte 2: Usar un módulo nf-core](./02_use_module.md) e ha integrado el módulo `FIND_CONCATENATE` en su pipeline.
    ...
    Ejecute estos comandos desde dentro del directorio `nfcore-build/`:
    ...
    Esto le proporciona un pipeline con el módulo `FIND_CONCATENATE` ya integrado.
...
En la versión actual del pipeline `core-hello`, estamos extrayendo el archivo de la tupla de salida de `FIND_CONCATENATE` para pasarlo a `COWPY`, como se muestra en la mitad superior del diagrama a continuación.
...
Como puede ver, cambiamos tanto la **entrada principal** como la **salida** a una tupla que sigue el patrón `tuple val(meta), path(input_file)` introducido en la Parte 2 de este entrenamiento.
...
Ahora que la salida de `FIND_CONCATENATE` y la entrada de `COWPY` tienen la misma 'forma', es decir, ambas consisten en una estructura `tuple val(meta), path(input_file)`, simplemente podemos conectarlas directamente en lugar de tener que extraer el archivo explícitamente de la salida del proceso `FIND_CONCATENATE`.
...
Ahora llamamos a `COWPY` directamente en `FIND_CONCATENATE.out.file_out`.
...
El pipeline debería ejecutarse exitosamente, con los metadatos ahora fluyendo desde `FIND_CONCATENATE` a través de `COWPY`.
...
La sintaxis `withName:` asigna esta configuración solo al proceso `COWPY`, y `#!groovy ext.args = { "-c ${params.character}" }` simplemente compone una cadena que incluirá el valor del parámetro `character`.
...
1. **En el bloque `script:`, agregamos la línea `#!groovy prefix = task.ext.prefix ?: "${meta.id}"`.**
...
2. **En la línea de comandos, reemplazamos `#!groovy cowpy-${input_file}` con `#!groovy ${prefix}.txt`.**
   Aquí es donde Nextflow inyectará el valor de `prefix` determinado por la línea anterior.

3. **En el bloque `output:`, reemplazamos `#!groovy path("cowpy-${input_file}")` con `#!groovy path("${prefix}.txt")`.**
...
- **`core-hello-results`** — El directorio de salida establecido con `--outdir` en la línea de comandos, que ha estado recibiendo los registros de nf-core y los resultados publicados por `FIND_CONCATENATE`.
...
"Oye", podría decir, "`FIND_CONCATENATE` está enviando sus salidas a `--outdir`. ¿Quizás deberíamos copiar su directiva `publishDir`?"
...
!!! info "Info"
    ...
    La sección 1.6 explica cómo agregar eso.
...
- **`path:`** Determina el directorio de salida basado en el nombre del proceso.
  El nombre completo de un proceso contenido en `task.process` incluye la jerarquía de importaciones de workflow y módulo (como `CORE_HELLO:HELLO:FIND_CONCATENATE`).
  ...
  Esto es lo que determina que los resultados de `FIND_CONCATENATE` se publiquen en `#!groovy ${params.outdir}/find/`.
...
Dicho esto, puede decidir que desea organizar sus salidas de manera diferente, y la buena noticia es que es fácil hacerlo.
...
### 1.6. Captura de versiones con topic channels

La reproducibilidad es un principio fundamental de nf-core: cada ejecución del pipeline registra exactamente qué versiones de software se usaron, para que los resultados puedan reproducirse y compararse entre ejecuciones.
El reporte `hello_software_versions.yml` en `pipeline_info/` es la forma en que los pipelines nf-core cumplen este requisito.

El mecanismo detrás de esto son los **topic channels**: una característica de difusión de Nextflow donde cualquier proceso puede publicar datos en un topic con nombre, y cualquier suscriptor recibe todo lo publicado en ese topic, independientemente de dónde provenga en el pipeline.
nf-core está implementando esto en todos los pipelines en 2026.
Los módulos publican su versión como una tupla directamente en el bloque de salida usando la palabra clave `topic:`, y el workflow se suscribe una vez para recopilarlas todas.

#### 1.6.1. Agregar reporte de versión a `COWPY`

Abra el archivo del módulo `cowpy.nf` y agregue una línea de salida de versión como se muestra a continuación.
...
La nueva línea publica una tupla `[process_name, tool_name, version]` en el canal topic `"versions"`.
No se necesitan cambios en el bloque de script — la versión se declara estáticamente en el bloque de salida.
...
Abra `core-hello-results/pipeline_info/hello_software_versions.yml` y ahora verá ambos módulos:
...
La recopilación del lado del workflow — el bloque `channel.topic("versions")` que vio en el workflow de marcador de posición en la Parte 1 — se suscribe al topic y escribe este reporte combinado automáticamente.

!!! info "Compatibilidad con versiones anteriores"

    La rama `versions_file` en el bloque del topic channel del workflow existe para manejar módulos que aún no han sido actualizados para usar `topic: versions` y todavía escriben un archivo `versions.yml` en el bloque de script con `emit: versions`.
...
- Comprenda cómo los topic channels recopilan versiones de software de todos los módulos automáticamente.

Para lectura adicional sobre las convenciones de módulos, consulte las [especificaciones de módulos nf-core](https://nf-co.re/docs/guidelines/components/modules).
...
!!! tip "Aprenda más sobre pruebas"
    ...
    El archivo de prueba generado usa nf-test, un framework de pruebas para pipelines y módulos de Nextflow. Para aprender cómo escribir y ejecutar estas pruebas, consulte la [misión secundaria nf-test](../side_quests/nf_test/index.md).
...
- **Salida `topic: versions`**: Reporte de versiones mediante topic channel, como se explica en la sección 1.6
...
- **Bloque `script:`**: Vacío — agregaremos el comando `cowpy` aquí
- **Bloque `stub:`**: Lo actualizaremos para producir las salidas correctas
...
=== "Antes"
    ...
    channels:
      ...
      - bioconda
    dependencies:
      # TODO nf-core: Listar los paquetes Conda requeridos.
      #               El software DEBE estar fijado al canal (p. ej. "bioconda"), versión (p. ej. "1.10").
      #               Para Conda, la compilación (p. ej. "h9402c20_2") debe EXCLUIRSE para soportar instalación en diferentes sistemas operativos.
      - "YOUR-TOOL-HERE"
    ```
...
La plantilla genera un bloque de script vacío que debe completar con el comando real de la herramienta.
...
- Completar el comando real de `cowpy` que usa tanto `$args` como `#!groovy ${prefix}.txt`
...
- El stub crea un archivo vacío `#!groovy ${prefix}.txt` que coincide con lo que produce el bloque de script
...
Cuando esté listo, continúe con [Parte 4: Validación de entrada](./04_input_validation.md) para aprender cómo agregar validación de entrada basada en esquemas a su pipeline.
