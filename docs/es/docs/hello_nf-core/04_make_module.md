...
=== "Antes"
    ...
    ```groovy title="core-hello/modules/local/cowpy.nf (extracto)" linenums="1" hl_lines="4"
    // Generate ASCII art with cowpy (https://github.com/jeffbuttars/cowpy)
    process COWPY {
...
!!! note "Nota"
    ...
    Actualmente solo contiene información de versión de `CAT_CAT`, porque `COWPY` aún no reporta su versión.
...
- Completar el comando real de `cowpy` que usa tanto `$args` como `${prefix}.txt`
