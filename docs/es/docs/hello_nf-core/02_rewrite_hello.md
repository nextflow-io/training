...
En esta segunda parte del curso de capacitación Hello nf-core, te mostramos cómo crear una versión compatible con nf-core del pipeline producido por el curso para principiantes [Hello Nextflow](../hello_nextflow/index.md).

Habrás notado en la primera sección de la capacitación que los pipelines de nf-core siguen una estructura bastante elaborada con muchos archivos accesorios.
Crear todo eso desde cero sería muy tedioso, por lo que la comunidad nf-core ha desarrollado herramientas para hacerlo a partir de una plantilla, para iniciar el proceso.

Vamos a mostrarte cómo usar estas herramientas para crear una estructura de pipeline y luego adaptar el código de pipeline 'regular' existente a la estructura nf-core.
...
Primero, creamos la estructura para el nuevo pipeline.
...
Esto creará una nueva estructura de pipeline usando la plantilla base de nf-core, personalizada con un nombre de pipeline, descripción y autor.
...
Esta TUI te pedirá que proporciones información básica sobre tu pipeline y te ofrecerá una selección de características para incluir o excluir en la estructura de tu pipeline.
...
- En la pantalla Template features, establece `Toggle all features` en **off**, luego **habilita** selectivamente las siguientes. Verifica tus selecciones y haz clic en **Continue**.
...
- En la pantalla Create GitHub repository, haz clic en **Finish without creating a repo**. Esto mostrará instrucciones para crear un repositorio de GitHub más tarde. Ignóralas y haz clic en **Close**.

Una vez que se cierre la TUI, deberías ver la siguiente salida en la consola.
...
No hay una confirmación explícita en la salida de la consola de que la creación del pipeline funcionó, pero deberías ver un nuevo directorio llamado `core-hello`.
...
Pero no te preocupes si todavía te sientes un poco perdido; recorreremos juntos las partes importantes en el transcurso de esta capacitación.

!!! note "Nota"

    Una diferencia importante en comparación con el pipeline `nf-core/demo` que examinamos en la primera parte de esta capacitación es que no hay un directorio `modules`.
    Esto se debe a que no elegimos incluir ninguno de los módulos predeterminados de nf-core.

### 1.2. Probar que la estructura es funcional

Lo creas o no, aunque aún no has agregado ningún módulo para que haga trabajo real, la estructura del pipeline puede ejecutarse usando el perfil de prueba, de la misma manera que ejecutamos el pipeline `nf-core/demo`.
...
Esto te muestra que toda la conexión básica está en su lugar.
...
### 1.3. Examinar el workflow de marcador de posición

Si miras dentro del archivo `main.nf`, verás que importa un workflow llamado `HELLO` desde `workflows/hello`.

Esto es equivalente al workflow `workflows/demo.nf` que encontramos en la Parte 1, y sirve como un workflow de marcador de posición para nuestro workflow de interés, con alguna funcionalidad de nf-core ya en su lugar.
...
En comparación con un workflow básico de Nextflow como el desarrollado en [Hello Nextflow](../hello_nextflow/index.md), notarás algunas cosas que son nuevas aquí (líneas resaltadas arriba):
...
!!! note "Workflows componibles en profundidad"
    ...
    La [Misión Secundaria Workflows de Workflows](../side_quests/workflows_of_workflows.md) explora la composición de workflows con mucha mayor profundidad, incluyendo cómo componer múltiples workflows juntos y gestionar flujos de datos complejos entre ellos. Estamos introduciendo la componibilidad aquí porque es un requisito fundamental de la arquitectura de plantilla nf-core, que usa workflows anidados para organizar la inicialización del pipeline, el workflow de análisis principal y las tareas de finalización en componentes separados y reutilizables.
...
Ahora sabes cómo crear una estructura de pipeline usando las herramientas de nf-core.
...
## 2. Hacer el workflow original Hello Nextflow componible

Ahora es momento de ponerse a trabajar integrando nuestro workflow en la estructura nf-core.
Como recordatorio, estamos trabajando con el workflow presentado en nuestro curso de capacitación [Hello Nextflow](../hello_nextflow/index.md).
...
Siéntete libre de ejecutarlo para asegurarte de que funciona:
...
Primero, démosle un nombre al workflow para que podamos referirnos a él desde un workflow padre.
...
### 2.2. Reemplazar la construcción del canal con `take`
...
Mientras estamos en ello, también podemos comentar la línea `params.greeting = 'greetings.csv'`
...
!!! note "Nota"
    ...
    Si tienes instalada la extensión del servidor de lenguaje Nextflow, el verificador de sintaxis iluminará tu código con líneas onduladas rojas.
...
=== "Después"
    ...
    ```groovy title="original-hello/hello.nf" linenums="22" hl_lines="1"
        ...
        // genera arte ASCII de los saludos con cowpy
        cowpy(collectGreetings.out.outfile, params.character)
    ```
...
Esta es una adición completamente nueva al código en comparación con el workflow original.

### 2.5. Recapitulación de los cambios completados
...
Antes de integrar nuestro workflow componible en la compleja estructura nf-core, verifiquemos que funciona correctamente.
...
// importa el código del workflow desde el archivo hello.nf
include { HELLO } from './hello.nf'

// declara el parámetro de entrada
params.greeting = 'greetings.csv'
...
workflow {
  ...
  // llama al workflow importado en el canal de saludos
  HELLO(greeting_ch)

  // visualiza las salidas emitidas por el workflow
...
!!! note "Nota"
    ...
    Si sigues esta convención, puedes omitir especificar el nombre del archivo de workflow en tu comando `nextflow run`.
    ...
    En ese caso, asegúrate de especificar el nombre del archivo de workflow en tu comando `nextflow run`.
...
Aprende cómo injertar un workflow componible básico en la estructura nf-core.

---

## 3. Ajustar la lógica del workflow actualizado en el workflow de marcador de posición

Ahora que hemos verificado que nuestro workflow componible funciona correctamente, volvamos a la estructura del pipeline nf-core que creamos en la sección 1.
Queremos integrar el workflow componible que acabamos de desarrollar en la estructura de plantilla nf-core, por lo que el resultado final debería verse algo así.

<figure class="excalidraw">
--8<-- "docs/en/docs/hello_nf-core/img/core-hello.svg"
</figure>

Entonces, ¿cómo hacemos que eso suceda? Echemos un vistazo al contenido actual del workflow `HELLO` en `core-hello/workflows/hello.nf` (la estructura nf-core).
...
Necesitamos agregar el código relevante de la versión componible del workflow original que desarrollamos en la sección 2.
...
!!! note "Nota"
    ...
    Vamos a ignorar la captura de versión para este primer paso y veremos cómo conectar eso en una parte posterior de esta capacitación.

### 3.1. Copiar los módulos y configurar las importaciones de módulos

Los cuatro procesos de nuestro workflow Hello Nextflow se almacenan como módulos en `original-hello/modules/`.
...
El proyecto nf-core tiene mucha funcionalidad preconstruida alrededor del concepto de la hoja de muestras, que típicamente es un archivo CSV que contiene datos en columnas.
Dado que eso es esencialmente lo que es nuestro archivo `greetings.csv`, mantendremos la declaración `take` actual como está, y simplemente actualizaremos el nombre del canal de entrada en el siguiente paso.
...
El manejo de entrada se realizará antes de este workflow (no en este archivo de código).
...
Ya hay algo de código allí que tiene que ver con capturar las versiones de las herramientas que ejecuta el workflow. Vamos a dejar eso solo por ahora (nos ocuparemos de las versiones de herramientas más tarde).
...
Este orden tiene sentido porque en un pipeline real, los procesos emitirían información de versión que se agregaría al canal `ch_versions` a medida que se ejecuta el workflow.
...
Esto se ve genial, pero todavía necesitamos actualizar el nombre del canal que estamos pasando al proceso `sayHello()` de `greeting_ch` a `ch_samplesheet` como se muestra a continuación, para que coincida con lo que está escrito bajo la palabra clave `take:`.

=== "Después"

    ```groovy title="core-hello/workflows/hello.nf" linenums="26"
        // emite un saludo (actualizado para usar la convención nf-core para hojas de muestras)
...
Esto concluye las modificaciones que necesitamos hacer al workflow HELLO en sí.
En este punto, hemos logrado la estructura general de código que nos propusimos implementar.

### Conclusión

Sabes cómo ajustar las piezas centrales de un workflow componible en un workflow de marcador de posición nf-core.

### ¿Qué sigue?

Aprende cómo adaptar cómo se manejan las entradas en la estructura del pipeline nf-core.

---

## 4. Adaptar el manejo de entradas

Ahora que hemos integrado exitosamente nuestra lógica de workflow en la estructura nf-core, necesitamos abordar una pieza más crítica: asegurar que nuestros datos de entrada se procesen correctamente.
...
Puede que recuerdes que cuando reescribimos el workflow Hello Nextflow para que fuera componible, movimos la declaración del parámetro de entrada un nivel hacia arriba, en el workflow de punto de entrada `main.nf`.
Así que echemos un vistazo al workflow de punto de entrada `main.nf` de nivel superior que se creó como parte de la estructura del pipeline:
...
Importante, no podemos encontrar ningún código que construya un canal de entrada en este nivel, solo referencias a una hoja de muestras proporcionada a través del parámetro `--input`.

Un poco de investigación revela que el manejo de entrada lo realiza el subworkflow `PIPELINE_INITIALISATION`, apropiadamente, que se importa desde `core-hello/subworkflows/local/utils_nfcore_hello_pipeline/main.nf`.
...
Esta es la fábrica de canales que analiza la hoja de muestras y la pasa en una forma que está lista para ser consumida por el workflow HELLO.
...
Este código involucra algunos pasos de análisis y validación que son altamente específicos para la hoja de muestras de ejemplo incluida con la plantilla del pipeline nf-core, que al momento de escribir esto es muy específica del dominio y no es adecuada para nuestro proyecto de pipeline simple.

### 4.2. Reemplazar el código del canal de entrada de la plantilla

La buena noticia es que las necesidades de nuestro pipeline son mucho más simples, por lo que podemos reemplazar todo eso por el código de construcción de canal que desarrollamos en el workflow original Hello Nextflow.

Como recordatorio, así es como se veía la construcción del canal (como se ve en el directorio de soluciones):
...
Así que solo necesitamos conectar eso en el workflow de inicialización, con cambios menores: actualizamos el nombre del canal de `greeting_ch` a `ch_samplesheet`, y el nombre del parámetro de `params.greeting` a `params.input` (ver línea resaltada).
...
Eso completa los cambios que necesitamos para que funcione el procesamiento de entrada.

En su forma actual, esto no nos permitirá aprovechar las capacidades integradas de nf-core para la validación de esquemas, pero podemos agregar eso más tarde.
...
Hablando de datos de prueba y parámetros, actualicemos el perfil de prueba para este pipeline para usar la mini-hoja de muestras `greetings.csv` en lugar de la hoja de muestras de ejemplo proporcionada en la plantilla.

Bajo `core-hello/conf`, encontramos dos perfiles de prueba de plantilla: `test.config` y `test_full.config`, que están destinados a probar una pequeña muestra de datos y una de tamaño completo.
Dado el propósito de nuestro pipeline, realmente no tiene sentido configurar un perfil de prueba de tamaño completo, así que siéntete libre de ignorar o eliminar `test_full.config`.
Nos vamos a enfocar en configurar `test.config` para ejecutarse en nuestro archivo `greetings.csv` con algunos parámetros predeterminados.
...
- **Rutas absolutas**: Al usar `${projectDir}`, creamos una ruta absoluta, lo cual es importante para los datos de prueba que se envían con el pipeline.
- **Ubicación de datos de prueba**: Los pipelines nf-core típicamente almacenan datos de prueba en el directorio `assets/` dentro del repositorio del pipeline para archivos de prueba pequeños, o hacen referencia a conjuntos de datos de prueba externos para archivos más grandes.

Y mientras estamos en ello, ajustemos los límites de recursos predeterminados para asegurar que esto se ejecute en máquinas muy básicas (como las VMs mínimas en Github Codespaces):
...
Ten en cuenta que tenemos que agregar `--validate_params false` a la línea de comando porque aún no configuramos la validación (eso vendrá más tarde).
...
Y la respuesta es bastante interesante: ahora hay dos lugares diferentes para buscar los resultados.
...
Ves que obtuvimos otro conjunto de reportes de ejecución además de los que obtuvimos de la primera ejecución, cuando el workflow todavía era solo un marcador de posición.
...
!!! note "Nota"
    ...
    Para ver que estas se ejecuten en paralelo, intenta aumentar la asignación de CPU de tu codespace y los límites de recursos en la configuración de prueba.

Eso es genial, ¡pero nuestros resultados reales del pipeline no están ahí!

Esto es lo que sucedió: no cambiamos nada en los módulos mismos, por lo que las salidas manejadas por las directivas `publishDir` a nivel de módulo todavía van a un directorio `results` como se especifica en el pipeline original.
...
Si queremos que estén organizados ordenadamente como lo estaban las salidas del pipeline demo, necesitaremos cambiar cómo configuramos las salidas para que se publiquen.
Te mostraremos cómo hacer eso más adelante en este curso de capacitación.

<!-- TODO: Update this once we've updated Hello Nextflow to use workflow-level outputs -->

¡Y ahí está! Puede parecer mucho trabajo para lograr el mismo resultado que el pipeline original, pero obtienes todos esos hermosos reportes generados automáticamente, y ahora tienes una base sólida para aprovechar características adicionales de nf-core, incluyendo validación de entrada y algunas capacidades ordenadas de manejo de metadatos que cubriremos en una sección posterior.
...
Como parte de eso, aprendiste cómo hacer un workflow componible, y cómo identificar los elementos de la plantilla nf-core que más comúnmente necesitan ser adaptados al desarrollar un pipeline personalizado de estilo nf-core.

### ¿Qué sigue?

¡Toma un descanso, eso fue trabajo duro! Cuando estés listo, continúa con [Parte 3: Usar un módulo nf-core](./03_use_module.md) para aprender cómo aprovechar los módulos mantenidos por la comunidad del repositorio nf-core/modules.
