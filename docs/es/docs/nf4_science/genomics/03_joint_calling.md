...
Ahora vamos a extenderlo para implementar el llamado de variantes conjunto, como se cubrió en la [Parte 1](01_method.md).
...
??? info "Cómo comenzar desde esta sección"
    ...
    Puedes probar que se ejecuta correctamente ejecutando el siguiente comando:
...
2. **Agregar un paso de genotipado conjunto que combine y genotipe los GVCFs por muestra.**
   Esto introduce el operador `collect()`, closures de Groovy para la construcción de líneas de comando y procesos con múltiples comandos.
...
Aprende a recopilar el contenido de un canal y pasarlo al siguiente proceso como una sola entrada.

---

## 2. Agregar un paso de genotipado conjunto

Ahora necesitamos recopilar los GVCFs por muestra, combinarlos en un almacén de datos GenomicsDB y ejecutar el genotipado conjunto para producir un VCF a nivel de cohorte.
...
El proceso de genotipado conjunto necesita dos tipos de entradas que aún no tenemos: un nombre de cohorte arbitrario y las salidas GVCF recopiladas de todas las muestras agrupadas juntas.
...
#### 2.1.2. Recopilar las salidas de HaplotypeCaller a través de las muestras
...
2. Debido a que nombramos las salidas usando `emit:` en la sección 1, podemos seleccionar los GVCFs con `.vcf` y los archivos de índice con `.idx`. Sin salidas nombradas, tendríamos que usar `.out[0]` y `.out[1]`.
3. El operador `collect()` agrupa todos los archivos en un solo elemento, por lo que `all_gvcfs_ch` contiene los tres GVCFs juntos, y `all_idxs_ch` contiene los tres archivos de índice juntos.

Podemos recopilar los GVCFs y sus archivos de índice por separado (en lugar de mantenerlos juntos en tuplas) porque Nextflow preparará todos los archivos de entrada juntos para la ejecución, por lo que los archivos de índice estarán presentes junto a los GVCFs.
...
Importante, necesitamos construir esta cadena dinámicamente a partir de cualquier archivo que esté en el canal recopilado.
Nextflow (a través de Groovy) proporciona una forma concisa de hacer esto:
...
1. `all_gvcfs.collect { gvcf -> "-V ${gvcf}" }` itera sobre cada ruta de archivo y antepone `-V ` a ella, produciendo `["-V A.g.vcf", "-V B.g.vcf", "-V C.g.vcf"]`.
2. `.join(' ')` las concatena con espacios: `"-V A.g.vcf -V B.g.vcf -V C.g.vcf"`.
...
Como anteriormente, se enumeran varias entradas aunque los comandos no las referencien directamente: `all_idxs`, `ref_index` y `ref_dict`.
Enumerarlas asegura que Nextflow prepare estos archivos en el directorio de trabajo junto a los archivos que sí aparecen en los comandos, que GATK espera encontrar según las convenciones de nomenclatura.

La variable `gvcfs_line` usa el closure de Groovy descrito anteriormente para construir los argumentos `-V` para GenomicsDBImport.
...
Para usarlo en el workflow, necesitarás importar el módulo y agregar una llamada de proceso.
...
Agrega destinos de publicación y entradas de bloque de salida para los resultados del genotipado conjunto.
...
Agrega entradas para los archivos VCF conjuntos.
...
Los primeros dos pasos están en caché de la ejecución anterior, y el nuevo paso `GATK_JOINTGENOTYPING` se ejecuta una vez con las entradas recopiladas de las tres muestras.
...
!!! note
    ...
    ¡Por eso usamos solo subconjuntos pequeños de datos para propósitos de capacitación!

### Conclusión

Sabes cómo recopilar salidas de un canal y agruparlas como una sola entrada para otro proceso.
También sabes cómo construir una línea de comando usando closures de Groovy, y cómo ejecutar múltiples comandos en un solo proceso.

### ¿Qué sigue?

Celebra tu éxito y toma un merecido descanso.

En la siguiente parte de este curso, aprenderás cómo ejecutar un pipeline de llamado de variantes listo para producción de nf-core y compararlo con el pipeline que construiste manualmente.
