...
Aquí vamos a utilizar herramientas y métodos diseñados para llamar variantes germinales cortas, _es decir_, SNPs e indels, en datos de secuenciación de genoma completo.
...
Por simplicidad, en este curso nos vamos a enfocar solo en la parte del llamado de variantes.
...
Antes de sumergirnos en escribir cualquier código de workflow para cualquiera de los enfoques, vamos a probar los comandos manualmente en algunos datos de prueba.

### Conjunto de datos

Proporcionamos los siguientes datos y recursos relacionados:

- **Un genoma de referencia** que consiste en una pequeña región del cromosoma 20 humano (de hg19/b37) y sus archivos accesorios (índice y diccionario de secuencia).
...
1. Generar un archivo de índice para un archivo de entrada BAM usando [Samtools](https://www.htslib.org/)
...
Los archivos de índice son una característica común de los formatos de archivo de bioinformática; contienen información sobre la estructura del archivo principal que permite a herramientas como GATK acceder a un subconjunto de los datos sin tener que leer todo el archivo.
...
Vamos a descargar un contenedor de Samtools, iniciarlo interactivamente y ejecutar el comando `samtools index` en uno de los archivos BAM.
...
Si no has descargado esta imagen antes, puede tomar un minuto completarse.
Una vez que termine, tendrás una copia local de la imagen del contenedor.

#### 1.1.2. Iniciar el contenedor de Samtools interactivamente

Para ejecutar el contenedor interactivamente, usa `docker run` con las opciones `-it`.
La opción `-v ./data:/data` monta el directorio local `data` en el contenedor para que las herramientas puedan acceder a los archivos de entrada.
...
Ahora deberías ver un archivo llamado `reads_mother.bam.bai` en el mismo directorio que el archivo BAM de entrada original.
...
Tu prompt ahora debería estar de vuelta a lo que era antes de iniciar el contenedor.

### 1.2. Llamar variantes con GATK HaplotypeCaller

Vamos a descargar un contenedor de GATK, iniciarlo interactivamente y ejecutar el comando `gatk HaplotypeCaller` en el archivo BAM que acabamos de indexar.
...
??? success "Salida del comando"
    ...
    Algunas capas muestran `Already exists` porque se comparten con la imagen del contenedor de Samtools que descargamos anteriormente.
...
#### 1.2.2. Iniciar el contenedor de GATK interactivamente

Inicia el contenedor de GATK interactivamente con el directorio de datos montado, tal como lo hicimos para Samtools.
...
Sin embargo, es un archivo de prueba pequeño, así que puedes usar `cat` para abrirlo y ver el contenido.
...
El archivo de salida VCF está acompañado de un archivo de índice llamado `reads_mother.vcf.idx` que fue creado automáticamente por GATK.
...
Tu prompt debería estar de vuelta a la normalidad.
...
Eso está bien para observar variantes de cada muestra de forma aislada, pero proporciona información limitada.
...
El análisis de 'genotipado conjunto' propiamente dicho se realiza luego con GATK GenotypeGVCFs.
...
1. Generar un archivo de índice para cada archivo de entrada BAM usando Samtools
...
#### 2.1.1. Iniciar el contenedor de Samtools interactivamente
...
Esto debería producir los archivos de índice en el mismo directorio que los archivos BAM correspondientes.
...
#### 2.2.1. Iniciar el contenedor de GATK interactivamente
...
Técnicamente esto no es un requisito, pero es una convención fuertemente recomendada.
...
En un GVCF, típicamente hay muchas de estas líneas no variantes, con un número menor de registros de variantes esparcidos entre ellas.
...
La segunda línea muestra el primer registro de variante en el archivo, que corresponde a la primera variante en el archivo VCF que vimos anteriormente.

Al igual que el VCF original, el archivo de salida GVCF también está acompañado de un archivo de índice, llamado `reads_mother.g.vcf.idx`.
...
Una vez que esto se complete, deberías tener tres archivos que terminan en `.g.vcf` en tu directorio actual (uno por muestra) y sus respectivos archivos de índice que terminan en `.g.vcf.idx`.

¡Pero no salgas del contenedor!
Vamos a usar el mismo contenedor en el siguiente paso.

### 2.3. Ejecutar genotipado conjunto
...
Puedes explorar en él pero rápidamente verás que este formato de almacén de datos no está destinado a ser leído directamente por humanos.
...
Tu prompt debería estar de vuelta a la normalidad.
Eso concluye las pruebas manuales de los comandos de llamado de variantes.

---

### Conclusión

Sabes cómo probar los comandos de indexación de Samtools y llamado de variantes de GATK en sus respectivos contenedores, incluyendo cómo generar GVCFs y ejecutar genotipado conjunto en múltiples muestras.
