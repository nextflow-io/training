...
Ahora aprenderemos dos mejores enfoques para gestionar las entradas: **archivos de parámetros** y **hojas de muestras**.

## 1. Uso de archivos de parámetros

### 1.1. El problema con las líneas de comandos largas
...
¿Qué pasa si necesitas ejecutar el mismo análisis nuevamente el próximo mes?
¿Qué pasa si un colaborador quiere usar exactamente tu configuración?

### 1.2. Solución: Usar un archivo de parámetros

Crea un archivo llamado `params.yaml`:
...
Ahora tu comando se convierte en:

```bash
nextflow run ./molkart -params-file params.yaml -resume
```

¡Eso es todo! El archivo de parámetros documenta tu configuración exacta y facilita volver a ejecutar o compartir.

### 1.3. Sobrescritura de parámetros

Aún puedes sobrescribir parámetros específicos desde la línea de comandos:
...
Además, puedes ver que la bandera `-resume` nos permitió reutilizar los resultados de preprocesamiento de la ejecución anterior, ahorrando tiempo.
Puedes usar este patrón para probar rápidamente diferentes variaciones del pipeline.

### Conclusión

Los archivos de parámetros hacen que tus análisis sean reproducibles y fáciles de compartir.
Úsalos para cualquier trabajo de análisis real.

### ¿Qué sigue?

Aprende cómo las hojas de muestras organizan información sobre múltiples muestras.
...
Una hoja de muestras es un archivo CSV que describe tus muestras de entrada.
...
Puedes mezclar tipos de rutas en la misma hoja de muestras.

### 2.3. Creación de tu propia hoja de muestras
...
!!! Warning "Advertencia"
    ...
    Observa que las rutas en la hoja de muestras son relativas a donde **ejecutas** Nextflow, no a donde se encuentra la hoja de muestras.

Finalmente, ejecutemos nf-core/molkart una vez más con la hoja de muestras con rutas de archivos locales:

`nextflow run ./molkart -params-file params.yaml -resume`

Como puedes ver, Nextflow ejecuta esta ejecución de manera similar a cuando los archivos se descargaron desde Github. Esta es una de las grandes características de Nextflow, organiza los datos correctamente para ti, independientemente de dónde se encuentren.

### Conclusión

Las hojas de muestras organizan conjuntos de datos de múltiples muestras de una manera que te permite definir explícitamente tus metadatos junto con las rutas de archivos.
