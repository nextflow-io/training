...
Si mira en el directorio de resultados, debería ver las mismas salidas que anteriormente, _es decir_, archivos individuales que contienen el arte ASCII de cada saludo hablado por el personaje correspondiente.
...
Entonces esto produce los mismos resultados que antes con código más simple.

Por supuesto, esto asume que puede modificar el código del proceso.
En algunos casos, puede tener que depender de procesos existentes que no tiene libertad de modificar, lo que limita sus opciones.
La buena noticia, si planea usar módulos del proyecto [nf-core](https://nf-co.re/), es que los módulos de nf-core están todos configurados para usar la estructura de tupla `[meta, file]` como estándar.
...
Cuando Nextflow ejecuta la línea de comando `cowpy` para esa muestra, `${meta.character}` se llena con una cadena vacía en la línea de comando `cowpy`, por lo que la herramienta `cowpy` arroja un error diciendo que no se proporcionó ningún valor para el argumento `-c`.
...
Si estamos usando la versión en la sección 3.3, Nextflow pasará todo el mapa de metadatos al proceso `COWPY` e intentará ejecutar el comando.
...
Esto sucede porque `meta.character` no existe, por lo que nuestro intento de acceder a él devuelve `null`. Como resultado, Nextflow literalmente conecta `null` en la línea de comando, que por supuesto no es reconocido por la herramienta `cowpy`.
...
1. Implementar validación de entrada en su workflow para asegurar que la hoja de datos contenga toda la información requerida. Puede encontrar una [introducción a la validación de entrada](../hello_nf-core/05_input_validation.md) en el curso de capacitación Hello nf-core.
...
Primero, a nivel de proceso, actualice la definición de entrada de la siguiente manera:
...
Luego, a nivel de workflow, use una operación de mapeo para extraer la propiedad `character` de los metadatos y hacerla un componente explícito de la tupla de entrada:
...
**Use el mapa de metadatos para información opcional y descriptiva, pero extraiga valores requeridos como entradas explícitas.**

El mapa de metadatos es excelente para mantener las estructuras de canal limpias y prevenir estructuras de canal arbitrarias, pero para elementos obligatorios que se referencian directamente en un proceso, extraerlos como entradas explícitas crea código más robusto y mantenible.
...
Si desea practicar el uso de información del mapa de metadatos desde dentro de un proceso, intente usar otras piezas de información del mapa de metadatos como `lang` y `lang_group` para personalizar cómo se nombran y/u organizan las salidas.
...
- La información del archivo puede expandirse durante la ejecución del pipeline

Aplicar este patrón en su propio trabajo le permitirá construir workflows de bioinformática robustos y mantenibles.

### Patrones clave

1.  **Leer y Estructurar Metadatos:** Leer archivos CSV y crear mapas de metadatos organizados que permanezcan asociados con sus archivos de datos.
...
2.  **Expandir Metadatos Durante el Workflow** Agregar nueva información a sus metadatos a medida que su pipeline progresa agregando salidas de procesos y derivando valores a través de lógica condicional.
