...
Obtendrás/construirás una imagen de contenedor para el paquete pip `quote`, que se utilizará en el ejercicio al final de esta sección.
...
# Comienza desde la imagen base de docker micromamba
FROM mambaorg/micromamba:1.5.10-noble
# Copia el archivo conda.yml dentro del contenedor
COPY --chown=$MAMBA_USER:$MAMBA_USER conda.yml /tmp/conda.yml
# Instala varias utilidades para que Nextflow las use y los paquetes en el archivo conda.yml
...
# Ejecuta el contenedor como el usuario root
USER root
# Establece la variable de entorno PATH para incluir el directorio de instalación de micromamba
...
## 2. Haz que la vaca cite científicos famosos

Esta sección contiene algunos ejercicios adicionales, para practicar lo que has aprendido hasta ahora.
Hacer estos ejercicios _no es obligatorio_ para entender las partes posteriores de la capacitación, pero proporcionan una forma divertida de reforzar tus aprendizajes al descubrir cómo hacer que la vaca cite científicos famosos.
...
### 2.1. Modifica el script `hello-containers.nf` para usar un proceso getQuote
...
### 2.2. Modifica tu pipeline de Nextflow para permitir que se ejecute en los modos `quote` y `sayHello`.
...
Cuando estés listo, continúa a la Parte 3 de esta serie de capacitación para aprender cómo aplicar lo que has aprendido hasta ahora a un caso de uso de análisis de datos más realista.
