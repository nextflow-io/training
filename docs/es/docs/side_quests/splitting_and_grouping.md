...
Nextflow proporciona herramientas poderosas para trabajar con datos de manera flexible. Una capacidad clave es dividir datos en diferentes flujos y luego agrupar elementos relacionados nuevamente. Esto es especialmente valioso en flujos de trabajo de bioinformática donde necesitas procesar diferentes tipos de muestras por separado antes de combinar resultados para el análisis.

Piénsalo como clasificar correo: separas las cartas por destino, procesas cada pila de manera diferente y luego recombinas los elementos que van a la misma persona. Nextflow usa operadores especiales para lograr esto con datos científicos. Este enfoque también se conoce comúnmente como el patrón **scatter/gather** en computación distribuida y flujos de trabajo de bioinformática.

El sistema de canales de Nextflow está en el corazón de esta flexibilidad. Los canales conectan diferentes partes de tu flujo de trabajo, permitiendo que los datos fluyan a través de tu análisis. Puedes crear múltiples canales desde una única fuente de datos, procesar cada canal de manera diferente y luego fusionar canales nuevamente cuando sea necesario. Este enfoque te permite diseñar flujos de trabajo que reflejan naturalmente las rutas de ramificación y convergencia de análisis bioinformáticos complejos.

### Objetivos de aprendizaje

En esta misión secundaria, aprenderás a dividir y agrupar datos usando los operadores de canal de Nextflow.
...
- Construir funciones reutilizables con closures nombrados para ayudarte a manipular estructuras de canales

Estas habilidades te ayudarán a construir flujos de trabajo que puedan manejar múltiples archivos de entrada y diferentes tipos de datos de manera eficiente, mientras mantienes una estructura de código limpia y mantenible.
...
Esa cubre los fundamentos de leer archivos CSV con `splitCsv` y crear mapas de metadatos, que usaremos intensivamente aquí.
...
Encontrarás un archivo de flujo de trabajo principal y un directorio `data` que contiene una hoja de muestras llamada `samplesheet.csv`.
...
La hoja de muestras contiene información sobre muestras de diferentes pacientes, incluyendo el ID del paciente, número de repetición de muestra, tipo (normal o tumor) y rutas a archivos de datos hipotéticos (que en realidad no existen, pero pretenderemos que sí).
...
Si no estás familiarizado con el análisis de cáncer, solo debes saber que esto corresponde a un modelo experimental que usa muestras pareadas tumor/normal para realizar análisis contrastivos.

Para el paciente A específicamente, tenemos dos conjuntos de réplicas técnicas (repeticiones).

!!! note

    No te preocupes si no estás familiarizado con este diseño experimental, no es crítico para entender este tutorial.

#### Revisar la asignación

Tu desafío es escribir un flujo de trabajo de Nextflow que:
...
3. **Una** pares coincidentes tumor/normal por ID de paciente y número de réplica
4. **Distribuya** muestras a través de intervalos genómicos para procesamiento paralelo
5. **Agrupe** muestras relacionadas nuevamente para análisis posteriores

Esto representa un patrón común de bioinformática donde necesitas dividir datos para procesamiento independiente, luego recombinar elementos relacionados para análisis comparativo.
...
Comencemos leyendo los datos de muestras con `splitCsv` y organizándolos en el patrón de mapa de metadatos. En `main.nf`, verás que ya hemos comenzado el flujo de trabajo.
...
Si completaste la misión secundaria [Metadatos en flujos de trabajo](./metadata.md), reconocerás este patrón. Usaremos `splitCsv` para leer el CSV y estructurar inmediatamente los datos con un mapa de metadatos para separar metadatos de rutas de archivos.
...
!!! info
    ...
    Aclararemos cuál queremos decir en contexto, pero esta distinción es importante para entender cuando trabajas con Nextflow.
...
Esto combina la operación `splitCsv` (leer el CSV con encabezados) y la operación `map` (estructurar datos como tuplas `[meta, file]`) en un solo paso. Aplica ese cambio y ejecuta el pipeline:
...
Ahora tenemos un canal donde cada elemento es una tupla `[meta, file]` - metadatos separados de rutas de archivos. Esta estructura nos permite dividir y agrupar nuestra carga de trabajo según campos de metadatos.
...
Ejecuta el flujo de trabajo nuevamente para ver el resultado filtrado:
...
En nuestro caso, queremos mantener solo muestras donde `meta.type == 'normal'`. El closure usa la tupla `meta,file` para referirse a cada muestra, accede al tipo de muestra con `meta.type` y verifica si es igual a `'normal'`.

Esto se logra con el único closure que introdujimos arriba:
...
En la sección anterior, separamos las muestras normales y de tumor en dos canales diferentes. Estas podrían procesarse independientemente usando procesos o flujos de trabajo específicos según su tipo. Pero ¿qué sucede cuando queremos comparar las muestras normales y de tumor del mismo paciente? En este punto, necesitamos unirlas nuevamente asegurándonos de emparejar las muestras según su campo `id`.
...
Si no tienes la salida de consola aún disponible, ejecutemos el pipeline para verificar nuestra estructura de datos y ver cómo necesitamos modificarla para unir en el campo `id`.
...
Podemos ver que el campo `id` es el primer elemento en cada mapa de metadatos. Para que `join` funcione, deberíamos aislar el campo `id` en cada tupla. Después de eso, podemos simplemente usar el operador `join` para combinar los dos canales.
...
Es un poco difícil de ver porque es muy ancho, pero deberías poder ver que las muestras se han unido por el campo `id`. Cada tupla ahora tiene el formato:
...
!!! warning
    ...
    El operador `join` descartará cualquier tupla no emparejada. En este ejemplo, nos aseguramos de que todas las muestras estuvieran emparejadas para tumor y normal, pero si esto no es cierto debes usar el parámetro `remainder: true` para mantener las tuplas no emparejadas. Consulta la [documentación](https://www.nextflow.io/docs/latest/operator.html#join) para más detalles.

Ahora sabes cómo usar `map` para aislar un campo en una tupla, y cómo usar `join` para combinar tuplas según el primer campo.
Con este conocimiento, podemos combinar exitosamente canales según un campo compartido.

A continuación, consideraremos la situación donde quieres unir en múltiples campos.

### 3.2. Unir en múltiples campos

Tenemos 2 réplicas para sampleA, pero solo 1 para sampleB y sampleC. En este caso pudimos unirlas efectivamente usando el campo `id`, pero ¿qué pasaría si estuvieran desincronizadas? ¡Podríamos mezclar las muestras normales y de tumor de diferentes réplicas!

Para evitar esto, podemos unir en múltiples campos. En realidad hay múltiples formas de lograr esto, pero nos vamos a enfocar en crear una nueva clave de unión que incluya tanto el `id` de la muestra como el número de `replicate`.
...
Ahora deberíamos ver que la unión está ocurriendo pero usando tanto los campos `id` como `repeat`. Ejecuta el flujo de trabajo:
...
Nota cómo tenemos una tupla de dos elementos (campos `id` y `repeat`) como primer elemento de cada resultado unido. Esto demuestra cómo elementos complejos pueden usarse como clave de unión, permitiendo emparejamientos bastante intrincados entre muestras de las mismas condiciones.

Si quieres explorar más formas de unir en diferentes claves, consulta la [documentación del operador join](https://www.nextflow.io/docs/latest/operator.html#join) para opciones y ejemplos adicionales.
...
Ahora tenemos una nueva clave de unión que no solo incluye los campos `id` y `repeat` sino que también retiene los nombres de campo para que podamos acceder a ellos más tarde por nombre, por ejemplo `meta.id` y `meta.repeat`.
...
Nota que también convertimos la ruta del archivo a un objeto Path usando `file()` para que cualquier proceso que reciba este canal pueda manejar el archivo correctamente (para más información consulta [Trabajar con archivos](./working_with_files.md)).
...
!!! note
    ...
    El operador `map` ha cambiado de usar `{ }` a usar `( )` para pasar el closure como argumento. Esto es porque el operador `map` espera un closure como argumento y `{ }` se usa para definir un closure anónimo. Al llamar un closure nombrado, usa la sintaxis `( )`.

Ejecuta el flujo de trabajo una vez más para verificar que todo sigue funcionando:
...
Ahora el closure devuelve una tupla donde el primer elemento contiene los campos `id` y `repeat`, y el segundo elemento contiene solo el campo `type`. Esto elimina redundancia almacenando la información de `id` y `repeat` una vez en la clave de agrupación, mientras mantiene toda la información necesaria.

Ejecuta el flujo de trabajo para ver cómo se ve esto:
...
Podemos ver que solo declaramos los campos `id` y `repeat` una vez en la clave de agrupación y tenemos el campo `type` en los datos de muestra. No hemos perdido ninguna información pero logramos hacer el contenido de nuestro canal más conciso.

### 3.6. Eliminar información redundante

Eliminamos información duplicada arriba, pero todavía tenemos otra información redundante en nuestros canales.
...
Dado que conocemos la posición de cada elemento en nuestro canal, podemos simplificar la estructura aún más eliminando los metadatos `[type:normal]` y `[type:tumor]`.
...
- **Optimización de Estructura de Datos**: Cómo optimizar la estructura del canal eliminando información redundante

Ahora tienes un flujo de trabajo que puede dividir una hoja de muestras, filtrar las muestras normales y de tumor, unirlas por ID de muestra y número de réplica, luego imprimir los resultados.

Este es un patrón común en flujos de trabajo de bioinformática donde necesitas emparejar muestras u otros tipos de datos después de procesarlos independientemente, por lo que es una habilidad útil. A continuación, veremos cómo repetir una muestra múltiples veces.

## 4. Distribuir muestras en intervalos

Un patrón clave en flujos de trabajo de bioinformática es distribuir análisis a través de regiones genómicas. Por ejemplo, el llamado de variantes puede paralelizarse dividiendo el genoma en intervalos (como cromosomas o regiones más pequeñas). Esta estrategia de paralelización mejora significativamente la eficiencia del pipeline al distribuir la carga computacional a través de múltiples núcleos o nodos, reduciendo el tiempo total de ejecución.

En la siguiente sección, demostraremos cómo distribuir nuestros datos de muestras a través de múltiples intervalos genómicos. Emparejaremos cada muestra con cada intervalo, permitiendo procesamiento paralelo de diferentes regiones genómicas. Esto multiplicará el tamaño de nuestro conjunto de datos por el número de intervalos, creando múltiples unidades de análisis independientes que pueden reunirse más tarde.

### 4.1. Distribuir muestras sobre intervalos usando `combine`

Comencemos creando un canal de intervalos. Para mantener la vida simple, solo usaremos 3 intervalos que definiremos manualmente. En un flujo de trabajo real, podrías leerlos desde un archivo de entrada o incluso crear un canal con muchos archivos de intervalos.
...
Podemos usar el operador `map` para ordenar y refactorizar nuestros datos de muestra para que sean más fáciles de entender. Movamos la cadena de intervalos al map de agrupación en el primer elemento.
...
A continuación, combinamos el `grouping_key` con el campo `interval`. El `grouping_key` es un map que contiene los campos `id` y `repeat`. Creamos un nuevo map con el `interval` y los fusionamos usando la adición de maps de Groovy (`+`):
...
Usar `map` para coaccionar tus datos a la estructura correcta puede ser complicado, pero es crucial para la manipulación efectiva de datos.
...
Para hacer esto, deberíamos separar nuestras variables de agrupación para que podamos usarlas en aislamiento.

El primer paso es similar a lo que hicimos en la sección anterior. Debemos aislar nuestra variable de agrupación como el primer elemento de la tupla. Recuerda, nuestro primer elemento es actualmente un map de campos `id`, `repeat` e `interval`:
...
!!! note
    ...
    Estamos descartando el campo `replicate` aquí. Esto es porque no lo necesitamos para procesamiento posterior. Después de completar este tutorial, ¡ve si puedes incluirlo sin afectar la agrupación posterior!
...
¡Eso es todo! Solo agregamos una sola línea de código. Veamos qué sucede cuando lo ejecutamos:
...
Nota que nuestros datos han cambiado de estructura y dentro de cada elemento del canal los archivos ahora están contenidos en tuplas como `[patientA_rep1_normal.bam, patientA_rep2_normal.bam]`. Esto es porque cuando usamos `groupTuple`, Nextflow combina los archivos individuales para cada muestra de un grupo. Esto es importante recordar al intentar manejar los datos posteriormente.

!!! note

    [`transpose`](https://www.nextflow.io/docs/latest/reference/operator.html#transpose) es lo opuesto a groupTuple. Desempaqueta los elementos en un canal y los aplana. ¡Intenta agregar `transpose` y deshacer la agrupación que realizamos arriba!
...
- **Aislar claves de agrupación**: Cómo usar `subMap` para extraer campos específicos para agrupación
...
Al modificar los datos a medida que fluyen a través del pipeline, puedes construir un pipeline escalable sin usar bucles o declaraciones while, ofreciendo varias ventajas sobre enfoques más tradicionales:
...
- El pipeline se vuelve más declarativo, enfocándose en qué debería suceder en lugar de cómo debería suceder
- Nextflow optimizará la ejecución para nosotros ejecutando operaciones independientes en paralelo

Dominar estas operaciones de canal te permitirá construir pipelines flexibles y escalables que manejen relaciones de datos complejas sin recurrir a bucles o programación iterativa, permitiendo que Nextflow optimice la ejecución y paralelice operaciones independientes automáticamente.

### Patrones clave

1.  **Crear datos de entrada estructurados:** Comenzando desde un archivo CSV con mapas de metadatos (basándose en patrones de [Metadatos en flujos de trabajo](./metadata.md))
...
2.  **Dividir datos en canales separados:** Usamos `filter` para dividir datos en flujos independientes según el campo `type`

    ```groovy
    channel.filter { it.type == 'tumor' }
    ```

3.  **Unir muestras coincidentes:** Usamos `join` para recombinar muestras relacionadas según los campos `id` y `repeat`

    - Unir dos canales por clave (primer elemento de tupla)
...
5.  **Agregar por claves de agrupación:** Usamos `groupTuple` para agrupar por el primer elemento en cada tupla, recolectando así muestras que comparten campos `id` e `interval` y fusionando réplicas técnicas.
