...
- Yazılım sürümü takibi için `versions` konu kanalına yayınlanan bir sürüm demeti.
...
Sürüm çıktısını şimdilik görmezden gelecek olursak, ana çıktıları da eşdeğerdir (birleştirilmiş bir dosya); ancak CAT_CAT ayrıca çıktı dosyasıyla birlikte metamap'i de yayınlar.
...
- `CAT_CAT` süreci tarafından üretilen demetten çıktı dosyasını `cowpy`'ye geçirmeden önce çıkarma.
...
- `#!groovy .map { files -> tuple(cat_meta, files) }`, `CAT_CAT`'in beklediği formatta `[metadata, dosyalar]` demeti oluşturur
...
`#!groovy .map { meta, file -> file }` işlemi, `CAT_CAT` tarafından üretilen `[metadata, dosya]` demetinden dosyayı yeni bir kanala, `ch_for_cowpy`'ye çıkarır.
