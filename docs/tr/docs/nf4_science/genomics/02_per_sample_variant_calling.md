...
4. **İş akışının toplu girdi dosyalarını içeren bir metin dosyasını kabul etmesini sağlayın.**
...
Şimdi, iş akışı bloğunda `SAMTOOLS_INDEX`'e bir çağrı ekleyelim ve girdi kanalını argüman olarak geçirelim.
...
!!! note
    ...
    Burada kullandığımız veri dosyaları çok küçük olsa da, genomik alanda çok büyük olabilirler.
    Sembolik bağlantılar, `work` dizininizi temizlediğinizde bozulacaktır, bu nedenle üretim iş akışları için varsayılan yayınlama modunu `'copy'` olarak geçersiz kılmak isteyebilirsiniz.
...
Yeni sürecimiz sağlanacak bir avuç ek dosya beklediğinden, `genomics.nf` dosyasında `Pipeline parameters` bölümü altında bunlar için parametre bildirimleri ekleyin:
...
Bu sürecin GATK komutunun gerektirdiğinden daha fazla girdiye sahip olduğunu fark edeceksiniz.
...
Çıktıları yayınlama bloğuna bağladığımızda yararlı olacak olan her çıktı kanalına bir ad atamak için `emit:` sözdizimini kullanıyoruz.
...
`genomics.nf` dosyasını yeni modülü içe aktaracak şekilde güncelleyin:
...
!!! note
    ...
    Girdilerin, sürecin girdi bloğunda listelendikleri sırayla süreç çağrısında sağlandığına dikkat edin.
...
Şimdi konsol çıktısına bakarsak, listelenen iki süreci görüyoruz.
...
Gerçek analiz işi yapan ve genomik dosya formatı tuhaflıklarıyla (yardımcı dosyalar gibi) başa çıkabilen iki adımlı modüler bir iş akışı yapmayı biliyorsunuz.
...
Hayır, şükürler olsun! Koda küçük bir ayarlama yapın ve Nextflow bunu da sizin için halledecektir.
...
Ardından test profilini üç örneği de listeleyecek şekilde güncelleyin:
...
Tesisat üç test örneğinin tümünde çalışacak şekilde ayarlandığına göre iş akışını çalıştırmayı deneyin.
...
Çalışma dizinlerini inceleyeceğiz ve neyin yanlış gittiğini anlamak için `view()` operatörünü kullanacağız.
...
Kanalın içeriğini görüntülemek için `GATK_HAPLOTYPECALLER` süreç çağrısından önce iş akışı gövdesine bu iki satırı ekleyin:
...
!!! note
    ...
    Sonuç olarak, karşılık gelen çıktılar orijinal girdilerin verildiği sıradan farklı bir sırada toplanabilir.
...
!!! tip
    ...
    Ancak konsol çıktınızı karıştıracaklar, bu nedenle sorunu gidermeyi bitirdiğinizde bunları kaldırmanızı öneririz.
...
!!! note
    ...
    Bir **demet**, bir fonksiyondan birden fazla değer döndürmek için yaygın olarak kullanılan sonlu, sıralı bir öğe listesidir. Demetler, ilişkilerini ve sıralarını korurken süreçler arasında birden fazla girdi veya çıktı geçirmek için özellikle yararlıdır.
...
Artık orijinal `reads_ch`'yi `GATK_HAPLOTYPECALLER` sürecine sağlamamıza gerek yok, çünkü BAM dosyası artık `SAMTOOLS_INDEX` tarafından çıktılanan kanala paketlenmiş durumda.
...
Bu sefer (ve her seferinde) her şey doğru çalışmalıdır:
...
Dosya listesi artık kodda hiç yaşamıyor, bu doğru yönde büyük bir adımdır.
...
Şimdi ona girdi dosya yollarını listeleyen bir dosya verdiğimizden, içerdiği dosya yollarını veri girdileri olarak ele alacak şekilde davranışını değiştirmemiz gerekiyor.

Bunu [Hello Nextflow'un Bölüm 2'sinde](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file) kullandığımız aynı kalıbı kullanarak yapabiliriz: dosyayı ayrıştırmak için [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) operatörünü uygulayarak, ardından her satırın ilk alanını seçmek için bir `map` işlemi.
...
Ancak, daha çok yönlü `splitCsv` operatörünü (`map` ile desteklenen) kullanarak, dosya yollarını içeren dosyaya meta veri eklemeye karar verirsek iş akışımızı geleceğe hazır hale getirebiliriz.
...
Evet! Aslında, Nextflow süreç çağrılarının tamamen aynı olduğunu doğru bir şekilde algılar ve `-resume` ile çalıştığımız için her şeyi yeniden çalıştırmaya bile zahmet etmez.
...
Daha genel olarak, genomik dosya formatlarının ve araç gereksinimlerinin tuhaflıklarını dikkate alarak gerçek iş yapan basit bir genomik boru hattı oluşturmak için temel Nextflow bileşenlerini ve mantığını nasıl kullanacağınızı öğrendiniz.
