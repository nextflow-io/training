...
Bu, [Bölüm 1: Yöntem genel bakışı](./01_method.md#1-per-sample-variant-calling)'nın ilk bölümündeki adımları otomatikleştirir; orada bu komutları konteynırlarında manuel olarak çalıştırmıştınız.
...
Her çıktı kanalına bir ad atamak için `emit:` sözdizimini kullanıyoruz; bu, çıktıları yayınlama bloğuna bağladığımızda yararlı olacaktır.
...
Satır başına bir dosya yolu listeleyen ve başka hiçbir şey içermeyen bir metin dosyası kadar basit olabilir ya da dosya ek meta veriler içerebilir; bu durumda genellikle örnek tablosu olarak adlandırılır.
...
Bunu [Hello Nextflow'un Bölüm 2](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file)'sinde kullandığımız aynı kalıbı kullanarak yapabiliriz: dosyayı ayrıştırmak için [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) operatörünü uygulayarak, ardından her satırdan dosya yolunu çıkarmak için bir `map` işlemi.
...
Hello Nextflow kursunda karşılaştığınıza kıyasla yeni olan bir şey, bu CSV'nin bir başlık satırına sahip olmasıdır; bu nedenle `splitCsv()` çağrısına `#!groovy header: true` ekliyoruz.
Bu, `map` işleminde sütunlara ada göre başvurmamızı sağlar: `#!groovy row.reads_bam`, her satırın `reads_bam` sütunundan dosya yolunu çıkarır.
...
Kendinizi tazelenmiş hissettiğinizde, [Bölüm 3](./03_joint_calling.md)'e geçin; orada bu basit örnek başına varyant çağırma iş akışını verilere ortak varyant çağırma uygulamak için nasıl dönüştüreceğinizi öğreneceksiniz.
