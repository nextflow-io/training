# Bölüm 3: Bir kohort üzerinde birleşik çağırma

Bölüm 2'de, her örneğin verisini bağımsız olarak işleyen örnek başına varyant çağırma boru hattı oluşturdunuz.
...
4. Kohort düzeyinde bir VCF üretmek için birleştirilmiş GVCF veri deposu üzerinde birleşik genotipleme çalıştırın

Bu bölüm doğrudan Bölüm 2 tarafından üretilen iş akışı üzerine inşa edilmiştir.
...
2. **Örnek başına GVCF'leri birleştiren ve genotipleyerek birleşik genotipleme adımı ekleyin.**
...
[Bölüm 1](01_method.md)'deki GVCF varyant çağırma komutunu hatırlayın:
...
- GATK kuralına göre karşılık gelen `.g.vcf` uzantısını kullanmak için çıktı dosya yolunu güncelleyin.
...
Ayrıca yeni dosya uzantısıyla eşleşmesi için çıktı bloğunu güncellememiz gerekiyor.
...
Ayrıca yeni GVCF çıktılarını yansıtmak için iş akışının yayınlama ve çıktı yapılandırmasını güncellememiz gerekiyor.
...
Şimdi eşleşmesi için çıktı bloğunu güncelleyin.
...
Şimdi örnek başına GVCF'leri toplamamız, bunları bir GenomicsDB veri deposunda birleştirmemiz ve kohort düzeyinde bir VCF üretmek için birleşik genotipleme çalıştırmamız gerekiyor.
...
Birleşik genotipleme süreci henüz sahip olmadığımız iki tür girdi gerektirir: rastgele bir kohort adı ve tüm örneklerden toplanan GVCF çıktılarının birlikte paketlenmesi.
...
GVCF'leri ve dizin dosyalarını ayrı ayrı toplayabiliriz (bunları demetler halinde birlikte tutmanın aksine) çünkü Nextflow tüm girdi dosyalarını yürütme için birlikte sahneler, dolayısıyla dizin dosyaları GVCF'lerin yanında mevcut olacaktır.
...
Bölüm 2'de kullandığımız aynı kalıbı izleyerek, süreç tanımını bir modül dosyasına yazacağız, iş akışına aktaracağız ve hazırladığımız girdiler üzerinde çağıracağız.

#### 2.2.1. Her GVCF'ye bir `-V` argümanı vermek için bir dize oluşturun
...
Ancak dizenin şöyle görünmesi gerekiyor:

```groovy
-V reads_mother.bam.g.vcf -V reads_father.bam.g.vcf -V reads_son.bam.g.vcf
```

Önemlisi, bu dizeyi toplanan kanaldaki dosyalar ne olursa olsun dinamik olarak oluşturmamız gerekiyor.
Nextflow (Groovy aracılığıyla) bunu yapmanın özlü bir yolunu sağlar:
...
3. Sonuç, komut şablonuna enterpolasyon yapabileceğimiz yerel bir `gvcfs_line` değişkenine (`def` ile tanımlanmış) atanır.
...
Ardından sürecin `script:` bloğunda bu dizenin tamamına `gvcfs_line` olarak başvurabileceksiniz.
...
=== "Önce"
    ...
    /*
     * GVCF'leri GenomicsDB veri deposunda birleştir ve kohort düzeyinde çağrılar üretmek için birleşik genotipleme çalıştır
     */
...
Birleşik genotipleme sonuçları için yayınlama hedefleri ve çıktı bloğu girdileri ekleyin.

#### 2.3.1. Birleşik VCF için yayınlama hedefleri ekleyin

İş akışının `publish:` bölümüne birleşik VCF'yi ve dizinini ekleyin:
...
#### 2.3.2. Birleşik VCF için çıktı bloğu girdileri ekleyin
...
İlk iki adım önceki çalıştırmadan önbelleğe alınmıştır ve yeni `GATK_JOINTGENOTYPING` adımı her üç örnekten toplanan girdiler üzerinde bir kez çalışır.
...
Artık otomatik, tamamen tekrarlanabilir bir birleşik varyant çağırma iş akışınız var!
...
!!! note
    ...
    Bu yüzden eğitim amaçları için yalnızca küçük veri alt kümelerini kullanıyoruz!

### Özet

Bir kanaldan çıktıları nasıl toplayacağınızı ve bunları başka bir sürece tek bir girdi olarak nasıl paketleyeceğinizi biliyorsunuz.
...
Başarınızı kutlayın ve hak ettiğiniz bir mola verin.

Kursun bir sonraki bölümünde, nf-core'dan üretime hazır bir varyant çağırma boru hattını nasıl çalıştıracağınızı öğrenecek ve bunu manuel olarak oluşturduğunuz boru hattıyla karşılaştıracaksınız.
