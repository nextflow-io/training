...
Germline SNP'leri ve indelleri tanımlamak için tüm genom dizileme örneklerine varyant çağırma uygulamanın iki yolunu göstereceğiz.
İlk olarak, her örnekten bağımsız olarak varyantları çağıran basit bir **örnek başına yaklaşım** ile başlayacağız.
...
- Bir aile üçlüsüne (anne, baba ve oğul) karşılık gelen **üç tüm genom dizileme örneği**, dosya boyutlarını küçük tutmak için kromozom 20'deki küçük bir veri dilimine indirilmiştir.
  Bu, zaten referans genoma haritalanmış Illumina kısa okuma dizileme verisidir ve [BAM](https://samtools.github.io/hts-specs/SAMv1.pdf) formatında (Binary Alignment Map, SAM'in sıkıştırılmış versiyonu, Sequence Alignment Map) sağlanmıştır.
...
Bunlar, bu kursun Bölüm 2'sinde bir Nextflow iş akışına sarmalayacağımız komutlardır.
...
Bir Samtools konteynerı çekeceğiz, etkileşimli olarak başlatacağız ve BAM dosyalarından biri üzerinde `samtools index` komutunu çalıştıracağız.
...
`-v ./data:/data` seçeneği, yerel `data` dizinini konteynera bağlar, böylece araçlar girdi dosyalarına erişebilir.

```bash
docker run -it -v ./data:/data community.wave.seqera.io/library/samtools:1.20--b5dfbd93de237464
```

İsteminiz `(base) root@a1b2c3d4e5f6:/tmp#` gibi bir şeye dönüşür ve artık konteyner içinde olduğunuzu gösterir.
Veri dosyalarına `/data` altından erişilebilir.

#### 1.1.3. İndeksleme komutunu çalıştırın

[Samtools belgeleri](https://www.htslib.org/doc/samtools-index.html) bize bir BAM dosyasını indekslemek için çalıştıracağımız komut satırını verir.
...
Bir GATK konteynerı çekeceğiz, etkileşimli olarak başlatacağız ve az önce indekslediğimiz BAM dosyası üzerinde `gatk HaplotypeCaller` komutunu çalıştıracağız.
...
Bu, ilk çekme işleminden daha hızlı olmalıdır çünkü iki konteyner imajı katmanlarının çoğunu paylaşır.
...
[GATK belgeleri](https://gatk.broadinstitute.org/hc/en-us/articles/21905025322523-HaplotypeCaller) bize bir BAM dosyası üzerinde varyant çağırma gerçekleştirmek için çalıştıracağımız komut satırını verir.
...
Çıktı dosyası `reads_mother.vcf`, konteynerdaki çalışma dizininizin içinde oluşturulur, bu nedenle çıktı dosya yolunu değiştirmediğiniz sürece VS Code dosya gezgininde görmezsiniz.
Ancak, küçük bir test dosyasıdır, bu nedenle içeriği açmak ve görüntülemek için `cat` kullanabilirsiniz.
Dosyanın başlangıcına kadar kaydırırsanız, birçok satır metaveri içeren bir başlık ve ardından satır başına bir varyant çağrısı listesi bulacaksınız.
...
Her satır, örneğin dizileme verilerinde tanımlanan olası bir varyantı açıklar. VCF formatını yorumlama konusunda rehberlik için [bu yararlı makaleye](https://www.ebi.ac.uk/training/online/courses/human-genetic-variation-introduction/variant-identification-and-analysis/understanding-vcf-format/) bakın.
...
Az önce kullandığımız varyant çağırma yaklaşımı, örnek başına varyant çağrıları oluşturur.
...
Bir örneğin GVCF'sinin özel yanı, sadece programın varyasyon kanıtı bulduğu konumları değil, genomun hedeflenen alanındaki tüm konumlar hakkında dizi veri istatistiklerini özetleyen kayıtlar içermesidir.
...
Bunlar, bu kursun Bölüm 3'ünde bir Nextflow iş akışına sarmalayacağımız komutlardır.
...
Şimdi tüm bu komutları test etmemiz gerekiyor, üç BAM dosyasının tümünü indekslemekle başlayarak.
...
İndeksleme komutunu üç BAM dosyasının her biri üzerinde çalıştırın:
...
Bu, konteynerdaki mevcut çalışma dizininde GVCF çıktı dosyası `reads_mother.g.vcf`'yi oluşturur.

İçeriği görüntülemek için `cat` kullanırsanız, bölüm 1'de oluşturduğumuz eşdeğer VCF'den çok daha uzun olduğunu göreceksiniz. Dosyanın başlangıcına bile kaydıramazsınız ve satırların çoğu VCF'de gördüklerimizden oldukça farklı görünür.
...
Bunlar, varyant çağırıcının varyasyon kanıtı bulmadığı varyant olmayan bölgeleri temsil eder, bu nedenle varyasyon yokluğundaki güven düzeyini açıklayan bazı istatistikleri yakalamıştır.
...
Tüm GVCF'lerden verileri bir veri deposunda birleştirmeyi, ardından ortak çağrılan varyantların nihai VCF'sini oluşturmak için ortak genotipleme analizini çalıştırmayı içeren iki adımlı bir yöntemdir.
...
Bu, konteynerdaki mevcut çalışma dizininde VCF çıktı dosyası `family_trio.vcf`'yi oluşturur.
...
Test aile üçlümüz için ilk varyant için çağrılan genotiplere bakarsak, babanın heterozigot-varyant (`0/1`), anne ve oğulun ise her ikisinin de homozigot-varyant (`1/1`) olduğunu görüyoruz.
...
Samtools indeksleme ve GATK varyant çağırma komutlarını ilgili konteynerlarında nasıl test edeceğinizi biliyorsunuz; GVCF'lerin nasıl oluşturulacağı ve birden fazla örnek üzerinde ortak genotiplemenin nasıl çalıştırılacağı dahil.
