...
Yukarıdaki örnek çıktıda, varyant çağrı verilerinin başlangıcını işaret eden son başlık satırını vurguladık.
...
!!! example "Örnek"
    ...
    Hemen [Bölüm 2](./02_per_sample_variant_calling.md)'ye geçmekten çekinmeyin, bu analizi bir Nextflow iş akışı olarak uygulamaya başlamak isterseniz.
...
Dosyanın içeriğinin ilk 200 satırını görüntülemek için `head -200 reads_mother.g.vcf` komutunu çalıştırırsanız, 1. bölümde oluşturduğumuz eşdeğer VCF'den çok daha uzun olduğunu göreceksiniz ve satırların çoğu VCF'de gördüklerimizden oldukça farklı görünür.
...
Varyant çağrı satırlarının aralarına serpiştirilmiş birçok varyant olmayan satırla birlikte olduğunu fark edeceksiniz, bunlar varyant çağırıcının varyasyon kanıtı bulmadığı varyant olmayan bölgeleri temsil eder.
...
Böyle bir GVCF'de, tipik olarak aralarına serpiştirilmiş daha az sayıda varyant kaydıyla birlikte bu tür varyant olmayan satırlar çoktur.
...
Konteynerdan çıkmadan önce, GVCF dosyalarını, nihai çok örnekli VCF'yi ve tüm indeks dosyalarını konteyner dışındaki dosya sistemine manuel olarak taşıyacağız.
