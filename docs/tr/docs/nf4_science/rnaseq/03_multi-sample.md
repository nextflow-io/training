...
??? info "Bu bölümden nasıl başlanır"
    ...
    Bu size tam bir tek-örnekli işleme iş akışı verir.
...
3. **Paired-end RNAseq verilerine geçiş.**
   Bu, paired-end girdiler için süreçleri uyarlamayı (demetler kullanarak), paired-end modülleri oluşturmayı ve ayrı bir test profili kurmayı kapsar.
...
Paired-end versiyonu kullanmak için `rnaseq_pe.nf` dosyasındaki içe aktarmayı güncelleyin:
