...
- **RNAseq verileri** (`reads/`): Altı örnekten FASTQ dosyaları, dosya boyutlarını küçük tutmak için küçük bir bölgeye indirilmiştir. Her örneğin çift uçlu okumaları vardır (örnek başına iki dosya), ancak yalnızca tek uçlu okumalarla çalışmaya başlıyoruz.
...
İlgili dört ana araç, kalite kontrol metriklerini toplamak için [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/), adaptör kırpma için [Trim Galore](https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) (Cutadapt ve kırpma sonrası QC için FastQC'yi bir araya getirir), referans genoma eklenmiş hizalama için [HISAT2](http://daehwankimlab.github.io/hisat2/) ve toplu QC raporu oluşturma için [MultiQC](https://multiqc.info/)'dir.

Bu araçlar GitHub Codespaces ortamında yüklü olmadığından, bunları Seqera Containers hizmeti aracılığıyla alınan konteynerler üzerinden kullanacağız (bkz. [Merhaba Konteynerler](../../hello_nextflow/05_hello_containers.md)).
...
Daha önce olduğu gibi, konteynerden çıktıktan sonra erişilebilir kalmaları için çıktı dosyalarını bağlı dosya sistemindeki bir dizine taşıyın.
...
FastQC, Cutadapt (Trim Galore aracılığıyla) ve HISAT2'den çıktıları ve diğer birçok aracı tanıyabilir.
...
FastQC ve Trim Galore'u iki örnek daha üzerinde, birbiri ardına çalıştırın.
...
HISAT2 hizalamasını yeni kırpılmış iki örnek üzerinde, birbiri ardına çalıştırın.
...
Burada aracın üç örneğin tümü için QC raporlarını bulduğunu görüyoruz: `fastqc`'den ilk QC, `cutadapt`'ten (via `trim_galore`) kırpma sonrası raporlar ve `hisat2` tarafından üretilen hizalama sonrası QC.
...
FastQC, Trim Galore, HISAT2 ve MultiQC komutlarını ilgili konteynerlerinde nasıl çalıştıracağınızı, birden fazla örneği nasıl işleyeceğinizi ve QC raporlarını nasıl toplayacağınızı biliyorsunuz.

### Sırada ne var?

Bir mola verin, ardından çalışmayı yürütmek için konteynerler kullanan iş akışlarına aynı komutları nasıl saracağınızı öğrenmek için [Bölüm 2](./02_single-sample.md)'ye geçin.
