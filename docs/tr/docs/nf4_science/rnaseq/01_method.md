...
Bu eğitimde, [Babraham Enstitüsü](https://www.bioinformatics.babraham.ac.uk/training/RNASeq_Course/Analysing%20RNA-Seq%20data%20Exercise.pdf)'nden Dr. Simon Andrews ve Laura Biggins tarafından [burada](https://www.babraham.ac.uk/) açıklanan yöntemi takip ediyoruz.

Amacımız, aşağıdaki işleme adımlarını uygulayan bir iş akışı geliştirmektir: toplu RNAseq örneğindeki okumalar üzerinde ilk kalite kontrolünü çalıştırmak, okumalardan adaptör dizilerini kırpmak, okumaları referans genoma hizalamak ve kapsamlı bir kalite kontrol (QC) raporu üretmek.
...
- **TRIM_GALORE:** Trim Galore kullanarak adaptör dizilerini kırpın ve kırpma sonrası QC gerçekleştirin (Cutadapt ve FastQC'yi birleştirir)
...
İhtiyacımız olan araçlar GitHub Codespaces ortamında yüklü değildir, bu nedenle bunları konteynerlar aracılığıyla kullanacağız (bkz. [Hello Containers](../../hello_nextflow/05_hello_containers.md)).
...
Hem `fastqc` hem de `trim_galore` yüklü olan bir konteyner imajını çekeceğiz, etkileşimli olarak başlatacağız ve örnek veri dosyalarından biri üzerinde kırpma ve QC komutlarını çalıştıracağız.
...
Bu, sistem imajı indirirken size aşağıdaki konsol çıktısını verir:
...
Komut isteminiz `(base) root@b645838b3314:/tmp#` gibi bir şeye dönüşecektir, bu da artık konteynerin içinde olduğunuzu gösterir.
...
Okuma verileri üzerinde kalite kontrol metriklerini toplamak için `fastqc` çalıştıralım.
...
Şimdi adaptör dizilerini kırpmak ve kırpma sonrası QC metriklerini toplamak için Cutadapt ve FastQC'yi birleştiren `trim_galore` çalıştıralım.

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

`--fastqc` bayrağı, komutun kırpma tamamlandıktan sonra otomatik olarak bir QC toplama adımı çalıştırmasına neden olur.

_Çıktı çok ayrıntılı olduğundan aşağıdaki kısaltılmıştır._
...
`hisat2` yüklü olan bir konteyner imajını çekeceğiz, etkileşimli olarak başlatacağız ve RNAseq verilerini referans genoma hizalamak için hizalama komutunu çalıştıracağız.
...
Komut, ilgili konteyner URI'si değiştirilerek öncekiyle aynıdır.

### 2.3. Hisat2 genom dizin dosyalarını oluşturun

Hisat2, genom referansının çok özel bir formatta sağlanmasını gerektirir ve sağladığımız `genome.fa` FASTA dosyasını doğrudan tüketemez, bu nedenle bu fırsatı ilgili kaynakları oluşturmak için kullanacağız.
...
Bunları birazdan kullanacağız, ancak önce bu genom dizin dosyalarıyla sıkıştırılmış bir tarball oluşturalım; bunlara daha sonra ihtiyacımız olacak ve bunları oluşturmak genellikle bir iş akışının parçası olarak yapmak istediğimiz bir şey değildir.

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

Bu, genom dizin dosyalarını içeren bir `genome_index.tar.gz` tarball'ını dosya sistemimizde `data/` dizininde saklar, bu da bu eğitimin 2. Bölümünde işimize yarayacaktır.

### 2.4. `hisat2` komutunu çalıştırın

Şimdi hizalama komutunu çalıştırabiliriz, bu komut `hisat2` ile hizalama adımını gerçekleştirir ve ardından çıktıyı BAM dosyası olarak yazmak için `samtools`'a yönlendirir.
...
`multiqc` yüklü olan bir konteyner imajını çekeceğiz, etkileşimli olarak başlatacağız ve öncesi/sonrası FastQC rapor dosyaları üzerinde bir rapor oluşturma komutu çalıştıracağız.
...
Burada aracın oluşturduğumuz üç QC raporunu da bulduğunu görüyoruz: `fastqc` ile yaptığımız ilk QC, `cutadapt`'tan (`trim_galore` aracılığıyla yapılan) kırpma sonrası rapor ve `hisat2` tarafından üretilen hizalama sonrası QC.
...
Tüm bireysel komutları ilgili konteynerlarda etkileşimli olarak test ettiniz.

### Sırada ne var?

Aynı komutları, işi yürütmek için konteynerlar kullanan çok adımlı bir iş akışına nasıl saracağınızı öğrenin.
