...
4. Dokümanlarda ve betiklerde tüm `{YER_TUTUCU}` değerlerini arayın ve içeriğinizle değiştirin.
...
| `{FINAL_OUTPUT_TYPE}`  | Son boru hattı çıktı türü       | joint-called VCFs    |
...
1. **Bölüm 1 (01_method.md)**: Metodolojiyi anlamak için Docker konteynerlarında manuel test
...
- Öğrencilerin dolduracağı iskelet dosyaları sağlayın (iş akışı + modüller)
