# Bölüm 3: Çoklu örnek birleştirme
...
??? info "Bu bölümden nasıl başlanır"
    ...
    Bu size eksiksiz bir tek örnek işleme iş akışı verir.
...
İş akışı davranışını uyarlamak için süreç komutlarını ve çıktılarını nasıl değiştireceğinizi biliyorsunuz.

### Sırada ne var?

Çoklu örnek birleştirme adımını ekleyin.
...
### 2.1. Birleştirme modülünü yazın

{MODULE_INSTRUCTIONS}

### 2.2. Örnek başına çıktıları toplayın ve birleştirme sürecine besleyin
...
Örnekleri ayrı ayrı işleyen ve tüm örnekler genelinde sonuçları birleştiren eksiksiz bir boru hattınız var.
Örnek başına çıktıları çoklu örnek analizi için birleştirmek üzere `collect()` gibi kanal operatörlerini nasıl kullanacağınızı biliyorsunuz.
