...
Bölüm 1-3'te Nextflow'u nasıl çalıştıracağımızı, bir nf-core pipeline'ını nasıl çalıştıracağımızı ve girdileri parametre dosyaları ile örnek listeleri kullanarak nasıl yöneteceğimizi öğrendik.
...
Yerel `nextflow.config` dosyamızdaki Docker yapılandırmasını devre dışı bıraktığımıza ve profilleri anladığımıza göre, `-profile` bayrağını kullanarak pipeline'ı yeniden çalıştıralım.
...
Test profilleri, pipeline'ın çalıştığını doğrulamanıza olanak tanımak için varsayılan girdi parametrelerini ve veri dosyalarını belirlemenin hızlı yollarını sağlar.
nf-core pipeline'ları her zaman en az iki test profili içerir:
...
nf-core'un process etiketi sistemini kullanarak bireysel süreçler için kaynak taleplerini nasıl özelleştireceğinizi öğrenin.
...
Basitlik için nf-core pipeline'ları, tüm pipeline'larda kaynak tahsisini standartlaştırmak amacıyla [**process etiketlerini**](https://www.nextflow.io/docs/latest/reference/process.html#process-label) kullanır.
...
`task.attempt` çarpanına dikkat edin; pipeline `process.maxRetries > 1` ile ayarlanmışsa bu, sonraki görev yeniden denemelerinin daha fazla kaynak talep etmesine olanak tanır.

### 4.2. Belirli süreçler için kaynakları geçersiz kılma

İnce ayarlı kontrol için süreçleri isme göre hedefleyin:
...
Bu pipeline'ı yukarıdaki geçersiz kılmayla çalıştırmayı denersek, `CELLPOSE` süreci etiketi tarafından tanımlanan varsayılan yerine 16 CPU ve 32 GB bellek talep edecektir.
Mevcut ortamımızda bu kadar RAM'imiz olmadığı için bu durum pipeline'ın başarısız olmasına neden olacaktır.
Bu tür başarısızlıkları bir sonraki bölümde nasıl önleyeceğimizi öğreneceğiz.

!!! Tip "İpucu"

    Süreç isimlerini bulmak için pipeline yürütme çıktısına bakın veya `.nextflow.log` dosyasını kontrol edin.
    Süreç isimleri `WORKFLOW:SUBWORKFLOW:PROCESS` desenini takip eder.
...
molkart'ı 16 CPU ve 32 GB bellek talep eden bir süreçle çalıştırmaya çalışsaydık (bölüm 4.2'de gösterildiği gibi), mevcut ortamımızda bu kadar kaynağımız olmadığı için başarısız olurdu.
Daha büyük düğümlere sahip bir küme ortamında bu tür talepler zamanlayıcıya gönderilirdi.

GitHub Codespaces gibi sınırlı ortamlarda, limitler olmadan Nextflow mevcut kaynakları aşan süreçleri çalıştırmayı reddederdi.
...
Bu, Nextflow'a şunu söyler: "Herhangi bir süreç 2 CPU'dan veya 7 GB bellekten fazlasını talep ederse, bunun yerine bu limitlerle sınırla."
...
- nf-core araçlarına daha derinlemesine dalmak için [Build with nf-core](../../nfcore_build/index.md) sayfasını keşfedin
- [Side Quests](../../side_quests/index.md) bölümünde daha ileri düzey konuları keşfedin
