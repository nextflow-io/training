...
!!! tip "İpucu"
    ...
    [Başlarken](00_orientation.md) sayfasında açıklandığı gibi artık `nextflow-run/` dizininde olmanız gerektiğini unutmayın.
...
??? info "`publishDir` kullanarak çıktıları yayınlamak için eski sözdizimi"
    ...
    Çok yakın zamana kadar, çıktıları yayınlamanın yerleşik yolu, `publishDir` yönergesi kullanarak her bir süreç düzeyinde yapmaktı.
...
- Üretimde bir pipeline çalıştırıyorsanız ve bir şeyler ters giderse, birçok durumda sorunu düzeltebilir ve pipeline'ı yeniden başlatabilirsiniz; başarısızlık noktasından itibaren çalışmaya devam eder, bu da size çok zaman ve hesaplama tasarrufu sağlayabilir.
...
Bu durumda ise oturum kimliği aynı kalır.
...
!!! Warning "Uyarı"
    ...
    Önemsediğiniz çıktıları kaydetmekten siz sorumlusunuz! Bu, `publish` yönergesi için `symlink` modu yerine `copy` modunu tercih etmemizin ana nedenidir.
...
Biraz ara verin! Nextflow sözdiziminin ve temel kullanım talimatlarının yapı taşlarını yeni özümsediniz.
...
Daha fazla bilgi: [4.1. `-resume` ile bir workflow'u yeniden başlatın](#41-re-launch-a-workflow-with--resume)
</quiz>
...
Daha fazla bilgi: [4.2. Geçmiş çalıştırmaların logunu inceleyin](#42-inspect-the-log-of-past-executions)
</quiz>
...
Daha fazla bilgi: [3.4. Komut satırı parametreleri için params sistemi](#34-the-params-system-of-command-line-parameters)
</quiz>
...
Daha fazla bilgi: [4.3. Eski çalışma dizinlerini silin](#43-delete-older-work-directories)
</quiz>
