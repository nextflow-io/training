# Bölüm 3: Bir nf-core modülü oluşturma

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } Yapay zeka destekli çeviri - [daha fazla bilgi ve iyileştirme önerileri](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>


Build with nf-core eğitim kursunun bu üçüncü bölümünde, modülleri taşınabilir ve sürdürülebilir kılan temel kuralları uygulayarak bir nf-core modülünün nasıl oluşturulacağını göstereceğiz.

nf-core projesi, Bölüm 1'de iş akışı için kullandığımıza benzer şekilde, düzgün yapılandırılmış modül şablonlarını otomatik olarak oluşturan bir komut (`nf-core modules create`) sağlar.
...
??? info "Bu bölümden nasıl başlanır"
    ...
    Bu bölüm, [Bölüm 2: Bir nf-core modülü kullanma](./02_use_module.md) kısmını tamamladığınızı ve `FIND_CONCATENATE` modülünü iş hattınıza entegre ettiğinizi varsayar.
    ...
    Bu komutları `nfcore-build/` dizini içinden çalıştırın:
...
Gördüğünüz gibi, hem **ana girdide** hem de **çıktıda** bu eğitimin Bölüm 2'sinde tanıtılan `tuple val(meta), path(input_file)` desenini izleyen bir demet kullanacak şekilde değiştirdik.
...
İş akışı tarafındaki koleksiyon — Bölüm 1'deki yer tutucu iş akışında gördüğünüz `channel.topic("versions")` bloğu — topic'e abone olur ve bu birleşik raporu otomatik olarak yazar.
...
Conda ortamı için, modül kodu `#!groovy conda "${moduleDir}/environment.yml"` belirtir; bu da `environment.yml` dosyasında yapılandırılması gerektiği anlamına gelir.
...
- Şablonun `#!groovy eval("cowpy --version")` ifadesi yerine statik bir sürüm string'i (`#!groovy val("1.1.5")`); bu, Bölüm 1.6'daki manuel modülle eşleşir (`cowpy` aracı `--version` bayrağını desteklememektedir)
...
Hazır olduğunuzda, iş hattınıza şema tabanlı girdi doğrulama eklemeyi öğrenmek için [Bölüm 4: Girdi doğrulama](./04_input_validation.md)'ya devam edin.
