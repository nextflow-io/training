...
Bu bölüm, Nextflow pipeline'ınızın yapılandırmasını nasıl kuracağınızı ve yöneteceğinizi keşfedecektir, böylece davranışını özelleştirebilecek, farklı ortamlara uyarlayabilecek ve kaynak kullanımını optimize edebileceksiniz - _iş akışı kodunun kendisinde tek bir satır bile değiştirmeden_.

Bunu yapmanın birden fazla yolu vardır ve bunlar birlikte kullanılabilir ve yapılandırma belgelerinde açıklanan [öncelik sırasına](https://nextflow.io/docs/latest/config.html) göre yorumlanır.
...
Bu eğitim kursunun 5. Bölümünü tamamlayarak üretilen betiğe eşdeğerdir, ancak çıktı hedeflerini değiştirdik:
...
Son ASCII sanat çıktısı, `cowpy-COLLECTED-batch-output.txt` adı altında `results/hello_config/` dizinindedir.
...
Şu anda, iş akışımız komut satırı aracılığıyla birkaç parametre değerini kabul edecek şekilde ayarlanmış durumda ve varsayılan değerler iş akışı betiğinin kendisindeki bir `params` bloğunda ayarlanmış.
...
Ancak, iş akışında parametreleri _bildirme_ (ki bu kesinlikle oraya aittir) ile _varsayılan değerleri_ sağlama (bunlar bir yapılandırma dosyasında daha uygun) endişelerini ayırma avantajına sahiptir.
...
`params` bloğunu iş akışından yapılandırma dosyasına kopyalamadığımıza dikkat edin.
...
Örneğin karakteri değiştirebilir ve yapılandırma dosyasında ayarlanan değerin iş akışı dosyasında ayarlanan değeri geçersiz kıldığını görmek için iş akışını çalıştırabilirsiniz.
...
Son ASCII sanat çıktısı, daha önce olduğu gibi `cowpy-COLLECTED-batch-output.txt` adı altında `results/hello_config/` dizinindedir.
...
Nextflow, YAML veya JSON formatında bir [parametre dosyası](https://nextflow.io/docs/latest/config.html#params-file) aracılığıyla parametreleri belirtmemize olanak tanır; bu, örneğin alternatif varsayılan değer setlerini ve çalıştırmaya özgü parametre değerlerini yönetmeyi ve dağıtmayı çok kolaylaştırır.
...
İş akışını bu parametre dosyasıyla çalıştırmak için, temel komuta `-params-file <dosyaadı>` eklemeniz yeterlidir.
...
Bir parametre dosyası kullanmak, yalnızca birkaç parametre belirtmeniz gerektiğinde aşırıya kaçmak gibi görünebilir, ancak bazı pipeline'lar düzinelerce parametre bekler.
...
'Yayınlanan' çıktılarımızın nasıl organize edileceğini kontrol ederken iki farklı önceliğimiz var:

- Üst düzey çıktı dizini
- Dosyaların bu dizin içinde nasıl organize edildiği
...
CLI aracılığıyla çıktı dizinini değiştirmenin yanı sıra, `outputDir` kullanarak yapılandırma dosyasında özel bir varsayılan değer de ayarlayabiliriz.
...
Örneğin, çıktı yolu bildiriminde `<process>.name` referansını kullanarak çıktılarımızı sürece göre organize edebiliriz:
...
!!! tip
    ...
    `params.batch` değerini `outputDir` yapılandırması yerine çıktı bloğu `path` değerine dahil etmek, CLI'de `-output-dir` ile üzerine yazılmayacağı anlamına gelir.
...
Yapılandırma dosyasında `workflow.output.mode` ayarlamak, iş akışı dosyasında ayarlananı geçersiz kılmak için yeterlidir, ancak yine de gereksiz kodu kaldıralım.
...
Bu şekilde özelleştirebileceğiniz başka birçok seçenek vardır, ancak umarım bu size seçenek yelpazesi ve tercihlerinize uyacak şekilde bunları etkili bir şekilde nasıl kullanacağınız konusunda bir fikir verir.

### Özet

Çıktılarınızın yayınlandığı dizinlerin adlandırmasını ve yapısını ve ayrıca iş akışı çıktı yayınlama modunu nasıl kontrol edeceğinizi biliyorsunuz.

### Sırada ne var?

Yazılım paketleme teknolojisiyle başlayarak, iş akışı yapılandırmanızı bilgi işlem ortamınıza nasıl uyarlayacağınızı öğrenin.

---

## 3. Bir yazılım paketleme teknolojisi seçin

Şimdiye kadar girdilerin nasıl girdiğini ve girdilerin nereden çıktığını kontrol eden yapılandırma öğelerine bakıyorduk. Şimdi iş akışı yapılandırmanızı bilgi işlem ortamınıza uyarlamaya daha spesifik olarak odaklanmanın zamanı geldi.
...
Şimdi URI'yi `conda` yönergesini kullanarak `cowpy` süreç tanımına ekliyoruz:
...
!!! note
    ...
    `cowpy` paketi oldukça küçük olduğu için bu hızlı çalışır, ancak büyük paketlerle çalışıyorsanız, ilk seferde normalden biraz daha uzun sürebilir ve konsol çıktısının tamamlanmadan önce bir dakika kadar 'takılı' kaldığını görebilirsiniz.
...
??? info "Docker ve Conda'yı karıştırma ve eşleştirme"
    ...
    Bu yönergeler süreç başına atandığından, 'karıştırma ve eşleştirme' mümkündür, _yani_ iş akışınızdaki bazı süreçleri Docker ile, diğerlerini örneğin Conda ile çalışacak şekilde yapılandırmak, kullandığınız bilgi işlem altyapısı her ikisini de destekliyorsa.
...
Çalıştırılmaya hazır görevlerin kaynakları mevcut kaynakları aşarsa, Nextflow son görevleri, önceki görevlerden bir veya daha fazlası bitene ve gerekli kaynakları serbest bırakana kadar yürütmeden geri tutar.
...
Yürütücüyü farklı bir arka ucu hedefleyecek şekilde ayarlamak için, kaynak tahsisleri için yukarıda açıklanan benzer sözdizimini kullanarak istediğiniz yürütücüyü belirtmeniz yeterlidir (tüm seçenekler için [yürütücü belgelerine](https://nextflow.io/docs/latest/executor.html) bakın).
...
Çoğu yüksek performanslı bilgi işlem platformu, belirli parametreleri (örneğin CPU sayısı ve bellek gibi kaynak tahsis istekleri ve sınırlamaları) ve kullanılacak iş kuyruğunun adını belirtmenize izin verir (ve bazen gerektirir).
...
[`cpus`](https://nextflow.io/docs/latest/reference/process.html#cpus), [`memory`](https://nextflow.io/docs/latest/reference/process.html#memory) ve [`queue`](https://nextflow.io/docs/latest/reference/process.html#queue) (diğer özellikler için [süreç yönergelerine](https://nextflow.io/docs/latest/reference/process.html#process-directives) bakın) gibi ilgili özellikleri sadece bir kez belirtebilmeniz için standartlaştırılmış bir sözdizimi sağlar.
...
Artık farklı türde bilgi işlem altyapıları kullanmak için yürütücüyü nasıl değiştireceğinizi biliyorsunuz.
...
Raporu incelemek ve kaynakları ayarlamak için bazı fırsatlar belirleyip belirleyemeyeceğinizi görmek için birkaç dakikanızı ayırın.
Kullanım sonuçlarını tahsis edilenin yüzdesi olarak gösteren sekmelere tıkladığınızdan emin olun.

Mevcut tüm özellikler için [Raporlar](https://nextflow.io/docs/latest/reports.html) belgelerine bakın.
...
Bu yapılandırmayla, tüm süreçler 1GB bellek ve tek bir CPU (ima edilen varsayılan) talep edecektir, `cowpy` süreci hariç, bu 2GB ve 2 CPU talep edecektir.
...
Süreçlerinizin farklı kaynak gereksinimleri olduğunda çok kullanışlıdır. Tahmin değil, gerçek verilere dayalı olarak her süreç için ayarladığınız kaynak tahsislerini doğru boyutlandırmanızı sağlar.

!!! tip

    Bu, kaynak kullanımınızı optimize etmek için yapabileceklerinizin sadece küçük bir tadımlığıdır.
    Nextflow'un kendisi, kaynak sınırlamaları nedeniyle başarısız olan işleri yeniden denemek için gerçekten düzgün [dinamik yeniden deneme mantığı](https://nextflow.io/docs/latest/process.html#dynamic-task-resources) yerleşik olarak bulunur.
...
Ancak, bu sınırları aşan kaynak tahsisleriyle iş akışını çalıştırmayı denerseniz, ardından `.command.run` betik dosyasındaki `sbatch` komutuna bakarsanız, yürütücüye gerçekten gönderilen isteklerin `resourceLimits` tarafından belirtilen değerlerde sınırlandırıldığını görürsünüz.
...
Kaynak kullanımını değerlendirmek için bir profilleme raporu nasıl oluşturacağınızı ve tüm süreçler ve/veya bireysel süreçler için kaynak tahsislerini nasıl değiştireceğinizi ve ayrıca HPC'de çalıştırmak için kaynak sınırlamalarını nasıl ayarlayacağınızı biliyorsunuz.
...
Size, üzerinde çalıştığınız projeye veya kullandığınız bilgi işlem ortamına bağlı olarak pipeline yapılandırmanızı özelleştirebileceğiniz birkaç yol gösterdik.
...
Nextflow, farklı yapılandırmaları tanımlayan istediğiniz sayıda [profil](https://nextflow.io/docs/latest/config.html#config-profiles) kurmanıza olanak tanır; bunları daha sonra yapılandırma dosyasının kendisini değiştirmek yerine bir komut satırı argümanı kullanarak çalışma zamanında seçebilirsiniz.
...
Gelecekte bu yapılandırmalarla her zaman birlikte ortaya çıkan başka yapılandırma öğeleri bulursak, bunları ilgili profile(lere) ekleyebiliriz.
...
Bunları iş akışı parametreleri için varsayılan değerleri ayarlamak için de kullanabiliriz, böylece başkalarının kendileri uygun girdi değerleri toplamak zorunda kalmadan iş akışını denemelerini kolaylaştırabiliriz.
...
Aynı yapılandırma öğeleri için değerler ayarlayan ve aynı yapılandırma dosyasında açıklanan profilleri birleştirirseniz, Nextflow çakışmayı en son okuduğu değeri kullanarak çözecektir (_yani_ dosyada daha sonra gelen her neyse).
...
Ve daha genel olarak, yapılandırmanın saklanabileceği çok sayıda yer vardır ve bazen aynı özellikler farklı yerlerde farklı değerlere ayarlanabilir.

Nextflow, herhangi bir çakışmayı çözmek için bir set [öncelik sırası](https://nextflow.io/docs/latest/config.html) uygular, ancak bunu kendiniz belirlemek zor olabilir.
...
Daha fazla bilgi: [1.1. Varsayılan değerleri `nextflow.config` dosyasına taşıyın](#11-varsayılan-değerleri-nextflowconfig-dosyasına-taşıyın)
</quiz>
...
Daha fazla bilgi: [3. Bir yazılım paketleme teknolojisi seçin](#3-bir-yazılım-paketleme-teknolojisi-seçin)
</quiz>
...
Daha fazla bilgi: [5.3. Belirli bir süreç için kaynak tahsisleri ayarlayın](#53-belirli-bir-süreç-için-kaynak-tahsisleri-ayarlayın)
</quiz>

<quiz>
Hangi komut satırı seçeneği bir kaynak kullanım raporu oluşturur?
- [ ] `-with-metrics`
- [ ] `-with-stats`
- [x] `-with-report`
- [ ] `-with-profile`

Daha fazla bilgi: [5.1. Bir kaynak kullanım raporu oluşturmak için iş akışını çalıştırın](#51-bir-kaynak-kullanım-raporu-oluşturmak-için-iş-akışını-çalıştırın)
</quiz>
...
Daha fazla bilgi: [5.5. Kaynak sınırları ekleyin](#55-kaynak-sınırları-ekleyin)
</quiz>
...
Daha fazla bilgi: [4. Bir yürütme platformu seçin](#4-bir-yürütme-platformu-seçin)
</quiz>
...
Daha fazla bilgi: [1.3. Bir parametre dosyası kullanın](#13-bir-parametre-dosyası-kullanın)
</quiz>
...
Daha fazla bilgi: [6. Önceden ayarlanmış yapılandırmalar arasında geçiş yapmak için profilleri kullanın](#6-önceden-ayarlanmış-yapılandırmalar-arasında-geçiş-yapmak-için-profilleri-kullanın)
</quiz>
