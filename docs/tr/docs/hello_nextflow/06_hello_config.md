...
Şimdiye kadar üzerinde çalıştığımız konunun basit bir uzantısı olan bir yapılandırma unsuruyla başlayacağız: girdi parametrelerinin yönetimi.

Şu anda iş akışımız, iş akışı betiğinin kendisindeki bir `params` bloğunda varsayılan değerler ayarlanmış olarak komut satırı üzerinden birkaç parametre değerini kabul edecek şekilde kurulmuştur.
...
Bununla birlikte, iş akışında parametreleri _bildirme_ (ki bu kesinlikle oraya aittir) ile _varsayılan değerler_ sağlama (bunlar bir yapılandırma dosyasında daha uygun) kaygılarını ayırma avantajına sahiptir.
...
Bu, dizin yolunu dinamik olarak ayarlamamıza olanak tanır; yalnızca statik dizeler kullanmakla sınırlı kalmayız.
...
Ayrıca çıktı başına alt dizin `path` bildirimlerini dinamik olarak ayarlayabiliriz.
...
Şimdiye kadar girdilerin nasıl girdiğini ve çıktıların nereye gittiğini kontrol eden yapılandırma öğelerine bakıyorduk. Şimdi iş akışı yapılandırmanızı bilgi işlem ortamınıza uyarlamaya daha spesifik olarak odaklanma zamanı.
...
[`cpus`](https://nextflow.io/docs/latest/reference/process.html#cpus), [`memory`](https://nextflow.io/docs/latest/reference/process.html#memory) ve [`queue`](https://nextflow.io/docs/latest/reference/process.html#queue) gibi ilgili özellikleri belirtebilmeniz için standartlaştırılmış bir sözdizimi sağlar (diğer özellikler için [süreç yönergelerine](https://nextflow.io/docs/latest/reference/process.html#process-directives) bakın); bunu yalnızca bir kez yapmanız yeterlidir.
...
Aynı zamanda, `cowpy` sürecinin diğerlerinden daha fazla kaynak gerektirdiğini varsayacağız; yalnızca bireysel bir süreç için tahsisleri nasıl ayarlayacağımızı gösterebilmemiz için.
...
Bu, mümkün olduğunda Docker kullanacak ve çıktıları `custom-outdir-config/test` altında üretecek; bu sefer karakter komedi ikilisi `dragonandcow`.
...
Daha genel olarak, yapılandırma öğelerinin depolanabileceği çok sayıda yer vardır ve bazen aynı özellikler farklı yerlerde farklı değerlere ayarlanabilir.
