...
Nextflow özellikle iki konuda çok iyidir: tekrarlanabilirlik ve taşınabilirlik. Yapılandırma dosyaları, bunlardan ikincisinin gerçekten parladığı yerdir. Altta yatan pipeline kodunu düzenlemek zorunda kalmadan bir Nextflow pipeline'ını farklı şekillerde ve farklı sistemlerde çalışacak biçimde yapılandırma yeteneği.
...
Bu bölümde birkaç konuyu inceleyeceğiz. Nextflow'un yapılandırma dosyalarını nasıl ele aldığı, nereden yüklediği, nasıl yazıldığı ve nasıl yapılandırıldığı ile pipeline'ın kendisi ve yapılandırma dosyasına ne gitmesi gerektiği arasındaki ayrımla başlayacağız.
...
Tamam, başlayalım. Yapılandırma dosyalarını yükleme konusuna gelince, Nextflow birçok farklı yerden çekebilir; bu iyi bir şeydir, ancak aynı zamanda biraz riskli de olabilir. Zira yapılandırma dosyasının nereden alındığını ve hangi sırayla yüklendiğini bazen anlamak güçleşebilir.

Bu yüzden buradaki bağlantıya tıklamanızı gerçekten öneririm; bu bizi Nextflow dokümanlarına götürür. Bu yapılandırma sayfasında, yapılandırmanın yüklendiği önemli yerler ve önemlisi bunların yüklendiği sıra listelenmektedir.
...
İsterseniz bu konumların tümüne yapılandırma dosyaları sağlayabilirsiniz; bunlar yinelemeli olarak yüklenecek, her biri yalnızca çakıştıkları yapılandırma kapsamlarında bir öncekinin üzerine yazacaktır.

Bu gerçekten güçlü bir sistemdir; mantıklı varsayılanlar belirleyebilir ve ardından yapılandırmada giderek daha spesifik hale gelebilirsiniz.

## 0. Isınma: hello-config.nf'yi çalıştırın

Tamam, bunu kapatalım ve Codespaces'e geçip başlayalım. Daha önce olduğu gibi burayı temizledim; önceki sonuç dizinlerimi, Nextflow'umu ve work dizinlerimi vb. kaldırdım. Bu dosyalar hâlâ etrafta dolaşıyor olsa bile endişelenmeyin. Sadece çok fazla zoom yaptığım için, aksi halde işler çok çabuk karışıyor.
...
Daha önce olduğu gibi, bu ana betikteki çıktı yolunu hello config olarak değiştirdik; böylece daha önce oluşturduğunuz sonuçlarla çakışmayacak.
...
Bu params bloğunu betikten alıp yapılandırma dosyasına koyacağım. Burada biraz dikkatli olmamız gerekiyor; çünkü şu anda yapılandırma ve betikler arasındaki sözdizimi biraz farklı. Yapılandırma dosyası tür bildirimlerini kabul edemez; bu parametreleri gerçekten tanımlamıyoruz, sadece referans veriyoruz. Bu yüzden bunlardan kurtulacağım.
...
Şimdiye kadar Nextflow'u pipeline betiğimizin olduğu aynı dizinden başlatıyorduk. Yani başlatma dizinimiz ve pipeline dizinimiz aynı şey gibi.
...
Sonra cd ile dizini tux-run'a değiştireceğim. Artık çalışma dizinimizin pipeline betiklerinin bulunduğu dizinle aynı olmadığını unutmayın.
...
Pipeline kodu hiç değişmedi, ama şimdi iki yapılandırma seti yüklenecek ve başlatma dizini yapılandırma dosyası, pipeline'ın nextflow.config'inde ayarlanan varsayılanların üzerine yazmalı ve farklı sonuç setleri almalıyız.
...
Şimdi results'a bakarsam, topladığımızı görebiliriz ve işte küçük tux karakterimiz. Yani o parametrelerin düzgün bir şekilde yorumlandığını görebilirsiniz.
...
Gördüğümüz gibi komut satırından çift tire ile parametre adlarıyla alabiliriz, ama aynı zamanda sadece parametrelerin olduğu bir YAML veya JSON dosyası da sağlayabiliriz.
...
Tüm dosyaların yayımlandığı bu temel dizini -output-dir adlı bir komut satırı bayrağı ile değiştirebiliriz. Nextflow run hello config yaparsam ve sonra -output-dir yaparsam, buna "custom-outdir-cli" diyeceğiz. Yazmak zahmetli. Sadece bu dosyaların nereden geldiğini hatırlayalım.

Bu bir temel Nextflow seçeneği ve çok yeni. Bu yakın zamanda eklendi ve yeni dil ayrıştırıcısı ile birlikte yapabileceğimiz şeylerden biri.
...
Tamam. Bunu çalıştırıyoruz. Pipeline'ımızda veya bu noktada yapılandırmamızda hiçbir şeyi değiştirmedik ve umarım tüm sonuçlarımızı farklı bir üst düzey dizine kaydetmeli. Bunu istediğiniz temelde herhangi bir yola ayarlayabileceğinizi hayal edebilirsiniz.

En üstte geldi. Bir custom-outdir-cli'miz var ve tüm dosyalar tam olarak aynı şekilde organize edilmiş, aynı alt dizinleri ve dosya adlarıyla. Yani bu, pipeline'ın sonuçlarını nasıl organize ettiği hakkında çok fazla düşünmeden nereye yayımladığını değiştirmenin gerçekten kolay bir yolu.
...
Burada params.batch olan bir params değişkeni dahil ettiğimizi görebilirsiniz; bu bu dizenin bir parçası. Bu, başka yerlerden gelen değişkenleri yeniden kullanabileceğimiz anlamına gelir. Bu durumda, Nextflow pipeline'ını çalıştırdığımızda --batch yaparsak, batch adının ne olduğuna göre özel yolumuzda bir alt dizin alacağız.
...
Ve gerçekten güzel olan şey, koddaki bu ekstra karmaşıklığın CLI'ye geçmemesi. Yani -output-dir ve hangi batch değişkenleriyle komutumuz çalıştırabiliriz; sadece pipeline'ı nasıl çalıştıracağımızı düşünerek ve kodda ne olduğunu çok fazla düşünmeden. Ve çıktı dosyalarımız gerçekten güzel bir şekilde, çok iyi organize bir şekilde oluşturulacak; bu pipeline'ı kullanan insanlar için güzel.
...
## 2.3. Yayımlama modunu workflow düzeyinde ayarlayın
...
Tamam. Kendimizi kontrol edelim çünkü görünüşe göre hata yapıyoruz. Tekrar çalıştıralım ve dosyaları yayımlamak için copy modunu doğru kullanıp kullanmadığımızı kontrol edelim. Betiği tekrar çalıştıracağız ve bu sefer sonuçları config-output-mode adlı bir dizine koyduk; oradaki dosyaların nasıl göründüğüne bakalım.
...
Tamam. Şimdiye kadar girişlere ve çıkışlara, iş akışının çalıştığı dosyalara odaklanıyorduk. Ama altyapı ne olacak? Başta söyledim ki Nextflow aynı pipeline'ın farklı bilgi işlem kurulumlarında çalıştırılmasına izin verir. Peki bu nasıl görünüyor?
...
Ama Nextflow'daki bu yapılandırma ayrımı ile ve Nextflow'un birçok farklı hesaplama altyapısı arka uçlarıyla etkileşim kurma yeteneği ile, pipeline kodunu hiç değiştirmeden almak ve bunu değiştirmek çok basit.
