...
Kanalların bir başka bölümü de operatörlerdir. Bunlar temel olarak kanal içeriklerini değiştirmek için kanallar üzerinde kullanabileceğimiz fonksiyonlardır. Hadi VS Code'a dalıp nerede olduğumuza bakalım.

Bu VS Code'da çok yakınlaştırdım, bu yüzden işleri temiz ve düzenli tutmak için tüm _.nextflow\*_ dosyalarını ve _work/_ dizinini, _results/_ dizinini ve Birinci Bölüm'deki her şeyi kaldırdım. Ve burada yeni baştan başlıyorum. Ama bunun için çok endişelenmeyin. İstemiyorsanız, bu dosyaları orada bırakabilirsiniz. Herhangi bir soruna neden olmazlar.
...
Farklı olan bir şey, buradaki output bloğundaki yolun artık bu bölüm için _hello_channels_ olması, bu da sonuç dosyalarının hala orada varsa results dizininizde farklı bir alt dizinde saklanacağı anlamına gelir. Bu yüzden çıktılar konusunda kafanız karışmadan başlamak için güzel ve temiz bir yer olmalı.
...
İş akışına gidiyoruz, bu da tüm iş akışı mantığımızın işleri bir araya getirmekle ilgili olduğu yer. Ve buraya gireceğim ve yeni bir kanal oluşturacağım.
...
Channel, kanallarla ilgili her şey için ad alanı gibidir. Daha önce Nextflow kullanıyorsanız küçük "c" harfi. Ve _".of"_, temel olarak bir kanal oluşturmanın bir yolu olan Channel factory denen bir şeydir.
...
Tamam, terminalime gideceğim ve iş akışını tekrar çalıştıracağım. Bu sefer herhangi bir _"--input"_ olmadan, ve çalışacak ve oluşturduğumuz kanalı kullanacak ve umarım _results/hello_channels/_ içinde bir dosyamız olmalı ve şimdi "Hello Channels!" diyor. Fantastik. Yani kanalımızdan umduğumuz bu. Harika.

## 1.4. Kanal içeriklerini incelemek için view() kullanın

Burada eklenecek bir şey daha var, kanallar üzerinde kullanabileceğimiz "_.view"_ adlı başka bir fonksiyona hızlı bir giriş.
...
Yani "_.view"_ yapın, ve sonra iş akışını tekrar çalıştırırsam, oluşturduğumuz anda bu kanalın içeriğinin ne olduğunu terminale yazdırmalı.

Nitekim, terminale buraya yazdırıldığını görebilirsiniz. _"Hello Channels!"_.

İsterseniz bunları satırlar arasında bölebileceğinizi unutmayın ve aslında Nextflow otomatik biçimlendirici bunu sizin için yapmaya çalışacaktır. Boşluk burada gerçekten önemli değil, bu yüzden bunları birbiri ardına zincirleyebilirsiniz.
...
Bu noktada işimizin bittiğini düşünebilirsiniz. Ama aslında burada bizi tökezletecek bir tuzak var. Çıktı dosyamıza bakarsak. İçinde _"Hello"_ olduğunu görebilirsiniz, ama diğer çıktılardan hiçbiri yok. Aslında, sadece bu.
...
Terminale bakarsak, üç kez çalıştığını ve farklı view çıktılarını görebiliriz. Ama work dizinine gidersem, _"cat work"_ yapabilirim. Bu hash'i koyun ve genişletin ve _output.txt_. Bu work dizinindeki dosyanın results dizininden farklı olduğunu görebilirsiniz ve bu _"Hej"_. Yani burada tam olarak çalışmayan bir şey var.

Ve anahtar şu ki, üç görev çalıştı. Nextflow çıktısı, işlem devam ederken bunu özetlemeye çalışır, böylece terminalinizin tamamını tamamen ele geçirmez ve bu ANSI Logging, ANSI kaçış kodlarını kullanır, temel olarak diğer görevlerin üzerine yazmıştır. Bu yüzden size güncellenen son olanı gösterir.
...
Yani komutu tekrar çalıştırırsam, ama bu sefer kapatmak için _"-ansi-log false"_ dersem, _$NO_COLOR_ veya _"$NXF_ANSI_LOG=false"_ ortam değişkenlerini de kullanabilirim. Sonra bu kaçış kodları olmadan Nextflow günlüğünün daha eski moda stilini kullanır. Akıllı güncellemeler olmadan doğrudan terminale yazdırır.
...
Bu artık biraz daha mantıklı. Umarım Nextflow'un bunu yaptığını anlarsınız, sadece bu work dizinleriyle terminalde size gösterdiği konuda biraz akıllıydı.
...
Şimdi bunu anlamak için iş akışı betiğimize geri dönmemiz gerekiyor. Kanalımızı burada oluşturuyoruz, sürecimize geçiriyoruz ve sürece bakarsak, greeting'i _"output.txt"_ adlı bir dosyaya yazıyoruz ve bu çıktı dosyasını buradan çıktı bloğuna geri geçiriyoruz, yayınlıyoruz.

Ancak, bu süreç bu üç farklı görevi her üç kez çalıştırdığında. Hepsi _"output.txt"_ adlı bir dosya oluşturur, tüm bu çıktı dosyaları results dizinine yayınlanır ve hepsi birbirinin üzerine yazar. Yani orada aldığınız sonuç dosyası her neyse, sadece oluşturulan son dosyadır, ama diğerlerinin hepsini ezmiştir. Bu gerçekten istediğimiz şey değil.
...
Yani buraya yapıştırabilirim, _"$\{greeting\}\_output.txt"_, ve sonra bunu buraya da yapıştırmam gerekiyor çünkü artık _output.txt_ adlı bir dosya oluşturmuyoruz. Yani bunu güncellemezsen, Nextflow hiç oluşturulmamış bir dosya beklediğini söyleyen bir hatayla çökecek.
...
Yani gerçek daha olgun Nextflow boru hatlarında, genellikle belirli bir örnekle ilişkili tüm meta verilere sahip bir meta nesnesi geçirirsiniz. Daha sonra buna dayalı olarak dinamik dosya adları oluşturabilirsiniz, bu çok daha pratiktir.
...
Hadi bunu bölüm bölüm inceleyelim. Hatanın nedeni olduğunu söylüyor ve sonra hatanın nedeni, eksik çıktı dosyaları. Yani temel olarak o çıktı bloğu bu dosyanın oluşturulması gerektiğini söyledi ve oluşturulmadı. Sonra bu yürütülen komut diyor. Yani bu temel olarak o _.command.sh_ dosyasının içeriği. Tüm bu değişkenler konulduktan sonra böyle görünüyordu.
...
Yani kullanılabilmesi için önce bu kanala bir şey yapmamız gerekiyor. Ve bu, kanal içeriklerini manipüle etmek için kanallar üzerinde kullanabileceğimiz özel fonksiyonlar olan operatörleri kullanmak için sahneyi hazırlıyor.

Bu durumda, _flatten_ adında bir şey kullanacağız. Bunu burada kanalın sonuna geçiriyoruz. Yani kanalı oluşturuyoruz ve sonra _flatten_ çalıştırıyoruz. Ve yine, üzerine gelirseniz, bu komut için belgeleri doğrudan VS Code'da gösterir, bu çok yardımcı. Tüm bu dokümanları Nextflow web sitesinde, dokümantasyonda da bulabilirsiniz.

Bu kodu şimdi çalıştırabilirdim ve işe yarayıp yaramadığını görebilirdim, ama aynı zamanda operatörler içinde ve Nextflow kodu içinde closure denen dinamik kod yapmanın nasıl yapılacağını tanıtmak için güzel bir fırsat.

Yani _flatten_ çalıştırmadan önce buraya bir view komutu ekleyeceğim. Ve burada bu kıvrımlı parantezler var, bu dinamik closure. Ve burada sadece view operatörü bağlamında yürütülecek bazı rastgele kodlar var.
...
Harika. Öncelikle boru hattı bu sefer çökmedi. Düzgün çalışan üç sürecimiz vardı ve küçük bir onay işaretimiz var. Ve sonra view ifadelerimizin işe yaradığını görebiliriz.

Flatten'dan önce var, bu daha önce hatadan gördüğümüz dizi, ve sonra _flatten_ çağrıldıktan sonra üç kez var, _"Hello", "Bonjour"_ ve dizideki diğer üç ayrı öğe var, bunlar şimdi umduğumuz gibi kanalda üç ayrı öğe.
...
Çok hızlı bir şekilde, daha önce kanal fabrikaları oluştururken _"."_ yaptığımı söylemek isterim, ve kanal oluşturmanın birçok farklı yolu olduğunu gördük ve bunlardan biri "_fromList"_ olarak adlandırılıyor. Ve bu aslında özellikle bu aynı işlemi yapmak için tasarlanmış. Yani sadece from list greetings away yapabilirdik ve bu işe yarayacak. Biraz daha temiz ve güzel bir sözdizimi. Ama bu gösterimin amaçları için, kanalın nasıl manipüle edildiğini ve farklı operatörlerin bir kanalın içeriğindeki içeriği nasıl değiştirebileceğini görebilmeniz için adım adım yapmak istedik.
...
Hadi greetings away'den kurtulalım. Ve şimdi bu kanal fabrikasını tekrar değiştireceğiz. Seçebileceğiniz bir sürü olduğunu söyledim ve _".fromPath"_ adında bir tane var. Ve ona söyleyeceğim, bu durumda, daha önce kullandığımız girdimiz olan _params.input_'u alsın.

Şimdi bu parametre henüz kullanılmaya hazır değil. Hala bunun bir string olduğunu söylüyoruz ve burada bir varsayılanla sabit kodlanmış, ama bu string'i geçersiz kılabiliriz. Şimdi bunun bir dosya olmasını istiyoruz. Yani tip farklı. Artık bir _String_ değil. Bir _Path_.

Ve sonra istediğimiz takdirde varsayılanı tekrar bir Path'e ayarlayabiliriz. Ve soldaki keşfet'e bakarsam, bu depoda, bu çalışma dizininde, data adında bir dizinimin olduğunu görebilirsiniz. Orada _"greetings.csv"_ adında bir dosyam var.
...
Burada diyor ki: \_echo çalıştırmaya çalıştık, ama bu CSV dosyasının içeriğini yankılamak yerine, sadece yolu yankıladı. Ve burada bu CSV dosyasına tam mutlak yol olduğunu görebilirsiniz.
...
Tekrar çalıştırırsam, yine başarısız olacak, ama bu kanalın içinde ne olduğunu bize gösterecek. Özellikle heyecan verici değil. Bu _path_ değişkeni. Yani bir terminale yazdırıldığı için burada sadece bir string olduğunu görebilirsiniz, ama bu dosya hakkındaki bilgileri ve meta verileri içeren bir _path_ nesnesi.
...
Log mesajından muhtemelen anlayabileceğiniz gibi, başka bir operatör, başka bir kanal operatörü olan _splitCsv_'yi kullanmak istiyoruz. Yani "_dot" "s"_ yaparsam, ve sonra otomatik önerildiğini görebilirsiniz. Oops, _splitCsv_ ve bazı parantezler.

Ve sonra _splitCsv_'den sonra, sadece daha sonra nasıl göründüğünü görebilmemiz için başka bir _view_ ifadesi koyacağım. Boru hattını çalıştıralım ve ne elde ettiğimize bakalım.

Tamam. Hala başarısız oldu, ama yeni ve heyecan verici bir şekilde, bu ilerleme.

Bu sefer yine, render edilmiş betiğimizle ilgili bir sorunumuz var. Şimdi. Artık son yolu almadık, ama sabit bir girdi olarak bir dizi geçirdiğimizde daha önce sahip olduğumuz hataya çok benzeyen bir değişkenler dizisi aldık.
...
Nextflow'daki diziler sıfır tabanlıdır, bu yüzden sadece ilk öğe diyeceğiz, bu satır sıfır. İkinci sütunu isteseydik, bir veya üçüncü sütun iki olabilir, ve böyle devam eder. Burada istediğimizi döndürebiliriz, ama sadece ilk değeri döndüreceğim.
...
Ve buradaki bu workflow bloğu gerçekten tüm o paralelleştirmeyi ve tüm akıllı mantığı oluşturduğunuz ve Nextflow'a iş akışı DAG'ınızı nasıl oluşturacağınızı ve boru hattınızı nasıl düzenleyeceğinizi açıkladığınız yerdir.
...
Bu bölümün sonu. Gidip kısa bir mola verin ve bir sonraki videoda sizi göreceğim, bölüm üç için Hello Workflow'dan geçeceğiz ve iş akışları hakkında biraz daha konuşacağız.
