...
Farklı olan bir şey, output bloğundaki yolun artık bu bölüm için _hello_channels_ olması; bu da sonuç dosyalarının, hâlâ orada varsa, results dizininde farklı bir alt dizinde saklanacağı anlamına geliyor. Çıktılar hakkında kafanız karışmadan başlamak için güzel ve temiz bir yer olmalı.
...
Bu güzel. Ama oldukça basit. Bu parametrede bir kez çalışan bir sürece giden bir değişkenimiz var ve gerçekten ölçeklenmiyor. Ona birçok farklı dosya veremiyoruz, birçok farklı selamlama veremiyoruz. Sadece bir tane var.
...
İşleri bir araya getirme konusundaki tüm iş akışı mantığımızın bulunduğu workflow'a gidiyoruz. Buraya gireceğim ve yeni bir kanal oluşturacağım.

Yeni bir kanal oluşturun.

Buna "_greeting_ch"_ diyeceğim. Bu değişkenin bir kanal olduğunu hatırlayabilmeniz için "_\_ch"_ yapmak bir gelenektir. Ama istediğiniz gibi adlandırabilirsiniz.
...
Ancak bu, work dizinleriyle ilgili bir sorunu çözdü; ama çıktı dosyasıyla ilgili sorunu çözmedi. Hâlâ sadece _"Hello"_ yazan bir çıktı dosyamız var.
...
Bir kanalda bir dizi kullanabilir miyim? Deneyelim. Bir dizi oluşturacağım; bunu dokümanlardan kopyaladım: _"greetings_array"_ ve _"Hello"_, _"Bonjour"_ ve _"Holà"_. Sonra bunu sabit kodlanmış string'lerimin yerine buraya koyacağım. Yani "channel.of" _"greetings_array"_ diyeceğim; bu diziyi bir kanala geçireceğim. Deneyelim.
...
Yani bu sefer çalışacağız; CSV'yi üç dizi nesnesine ayrıştırdık ve sonra düzleştirdik. Ve bu sefer geçti. Nextflow boru hattı çalıştı. Ancak _flatten_'ın gerçekten işe koyulduğunu ve her şeyi düzleştirdiğini görebilirsiniz. Ve böylece her satır için üç bağımsız dizi girişi alıyoruz. Bu yüzden süreci her CSV satırı için üç kez çalıştırdı. Ve şimdi bir sürü sonuç dosyamız var; 123, 456 ve her türlü şey; sadece gerçekten istediğimiz CSV'nin ilk sütunu değil.
...
Nitekim, _splitCsv_'den sonra dizilerimiz var ve sonra _map_'ten sonra güzel temiz string'lerimiz var; sadece _"Hello"_, _"Bonjour"_ ve _"Holà"_. Ve boru hattı şimdi istediğimizi yapıyor. Fantastik.
