...
Başlamanız için, nasıl yapılandırıldığını anlamanız amacıyla iş akışı betiğini açalım.
Sonra çalıştırıp çıktılarını arayacağız.

### 1.1. Kodu inceleyin

`hello-world.nf` betiğini mevcut dizininizde bulacaksınız; bu dizin `hello-nextflow` olmalıdır. Düzenleyici bölmesinde açın.
...
`output` tanımı, bunun bir yol olarak ele alınması gerektiğini Nextflow'a söyleyen `path` niteleyicisini içerir (hem dizin yollarını hem de dosyaları kapsar).
...
Değişken girdilerin nasıl ekleneceğini bu eğitim modülünün ilerleyen bölümlerinde öğreneceksiniz; daha fazla süreç eklemeyi ve bunları kanallarla bağlamayı ise bu kursun 3. Bölümünde öğreneceksiniz.
...
Her süreç çağrısı için Nextflow, benzersiz olması amacıyla hash ile adlandırılmış iç içe bir alt dizin oluşturur; gerekli tüm girdileri hazırlar (varsayılan olarak sembolik bağlantılar kullanarak), yardımcı dosyalar yazar ve sürecin herhangi bir çıktısını ve günlüklerini yazar.
...
Açın ve `Hello World!` selamlamasını bulacaksınız; minimalist iş akışımızın amacı da buydu.
...
`.command.sh` dosyası özellikle kullanışlıdır çünkü tüm defter tutma ve görev/ortam kurulumu dahil olmaksızın Nextflow'un yürüttüğü ana komutu gösterir.
...
Basit bir Nextflow betiğini nasıl çözeceğinizi, çalıştıracağınızı ve work dizininde çıktı ile ilgili günlük dosyalarını nasıl bulacağınızı biliyorsunuz.
...
Terminal çıktısı artık yayınlanan çıktıları ve yazıldıkları dizini listeleyen bir `Outputs:` özeti ile sona erer.

Dosya gezgininizi kontrol edin: bu sefer Nextflow `results/` adında yeni bir dizin de oluşturdu.
...
Sonuçları düzenlemek için kullanılan dizinleri adlandırmak amacıyla süreç adını veya diğer değişkenleri kullanmak ve üst düzey çıktı dizininin varsayılan adını değiştirmek (`-o` CLI bayrağı veya `outputDir` yapılandırma değişkeni tarafından kontrol edilir) de mümkündür.
...
`sayHello` sürecinin çıktıları için az önce yaptığımız şeyi başarmak amacıyla, bunun yerine süreç tanımına aşağıdaki satırı eklemiş olurduk:
...
??? question "İşe yaramadıysa"
    ...
    O zaman muhtemelen eski v1 Nextflow dil ayrıştırıcısını kullanıyorsunuz.
    Bu kursun başında bahsedilmişti, ama belki kaçırmışsınızdır.
    [Nextflow sürümleri](../info/nxf_versions.md) yardım materyaline bakın.

    v2 ayrıştırıcısı Nextflow 26.04 ve sonrasında varsayılan olarak gelir; bu nedenle bu sorunu yalnızca daha eski sürümlerde göreceksiniz.
...
Bu, Nextflow'un yalnızca son çalıştırmadan bu yana eklediğiniz veya değiştirdiğiniz süreçleri ya da yeni ayarlar veya girdiler sağladığınız süreçleri çalıştıracağı anlamına gelir.
...
- Bir iş akışını üretimde çalıştırıyorsanız ve bir şeyler ters giderse, birçok durumda sorunu düzeltip iş akışını yeniden başlatabilirsiniz; iş akışı arıza noktasından çalışmaya devam eder. Bu da size çok zaman ve hesaplama tasarrufu sağlayabilir.
...
Süreç durumu satırına (5. satır) eklenen `cached:` kısmına bakın; bu, Nextflow'un bu işi zaten yaptığını ve önceki başarılı çalıştırmanın sonucunu yeniden kullandığını gösterir.
...
Hazır olduğunuzda, iş akışınıza girdileri beslemek için kanalları nasıl kullanacağınızı öğrenmek amacıyla [**Bölüm 2: Hello Channels**](./02_hello_channels.md)'a geçin; bu, Nextflow'un yerleşik veri akışı paralelliğinden ve diğer güçlü özelliklerden yararlanmanızı sağlayacaktır.
...
Daha fazla bilgi: [1.1.1. Süreç tanımı](#111-the-process-definition)
</quiz>
...
Daha fazla bilgi: [1.2.2. `work` dizininde çıktıyı ve günlükleri bulun](#122-find-the-output-and-logs-in-the-work-directory)
</quiz>
...
Daha fazla bilgi: [4.1. `-resume` ile bir iş akışını yeniden başlatın](#41-re-launch-a-workflow-with--resume)
</quiz>
...
Daha fazla bilgi: [2.3. Yayınlama modunu kopyalamaya ayarlayın](#23-set-the-publish-mode-to-copy)
</quiz>
...
Daha fazla bilgi: [3.2. Kullanıcı girdisini yakalamak için komut satırı parametresi ayarlayın](#32-set-up-a-command-line-parameter-to-capture-user-input)
</quiz>
