...
Nextflow kanalları, içerikler üzerinde operatörler kullanarak işlem yapmamıza olanak tanıyan bir şekilde oluşturulmuştur; operatörleri bu bölümde daha ayrıntılı ele alacağız.

Şimdilik, bir kanalın içeriklerini incelemek için [`view()`](https://www.nextflow.io/docs/latest/reference/operator.html#view) adlı çok basit bir operatörü nasıl kullanacağınızı göstereceğiz.
...
Bunları `'Hello'`, `'Bonjour'` ve `'Hola'` yapalım.
...
Ancak aynı results dizinine yayımlandıklarında, ilk kopyalanan bir sonraki tarafından üzerine yazılır ve bu böyle devam eder.

### 2.2. Çıktı dosya adlarının benzersiz olmasını sağlayın

Tüm çıktıları aynı results dizinine yayımlamaya devam edebiliriz, ancak benzersiz adlara sahip olmalarını sağlamamız gerekiyor.
...
!!! tip "İpucu"
    ...
    Bunu daha sonra Nextflow eğitiminizde öğreneceksiniz (bkz. [Meta veri yan görevi](../side_quests/metadata/index.md)).
...
Şimdi kanal fabrikasında şu anda sabit kodlanmış `'Hello','Bonjour','Hola'` değerlerini az önce oluşturduğumuz `greetings_array` ile değiştireceğiz.
...
Görünüşe göre Nextflow, dizideki üç dizeyi ayrı değerler olarak kullanmak yerine, `[Hello, Bonjour, Hola]`'yı bir dize değeri olarak kullanarak tek bir süreç çağrısı çalıştırmaya çalıştı.
...
İkinci bir `.view` ifadesi eklediğimizi ve her biri için boş parantezleri (`()`) biraz kod içeren süslü parantezlerle değiştirdiğimizi görüyorsunuz, örneğin `#!groovy { greeting -> "Before flatten: $greeting" }`.
...
Dosya yollarını işlemek için yerleşik işlevselliğe sahip yeni bir kanal fabrikası olan [`channel.fromPath()`](https://nextflow.io/docs/latest/reference/channel.html#frompath) kullanmaya geçmemiz gerekiyor.
...
Daha fazla bilgi: [1.1. Bir girdi kanalı oluşturun](#11-create-an-input-channel)
</quiz>
...
Daha fazla bilgi: [2. İş akışını birden fazla girdi değeri üzerinde çalışacak şekilde değiştirin](#2-modify-the-workflow-to-run-on-multiple-input-values)
</quiz>
...
Daha fazla bilgi: [3.2.1. `flatten()` operatörünü ekleyin](#321-add-the-flatten-operator)
</quiz>
...
Daha fazla bilgi: [1.4. Kanal içeriklerini incelemek için `view()` kullanın](#14-use-view-to-inspect-the-channel-contents)
</quiz>
...
Daha fazla bilgi: [4.2. Dosyayı ayrıştırmak için `splitCsv()` operatörünü kullanın](#42-use-the-splitcsv-operator-to-parse-the-file)
</quiz>
...
Daha fazla bilgi: [4.3. Selamlamaları çıkarmak için `map()` operatörünü kullanın](#43-use-the-map-operator-to-extract-the-greetings)
</quiz>
...
Daha fazla bilgi: [2.2. Çıktı dosya adlarının benzersiz olmasını sağlayın](#22-ensure-the-output-file-names-will-be-unique)
</quiz>
