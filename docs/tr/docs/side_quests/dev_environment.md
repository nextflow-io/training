...
Bu kılavuzdaki bazı özellikler opsiyonel klavye kısayollarını kullanacaktır. Bu materyale tarayıcı üzerinden GitHub Codespaces aracılığıyla erişiyor olabilirsiniz; bu durumda bazen kısayollar beklendiği gibi çalışmayabilir, çünkü sisteminizde başka şeyler için kullanılıyorlar.
...
1. VS Code'u açın
2. Sol taraftaki eklentiler simgesine tıklayarak Extensions görünümüne gidin: ![eklentiler simgesi](img/extensions_icon.png) (VS Code'u yerel olarak çalıştırıyorsanız kısayol `Ctrl/Cmd+Shift+X`)
3. "Nextflow" arayın
...
Bu görsel organizasyon, birden fazla süreç ve karmaşık veri akışları içeren iş akışlarıyla çalışırken paha biçilmez hale gelir.
...
`complex_workflow.nf` hala açıkken, VS Code'un üst kısmındaki arama çubuğuna `@` yazarak dosyadaki tüm sembollerin genel görünümünü elde edebilirsiniz (klavye kısayolu `Ctrl/Cmd+Shift+O`'dur, ancak Codespaces'te çalışmayabilir). Bu, geçerli dosyadaki tüm sembolleri listeleyen sembol gezinme panelini açar:
...
Hızlı dosya gezinmesi, bölünmüş editörler ve proje genelinde arama kullanarak karmaşık çok dosyalı projeleri yönetebilir ve iş akışları ile modüller arasında verimli çalışabilirsiniz.
...
Git değişikliklerini görmek ve commit'leri doğrudan editörde hazırlamak için source control düğmesini (![Source control simgesi](img/source_control_icon.png)) kullanarak Source Control panelini açın (`Ctrl+Shift+G` veya VS Code'u yerel olarak çalıştırıyorsanız `Cmd+Shift+G`).
...
Bu IDE özelliklerinin gerçek gücü, daha büyük ve daha karmaşık projeler üzerinde çalışırken ortaya çıkar. Onları iş akışınıza kademeli olarak dahil etmeye başlayın — birkaç oturum içinde doğal hale gelecekler ve Nextflow geliştirmeye yaklaşım şeklinizi dönüştüreceklerdir.
