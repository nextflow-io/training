...
- `channel.fromPath()` ve `channel.fromFilePairs()` ile dosya işlemeyi otomatikleştirmek için kanal kullanma
...
- Doğru girdi işleme ile dosya işlemlerini Nextflow süreçlerine entegre etme
- Meta veri odaklı dizin yapıları kullanarak süreç çıktılarını düzenleme
...
Bu, workflow'unda tek bir dosya yoluna başvuran, ardından onu sınıfıyla birlikte konsola yazdıran (herhangi bir süreç olmadan) mini bir iş akışıdır.
...
### 1.4. Dosyayı bir sürece besleme

Dizeler ve Path nesneleri arasındaki fark, süreçlerle gerçek iş akışları oluşturmaya başladığınızda kritik hale gelir.
Şimdiye kadar Nextflow'un girdi dosyamızı bir dosya olarak işlediğini doğruladık, ancak bu dosyayı bir süreçte gerçekten çalıştırıp çalıştıramayacağımıza bakalım.

#### 1.4.1. Süreci içe aktarma ve kodu inceleme
...
Gördüğünüz gibi, dosyayı açan ve kaç satır içerdiğini sayan oldukça basit bir betik.

??? info "`debug true` ne yapar?"

    Süreç tanımındaki `debug true` yönergesi, Nextflow'un betiğinizden çıktıyı (satır sayısı "40" gibi) doğrudan yürütme günlüğünde yazdırmasına neden olur.
    Bu olmadan, yalnızca süreç yürütme durumunu görürsünüz, ancak betiğinizden gerçek çıktıyı görmezsiniz.

    Nextflow süreçlerini hata ayıklama hakkında daha fazla bilgi için [Nextflow İş Akışlarında Hata Ayıklama](debugging.md) yan görevine bakın.

#### 1.4.2. `COUNT_LINES`'a bir çağrı ekleme

Artık süreç iş akışında kullanılabilir olduğundan, girdi dosyası üzerinde çalıştırmak için `COUNT_LINES` sürecine bir çağrı ekleyebiliriz.
...
Bu, bir süreç içinde dosya üzerinde uygun şekilde işlem yapabildiğimizi gösterir.
...
Nextflow sorunu hemen algıladı ve süreci başlatmadan önce durdurdu.

#### 1.5.2. Süreç düzeyinde hata

Nextflow'a girdiyi bir dosya olarak işlemesini söylemeyi unutabileceğimiz diğer yer süreç tanımındadır.

!!! warning "Uyarı"

    Bu testin doğru çalışması için, iş akışını bozuk durumunda tutun (`file()` yerine düz dize kullanarak).
    Süreçte `val` ile birleştirildiğinde, bu aşağıda gösterilen hatayı üretir.
...
Bu, süreç hata ayıklama bilgisi çıkarmak üzere ayarlandığı için hata hakkında çok fazla ayrıntı gösterir.
...
Bu nedenle süreç göreli dizeyi, `data/patientA_rep1_normal_R1_001.fastq.gz`'yi kullanmaya çalıştı, ancak bu dosya süreç çalışma dizini içinde mevcut değil.
...
- Süreç Girdi Sonuçları: Doğru dosya işleme, dosyaların süreçler tarafından kullanılmak üzere doğru şekilde hazırlanmasını ve erişilebilir olmasını sağlamak için dizeler yerine Path nesneleri gerektirir.
...
Bu hazırlanan dosya daha sonra yerel bir dosya olarak işlenebilir ve ilgili süreç dizinine sembolik olarak bağlanabilir.
...
!!! note "Not"
    ...
    Bu ilkenin tek önemli istisnası, HTTPS birden fazla dosyayı listeleyemediği için HTTPS ile glob desenleri veya dizin yolları kullanamamanızdır; bu nedenle tam dosya URL'lerini belirtmeniz gerekir.
...
- Nextflow, bu yollar süreçlere beslendiği sürece verileri otomatik olarak indirir ve doğru yere hazırlar
...
## 3. `fromPath()` kanal factory'sini kullanma

Şimdiye kadar tek seferde tek bir dosyayla çalışıyorduk, ancak Nextflow'da genellikle işlenecek birden fazla girdi dosyasıyla bir girdi kanalı oluşturmak isteyeceğiz.
...
!!! tip "İpucu: `file()` vs `channel.fromPath()` ne zaman kullanılır"

    - Doğrudan manipülasyon için tek bir Path nesnesine ihtiyacınız olduğunda (bir dosyanın var olup olmadığını kontrol etme, özelliklerini okuma veya tek bir süreç çağrısına geçirme) `file()` kullanın
    - Özellikle glob desenleriyle birden fazla dosya tutabilen veya dosyalar birden fazla süreçten geçecekse bir kanala ihtiyacınız olduğunda `channel.fromPath()` kullanın

[`channel.fromPath()`](https://www.nextflow.io/docs/latest/reference/channel.html#frompath) burada devreye girer: bir veya daha fazla statik dosya dizesinden ve glob desenlerinden bir kanal oluşturmak için ihtiyacımız olan tüm işlevselliği bir araya getiren kullanışlı bir kanal factory'si.

### 3.1. Kanal factory'sini ekleme
...
`channel.fromPath()` kullanmak, bir dosya listesiyle doldurulmuş yeni bir kanal oluşturmanın uygun bir yoludur.

### 3.2. Kanaldaki dosyaların özelliklerini görüntüleme

Kanal factory'sini ilk kullanışımızda, kodu basitleştirdik ve sadece dosya adını yazdırdık.
...
İşte, öncekiyle aynı sonuçlar ancak şimdi dosya bir kanalda, bu yüzden daha fazla ekleyebiliriz.
...
Şimdi tek yapmamız gereken kanal factory'sindeki dosya yolunu bu glob desenini kullanacak şekilde güncellemektir:
...
Gördüğünüz gibi, şimdi kanalımızda iki Path nesnemiz var; bu, Nextflow'un dosya adı genişletmesini doğru şekilde yaptığını ve her iki dosyayı da beklendiği gibi yükleyip işlediğini gösteriyor.
...
- `channel.fromPath()` bir desenle eşleşen dosyalarla bir kanal oluşturur
- Her dosya kanalda ayrı bir öğe olarak yayılır
...
- `.view()` metodu kanal içeriklerinin incelenmesine olanak tanır
...
Şimdi kanaldaki her öğenin demeti meta veri listesini (_örneğin_ `[patientA, rep1, normal, R1, 001]`) ve orijinal dosya nesnesini içeriyor.
...
Nextflow, paylaşılan bir adlandırma desenine göre dosyaları otomatik olarak gruplayan `channel.fromFilePairs()` adlı eşleştirilmiş dosyalarla çalışmak için özel bir kanal factory'si sağlar. Bu, eşleştirilmiş dosyaları daha az çabayla daha sıkı bir şekilde ilişkilendirmenize olanak tanır.

Bundan yararlanmak için iş akışımızı değiştireceğiz.
İki adım alacak:

1. Kanal factory'sini `channel.fromFilePairs()`'a geçirme
2. Meta veriyi çıkarma ve eşleme

### 5.1. Kanal factory'sini `channel.fromFilePairs()`'a geçirme
...
Kanal factory'sini değiştirdik ve dosya eşleştirme desenini uyarladık; bunu yaparken map işlemini yorumladık.
...
Bunun nedeni kanal factory'sini değiştirmiş olmamızdır.
Şimdiye kadar, orijinal girdi kanalı yalnızca dosya yollarını içeriyordu.
Yaptığımız tüm meta veri manipülasyonu aslında kanal içeriklerini etkilemedi.

Artık `.fromFilePairs` kanal factory'sini kullandığımıza göre, ortaya çıkan kanalın içerikleri farklı.
Sadece bir kanal öğesi görüyoruz; iki öğe içeren bir demetten oluşuyor: iki dosya tarafından paylaşılan `simpleName` kısmı, bir tanımlayıcı görevi görüyor ve iki dosya nesnesini içeren bir demet, `id, [ file1, file2 ]` formatında.
...
`COUNT_LINES`'ı süreci değiştirmeden aynı şekilde çalıştırmak istersek, dosya yollarını çıkarmak için bir eşleme işlemi uygulamamız gerekir.
Ancak bunu yapmayacağız, çünkü nihai hedefimiz dosya çiftlerini uygun şekilde işleyen farklı bir süreç, `ANALYZE_READS` kullanmaktır.
...
İşte burada: çıktı demetinin ilk pozisyonunda meta veri map'imiz (`[id:patientA, replicate:1, type:normal]`) var, ardından amaçlandığı gibi eşleştirilmiş dosyalar demeti geliyor.
...
- Eşleştirilmiş dosyalar `[id, [file1, file2]]` demetleri olarak gruplanabilir
...
Şimdi, bir Nextflow süreci içinde dosya işlemlerini nasıl kullanacağımızı pekiştirmek için tüm bunları basit bir süreçte bir araya getirelim.

Size, bir meta veri demeti ve bir çift girdi dosyası alan ve bunları analiz eden `ANALYZE_READS` adlı önceden yazılmış bir süreç modülü sağlıyoruz.
...
### 6.1. Süreci içe aktarma ve kodu inceleme
...
### 6.2. İş akışında süreci çağırma

Artık süreç iş akışında kullanılabilir olduğundan, çalıştırmak için `ANALYZE_READS` sürecine bir çağrı ekleyebiliriz.

Örnek verilerimizde çalıştırmak için iki şey yapmamız gerekecek:

1. Yeniden eşlenen kanala bir ad verme
2. Sürece bir çağrı ekleme

#### 6.2.1. Yeniden eşlenen girdi kanalını adlandırma

Daha önce eşleme manipülasyonlarını doğrudan girdi kanalına uygulamıştık.
Yeniden eşlenen içerikleri `ANALYZE_READS` sürecine beslemek için (ve bunu açık ve okunması kolay bir şekilde yapmak için) `ch_samples` adlı yeni bir kanal oluşturmak istiyoruz.

Bunu [`set`](https://www.nextflow.io/docs/latest/reference/operator.html#set) operatörünü kullanarak yapabiliriz.

Ana iş akışında, `.view()` operatörünü `.set { ch_samples }` ile değiştirin ve kanala adla başvurabildiğimizi test eden bir satır ekleyin.
...
Bu, artık kanala adla başvurabildiğimizi doğrular.

#### 6.2.2. Veri üzerinde süreci çağırma
...
Bu süreç çıktılarını bir `results` dizinine yayımlamak üzere ayarlanmış, bu yüzden oraya bakın.
...
`channel.fromFilePairs()`'ın girdi olarak bir _glob_ kabul ettiğini unutmayın; bu da desenle eşleşen herhangi sayıda dosyayı kabul edebileceği anlamına gelir.
...
### 6.4. Yayımlanan dosyaları benzersiz yapma

Hasta meta verisine erişimimiz olduğundan, bunu dizin yapısında veya dosya adlarının kendisinde farklılaştırıcı meta veri ekleyerek yayımlanan dosyaları benzersiz yapmak için kullanabiliriz.
...
2. Hasta özelliklerine göre süreçlerde kararlar verme
...
- Demetlerdeki meta veri, sonuçların yapılandırılmış organizasyonuna olanak tanır
- Bu yaklaşım, açık veri kaynağı ile bakımı kolay iş akışları oluşturur
- Süreçler meta veri ve dosya demetlerini girdi olarak alabilir
- `tag` yönergesi, yürütme günlüklerinde süreç tanımlama sağlar
...
3.  **`fromPath()` kanal factory'si ile dosyaları yükleme:** `channel.fromPath()` ile dosya desenlerinden kanallar oluşturduk ve nesne türleri dahil dosya özelliklerini görüntüledik.
...
6.  **Süreçlerde Dosya İşlemlerini Kullanma:** Doğru girdi işleme ile dosya işlemlerini Nextflow süreçlerine entegre ettik, çıktıları meta veriye göre organize etmek için `publishDir` kullandık.

    - Süreç girdileriyle bir meta map ilişkilendirme
