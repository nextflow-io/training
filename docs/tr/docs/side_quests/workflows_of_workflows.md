...
- Yeni pipeline'lar oluşturmak için workflow'ları karıştırma ve eşleştirme
...
Çalışıyor! Adlandırılmış greeting workflow'unu, adlandırılmamış bir giriş `workflow` bloğuna sahip bir ana workflow'a sardık. Ana workflow, `GREETING_WORKFLOW` workflow'unu neredeyse (tam olarak değil) bir process gibi kullanıyor ve `names` kanalını argüman olarak geçiriyor.
...
_Ancak, workflow'ları çağırmanın process'leri çağırmak gibi olduğunu ancak aslında aynı şey olmadığını not etmek önemlidir. Örneğin, N boyutunda bir kanal ile çağırarak bir workflow'u N kez çalıştıramazsınız - workflow'a N boyutunda bir kanal geçirmeniz ve dahili olarak yinelemeniz gerekir._
...
1.  **Workflow yapısı**: `take:` ve `emit:` sözdizimini kullanarak her workflow için net girdiler ve çıktılar tanımladık, bileşenler arasında iyi tanımlanmış arayüzler oluşturduk ve workflow mantığını `main:` bloğu içine sardık.
    ...
    workflow EXAMPLE_WORKFLOW {
        ...
        main:
            ...
            // Process'ler çağrılır ve kanallar manipüle edilir
