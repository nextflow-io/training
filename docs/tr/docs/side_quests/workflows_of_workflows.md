...
Bir pipeline geliştirirken, kendinizi farklı veri türleri veya analiz adımları için benzer süreç dizileri oluştururken bulursunuz. Bu süreç dizilerini kopyalayıp yapıştırarak bakımı zor çoğaltılmış kod oluşturabilirsiniz; ya da anlaşılması ve değiştirilmesi güç tek bir devasa iş akışı oluşturabilirsiniz.
...
- Temel Nextflow kavramları ve mekanizmalarını (süreçler, kanallar, operatörler, modüller) rahatça kullanabiliyor olmalısınız.
...
## 1. Greeting Workflow'unu Oluşturma
...
workflow {
    ...
    // Süreçleri zincirle: doğrula -> selamlama oluştur -> zaman damgası ekle
...
Bu, 'Hello Nextflow' eğitiminde gördüklerinize benzer bir yapıya sahip, bağımsız olarak test edebileceğimiz tam bir workflow'dur. Şimdi deneyelim:
...
Bu beklendiği gibi çalışıyor; ancak composable hale getirmek için değiştirmemiz gereken birkaç şey var.
...
Greeting workflow'unu bu yapıya uygun hale getirelim. Kodu aşağıdaki gibi değiştirin:
...
workflow GREETING_WORKFLOW {
    take:
        names_ch        // İsimler içeren girdi kanalı
...
Workflow içeriği de `main:` bloğunun içine yerleştirilmiştir. Ayrıca `names_ch` girdi kanalı bildirimini kaldırdığımızı not edin; çünkü artık workflow'a argüman olarak geçiriliyor.
...
Bu size başka bir yeni kavram hakkında bilgi veriyor: 'giriş workflow'u'. Giriş workflow'u, bir Nextflow betiği çalıştırdığınızda çağrılan workflow'dur. Varsayılan olarak Nextflow, mevcut olduğunda adlandırılmamış bir workflow'u giriş workflow'u olarak kullanır; şimdiye kadar yaptığınız şey de buydu. Bu tür workflow blokları şöyle başlar:
...
`take:`/`emit:` sözdizimini workflow'u doğrudan çağırabilmek için eklemedik; diğer workflow'larla compose edebilmek için ekledik. Çözüm, adlandırılmış workflow'umuzu içe aktaran ve çağıran adlandırılmamış bir giriş workflow'una sahip bir ana betik oluşturmaktır.
...
Bu dosyadaki workflow girişinin adlandırılmamış olduğunu not edin; çünkü onu bir giriş workflow'u olarak kullanacağız.
...
- **Giriş Noktaları**: Nextflow'un bir betiği çalıştırmak için adlandırılmamış bir giriş workflow'una ihtiyaç duyduğunu anlama
...
workflow TRANSFORM_WORKFLOW {
    take:
        input_ch         // Mesajlar içeren girdi kanalı
...
_Ancak, workflow'ları çağırmanın süreçleri çağırmak gibi olduğunu ama aslında aynı şey olmadığını not etmek önemlidir. Örneğin, N boyutunda bir kanalla çağırarak bir workflow'u N kez çalıştıramazsınız; workflow'a N boyutunda bir kanal geçirmeniz ve dahili olarak yinelemeniz gerekir._

Bu teknikleri kendi çalışmanıza uygulamak, bakımı kolay ve ölçeklenebilir kalırken karmaşık biyoinformatik görevleri işleyebilen daha sofistike Nextflow pipeline'ları oluşturmanıza olanak tanıyacaktır.
...
1.  **Workflow yapısı**: `take:` ve `emit:` sözdizimini kullanarak her workflow için net girdiler ve çıktılar tanımladık; bileşenler arasında iyi tanımlanmış arayüzler oluşturduk ve workflow mantığını `main:` bloğu içine sardık.
    ...
    workflow EXAMPLE_WORKFLOW {
        take:
            // Girdi kanalları burada bildirilir
            input_ch

        main:
            // Workflow mantığı buraya gelir
            // Süreçler çağrılır ve kanallar manipüle edilir
            result_ch = SOME_PROCESS(input_ch)

        emit:
            // Çıktı kanalları burada bildirilir
            output_ch = result_ch
    }
    ```
