...
`#!groovy { row -> return row }` closure'ı bir `row` parametresi alır (herhangi bir isim olabilir: `item`, `sample` vb.).
...
!!! note "Not"
    ...
    **Map'ler ve Meta Veri**: Map'ler, Nextflow'da meta veriyle çalışmanın temelini oluşturur. Meta veri map'leriyle çalışma hakkında daha ayrıntılı bir açıklama için [Meta Veriyle Çalışma](../metadata/index.md) yan görevine bakın.
...
!!! tip "İpucu: Spread ve Collect'i Ne Zaman Kullanmalı"
    ...
    - **Collect kullanın** dönüşümler veya karmaşık mantık için: `#!groovy samples.collect { it.id.toUpperCase() }`, `#!groovy samples.collect { [it.id, it.quality > 40] }`
...
Bunu çalıştırırsanız bir hata fark edeceksiniz; Nextflow, `#!groovy ${USER}`'ı mevcut olmayan bir Nextflow değişkeni olarak yorumlamaya çalışır.
...
- **Değişken enterpolasyonu**: Nextflow'un dizeleri ne zaman yorumladığını, kabuğun ne zaman yorumladığını anlamak
  - `#!groovy ${var}` - Nextflow değişkenleri (iş akışı derleme zamanında Nextflow tarafından enterpolasyon yapılır)
  - `#!groovy \${var}` - Kabuk ortam değişkenleri (kaçırılmış, çalışma zamanında bash'e iletilir)
  - `#!groovy \$(cmd)` - Kabuk komut ikamesi (kaçırılmış, çalışma zamanında bash tarafından çalıştırılır)
...
`#!groovy { meta.depth > 40000000 ? 2 : 1 }` closure'ı **üçlü operatörü** (Bölüm 1.1'de ele alındı) kullanır ve her görev için değerlendirilerek örnek başına kaynak tahsisine olanak tanır. Yüksek derinlikli örnekler (>40M okuma) 2 CPU alırken diğerleri 1 CPU alır.
