...
İş akışınızın her bir parçasının yapması gerekeni sistematik olarak test edebilmek, tekrarlanabilirlik ve uzun vadeli bakım için kritik öneme sahiptir ve geliştirme sürecinde büyük bir yardımcı olabilir.

Test etmenin neden bu kadar önemli olduğu hakkında biraz konuşalım. Bir iş akışı geliştiriyorsanız, yapacağınız ilk şeylerden biri geçerli olduğunu bildiğiniz ve sonuç üretmesi gereken bazı test verilerini almaktır. İlk süreci pipeline'a ekler ve girdilerinize bağlayarak çalışmasını sağlarsınız. Ardından, her şeyin çalıştığını kontrol etmek için test verileri üzerinde çalıştırırsınız. Bunun işe yaradığını varsayarsak, bir sonraki sürece geçer ve test verilerini tekrar çalıştırırsınız. Memnun olduğunuz bir pipeline elde edene kadar bu işlemi tekrarlarsınız.

Sonra, belki `--skip_process` gibi basit bir doğru veya yanlış parametresi eklersiniz. Şimdi pipeline'ı iki kez çalıştırmalısınız, her parametreyle bir kez, beklendiği gibi çalıştığından emin olmak için. Ama bekleyin, `--skip_process`'in gerçekten süreci atladığını nasıl kontrol ederiz? Çıktıları incelemeli veya log dosyalarını kontrol etmeliyiz! Bu zahmetli ve hataya açıktır.

İş akışınızı geliştirdikçe, her iterasyonu manuel olarak test etmenin yavaş ve hataya açık olacağı kadar karmaşık hale gelecektir. Ayrıca, bir hata bulursanız, hatanın pipeline'ınızda tam olarak nereden kaynaklandığını belirlemek çok zor olacaktır. İşte burada test devreye girer.
...
- **Şeffaflık**: Testler, bir pipeline'ın nerede başarısız olduğunu gösterir ve sorunu izlemeyi kolaylaştırır. Ayrıca bir sürecin veya iş akışının nasıl çalıştırılacağını gösteren bir belgeleme biçimi olarak işlev görürler
- **Hız**: Testler otomatik olduğu için çok hızlı ve tekrar tekrar çalıştırılabilir. Yeni hatalar ekleme korkusu olmadan hızlı bir şekilde iterasyon yapabilirsiniz
...
3. **Pipeline düzeyinde testler**: Pipeline'ın bir bütün olarak test edilmesi için
4. **Performans testleri**: Pipeline'ın hızı ve verimliliği için
5. **Stres testleri**: Pipeline'ın sınırlarını belirlemek için aşırı koşullar altında performansını değerlendirme

Bireysel süreçleri test etmek, diğer dillerdeki birim testlere benzer. İş akışını veya tüm pipeline'ı test etmek, diğer dillerde entegrasyon testleri olarak adlandırılan, bileşenlerin etkileşimlerini test ettiğimiz şeye benzer.

[**nf-test**](https://www.nf-test.com/), modül, iş akışı ve pipeline düzeyinde testler yazmanıza olanak tanıyan bir araçtır. Kısacası, pipeline'ın her bir parçasının _izole bir şekilde_ beklendiği gibi çalıştığını sistematik olarak kontrol etmenizi sağlar.

### Öğrenme hedefleri

Bu yan görevde, pipeline için bir iş akışı düzeyinde test ve çağırdığı üç süreç için modül düzeyinde testler yazmak üzere nf-test kullanmayı öğreneceksiniz.

Bu yan görevin sonunda, aşağıdaki teknikleri etkili bir şekilde kullanabileceksiniz:

- Projenizde nf-test'i başlatma
- Modül düzeyinde ve iş akışı düzeyinde testler oluşturma
- Yaygın doğrulama türlerini ekleme
- Snapshot'ların ne zaman kullanılacağını ve içerik doğrulamalarının ne zaman kullanılacağını anlama
- Tüm bir proje için testleri çalıştırma

Bu beceriler, pipeline projelerinizde kapsamlı bir test stratejisi uygulamanıza yardımcı olacak ve bunların daha sağlam ve sürdürülebilir olmasını sağlayacaktır.
...
- Temel Nextflow kavramlarını ve mekanizmalarını (süreçler, kanallar, operatörler, dosyalarla çalışma, meta veriler) rahatça kullanabiliyor olmalısınız
...
??? example "Hello Nextflow iş akışı ne yapar?"
    ...
    [Hello Nextflow](../hello_nextflow/index.md) eğitimini yapmadıysanız, bu basit iş akışının ne yaptığına dair hızlı bir genel bakış:

    İş akışı, selamlamalar içeren bir CSV dosyası alır, bunlar üzerinde art arda dört dönüşüm adımı çalıştırır ve selamlamaları söyleyen eğlenceli bir karakterin ASCII resmini içeren tek bir metin dosyası çıktısı verir.
    ...
    Sonuçlar `results/` adlı bir dizinde yayınlanır ve pipeline'ın nihai çıktısı (varsayılan parametrelerle çalıştırıldığında) büyük harfli selamlamaları söyleyen bir karakterin ASCII sanatını içeren düz bir metin dosyasıdır.
...
```console title="İş akışını çalıştırmanın sonucu"
...
"Bekle, ne? Sadece iş akışını çalıştırdım ve işe yaradı! Bu nasıl bir test?"

İyi soru!

Az önce ne olduğunu inceleyelim.

İş akışını varsayılan parametrelerle çalıştırdınız, çalıştığını doğruladınız ve sonuçlardan memnunsunuz. Bu, test etmenin özüdür. Hello Nextflow eğitim kursunu tamamladıysanız, her bölüme başlangıç noktası olarak kullandığımız iş akışını çalıştırarak, her şeyin doğru şekilde ayarlandığını doğruladığımızı fark etmişsinizdir.

Yazılım testi esasen bu süreci bizim için yapar.

#### Görevi gözden geçirin

Göreviniz, bu iş akışına nf-test kullanarak standartlaştırılmış testler eklemektir, böylece herhangi bir değişiklik yapılması durumunda her parçanın beklendiği gibi çalışmaya devam ettiğini doğrulamak kolay olur.
...
`nf-test`, nf-test dosyaları oluşturmak için bir dizi araçla birlikte gelir ve bu da işin çoğunu bizim için yapar. Bunlar `generate` alt komutu altında gelir. Pipeline için bir test oluşturalım:
...
Bu, `tests` dizini içinde bir `main.nf.test` dosyası oluşturacaktır. Bu bizim pipeline düzeyinde test dosyamızdır. `tree tests/` komutunu çalıştırırsanız şöyle bir şey görmelisiniz:
...
`main.nf.test` dosyası bizim pipeline düzeyinde test dosyamızdır. Açalım ve içeriğine bakalım.
...
`nextflow_pipeline` bloğu, tüm pipeline düzeyinde testler için giriş noktasıdır. Aşağıdakileri içerir:
...
- `then`: Yapılması gereken doğrulamalar. Bu, pipeline'ın beklenen sonuçlarını içerir.

Sade Türkçe ile, testin mantığı şöyle okunur:
"**Bu** _parametreler_ bu _pipeline_'a sağlandığında, **o zaman** bu sonuçları görmeyi bekliyoruz."

Bu işlevsel bir test değil, bir sonraki bölümde bunu işlevsel bir teste nasıl dönüştüreceğimizi göstereceğiz.

### Test Adları Hakkında Bir Not

Yukarıdaki örnekte, sadece pipeline'ın başarıyla çalışıp çalışmadığını kontrol eden temel bir test için uygun olan varsayılan "Should run without failures" adını kullandık. Ancak, daha spesifik test durumları ekledikçe, gerçekte neyi test ettiğimizi belirten daha açıklayıcı adlar kullanmalıyız. Örneğin:
...
Daha sonra daha fazla doğrulama ve spesifik test durumları eklediğimizde, her testin neyi doğruladığını açık hale getirmek için bu daha açıklayıcı adları kullanacağız.

### 1.2. Testi çalıştırın

Testi çalıştırarak ne olduğunu görelim.
...
nf-test'in pipeline'ın başarısız olduğunu nasıl bildirdiğine ve Nextflow'dan gelen hata mesajını nasıl sağladığına dikkat edin:
...
Testte pipeline'ın köküne nasıl işaret edeceğimizi merak ediyor olabilirsiniz. Bu yaygın bir durum olduğundan, nf-test hayatımızı kolaylaştırmak için bir dizi global değişkene sahiptir. Tam listeyi [buradan](https://www.nf-test.com/docs/testcases/global_variables/) bulabilirsiniz, ancak şimdilik pipeline projesinin kökü anlamına gelen `projectDir` değişkenini kullanacağız.
...
Testi tekrar çalıştırarak işe yarayıp yaramadığını görelim.
...
### 1.3. Doğrulamalar ekleyin

Basit bir kontrol, pipeline'ımızın beklediğimiz tüm süreçleri çalıştırdığından ve hiçbirini sessizce atlamadığından emin olmaktır. Pipeline'ımızın 6 süreç çalıştırdığını hatırlayın, 3 selamlama için biri `sayHello` ve biri `convertToUpper` olarak adlandırılır.

Pipeline'ın beklenen sayıda süreci çalıştırdığını kontrol etmek için testimize bir doğrulama ekleyelim. Ayrıca test adımızı test ettiğimiz şeyi daha iyi yansıtacak şekilde güncelleyeceğiz.
...
Test adı artık gerçekte neyi doğruladığımızı daha iyi yansıtıyor - sadece pipeline'ın başarısız olmadan çalışması değil, aynı zamanda beklenen sayıda süreci çalıştırması.
...
```console title="Pipeline doğrulamalarla başarılı"
🚀 nf-test 0.9.3
https://www.nf-test.com
(c) 2021 - 2024 Lukas Forer and Sebastian Schoenherr
...
Başarılı! Pipeline başarıyla çalışıyor ve test geçiyor. Şimdi genel durumun yanı sıra pipeline'ın ayrıntılarını da test etmeye başladık.

### 1.4. Çıktıyı test edin

Çıktı dosyasının oluşturulduğunu kontrol etmek için testimize bir doğrulama ekleyelim. Sonuçları yorumlamayı kolaylaştırmak için bilgilendirici bir adla ayrı bir test olarak ekleyeceğiz.
...
```console title="Pipeline dosya doğrulamalarıyla başarılı"
> nf-test test tests/main.nf.test
...
Başarılı! Testler geçiyor çünkü pipeline başarıyla tamamlandı, doğru sayıda süreç çalıştı ve çıktı dosyaları oluşturuldu. Bu aynı zamanda testleriniz için bu bilgilendirici adları sağlamanın ne kadar yararlı olduğunu da göstermelidir.

Bu sadece yüzey, pipeline'ın ayrıntılarını kontrol etmek için doğrulamalar yazmaya devam edebiliriz, ancak şimdilik pipeline'ın iç kısımlarını test etmeye geçelim.
...
Pipeline'ın her parçası için test yazmak zorunda değiliz, ancak ne kadar çok testimiz olursa pipeline hakkında o kadar kapsamlı olabiliriz ve beklendiği gibi çalıştığından o kadar emin olabiliriz. Bu bölümde pipeline'daki her iki süreci de bireysel birimler olarak test edeceğiz.
...
Dosyayı açalım ve içeriğine bakalım.
...
Testi çalıştırarak işe yarayıp yaramadığını görelim.
...
Test başarısız oluyor çünkü `sayHello` süreci 1 girdi bildiriyor ancak 0 argümanla çağrıldı. Sürece bir girdi ekleyerek bunu düzeltelim. [Hello Workflow](../hello_nextflow/03_hello_workflow.md) bölümünden (ve yukarıdaki ısınma bölümünden) hatırlayın ki `sayHello` sürecimiz sağlamamız gereken tek bir değer girdisi alır. Ayrıca test adını test ettiğimiz şeyi daha iyi yansıtacak şekilde düzeltmeliyiz.
...
Başarılı! Test geçiyor çünkü `sayHello` süreci başarıyla çalıştı ve çıktı oluşturuldu.

### 2.2. Test tarafından oluşturulan snapshot'ı inceleyin

`tests/main.sayhello.nf.test` dosyasına bakarsak, doğrulama bloğunda bir `snapshot()` yöntemi kullandığını görebiliriz:

```groovy title="tests/main.sayhello.nf.test"
assert snapshot(process.out).match()
```

Bu, nf-test'e `sayHello` sürecinin çıktısının bir snapshot'ını oluşturmasını söylüyor. Snapshot dosyasının içeriğine bakalım.
...
Bu, açıkça test ettiğimiz `sayHello` süreci tarafından oluşturulan çıktıları temsil eder. Testi yeniden çalıştırırsak, program yeni çıktının başlangıçta kaydedilen çıktıyla eşleşip eşleşmediğini kontrol edecektir. Bu, süreç çıktılarının değişmediğini test etmenin hızlı, basit bir yoludur, bu yüzden nf-test bunu varsayılan olarak sağlar.

!!!warning

    Bu, orijinal çalıştırmada kaydettiğimiz çıktının doğru olduğundan emin olmamız gerektiği anlamına gelir!

Gelecekteki geliştirme sürecinde, kodda çıktının farklı olmasına neden olan bir şey değişirse, test başarısız olacak ve değişikliğin beklenip beklenmediğini belirlememiz gerekecek.

- Kodda bir şeyin bozulduğu ortaya çıkarsa, düzeltmemiz gerekecek ve düzeltilen kodun testi geçmesi beklenir.
- Beklenen bir değişiklikse (örneğin, araç geliştirildi ve sonuçlar daha iyi) o zaman yeni çıktıyı eşleştirilecek referans olarak kabul etmek için snapshot'ı güncellememiz gerekecek. nf-test bu amaç için `--update-snapshot` parametresine sahiptir.

Testi tekrar çalıştırabiliriz ve testin geçmesi gerekir:

```console title="nf-test süreci snapshot ile başarılı"
> nf-test test tests/main.sayhello.nf.test
...
Başarılı! Test geçiyor çünkü `sayHello` süreci başarıyla çalıştı ve çıktı snapshot ile eşleşti.

### 2.3. Snapshot'lara Alternatif: Doğrudan İçerik Doğrulamaları

Snapshot'lar çıktıdaki herhangi bir değişikliği yakalamak için harika olsa da, bazen tüm dosyanın eşleşmesi konusunda bu kadar katı olmadan belirli içeriği doğrulamak istersiniz. Örneğin:

- Çıktının bazı kısımları değişebilir (zaman damgaları, rastgele kimlikler vb.) ancak belirli anahtar içerik mevcut olmalıdır
- Çıktıda belirli kalıpları veya değerleri kontrol etmek istediğinizde
- Testi başarının ne olduğu konusunda daha açık hale getirmek istediğinizde

İşte testimizi belirli içeriği kontrol edecek şekilde nasıl değiştirebileceğimiz:
...
nf-test'in süreç çıktılarını liste listesi olarak gördüğünü unutmayın, bu nedenle `process.out[0][0]` bu süreçten ilk kanal öğesinin (veya 'emisyonun') ilk kısmını alıyor.
...
- Daha karmaşık doğrulamalara izin verir (regex kalıpları, sayısal karşılaştırmalar vb.)
...
`tests/main.converttoupper.nf.test` dosyasını açalım ve içeriğine bakalım:
...
Bu, `sayHello` sürecine benzer bir testtir, ancak `convertToUpper` sürecini test ediyor. Tıpkı `sayHello` gibi, `convertToUpper` süreci de tek bir yol girdisi alır, ancak biz bir tane belirtmedik, bu yüzden bunun başarısız olacağını biliyoruz.
...
- Test etmek için özel bir dosya oluşturabiliriz
- Mevcut data/greetings.csv dosyasını yeniden kullanabiliriz
- Test içinde anında oluşturabiliriz

Şimdilik, pipeline düzeyinde testte kullandığımız örneği kullanarak mevcut data/greetings.csv dosyasını yeniden kullanalım. Daha önce olduğu gibi, testi test ettiğimiz şeyi daha iyi yansıtacak şekilde adlandırabiliriz, ancak bu sefer (diğer süreçte yaptığımız gibi belirli dizeleri kontrol etmek yerine) içeriği 'snapshot' ile bırakalım.
...
Not: `tests/main.converttoupper.nf.test.snap` konumunda `convertToUpper` süreci için bir snapshot dosyası oluşturduk. Testi tekrar çalıştırırsak, nf-test'in tekrar geçtiğini görmeliyiz.
...
Bir Nextflow süreci için testler yazmayı ve bunları çalıştırmayı biliyorsunuz.

### Sırada ne var?

Her şey için testleri aynı anda nasıl çalıştıracağınızı öğrenin!
...
Tüm depo üzerinde nf-test çalıştıralım.

### 3.1. Tüm depo üzerinde nf-test çalıştırın

`nf-test test` komutunu çalıştırarak tüm depo üzerinde nf-test çalıştırabiliriz.

```bash
nf-test test .
```

Not: Mevcut dizinimizden her şeyi çalıştırmak için sadece `.` kullanıyoruz. Bu her testi içerecektir!

```console title="nf-test depo başarılı"
> nf-test test .
...
Buna bakın! Tek bir komutla her süreç için 1 ve tüm pipeline için 2 olmak üzere 4 test çalıştırdık. Bunun büyük bir kod tabanında ne kadar güçlü olduğunu hayal edin!

---

## Özet

Bu yan görevde, bireysel süreçler için testler oluşturmak ve çalıştırmak için nf-test'in özelliklerinden yararlanmayı ve tüm pipeline için uçtan uca testler yapmayı öğrendiniz.
Artık çıktı doğrulamasına yönelik iki ana yaklaşımın, snapshot'ların ve doğrudan içerik doğrulamalarının farkındasınız ve hangisinin ne zaman kullanılacağını biliyorsunuz.
Ayrıca testleri tek tek veya tüm bir proje için nasıl çalıştıracağınızı da biliyorsunuz.

Bu teknikleri kendi çalışmanızda uygulamak şunları sağlamanıza olanak tanır:
...
- Sorunların hızlı bir şekilde tanımlanması ve düzeltilmesi
...
3. Çıktı doğrulamasına yönelik iki yaklaşım:
   - Tam çıktı doğrulaması için snapshot'ları kullanma
   - Belirli içerik kontrolleri için doğrudan içerik doğrulamalarını kullanma
...
- Testlerinize daha kapsamlı doğrulamalar ekleyin
...
**Unutmayın:** Testler, kodunuzun nasıl davranması gerektiğinin canlı belgesidir. Ne kadar çok test yazarsanız ve doğrulamalarınız ne kadar spesifik olursa, pipeline'ınızın güvenilirliğinden o kadar emin olabilirsiniz.
