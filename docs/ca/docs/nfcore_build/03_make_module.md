...
Després d'això, et mostrarem com utilitzar la creació de mòduls basada en plantilles per treballar de manera més eficient en el futur.

??? info "Com començar des d'aquesta secció"

    Aquesta secció assumeix que has completat la [Part 2: Utilitzar un mòdul nf-core](./02_use_module.md) i has integrat el mòdul `FIND_CONCATENATE` al teu pipeline.

    Si no vas completar la Part 2 o vols començar de nou per a aquesta part, pots utilitzar la solució `core-hello-part2` com a punt de partida.
    Executa aquestes comandes des de dins del directori `nfcore-build/`:
    ...
    Això et proporciona un pipeline amb el mòdul `FIND_CONCATENATE` ja integrat.
    Pots comprovar que s'executa correctament executant la següent comanda:
...
En aquesta secció, aplicarem les convencions nf-core al mòdul local `cowpy` del teu pipeline `core-hello`, transformant-lo en un mòdul que segueix els estàndards de la comunitat nf-core.

Aquest és el codi actual del mòdul de procés `cowpy`:

```groovy title="core-hello/modules/local/cowpy.nf" linenums="1"
// Genera art ASCII amb cowpy (https://github.com/jeffbuttars/cowpy)
process cowpy {
...
!!! warning "Directori de treball"
    ...
    Assegura't que estàs al directori `core-hello` (l'arrel del teu pipeline) per a totes les edicions de fitxers i execucions de comandes d'aquesta secció.
...
Necessitem fer tres conjunts de canvis:
...
Obre el fitxer del mòdul `cowpy.nf` (sota `core-hello/modules/local/`) i modifica el nom del procés a majúscules:
...
Si el nom del procés estigués compost per diverses paraules, per exemple si tinguéssim un procés anomenat `MyCowpyTool` originalment en camel case, la convenció nf-core seria utilitzar guions baixos per separar-les, resultant en `MY_COWPY_TOOL`.

#### 1.1.2. Actualitzar la declaració d'importació del mòdul

Els noms de procés distingeixen entre majúscules i minúscules, així que ara que hem canviat el nom del procés, necessitem actualitzar la declaració d'importació del mòdul en conseqüència a la capçalera del workflow de `hello.nf`:
...
=== "Després"
    ...
    //
    // Agrupa i desa les versions del programari
    //
...
Assegura't de fer **ambdós** canvis, altrament obtindràs un error quan executis això.
...
??? success "Sortida de la comanda"
    ...
    Core Nextflow options
      ...
      launchDir                 : /workspaces/training/nfcore-build/core-hello
      workDir                   : /workspaces/training/nfcore-build/core-hello/work
      projectDir                : /workspaces/training/nfcore-build/core-hello
      userName                  : root
      profile                   : test,docker
      configFiles               : /workspaces/training/nfcore-build/core-hello/nextflow.config
...
A la versió actual del pipeline `core-hello`, estem extraient el fitxer de la tupla de sortida de `FIND_CONCATENATE` per passar-lo a `COWPY`, com es mostra a la meitat superior del diagrama següent.

<figure class="excalidraw">
    --8<-- "docs/en/docs/nfcore_build/img/cowpy-inputs.svg"
</figure>
...
Per aconseguir-ho, necessitarem fer els següents canvis:
...
Torna al fitxer del mòdul `cowpy.nf` i modifica'l per acceptar tuples de metadades com es mostra a continuació.
...
Com pots veure, hem canviat tant l'**entrada principal** com la **sortida** a una tupla que segueix el patró `tuple val(meta), path(input_file)` introduït a la Part 2 d'aquesta formació.
Per a la sortida, també hem aprofitat per afegir `emit: cowpy_output` per donar un nom descriptiu al canal de sortida.

Ara que hem canviat el que el procés espera, necessitem actualitzar el que li proporcionem a la crida del procés.

#### 1.2.2. Actualitzar la crida al procés al workflow

La bona notícia és que aquest canvi simplificarà la crida al procés.
Ara que la sortida de `FIND_CONCATENATE` i l'entrada de `COWPY` tenen la mateixa 'forma', és a dir, ambdues consisteixen en una estructura `tuple val(meta), path(input_file)`, podem simplement connectar-les directament en lloc d'haver d'extreure el fitxer explícitament de la sortida del procés `FIND_CONCATENATE`.

Obre el fitxer del workflow `hello.nf` (sota `core-hello/workflows/`) i actualitza la crida a `COWPY` com es mostra a continuació.
...
Ara cridem `COWPY` sobre `FIND_CONCATENATE.out.file_out` directament.
...
Tècnicament això no és necessari, però és una bona pràctica referir-se a sortides amb nom sempre que sigui possible.
...
El pipeline hauria d'executar-se correctament, amb les metadades fluint ara de `FIND_CONCATENATE` a través de `COWPY`.
...
En lloc de declarar entrades de procés per a cada opció de l'eina, escrius el mòdul per referenciar `ext.args` en la construcció de la seva línia de comandes.
Llavors només cal configurar la variable `ext.args` per contenir els arguments i valors que vols utilitzar al fitxer `modules.config`, que consolida els detalls de configuració per a tots els mòduls.
...
Necessitarem fer els següents canvis:
...
Fem-ho.
Obre el fitxer del mòdul `cowpy.nf` (sota `core-hello/modules/local/`) i modifica'l per referenciar `ext.args` com es mostra a continuació.
...
Pots veure que hem fet tres canvis.
...
2. **Al bloc `script:`, hem afegit la línia `def args = task.ext.args ?: ''`.**
   ...
   Nota que mentre generalment ens referim a `ext.args`, aquest codi ha de referenciar `task.ext.args` per extreure la configuració `ext.args` a nivell de mòdul.
...
La sintaxi `withName:` assigna aquesta configuració només al procés `COWPY`, i `#!groovy ext.args = { "-c ${params.character}" }` simplement compon una cadena que inclourà el valor del paràmetre `character`.
Nota l'ús de claus, que indiquen a Nextflow que avaluï el valor del paràmetre en temps d'execució.

Té sentit? Afegim-ho.

Obre `conf/modules.config` i afegeix el codi de configuració dins del bloc `process {}` com es mostra a continuació.
...
Esperem que puguis imaginar tenir tots els mòduls d'un pipeline amb els seus `ext.args` especificats en aquest fitxer, amb els següents beneficis:
...
Com que el mòdul `COWPY` ja no requereix el paràmetre `character` com a entrada, necessitem actualitzar la crida del workflow en conseqüència.
...
Executa aquesta comanda utilitzant `kosh`, una de les opcions més... enigmàtiques:
...
Troba la sortida al navegador de fitxers o utilitza el hash de la tasca (la part `38/eb29ea` a l'exemple anterior) per mirar el fitxer de sortida:
...
Hauries de veure l'art ASCII mostrat amb el caràcter `kosh`, confirmant que la configuració `ext.args` ha funcionat!

??? info "(Opcional) Inspeccionar el fitxer de comandes"

    Si vols veure exactament com s'ha aplicat la configuració, pots inspeccionar el fitxer `.command.sh`:

    ```bash
    cat work/38/eb29ea*/.command.sh
    ```

    Veuràs la comanda `cowpy` amb l'argument `-c kosh`:
...
Pren-te un moment per pensar sobre el que hem aconseguit aquí.
...
!!! info "Info"
    ...
    El sistema `ext.args` té capacitats addicionals potents no cobertes aquí, incloent canviar valors d'arguments dinàmicament basant-se en metadades. Consulta les [especificacions de mòduls nf-core](https://nf-co.re/docs/guidelines/components/modules) per a més detalls.
...
Obre el fitxer del mòdul `cowpy.nf` (sota `core-hello/modules/local/`) i modifica'l per referenciar `ext.prefix` com es mostra a continuació.
...
1. **Al bloc `script:`, hem afegit la línia `#!groovy prefix = task.ext.prefix ?: "${meta.id}"`.**
   ...
   Nota que mentre generalment ens referim a `ext.prefix`, aquest codi ha de referenciar `task.ext.prefix` per extreure la configuració `ext.prefix` a nivell de mòdul.

2. **A la línia de comandes, hem reemplaçat `#!groovy cowpy-${input_file}` amb `#!groovy ${prefix}.txt`.**
   Aquí és on Nextflow injectarà el valor de `prefix` determinat per la línia anterior.

3. **Al bloc `output:`, hem reemplaçat `#!groovy path("cowpy-${input_file}")` amb `#!groovy path("${prefix}.txt")`.**
...
Ho farem configurant `ext.prefix` a `modules.config`, igual que vam fer per al paràmetre `character` amb `ext.args`, excepte que aquesta vegada el bloc `withName: 'COWPY' {}` ja existeix, i només necessitem afegir la següent línia:
...
Nota que un cop més utilitzem claus, aquesta vegada per indicar a Nextflow que avaluï el valor de `meta.id` en temps d'execució.
...
En cas que et preguntis, el closure `ext.prefix` té accés a la peça correcta de metadades perquè la configuració s'avalua en el context de l'execució del procés, on les metadades estan disponibles.
...
Mira la sortida al directori de resultats.
Hauries de veure el fitxer de sortida de cowpy amb el mateix nomenament que abans: `cowpy-test.txt`, basat en el nom de lot per defecte.
...
Alternativament, també pots provar d'executar això de nou amb un paràmetre `--batch` diferent especificat a la línia de comandes per satisfer-te que aquesta part encara és personalitzable sobre la marxa.

Això demostra com `ext.prefix` et permet mantenir la teva convenció de nomenament preferida mentre mantens la interfície del mòdul flexible.
...
- **Predictible**: És fàcil saber com s'anomenaran els fitxers de sortida

Força bé, oi?
Bé, hi ha un canvi més important que necessitem fer per millorar el nostre mòdul perquè s'ajusti a les directrius nf-core.

### 1.5. Centralitzar la configuració de publicació

Potser has notat que hem estat publicant sortides a dos directoris diferents:
...
- **`core-hello-results`** — El directori de sortida establert amb `--outdir` a la línia de comandes, que ha estat rebent els logs nf-core i els resultats publicats per `FIND_CONCATENATE`.
...
"Ei," podries dir, "`FIND_CONCATENATE` està enviant les seves sortides a `--outdir`. Potser hauríem de copiar la seva directiva `publishDir`?"

Sí, és una gran idea.

Excepte que no té una directiva `publishDir`. (Endavant, mira el codi del mòdul.)
...
Fem-ho.
Obre el fitxer del mòdul `cowpy.nf` (sota `core-hello/modules/local/`) i elimina la directiva `publishDir` com es mostra a continuació.
...
Mira el teu directori de treball actual.
...
Pots veure que Nextflow ha creat aquesta jerarquia de directoris basant-se en els noms del workflow i del mòdul.

!!! info "Info"

    Potser notes `hello_software_versions.yml` a `pipeline_info/`.
    Actualment només conté informació de versions de `FIND_CONCATENATE`, perquè `COWPY` encara no informa de la seva versió.
    La secció 1.6 explica com afegir-ho.
...
- **`path:`** Determina el directori de sortida basat en el nom del procés.
  El nom complet d'un procés contingut a `task.process` inclou la jerarquia d'importacions de workflow i mòdul (com ara `CORE_HELLO:HELLO:FIND_CONCATENATE`).
  Les operacions `tokenize` eliminen aquesta jerarquia per obtenir només el nom del procés, després prenen la primera part abans de qualsevol guió baix (si s'escau), i la converteixen a minúscules.
  Això és el que determina que els resultats de `FIND_CONCATENATE` es publiquin a `#!groovy ${params.outdir}/find/`.
...
La sortida es veu encara millor quan tots els mòduls d'un pipeline adopten aquesta convenció, així que sent lliure d'anar a eliminar les directives `publishDir` dels altres mòduls del teu pipeline.
Aquest valor per defecte s'aplicarà fins i tot a mòduls que no vam modificar explícitament per seguir les directrius nf-core.

Dit això, pots decidir que vols organitzar les teves entrades de manera diferent, i la bona notícia és que és fàcil fer-ho.

#### 1.5.3. Sobreescriure el valor per defecte

Per sobreescriure la directiva `publishDir` per defecte, simplement pots afegir les teves pròpies directives al fitxer `conf/modules.config`.

Per exemple, podries sobreescriure el valor per defecte per a un sol procés utilitzant el selector `withName:`, com en aquest exemple on afegim una directiva `publishDir` personalitzada per al procés `COWPY`.
...
No farem realment aquest canvi, però sent lliure de jugar amb això i veure quina lògica pots implementar.

El punt és que aquest sistema et permet tenir el millor dels dos mons: consistència per defecte i la flexibilitat de personalitzar la configuració sota demanda.
...
- **Personalització fàcil**: Sobreescriu el comportament de publicació a la configuració, no al codi del mòdul
- **Mòduls portables**: Els mòduls no codifiquen ubicacions de sortida

### 1.6. Captura de versions amb topic channels

La reproductibilitat és un principi fonamental d'nf-core: cada execució del pipeline registra exactament quines versions de programari s'han utilitzat, de manera que els resultats es puguin reproduir i comparar entre execucions.
L'informe `hello_software_versions.yml` a `pipeline_info/` és com els pipelines nf-core compleixen aquest requisit.

El mecanisme que hi ha darrere és els **topic channels**: una funcionalitat de difusió de Nextflow on qualsevol procés pot publicar dades a un tema amb nom, i qualsevol subscriptor rep tot el que s'ha publicat a aquell tema, independentment d'on provingui al pipeline.
nf-core l'està desplegant a tots els pipelines el 2026.
Els mòduls publiquen la seva versió com a tupla directament al bloc output utilitzant la paraula clau `topic:`, i el workflow se subscriu una vegada per recollir-les totes.

#### 1.6.1. Afegir informació de versió a `COWPY`

Obre el fitxer del mòdul `cowpy.nf` i afegeix una línia de sortida de versió com es mostra a continuació.
...
La nova línia publica una tupla `[nom_procés, nom_eina, versió]` al canal de tema `"versions"`.
No cal fer cap canvi al bloc script — la versió es declara estàticament al bloc output.

#### 1.6.2. Executar el pipeline i inspeccionar l'informe de versions
...
Obre `core-hello-results/pipeline_info/hello_software_versions.yml` i ara veuràs ambdós mòduls:
...
La col·lecció del costat del workflow — el bloc `channel.topic("versions")` que vas veure al workflow de marcador de posició a la Part 1 — se subscriu al tema i escriu aquest informe combinat automàticament.

!!! info "Compatibilitat amb versions anteriors"

    La branca `versions_file` al bloc del topic channel del workflow existeix per gestionar mòduls que encara no s'han actualitzat per utilitzar `topic: versions` i encara escriuen un fitxer `versions.yml` al bloc script amb `emit: versions`.
    Ambdós estils estan suportats simultàniament durant la transició.

### Conclusió

Ara saps com adaptar mòduls locals per seguir les convencions nf-core:

- Dissenya els teus mòduls per acceptar i propagar tuples de metadades;
- Utilitza `ext.args` per mantenir les interfícies de mòdul mínimes i portables;
- Utilitza `ext.prefix` per a nomenament de fitxers de sortida configurable i estandarditzat;
- Adopta la directiva `publishDir` centralitzada per defecte per a una estructura de directori de resultats consistent;
- Comprèn com els topic channels recullen versions de programari de tots els mòduls automàticament.

Per a més informació sobre les convencions de mòduls, consulta les [especificacions de mòduls nf-core](https://nf-co.re/docs/guidelines/components/modules).

### Què segueix?

Aprèn com utilitzar les eines integrades d'nf-core basades en plantilles per crear mòduls de manera fàcil.

---

## 2. Crear un mòdul amb les eines nf-core

Ara que has après els patrons de mòduls nf-core aplicant-los manualment, vegem com crearies mòduls a la pràctica.
...
La comanda `nf-core modules create` genera una plantilla de mòdul que ja segueix totes les convencions que has après.
...
La comanda s'executa de manera interactiva, guiant-te a través de la configuració.
...
Se't demanarà diverses opcions de configuració:

- **Informació de l'autor**: El teu nom d'usuari de GitHub per a l'atribució
...
L'eina gestiona la complexitat de trobar informació de paquets i configurar l'estructura, permetent-te centrar-te en implementar la lògica específica de l'eina.

#### 2.1.2. Examinar l'estructura del mòdul

L'eina crea una estructura de mòdul completa a `modules/local/` (o `modules/nf-core/` si estàs al repositori nf-core/modules):
...
!!! tip "Aprèn més sobre proves"

    El fitxer de prova generat utilitza nf-test, un framework de proves per a pipelines i mòduls Nextflow. Per aprendre com escriure i executar aquestes proves, consulta la [missió secundària nf-test](../side_quests/nf_test/index.md).

El `main.nf` generat inclou tots els patrons que acabes d'aprendre, més algunes funcionalitats addicionals:
...
Nota com tots els patrons que vas aplicar manualment anteriorment ja hi són!

La plantilla també inclou diverses convencions addicionals d'nf-core.
Algunes d'aquestes funcionen immediatament, mentre que d'altres són marcadors de posició que necessitarem omplir, com es descriu a continuació.

**Funcionalitats que funcionen tal qual:**

- **`tag "$meta.id"`**: Afegeix l'ID de mostra als noms de procés als logs per a un seguiment més fàcil
- **`label 'process_single'`**: Etiqueta de recursos per configurar requisits de CPU/memòria
- **Bloc `when:`**: Permet execució condicional mitjançant configuració `task.ext.when`
- **Sortida `topic: versions`**: Informació de versions via topic channel, tal com es cobreix a la secció 1.6
...
- **Bloc `script:`**: Buit — afegirem la comanda `cowpy` aquí
- **Bloc `stub:`**: L'actualitzarem per produir les sortides correctes
- **Container i entorn**: Marcadors de posició que omplirem amb informació de paquets
...
Per al contenidor, pots utilitzar [Seqera Containers](https://seqera.io/containers/) per construir automàticament un contenidor des de qualsevol paquet Conda, incloent paquets conda-forge.
En aquest cas estem utilitzant el mateix contenidor preconstruït que anteriorment.
...
Per a l'entorn Conda, el codi del mòdul especifica `#!groovy conda "${moduleDir}/environment.yml"` el que significa que hauria d'estar configurat al fitxer `environment.yml`.
...
No obstant això, `cowpy` està disponible a conda-forge, així que pots completar l'`environment.yml` així:
...
=== "Abans"
    ...
    channels:
      ...
      - bioconda
    dependencies:
      # TODO nf-core: Llista els paquets Conda necessaris.
      #               El programari HA d'estar fixat al canal (p. ex. "bioconda"), versió (p. ex. "1.10").
      #               Per a Conda, la build (p. ex. "h9402c20_2") s'ha d'EXCLOURE per suportar la instal·lació en diferents sistemes operatius.
      - "YOUR-TOOL-HERE"
    ```
...
La plantilla generada inclou declaracions d'entrada i sortida genèriques que necessitaràs personalitzar per a la teva eina específica.
Mirant enrere al nostre mòdul `COWPY` manual de la secció 1, podem utilitzar-lo com a guia.

Actualitza els blocs d'entrada i sortida:
...
- El nom del fitxer de sortida utilitzant el patró de prefix configurable (`#!groovy ${prefix}.txt` en lloc del comodí `*`)
- Un nom emit descriptiu (`cowpy_output` en lloc de `output` genèric)
- Una cadena de versió estàtica (`#!groovy val("1.1.5")`) en lloc del `#!groovy eval("cowpy --version")` de la plantilla, coincidint amb el mòdul manual de la secció 1.6 (l'eina `cowpy` no exposa un flag `--version`)

Si estàs utilitzant el servidor de llenguatge Nextflow per validar la sintaxi, la part `#!groovy ${prefix}` es marcarà com un error en aquesta etapa perquè encara no l'hem afegit al bloc script.
Fem-ho ara.

#### 2.3.2. El bloc script

La plantilla genera un bloc script buit que has d'omplir amb la comanda real de l'eina.

Basant-nos en el mòdul que vam escriure manualment anteriorment, hauríem de fer les següents edicions:
...
- Omplir la comanda `cowpy` real que utilitza tant `$args` com `#!groovy ${prefix}.txt`

Nota que si no haguéssim fet ja el treball d'afegir la configuració `ext.args` i `ext.prefix` per al procés `COWPY` al fitxer `modules.config`, hauríem de fer-ho ara.

#### 2.3.3. Implementar el bloc stub

En el context de Nextflow, un bloc [stub](https://www.nextflow.io/docs/latest/process.html#stub) et permet definir un script lleuger i fictici utilitzat per a prototipatge ràpid i proves de la lògica d'un pipeline sense executar la comanda real.

No et preocupis massa si això sembla misteriós; ho incloem per completesa però també pots simplement eliminar la secció stub si no vols tractar amb ella, ja que és completament opcional.
...
- Eliminar la línia `echo $args` (que era només codi marcador de posició de la plantilla)
- El stub crea un fitxer `#!groovy ${prefix}.txt` buit coincidint amb el que produeix el bloc script

Això et permet provar la lògica del workflow i la gestió de fitxers sense esperar que l'eina real s'executi.

Un cop hagis completat la configuració de l'entorn (secció 2.2), entrades/sortides (secció 2.3.1), bloc script (secció 2.3.2) i bloc stub (secció 2.3.3), el mòdul està llest per provar!
...
Això produeix els mateixos resultats que anteriorment.

### Conclusió

Ara saps com utilitzar les eines integrades d'nf-core per crear mòduls de manera eficient utilitzant plantilles en lloc d'escriure tot des de zero.

### Què segueix?

Aprèn quins són els beneficis de contribuir mòduls a nf-core i quins són els principals passos i requisits implicats.
...
Contribuir els teus mòduls a nf-core:

- Fa que les teves eines estiguin disponibles per a tota la comunitat nf-core a través del catàleg de mòduls a [nf-co.re/modules](https://nf-co.re/modules)
- Assegura manteniment i millores contínues de la comunitat
- Proporciona garantia de qualitat mitjançant revisió de codi i proves automatitzades
- Dóna visibilitat i reconeixement al teu treball

### 3.2. Llista de verificació del contribuïdor

Per contribuir un mòdul a nf-core, necessitaràs passar pels següents passos:

1. Comprovar si ja existeix a [nf-co.re/modules](https://nf-co.re/modules)
2. Fer un fork del repositori [nf-core/modules](https://github.com/nf-core/modules)
3. Utilitzar `nf-core modules create` per generar la plantilla
4. Omplir la lògica del mòdul i les proves
5. Provar amb `nf-core modules test tool/subtool`
6. Fer lint amb `nf-core modules lint tool/subtool`
7. Enviar una pull request

Per a instruccions detallades, consulta el [tutorial de components nf-core](https://nf-co.re/docs/tutorials/nf-core_components/components).
...
- **Suport de la comunitat**: [Slack d'nf-core](https://nf-co.re/join) - Uneix-te al canal `#modules`

### Conclusió

Ara saps com crear mòduls nf-core! Has après els quatre patrons clau que fan que els mòduls siguin portables i mantenibles:
...
- **Publicació centralitzada** mitjançant `publishDir` configurat a `modules.config` en lloc de codificat als mòduls

Transformant `COWPY` pas a pas, has desenvolupat una comprensió profunda d'aquests patrons, fent-te capaç de treballar amb, depurar i crear mòduls nf-core.
A la pràctica, utilitzaràs `nf-core modules create` per generar mòduls estructurats correctament amb aquests patrons integrats des del principi.

Finalment, has après com contribuir mòduls a la comunitat nf-core, fent que les eines estiguin disponibles per a investigadors d'arreu del món mentre et beneficies del manteniment continu de la comunitat.

### Què segueix?

Quan estiguis llest, continua a la [Part 4: Validació d'entrada](./04_input_validation.md) per aprendre com afegir validació d'entrada basada en esquema al teu pipeline.
