...
Aquest entorn de formació conté tot el programari, el codi i les dades necessaris per treballar el curs de formació, de manera que no cal que instal·leu res vosaltres mateixos.
...
Per a aquest curs, canvieu al directori `seqera-scale/`:
...
El fitxer **`.seqera_config`** és un esquelet que omplireu durant la secció 3 per configurar la CLI `tw` amb el vostre token d'accés i espai de treball de Seqera.
...
**Per continuar a [Part 1: Llançar pipelines des de la interfície web](./01_run_with_seqera.md), feu clic a la fletxa a la cantonada inferior dreta d'aquesta pàgina.**
