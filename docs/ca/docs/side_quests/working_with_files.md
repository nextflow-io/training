...
- [ ] Entenc l'objectiu d'aquest curs i els seus requisits previs
...
??? info "Què és `.class`?"
    ...
    A Nextflow, `.class` ens indica quin tipus d'objecte estem treballant. És com preguntar "quin tipus de cosa és això?" per esbrinar si és una cadena, un nombre, un fitxer o alguna altra cosa.
    Això ens ajudarà a il·lustrar la diferència entre una cadena simple i un objecte Path en les següents seccions.
...
Com podeu veure, Nextflow ha imprès la ruta de cadena exactament com l'hem escrita.
...
Veieu els diversos atributs del fitxer impresos a la consola anterior.
...
Tingueu en compte que per a fitxers més grans, el pas de descàrrega trigarà més temps en comparació amb l'execució sobre fitxers locals.
Tanmateix, Nextflow comprova si ja té una còpia preparada per evitar descàrregues innecessàries.
Així que si executeu de nou sobre el mateix fitxer i no heu eliminat el fitxer preparat, Nextflow utilitzarà la còpia preparada.
...
Aquesta és una gran oportunitat per utilitzar un magatzem clau-valor, on cada element té un conjunt de claus i els seus valors associats, així que podeu referir-vos fàcilment a cada clau per obtenir el valor corresponent.
...
- **Estructura imbricada**: La llista exterior `[..., myFile]` aparella el map de metadades amb l'objecte de fitxer original
...
Ara les metadades estan clarament etiquetades (_p. ex._ `[id:patientA, replicate:1, type:normal, readNum:2]`) així que és molt més fàcil dir què és què.
...
Afegirem això de nou més tard, amb algunes modificacions.
...
I aquí està, totes les nostres metadades, ordenades de manera neta. Això és un èxit!
...
- Aquest enfocament crea workflows mantenibles amb una clara procedència de dades
