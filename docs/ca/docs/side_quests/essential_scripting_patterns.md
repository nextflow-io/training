...
Ara afegim més scripting - aquesta vegada utilitzant un operador ternari per prendre decisions basades en valors de dades.
...
!!! Note "Nota"
    ...
    Mai modifiqueu mapes passats a closures - sempre creeu-ne de nous utilitzant `+` (per exemple). A Nextflow, les mateixes dades sovint flueixen a través de múltiples operacions simultàniament. Modificar un mapa in situ pot causar efectes secundaris impredictibles quan altres operacions referencien aquest mateix objecte. Crear nous mapes assegura que cada operació tingui la seva pròpia còpia neta.
...
Generem una estructura de canal que comprèn una tupla de 2 elements: el mapa de metadades enriquit i el camí de fitxer corresponent. Aquest és un patró comú a Nextflow per passar dades als processos.
...
Aquesta estructura de tupla `[meta, file]` és un patró comú a Nextflow per passar tant metadades com fitxers associats als processos.
...
Relacionat amb el mètode `collect` de la List està l'operador spread (`*.`), que proporciona una manera concisa d'extreure propietats de col·leccions. És essencialment sucre sintàctic per a un patró comú de `collect`.
...
L'operador spread `*.` és una forma abreujada per a un patró comú de collect:
...
Tornarem al nostre workflow `main.nf` i afegirem alguna lògica de coincidència de patrons per extreure informació addicional de mostra dels noms de fitxers. Els fitxers FASTQ del nostre conjunt de dades segueixen convencions de nomenclatura d'estil Illumina amb noms com `SAMPLE_001_S1_L001_R1_001.fastq.gz`. Aquests poden semblar críptics, però en realitat codifiquen metadades útils com l'ID de mostra, el número de carril i la direcció de lectura. Utilitzarem capacitats regex per analitzar aquests noms.
...
Arreglem això afegint lògica condicional al bloc `script:` del procés `FASTP`. Una sentència if/else comprova el recompte de fitxers de lectura i ajusta la comanda en conseqüència.
...
Arreglem això escapant les variables de shell i les substitucions de comandes amb una barra invertida (`\`):
...
- **Àmbit de funcions**: Les funcions definides a nivell d'script són accessibles a tot el vostre workflow de Nextflow
...
En aquest exemple hem triat un exemple que va sol·licitar 2 CPUs (`--cpu-shares 2048`), perquè era una mostra d'alta profunditat, però hauríeu de veure diferents assignacions de CPU depenent de la profunditat de la mostra. Proveu això també per a les altres tasques.
...
Per fer el nostre workflow més robust, podem implementar una estratègia de reintent que augmenta l'assignació de memòria en cada intent, utilitzant una vegada més una closure de Groovy. Modifiqueu la directiva `memory` per multiplicar la memòria base per `task.attempt`, i afegiu les directives `errorStrategy 'retry'` i `maxRetries 2`:
...
Anteriorment, vam utilitzar `.map()` amb scripting per transformar dades de canal. Ara utilitzarem lògica condicional per controlar quins processos s'executen basant-se en dades—essencial per a workflows flexibles que s'adapten a diferents tipus de mostra.
...
Hem proporcionat un procés simple de Trimgalore a `modules/trimgalore.nf`, doneu-hi un cop d'ull si voleu, però els detalls no són importants per a aquest exercici. El punt clau és que volem enrutar mostres basant-nos en les seves metadades.
...
En aquesta secció, heu après a utilitzar lògica condicional per controlar l'execució del workflow utilitzant les interfícies de closure d'operadors de Nextflow com `.branch{}` i `.filter{}`, aprofitant la truthiness per escriure expressions condicionals concises.
...
No hi ha fallada! El workflow ara gestiona el camp absent amb gràcia. Quan `row.run_id` és `null`, l'operador `?.` evita la crida a `.toUpperCase()`, i `run_id` es converteix en `null` en lloc de causar una excepció.
...
- **Combinació**: `value?.method() ?: 'default'` és el patró comú
...
Fem-ho més útil afegint lògica condicional:
...
- **Gestió robusta**: Els operadors de navegació segura i Elvis fan que els workflows siguin resilients a dades absents
- **Processament flexible**: La lògica condicional permet que els vostres workflows processin diferents tipus de mostra adequadament
...
4.  **Directives de Recursos Dinàmiques amb Closures**: Heu explorat l'ús de closures en directives de processos per a l'assignació adaptativa de recursos basada en característiques d'entrada.
...
5.  **Lògica Condicional i Control de Processos**: Heu afegit enrutament intel·ligent utilitzant operadors `.branch()` i `.filter()`, aprofitant la truthiness per a expressions condicionals concises.
...
- [Sintaxi d'Scripts de Nextflow](https://www.nextflow.io/docs/latest/script.html)
