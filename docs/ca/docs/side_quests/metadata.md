...
??? info "(Opcional) Més sobre mapes"
    ...
    Tot i que no té un bloc `workflow` adequat, Nextflow pot executar-ho com si fos un workflow:
...
Ara que hem llegit correctament el full de dades i tenim accés a les dades de cada fila, podem començar a implementar la nostra lògica de pipeline.
...
També fa que el procés sigui difícil de compartir amb altres que podrien tenir dades d'entrada lleugerament diferents, i podríeu acabar havent de codificar variables dins del procés que no són necessàries pel bloc de script.
...
Ara que tenim les nostres metadades carregades, fem alguna cosa amb elles!
...
La definició de sortida està estructurada com una tupla amb una estructura similar a l'entrada, excepte que també conté `stdout` com a tercer element.
...
Això serà útil en el següent pas.
...
Sí, això quadra!
Hem reorganitzat ordenadament la sortida del procés de `meta, file, lang_id` de manera que `lang_id` ara és una de les claus al mapa meta, i les tuples del canal s'ajusten de nou al model `meta, file`.
...
Intenteu escriure-ho vosaltres mateixos si ja sabeu com escriure declaracions condicionals a Nextflow.
...
Com podeu veure, aquest procés està dissenyat actualment per prendre un fitxer d'entrada (que conté el text a mostrar) i un valor que especifica el personatge que s'hauria de dibuixar en art ASCII, normalment proporcionat al nivell de workflow per un paràmetre de línia de comandes.
...
#### 3.2.1. Anomenar el canal de sortida anterior
...
Per escriure la crida al procés `COWPY`, només necessitem saber com extreure l'objecte fitxer corresponent i les metadades de cada element del canal.
...
Això confirma que som capaços d'accedir al fitxer i al personatge per a cada element del canal.
...
??? abstract "Directori i contingut de fitxer d'exemple"
...
Això mostra que vam poder utilitzar la informació del mapa meta per parametritzar la comanda al segon pas del pipeline.

No obstant això, com s'ha indicat anteriorment, part del codi implicat era una mica feixuc, ja que vam haver de desempaquetar metadades mentre encara estàvem en el context del cos del workflow.
...
Hem reemplaçat la referència al valor `character` passat anteriorment com a entrada independent amb el valor mantingut al mapa meta, al qual ens referim utilitzant `meta.character`.
...
Eliminem els resultats de l'execució anterior i executem-ho:
...
Diguem que eliminem el valor de character per a una de les entrades al nostre full de dades per simular un error de recollida de dades:
...
Ara diguem que eliminem la columna `character` completament del nostre full de dades:
...
Per exemple, intenteu modificar el codi per produir aquest resultat:
