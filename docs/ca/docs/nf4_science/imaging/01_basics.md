...
!!! Tip "Consell"
    ...
    Tanmateix, els fitxers de registre estan configurats per ser invisibles al terminal, així que si voleu utilitzar `ls` o `tree` per visualitzar-los, haureu d'establir l'opció rellevant per mostrar fitxers invisibles.
...
Hauríeu de reconèixer immediatament el fitxer `output.txt`, que de fet és la sortida original del procés `sayHello` que s'ha publicat al directori `results`.
...
Aquests són els fitxers auxiliars i de registre que Nextflow ha escrit com a part de l'execució de la tasca:
...
- **`.command.sh`**: La comanda que realment ha executat la crida del procés
- **`.exitcode`**: El codi de sortida resultant de la comanda

El fitxer `.command.sh` és especialment útil perquè us mostra la comanda principal que Nextflow ha executat sense incloure tota la comptabilitat i la configuració de tasca/entorn.
...
!!! Tip "Consell"
    ...
    Quan alguna cosa va malament i necessiteu solucionar problemes sobre què ha passat, pot ser útil mirar l'script `command.sh` per comprovar exactament quina comanda ha compost Nextflow basant-se en les instruccions del workflow, la interpolació de variables, etc.
...
Observeu com les sortides i els registres dels directoris de tasca aïllats es preserven, mentre que el contingut del directori `results` se sobreescriu amb la sortida d'execucions posteriors.
...
En un pipeline del món real, un procés normalment conté informació addicional com ara directives de procés, que introduirem d'aquí a poc.
...
Específicament, en aquest mode, qualsevol procés que ja s'hagi executat amb exactament el mateix codi, configuració i entrades se saltarà.
