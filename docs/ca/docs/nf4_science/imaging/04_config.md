...
Els perfils són conjunts amb nom de configuració que es poden activar amb la bandera `-profile` mitjançant la comanda `nextflow run`.
...
### 2.3. Tornar a executar amb perfils en lloc de nextflow.config

Ara que hem desactivat la configuració de docker al nostre fitxer local `nextflow.config` i entenem els perfils, tornem a executar el pipeline utilitzant la bandera `-profile`.
...
!!! Tip "Consell"
    ...
    Per trobar noms de processos, mira la sortida d'execució del pipeline o comprova `.nextflow.log`.
...
!!! Warning "Advertència"
    ...
    El pipeline pot necessitar utilitzar algorismes menys intensius en memòria o processar dades en fragments més petits.
...
Aquestes habilitats de configuració són transferibles a qualsevol pipeline de Nextflow i t'ajudaran a executar workflows de manera eficient a màquines locals, clústers HPC i plataformes de núvol.
...
- Explora [Build with nf-core](../../nfcore_build/index.md) per aprofundir en les eines d'nf-core
- Explora temes més avançats a les [Side Quests](../../side_quests/index.md)
