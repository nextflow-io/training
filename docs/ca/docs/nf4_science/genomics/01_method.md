...
!!! tip "Consell"
    ...
    Assegureu-vos que esteu al directori `nf4-science/genomics` de manera que l'última part del camí mostrat quan escriviu `pwd` sigui `genomics`.
...
Aquestes són les comandes que embolicarem en un workflow Nextflow a la Part 2 d'aquest curs.
...
Això és important a causa de com de grans poden arribar a ser aquests fitxers.

Els fitxers BAM sovint es proporcionen sense un índex, així que el primer pas en molts workflows d'anàlisi és generar-ne un utilitzant `samtools index`.
...
!!! example "Escriviu-ho com un workflow!"
    ...
    Sentiu-vos lliures de passar directament a la [Part 2](./02_per_sample_variant_calling.md) si voleu començar a implementar aquesta anàlisi com un workflow Nextflow.
...
Això està bé per mirar variants de cada mostra de manera aïllada, però proporciona informació limitada.
Sovint és més interessant mirar com difereixen les crides de variants entre múltiples mostres.
...
El que és especial del GVCF d'una mostra és que conté registres que resumeixen estadístiques de dades de seqüència sobre totes les posicions a l'àrea objectiu del genoma, no només les posicions on el programa va trobar evidència de variació.
...
Aquestes són les comandes que embolicarem en un workflow Nextflow a la Part 3 d'aquest curs.
...
Ja vam descarregar la imatge del contenidor Samtools, així que podem iniciar-la directament:
...
Ja vam descarregar la imatge del contenidor GATK abans, així que podem iniciar-la directament:
...
Això fa possible distingir entre dues figures de cas molt diferents: (1) hi ha dades de bona qualitat que mostren que la mostra és homozigota-referència, i (2) no hi ha prou dades bones disponibles per fer una determinació en qualsevol sentit.
...
Podeu explorar-lo però ràpidament veureu que aquest format de magatzem de dades no està destinat a ser llegit directament per humans.
...
??? success "Sortida de la comanda"
    ...
    17:38:45.561 INFO  NativeGenomicsDB - pid=221 tid=222 No valid combination operation found for INFO field MLEAC  - the field will NOT be part of INFO fields in the generated```console
 VCF records
...
Hem destacat una vegada més l'última línia de capçalera, que marca l'inici de les dades de crida de variants.
...
Aquesta és en última instància el tipus d'informació que estem buscant extreure del conjunt de dades!
...
Feu una pausa, després aneu a la [Part 2](./02_per_sample_variant_calling.md) per aprendre com embolicar aquestes mateixes comandes en workflows que utilitzen contenidors per executar el treball.
