# Part 2: Crida de variants per mostra
...
2. Executar GATK HaplotypeCaller a cada fitxer BAM d'entrada per generar crides de variants per mostra en format VCF (Variant Call Format)
...
Aquests fitxers no són funcionals; el seu propòsit és només servir com a base perquè els ompliu amb les parts interessants del codi.
...
=== "Després"
    ...
    workflow {
        ...
        main:
        // Crea el canal d'entrada (fitxer únic via paràmetre CLI)
...
Necessitem escriure la definició del procés al fitxer del mòdul, importar-lo al workflow utilitzant una declaració include i cridar-lo amb l'entrada.
...
#### 1.2.3. Cridar el procés d'indexació amb l'entrada
...
=== "Després"
    ...
    workflow {
        ...
        // Crea el fitxer d'índex per al fitxer BAM d'entrada
...
Assigneu la sortida de `SAMTOOLS_INDEX` a un objectiu anomenat `bam_index`.
...
#### 1.3.2. Configurar l'objectiu de sortida al bloc `output {}`

El bloc `output {}` es troba fora del workflow i especifica on es publica cada objectiu anomenat.
Afegim un objectiu per a `bam_index` que publiqui en un subdirectori `bam/`.
...
!!! note "Nota"
    ...
    Per defecte, Nextflow publica els fitxers de sortida com a enllaços simbòlics, cosa que evita duplicacions innecessàries.
...
Afegir un segon pas que prengui la sortida del procés d'indexació i l'utilitzi per executar la crida de variants.

---

## 2. Afegir un segon procés per executar GATK HaplotypeCaller al fitxer BAM indexat

Ara que tenim un índex per al nostre fitxer d'entrada, podem passar a configurar el pas de crida de variants.
...
2. Escriure el procés de crida de variants i cridar-lo al workflow
3. Configurar la gestió de sortides

### 2.1. Configurar les entrades

El pas de crida de variants requereix diversos fitxers d'entrada addicionals.
...
=== "Després"
    ...
    workflow {
        ...
        // Carrega les rutes de fitxer per als fitxers accessoris (referència i intervals)
...
La sintaxi `file()` indica explícitament a Nextflow que tracti aquestes entrades com a rutes de fitxers.
Podeu aprendre més sobre això a la Side Quest [Working with files](../../side_quests/working_with_files.md).

### 2.2. Escriure el procés de crida de variants i cridar-lo al workflow

Necessitem escriure la definició del procés al fitxer del mòdul, importar-lo al workflow utilitzant una declaració include i cridar-lo amb les lectures d'entrada més la sortida del pas d'indexació i els fitxers accessoris.

#### 2.2.1. Omplir el mòdul per al procés de crida de variants
...
=== "Després"
    ...
    /*
     * Crida variants amb GATK HaplotypeCaller
     */
    process GATK_HAPLOTYPECALLER {
...
=== "Després"
    ...
    ```groovy title="genomics.nf" linenums="33" hl_lines="4-12"
        ...
        // Crida variants del fitxer BAM indexat
...
!!! note "Nota"
    ...
    Noteu que les entrades es proporcionen exactament en el mateix ordre a la crida del procés que estan llistades al bloc d'entrada del procés.
...
#### 2.3.1. Afegir objectius de publicació per a les sortides de crida de variants
...
Afegiu entrades per als objectius `vcf` i `vcf_idx` al bloc `output {}`, publicant ambdós en un subdirectori `vcf/`:
...
El VCF i el seu índex es publiquen com a objectius separats que ambdós van al subdirectori `vcf/`.
...
Sabeu com fer un workflow modular de dos passos que fa un treball d'anàlisi real i és capaç de tractar amb les idiosincràsies dels formats de fitxers genòmics com els fitxers accessoris.
...
Aquesta és una manera ràpida de provar l'execució de múltiples mostres; al següent pas canviarem a un enfocament més escalable utilitzant un fitxer d'entrades.
...
Bé, això és estrany, considerant que vam indexar explícitament els fitxers BAM al primer pas del workflow. Podria haver-hi algun problema amb la infraestructura?

### 3.3. Solucionar el problema

Inspeccionarem els directoris de treball i utilitzarem l'operador `view()` per esbrinar què va malament.
...
Presteu especial atenció als noms del fitxer BAM i l'índex BAM que es llisten en aquest directori: `reads_son.bam` i `reads_father.bam.bai`.

Què dimonis? Nextflow ha preparat un fitxer d'índex al directori de treball d'aquesta crida de procés, però és el incorrecte. Com pot haver passat això?

#### 3.3.2. Utilitzar l'[operador view()](https://www.nextflow.io/docs/latest/reference/operator.html#view) per inspeccionar el contingut del canal

Afegiu aquestes dues línies al cos del workflow abans de la crida del procés `GATK_HAPLOTYPECALLER` per veure el contingut del canal:
...
Un cop més, això pot tenir èxit o fallar. Aquí teniu com es veu la sortida de les dues crides `.view()` per a una execució fallida:
...
!!! tip "Consell"
    ...
    Tanmateix, embrutaran la vostra sortida de consola, així que recomanem eliminar-les quan hàgiu acabat de solucionar el problema.
...
A continuació, necessitarem actualitzar el workflow per reflectir la nova estructura de tupla a la crida del procés i els objectius de publicació.
...
Finalment, necessitem actualitzar els objectius de publicació per reflectir la nova estructura de sortida.

#### 3.4.4. Actualitzar l'objectiu de publicació per a la sortida BAM indexada

Com que la sortida de SAMTOOLS_INDEX ara és una tupla que conté tant el fitxer BAM com el seu índex, canvieu el nom de l'objectiu de publicació de `bam_index` a `indexed_bam` per reflectir millor el seu contingut:
...
Agrupant els fitxers associats en tuples, vam assegurar que els fitxers correctes sempre viatgen junts pel workflow.
...
Una manera molt comuna de proporcionar múltiples fitxers de dades d'entrada a un workflow és fer-ho amb un fitxer de text que conté les rutes dels fitxers.
Pot ser tan senzill com un fitxer de text que llista una ruta de fitxer per línia i res més, o el fitxer pot contenir metadades addicionals, en aquest cas sovint s'anomena samplesheet.

Aquí us mostrarem com fer el cas senzill.
...
!!! warning "Advertència"
    ...
    Les rutes de fitxer al CSV són rutes absolutes que han de coincidir amb el vostre entorn.
...
=== "Després"
    ...
    ```groovy title="genomics.nf" linenums="24" hl_lines="1-4"
        // Crea el canal d'entrada des d'un fitxer CSV que llista les rutes dels fitxers d'entrada
        reads_ch = channel.fromPath(params.input)
...
Això hauria de produir el mateix resultat que abans. El nostre workflow senzill de crida de variants ara té totes les característiques bàsiques que volíem.

### Conclusió

Sabeu com fer un workflow modular de múltiples passos per indexar un fitxer BAM i aplicar crida de variants per mostra utilitzant GATK.

Més en general, heu après com utilitzar components i lògica essencials de Nextflow per construir un pipeline genòmic senzill que fa un treball real, tenint en compte les idiosincràsies dels formats de fitxers genòmics i els requisits de les eines.

### Què segueix?

Feu una pausa! Això ha estat molt.

Quan us sentiu refrescats, aneu a la [Part 3](./03_joint_calling.md), on aprendreu com transformar aquest workflow senzill de crida de variants per mostra per aplicar crida de variants conjunta a les dades.
