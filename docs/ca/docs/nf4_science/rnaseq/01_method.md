...
- **Dades RNAseq** (`reads/`): fitxers FASTQ de sis mostres, reduïts a una petita regió per mantenir les mides de fitxer reduïdes. Cada mostra té lectures paired-end (dos fitxers per mostra), tot i que comencem treballant només amb lectures single-end.
...
El mètode referenciat anteriorment ens proporciona la línia de comandes per executar QC sobre un sol fitxer.
