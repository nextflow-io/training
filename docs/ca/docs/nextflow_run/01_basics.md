...
Descobriu què cal per executar un workflow de Nextflow que aconsegueixi el mateix resultat.
...
??? question "No veieu el mateix?"
    ...
    Tanmateix, els fitxers de registre estan configurats per ser invisibles al terminal, així que si voleu utilitzar `ls` o `tree` per veure'ls, haureu de configurar l'opció rellevant per mostrar fitxers invisibles.
...
Quan alguna cosa va malament i necessiteu solucionar problemes sobre què ha passat, pot ser útil mirar l'script `command.sh` per comprovar exactament quina comanda va compondre Nextflow basant-se en les instruccions del workflow, interpolació de variables, etc.

### 2.5. Tornar a executar el workflow amb diferents salutacions

Proveu de tornar a executar el workflow unes quantes vegades amb valors diferents per a l'argument `--input`, després mireu els directoris de tasca.
...
Apreneu a gestionar les vostres execucions de workflow convenientment.
...
Aquest fitxer us dóna la marca de temps, nom d'execució, estat, ID de revisió, ID de sessió i línia de comandes completa per a cada execució de Nextflow que s'ha llançat des del directori de treball actual.
...
El nom d'execució és la cadena de dues parts generada per màquina mostrada entre claudàtors a la línia de sortida de consola `Launching (...)`.
...
A la següent secció d'aquesta formació, mirarem quatre versions successivament més realistes del pipeline Hello World que demostraran com Nextflow us permet processar múltiples entrades eficientment, executar workflows compostos de múltiples passos connectats entre si, aprofitar components de codi modulars i utilitzar contenidors per a una major reproducibilitat i portabilitat.
...
- [ ] La suma de verificació del fitxer de sortida
...
- [ ] `-v` (detallat) per veure sortida detallada
- [ ] `-a` (tot) per seleccionar tots els directoris
- [ ] `-q` (silenciós) per suprimir advertències
