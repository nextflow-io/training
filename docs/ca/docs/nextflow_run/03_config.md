...
Nextflow els traduirà a les instruccions apropiades per a l'executor escollit.
...
!!! tip "Consell"
    ...
    Nextflow mateix té una [lògica de reintent dinàmic](https://nextflow.io/docs/latest/process.html#dynamic-task-resources) realment interessant integrada per reintentar treballs que fallen a causa de limitacions de recursos.
...
??? example "Escenari"
    ...
    Canvieu regularment entre executar pipelines al vostre portàtil per a desenvolupament i a l'HPC de la vostra institució per a execucions de producció.
...
Si combineu perfils que estableixen valors per als mateixos elements de configuració i estan descrits al mateix fitxer de configuració, Nextflow resoldrà el conflicte utilitzant el valor que va llegir per últim (_és a dir_, el que ve més tard al fitxer).
...
Apreneu a executar pipelines directament des de repositoris remots com GitHub.
...
Doneu-vos una gran palmadeta a l'esquena!
