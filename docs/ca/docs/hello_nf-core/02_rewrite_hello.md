...
Aquesta TUI us demanarà que proporcioneu informació bàsica sobre el vostre pipeline i us oferirà una selecció de característiques per incloure o excloure a l'estructura del vostre pipeline.
...
Però no us preocupeu si encara us sentiu una mica perduts; recorrerem les parts importants junts durant el curs d'aquesta formació.
...
Podeu donar una ullada als informes per veure què es va executar, i la resposta és: res en absolut!
...
Aquestes són característiques opcionals de Nextflow que fan que el workflow sigui **composable**, és a dir, que es pot cridar des de dins d'un altre workflow.
...
!!! note "Nota"
    ...
    Ignorarem la captura de versions per a aquesta primera passada i veurem com cablear-ho en una part posterior d'aquesta formació.
...
La gestió de l'entrada es farà abans d'aquest workflow (no en aquest fitxer de codi).
...
Ara la lògica del workflow està correctament cablejada.
...
## 4. Adaptar la gestió d'entrades
...
La plantilla nf-core ve amb una gestió d'entrades sofisticada dissenyada per a conjunts de dades genòmiques complexes, així que necessitem adaptar-la per treballar amb el nostre fitxer `greetings.csv` més simple.

### 4.1. Identificar on es gestionen les entrades

El primer pas és esbrinar on es fa la gestió d'entrades.
...
Una mica de recerca revela que la gestió d'entrades es fa pel subworkflow `PIPELINE_INITIALISATION`, apropiadament, que s'importa de `core-hello/subworkflows/local/utils_nfcore_hello_pipeline/main.nf`.
...
Aquesta és la fàbrica de canals que analitza el samplesheet i el passa en una forma que està llesta per ser consumida pel workflow HELLO.
...
Per ara, ens centrem en mantenir-ho tan simple com sigui possible per arribar a alguna cosa que puguem executar amb èxit en dades de test.

### 4.3. Actualitzar el perfil de test

Parlant de dades de test i parametres, actualitzem el perfil de test per a aquest pipeline per utilitzar el mini-samplesheet `greetings.csv` en lloc del samplesheet d'exemple proporcionat a la plantilla.
...
Normalment els fitxers de test petits s'emmagatzemen al directori `assets`, així que copiem el fitxer des del nostre directori de treball.

```bash
cp greetings.csv core-hello/assets/.
```

Ara el fitxer `greetings.csv` està llest per ser utilitzat com a entrada de test.
...
- **Camins absoluts**: Utilitzant `${projectDir}`, creem un camí absolut, que és important per a dades de test que s'envien amb el pipeline.
- **Ubicació de dades de test**: Els pipelines nf-core normalment emmagatzemen dades de test al directori `assets/` dins del repositori del pipeline per a fitxers de test petits, o fan referència a conjunts de dades de test externs per a fitxers més grans.
...
I aquí està! Pot semblar molta feina per aconseguir el mateix resultat que el pipeline original, però obteniu tots aquests informes encantadors generats automàticament, i ara teniu una base sòlida per aprofitar característiques addicionals d'nf-core, incloent validació d'entrades i algunes capacitats de gestió de metadades interessants que cobrirem en una secció posterior.
