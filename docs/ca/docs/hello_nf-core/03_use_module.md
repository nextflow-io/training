...
Això mostra documentació sobre el mòdul, incloent les seves entrades, sortides i informació d'ús bàsic.
...
Això ens permetrà determinar si podem tractar el nou mòdul com una substitució directa o si haurem d'adaptar part del cablejat.
...
Això pot semblar molt a tenir en compte només mirant el codi, així que aquí teniu un diagrama per ajudar-vos a visualitzar com tot encaixa.
...
En el context dels pipelines Nextflow, les unitats de dades poden ser qualsevol cosa que vulgueu: mostres individuals, lots de mostres o conjunts de dades sencers.
...
- Empaquetar el metamap en una tupla amb el conjunt de fitxers d'entrada a concatenar (provinent de `convertToUpper`);
...
Sabeu com avaluar la interfície d'entrada i sortida d'un nou mòdul per identificar els seus requisits, i heu après com els pipelines nf-core utilitzen els metamaps per mantenir les metadades estretament associades amb les dades mentre flueixen pel pipeline.

### Què segueix?

Integrar el nou mòdul a un workflow.
...
Excepte que no volem codificar el valor `id`; volem utilitzar el valor del parametre `params.batch`.
