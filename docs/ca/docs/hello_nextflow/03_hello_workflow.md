...
En aquest mòdul de formació, aprendreu com connectar processos junts en un workflow de múltiples passos.
...
Aquesta és una substitució de text molt naïf que no té en compte les lletres accentuades, així que per exemple 'Hola' es convertirà en 'HOLà', però farà una feina prou bona per demostrar els conceptes de Nextflow i això és el que importa.
...
Per descomptat, això significa que estem repetint alguna informació aquí, que podria esdevenir inconvenient si volguéssim actualitzar la ubicació per a totes les sortides de la mateixa manera.
...
Deixa de banda la(es) definició(ns) d'entrada i la primera meitat de la comanda script perquè necessitem esbrinar com escriure-ho.
