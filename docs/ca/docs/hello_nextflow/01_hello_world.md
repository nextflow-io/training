...
L'execució de pipelines consisteix principalment a llegir dades de fitxers i escriure resultats en altres fitxers, així que modifiquem la comanda per escriure la sortida de text a un fitxer per fer l'exemple una mica més rellevant.
...
Descobriu com seria això escrit com a workflow de Nextflow.
...
Això ens permet recuperar fàcilment fitxers de sortida sense haver d'excavar pel subdirectori work.
...
Aquest fragment és realment senzill.
Només cal afegir `mode 'copy'` a la definició de sortida de nivell de workflow rellevant:
...
Us mostrarem com utilitzar-les a temps degut en el vostre viatge amb Nextflow.
...
No obstant això, no recomanem utilitzar-lo en cap treball nou, ja que eventualment no es permetrà en versions futures del llenguatge Nextflow.
...
La variable `greeting` té el prefix `val` per indicar a Nextflow que és un valor (no un camí).
...
La sintaxi general és declarar `params.<nom_paràmetre>` per indicar a Nextflow que esperi un paràmetre `--<nom_paràmetre>` a la línia de comandes.

Aquí, volem crear un paràmetre anomenat `--input`, així que hem de declarar `params.input` en algun lloc del workflow.
En principi podem escriure-ho en qualsevol lloc; però com que el volem donar a la crida del procés `sayHello()`, podem connectar-lo allà directament escrivint `sayHello(params.input)`.
...
Si heu fet totes aquestes edicions correctament, hauríeu d'obtenir una altra execució reeixida.
...
!!! tip "Consell"
    ...
    Podeu distingir fàcilment els paràmetres de nivell de Nextflow dels paràmetres de nivell de pipeline.
...
??? question "Si no ha funcionat"
    ...
    Consulteu el material d'ajuda [Versions de Nextflow](../info/nxf_versions.md).
...
Busqueu el fragment `cached:` que s'ha afegit a la línia d'estat del procés (línia 5), que significa que Nextflow ha reconegut que ja ha fet aquesta feina i simplement ha reutilitzat el resultat de l'execució reeixida anterior.
...
Busqueu l'execució reeixida més recent on no vau utilitzar `-resume`; en el nostre cas el nom d'execució era `golden_cantor`.
...
!!! Warning "Advertència"
    ...
    Sou responsables de desar qualsevol sortida que us importi o en què tingueu previst confiar! Aquesta és la raó principal per la qual preferim utilitzar el mode `copy` en lloc del mode `symlink` per a la directiva `publish`.
