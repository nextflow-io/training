...
Això està bé. Però és, és força simplista. Tenim una variable en aquest paràmetre, que va a un procés que s'executa una vegada, i no escala realment. I no podem donar-li molts fitxers diferents per crear aquí. No podem donar-li moltes salutacions diferents. Només en tenim una.
...
Tingueu en compte que he codificat aquesta cadena de moment. Això és una mica un pas enrere després del nostre bon paràmetre que vam utilitzar al final del capítol anterior, però només manté les coses simples per començar perquè pugueu veure la lògica.
...
D'acord, així que el nostre canal té una cosa que és bonica, però és bàsicament el mateix que era abans. Així que fem-ho una mica més complicat. Afegim algunes coses més al nostre canal.
...
Puc utilitzar un array en un canal? Provem-ho. Crearé un array, i he copiat això dels docs, _"greetings_array"_ i _"Hello", "Bonjour"_ i _"Holà"_. I després posaré això aquí en lloc de les meves cadenes codificades. Així que diré "channel.of" _"greetings_array"_, passant aquest array a un canal. Provem-ho.
...
En aquest cas, utilitzarem quelcom anomenat _flatten_. Que passem al final del canal aquí. Així que creem el canal i després executem _flatten_. I de nou, si hi passo per sobre, em mostra la documentació per a aquesta comanda directament a VS Code, que és molt útil. També podeu trobar tots aquests docs al lloc web de Nextflow, la documentació.
...
Així que afegiré de nou una comanda view aquí abans d'executar _flatten_. I aquí aquesta té aquestes claus ondulades, que és el closure dinàmic. I només hi ha algun codi arbitrari dins d'aquí que s'executarà, dins del context d'un operador view.
...
Molt ràpidament, només mencionaria que quan estava creant Channel factories abans, vaig fer _"."_, i després vam veure que hi havia moltes maneres diferents de crear canals, i una d'elles s'anomena "_fromList"_. I això està realment dissenyat específicament per fer aquesta mateixa operació. Així que podríem haver fet simplement from list greetings away, i això funcionarà. És una sintaxi lleugerament neta i més bonica. Però per als propòsits d'aquesta demostració, volíem fer-ho una mica més pas a pas perquè poguéssiu veure com s'està manipulant el canal i com diferents operadors poden canviar el que hi ha al contingut d'un canal.

## 4. Llegir valors d'entrada des d'un fitxer CSV

D'acord, com podem fer això una mica més realista? Probablement no voldreu estar creant molt codi al vostre pipeline de Nextflow amb arrays codificats. Probablement voldreu prendre les dades de fora quan llanceu, i aquestes dades gairebé certament estaran en fitxers.
...
D'acord, fem una ullada a aquest canal i vegem com es veu. Podem fer _".view",_ i he copiat això del lloc web. Així que _.view_, i tenim un closure dinàmic aquí i diem un nom de variable "_csv"_ com a entrada. Així que això són els continguts del canal, i diem abans de splitCsv, i així és com es veu.

Si l'executo de nou, encara fallarà, però ens mostrarà què hi ha dins d'aquest canal. No és particularment emocionant. És aquesta variable _path_. Així que podeu veure que només és una cadena aquí perquè s'està imprimint a un terminal, però és un objecte _path_, que conté la informació i metadades sobre aquest fitxer.
...
Per fer això, utilitzarem _map_. Bàsicament _map_ només diu, executa algun codi, alguna funció sobre cada element que em donen i fes algun tipus de transformació sobre ell. I perquè és tan flexible, el veureu aparèixer en el codi de Nextflow tot el temps.

Per si mateix, no fa res. Així que no volem parèntesis regulars, volem un closure aquí i necessitem dir-li què fer. Així que diré _"row"_, perquè se li estan donant files del CSV, així que és un nom de variable lògic. És l'entrada. I vull retornar només el primer element d'aquest array.
...
Efectivament, després de _splitCsv_ tenim els nostres arrays, i després després del _map,_ tenim les nostres cadenes netes i boniques, només _"Hello", "Bonjour"_ i _"Holà"_. I el pipeline ara està fent el que volem. Fantàstic.

Així que podem desfer-nos de totes aquestes comandes view ara. Ja no les necessitem.

## Resum

Hem acabat el nostre tipus de depuració i aquest és el codi amb què acabem. Prenent el nostre paràmetre CLI anomenat _input_, que està classificat com a _Path_. Nextflow troba el path, el carrega, i entén el fitxer CSV. Retorna totes les diferents files. I després mapegem només el primer element d'aquesta fila al canal que tipus de dóna els continguts del canal, que es passa al procés.
...
D'acord. Esperem que ara estigueu aconseguint una sensació del que són els canals i operadors de Nextflow i com funcionen els operadors sobre els canals i com podeu crear-los.
...
Els canals no són el concepte més fàcil d'entendre. Així que feu una pausa, penseu una mica sobre això, potser llegiu de nou el material, i assegureu-vos realment que teniu aquests conceptes clars perquè això és clau per a la vostra comprensió de Nextflow i com millor entengueu els canals i els diferents operadors de canal i les diferents Channel factories. Més diversió tindreu escrivint Nextflow i més poderosos seran els vostres pipelines.
