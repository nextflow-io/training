...
Un cop esteu dins d'aquell contenidor, normalment funciona com un sistema operatiu complet. Està tallat del món exterior. Està separat de tot la resta, i això és una cosa bona. Així és com obtenim una reproducibilitat tan bona amb Nextflow.
...
És molt comú que el programari que necessitem estigui empaquetat a Conda, a Bioconda, o canals conda-forge o altres canals més específics de domini. I Wave i Seqera Containers és realment bo construint imatges a partir d'això.
...
Wave i Seqera Containers poden fer molt més que això, però aquest és una mena de cas d'ús simple, que és el més comú. I hauria de dir que aquestes imatges s'allotgen durant almenys cinc anys. Així que podeu construir aquestes URL als vostres pipelines i saber que no desapareixeran aviat.
...
Així que si torno enrere i agafo la meva comanda run i eliminaré cowpy al final allà, perquè en realitat no vull executar cowpy. Vull executar un terminal Bash.
...
Per descomptat, no és súper útil només tenir el contenidor i cap accés a res en absolut. Pot ser segur i reproducible, però no és molt útil.
...
I llavors aquí baix al bloc de sortida, mycowpy, claus path. Ups. Hello containers. Mode, copy.
...
D'acord. Última petita cosa. Executem aquesta comanda de nou ràpidament, resume aquesta vegada, i donem una ullada ràpida al directori de treball per veure què és el que Nextflow està fent sota el capó per fer que tot això funcioni per a nosaltres.
...
I així podeu veure que Nextflow està agafant la lògica del workflow, que és la cosa que realment ens importa, que és específica de la nostra anàlisi, i està fent tota la cosa intel·ligent entre bastidors per fer que Docker funcioni al nostre sistema.
...
I això és realment el superpoder de Nextflow. És reproducibilitat i portabilitat. I amb Nextflow podeu compartir realment el vostre workflow i altres persones poden executar-lo als seus sistemes i simplement funcionarà.
