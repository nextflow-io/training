...
Així que comencem de manera molt simple. Comencem amb "echo", per imprimir alguna cosa al terminal. "Hello World". Premo enter i això va al terminal. Hello World. Esperem que això no sigui una sorpresa per a ningú que estigui veient aquest curs.

D'acord, fem alguna cosa amb això. En lloc de només imprimir-ho al terminal, escrivim-ho a un fitxer. Premeré la tecla de cursor amunt del meu teclat, que recorre l'historial de Bash, així que em dóna la meva última comanda, i hi afegiré al final, un petit símbol de major que, que redirigeix la sortida d'aquesta comanda a un fitxer, i l'anomenaré output.txt.

Enter de nou, per executar aquesta comanda, res al terminal aquesta vegada, però podem veure al costat esquerre que ha aparegut el nou fitxer aquí, anomenat output.txt.

Podem veure això al terminal amb alguna cosa com cat. Així que cat output.txt i efectivament diu "Hello World". També podem fer doble clic i s'obre a l'editor de codi a VS Code.
...
Com he dit, tots els diferents capítols d'aquest curs comencen amb un script i aquest s'anomena Hello World. Així que trobaré Hello World. El previsualitza si hi faig un sol clic, faré doble clic per obrir-lo a l'editor aquí. I eliminaré ràpidament el terminal.
...
Realment només hi ha un requisit per al procés, que és que inclogui algun tipus de bloc script o exec. Això està entre les triples cometes aquí, i aquest és el script bash que s'escriu al directori de treball quan executem el pipeline i és la cosa que realment s'executa al vostre ordinador o servidor.
...
Fins i tot puc fer clic a previsualitzar DAG aquí a VS Code. El DAG és una representació d'una estructura de flux de dades al pipeline, i podem veure-ho representat al costat com un diagrama mermaid. En aquest cas és molt simple. Hi ha una caixa, que és el workflow i un procés, que s'anomena sayHello, però això podria semblar més interessant a mesura que avancem.
...
I realment la cosa important que estem buscant aquí és _aquí_, que és un resum de les diferents tasques que s'han executat.

Si el vostre es veu així amb un petit tic verd, doncs molt bé. Acabeu d'executar el vostre primer pipeline. Fantàstic.

Ens diu aquí el nom del procés, que s'ha executat, que s'anomenava Say Hello, i ens va dir que s'ha executat una vegada i que ha tingut èxit. Això s'actualitza a mesura que avanceu, així que quan esteu executant un pipeline més gran, veureu el progrés representat aquí. Però com que això és tan petit, s'executa bàsicament immediatament.

## 1.2.2. Trobar la sortida i els registres al directori work

Ara quan executeu un pipeline de Nextflow, cadascun d'aquests processos està unit, i cada procés, com he dit abans, pot generar tasques una o múltiples. Així que en aquest cas, teníem una sola tasca d'aquest procés. Només s'ha executat una vegada i això s'ha fet sota aquest _hash_ de tasca.
...
Quan llisto aquests fitxers, podeu veure que el fitxer output.txt s'ha generat. Podeu veure-ho aquí també. I hi ha un munt de fitxers ocults, que no es mostren amb el meu "ls" regular.

Si faig clic a output.txt, efectivament, tenim la nostra sortida. Fantàstic. Així que el pipeline ha funcionat.
...
Després hi ha _.command.error_, _.command.log_ i _.command.out_. Aquestes són totes sortides de la comanda bash o aquest script que s'ha executat. Això és error estàndard. Això és sortida estàndard, i això són les dues combinades tal com van sortir. Així que obteniu l'ordre lògic.
...
Accidentalment vaig fer un error tipogràfic allà, però aquesta és una bona oportunitat per assenyalar també algunes de les característiques de l'extensió Nextflow VS Code. Podeu veure que immediatament em va donar una petita línia vermella ondulada sota això dient que alguna cosa està malament. I si hi passo per sobre, em dirà que aquesta variable no està definida. No sé què és.
...
Ara és porpra. L'analitzador de sintaxi de Nextflow sap que això és un procés i quan hi passo per sobre, em dóna una representació reduïda de com es veu aquest procés. Així que puc veure molt ràpidament d'una ullada que no pren cap entrada i ens dóna aquesta sortida. Així que treballar a VS Code amb aquesta extensió us dóna molta informació contextual mentre esteu escrivint codi.

Tingueu en compte que podem referir-nos a la sortida d'aquest procés amb la sintaxi _.out_. I de moment podem anomenar-ho com vulguem, és només un nom de variable arbitrari.
...
Si faig _"ls -l results"_, veureu que això està realment enllaçat simbòlicament al directori work. Així que això no és un fitxer real, està enllaçat al directori work i ha recollit tots els fitxers allà per a nosaltres.
...
Ara, si vaig a _"ls -l results/hello_world/"_ podeu veure que això és un fitxer real i ja no és un enllaç simbòlic, i Nextflow ho va copiar. Bo de saber. Així que path i mode són coses que us trobareu escrivint força.
...
No obstant això, com estem en aquesta fase de transició ara mateix, és molt possible que vegeu pipelines en estat salvatge, que utilitzeu alguna cosa anomenada publishDir, que és l'antiga manera de fer-ho, i això es defineix no al nivell de workflow i output, sinó que es defineix a nivell de procés.
...
Però seria més interessant si no estigués tot codificat. Així que a continuació, pensem en com dir a Nextflow que aquest procés pot prendre una entrada variable, que és alguna cosa que podem controlar en temps d'execució quan llancem un workflow.
...
La part val és l'equivalent d'un path aquí a baix. Diu a Nextflow que això és una variable, com una cadena en aquest cas. I si hi passeu per sobre de nou, us diu de l'extensió del que això significa.

A continuació direm a Nextflow què fer amb això. No és suficient només dir que hi ha una variable. Heu de dir a l'script com utilitzar aquesta variable. I així que eliminaré aquesta cadena codificada aquí, i posaré una variable.
...
Podríem codificar-ho de nou, com Hello World, i això funcionaria bé, però òbviament no ens dóna realment cap avantatge. Volíem poder configurar això en temps d'execució, així que volem poder fer-ho a la CLI, quan llancem Nextflow.
...
Així que hem passat dinàmicament una entrada d'una CLI a un paràmetre. Hem passat això com una entrada al procés i el procés ho ha interpretat i ho ha posat en un bloc script, que després ha canviat dinàmicament la sortida d'aquest resultat de script. Bastant interessant.
...
## 3.4. Utilitzar valors per defecte per als paràmetres de línia de comandes
...
Així que diu que esperem que la seva entrada sigui obligatòria, però no es va especificar a la línia de comandes. Molt bé.
...
A la sortida aquí, podeu veure que els resultats estaven en memòria cau. I de fet, aquest hash de tasca aquí és exactament el mateix que l'execució anterior, i només ha reutilitzat aquest directori work en la seva totalitat. Les entrades i les sortides i l'script no van ser modificats. I així que només pren aquest fitxer d'això i si hi ha passos posteriors al procés, els passaria al següent pas del pipeline.

Així que encara està executant tot el pipeline de principi a fi, però està utilitzant resultats en memòria cau per a cadascuna d'aquestes tasques, on pot.

Ara, quan feu _-resume_, només reprèn l'última execució de pipeline al vostre directori de treball, el que sigui que fos. Però en realitat podeu reprendre des de qualsevol execució anterior que hàgiu fet allà. I n'hem fet força ara.
...
Si teniu curiositat, per cert, tots aquests adjectius i noms de científics estan al codi font de Nextflow. És una manera realment bona d'aconseguir la vostra primera pull request a Nextflow anant i trobant-ho i afegint el vostre científic favorit.

I de totes maneres, així que vaig fer això i va tornar i va mirar els resultats en memòria cau d'aquesta execució de workflow, va adonar-se que encara podia reutilitzar-los, i ho va fer. Així que vaig obtenir els resultats en memòria cau de nou.

## 4.3. Eliminar directoris work antics

Això és genial. Què passa si vull netejar aquests directoris work? Hi ha càrregues d'ells aquí. Hi ha càrregues de fitxers. Potser sé amb certesa que vull reprendre de les últimes parelles d'execucions de pipeline, però no m'importen totes les anteriors.
...
Això sembla sensible. Així que faré la mateixa comanda de nou, però en lloc de _"-n"_ faré _"-f"_ per fer realment la neteja. I aquesta vegada ha eliminat realment tots aquests directoris. I si entro i miro els directoris work, ara està semblant molt més lleuger. Fantàstic.
