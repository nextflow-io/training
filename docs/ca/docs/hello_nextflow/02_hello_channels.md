...
Per això serveixen els [**canals**](https://nextflow.io/docs/latest/channel.html) de Nextflow.
...
En aquesta part del curs, aprendreu a utilitzar un canal per gestionar múltiples entrades de diverses fonts diferents.
...
Apreneu a utilitzar canals per fer que el workflow itere sobre múltiples valors d'entrada.
...
Al bloc process, feu els següents canvis de codi:
...
!!! tip "Consell"
    ...
    A la definició de sortida, HEU d'utilitzar cometes dobles al voltant de l'expressió del nom de fitxer de sortida (NO cometes simples), altrament fallarà.
...
Executem-lo. Tingueu en compte que hem tornat a executar amb la configuració de registre ANSI per defecte.
...
Sí! I cadascuna té el contingut esperat.
...
Aquesta vegada funciona I ens dóna la informació addicional sobre com es veu el contingut del canal abans i després d'executar l'operador `flatten()`.
...
!!! tip "Consell"
    ...
    Aquí hem escollit no utilitzar-lo per demostrar l'ús d'un operador en un cas d'ús simple.
...
Apreneu a fer que el workflow prengui un fitxer com a font dels valors d'entrada.
...
Oh no, no funciona. Doneu una ullada a l'inici de la sortida de la consola i al missatge d'error.
...
Feu un gran descans, heu treballat dur en aquest!
