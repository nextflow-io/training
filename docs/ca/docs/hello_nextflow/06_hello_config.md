...
Hi ha múltiples maneres de fer-ho, que es poden utilitzar en combinació i s'interpreten segons l'[ordre de precedència](https://nextflow.io/docs/latest/config.html) descrit a la documentació de configuració.

En aquesta part del curs, us mostrarem el mecanisme de fitxer de configuració més simple i comú, el fitxer [`nextflow.config`](https://nextflow.io/docs/latest/config.html), que ja vau trobar a la Part 5: Hello Containers.
...
No obstant això, potser voleu sobreescriure aquests valors per defecte sense haver d'especificar paràmetres a la línia de comandes o modificar el fitxer d'script original.
...
Aquest és l'enfocament més simple, tot i que possiblement el menys flexible ja que el fitxer principal `nextflow.config` no és quelcom que vulgueu estar editant per a cada execució.
...
Però en l'esperit de moure la configuració completament al fitxer de configuració, eliminem aquests valors del fitxer de workflow completament.
...
Utilitzar un fitxer de paràmetres pot semblar excessiu quan només teniu uns pocs paràmetres per especificar, però alguns pipelines esperen desenes de paràmetres.
...
Sabeu com aprofitar opcions clau de configuració per gestionar entrades de workflow.
...
Fins ara hem estat codificant tots els camins per a les declaracions de sortida a nivell de workflow, i com vam notar quan vam començar a afegir múltiples sortides, pot haver-hi una mica de repetició involucrada.
...
Això publica sortides a `custom-outdir-cli/` en lloc de `results/`:
...
#### 2.1.2. Eliminar camins codificats del bloc output

El prefix `hello_config/` estava codificat en capítols anteriors, però ja que ara estem aprenent a configurar camins de sortida de manera flexible, podem eliminar aquesta codificació.
...
Això publica sortides a `custom-outdir-config/my_run/`.
...
Això publica sortides a `custom-outdir-config-2/rep2/`, amb el camí base especificat _i_ el subdirectori del nom del batch _i_ resultats agrupats per procés:
...
Finalment, en l'esperit de reduir la quantitat de codi repetitiu, podem reemplaçar les declaracions `mode` per sortida amb una sola línia a la configuració.
...
Això publica sortides a `config-output-mode/`, i encara són totes còpies adequades, no enllaços simbòlics.
...
Hi ha moltes altres opcions que podeu personalitzar d'aquesta manera, però esperem que això us doni una idea de la gamma d'opcions i com utilitzar-les efectivament per adaptar-se a les vostres preferències.

### Conclusió

Sabeu com controlar la nomenclatura i estructura dels directoris on es publiquen les vostres sortides, així com el mode de publicació de sortida del workflow.
...
!!! tip "Consell"
    ...
    Recomanem utilitzar la consulta de cerca de [Seqera Containers](https://seqera.io/containers/), que us donarà un URI que podeu copiar i enganxar, fins i tot si no esteu planejant crear un contenidor a partir d'ell.
...
??? info "Barrejar i combinar Docker i Conda"
    ...
    Ja que aquestes directives s'assignen per procés, és possible 'barrejar i combinar', _és a dir_ configurar alguns dels processos del vostre workflow per executar amb Docker i altres amb Conda, per exemple, si la infraestructura de càlcul que esteu utilitzant suporta ambdós.
    ...
    I com s'ha notat abans, Nextflow suporta múltiples altres tecnologies d'empaquetament de programari i contenidors, així que no esteu limitats només a aquestes dues.
...
L'elecció d'executor s'estableix per una directiva de procés anomenada `executor`.
...
Per establir l'executor per apuntar a un backend diferent, simplement especificaríeu l'executor que voleu utilitzant una sintaxi similar a la descrita anteriorment per a assignacions de recursos (vegeu [documentació d'executors](https://nextflow.io/docs/latest/executor.html) per a totes les opcions).
...
Malauradament, cadascun d'aquests sistemes utilitza diferents tecnologies, sintaxis i configuracions per definir com s'hauria de definir i enviar una tasca al planificador rellevant.

??? abstract "Exemples"

    Per exemple, la mateixa tasca que requereix 8 CPUs i 4GB de RAM per ser executada a la cua "my-science-work" necessita ser expressada de les següents maneres diferents depenent del backend.
...
!!! tip "Consell"
    ...
    Nextflow mateix té una [lògica de reintent dinàmic](https://nextflow.io/docs/latest/process.html#dynamic-task-resources) molt elegant integrada per reintentar tasques que fallen a causa de limitacions de recursos.
...
No obstant això, si intentéssiu executar el workflow amb assignacions de recursos que excedeixen aquests límits, després miréssiu la comanda `sbatch` al fitxer d'script `.command.run`, veuríeu que les sol·licituds que realment s'envien a l'executor estan limitades als valors especificats per `resourceLimits`.
...
!!! tip "Consell"
    ...
    Per a més detalls, vegeu la Missió Secundària [Treballant amb Fitxers](../side_quests/working_with_files.md)
...
Això esdevé especialment útil per a projectes complexos que involucren múltiples capes de configuració.
...
Celebreu i doneu-vos una gran palmadeta a l'esquena! Heu completat el vostre primer curs de desenvolupador de Nextflow.
...
Més informació: [1.1. Moure valors per defecte a `nextflow.config`](#11-move-default-values-to-nextflowconfig)
</quiz>
...
<quiz>
Per a què es poden utilitzar els perfils? (Seleccioneu tot el que correspongui)
