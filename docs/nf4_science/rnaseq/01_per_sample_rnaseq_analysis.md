...
In the first part of this course, we show you how to build a simple bulk RNAseq analysis pipeline that applies **[first method]** to individual sequencing samples.
...
A full RNAseq analysis pipeline typically involves a lot of steps, including **[important methods]**.
For simplicity, in this part of the course we are going to focus on just the **[first method]** part.
...
- **[example data]** corresponding to **[contents]**, which have been subset to a **[region?]** to keep the file sizes small.
  This is **[describe data]**.

### Workflow

In this part of the course, we're going to develop a workflow that does the following:

1. **[first method task 1]**
2. **[first method task 2]**

<figure class="excalidraw">
--8<-- "docs/nf4_science/rnaseq/img/IMG.svg"
</figure>

!!! note <!-- NOT SURE IF APPLICABLE -->
...
## 0. Warmup: Test the **[relevant tools]** commands interactively

First we want to try out the commands manually before we attempt to wrap them in a workflow.
The tools we need (**[cite tools]**) are not installed in the GitHub Codespaces environment, so we'll use them via containers (see [Hello Containers](../../hello_nextflow/05_hello_containers.md)).
...
We're going to pull down a **[tool]** container, spin it up interactively and run the **[tool command]** command on one of the **[example data]** files.

#### 0.1.1. Pull the **[tool]** container

```bash
docker pull <CONTAINER URI>
```

#### 0.1.2. Spin up the **[tool]** container interactively

```bash
docker run -it -v ./data:/data <CONTAINER URI>
```

#### 0.1.3. Run the **[method]** command

The [**[tool]** documentation](URL) gives us the command line to run to **[do the thing]**.
...
This should complete immediately, and you should now see a file called **[output file]** in the same directory as the original input file.

```console title="Directory contents"
data/TODO
```

#### 0.1.4. Exit the **[tool]** container

```bash
exit
```

### 0.2. **[method]** with **[tool]**

We're going to pull down a **[tool]** container, spin it up interactively and run the **[tool command]** command on **[previous output]**.

#### 0.2.1. Pull the **[tool]** container

```bash
docker pull <CONTAINER URI>
```

#### 0.2.2. Spin up the **[tool]** container interactively

```bash
docker run -it -v ./data:/data <CONTAINER URI>
```

#### 0.2.3. Run the **[method]** command

The [**[tool]**](URL) gives us the command line to run to perform variant calling on a BAM file.

We need to provide <!-- the BAM input file (`-I`) as well as the reference genome (`-R`), a name for the output file (`-O`) and a list of genomic intervals to analyze (`-L`) --> **[command breakdown]**.
...
The output file **[output file]** is created inside your working directory in the container, so you won't see it in the VS Code file explorer unless you change the output file path.
...
```console title="[OUTPUT]" linenums="26"
TODO
```

**[describe outputs]**

#### 0.2.4. Exit the **[tool]** container
...
You know how to test the **[tool]** **[method]** and **[tool]** **[method]** commands in their respective containers.
