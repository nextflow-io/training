# Partie 3 : Appel de variants conjoint sur une cohorte

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } Traduction assistée par IA - [en savoir plus et suggérer des améliorations](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>

Dans la Partie 2, vous avez construit un pipeline d'appel de variants par échantillon qui traitait les données de chaque échantillon de manière indépendante.
Nous allons maintenant l'étendre pour implémenter l'appel de variants conjoint, comme présenté dans la [Partie 1](01_method.md).
...
Cette partie s'appuie directement sur le workflow produit par la Partie 2.

??? info "Comment commencer à partir de cette section"

    Cette section du cours suppose que vous avez terminé la [Partie 2 : Appel de variants par échantillon](./02_per_sample_variant_calling.md) et que vous disposez d'un pipeline `genomics.nf` fonctionnel.
    ...
    Cela vous donne un workflow complet d'appel de variants par échantillon.
    Vous pouvez tester qu'il s'exécute correctement en exécutant la commande suivante :
...
Nous avons divisé cela en deux étapes :

1. **Modifier l'étape d'appel de variants par échantillon pour produire un GVCF.**
   Cela couvre la mise à jour des commandes et des sorties du processus.
2. **Ajouter une étape de génotypage conjoint qui combine et génotype les GVCF par échantillon.**
   Cela introduit l'opérateur `collect()`, les closures Groovy pour la construction de lignes de commande, et les processus multi-commandes.
...
Le pipeline de la Partie 2 produit des fichiers VCF, mais l'appel conjoint nécessite des fichiers GVCF.
Nous devons activer le mode d'appel de variants GVCF et mettre à jour l'extension du fichier de sortie.

Rappelez-vous la commande d'appel de variants GVCF de la [Partie 1](01_method.md) :
...
### 1.2. Mettre à jour l'extension du fichier de sortie dans le bloc de sorties du processus
...
Puisque nous produisons maintenant des GVCF au lieu de VCF, nous devrions mettre à jour la section `publish:` du workflow pour utiliser des noms plus descriptifs.
...
La sortie Nextflow semble identique à avant, mais les fichiers `.g.vcf` et leurs fichiers d'index sont maintenant organisés dans des sous-répertoires.
...
Lorsque vous modifiez le nom du fichier de sortie d'une commande d'outil, le bloc `output:` du processus et la configuration de publication/sortie doivent être mis à jour pour correspondre.

### Et ensuite ?

Apprenez à collecter le contenu d'un canal et à le transmettre au processus suivant comme une seule entrée.

---

## 2. Ajouter une étape de génotypage conjoint

Nous devons maintenant collecter les GVCF par échantillon, les combiner dans un magasin de données GenomicsDB, et exécuter le génotypage conjoint pour produire un VCF au niveau de la cohorte.
Comme présenté dans la [Partie 1](01_method.md), il s'agit d'une opération à deux outils : GenomicsDBImport combine les GVCF, puis GenotypeGVCFs produit les appels de variants finaux.
...
La première commande prend les GVCF par échantillon et un fichier d'intervalles, et produit un magasin de données GenomicsDB.
La seconde prend ce magasin de données, un génome de référence, et produit le VCF final au niveau de la cohorte.
L'URI du conteneur est le même que pour HaplotypeCaller : `community.wave.seqera.io/library/gatk4:4.5.0.0--730ee8817e436867`.

### 2.1. Configurer les entrées

Le processus de génotypage conjoint nécessite deux types d'entrées que nous n'avons pas encore : un nom de cohorte arbitraire, et les sorties GVCF collectées de tous les échantillons regroupées ensemble.

#### 2.1.1. Ajouter un paramètre `cohort_name`

Nous devons fournir un nom arbitraire pour la cohorte.
Plus tard dans la série de formations, vous apprendrez à utiliser les métadonnées d'échantillon pour ce genre de chose, mais pour l'instant nous déclarons simplement un paramètre CLI en utilisant `params` et lui donnons une valeur par défaut pour plus de commodité.
...
#### 2.1.2. Rassembler les sorties de HaplotypeCaller à travers les échantillons

Si nous devions brancher le canal de sortie de `GATK_HAPLOTYPECALLER` directement dans le nouveau processus, Nextflow appellerait le processus sur chaque GVCF d'échantillon séparément.
Nous voulons regrouper les trois GVCF (et leurs fichiers d'index) afin que Nextflow les transmette tous ensemble à un seul appel de processus.
...
=== "Après"
    ...
    ```groovy title="genomics.nf" hl_lines="4-6"
        ...
        // Collecter les sorties d'appel de variants à travers les échantillons
...
Nous pouvons collecter les GVCF et leurs fichiers d'index séparément (par opposition à les garder ensemble dans des tuples) car Nextflow mettra en scène tous les fichiers d'entrée ensemble pour l'exécution, donc les fichiers d'index seront présents aux côtés des GVCF.

!!! tip

    Vous pouvez utiliser l'opérateur `view()` pour inspecter le contenu des canaux avant et après l'application d'opérateurs de canal.

### 2.2. Écrire le processus de génotypage conjoint et l'appeler dans le workflow
...
Avant de commencer à remplir la définition du processus, il y a une chose à résoudre.
...
Surtout, nous devons construire cette chaîne dynamiquement à partir de tous les fichiers présents dans le canal collecté.
Nextflow (via Groovy) fournit un moyen concis de faire cela :
...
1. `all_gvcfs.collect { gvcf -> "-V ${gvcf}" }` itère sur chaque chemin de fichier et préfixe `-V ` à celui-ci, produisant `["-V A.g.vcf", "-V B.g.vcf", "-V C.g.vcf"]`.
2. `.join(' ')` les concatène avec des espaces : `"-V A.g.vcf -V B.g.vcf -V C.g.vcf"`.
3. Le résultat est assigné à une variable locale `gvcfs_line` (définie avec `def`), que nous pouvons interpoler dans le modèle de commande.

Cette ligne va à l'intérieur du bloc `script:` du processus, avant le modèle de commande.
Vous pouvez placer du code Groovy arbitraire entre `script:` et l'ouverture `"""` du modèle de commande.

Ensuite, vous pourrez faire référence à toute cette chaîne comme `gvcfs_line` dans le bloc `script:` du processus.

#### 2.2.2. Remplir le module pour le processus de génotypage conjoint

Nous pouvons maintenant nous attaquer à l'écriture du processus complet.

Ouvrez `modules/gatk_jointgenotyping.nf` et examinez le squelette de la définition du processus.

Allez-y et remplissez la définition du processus en utilisant les informations fournies ci-dessus, puis vérifiez votre travail par rapport à la solution dans l'onglet "Après" ci-dessous.
...
=== "Avant"
    ...
    /*
     * Combiner les GVCF dans un magasin de données GenomicsDB et exécuter le génotypage conjoint pour produire des appels au niveau de la cohorte
     */
...
Il y a plusieurs choses qui méritent d'être soulignées ici.
...
Les lister garantit que Nextflow met en scène ces fichiers dans le répertoire de travail aux côtés des fichiers qui apparaissent dans les commandes, ce que GATK s'attend à trouver en fonction des conventions de nommage.

La variable `gvcfs_line` utilise la closure Groovy décrite ci-dessus pour construire les arguments `-V` pour GenomicsDBImport.

Ce processus exécute deux commandes en série, tout comme vous le feriez dans le terminal.
GenomicsDBImport combine les GVCF par échantillon dans un magasin de données, puis GenotypeGVCFs lit ce magasin de données et produit le VCF final au niveau de la cohorte.
Le magasin de données GenomicsDB (`${cohort_name}_gdb`) est un artefact intermédiaire utilisé uniquement dans le processus ; il n'apparaît pas dans le bloc de sortie.
...
#### 2.2.4. Ajouter l'appel du processus
...
=== "Après"
    ...
    ```groovy title="genomics.nf" hl_lines="3-12"
        ...
        // Combiner les GVCF dans un magasin de données GenomicsDB et appliquer le génotypage conjoint
...
Nous devons publier les sorties VCF conjointes.
Ajoutez des cibles de publication et des entrées de bloc de sortie pour les résultats du génotypage conjoint.

#### 2.3.1. Ajouter des cibles de publication pour le VCF conjoint

Ajoutez le VCF conjoint et son index à la section `publish:` du workflow :
...
#### 2.3.2. Ajouter des entrées de bloc de sortie pour le VCF conjoint

Ajoutez des entrées pour les fichiers VCF conjoints.
Nous les placerons à la racine du répertoire de résultats puisqu'il s'agit de la sortie finale.
...
Si vous ouvrez le fichier VCF conjoint, vous pouvez vérifier que le workflow a produit les appels de variants attendus.
...
Vous disposez maintenant d'un workflow d'appel de variants conjoint automatisé et entièrement reproductible !
...
Dans la prochaine partie de ce cours, vous apprendrez à exécuter un pipeline d'appel de variants prêt pour la production depuis nf-core et à le comparer au pipeline que vous avez construit manuellement.
