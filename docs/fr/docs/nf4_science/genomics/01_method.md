# Partie 1 : Vue d'ensemble de la méthode et tests manuels
...
Nous allons ici utiliser des outils et des méthodes conçus pour appeler des variants germinaux courts, _c'est-à-dire_ des SNP et des indels, dans des données de séquençage de génome entier.

![Pipeline GATK](img/gatk-pipeline.png)

Un pipeline complet d'appel de variants implique généralement de nombreuses étapes, notamment le mapping sur la référence (parfois appelé alignement du génome) et le filtrage et la priorisation des variants.
Pour simplifier, dans ce cours, nous allons nous concentrer uniquement sur la partie appel de variants.

### Méthodes

Nous allons vous montrer deux façons d'appliquer l'appel de variants à des échantillons de séquençage de génome entier pour identifier les SNP et indels germinaux.
Nous commencerons par une **approche par échantillon** simple qui appelle les variants indépendamment pour chaque échantillon.
...
- **Un génome de référence** constitué d'une petite région du chromosome 20 humain (de hg19/b37) et de ses fichiers accessoires (index et dictionnaire de séquence).
- **Trois échantillons de séquençage de génome entier** correspondant à un trio familial (mère, père et fils), qui ont été réduits à une petite tranche de données sur le chromosome 20 pour maintenir des tailles de fichiers réduites.
...
Les deux principaux outils impliqués sont [Samtools](https://www.htslib.org/), une boîte à outils largement utilisée pour manipuler les fichiers d'alignement de séquences, et [GATK](https://gatk.broadinstitute.org/) (Genome Analysis Toolkit), un ensemble d'outils pour la découverte de variants développé au Broad Institute.
...
L'appel de variants par échantillon traite chaque échantillon indépendamment : l'outil d'appel de variants examine les données de séquençage pour un échantillon à la fois et identifie les positions où l'échantillon diffère de la référence.
...
C'est important en raison de la taille que ces fichiers peuvent atteindre.

Les fichiers BAM sont souvent fournis sans index, donc la première étape de nombreux workflows d'analyse consiste à en générer un en utilisant `samtools index`.
...
#### 1.1.2. Démarrer le conteneur Samtools de manière interactive

Pour exécuter le conteneur de manière interactive, utilisez `docker run` avec les options `-it`.
...
Votre invite de commande change en quelque chose comme `(base) root@a1b2c3d4e5f6:/tmp#`, indiquant que vous êtes maintenant à l'intérieur du conteneur.
...
Vous devriez maintenant voir un fichier appelé `reads_mother.bam.bai` dans le même répertoire que le fichier d'entrée BAM original.
...
#### 1.2.2. Démarrer le conteneur GATK de manière interactive

Démarrez le conteneur GATK de manière interactive avec le répertoire de données monté, comme nous l'avons fait pour Samtools.
...
??? success "Sortie de la commande"
    ...
    L'outil produit une sortie de journalisation détaillée. Les lignes surlignées confirment la réussite de l'exécution.
...
Le fichier de sortie `reads_mother.vcf` est créé dans votre répertoire de travail dans le conteneur, vous ne le verrez donc pas dans l'explorateur de fichiers VS Code à moins de modifier le chemin du fichier de sortie.
Cependant, c'est un petit fichier de test, vous pouvez donc utiliser `cat` pour l'ouvrir et visualiser son contenu.
...
Le fichier de sortie VCF est accompagné d'un fichier d'index appelé `reads_mother.vcf.idx` qui a été automatiquement créé par GATK.
...
GATK offre une méthode alternative appelée appel de variants conjoint à cette fin.

L'appel de variants conjoint implique la génération d'un type spécial de sortie de variants appelé GVCF (pour Genomic VCF) pour chaque échantillon, puis la combinaison des données GVCF de tous les échantillons et l'exécution d'une analyse statistique de « génotypage conjoint ».

![Analyse conjointe](img/joint-calling.png)

Ce qui est spécial dans le GVCF d'un échantillon, c'est qu'il contient des enregistrements résumant les statistiques des données de séquence pour toutes les positions dans la zone ciblée du génome, pas seulement les positions où le programme a trouvé des preuves de variation.
...
Nous devons maintenant tester toutes ces commandes, en commençant par l'indexation des trois fichiers BAM.
...
Nous devons maintenant indexer les trois échantillons afin que GATK HaplotypeCaller puisse les traiter.

#### 2.1.1. Démarrer le conteneur Samtools de manière interactive

Nous avons déjà téléchargé l'image du conteneur Samtools, nous pouvons donc le démarrer directement :
...
#### 2.2.1. Démarrer le conteneur GATK de manière interactive

Nous avons déjà téléchargé l'image du conteneur GATK précédemment, nous pouvons donc le démarrer directement :
...
Si vous utilisez `cat` pour visualiser le contenu, vous verrez qu'il est beaucoup plus long que le VCF équivalent que nous avons généré dans la section 1. Vous ne pouvez même pas faire défiler jusqu'au début du fichier, et la plupart des lignes semblent assez différentes de ce que nous avons vu dans le VCF.
...
Celles-ci représentent des régions non-variantes où l'outil d'appel de variants n'a trouvé aucune preuve de variation, il a donc capturé certaines statistiques décrivant son niveau de confiance dans l'absence de variation.
Cela permet de distinguer deux cas très différents : (1) il existe des données de bonne qualité montrant que l'échantillon est homozygote-référence, et (2) il n'y a pas suffisamment de bonnes données disponibles pour faire une détermination dans un sens ou dans l'autre.
...
La sortie de cette étape est effectivement un répertoire contenant un ensemble de répertoires imbriqués supplémentaires contenant les données de variants combinées sous la forme de plusieurs fichiers différents.
...
Cette deuxième étape utilise encore un autre outil GATK, appelé GenotypeGVCFs, pour recalculer les statistiques de variants et les génotypes individuels à la lumière des données disponibles pour tous les échantillons de la cohorte.
...
C'est un autre fichier raisonnablement petit, vous pouvez donc utiliser `cat` sur ce fichier pour visualiser son contenu, et faire défiler vers le haut pour trouver les premières lignes de variants.
...
C'est finalement l'information que nous cherchons à extraire du jeu de données !
...
Votre invite de commande devrait être revenue à la normale.
Cela conclut les tests manuels des commandes d'appel de variants.
