...
Cette formation est conçue pour Nextflow 25.10.2 ou ultérieur **avec l'analyseur syntaxique v2 ACTIVÉ**.
...
Une fois votre codespace en cours d'exécution, vous devez faire deux choses avant de vous lancer dans la formation : définir votre répertoire de travail pour ce cours spécifique et examiner les ressources fournies.
...
- **Le répertoire `solutions`** contient des fichiers de modules complétés et une solution de la Partie 2 qui peut servir de point de départ pour la Partie 3.
...
**Pour continuer vers la [Partie 1 : Aperçu de la méthode et tests manuels](./01_method.md), cliquez sur la flèche dans le coin inférieur droit de cette page.**
