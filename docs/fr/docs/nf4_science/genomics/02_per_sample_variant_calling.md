...
Maintenant nous devons dire à Nextflow où placer la sortie publiée.
...
#### 2.1.1. Ajouter des déclarations de paramètres pour les fichiers accessoires
...
=== "Après"
    ...
    /*
     * Appelle les variants avec GATK HaplotypeCaller
     */
    process GATK_HAPLOTYPECALLER {
...
GATK sait chercher le fichier d'index BAM et les fichiers accessoires du génome de référence en se basant sur des conventions de nommage, mais Nextflow est indépendant du domaine et ne connaît pas ces conventions.
...
Maintenant nous devons dire à Nextflow où placer les nouvelles sorties.
...
C'est la sortie qui nous intéresse de générer pour chaque échantillon de notre étude.

### À retenir

Vous savez comment créer un workflow modulaire en deux étapes qui effectue un vrai travail d'analyse et est capable de gérer les particularités des formats de fichiers génomiques comme les fichiers accessoires.
...
Non, heureusement ! Faites juste une petite modification du code et Nextflow gérera cela pour vous aussi.
...
!!! tip
    ...
    Cependant elles encombrent votre sortie console, donc nous recommandons de les supprimer lorsque vous avez fini de résoudre le problème.
...
Cela peut être aussi simple qu'un fichier texte listant un chemin de fichier par ligne et rien d'autre, ou le fichier peut contenir des métadonnées supplémentaires, auquel cas il est souvent appelé une feuille d'échantillons (samplesheet).
...
!!! tip
    ...
    Si vous n'êtes pas sûr·e de comprendre ce que font les opérateurs ici, c'est une autre excellente opportunité d'utiliser l'opérateur `.view()` pour voir à quoi ressemble le contenu du canal avant et après leur application.
...
Plus généralement, vous avez appris à utiliser les composants et la logique essentiels de Nextflow pour construire un simple pipeline génomique qui fait un vrai travail, en tenant compte des particularités des formats de fichiers génomiques et des exigences des outils.
