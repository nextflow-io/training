...
Dans la Partie 1, nous avons exécuté un simple workflow Hello World pour comprendre les bases de l'exécution Nextflow.
...
[nf-core](https://nf-co.re/) est une collection communautaire de pipelines Nextflow de haute qualité.
Tous les pipelines nf-core suivent la même structure et les mêmes conventions, ce qui signifie qu'une fois que vous apprenez à en exécuter un, vous pouvez tous les exécuter.
...
- **Contrôle qualité** : Rapports QC automatisés utilisant MultiQC
...
3. **Attribution de spots** : Attribuer les spots de transcription aux cellules segmentées
4. **Contrôle qualité** : Générer des rapports QC complets
...
## 2. Exécuter molkart avec les données de test
...
!!! note "Pourquoi clonons-nous localement ?"
    ...
    Nextflow télécharge automatiquement la version demandée du pipeline pour vous dans `$HOME/.nextflow/assets/nf-core/molkart` et l'exécute depuis là.
    Cependant, pour cette formation, nous clonons le pipeline dans un répertoire local différent afin de pouvoir inspecter le code plus facilement.

### 2.1. Comprendre les exigences de conteneurs

Avant d'exécuter le pipeline complet, apprenons pourquoi les conteneurs sont essentiels pour les pipelines nf-core.

Essayons d'exécuter le pipeline en utilisant le jeu de données de test et les paramètres de la configuration de test molkart :
...
- `--outdir` : Où sauvegarder les résultats

!!! Warning "Cette commande échouera - c'est intentionnel !"

    Nous exécutons délibérément ceci sans conteneurs pour démontrer pourquoi ils sont nécessaires.
...
1. Le pipeline s'attend à ce que des logiciels de bioinformatique spécialisés soient installés
...
1. Lire le paramètre `docker.enabled = true` depuis la configuration
2. Télécharger les images Docker requises (première fois seulement)
...
!!! Tip "Pourquoi les conteneurs sont importants"
    ...
    La plupart des pipelines nf-core **nécessitent** la conteneurisation (Docker, Singularity, Podman, etc.) parce que :

    - Ils utilisent des logiciels de bioinformatique spécialisés non disponibles dans les environnements standard
...
Pendant l'exécution du pipeline, vous verrez une sortie similaire à celle-ci :
...
- Les noms de processus incluent le chemin complet du module (par exemple, `NFCORE_MOLKART:MOLKART:MINDAGAP_MINDAGAP`)
...
- **Progression** : Pourcentage complété et comptage (par exemple, `1 of 1 ✔`)
...
Chaque sous-répertoire contient les sorties d'une étape spécifique du pipeline :

- **mindagap/** : Images remplies de grille de l'étape de prétraitement MindaGap
...
- **anndata/** : Objets AnnData contenant les matrices cellule-par-transcrit et les coordonnées spatiales
- **molkartqc/** : Métriques de contrôle qualité pour l'attribution de spots
- **multiqc/** : Rapport de contrôle qualité complet
...
Nous n'avons exécuté qu'un seul échantillon dans ce jeu de données de test, mais dans une expérience réelle nous aurions ces tables pour chaque échantillon.
...
- Quelles tâches ont été mises en cache vs. exécutées

!!! Tip

    Ces rapports sont incroyablement utiles pour optimiser l'allocation des ressources et résoudre les problèmes de performance.
...
Apprenez à propos du répertoire de travail et comment Nextflow gère les fichiers intermédiaires.
...
Choisissez l'un des hashs de processus de segmentation de la sortie console (par exemple, `[3m/4n5o6p]`) et regardez à l'intérieur :
...
- **Fichiers .command.\*** : Scripts d'exécution Nextflow et journaux (comme avant)
...
!!! Tip
    ...
    Si un processus échoue, vous pouvez naviguer vers son répertoire de travail, examiner `.command.err` pour les messages d'erreur, et même ré-exécuter `.command.sh` manuellement pour déboguer le problème.
...
Apprenez à propos du cache Nextflow et comment reprendre les exécutions de pipeline échouées.
...
2. Si les entrées, le code et les paramètres sont identiques, réutilise le résultat mis en cache
3. Ré-exécute uniquement les tâches qui ont changé ou échoué
...
Remarquez `cached: 2` ou `cached: 1` pour chaque processus - rien n'a été ré-exécuté !

### 5.3. Quand resume est utile

Resume est particulièrement précieux lorsque :

- Un pipeline échoue en raison de limites de ressources (mémoire insuffisante, limite de temps dépassée)
- Vous devez modifier des processus en aval sans ré-exécuter les étapes en amont
- Votre connexion réseau tombe pendant le téléchargement de données
- Vous voulez ajouter des sorties supplémentaires sans refaire les calculs

!!! Warning

    Resume ne fonctionne que si vous n'avez pas changé les données d'entrée, le code du pipeline ou les paramètres.
    Si vous changez l'un de ces éléments, Nextflow ré-exécutera correctement les tâches affectées.

### À retenir

Vous savez comment utiliser `-resume` pour ré-exécuter efficacement les pipelines sans répéter les tâches réussies.

### Et ensuite ?

Maintenant que vous pouvez exécuter nf-core/molkart avec des données de test, vous êtes prêt·e à apprendre comment le configurer pour vos propres jeux de données.
