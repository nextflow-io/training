...
Dans les parties 1 à 3, nous avons appris à exécuter Nextflow, à exécuter un pipeline nf-core et à gérer les entrées avec des fichiers de paramètres et des samplesheets.
...
### 1.2. Précédence de la configuration
...
Cette approche en couches vous permet de conserver les valeurs par défaut dans le pipeline, de les remplacer par des paramètres spécifiques à l'utilisateur·trice et d'effectuer des ajustements rapides en ligne de commande.
...
Commentons ou modifions la ligne `docker.enabled = true` de la partie 2, et voyons comment nous pouvons obtenir le même résultat en utilisant un profil dans molkart.

---

## 2. Utiliser les profils

### 2.1. Que sont les profils ?

Les profils sont des ensembles nommés de configuration qui peuvent être activés avec le flag `-profile` via la commande `nextflow run`.
Ils facilitent le passage d'un scénario de calcul à un autre sans modifier les fichiers de configuration.

Tous les pipelines nf-core sont fournis avec un certain nombre de profils par défaut que nous pouvons utiliser.

### 2.2. Inspecter les profils intégrés
...
- `docker` : Utilise les conteneurs Docker (le plus courant pour le développement local)
- `singularity` : Utilise Singularity/Apptainer (courant sur HPC)
- `conda` : Utilise les environnements Conda
- `apptainer` : Utilise les conteneurs Apptainer

### 2.3. Ré-exécuter avec des profils au lieu de nextflow.config

Maintenant que nous avons désactivé la configuration docker dans notre fichier `nextflow.config` local et que nous comprenons les profils, ré-exécutons le pipeline en utilisant le flag `-profile`.
...
Détaillons ce que fait chaque flag :

- `-profile docker` : Active le profil Docker du `nextflow.config` de molkart, qui définit `docker.enabled = true`
- `-params-file params.yaml` : Charge tous les paramètres du pipeline à partir de notre fichier YAML
- `-resume` : Réutilise les résultats mis en cache des exécutions précédentes
...
## 3. Créer des profils personnalisés
...
2. HPC universitaire avec ordonnanceur Slurm et Singularity
...
!!! Note
    ...
    Nous ne pouvons pas tester le profil HPC dans cet environnement de formation car nous n'avons pas accès à un ordonnanceur Slurm.
...
Les profils personnalisés vous permettent de basculer entre différents environnements informatiques avec un seul flag en ligne de commande.
Utilisez `nextflow config` pour inspecter la configuration résolue avant l'exécution.

### Et ensuite ?

Apprenez à personnaliser les demandes de ressources pour des processus individuels en utilisant le système d'étiquettes de processus nf-core.

---

## 4. Personnaliser les demandes de ressources

### 4.1. Comprendre les étiquettes de processus dans les pipelines nf-core

Pour simplifier, les pipelines nf-core utilisent des [**étiquettes de processus**](https://www.nextflow.io/docs/latest/reference/process.html#process-label) pour standardiser l'allocation des ressources dans tous les pipelines.
...
Remarquez le multiplicateur `task.attempt` - cela permet aux nouvelles tentatives de tâches de demander plus de ressources, si le pipeline est configuré avec `process.maxRetries > 1`.

### 4.2. Remplacer les ressources pour des processus spécifiques
...
Cela entraînera l'échec du pipeline dans notre environnement actuel car nous n'avons pas autant de RAM disponible.
...
Les pipelines nf-core utilisent des étiquettes de processus pour standardiser l'allocation des ressources.
...
## 5. Gérer les ressources dans des environnements contraints
...
Dans un environnement de cluster avec des nœuds plus grands, de telles demandes seraient soumises à l'ordonnanceur.

Dans des environnements contraints comme GitHub Codespaces, sans limites, Nextflow refuserait d'exécuter les processus qui dépassent les ressources disponibles.

### 5.2. Définir des limites de ressources

La directive `resourceLimits` plafonne les demandes de ressources à des valeurs spécifiées :
...
Cela indique à Nextflow : "Si un processus demande plus de 2 CPUs ou 7 GB de mémoire, plafonnez-le à ces limites."

### 5.3. Ajouter des limites de ressources aux profils personnalisés
...
!!! Warning "Avertissement"
    ...
    Définir des limites de ressources trop basses peut entraîner l'échec des processus ou leur exécution lente.
    Le pipeline peut avoir besoin d'utiliser des algorithmes moins gourmands en mémoire ou de traiter les données en plus petits morceaux.
...
Différents profils peuvent avoir différentes limites appropriées à leur environnement.

### Et ensuite ?

Vous avez terminé la formation principale Nextflow pour la bio-imagerie !
...
- **Précédence de la configuration** : Comment Nextflow résout les paramètres à partir de plusieurs sources
...
- **Limites de ressources** : Gestion des environnements contraints avec `resourceLimits`
- **Inspection de la configuration** : Utilisation de `nextflow config` pour déboguer et vérifier les paramètres

Ces compétences en configuration sont transférables à n'importe quel pipeline Nextflow et vous aideront à exécuter des workflows efficacement sur des machines locales, des clusters HPC et des plateformes cloud.
...
- Remplissez le questionnaire du cours pour fournir vos commentaires
