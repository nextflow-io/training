...
| `{PROCESS_LIST}`        | Liste des noms de processus séparés par des virgules |
...
- Utiliser `??? success "Sortie de la commande"` pour la sortie attendue repliable
- Utiliser `??? abstract "Contenu du répertoire"` pour les arborescences de répertoires repliables
