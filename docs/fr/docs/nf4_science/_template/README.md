...
| `{ACCESSORY_FILES}`          | Types de fichiers accessoires utilisés     | index files and reference genome resources                        |
...
| Placeholder            | Description                            | Exemple (Genomics)   |
| ---------------------- | -------------------------------------- | -------------------- |
| `{PRIMARY_INPUT_TYPE}` | Type de fichier d'entrée principal     | BAM file             |
...
| `{FINAL_OUTPUT_TYPE}`  | Type de sortie finale du pipeline      | joint-called VCFs    |
...
| `{PROCESS_LIST}`        | Liste séparée par des virgules des noms de processus |
...
2. **Partie 2 (02_single_sample.md)** : Encapsuler les commandes dans Nextflow ; échantillon unique, puis par lot
