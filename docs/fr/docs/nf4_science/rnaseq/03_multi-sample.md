# Partie 3 : Implémentation multi-échantillons en paired-end

Dans cette dernière partie du cours, nous allons faire passer notre workflow simple au niveau supérieur en le transformant en un puissant outil d'automatisation par lots capable de traiter un nombre arbitraire d'échantillons.
Et pendant que nous y sommes, nous allons également le modifier pour qu'il accepte des données paired-end, qui sont plus courantes dans les études récentes.

Nous procéderons en trois étapes :

1. Faire en sorte que le workflow accepte plusieurs échantillons en entrée et parallélise l'exécution
2. Ajouter la génération de rapports QC complets
3. Passer aux données RNAseq paired-end
...
### 1.2. Mettre à jour la factory de canal d'entrée pour gérer un CSV en entrée

Nous allons vouloir charger le contenu du fichier dans le canal au lieu du simple chemin du fichier, donc nous utilisons l'opérateur `.splitCsv()` pour analyser le format CSV, puis l'opérateur `.map()` pour récupérer l'information spécifique que nous voulons (le chemin du fichier FASTQ).
...
### 1.3. Exécuter le workflow pour tester qu'il fonctionne
...
Tout cela produit beaucoup de rapports QC, et nous ne voulons pas avoir à fouiller dans des rapports individuels.
...
Créons un fichier de module appelé `modules/multiqc.nf` pour héberger le processus `MULTIQC` :
...
Donc l'exemple de syntaxe devient :
...
Mais comme nous voulons les agréger pour tous les échantillons, nous devons ajouter l'opérateur `collect()` afin de rassembler les rapports de tous les échantillons en un seul appel à `MULTIQC`.
...
Dans le contexte du bloc workflow complet, cela finit par ressembler à ceci :
...
### 2.5. Exécuter le workflow pour tester qu'il fonctionne
...
Ce dernier fichier `all_single-end.html` est le rapport agrégé complet, commodément empaqueté dans un seul fichier HTML facile à parcourir.

---

## 3. Activer le traitement de données RNAseq paired-end

Pour le moment, notre workflow ne peut gérer que des données RNAseq single-end.
Il est de plus en plus courant de voir des données RNAseq paired-end, nous voulons donc pouvoir les gérer.

Rendre le workflow complètement agnostique du type de données nécessiterait d'utiliser des fonctionnalités légèrement plus avancées du langage Nextflow, donc nous n'allons pas le faire ici, mais nous pouvons créer une version de traitement paired-end pour démontrer ce qui doit être adapté.

### 3.1. Faire une copie du workflow appelée `rnaseq_pe.nf`

```bash
cp rnaseq.nf rnaseq_pe.nf
```

### 3.2. Modifier le `input_csv` par défaut pour pointer vers les données paired-end

Nous fournissons un second fichier CSV contenant les identifiants d'échantillons et les chemins de fichiers FASTQ paired dans le répertoire `data/`
...
### 3.3. Mettre à jour la factory de canal

Nous devons dire à l'opérateur `.map()` de récupérer maintenant les deux chemins de fichiers FASTQ.
...
### 3.4. Créer une version paired-end du processus FASTQC

Faisons une copie du module pour avoir les deux versions sous la main.

```bash
cp modules/fastqc.nf modules/fastqc_pe.nf
```

Ouvrez le nouveau fichier de module `fastqc_pe.nf` dans l'éditeur de code et effectuez les modifications de code suivantes :

- Changez `fastqc $reads` en `fastqc ${reads}` dans le bloc `script` (ligne 17) pour que l'entrée `reads` soit déballée, puisqu'il s'agit maintenant d'un tuple de deux chemins au lieu d'un seul chemin.
...
Techniquement, cela généralise le processus `FASTQC` d'une manière qui le rend capable de gérer des données RNAseq single-end ou paired-end.

Enfin, mettez à jour l'instruction d'import du module pour utiliser la version paired-end du module.

```groovy title="rnaseq_pe.nf" linenums="4"
include { FASTQC } from './modules/fastqc_pe.nf'
```

### 3.5. Créer une version paired-end du processus TRIM_GALORE

Faites une copie du module pour avoir les deux versions sous la main.

```bash
cp modules/trim_galore.nf modules/trim_galore_pe.nf
```

Ouvrez le nouveau fichier de module `trim_galore_pe.nf` dans l'éditeur de code et effectuez les modifications de code suivantes :
...
Le processus `TRIM_GALORE` produit maintenant un canal de sortie supplémentaire, nous devons donc le fournir à MultiQC.
...
### 3.7. Créer une version paired-end du processus HISAT2_ALIGN
...
Ouvrez le nouveau fichier de module `hisat2_align_pe.nf` dans l'éditeur de code et effectuez les modifications de code suivantes :
...
### 3.8. Exécuter le workflow pour tester qu'il fonctionne
...
Et voilà ! Nous avons maintenant deux versions légèrement divergentes de notre workflow, une pour les données de lectures single-end et une pour les données paired-end.
La prochaine étape logique serait de faire en sorte que le workflow accepte l'un ou l'autre type de données à la volée, ce qui est hors du cadre de ce cours, mais nous pourrions aborder cela dans une suite.

---

### À retenir

Vous savez comment adapter un workflow mono-échantillon pour paralléliser le traitement de plusieurs échantillons, générer un rapport QC complet et adapter le workflow pour utiliser des données de lectures paired-end si nécessaire.

### Et ensuite ?

Félicitations, vous avez terminé le mini-cours Nextflow For RNAseq ! Célébrez votre succès et prenez une pause bien méritée !

Ensuite, nous vous demandons de compléter une très courte enquête sur votre expérience avec ce cours de formation, puis nous vous dirigerons vers une page avec des liens vers d'autres ressources de formation et des liens utiles.
