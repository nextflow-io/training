...
Il existe plusieurs méthodes valides pour traiter et analyser des données RNAseq en vrac.
Pour cette formation, nous suivons la méthode décrite [ici](https://www.bioinformatics.babraham.ac.uk/training/RNASeq_Course/Analysing%20RNA-Seq%20data%20Exercise.pdf) par les Dr. Simon Andrews et Laura Biggins au [Babraham Institute](https://www.babraham.ac.uk/).

Notre objectif est de développer un workflow qui implémente les étapes de traitement suivantes : effectuer un contrôle qualité initial sur les lectures d'un échantillon RNAseq en vrac, découper les séquences d'adaptateurs des lectures, aligner les lectures sur un génome de référence et produire un rapport de contrôle qualité (QC) complet.

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/rnaseq/img/preprocess.svg"
</figure>

- **FASTQC :** Effectuer le QC sur les données de lecture avant découpage avec FastQC
- **TRIM_GALORE :** Découper les séquences d'adaptateurs et effectuer le QC après découpage avec Trim Galore (regroupe Cutadapt et FastQC)
- **HISAT2_ALIGN :** Aligner les lectures sur le génome de référence avec Hisat2
- **MULTIQC :** Générer un rapport QC complet avec MultiQC

Cependant, avant de nous lancer dans l'écriture du code du workflow, nous allons tester les commandes manuellement sur des données de test.
...
## 1. QC initial et découpage des adaptateurs

Nous allons récupérer une image de conteneur qui a `fastqc` et `trim_galore` installés, la lancer de manière interactive et exécuter les commandes de découpage et de QC sur l'un des fichiers de données d'exemple.
...
### 1.2. Lancer le conteneur de manière interactive
...
Votre invite de commande changera pour quelque chose comme `(base) root@b645838b3314:/tmp#`, ce qui indique que vous êtes maintenant à l'intérieur du conteneur.
...
Vous pouvez trouver les fichiers de sortie dans le même répertoire que les données d'origine :
...
### 1.4. Découper les séquences d'adaptateurs avec `trim_galore`

Maintenant, exécutons `trim_galore`, qui regroupe Cutadapt et FastQC, pour découper les séquences d'adaptateurs et collecter les métriques QC après découpage.

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

L'option `--fastqc` fait en sorte que la commande exécute automatiquement une étape de collecte QC une fois le découpage terminé.
...
Nous allons récupérer une image de conteneur qui a `hisat2` installé, la lancer de manière interactive et exécuter la commande d'alignement pour aligner les données RNAseq sur un génome de référence.
...
### 2.2. Lancer le conteneur `hisat2` de manière interactive

```bash
docker run -it -v ./data:/data community.wave.seqera.io/library/hisat2_samtools:5e49f68a37dc010e
```

La commande est la même qu'auparavant, avec l'URI du conteneur approprié.
...
Nous les utiliserons dans un instant, mais créons d'abord une archive tar compressée avec ces fichiers d'index du génome ; nous en aurons besoin plus tard et générer ces fichiers n'est généralement pas quelque chose que nous voulons faire dans le cadre d'un workflow.
...
Nous pouvons maintenant exécuter la commande d'alignement, qui effectue l'étape d'alignement avec `hisat2` puis redirige la sortie vers `samtools` pour écrire la sortie sous forme de fichier BAM.

L'entrée de données de lecture est le fichier `/data/trimmed/ENCSR000COQ1_1_trimmed.fq.gz` que nous avons généré avec `trim_galore` à l'étape précédente.
...
Cela s'exécute presque instantanément car il s'agit d'un très petit fichier de test.
...
Nous allons récupérer une image de conteneur qui a `multiqc` installé, la lancer de manière interactive et exécuter une commande de génération de rapport sur les fichiers de rapport FastQC avant/après.
...
### 3.2. Lancer le conteneur `multiqc` de manière interactive
...
Ici, nous voyons que l'outil a trouvé les trois rapports QC que nous avons générés : le QC initial que nous avons effectué avec `fastqc`, le rapport post-découpage de `cutadapt` (créé via `trim_galore`) et le QC post-alignement produit par `hisat2`.

Les fichiers de sortie se trouvent une fois de plus dans le répertoire de travail :
...
Vous avez testé toutes les commandes individuelles de manière interactive dans les conteneurs appropriés.
