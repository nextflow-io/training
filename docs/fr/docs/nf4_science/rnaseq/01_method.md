...
L'alignement a produit un fichier BAM et un fichier de log avec les statistiques d'alignement.
...
Une fois terminé, vous devriez avoir des fichiers BAM et de log pour les deux échantillons dans le répertoire de travail.
...
Ici, nous voyons que l'outil a trouvé les rapports QC pour les trois échantillons : le QC initial de `fastqc`, les rapports après coupe de `cutadapt` (via `trim_galore`) et les résumés d'alignement produits par `hisat2`.
