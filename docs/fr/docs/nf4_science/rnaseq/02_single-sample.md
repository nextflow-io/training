...
Dans cette partie du cours, nous allons écrire le workflow le plus simple possible qui encapsule toutes les commandes que nous avons exécutées dans la Partie 1 pour automatiser leur exécution, et nous viserons à traiter un échantillon à la fois.

Nous procéderons en trois étapes :

1. Écrire un workflow à une seule étape qui exécute le contrôle qualité initial
2. Ajouter le trimming des adaptateurs et le contrôle qualité post-trimming
3. Ajouter l'alignement sur le génome de référence

!!! warning "Prérequis"

    Vous devez avoir suivi la Partie 1 du cours avant de commencer cette leçon.
    Plus précisément, les sections 2.1-3 créent le fichier d'index du génome (`data/genome_index.tar.gz`) requis pour l'étape d'alignement dans cette leçon.
...
Nous vous fournissons un fichier de workflow, `rnaseq.nf`, qui décrit les principales parties du workflow.
...
Gardez à l'esprit que ce code de workflow est correct mais qu'il n'est pas fonctionnel ; son objectif est simplement de servir de squelette que vous utiliserez pour écrire le workflow réel.
...
### 1.2. Créer un module pour le processus de collecte des métriques de contrôle qualité
...
!!! note
    ...
    Même si les fichiers de données que nous utilisons ici sont très petits, en génomique ils peuvent devenir très volumineux. À des fins de démonstration dans l'environnement de formation, nous utilisons le mode de publication 'symlink' pour éviter les copies de fichiers inutiles. Vous ne devriez pas faire cela dans vos workflows finaux, car vous perdrez les résultats lorsque vous nettoierez votre répertoire `work`.
...
Utilisez une factory de canal `.fromPath()` basique pour créer le canal d'entrée :
...
Nous pourrions utiliser le paramètre `--reads` pour spécifier une entrée depuis la ligne de commande, mais pendant le développement nous pouvons être paresseux·ses et simplement utiliser la valeur de test par défaut que nous avons configurée.
...
Cela devrait s'exécuter très rapidement si vous avez suivi la Partie 1 et avez déjà téléchargé le conteneur.
...
Nous allons utiliser le wrapper Trim_Galore, qui regroupe Cutadapt pour le trimming lui-même et FastQC pour le contrôle qualité post-trimming.

### 2.1. Créer un module pour le processus de trimming et de contrôle qualité
...
Cela devrait également s'exécuter très rapidement, puisque nous travaillons sur un fichier d'entrée si petit.
...
Enfin, nous pouvons exécuter l'étape d'alignement sur le génome en utilisant Hisat2, qui émettra également des métriques de contrôle qualité de type FastQC.
...
### 3.4. Appeler le processus `HISAT2_ALIGN` sur les lectures trimmées produites par `TRIM_GALORE`

Les lectures trimmées se trouvent dans le canal de sortie `TRIM_GALORE.out.trimmed_reads` produit par l'étape précédente.
...
_Nous ajouterons l'agrégation des rapports MultiQC dans la Partie 2, après avoir modifié le workflow pour accepter plusieurs échantillons à la fois._
...
Apprenez à modifier le workflow pour traiter plusieurs échantillons en parallèle, agréger les rapports de contrôle qualité de toutes les étapes pour tous les échantillons, et permettre l'exécution du workflow sur des données RNAseq paired-end.
