...
Tout au long de ce cours de formation, nous travaillerons dans le répertoire `nf4-science/rnaseq/`, dans lequel vous devez vous déplacer lorsque vous ouvrez l'espace de travail de formation.
Ce répertoire contient tous les fichiers de code, les données de test et les fichiers accessoires dont vous aurez besoin.

N'hésitez pas à explorer le contenu de ce répertoire ; le moyen le plus simple de le faire est d'utiliser l'explorateur de fichiers sur le côté gauche de l'espace de travail de formation dans l'interface VSCode.
...
- **Le fichier `rnaseq.nf`** est le plan du script de workflow que nous allons développer.
...
- **Le répertoire `data`** contient les données d'entrée et les ressources associées :
  ...
  - _Un génome de référence_ appelé `genome.fa` constitué d'une petite région du chromosome humain 20 (de hg19/b37).
  - _Des données RNAseq_ qui ont été réduites à une petite région pour limiter la taille des fichiers, dans le répertoire `reads/`.
  - _Des fichiers CSV_ listant les identifiants et les chemins des fichiers de données d'exemple, pour un traitement par lots.

- **Le répertoire `solutions`** contient les scripts de workflow et les modules complétés qui résultent de chaque étape du cours.
