...
-   :material-information-outline:{ .lg .middle } __Informations complémentaires__
    ...
    ??? people "Informations pour les formateur·trices"
        ...
        Si vous êtes formateur·trice et organisez vos propres formations, vous êtes libre d'utiliser nos supports directement depuis le portail de formation à condition d'attribuer le crédit approprié. Voir 'Crédits et contributions' ci-dessous pour plus de détails.
    ...
    ??? licensing "Licence open-source et politique de contribution"
        ...
        Nous accueillons favorablement les améliorations, corrections et rapports de bugs de la communauté. Chaque page dispose d'une icône :material-file-edit-outline: en haut à droite de la page qui renvoie au dépôt de code, où vous pouvez signaler des problèmes ou proposer des modifications au matériel source de formation via une pull request. Voir le `README.md` dans le dépôt pour plus de détails.

</div>

!!! note "Traduction assistée par IA"

    Cette traduction a été créée avec l'assistance de l'IA et révisée par des humains. Nous accueillons vos retours et suggestions d'amélioration. Consultez notre [guide de traduction](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md) pour contribuer.
...
-   :material-walk:{ .lg .middle } __Parcours d'introduction__
    ...
    ??? courses "**Nextflow for RNAseq** (bulk RNAseq)"
        ...
        Pour les chercheur·euses qui souhaitent apprendre à développer leurs propres pipelines RNAseq. Le cours utilise un cas d'usage de traitement de RNAseq en masse pour démontrer comment développer un pipeline RNAseq simple mais fonctionnel.
    ...
    ??? courses "**Nextflow for Imaging** (spatial omics)"
        ...
        Pour les chercheur·euses en imagerie et omiques spatiales qui souhaitent apprendre à exécuter et personnaliser des pipelines d'analyse. Le cours utilise le pipeline nf-core/molkart pour fournir un pipeline biologiquement pertinent démontrant comment exécuter, configurer et gérer les entrées pour les workflows de pipelines Nextflow.
...
-   :material-run:{ .lg .middle } __Parcours avancé__
    ...
    ??? courses "**Side Quests :** Approfondissements sur des sujets autonomes"
        ...
        [Parcourir les Quêtes secondaires :material-arrow-right:](side_quests/){ .md-button .md-button--secondary }
