...
Ensuite, vous avez travaillé à travers des versions de plus en plus complexes de ce workflow et avez appris à reconnaître les concepts et mécanismes essentiels qui alimentent les pipelines Nextflow, y compris les canaux et les opérateurs, la modularisation du code, et les conteneurs.
...
1. **`sayHello` :** Écrit chaque salutation dans son propre fichier de sortie (_par exemple_ « Hello-output.txt »)
2. **`convertToUpper` :** Convertit chaque salutation en majuscules (_par exemple_ « HELLO »)
...
- Décrire des concepts avancés tels que les opérateurs et les fabriques de canal
...
- Apprenez les techniques de dépannage avec la [Quête secondaire Debugging](../side_quests/debugging.md)
