...
Cette formation est conçue pour **Nextflow 25.10.2** ou ultérieur **avec l'analyseur syntaxique v2 DÉSACTIVÉ**.
...
Veuillez vous assurer que vous utilisez les paramètres corrects comme documenté [ici](../info/nxf_versions.md).

La formation nécessite également **nf-core tools 3.4.1**.
Si vous utilisez une version différente des outils nf-core, vous pourriez avoir des difficultés à suivre.

Vous pouvez vérifier quelle version est installée dans votre environnement en utilisant la commande `nf-core --version`.
...
- **Le fichier `greetings.csv`** est un CSV contenant des données tabulaires minimales que nous utilisons à des fins de test.

- **Le répertoire `original-hello`** contient une copie du code source produit en suivant la série complète de formation Hello Nextflow (avec Docker activé).
...
- [ ] Je me suis assuré·e que l'analyseur syntaxique est défini sur **v1**
