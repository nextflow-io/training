...
L'un des grands avantages de travailler avec nf-core est la possibilité de tirer parti de modules pré-construits et testés provenant du dépôt [nf-core/modules](https://github.com/nf-core/modules).
...
Pour démontrer comment cela fonctionne, nous allons remplacer le module personnalisé `collectGreetings` par le module `cat/cat` de nf-core/modules dans le pipeline `core-hello`.
...
Nous allons chercher à remplacer le processus `collectGreetings`, qui utilise la commande Unix `cat` pour concaténer plusieurs fichiers de messages d'accueil en un seul.
La concaténation de fichiers est une opération très courante, il est donc logique qu'il existe peut-être déjà un module dans nf-core conçu à cet effet.

Plongeons-nous dans le sujet.

### 1.1. Parcourir les modules disponibles sur le site web nf-core

Le projet nf-core maintient un catalogue centralisé de modules à l'adresse [https://nf-co.re/modules](https://nf-co.re/modules).
...
Parmi eux, vous devriez voir un module appelé `cat_cat` qui est à usage général.

!!! note "Convention de nommage des modules"

    Le trait de soulignement (`_`) est utilisé comme substitut du caractère barre oblique (`/`) dans les noms de modules.
...
Cliquez sur la boîte du module `cat_cat` pour afficher la documentation du module.
...
Cela affiche la documentation sur le module, y compris ses entrées, sorties et informations d'utilisation de base.
...
Plus précisément, la commande `nf-core modules install` permet d'automatiser la récupération du code et de le rendre disponible pour votre projet en une seule étape.
...
(Si ce n'est pas le cas, passez à "Enfin, l'outil procédera à l'installation du module.")
...
Cependant, pour réellement utiliser le nouveau module, nous devons l'importer dans notre pipeline.

### 1.5. Mettre à jour les imports de modules

Remplaçons l'instruction `include` pour le module `collectGreetings` par celle pour `CAT_CAT` dans la section des imports du workflow `workflows/hello.nf`.

Pour rappel, l'outil d'installation de module nous a donné l'instruction exacte à utiliser :

```groovy title="Instruction d'import produite par la commande d'installation"
include { CAT_CAT } from '../modules/nf-core/cat/cat/main'`
```
...
- **Module nf-core** : `'../modules/nf-core/cat/cat/main'` (référence `main.nf`)
- **Module local** : `'../modules/local/collectGreetings.nf'` (référence à un fichier unique)

Le module est maintenant disponible pour le workflow, donc tout ce que nous avons à faire est de remplacer l'appel à `collectGreetings` pour utiliser `CAT_CAT`. N'est-ce pas ?

Pas si vite.

À ce stade, vous pourriez être tenté·e de vous lancer et de commencer à modifier le code, mais il vaut la peine de prendre un moment pour examiner attentivement ce que le nouveau module attend et ce qu'il produit.

Nous allons aborder cela comme une section séparée car cela implique un nouveau mécanisme que nous n'avons pas encore couvert : les metamaps.
...
(Pour information, il existe une commande `uninstall` pour se débarrasser des modules dont vous décidez que vous ne voulez plus.)

!!! note

    Le processus CAT_CAT inclut une gestion plutôt astucieuse de différents types de compression, extensions de fichiers, etc. qui ne sont pas strictement pertinents pour ce que nous essayons de vous montrer ici, donc nous allons ignorer la plupart de cela et nous concentrer uniquement sur les parties qui sont importantes.
...
À la fin, `collectGreetings` produit un seul chemin de fichier, émis avec le tag `outfile`.
...
À la fin, CAT_CAT délivre ses sorties en deux parties :

- Un autre tuple contenant le metamap et le fichier de sortie concaténé, émis avec le tag `file_out` ;
- Un fichier `versions.yml` qui capture des informations sur la version du logiciel qui a été utilisée, émis avec le tag `versions`.
...
Cela peut sembler beaucoup à retenir en regardant simplement le code, voici donc un diagramme pour vous aider à visualiser comment tout s'assemble.
...
En ignorant le fichier de versions pour l'instant, leur sortie principale est également équivalente (un fichier concaténé), sauf que CAT_CAT émet également le metamap en conjonction avec le fichier de sortie.
...
Nous venons de vous dire que le module CAT_CAT attend un metamap comme partie de son tuple d'entrée.
Prenons quelques minutes pour examiner de plus près ce que c'est.

Le **metamap**, souvent appelé **metamap** en abrégé, est une map de style Groovy contenant des informations sur des unités de données.
...
Par convention, un metamap nf-core est nommé `meta` et contient le champ obligatoire `id`, qui est utilisé pour nommer les sorties et suivre les unités de données.

Par exemple, un metamap typique pourrait ressembler à ceci :
...
En conséquence, chaque unité de données voyage à travers le pipeline avec les métadonnées pertinentes attachées.
...
Cela se traduit approximativement comme suit : si un `prefix` est fourni via le système de paramètres de tâche externe (`task.ext`), utilisez-le pour nommer le fichier de sortie ; sinon créez-en un en utilisant `${meta.id}`, qui correspond au champ `id` dans le metamap.
...
- Remplacer l'appel de `collectGreetings()` par `CAT_CAT` ;
...
!!! note
    ...
    Tous les changements montrés ci-dessous sont effectués dans la logique du workflow dans le bloc `main` du fichier de workflow `core-hello/workflows/hello.nf`.

### 3.1. Créer un metamap

Tout d'abord, nous devons créer un metamap pour `CAT_CAT`, en gardant à l'esprit que les modules nf-core exigent que le metamap contienne au moins un champ `id`.

Puisque nous n'avons pas besoin d'autres métadonnées, nous pouvons rester simple et utiliser quelque chose comme ceci :
...
=== "Après"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="7-8"
        ...
        // crée un metamap avec le nom du lot comme ID
        def cat_meta = [ id: params.batch ]

        // génère de l'art ASCII des messages d'accueil avec cowpy
...
Cela crée un metamap simple où l'`id` est défini sur notre nom de lot (qui sera `test` lors de l'utilisation du profil test).
...
=== "Après"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="10-11"
        ...
        // crée un canal avec des métadonnées et des fichiers au format tuple
...
- `.collect()` rassemble tous les fichiers de la sortie de `convertToUpper` dans une seule liste
...
=== "Après"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="13-14"
        ...
        // concatène les fichiers en utilisant le module nf-core cat/cat
        CAT_CAT(ch_for_cat)
...
Cela complète la partie la plus délicate de cette substitution, mais nous n'avons pas encore tout à fait terminé : nous devons encore mettre à jour la façon dont nous passons la sortie concaténée au processus `cowpy`.
...
=== "Après"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="16-17 20"
        ...
        // concatène les messages d'accueil
        CAT_CAT(ch_for_cat)

        // extrait le fichier du tuple car cowpy n'utilise pas encore les métadonnées
...
- Créer un metamap simple pour l'utiliser avec un module nf-core
