...
Ensuite, nous vous montrerons comment utiliser la création de modules basée sur des modèles pour travailler plus efficacement à l'avenir.
...
## 1. Transformer `cowpy` en module nf-core

Dans cette section, nous allons appliquer les conventions nf-core au module local `cowpy` dans votre pipeline `core-hello`, le transformant en un module qui suit les standards de la communauté nf-core.
...
Nous allons appliquer les conventions nf-core suivantes de manière incrémentale :
...
4. **Standardiser le nommage des sorties avec `ext.prefix`** pour promouvoir la cohérence.
...
!!! warning "Répertoire de travail"
    ...
    Assurez-vous d'être dans le répertoire `core-hello` (la racine de votre pipeline) pour toutes les modifications de fichiers et exécutions de commandes dans cette section.
...
Il s'agit purement d'une convention stylistique (il n'y a pas de justification technique) mais puisque c'est la norme pour les modules nf-core, conformons-nous.

Nous devons effectuer trois séries de modifications :

1. Mettre à jour le nom du processus dans le module
2. Mettre à jour l'instruction d'import du module dans l'en-tête du workflow
...
#### 1.1.2. Mettre à jour l'instruction d'import du module

Les noms de processus sont sensibles à la casse, donc maintenant que nous avons changé le nom du processus, nous devons mettre à jour l'instruction d'import du module en conséquence dans l'en-tête du workflow de `hello.nf` :
...
Nous pourrions utiliser un alias dans l'instruction d'import pour éviter de devoir mettre à jour les appels au processus, mais cela irait quelque peu à l'encontre de l'objectif d'adopter la convention de mise en majuscules.
...
Très bien, cela fonctionne ! Passons maintenant à des modifications plus substantielles.

### 1.2. Mettre à jour `COWPY` pour utiliser des tuples de métadonnées

Dans la version actuelle du pipeline `core-hello`, nous extrayons le fichier du tuple de sortie de `CAT_CAT` pour le passer à `COWPY`, comme montré dans la moitié supérieure du diagramme ci-dessous.

<figure class="excalidraw">
    --8<-- "docs/en/docs/hello_nf-core/img/cowpy-inputs.svg"
</figure>

Il serait préférable que `COWPY` accepte directement les tuples de métadonnées, permettant aux métadonnées de circuler à travers le workflow, comme montré dans la moitié inférieure du diagramme.
...
Une fois que nous aurons fait tout cela, nous exécuterons le pipeline pour tester que tout fonctionne toujours comme avant.

#### 1.2.1. Mettre à jour les définitions d'entrée et de sortie

Retournez au fichier du module `cowpy.nf` et modifiez-le pour accepter des tuples de métadonnées comme montré ci-dessous.
...
Comme vous pouvez le voir, nous avons changé à la fois l'**entrée principale** et la **sortie** en un tuple qui suit le modèle `tuple val(meta), path(input_file)` introduit dans la Partie 3 de cette formation.
Pour la sortie, nous avons également profité de cette occasion pour ajouter `emit: cowpy_output` afin de donner un nom descriptif au canal de sortie.

Maintenant que nous avons changé ce que le processus attend, nous devons mettre à jour ce que nous lui fournissons dans l'appel du processus.

#### 1.2.2. Mettre à jour l'appel du processus dans le workflow

La bonne nouvelle est que cette modification va simplifier l'appel du processus.
Maintenant que la sortie de `CAT_CAT` et l'entrée de `COWPY` ont la même 'forme', c'est-à-dire qu'elles consistent toutes deux en une structure `tuple val(meta), path(input_file)`, nous pouvons simplement les connecter directement au lieu de devoir extraire explicitement le fichier de la sortie du processus `CAT_CAT`.

Ouvrez le fichier de workflow `hello.nf` (sous `core-hello/workflows/`) et mettez à jour l'appel à `COWPY` comme montré ci-dessous.
...
Par conséquent, nous n'avons plus besoin de construire le canal `ch_for_cowpy`, donc cette ligne (et sa ligne de commentaire) peut être entièrement supprimée.
...
Ce n'est techniquement pas requis, mais c'est une bonne pratique de se référer aux sorties nommées chaque fois que possible.
...
Maintenant, voyons ce que nous pouvons faire d'autre pour profiter des modèles de modules nf-core.
...
Par conséquent, nous devons fournir une valeur chaque fois que nous appelons le processus, même si nous serions satisfait·es des valeurs par défaut définies par l'outil.
Pour `COWPY`, ce n'est certes pas un gros problème, mais pour les outils avec de nombreux paramètres optionnels, cela peut devenir assez fastidieux.

Le projet nf-core recommande d'utiliser une fonctionnalité Nextflow appelée [`ext.args`](https://www.nextflow.io/docs/latest/reference/process.html#ext) pour gérer les arguments de l'outil plus commodément via des fichiers de configuration.
...
Ensuite, il suffit de configurer la variable `ext.args` pour contenir les arguments et valeurs que vous souhaitez utiliser dans le fichier `modules.config`, qui centralise les détails de configuration pour tous les modules.
...
Faisons-le.
Ouvrez le fichier du module `cowpy.nf` (sous `core-hello/modules/local/`) et modifiez-le pour référencer `ext.args` comme montré ci-dessous.
...
1. **Dans le bloc `input:`, nous avons supprimé l'entrée `val character`.**
   Dorénavant, nous fournirons cet argument via la configuration `ext.args` comme décrit plus loin.
...
2. **Dans le bloc `script:`, nous avons ajouté la ligne `def args = task.ext.args ?: ''`.**
   ...
   Notez que bien que nous nous référions généralement à `ext.args`, ce code doit référencer `task.ext.args` pour extraire la configuration `ext.args` au niveau du module.
...
Spécifiquement, nous allons ajouter ce petit morceau de code au bloc `process {}` :
...
Ouvrez `conf/modules.config` et ajoutez le code de configuration à l'intérieur du bloc `process {}` comme montré ci-dessous.
...
Vous pouvez probablement imaginer avoir tous les modules d'un pipeline avec leurs `ext.args` spécifiés dans ce fichier, avec les avantages suivants :

- L'**interface du module reste simple** - Elle n'accepte que les entrées essentielles de métadonnées et de fichiers
- Le **pipeline expose toujours `params.character`** - Les utilisateur·trices finaux·ales peuvent toujours le configurer comme avant
...
En utilisant le fichier `modules.config` comme l'endroit où tous les pipelines centralisent la configuration par module, nous rendons nos modules plus réutilisables à travers différents pipelines.
...
Testons que le workflow fonctionne toujours comme prévu, en spécifiant un personnage différent pour vérifier que la configuration `ext.args` fonctionne.
...
Vérifions que la configuration `ext.args` a fonctionné en consultant la sortie.
Trouvez la sortie dans l'explorateur de fichiers ou utilisez le hash de la tâche (la partie `38/eb29ea` dans l'exemple ci-dessus) pour consulter le fichier de sortie :
...
Vous devriez voir l'art ASCII affiché avec le personnage `kosh`, confirmant que la configuration `ext.args` a fonctionné !
...
Cette approche garde l'interface du module concentrée sur les données essentielles (fichiers, métadonnées et tout paramètre obligatoire par échantillon), tandis que les options qui contrôlent le comportement de l'outil sont gérées séparément via la configuration.
...
- **Flexibilité** : Les utilisateur·trices peuvent spécifier les arguments de l'outil via la configuration, y compris des valeurs spécifiques à l'échantillon
- **Cohérence** : Tous les modules nf-core suivent ce modèle
- **Portabilité** : Les modules peuvent être réutilisés sans options d'outil codées en dur
- **Pas de modifications du workflow** : Ajouter ou modifier des options d'outil ne nécessite pas de mettre à jour le code du workflow
...
### 1.4. Standardiser le nommage des sorties avec `ext.prefix`

Maintenant que nous avons donné au processus `COWPY` accès à la metamap, nous pouvons commencer à profiter d'un autre modèle nf-core utile : nommer les fichiers de sortie en fonction des métadonnées.

Ici, nous allons utiliser une fonctionnalité Nextflow appelée `ext.prefix` qui nous permettra de standardiser le nommage des fichiers de sortie à travers les modules en utilisant `meta.id` (l'identifiant inclus dans la metamap), tout en étant capable de configurer les modules individuellement si désiré.
...
(Aucune modification nécessaire au workflow.)

Une fois que nous aurons fait cela, nous exécuterons le pipeline pour tester que tout fonctionne toujours comme avant.

#### 1.4.1. Mettre à jour le module `COWPY`

Ouvrez le fichier du module `cowpy.nf` (sous `core-hello/modules/local/`) et modifiez-le pour référencer `ext.prefix` comme montré ci-dessous.
...
1. **Dans le bloc `script:`, nous avons ajouté la ligne `prefix = task.ext.prefix ?: "${meta.id}"`.**
   Cette ligne utilise l'opérateur `?:` pour déterminer la valeur de la variable `prefix` : le contenu de `task.ext.prefix` s'il n'est pas vide, ou l'identifiant de la metamap (`meta.id`) s'il l'est.
   Notez que bien que nous nous référions généralement à `ext.prefix`, ce code doit référencer `task.ext.prefix` pour extraire la configuration `ext.prefix` au niveau du module.
...
3. **Dans le bloc `output:`, nous avons remplacé `path("cowpy-${input_file}")` par `path("${prefix}.txt")`.**
   Cela réitère simplement ce que sera le chemin du fichier selon ce qui est écrit dans la ligne de commande.

Par conséquent, le nom du fichier de sortie est maintenant construit en utilisant une valeur par défaut sensée (l'identifiant de la metamap) combinée avec l'extension de format de fichier appropriée.

#### 1.4.2. Configurer `ext.prefix` dans le fichier `modules.config`

Dans ce cas, la valeur par défaut sensée n'est pas suffisamment expressive à notre goût ; nous voulons utiliser un modèle de nommage personnalisé qui inclut le nom de l'outil, `cowpy-<id>.txt`, comme nous l'avions avant.
...
Notez qu'une fois de plus nous utilisons des accolades, cette fois pour indiquer à Nextflow d'évaluer la valeur de `meta.id` au moment de l'exécution.
...
Au cas où vous vous poseriez la question, la closure `ext.prefix` a accès au bon morceau de métadonnées car la configuration est évaluée dans le contexte de l'exécution du processus, où les métadonnées sont disponibles.
...
Jetez un œil à la sortie dans le répertoire de résultats.
Vous devriez voir le fichier de sortie cowpy avec le même nommage qu'avant : `cowpy-test.txt`, basé sur le nom de lot par défaut.
...
N'hésitez pas à modifier la configuration `ext.prefix` dans `conf/modules.config` pour vous assurer que vous pouvez changer le modèle de nommage sans avoir à apporter de modifications au code du module ou du workflow.

Alternativement, vous pouvez également essayer de réexécuter ceci avec un paramètre `--batch` différent spécifié sur la ligne de commande pour vous assurer que cette partie est toujours personnalisable à la volée.

Cela démontre comment `ext.prefix` vous permet de maintenir votre convention de nommage préférée tout en gardant l'interface du module flexible.

Pour résumer les avantages de cette approche :

- **Nommage standardisé** : Les fichiers de sortie sont généralement nommés en utilisant les identifiants d'échantillon des métadonnées
- **Configurable** : Les utilisateur·trices peuvent remplacer le nommage par défaut si nécessaire
...
Plutôt bien, non ?
Eh bien, il y a encore une modification importante que nous devons faire pour améliorer notre module afin qu'il corresponde aux directives nf-core.
...
- **`results`** — Le répertoire de sortie original que nous avons utilisé depuis le début pour nos modules locaux, défini individuellement en utilisant des directives `publishDir` par module ;
...
Bien sûr, nous pourrions aller dans chacun de nos modules locaux et mettre à jour manuellement la directive `publishDir` pour utiliser le répertoire `core-hello-results`, mais qu'en est-il la prochaine fois que nous décidons de changer le répertoire de sortie ?

Avoir des modules individuels prendre des décisions de publication n'est clairement pas la voie à suivre, surtout dans un monde où le même module pourrait être utilisé dans beaucoup de pipelines différents, par des personnes qui ont des besoins ou des préférences différents·es.
...
Spécifiquement, le modèle nf-core déclare une directive `publishDir` par défaut (avec une structure de répertoires prédéfinie) qui s'applique à tous les modules sauf si une directive de remplacement est fournie.
...
Faisons cela.
Ouvrez le fichier du module `cowpy.nf` (sous `core-hello/modules/local/`) et supprimez la directive `publishDir` comme montré ci-dessous.
...
Maintenant le `core-hello-results` contient également les sorties du module `COWPY`.
...
Cela peut sembler compliqué, alors examinons chacun des trois composants :

- **`path:`** Détermine le répertoire de sortie en fonction du nom du processus.
  Le nom complet d'un processus contenu dans `task.process` inclut la hiérarchie des imports de workflow et de module (comme `CORE_HELLO:HELLO:CAT_CAT`).
  Les opérations `tokenize` suppriment cette hiérarchie pour obtenir juste le nom du processus, puis prennent la première partie avant tout underscore (le cas échéant), et la convertissent en minuscules.
...
- **`saveAs:`** Filtre quels fichiers publier.
  Cet exemple exclut les fichiers `versions.yml` en retournant `null` pour eux, les empêchant d'être publiés.

Cela fournit une logique cohérente pour organiser les sorties.

La sortie est encore meilleure lorsque tous les modules d'un pipeline adoptent cette convention, alors n'hésitez pas à aller supprimer les directives `publishDir` des autres modules de votre pipeline.
...
Nous n'allons pas réellement faire cette modification, mais n'hésitez pas à jouer avec cela et à voir quelle logique vous pouvez implémenter.

Le point est que ce système vous donne le meilleur des deux mondes : cohérence par défaut et flexibilité pour personnaliser la configuration à la demande.

Pour résumer, vous obtenez :

- **Source unique de vérité** : Toute la configuration de publication se trouve dans `modules.config`
- **Valeur par défaut utile** : Les processus fonctionnent immédiatement sans configuration par module
...
Cela complète l'ensemble des fonctionnalités de module nf-core que vous devriez absolument apprendre à utiliser, mais il y en a d'autres que vous pouvez découvrir dans les [spécifications des modules nf-core](https://nf-co.re/docs/guidelines/components/modules).
...
- Utilisez `ext.args` pour garder les interfaces de module minimales et portables ;
- Utilisez `ext.prefix` pour un nommage de fichier de sortie configurable et standardisé ;
- Adoptez la directive `publishDir` centralisée par défaut pour une structure de répertoire de résultats cohérente.

### Et ensuite ?

Apprenez à utiliser les outils intégrés de nf-core basés sur des modèles pour créer des modules de manière efficace.
...
Le flag `--empty-template` crée un modèle de démarrage propre sans code supplémentaire, facilitant la visualisation de la structure essentielle.

La commande s'exécute de manière interactive, vous guidant à travers la configuration.
Elle recherche automatiquement les informations sur l'outil dans les dépôts de paquets comme Bioconda et bio.tools pour pré-remplir les métadonnées.

Vous serez invité·e à fournir plusieurs options de configuration :

- **Informations sur l'auteur·trice** : Votre nom d'utilisateur·trice GitHub pour l'attribution
- **Label de ressource** : Un ensemble prédéfini d'exigences de calcul.
  Le projet nf-core fournit des labels standard comme `process_single` pour les outils légers et `process_high` pour les outils exigeants.
  Ces labels aident à gérer l'allocation des ressources à travers différents environnements d'exécution.
- **Exigence de métadonnées** : Si le module a besoin d'informations spécifiques à l'échantillon via une map `meta` (généralement oui pour les modules de traitement de données).

L'outil gère la complexité de trouver les informations sur le paquet et de configurer la structure, vous permettant de vous concentrer sur l'implémentation de la logique spécifique de l'outil.
...
Certaines d'entre elles fonctionnent immédiatement, tandis que d'autres sont des espaces réservés que nous devrons remplir, comme décrit ci-dessous.

**Fonctionnalités qui fonctionnent telles quelles :**

- **`tag "$meta.id"`** : Ajoute l'identifiant d'échantillon aux noms de processus dans les journaux pour un suivi plus facile
- **`label 'process_single'`** : Label de ressource pour configurer les exigences CPU/mémoire
...
- **Container et environnement** : Espaces réservés que nous remplirons avec les informations sur le paquet
...
Dans ce cas, nous utilisons le même conteneur pré-construit que précédemment.
...
Pour l'environnement Conda, le code du module spécifie `conda "${moduleDir}/environment.yml"` ce qui signifie qu'il devrait être configuré dans le fichier `environment.yml`.

L'outil de création de module nous a averti·es qu'il ne pouvait pas trouver le paquet `cowpy` dans Bioconda (le canal principal pour les outils bioinformatiques).
...
Pour une soumission à nf-core, nous devrions suivre les valeurs par défaut de plus près, mais pour notre propre usage, nous pouvons simplifier le code de cette manière.
...
- Le nom du paramètre du fichier d'entrée (`input_file` au lieu de `input` générique)
...
Ne vous inquiétez pas trop si cela semble mystérieux ; nous incluons ceci pour être complet·es mais vous pouvez aussi simplement supprimer la section stub si vous ne voulez pas vous en occuper, car elle est complètement optionnelle.
...
- Supprimer la ligne `echo $args` (qui était juste du code d'espace réservé du modèle)
...
Tout ce que nous devons faire pour essayer cette nouvelle version du module `COWPY` est de changer l'instruction d'import dans le fichier de workflow `hello.nf` pour pointer vers le nouveau fichier.
...
### 3.2. Liste de contrôle du contributeur·trice
...
- **`ext.args`** simplifie les interfaces de module en gérant les arguments optionnels via la configuration
- **`ext.prefix`** standardise le nommage des fichiers de sortie
- La **publication centralisée** via `publishDir` configurée dans `modules.config` plutôt que codée en dur dans les modules
