...
Dans cette cinquième partie du cours de formation Hello nf-core, nous vous montrons comment utiliser le plugin nf-schema pour valider les entrées et paramètres du pipeline.
...
Le pipeline échoue immédiatement avec des messages d'erreur clairs et exploitables. Cela économise du temps, des ressources de calcul et de la frustration.
...
- **Conversion en canaux** : Convertit les feuilles d'échantillons validées en canaux Nextflow
- **Génération de texte d'aide** : Génère automatiquement la sortie `--help` à partir des définitions de schéma
- **Résumé des paramètres** : Affiche quels paramètres diffèrent des valeurs par défaut
...
??? info "Que sont les plugins Nextflow ?"
...
Un pipeline nf-core utilisera deux fichiers de schéma séparés, qui correspondent à deux types de validation :

| Fichier de schéma          | Objectif                  | Valide                                                       |
| -------------------------- | ------------------------- | ------------------------------------------------------------ |
| `nextflow_schema.json`     | Validation des paramètres | Drapeaux de ligne de commande : `--input`, `--outdir`, `--batch` |
| `assets/schema_input.json` | Validation des données d'entrée | Contenu des feuilles d'échantillons et fichiers d'entrée |
...
**La validation des paramètres** valide les paramètres de ligne de commande (drapeaux comme `--outdir`, `--batch`, `--input`) :

- Vérifie les types, plages et formats des paramètres
- Garantit que les paramètres requis sont fournis
- Valide que les chemins de fichiers existent
- Définie dans `nextflow_schema.json`

**La validation des données d'entrée** valide la structure des feuilles d'échantillons et fichiers manifestes (fichiers CSV/TSV qui décrivent vos données) :
...
- Garantit que les champs requis sont présents
- Définie dans `assets/schema_input.json`

!!! warning "Ce que la validation des données d'entrée ne fait PAS"

    La validation des données d'entrée vérifie la structure des *fichiers manifestes* (feuilles d'échantillons, fichiers CSV), PAS le contenu de vos fichiers de données réels (FASTQ, BAM, VCF, etc.).

    Pour les données à grande échelle, la validation du contenu des fichiers (comme la vérification de l'intégrité BAM) devrait se faire dans les processus du pipeline s'exécutant sur les nœuds de travail, pas pendant l'étape de validation sur la machine d'orchestration.

### 0.4. Quand la validation doit-elle se produire ?

```mermaid
graph LR
    A[L'utilisateur·trice exécute le pipeline] --> B[Validation des paramètres]
    B -->|✓ Valide| C[Validation des données d'entrée]
    B -->|✗ Invalide| D[Erreur : Corriger les paramètres]
    C -->|✓ Valide| E[Le pipeline s'exécute]
    C -->|✗ Invalide| F[Erreur : Corriger les données d'entrée]
```

La validation devrait se produire **avant** l'exécution de tout processus du pipeline, pour fournir un retour rapide et éviter de gaspiller du temps de calcul.
...
Commençons par ajouter la validation des paramètres à notre pipeline. Cela valide les drapeaux de ligne de commande comme `--input`, `--outdir` et `--batch`.

### 1.1. Configurer la validation pour ignorer la validation du fichier d'entrée
...
- La validation est effectuée par le sous-workflow `UTILS_NFSCHEMA_PLUGIN` pendant l'initialisation du pipeline

Le comportement de validation est contrôlé via la portée `validation{}` dans `nextflow.config`.

Puisque nous allons travailler d'abord sur la validation des paramètres (cette section) et ne configurerons le schéma des données d'entrée qu'à la section 2, nous devons temporairement dire à nf-schema d'ignorer la validation du contenu du fichier du paramètre `input`.

Ouvrez `nextflow.config` et trouvez le bloc `validation` (autour de la ligne 246). Ajoutez `ignoreParams` pour ignorer la validation du fichier d'entrée :
...
- **`ignoreParams`** : Ignorer la validation du contenu du fichier du paramètre `input` (temporaire ; nous réactiverons cela à la section 2)
- **`monochromeLogs`** : Désactiver la sortie colorée dans les messages de validation lorsque défini à `true` (contrôlé par `params.monochrome_logs`)
...
!!! note "Pourquoi ignorer le paramètre input ?"
    ...
    Nous supprimerons ce paramètre à la section 2 après avoir configuré le schéma des données d'entrée.

### 1.2. Examiner le schéma des paramètres

Regardons une section du fichier `nextflow_schema.json` fourni avec notre modèle de pipeline :

```bash
grep -A 25 '"input_output_options"' nextflow_schema.json
```

Le schéma des paramètres est organisé en groupes. Voici le groupe `input_output_options` :
...
Chaque entrée décrite ici a les propriétés clés suivantes qui peuvent être validées :
...
- **`pattern`** : Expression régulière que la valeur doit correspondre
- **`required`** : Tableau des noms de paramètres qui doivent être fournis
- **`mimetype`** : Type MIME de fichier attendu pour la validation

Si vous avez l'œil aiguisé, vous remarquerez peut-être que le paramètre d'entrée `batch` que nous avons utilisé n'est pas encore défini dans le schéma.
...
Au lieu de cela, nf-core fournit un outil GUI interactif qui gère la syntaxe JSON Schema pour vous et valide vos modifications :
...
Votre navigateur s'ouvrira en affichant le constructeur de schéma de paramètres :
...
3. Remplissez les détails du paramètre :
   ...
   - Optionnellement, sélectionnez une icône dans le sélecteur d'icônes (par exemple, `fas fa-layer-group`)
...
Essayez maintenant avec un ensemble de paramètres valides :
...
Vous avez appris à utiliser l'outil interactif `nf-core pipelines schema build` pour ajouter des paramètres à `nextflow_schema.json` et vu la validation des paramètres en action.
L'interface web gère toute la syntaxe JSON Schema pour vous, facilitant la gestion de schémas de paramètres complexes sans édition JSON manuelle sujette aux erreurs.

### Et ensuite ?

Maintenant que la validation des paramètres fonctionne, ajoutons la validation pour le contenu du fichier de données d'entrée.

---

## 2. Validation des données d'entrée (schema_input.json)

Nous allons ajouter la validation pour le contenu de notre fichier CSV d'entrée.
Alors que la validation des paramètres vérifie les drapeaux de ligne de commande, la validation des données d'entrée garantit que les données à l'intérieur du fichier CSV sont structurées correctement.

### 2.1. Comprendre le format greetings.csv
...
C'est un CSV simple avec :

- Trois colonnes (pas d'en-tête)
- Sur chaque ligne : un message d'accueil, une langue et un score
- Les deux premières colonnes sont des chaînes de texte sans exigences de format spéciales
...
2. Traiter le premier élément de chaque ligne comme une chaîne de message d'accueil
3. Garantir que les messages d'accueil ne sont pas vides et ne commencent pas par un espace
4. Garantir que le champ langue correspond à l'un des codes de langue pris en charge (en, fr, es, it, de)
5. Garantir que le champ score est un entier avec une valeur entre 0 et 100
...
Le modèle de pipeline nf-core inclut un `assets/schema_input.json` par défaut conçu pour les données de séquençage paired-end.
Nous devons le remplacer par un schéma plus simple pour notre cas d'usage de messages d'accueil.
...
- **`properties`** : Remplacement de `sample`, `fastq_1` et `fastq_2` par `greeting`, `language` et `score`
  - **`type:`** Impose soit string (`greeting`, `language`) soit integer (`score`)
  - **`pattern: "^\\S.*$"`** : Le message d'accueil doit commencer par un caractère non-espace (mais peut contenir des espaces après)
  - **`"enum": ["en", "fr", "es", "it", "de"]`** : Le code de langue doit être dans l'ensemble pris en charge
...
Pour notre cas simple, nous devons ajouter une ligne d'en-tête à notre fichier de messages d'accueil :
...
4. Lance des messages d'erreur utiles si la validation échoue
...
3. Extraire uniquement les chaînes de messages d'accueil pour notre workflow
...
Cela complète l'implémentation de la validation des données d'entrée en utilisant `samplesheetToList` et les schémas JSON.

Maintenant que nous avons configuré le schéma des données d'entrée, nous pouvons supprimer le paramètre d'ignore temporaire que nous avons ajouté plus tôt.

### 2.6. Réactiver la validation de l'entrée
...
### 2.7. Tester la validation de l'entrée
...
Excellent ! Le pipeline s'exécute avec succès et la validation passe silencieusement.
L'avertissement concernant `--character` est juste informatif puisqu'il n'est pas défini dans le schéma.
Si vous le souhaitez, utilisez ce que vous avez appris pour ajouter également la validation pour ce paramètre !
...
Cela ne correspond pas à notre schéma, donc la validation devrait lancer une erreur.
...
- Quel fichier a échoué la validation
...
Si vous souhaitez pratiquer cela, n'hésitez pas à créer d'autres fichiers d'entrée de messages d'accueil qui violent le schéma d'autres manières amusantes.
