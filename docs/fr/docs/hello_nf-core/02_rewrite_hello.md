...
Créer tout cela à partir de zéro serait très fastidieux, c'est pourquoi la communauté nf-core a développé des outils pour le faire à partir d'un modèle, afin d'amorcer le processus.

Nous allons vous montrer comment utiliser ces outils pour créer une structure de pipeline, puis adapter le code de pipeline « classique » existant sur cette structure nf-core.
...
Cela créera une nouvelle structure de pipeline en utilisant le modèle de base nf-core, personnalisé avec un nom de pipeline, une description et un·e auteur·trice.

```bash
nf-core pipelines create
```

L'exécution de cette commande ouvrira une interface utilisateur textuelle (TUI) pour la création du pipeline :
...
Cette TUI vous demandera de fournir des informations de base sur votre pipeline et vous proposera un choix de fonctionnalités à inclure ou exclure dans votre structure de pipeline.
...
- Sur l'écran Template features, définissez `Toggle all features` sur **off**, puis **activez** sélectivement les éléments suivants. Vérifiez vos sélections et cliquez sur **Continue**.
...
Une fois la TUI fermée, vous devriez voir la sortie console suivante.
...
Il n'y a pas de confirmation explicite dans la sortie console que la création du pipeline a fonctionné, mais vous devriez voir un nouveau répertoire appelé `core-hello`.

Affichez le contenu du nouveau répertoire pour voir combien de travail vous vous êtes épargné·e en utilisant le modèle.
...
C'est beaucoup de fichiers !

Vous devriez en reconnaître beaucoup comme étant les mêmes que ceux que nous avons rencontrés lors de l'exploration de la structure du pipeline `nf-core/demo`.
Mais ne vous inquiétez pas si vous vous sentez encore un peu perdu·e ; nous allons parcourir ensemble les parties importantes au cours de cette formation.
...
Jetons un coup d'œil à ce qui se trouve réellement dans le code.
...
Cela équivaut au workflow `workflows/demo.nf` que nous avons rencontré dans la Partie 1, et sert de workflow de substitution pour notre workflow d'intérêt, avec certaines fonctionnalités nf-core déjà en place.
...
workflow HELLO {
    ...
    //
    // Rassembler et sauvegarder les versions logicielles
    //
...
Comparé à un workflow Nextflow de base comme celui développé dans [Hello Nextflow](../hello_nextflow/index.md), vous remarquerez quelques nouveautés ici (lignes surlignées ci-dessus) :
...
Apprenez à rendre un workflow simple composable en prélude à le rendre compatible nf-core.

---

## 2. Rendre le workflow Hello Nextflow original composable

Il est maintenant temps de se mettre au travail pour intégrer notre workflow dans la structure nf-core.
...
Ouvrons le fichier workflow `hello.nf` pour inspecter le code, qui est montré en entier ci-dessous (sans compter les processus, qui sont dans les modules) :
...
workflow {
  ...
  // rassembler tous les messages de bienvenue dans un fichier
  collectGreetings(convertToUpper.out.collect(), params.batch)

  // générer de l'art ASCII des messages de bienvenue avec cowpy
  cowpy(collectGreetings.out.outfile, params.character)
}
```

Comme vous pouvez le voir, ce workflow a été écrit comme un workflow simple sans nom qui peut être exécuté seul.
Pour le rendre exécutable depuis un workflow parent comme le modèle nf-core l'exige, nous devons le rendre **composable**.
...
### 2.2. Remplacer la construction du canal par `take`

Maintenant, remplacez la construction du canal par une simple instruction `take` déclarant les entrées attendues.

=== "Après"

    ```groovy title="original-hello/hello.nf" linenums="18"
        take:
        // canal de messages de bienvenue
        greeting_ch
    ```
...
!!! note
    ...
    Si vous avez l'extension du serveur de langage Nextflow installée, le vérificateur de syntaxe mettra en évidence votre code avec des ondulations rouges.
    C'est parce que si vous mettez une instruction `take:`, vous devez aussi avoir un `main:`.

    Nous ajouterons cela à l'étape suivante.

### 2.3. Préfacer les opérations du workflow avec l'instruction `main`

Ensuite, ajoutez une instruction `main` avant le reste des opérations appelées dans le corps du workflow.
...
Cela dit essentiellement « voici ce que fait ce workflow ».

### 2.4. Ajouter l'instruction `emit`

Enfin, ajoutez une instruction `emit` déclarant quelles sont les sorties finales du workflow.
...
C'est un ajout complètement nouveau au code par rapport au workflow original.

### 2.5. Récapitulatif des modifications effectuées
...
workflow {
  ...
  // appeler le workflow importé sur le canal de messages de bienvenue
  HELLO(greeting_ch)

  // afficher les sorties émises par le workflow
...
- Tout ce qui est lié à l'introduction des entrées dans le workflow (paramètre d'entrée et construction du canal) est maintenant déclaré dans ce workflow parent.

!!! note

    Nommer le fichier workflow d'entrée `main.nf` est une convention, pas une exigence.

    Si vous suivez cette convention, vous pouvez omettre de spécifier le nom du fichier workflow dans votre commande `nextflow run`.
    Nextflow cherchera automatiquement un fichier nommé `main.nf` dans le répertoire d'exécution.

    Cependant, vous pouvez nommer le fichier workflow d'entrée autrement si vous préférez.
    Dans ce cas, assurez-vous de spécifier le nom du fichier workflow dans votre commande `nextflow run`.
...
Cela signifie que nous avons réussi à mettre à niveau notre workflow HELLO pour qu'il soit composable.

### À retenir

Vous savez comment rendre un workflow composable en lui donnant un nom et en ajoutant des instructions `take`, `main` et `emit`, et comment l'appeler depuis un workflow d'entrée.

### Et ensuite ?

Apprenez à greffer un workflow composable de base sur la structure nf-core.

---

## 3. Adapter la logique du workflow mis à jour dans le workflow de substitution

Maintenant que nous avons vérifié que notre workflow composable fonctionne correctement, revenons à la structure de pipeline nf-core que nous avons créée dans la section 1.
...
Alors comment faire ? Jetons un coup d'œil au contenu actuel du workflow `HELLO` dans `core-hello/workflows/hello.nf` (la structure nf-core).
...
Dans l'ensemble, ce code fait très peu de choses à part quelques tâches administratives liées à la capture de la version de tous les outils logiciels qui sont exécutés dans le pipeline.

Nous devons ajouter le code pertinent de la version composable du workflow original que nous avons développé dans la section 2.

Nous allons aborder cela dans les étapes suivantes :

1. Copier les modules et configurer les importations de modules
...
### 3.1. Copier les modules et configurer les importations de modules

Les quatre processus de notre workflow Hello Nextflow sont stockés comme modules dans `original-hello/modules/`.
Nous devons copier ces modules dans la structure du projet nf-core (sous `core-hello/modules/local/`) et ajouter des instructions d'importation au fichier workflow nf-core.
...
Vous devriez maintenant voir le répertoire de modules listé sous `core-hello/`.
...
Maintenant configurons les instructions d'importation de modules.

Voici les instructions d'importation dans le workflow `original-hello/hello.nf` :
...
Ouvrez le fichier `core-hello/workflows/hello.nf` et transposez ces instructions d'importation comme indiqué ci-dessous.
...
- Nous avons adapté le formatage des instructions d'importation pour suivre la convention de style nf-core.
...
Le projet nf-core a beaucoup de fonctionnalités préconstruites autour du concept de samplesheet, qui est généralement un fichier CSV contenant des données en colonnes.
...
Il y a déjà du code là-dedans qui a à voir avec la capture des versions des outils qui sont exécutés par le workflow. Nous allons laisser cela tranquille pour l'instant (nous traiterons les versions des outils plus tard).
Nous garderons l'initialisation `ch_versions = channel.empty()` en haut, puis insérerons notre logique de workflow, en gardant le code de rassemblement des versions à la fin.
Cet ordre a du sens car dans un vrai pipeline, les processus émettraient des informations de version qui seraient ajoutées au canal `ch_versions` au fur et à mesure que le workflow s'exécute.
...
Cela a l'air bien, mais nous devons encore mettre à jour le nom du canal que nous passons au processus `sayHello()` de `greeting_ch` à `ch_samplesheet` comme indiqué ci-dessous, pour correspondre à ce qui est écrit sous le mot-clé `take:`.

=== "Après"

    ```groovy title="core-hello/workflows/hello.nf" linenums="26"
        // émettre un message de bienvenue (mis à jour pour utiliser la convention nf-core pour les samplesheets)
...
Cela conclut les modifications que nous devons apporter au workflow HELLO lui-même.
À ce stade, nous avons atteint la structure de code globale que nous nous étions fixée comme objectif.
...
Apprenez à adapter la façon dont les entrées sont gérées dans la structure de pipeline nf-core.
...
Le modèle nf-core est livré avec une gestion d'entrée sophistiquée conçue pour des ensembles de données génomiques complexes, nous devons donc l'adapter pour qu'elle fonctionne avec notre fichier `greetings.csv` plus simple.

### 4.1. Identifier où les entrées sont gérées

La première étape consiste à déterminer où la gestion des entrées est effectuée.

Vous vous souvenez peut-être que lorsque nous avons réécrit le workflow Hello Nextflow pour qu'il soit composable, nous avons déplacé la déclaration du paramètre d'entrée d'un niveau vers le haut, dans le workflow d'entrée `main.nf`.
Jetons donc un coup d'œil au workflow d'entrée `main.nf` de niveau supérieur qui a été créé dans le cadre de la structure du pipeline :
...
//
// WORKFLOW : Exécuter le pipeline d'analyse principal en fonction du type d'entrée
//
workflow CORE_HELLO {
...
workflow {
    ...
    main:
    //
    // SUBWORKFLOW : Exécuter les tâches d'initialisation
    //
    ...
    //
    // WORKFLOW : Exécuter le workflow principal
    //
    CORE_HELLO (
        PIPELINE_INITIALISATION.out.samplesheet
    )
    //
    // SUBWORKFLOW : Exécuter les tâches de finalisation
    //
    PIPELINE_COMPLETION (
        params.outdir,
        params.monochrome_logs,
...
Le projet nf-core fait un usage intensif de subworkflows imbriqués, donc cette partie peut être un peu déroutante au premier abord.

Ce qui importe ici, c'est qu'il y a deux workflows définis :

- `CORE_HELLO` est un wrapper mince pour exécuter le workflow HELLO que nous venons de finir d'adapter dans `core-hello/workflows/hello.nf`.
- Un workflow sans nom qui appelle `CORE_HELLO` ainsi que deux autres subworkflows, `PIPELINE_INITIALISATION` et `PIPELINE_COMPLETION`.
...
Un peu de recherche révèle que la gestion des entrées est effectuée par le subworkflow `PIPELINE_INITIALISATION`, ce qui est approprié, qui est importé depuis `core-hello/subworkflows/local/utils_nfcore_hello_pipeline/main.nf`.

Si nous ouvrons ce fichier et faisons défiler vers le bas, nous arrivons à ce morceau de code :

```groovy title="core-hello/subworkflows/local/utils_nfcore_hello_pipeline/main.nf" linenums="76"
    //
    // Créer un canal à partir du fichier d'entrée fourni via params.input
    //
...
Ce code implique certaines étapes d'analyse et de validation qui sont très spécifiques à l'exemple de samplesheet inclus avec le modèle de pipeline nf-core, qui au moment de la rédaction est très spécifique au domaine et ne convient pas à notre projet de pipeline simple.

### 4.2. Remplacer le code du canal d'entrée du modèle

La bonne nouvelle est que les besoins de notre pipeline sont beaucoup plus simples, nous pouvons donc remplacer tout cela par le code de construction de canal que nous avons développé dans le workflow Hello Nextflow original.

Pour rappel, voici à quoi ressemblait la construction du canal (comme vu dans le répertoire des solutions) :
...
Nous devons donc simplement intégrer cela dans le workflow d'initialisation, avec des modifications mineures : nous mettons à jour le nom du canal de `greeting_ch` à `ch_samplesheet`, et le nom du paramètre de `params.greeting` à `params.input` (voir ligne surlignée).
...
Cela complète les modifications que nous devons apporter pour que le traitement des entrées fonctionne.

Dans sa forme actuelle, cela ne nous permettra pas de profiter des capacités intégrées de nf-core pour la validation de schéma, mais nous pourrons ajouter cela plus tard.
...
Sous `core-hello/conf`, nous trouvons deux profils de test modélisés : `test.config` et `test_full.config`, qui sont destinés à tester un petit échantillon de données et un échantillon de taille complète.
Étant donné l'objectif de notre pipeline, il n'y a pas vraiment d'intérêt à configurer un profil de test de taille complète, alors n'hésitez pas à ignorer ou supprimer `test_full.config`.
Nous allons nous concentrer sur la configuration de `test.config` pour qu'il s'exécute sur notre fichier `greetings.csv` avec quelques paramètres par défaut.

#### 4.3.1. Copier le fichier `greetings.csv`

Nous devons d'abord copier le fichier `greetings.csv` à un endroit approprié dans notre projet de pipeline.
Généralement, les petits fichiers de test sont stockés dans le répertoire `assets`, copions donc le fichier depuis notre répertoire de travail.
...
Nous pouvons maintenant mettre à jour le fichier `test.config` comme suit :
...
=== "Avant"
    ...
    params {
        ...
        // Données d'entrée
        // TODO nf-core: Spécifier les chemins vers vos données de test sur nf-core/test-datasets
        // TODO nf-core: Donner tous les paramètres requis pour le test afin que les drapeaux de ligne de commande ne soient pas nécessaires
...
- **Utilisation de `${projectDir}`** : C'est une variable implicite Nextflow qui pointe vers le répertoire où se trouve le script de workflow principal (la racine du pipeline). Son utilisation garantit que le chemin fonctionne quel que soit l'endroit d'où le pipeline est exécuté.
- **Chemins absolus** : En utilisant `${projectDir}`, nous créons un chemin absolu, ce qui est important pour les données de test qui sont livrées avec le pipeline.
...
Et pendant que nous y sommes, resserrons les limites de ressources par défaut pour nous assurer que cela fonctionnera sur des machines très basiques (comme les VM minimales dans Github Codespaces) :
...
Cela complète les modifications de code que nous devons faire.
...
Comme vous pouvez le voir, cela a produit le résumé typique nf-core au début grâce au subworkflow d'initialisation, et les lignes pour chaque module montrent maintenant les noms complets PIPELINE:WORKFLOW:module.
...
Comme vous vous en souvenez peut-être, notre première exécution du workflow nouvellement créé a produit un répertoire appelé `core-hello-results/` qui contenait divers rapports d'exécution et métadonnées.
...
!!! note
    ...
    Pour voir celles-ci s'exécuter en parallèle, essayez d'augmenter l'allocation CPU de votre codespace et les limites de ressources dans la configuration de test.

C'est génial, mais nos résultats de pipeline réels ne sont pas là !

Voici ce qui s'est passé : nous n'avons rien changé aux modules eux-mêmes, donc les sorties gérées par les directives `publishDir` au niveau des modules vont toujours dans un répertoire `results` comme spécifié dans le pipeline original.
...
Si nous voulons qu'ils soient bien organisés comme l'étaient les sorties du pipeline de démonstration, nous devrons changer la façon dont nous configurons les sorties à publier.
Nous vous montrerons comment faire cela plus tard dans ce cours de formation.

<!-- TODO: Mettre à jour ceci une fois que nous aurons mis à jour Hello Nextflow pour utiliser les sorties au niveau du workflow -->

Et voilà ! Cela peut sembler beaucoup de travail pour accomplir le même résultat que le pipeline original, mais vous obtenez tous ces beaux rapports générés automatiquement, et vous avez maintenant une base solide pour profiter de fonctionnalités supplémentaires de nf-core, y compris la validation des entrées et certaines capacités de gestion de métadonnées intéressantes que nous couvrirons dans une section ultérieure.

---

### À retenir

Vous savez comment convertir un pipeline Nextflow classique en un pipeline de style nf-core en utilisant le modèle nf-core.
...
Faites une pause, c'était un travail difficile ! Lorsque vous êtes prêt·e, passez à [Partie 3 : Utiliser un module nf-core](./03_use_module.md) pour apprendre à exploiter les modules maintenus par la communauté depuis le dépôt nf-core/modules.
