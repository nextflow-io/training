...
## Version de syntaxe Nextflow actuellement prise en charge et prérequis
...
Vous pouvez accéder aux versions antérieures des supports de formation qui fonctionnent avec les versions de Nextflow **antérieures à 25.10.2** via le menu déroulant en haut de chaque page qui affiche le numéro de version des supports de formation.
...
L'analyseur v2 fournit également de meilleurs messages d'erreur et détecte davantage d'erreurs lors de l'analyse plutôt qu'à l'exécution.
L'analyseur v2 deviendra l'analyseur par défaut dans Nextflow 26.04.
...
Pour rendre cela permanent (en attendant que v2 devienne l'analyseur par défaut dans Nextflow 26.04), ajoutez la commande export à votre profil shell (`~/.bashrc`, `~/.zshrc`, etc.) :
...
À partir de Nextflow 26.04, l'analyseur v2 deviendra l'analyseur par défaut et ce paramètre ne sera plus nécessaire.
...
<!-- Sera-t-il possible de le désactiver dans les versions après 26.04 ? -->
...
Ces deux fonctionnalités sont couvertes dans le cadre de la formation débutant à partir de la version 3.0 des supports de formation.

Selon la génération du code Nextflow que vous avez l'intention de migrer, vous pourrez peut-être effectuer la majeure partie du travail avec le linter Nextflow en utilisant la commande `nextflow lint -format`.
