# Partie 3 : Créer un module nf-core
...
??? info "Comment commencer à partir de cette section"
    ...
    Cette section suppose que vous avez terminé la [Partie 2 : Utiliser un module nf-core](./02_use_module.md) et que vous avez intégré le module `FIND_CONCATENATE` dans votre pipeline.

    Si vous n'avez pas terminé la Partie 2 ou souhaitez repartir de zéro pour cette partie, vous pouvez utiliser la solution `core-hello-part2` comme point de départ.
    Exécutez ces commandes depuis l'intérieur du répertoire `nfcore-build/` :
    ...
    Cela vous donne un pipeline avec le module `FIND_CONCATENATE` déjà intégré.
...
Dans la version actuelle du pipeline `core-hello`, nous extrayons le fichier du tuple de sortie de `FIND_CONCATENATE` pour le passer à `COWPY`, comme indiqué dans la moitié supérieure du diagramme ci-dessous.
...
Comme vous pouvez le voir, nous avons modifié à la fois l'**entrée principale** et la **sortie** en un tuple qui suit le modèle `tuple val(meta), path(input_file)` introduit dans la Partie 2 de cette formation.
...
Maintenant que la sortie de `FIND_CONCATENATE` et l'entrée de `COWPY` ont la même 'forme', c'est-à-dire qu'elles consistent toutes deux en une structure `tuple val(meta), path(input_file)`, nous pouvons simplement les connecter directement au lieu d'avoir à extraire explicitement le fichier de la sortie du processus `FIND_CONCATENATE`.
...
Nous appelons maintenant `COWPY` directement sur `FIND_CONCATENATE.out.file_out`.
...
Le pipeline devrait s'exécuter avec succès, avec les métadonnées circulant maintenant de `FIND_CONCATENATE` à travers `COWPY`.
...
La syntaxe `withName:` assigne cette configuration uniquement au processus `COWPY`, et `#!groovy ext.args = { "-c ${params.character}" }` compose simplement une chaîne qui inclura la valeur du paramètre `character`.
...
1. **Dans le bloc `script:`, nous avons ajouté la ligne `#!groovy prefix = task.ext.prefix ?: "${meta.id}"`.**
...
2. **Dans la ligne de commande, nous avons remplacé `#!groovy cowpy-${input_file}` par `#!groovy ${prefix}.txt`.**
   C'est ici que Nextflow injectera la valeur de `prefix` déterminée par la ligne ci-dessus.

3. **Dans le bloc `output:`, nous avons remplacé `#!groovy path("cowpy-${input_file}")` par `#!groovy path("${prefix}.txt")`.**
...
- **`core-hello-results`** — Le répertoire de sortie défini avec `--outdir` sur la ligne de commande, qui a reçu les journaux nf-core et les résultats publiés par `FIND_CONCATENATE`.
...
"Hé," pourriez-vous dire, "`FIND_CONCATENATE` envoie ses sorties au `--outdir`. Peut-être devrions-nous copier sa directive `publishDir` ?"
...
!!! info "Info"
    ...
    Vous remarquerez peut-être `hello_software_versions.yml` dans `pipeline_info/`.
    Il ne contient pour l'instant que les informations de version de `FIND_CONCATENATE`, car `COWPY` ne rapporte pas encore sa version.
    La section 1.6 explique comment ajouter cela.
...
- **`path:`** Détermine le répertoire de sortie en fonction du nom du processus.
  Le nom complet d'un processus contenu dans `task.process` inclut la hiérarchie des importations de workflow et de modules (comme `CORE_HELLO:HELLO:FIND_CONCATENATE`).
  ...
  C'est ce qui détermine que les résultats de `FIND_CONCATENATE` sont publiés dans `#!groovy ${params.outdir}/find/`.
...
Cela dit, vous pouvez décider que vous voulez organiser vos sorties différemment, et la bonne nouvelle est qu'il est facile de le faire.
...
### 1.6. Capture des versions avec les topic channels

La reproductibilité est un principe fondamental de nf-core : chaque exécution de pipeline enregistre exactement quelles versions de logiciels ont été utilisées, afin que les résultats puissent être reproduits et comparés entre les exécutions.
Le rapport `hello_software_versions.yml` dans `pipeline_info/` est la façon dont les pipelines nf-core satisfont cette exigence.

Le mécanisme sous-jacent repose sur les **topic channels** : une fonctionnalité de diffusion de Nextflow où n'importe quel processus peut publier des données vers un topic nommé, et n'importe quel abonné reçoit tout ce qui a été publié vers ce topic — peu importe d'où cela provient dans le pipeline.
nf-core déploie cette fonctionnalité dans tous les pipelines en 2026.
Les modules publient leur version sous forme de tuple directement dans le bloc de sortie en utilisant le mot-clé `topic:`, et le workflow s'abonne une seule fois pour les collecter tous.

#### 1.6.1. Ajouter le rapport de version à `COWPY`

Ouvrez le fichier du module `cowpy.nf` et ajoutez une ligne de sortie de version comme indiqué ci-dessous.
...
La nouvelle ligne publie un tuple `[process_name, tool_name, version]` vers le topic channel `"versions"`.
Aucune modification du bloc script n'est nécessaire — la version est déclarée statiquement dans le bloc de sortie.

#### 1.6.2. Exécuter le pipeline et inspecter le rapport de versions
...
Ouvrez `core-hello-results/pipeline_info/hello_software_versions.yml` et vous verrez maintenant les deux modules :
...
La collecte côté workflow — le bloc `channel.topic("versions")` que vous avez vu dans le workflow de substitution de la Partie 1 — s'abonne au topic et écrit ce rapport combiné automatiquement.

!!! info "Compatibilité ascendante"

    La branche `versions_file` dans le bloc topic channel du workflow existe pour gérer les modules qui n'ont pas encore été mis à jour pour utiliser `topic: versions` et qui écrivent toujours un fichier `versions.yml` dans le bloc script avec `emit: versions`.
    Les deux styles sont pris en charge simultanément pendant la transition.
...
- Comprenez comment les topic channels collectent automatiquement les versions de logiciels de tous les modules.

Pour approfondir les conventions des modules, consultez les [spécifications des modules nf-core](https://nf-co.re/docs/guidelines/components/modules).
...
!!! tip "En savoir plus sur les tests"
    ...
    Le fichier de test généré utilise nf-test, un framework de tests pour les pipelines et modules Nextflow. Pour apprendre comment écrire et exécuter ces tests, consultez la [quête secondaire nf-test](../side_quests/nf_test/index.md).
...
Le modèle inclut également plusieurs conventions supplémentaires de nf-core.
Certaines fonctionnent directement, tandis que d'autres sont des espaces réservés que nous devrons remplir, comme décrit ci-dessous.
...
- **Sortie `topic: versions`** : Rapport de version via topic channel, comme couvert dans la section 1.6
...
- **Bloc `script:`** : Vide — nous y ajouterons la commande `cowpy`
- **Bloc `stub:`** : Nous le mettrons à jour pour produire les sorties correctes
...
Pour le container, vous pouvez utiliser [Seqera Containers](https://seqera.io/containers/) pour construire automatiquement un container à partir de n'importe quel paquet Conda, y compris les paquets conda-forge.
Dans ce cas, nous utilisons le même container préconstruit qu'auparavant.
...
Pour l'environnement Conda, le code du module spécifie `#!groovy conda "${moduleDir}/environment.yml"`, ce qui signifie qu'il doit être configuré dans le fichier `environment.yml`.

L'outil de création de module nous a averti qu'il ne pouvait pas trouver le paquet `cowpy` dans Bioconda (le canal principal pour les outils de bioinformatique).
Cependant, `cowpy` est disponible dans conda-forge, vous pouvez donc compléter le `environment.yml` comme suit :
...
Le modèle généré inclut des déclarations génériques d'entrée et de sortie que vous devrez personnaliser pour votre outil spécifique.
En nous référant à notre module `COWPY` manuel de la section 1, nous pouvons l'utiliser comme guide.
...
- Le nom du fichier de sortie utilisant le modèle de préfixe configurable (`#!groovy ${prefix}.txt` au lieu du joker `*`)
- Un nom d'émission descriptif (`cowpy_output` au lieu du générique `output`)
- Une chaîne de version statique (`#!groovy val("1.1.5")`) à la place du `#!groovy eval("cowpy --version")` du modèle, correspondant au module manuel de la section 1.6 (l'outil `cowpy` n'expose pas de drapeau `--version`)

Si vous utilisez le serveur de langage Nextflow pour valider la syntaxe, la partie `#!groovy ${prefix}` sera signalée comme erreur à ce stade car nous ne l'avons pas encore ajoutée au bloc script.
Passons à cela maintenant.

#### 2.3.2. Le bloc script

Le modèle génère un bloc script vide que vous devez remplir avec la commande réelle de l'outil.
...
- Remplir la commande `cowpy` réelle qui utilise à la fois `$args` et `#!groovy ${prefix}.txt`
...
Dans le contexte Nextflow, un bloc [stub](https://www.nextflow.io/docs/latest/process.html#stub) permet de définir un script léger et fictif utilisé pour le prototypage rapide et le test de la logique d'un pipeline sans exécuter la commande réelle.
...
- Le stub crée un fichier vide `#!groovy ${prefix}.txt` correspondant à ce que le bloc script produit
...
- Rend vos outils disponibles à l'ensemble de la communauté nf-core via le catalogue de modules sur [nf-co.re/modules](https://nf-co.re/modules)
...
Pour des instructions détaillées, consultez le [tutoriel des composants nf-core](https://nf-co.re/docs/tutorials/nf-core_components/components).

### 3.3. Ressources

- **Tutoriel des composants** : [Guide complet pour créer et contribuer des modules](https://nf-co.re/docs/tutorials/nf-core_components/components)
- **Spécifications des modules** : [Exigences techniques et directives](https://nf-co.re/docs/guidelines/components/modules)
- **Support communautaire** : [nf-core Slack](https://nf-co.re/join) - Rejoignez le canal `#modules`
...
En transformant `COWPY` étape par étape, vous avez développé une compréhension approfondie de ces modèles, vous rendant capable de travailler avec, déboguer et créer des modules nf-core.
...
Quand vous êtes prêt·e, continuez avec la [Partie 4 : Validation des entrées](./04_input_validation.md) pour apprendre comment ajouter une validation des entrées basée sur un schéma à votre pipeline.
