# Partie 2 : Utiliser un module nf-core

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } Traduction assistée par IA - [en savoir plus et suggérer des améliorations](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>


Dans cette deuxième partie du cours de formation Build with nf-core, nous vous montrons comment trouver, installer et utiliser un module nf-core existant dans votre pipeline.
...
Pour démontrer comment cela fonctionne, nous remplacerons le module personnalisé `collectGreetings` par le module `find/concatenate` de nf-core/modules dans le pipeline `core-hello`.

??? info "Comment commencer à partir de cette section"

    Cette section du cours suppose que vous avez terminé la [Partie 1 : Réécrire Hello pour nf-core](./01_rewrite_hello.md) et que vous disposez d'un pipeline `core-hello` fonctionnel.

    Si vous n'avez pas terminé la Partie 1 ou si vous souhaitez repartir de zéro pour cette partie, vous pouvez utiliser la solution `core-hello-part1` comme point de départ.
    Exécutez cette commande depuis le répertoire `nfcore-build/` :
...
Parmi eux, vous devriez voir un module appelé `find_concatenate` qui est générique.

!!! info "Convention de nommage des modules"
...
Cliquez sur la boîte du module `find_concatenate` pour voir la documentation du module.

La page du module affiche :

- Une brève description : "A module for concatenation of gzipped or uncompressed files getting around UNIX terminal argument size"
- La commande d'installation : `nf-core modules install find/concatenate`
...
??? success "Sortie de la commande (tronquée)"
...
Gardez simplement à l'esprit que l'approche `grep` ne récupérera que les résultats avec le terme de recherche dans leur nom.
...
Vous pouvez ignorer le message `INFO Reinstalling modules found in 'modules.json' but missing from directory` ; il est émis par nf-core/tools 4.0.2 pour tout module que vous interrogez avec `info`, qu'il soit ou non réellement installé, et n'a aucun effet puisque la commande `info` n'écrit aucun fichier.

### 1.4. Installer le module find/concatenate
...
Assurez-vous que votre répertoire de travail actuel est la racine du projet de pipeline `core-hello`, puis exécutez la commande d'installation :
...
La commande télécharge les fichiers du module dans `modules/nf-core/find/concatenate/` et met à jour `modules.json` pour suivre le module installé.
Vous pouvez ignorer l'erreur `NotADirectoryError` à la fin ; elle survient parce que nf-core/tools 4.0.2 s'attend à ce que chaque module local soit dans son propre répertoire (`modules/local/<name>/main.nf`), alors que `core-hello` utilise encore des modules locaux sous forme de fichiers uniques à ce stade.
Cependant, le module `find/concatenate` est installé correctement et `modules.json` est mis à jour comme prévu.
Nous convertirons `cowpy` vers la structure en répertoire dans la Partie 4.

Vérifions que les fichiers du module sont bien en place :
...
Vous pouvez également confirmer l'installation en inspectant `modules.json`, qui liste maintenant `find/concatenate` sous le dépôt nf-core/modules.
...
Cela confirme que le module `find/concatenate` fait maintenant partie du code source de votre projet.
Cependant, pour utiliser réellement le nouveau module, nous devons l'importer dans notre pipeline.

Enfin, vous pouvez également utiliser la commande `nf-core modules list local` pour vérifier quels modules sont actuellement suivis dans votre pipeline.
...
Cela affiche `find/concatenate` dans le tableau résultant, accompagné de son dépôt, de son SHA de version, de son message et de sa date.

### 1.5. Mettre à jour les importations de module

Remplaçons l'instruction `include` pour le module `collectGreetings` par celle pour `FIND_CONCATENATE` dans la section des importations du workflow `workflows/hello.nf`.
...
- **Module nf-core** : `'../modules/nf-core/find/concatenate/main'` (référence à `main.nf`)
- **Module local** : `'../modules/local/collectGreetings.nf'` (référence à un fichier unique)

Le module est maintenant disponible pour le workflow, donc tout ce que nous devons faire est de remplacer l'appel à `collectGreetings` pour utiliser `FIND_CONCATENATE`. N'est-ce pas ?
...
!!! info "Info"
    ...
    Le processus FIND_CONCATENATE inclut une gestion assez intelligente de différents types de compression, d'extensions de fichiers, etc. qui ne sont pas strictement pertinents pour ce que nous essayons de vous montrer ici, donc nous ignorerons la plupart de ces éléments et nous concentrerons uniquement sur les parties importantes.
...
En comparaison, l'interface du module `find/concatenate` est plus complexe :
...
Le module FIND_CONCATENATE prend une seule entrée, mais cette entrée est un tuple contenant deux choses :
...
Une fois terminé, FIND_CONCATENATE livre ses sorties en deux parties :

- Un tuple contenant le metamap et le fichier de sortie concaténé, émis avec l'étiquette `file_out` ;
- Trois tuples de version (un par outil utilisé) publiés dans le canal topic `versions` pour le suivi des versions de logiciels.
...
En ignorant la sortie versions pour l'instant, leur sortie principale est également équivalente (un fichier concaténé), sauf que FIND_CONCATENATE émet également le metamap conjointement avec le fichier de sortie.
...
Nous venons de vous dire que le module FIND_CONCATENATE attend une map de métadonnées comme partie de son tuple d'entrée.
...
Maintenant, plaçons cela dans le contexte du processus `FIND_CONCATENATE`, qui attend que les fichiers d'entrée soient empaquetés dans un tuple avec un metamap, et produit également le metamap comme partie du tuple de sortie.
...
Vous vous souvenez que nous vous avons dit que le fichier produit par `FIND_CONCATENATE` sera nommé en fonction d'un identifiant qui fait partie des métadonnées ?
...
!!! info "En savoir plus sur les métadonnées"

    Pour une introduction complète au travail avec les métadonnées dans les workflows Nextflow, y compris comment lire les métadonnées à partir de samplesheets et les utiliser pour personnaliser le traitement, consultez la quête secondaire [Métadonnées dans les workflows](../side_quests/metadata/index.md).

### 2.3. Résumer les changements à effectuer

Sur la base de ce que nous avons examiné, voici les changements majeurs que nous devons apporter à notre pipeline pour utiliser le module `find/concatenate` :
...
- Changer l'appel de `collectGreetings()` à `FIND_CONCATENATE` ;
- Extraire le fichier de sortie du tuple produit par le processus `FIND_CONCATENATE` avant de le passer à `cowpy`.

Cela devrait faire l'affaire ! Maintenant que nous avons un plan, nous sommes prêt·es à nous lancer.
...
## 3. Intégrer FIND_CONCATENATE dans le workflow `hello.nf`
...
Tout d'abord, nous devons créer une map de métadonnées pour `FIND_CONCATENATE`, en gardant à l'esprit que les modules nf-core exigent que le metamap contienne au moins un champ `id`.
...
- `#!groovy .map { files -> tuple(cat_meta, files) }` crée un tuple de `[métadonnées, fichiers]` dans le format attendu par `FIND_CONCATENATE`

C'est tout ce que nous devons faire pour configurer le tuple d'entrée pour `FIND_CONCATENATE`.

### 3.3. Appeler le module FIND_CONCATENATE

Maintenant, appelez `FIND_CONCATENATE` sur le canal nouvellement créé :
...
Cependant, le processus `FIND_CONCATENATE` produit un tuple qui inclut le metamap en plus du fichier de sortie.

Puisque `cowpy` n'accepte pas encore les tuples de métadonnées (nous corrigerons cela dans la prochaine partie du cours), nous devons extraire le fichier de sortie du tuple produit par `FIND_CONCATENATE` avant de le transmettre à `cowpy` :
...
L'opération `#!groovy .map { meta, file -> file }` extrait le fichier du tuple `[métadonnées, fichier]` produit par `FIND_CONCATENATE` dans un nouveau canal, `ch_for_cowpy`.
...
Testons que le workflow fonctionne avec le module `find/concatenate` nouvellement intégré :
...
Remarquez que `FIND_CONCATENATE` apparaît maintenant dans la liste d'exécution des processus au lieu de `collectGreetings`.
