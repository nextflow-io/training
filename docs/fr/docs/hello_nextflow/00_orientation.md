...
:green_book: La transcription vidéo est disponible [ici](./transcripts/00_orientation.md).
///
...
!!! tip
    ...
    - :material-bookmark: Chapitres vidéo dans la timeline qui correspondent aux titres de page.
...
- **Les fichiers `.nf`** sont des scripts de workflow nommés en fonction de la partie du cours dans laquelle ils sont utilisés.
...
- **Le fichier `greetings.csv`** sous `data/` contient les données d'entrée que nous utiliserons dans la majeure partie du cours. Il est décrit dans la Partie 2 (Canaux), lorsque nous l'introduisons pour la première fois.
