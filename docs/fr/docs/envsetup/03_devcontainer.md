...
Si vous disposez d'une installation Docker locale ou êtes prêt·e à en installer une, la façon la plus simple de travailler localement avec ces supports est d'utiliser la fonctionnalité devcontainer de Visual Studio Code. Cette approche fournit tous les outils et dépendances nécessaires sans nécessiter d'installation manuelle.
...
- Lancez VS Code
- Sélectionnez **File -> Open Folder** dans le menu
- Naviguez vers le dossier du dépôt training que vous venez de cloner et sélectionnez-le
- Cliquez sur **Open**
...
![Invite Reopen in Container](img/reopen_prompt.png)

![Sélection de la configuration locale](img/select_local_config.png)

Attendez que le conteneur se construise. Cela peut prendre quelques minutes la première fois car il télécharge et configure tous les composants nécessaires.

Une fois le conteneur construit et en cours d'exécution, vous disposerez d'un environnement entièrement configuré avec tous les outils nécessaires installés, notamment :
...
- **Cohérence** : Garantit un environnement de développement cohérent sur différentes machines
- **Simplicité** : Toutes les dépendances sont pré-installées et configurées
- **Isolation** : L'environnement de développement est isolé de votre système local
- **Reproductibilité** : Toutes les personnes utilisant le devcontainer obtiennent la même configuration
...
5. Pour les problèmes persistants, consultez le [guide de dépannage VS Code Dev Containers](https://code.visualstudio.com/docs/devcontainers/troubleshooting)
