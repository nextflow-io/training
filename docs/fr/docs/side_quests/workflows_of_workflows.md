...
- Un `GREETING_WORKFLOW` qui valide les noms, crée des messages d'accueil et ajoute des horodatages
...
## 1. Créer le Workflow de Messages d'Accueil

Commençons par créer un workflow qui valide les noms et génère des messages d'accueil horodatés.
...
- Les entrées sont déclarées avec le mot-clé `take:`
- Le contenu du workflow est placé dans le bloc `main:`
- Les sorties sont déclarées avec le mot-clé `emit:`

Mettons à jour le workflow de messages d'accueil pour correspondre à cette structure. Modifiez le code comme suit :
...
Le contenu du workflow est également placé dans le bloc `main:`. Notez également que nous avons supprimé la déclaration du canal d'entrée `names_ch`, car il est maintenant passé comme argument au workflow.
...
Mais notre workflow de messages d'accueil n'a pas de workflow sans nom, nous avons plutôt un workflow nommé :
...
Ça fonctionne ! Nous avons enveloppé le workflow de messages d'accueil nommé dans un workflow principal avec un bloc `workflow` d'entrée sans nom. Le workflow principal utilise le workflow `GREETING_WORKFLOW` presque (pas tout à fait) comme un processus, et passe le canal `names` comme argument.
...
Vous avez maintenant un workflow de messages d'accueil fonctionnel qui :

- Prend un canal de noms en entrée
- Valide chaque nom
- Crée un message d'accueil pour chaque nom valide
- Ajoute des horodatages aux messages d'accueil
- Expose à la fois les messages d'accueil originaux et horodatés comme sorties

Cette approche modulaire vous permet de tester le workflow de messages d'accueil indépendamment ou de l'utiliser comme composant dans des pipelines plus grands.

---

## 2. Ajouter le Workflow de Transformation

Créons maintenant un workflow qui applique des transformations de texte aux messages d'accueil.
...
Nous ne répéterons pas l'explication de la syntaxe composable ici, mais notez que le workflow nommé est à nouveau déclaré avec un bloc `take:` et `emit:`, et le contenu du workflow est placé dans le bloc `main:`.
...
Si vous jetez un œil à l'un de ces fichiers inversés, vous verrez qu'il s'agit de la version en majuscules du message d'accueil inversé :
...
- Traite les noms à travers le workflow de messages d'accueil
- Alimente les messages d'accueil horodatés dans le workflow de transformation
- Produit à la fois des versions en majuscules et inversées des messages d'accueil
...
2.  **Imports de workflows :** Nous avons construit deux modules de workflow indépendants et les avons importés dans un pipeline principal avec des instructions include.
    ...
    - Inclure avec alias pour éviter les conflits de noms
