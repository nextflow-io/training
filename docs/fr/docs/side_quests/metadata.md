...
Chaque fichier est accompagné de ses propres informations supplémentaires : ce qu'il est, d'où il provient et ce qui le rend spécial.
...
Les métadonnées suivent des détails importants sur les fichiers et les conditions expérimentales, et aident à adapter les analyses aux caractéristiques uniques de chaque jeu de données.

Pensez-y comme à un catalogue de bibliothèque : alors que les livres contiennent le contenu réel (données brutes), les fiches du catalogue fournissent des informations essentielles sur chaque livre—quand il a été publié, qui l'a écrit, où le trouver (métadonnées).
...
- Regrouper les fichiers associés pour une analyse conjointe
...
Ces compétences vous aideront à construire des pipelines plus robustes et flexibles capables de gérer des relations de fichiers et des exigences de traitement complexes.
...
La feuille de données liste les chemins vers les fichiers de données et certaines métadonnées associées, organisées en 3 colonnes :

- `id` : explicite, un ID donné au fichier
- `character` : un nom de personnage, que nous utiliserons plus tard pour dessiner différentes créatures
- `data` : chemins vers des fichiers `.txt` qui contiennent des salutations dans différentes langues
...
3. **Personnaliser** le traitement pour chaque fichier en fonction de sa langue et de ses métadonnées
4. **Organiser** les sorties par groupe de langues

Cela représente un modèle de workflow typique où les métadonnées spécifiques aux fichiers guident les décisions de traitement ; exactement le type de problème que les maps de métadonnées résolvent élégamment.
...
Voyons ce qui en sort, d'accord ?
...
Par exemple, nous pourrions accéder à l'ID du fichier avec `id` ou au chemin du fichier txt avec `recording`.
...
??? info "(Optionnel) En savoir plus sur les maps"
    ...
    Même s'il n'a pas de bloc `workflow` approprié, Nextflow peut exécuter ceci comme s'il s'agissait d'un workflow :
...
Succès ! Nous avons profité de la structure de map dérivée de notre feuille de données pour accéder aux valeurs de colonnes individuelles pour chaque ligne.

Maintenant que nous avons lu avec succès la feuille de données et que nous avons accès aux données de chaque ligne, nous pouvons commencer à implémenter notre logique de pipeline.

### 1.3. Organiser les métadonnées dans une 'meta map'

Dans l'état actuel du workflow, les fichiers d'entrée (sous la clé `recording`) et les métadonnées associées (`id`, `character`) sont tous au même niveau, comme s'ils étaient tous dans un grand sac.
...
Pour éviter ce problème, nous devons trouver un moyen de garder la structure du canal cohérente indépendamment du nombre de colonnes que contient cette feuille de données.
...
Maintenant, chaque élément du canal contient la meta map en premier et l'objet fichier correspondant en second :
...
- **Comment lire des feuilles de données :** Utiliser `splitCsv` pour lire des fichiers CSV avec des informations d'en-tête et transformer les lignes en données structurées
- **Comment créer une meta map :** Séparer les métadonnées des données de fichier en utilisant la structure de tuple `[ [id:value, ...], file ]`
...
Ce modèle `tuple val(meta), path(file), <output>` garde les métadonnées associées à la fois aux données d'entrée et aux sorties au fur et à mesure qu'elles circulent dans le pipeline.

Notez que nous utilisons ici le qualificateur de sortie [`stdout`](https://www.nextflow.io/docs/latest/process.html#outputs) de Nextflow car l'outil affiche sa sortie directement sur la console plutôt que d'écrire un fichier ; et nous utilisons `sed` dans la ligne de commande pour supprimer le score de probabilité, nettoyer la chaîne en supprimant les caractères de nouvelle ligne, et ne retourner que la prédiction de langue.
...
Excellent ! Nous avons maintenant une prédiction de la langue que parle chaque personnage.
...
Cela s'avérera utile dans la prochaine étape.
...
!!! note
    ...
    Comme vous l'aurez déjà appris, vous ne pouvez pas vous fier à l'ordre des éléments dans les canaux pour faire correspondre les résultats entre eux.
...
D'un point de vue technique, il est _possible_ de le faire, mais ce n'est pas sûr.

Donc à la place, nous allons créer une nouvelle meta map contenant le contenu de la meta map existante plus une nouvelle paire clé-valeur `lang: lang_id` contenant les nouvelles informations, en utilisant l'opérateur `+` (une fonctionnalité de Groovy).
...
Si vous n'êtes pas encore familier·ère avec l'opérateur `+`, ou si cela semble déroutant, prenez quelques minutes pour parcourir l'explication détaillée ci-dessous.
...
??? info "Création de la nouvelle meta map en utilisant l'opérateur `+`"
    ...
    **Premièrement, vous devez savoir que nous pouvons fusionner le contenu de deux maps en utilisant l'opérateur Groovy `+`.**
    ...
    Nous pouvons les fusionner comme ceci :
    ...
    **Maintenant transposons cela dans le contexte d'une opération Nextflow `channel.map()`.**
    ...
    Espérons que vous puissiez voir que si nous changeons `map1` en `meta`, c'est essentiellement tout ce dont nous avons besoin pour ajouter la prédiction de langue à notre meta map dans notre workflow.
...
Une fois que vous êtes confiant·e de comprendre ce que fait ce code, exécutez le workflow pour voir si cela a fonctionné :
...
Oui, ça fonctionne !
Nous avons soigneusement réorganisé la sortie du processus de `meta, file, lang_id` de sorte que `lang_id` soit maintenant l'une des clés dans la meta map, et les tuples du canal correspondent à nouveau au modèle `meta, file`.

<!-- TODO (future) Should we also show how to remove a key using subMap?! Or note where to find that. -->

### 2.4. Attribuer un groupe de langues en utilisant des conditionnelles

Maintenant que nous avons nos prédictions de langue, utilisons l'information pour attribuer de nouveaux regroupements.
...
Plus précisément, nous allons définir une variable appelée `lang_group`, utiliser une logique conditionnelle simple pour déterminer quelle valeur attribuer au `lang_group` pour chaque donnée.
...
Vous devriez finir par effectuer les modifications suivantes au workflow :
...
- Nous utilisons `def lang_group = "unknown"` pour créer la variable `lang_group` avec la valeur par défaut définie sur `unknown`.
...
- **Appliquer les métadonnées d'entrée aux canaux de sortie** : Copier les métadonnées de cette manière nous permet d'associer les résultats plus tard en fonction du contenu des métadonnées.
- **Créer des clés personnalisées** : Vous avez créé deux nouvelles clés dans votre meta map, en les fusionnant avec `meta + [new_key:value]` dans la meta map existante. Une basée sur une valeur calculée à partir d'un processus, et une basée sur une condition que vous avez définie dans l'opérateur `map`.
...
Même si vous n'utilisez pas de métadonnées dans le cadre d'un processus, garder la meta map associée aux données comme ceci facilite le regroupement de toutes les informations pertinentes.
...
Plus précisément, nous allons ajouter une deuxième étape à notre workflow pour dessiner chaque animal en art ASCII et le faire dire le texte enregistré dans une bulle de dialogue.
...
??? info "Que fait `cowpy` ?"
    ...
    C'est une implémentation python de l'outil classique [cowsay](https://en.wikipedia.org/wiki/Cowsay) de Tony Monroe.
...
Comme vous pouvez le voir, ce processus est actuellement conçu pour prendre un fichier d'entrée (contenant le texte à afficher) et une valeur spécifiant le personnage qui devrait être dessiné en art ASCII, généralement fournie au niveau du workflow par un paramètre de ligne de commande.
...
Utilisons cela pour définir le personnage que le processus devrait utiliser pour chaque entrée.
...
2. Déterminer comment accéder à l'information d'intérêt
3. Ajouter un appel au deuxième processus et fournir l'information de manière appropriée.
...
Dans le workflow principal, remplacez l'opérateur `.view()` par `.set { ch_languages }`, et ajoutez une ligne testant que nous pouvons nous référer au canal par son nom.
...
Pour écrire l'appel au processus `COWPY`, nous devons juste savoir comment extraire l'objet fichier correspondant et les métadonnées de chaque élément dans le canal.

Comme c'est souvent le cas, la façon la plus simple de le faire est d'utiliser une opération `map`.

Notre canal contient des tuples structurés comme `[meta, file]`, donc nous pouvons accéder à l'objet `file` directement, et nous pouvons accéder à la valeur `character` stockée dans la meta map en y faisant référence comme `meta.character`.
...
Notez que nous utilisons des closures (telles que `{ file -> "File: " + file }`) pour rendre la sortie des opérations `.view` plus lisible.
...
_Les chemins de fichiers et les valeurs de personnages peuvent apparaître dans un ordre différent dans votre sortie._

Cela confirme que nous sommes capables d'accéder au fichier et au personnage pour chaque élément dans le canal.
...
Cela montre que nous avons pu utiliser l'information dans la meta map pour paramétrer la commande dans la deuxième étape du pipeline.
...
Cette approche fonctionne bien pour utiliser un petit nombre de champs de la meta map, mais ne passerait pas bien à l'échelle si nous voulions en utiliser beaucoup plus.

Il existe un autre opérateur appelé `multiMap()` qui nous permet de rationaliser cela un peu, mais même dans ce cas ce n'est pas idéal.

??? info "(Optionnel) Version alternative avec `multiMap()`"

    Au cas où vous vous poseriez la question, nous ne pouvions pas simplement écrire une seule opération `map()` qui produit à la fois le `file` et le `character`, car cela les retournerait comme un tuple.
...
Dans les deux cas, il est gênant que nous devions faire un peu de déballage au niveau du workflow.

Il serait préférable de pouvoir fournir toute la meta map au processus et de choisir ce dont nous avons besoin une fois là-bas.

### 3.3. Passer et utiliser toute la meta map
...
Effectuez les modifications suivantes au fichier de module `cowpy.nf` :
...
Cela nous permet d'utiliser la structure de tuple `[meta, file]` que nous avons couverte plus tôt dans le tutoriel.

Notez que nous n'avons pas mis à jour la définition de sortie du processus pour produire la meta map, afin de garder le tutoriel rationalisé, mais n'hésitez pas à le faire vous-même comme exercice en suivant le modèle du processus `IDENTIFY_LANGUAGE`.

#### 3.3.2. Mettre à jour la commande pour utiliser le champ de la meta map

Toute la meta map est maintenant disponible à l'intérieur du processus, donc nous pouvons nous référer à l'information qu'elle contient directement depuis l'intérieur du bloc de commande.
...
Maintenant mettons à jour l'appel de processus en conséquence.

#### 3.3.3. Mettre à jour l'appel de processus et l'exécuter

Le processus s'attend maintenant à ce que son entrée utilise la structure de tuple `[meta, file]`, qui est ce que le processus précédent produit, donc nous pouvons simplement passer tout le canal `ch_languages` au processus `COWPY`.

Effectuez les modifications suivantes au workflow principal :
...
Si vous regardez dans le répertoire results, vous devriez voir les mêmes sorties que précédemment, _c'est-à-dire_ des fichiers individuels contenant l'art ASCII de chaque salutation prononcée par le personnage correspondant.
...
Donc cela produit les mêmes résultats qu'auparavant avec un code plus simple.
...
La bonne nouvelle, si vous prévoyez d'utiliser des modules du projet [nf-core](https://nf-co.re/), est que les modules nf-core sont tous configurés pour utiliser la structure de tuple `[meta, file]` comme standard.

### 3.4. Dépannage des entrées requises manquantes

La valeur `character` est requise pour que le processus `COWPY` s'exécute avec succès.
Si nous ne définissons pas de valeur par défaut pour elle dans un fichier de configuration, nous DEVONS fournir une valeur pour elle dans la feuille de données.

**Que se passe-t-il si nous ne le faisons pas ?**
Cela dépend de ce que contient la feuille de données d'entrée et de quelle version du workflow nous exécutons.

#### 3.4.1. La colonne character existe mais est vide

Disons que nous supprimons la valeur de character pour l'une des entrées dans notre feuille de données pour simuler une erreur de collecte de données :
...
Pour l'une ou l'autre version du workflow que nous avons utilisée ci-dessus, la clé `character` sera créée pour toutes les entrées lorsque la feuille de données sera lue, mais pour `sampleA` la valeur sera une chaîne vide.
...
Lorsque Nextflow exécute la ligne de commande `cowpy` pour cet échantillon, `${meta.character}` est rempli avec une chaîne vide dans la ligne de commande `cowpy`, donc l'outil `cowpy` génère une erreur disant qu'aucune valeur n'a été fournie pour l'argument `-c`.

#### 3.4.2. La colonne character n'existe pas dans la feuille de données

Maintenant disons que nous supprimons entièrement la colonne `character` de notre feuille de données :
...
Dans ce cas, la clé `character` ne sera pas créée du tout lorsque la feuille de données sera lue.
...
En ce qui concerne Nextflow, ce workflow s'est exécuté avec succès !
Cependant, aucune des sorties que nous voulons ne sera produite.

##### 3.4.2.2. Valeur accédée au niveau du processus

Si nous utilisons la version de la section 3.3, Nextflow passera toute la meta map au processus `COWPY` et tentera d'exécuter la commande.

Cela causera une erreur, mais une différente par rapport au premier cas.
...
Cela se produit parce que `meta.character` n'existe pas, donc notre tentative d'y accéder retourne `null`. En conséquence, Nextflow insère littéralement `null` dans la ligne de commande, ce qui n'est bien sûr pas reconnu par l'outil `cowpy`.

#### 3.4.3. Solutions

Outre la fourniture d'une valeur par défaut dans le cadre de la configuration du workflow, il y a deux choses que nous pouvons faire pour gérer cela de manière plus robuste :

1. Implémenter la validation des entrées dans votre workflow pour vous assurer que la feuille de données contient toutes les informations requises. Vous pouvez trouver une [introduction à la validation des entrées](../hello_nf-core/05_input_validation.md) dans le cours de formation Hello nf-core. <!-- TODO (future) pending a proper Validation side quest -->

2. Si vous voulez vous assurer que quiconque utilise votre module de processus peut immédiatement identifier les entrées requises, vous pouvez également faire de la propriété de métadonnées requise une entrée explicite.

Voici un exemple de comment cela fonctionnerait.

Premièrement, au niveau du processus, mettez à jour la définition d'entrée comme suit :
...
La meta map est excellente pour garder les structures de canaux propres et prévenir les structures de canaux arbitraires, mais pour les éléments obligatoires qui sont directement référencés dans un processus, les extraire comme entrées explicites crée un code plus robuste et maintenable.

### À retenir

Dans cette section, vous avez appris comment utiliser les métadonnées pour personnaliser l'exécution d'un processus, en y accédant soit au niveau du workflow soit au niveau du processus.

---

## Exercice supplémentaire

Si vous souhaitez pratiquer l'utilisation des informations de la meta map depuis l'intérieur d'un processus, essayez d'utiliser d'autres informations de la meta map telles que `lang` et `lang_group` pour personnaliser la façon dont les sorties sont nommées et/ou organisées.

Par exemple, essayez de modifier le code pour produire ce résultat :

```console title="Contenu du répertoire results"
results/
├── germanic
│   ├── de-guten_tag.txt
│   ├── de-hallo.txt
...
Ce modèle consistant à garder les métadonnées explicites et attachées aux données est une bonne pratique fondamentale dans Nextflow, offrant plusieurs avantages par rapport au codage en dur des informations de fichiers :

- Les métadonnées de fichiers restent associées aux fichiers tout au long du workflow
...
- Les informations de fichiers peuvent être étendues pendant l'exécution du pipeline

Appliquer ce modèle dans votre propre travail vous permettra de construire des workflows bioinformatiques robustes et maintenables.

### Modèles clés

1.  **Lecture et structuration des métadonnées :** Lire des fichiers CSV et créer des maps de métadonnées organisées qui restent associées à vos fichiers de données.
...
2.  **Expansion des métadonnées pendant le workflow** Ajouter de nouvelles informations à vos métadonnées au fur et à mesure que votre pipeline progresse en ajoutant des sorties de processus et en dérivant des valeurs par logique conditionnelle.

    - Ajouter de nouvelles clés basées sur la sortie du processus
    ...
    - Ajouter de nouvelles clés en utilisant une clause conditionnelle
