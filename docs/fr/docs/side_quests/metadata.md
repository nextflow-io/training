...
Notez que nous n'avons pas mis à jour la définition de sortie du processus pour sortir la meta map, afin de garder le tutoriel rationalisé, mais n'hésitez pas à le faire vous-même comme exercice en suivant le modèle du processus `IDENTIFY_LANGUAGE`.

#### 3.3.2. Mettre à jour la commande pour utiliser le champ de la meta map

La meta map entière est maintenant disponible à l'intérieur du processus, nous pouvons donc référer aux informations qu'elle contient directement depuis le bloc de commande.
...
Nous avons remplacé la référence à la valeur `character` précédemment passée comme entrée autonome par la valeur contenue dans la meta map, que nous référons en utilisant `meta.character`.

Maintenant mettons à jour l'appel de processus en conséquence.

#### 3.3.3. Mettre à jour l'appel de processus et l'exécuter

Le processus attend maintenant que son entrée utilise la structure de tuple `[meta, file]`, qui est ce que le processus précédent produit, nous pouvons donc simplement passer le canal `ch_languages` entier au processus `COWPY`.
...
Dans certains cas, vous pourriez devoir vous appuyer sur des processus existants que vous n'êtes pas libre de modifier, ce qui limite vos options.
...
Disons que nous supprimons la valeur character pour l'une des entrées dans notre feuille de données pour simuler une erreur de collecte de données :
...
Si nous utilisons la version de la section 3.3, Nextflow passera la meta map entière au processus `COWPY` et tentera d'exécuter la commande.
...
1. Implémenter une validation d'entrée dans votre workflow pour vous assurer que la feuille de données contient toutes les informations requises. Vous pouvez trouver une [introduction à la validation d'entrée](../hello_nf-core/05_input_validation.md) dans le cours de formation Hello nf-core.
...
D'abord, au niveau du processus, mettez à jour la définition d'entrée comme suit :
...
Si vous souhaitez pratiquer l'utilisation des informations de la meta map depuis l'intérieur d'un processus, essayez d'utiliser d'autres éléments d'information de la meta map tels que `lang` et `lang_group` pour personnaliser la façon dont les sorties sont nommées et/ou organisées.
...
- L'organisation des sorties peut refléter les métadonnées de fichiers
...
2.  **Expansion des métadonnées pendant le workflow** Ajouter de nouvelles informations à vos métadonnées au fur et à mesure que votre pipeline progresse en ajoutant des sorties de processus et en dérivant des valeurs par logique conditionnelle.
    ...
    - Ajouter de nouvelles clés basées sur la sortie de processus
