...
- **Opérations sécurisées :** Gérer les données manquantes avec élégance grâce aux opérateurs null-safe et valider les entrées avec des messages d'erreur clairs
...
Commençons par un workflow simple qui lit simplement le fichier CSV (nous l'avons déjà fait pour vous dans `main.nf`) :
...
`view()` retourne une sortie pour chaque émission de canal, nous savons donc que cette sortie unique contient tous les 3 éléments d'origine regroupés en une liste.
...
Effectuez la modification suivante de votre workflow `main.nf` existant :
...
Une logique de workflow complexe en ligne dans les opérateurs de canal ou les définitions de processus réduit la lisibilité et la maintenabilité. Les **fonctions** vous permettent d'extraire cette logique dans des composants nommés et réutilisables.
...
Précédemment, nous avons utilisé `.map()` avec du scripting pour transformer les données de canal. Maintenant, nous utiliserons la logique conditionnelle pour contrôler quels processus s'exécutent en fonction des données -- essentiel pour des workflows flexibles s'adaptant à différents types d'échantillons.

Les [opérateurs de dataflow](https://www.nextflow.io/docs/latest/reference/operator.html) de Nextflow prennent des closures évaluées à l'exécution, permettant à la logique conditionnelle de piloter les décisions de workflow en fonction du contenu du canal.
...
## 6. Opérateurs de Navigation Sécurisée et Elvis
...
Disons que nous voulons ajouter la prise en charge d'informations optionnelles sur l'exécution de séquençage. Dans certains laboratoires, les échantillons peuvent avoir un champ supplémentaire pour l'ID d'exécution de séquençage ou le numéro de lot, mais notre CSV actuel n'a pas cette colonne. Essayons d'y accéder quand même.
...
Le problème est que `row.run_id` retourne `null` car la colonne `run_id` n'existe pas dans notre CSV. Lorsque nous essayons d'appeler `.toUpperCase()` sur `null`, cela plante. C'est là que l'opérateur de navigation sécurisée sauve la mise.

### 6.2. Opérateur de Navigation Sécurisée (`?.`)

L'opérateur de navigation sécurisée (`?.`) retourne `null` au lieu de lever une exception lorsqu'il est appelé sur une valeur `null`. Si l'objet avant `?.` est `null`, l'expression entière s'évalue à `null` sans exécuter la méthode.

Mettez à jour la fonction pour utiliser la navigation sécurisée :
...
Maintenant que nous utilisons la navigation sécurisée, `run_id` sera `null` pour les échantillons sans ce champ. Utilisons l'opérateur Elvis pour fournir une valeur par défaut et l'ajouter à notre map `sample_meta` :
...
Parfait ! Maintenant, tous les échantillons ont un champ `run` avec soit leur ID d'exécution réel (en majuscules) soit la valeur par défaut 'UNSPECIFIED'. La combinaison de `?.` et `?:` fournit à la fois la sécurité (pas de plantages) et des valeurs par défaut sensées.

Retirez maintenant l'opérateur `.view()` maintenant que nous avons confirmé que cela fonctionne.

!!! tip "Combiner Navigation Sécurisée et Elvis"
...
- **Navigation sécurisée (`?.`)** : Empêche les plantages sur les valeurs null - retourne null au lieu de lever une exception
...
- **Gestion robuste** : Les opérateurs de navigation sécurisée et Elvis rendent les workflows résilients aux données manquantes
...
2.  **Traitement Avancé de Chaînes** : Vous avez maîtrisé les expressions régulières pour analyser les noms de fichiers, la génération dynamique de scripts dans les processus, et l'interpolation de variables (Nextflow vs Bash vs Shell).
...
3.  **Créer des Fonctions Réutilisables** : Vous avez appris à extraire la logique complexe dans des fonctions nommées qui peuvent être appelées depuis les opérateurs de canal, rendant les workflows plus lisibles et maintenables.
...
5.  **Logique Conditionnelle et Contrôle de Processus** : Vous avez ajouté un routage intelligent en utilisant les opérateurs `.branch()` et `.filter()`, en tirant parti de la véracité pour des expressions conditionnelles concises.
    ...
    - Évaluation booléenne avec la Vérité Groovy
...
6.  **Opérateurs de Navigation Sécurisée et Elvis** : Vous avez rendu le pipeline robuste contre les données manquantes en utilisant `?.` pour l'accès sécurisé aux propriétés null et `?:` pour fournir des valeurs par défaut.
