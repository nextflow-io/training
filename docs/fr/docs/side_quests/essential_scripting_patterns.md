# Modèles de script essentiels Nextflow
...
Vous pouvez écrire beaucoup de Nextflow sans vous aventurer au-delà de la syntaxe de base pour les variables, les maps et les listes. La plupart des tutoriels Nextflow se concentrent sur l'orchestration de workflow (channels, processus et flux de données), et vous pouvez aller étonnamment loin avec juste cela.

Cependant, lorsque vous devez manipuler des données, analyser des noms de fichiers complexes, implémenter une logique conditionnelle ou construire des workflows de production robustes, il est utile de penser à deux aspects distincts de votre code : le **flux de données** (channels, opérateurs, processus et workflows) et le **scripting** (le code à l'intérieur des closures, fonctions et scripts de processus). Bien que cette distinction soit quelque peu arbitraire -- c'est tout du code Nextflow -- elle fournit un modèle mental utile pour comprendre quand vous orchestrez votre pipeline par rapport à quand vous manipulez des données. Maîtriser les deux améliore considérablement votre capacité à écrire des workflows clairs et maintenables.
...
Nous transformerons un simple workflow de lecture de CSV en un pipeline bioinformatique sophistiqué, en le faisant évoluer étape par étape à travers des défis réalistes :

- **Comprendre les frontières :** Distinguer entre les opérations de flux de données et le scripting, et comprendre comment ils fonctionnent ensemble
...
- **Logique dynamique :** Construire des processus qui s'adaptent à différents types d'entrée et utiliser des closures pour l'allocation dynamique de ressources
...
- **Opérations sûres :** Gérer les données manquantes avec élégance avec des opérateurs null-safe et valider les entrées avec des messages d'erreur clairs
- **Gestionnaires basés sur la configuration :** Utiliser des gestionnaires d'événements de workflow pour la journalisation, les notifications et la gestion du cycle de vie
...
- Être à l'aise avec l'utilisation des concepts et mécanismes de base de Nextflow (processus, channels, opérateurs, travail avec des fichiers, métadonnées)
...
Nous commencerons par des concepts fondamentaux et construirons jusqu'à des modèles avancés.
...
Vous trouverez un fichier de workflow principal et un répertoire `data` contenant des fichiers de données d'exemple.
...
Notre CSV d'exemple contient des informations sur des échantillons biologiques qui nécessitent un traitement différent en fonction de leurs caractéristiques :
...
## 1. Flux de données vs Scripting : Comprendre les frontières

### 1.1. Identifier ce qui est quoi

Lors de l'écriture de workflows Nextflow, il est important de distinguer entre le **flux de données** (comment les données se déplacent à travers les channels et les processus) et le **scripting** (le code qui manipule les données et prend des décisions). Construisons un workflow démontrant comment ils fonctionnent ensemble.
...
Le bloc `workflow` définit notre structure de pipeline, tandis que `channel.fromPath()` crée un channel à partir d'un chemin de fichier. L'opérateur `.splitCsv()` traite le fichier CSV et convertit chaque ligne en une structure de données map.
...
Vous verrez la même sortie qu'avant, car nous retournons simplement l'entrée inchangée. Cela confirme que l'opérateur map fonctionne correctement. Maintenant, commençons à transformer les données.
...
La map `sample_meta` est une structure de données clé-valeur (comme les dictionnaires en Python, les objets en JavaScript ou les hashes en Ruby) stockant des informations connexes : ID d'échantillon, organisme, type de tissu, profondeur de séquençage et score de qualité.
...
Maintenant, ajoutons plus de scripting - cette fois en utilisant un opérateur ternaire pour prendre des décisions basées sur les valeurs de données.
...
#### 1.1.6. Combinaison de Maps et retour de résultats
...
Sortons une structure de channel comprenant un tuple de 2 éléments : la map de métadonnées enrichie et le chemin de fichier correspondant. C'est un modèle courant dans Nextflow pour passer des données aux processus.
...
Notre workflow démontre le modèle de base : les **opérations de flux de données** (`workflow`, `channel.fromPath()`, `.splitCsv()`, `.map()`, `.view()`) orchestrent comment les données se déplacent à travers le pipeline, tandis que le **scripting** (maps `[key: value]`, méthodes de chaînes, conversions de types, opérateurs ternaires) à l'intérieur de la closure `.map()` gère la transformation des éléments de données individuels.

### 1.2. Comprendre les différents types : Channel vs List

Jusqu'ici, tout va bien, nous pouvons distinguer entre les opérations de flux de données et le scripting. Mais qu'en est-il lorsque le même nom de méthode existe dans les deux contextes ?

Un exemple parfait est la méthode `collect`, qui existe à la fois pour les types de channel et les types List dans la bibliothèque standard Nextflow. La méthode `collect()` sur une List transforme chaque élément, tandis que l'opérateur `collect()` sur un channel rassemble toutes les émissions de channel en un channel à élément unique.

Démontrons cela avec quelques données d'exemple, en commençant par nous rafraîchir la mémoire sur ce que fait l'opérateur `collect()` de channel. Consultez `collect.nf` :

```groovy title="collect.nf" linenums="1"
def sample_ids = ['sample_001', 'sample_002', 'sample_003']

// channel.collect() - regroupe plusieurs émissions de channel en une
ch_input = channel.fromList(sample_ids)
ch_input.view { sample -> "Élément de channel individuel : ${sample}" }
...
- Rassembler tous les éléments dans une seule liste avec l'opérateur `collect()` du channel
- Imprimer le résultat collecté (élément unique contenant tous les IDs d'échantillons) avec un second `view()`

Nous avons changé la structure du channel, mais nous n'avons pas changé les données elles-mêmes.
...
??? success "Sortie de la commande"
    ...
    Élément de channel individuel : sample_001
    Élément de channel individuel : sample_002
    Élément de channel individuel : sample_003
    Résultat de channel.collect() : [sample_001, sample_002, sample_003] (3 éléments regroupés en 1)
    ```

`view()` retourne une sortie pour chaque émission de channel, nous savons donc que cette sortie unique contient tous les 3 éléments d'origine regroupés en une liste.
...
```bash title="Test de l'opérateur spread"
nextflow run collect.nf
```
...
// Fonctionne également avec les appels de méthode :
...
- **Flux de données vs scripting** : Les opérateurs de channel orchestrent comment les données circulent à travers votre pipeline, tandis que le scripting transforme les éléments de données individuels
...
- **Le contexte compte** : Soyez toujours conscient·e de si vous travaillez avec des channels (flux de données) ou des structures de données (scripting)
...
Maîtriser le traitement de chaînes sépare les workflows fragiles des pipelines robustes. Cette section couvre l'analyse de noms de fichiers complexes, la génération dynamique de scripts et l'interpolation de variables.
...
```bash title="Test de correspondance de motifs"
nextflow run main.nf
```
...
Le processus prend des fichiers FASTQ en entrée et exécute l'outil `fastp` pour découper les adaptateurs et filtrer les lectures de faible qualité. Malheureusement, la personne qui a écrit ce processus n'a pas prévu les lectures single-end que nous avons dans notre ensemble de données d'exemple. Ajoutons-le à notre workflow et voyons ce qui se passe :
...
Ensuite, modifiez le bloc `workflow` pour connecter le channel `ch_samples` au processus `FASTP` :
...
```console title="Vérification des commandes exécutées"
cat work/31/a8ad4d95749e685a6d842d3007957f/.command.sh | grep "docker run"
```
...
Maintenant ça marche ! La barre oblique inverse (`\`) dit à Nextflow "n'interprète pas ceci, passe-le à Bash."

### À retenir

Dans cette section, vous avez appris des techniques de **traitement de chaînes** :

- **Expressions régulières pour l'analyse de fichiers** : Utilisation de l'opérateur `=~` et des motifs regex (`~/pattern/`) pour extraire des métadonnées à partir de conventions de nommage de fichiers complexes
- **Génération dynamique de scripts** : Utilisation de logique conditionnelle (if/else, opérateurs ternaires) pour générer différentes chaînes de script en fonction des caractéristiques d'entrée
...
La logique de workflow complexe intégrée dans les opérateurs de channels ou les définitions de processus réduit la lisibilité et la maintenabilité. Les **fonctions** vous permettent d'extraire cette logique dans des composants nommés et réutilisables.
...
Ensuite, nous explorerons comment utiliser les closures dans les directives de processus pour une allocation dynamique de ressources.

---

## 4. Directives de ressources dynamiques avec closures

Jusqu'à présent, nous avons utilisé du scripting dans le bloc `script` des processus. Mais les **closures** (introduites dans la section 1.1) sont également incroyablement utiles dans les directives de processus, en particulier pour l'allocation dynamique de ressources. Ajoutons des directives de ressources à notre processus FASTP qui s'adaptent en fonction des caractéristiques de l'échantillon.
