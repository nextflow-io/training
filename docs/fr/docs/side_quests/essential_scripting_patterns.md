...
Maintenant ça fonctionne ! La barre oblique inverse (`\`) indique à Nextflow "n'interprète pas ceci, passe-le à Bash."
...
Une logique de workflow complexe en ligne dans les opérateurs de channel ou les définitions de process réduit la lisibilité et la maintenabilité. Les **fonctions** vous permettent d'extraire cette logique dans des composants nommés et réutilisables.
...
La sortie devrait montrer les deux processes se terminant avec succès. Le workflow est maintenant beaucoup plus propre et plus facile à maintenir, avec toute la logique complexe de traitement des métadonnées encapsulée dans la fonction `separateMetadata`.
...
Ensuite, nous explorerons comment utiliser des closures dans les directives de process pour l'allocation dynamique de ressources.

---

## 4. Directives de Ressources Dynamiques avec Closures

Jusqu'à présent, nous avons utilisé le scripting dans le bloc `script` des processes. Mais les **closures** (introduites dans la Section 1.1) sont également incroyablement utiles dans les directives de process, en particulier pour l'allocation dynamique de ressources. Ajoutons des directives de ressources à notre process FASTP qui s'adaptent en fonction des caractéristiques de l'échantillon.

### 4.1. Allocation de ressources spécifique à l'échantillon

Actuellement, notre process FASTP utilise des ressources par défaut. Rendons-le plus intelligent en allouant plus de CPUs pour les échantillons à haute profondeur. Modifiez `modules/fastp.nf` pour inclure une directive `cpus` dynamique et une directive `memory` statique :
...
!!! note "Accéder aux Variables d'Entrée dans les Directives"
...
Exécutez à nouveau le workflow avec l'option `-ansi-log false` pour faciliter la visualisation des hashs de tâche.
...
Un autre modèle puissant consiste à utiliser `task.attempt` pour les stratégies de nouvelle tentative. Pour montrer pourquoi c'est utile, nous allons commencer par réduire l'allocation de mémoire à FASTP à moins qu'il n'en ait besoin. Changez la directive `memory` dans `modules/fastp.nf` à `1.GB` :
...
Cela indique que le process a été tué pour avoir dépassé les limites de mémoire.
...
Maintenant, si le process échoue en raison d'une mémoire insuffisante, Nextflow réessaiera avec plus de mémoire :
...
## 5. Logique Conditionnelle et Contrôle de Process

Précédemment, nous avons utilisé `.map()` avec du scripting pour transformer les données de channel. Maintenant, nous allons utiliser une logique conditionnelle pour contrôler quels processes s'exécutent en fonction des données—essentiel pour des workflows flexibles s'adaptant à différents types d'échantillons.

Les [opérateurs de dataflow](https://www.nextflow.io/docs/latest/reference/operator.html) de Nextflow prennent des closures évaluées au moment de l'exécution, permettant à la logique conditionnelle de piloter les décisions de workflow en fonction du contenu du channel.

### 5.1. Routage avec `.branch()`

Par exemple, supposons que nos échantillons de séquençage doivent être rognés avec FASTP uniquement s'il s'agit d'échantillons humains avec une couverture au-dessus d'un certain seuil. Les échantillons de souris ou les échantillons à faible couverture devraient être exécutés avec Trimgalore à la place (c'est un exemple artificiel, mais il illustre le point).

Nous avons fourni un process Trimgalore simple dans `modules/trimgalore.nf`, jetez-y un œil si vous le souhaitez, mais les détails ne sont pas importants pour cet exercice. Le point clé est que nous voulons router les échantillons en fonction de leurs métadonnées.
...
... puis modifiez votre workflow `main.nf` pour brancher les échantillons en fonction de leurs métadonnées et les router à travers le process de rognage approprié, comme ceci :
...
### 5.2. Utiliser `.filter()` avec la Véracité
...
- **Vrai** : Valeurs non nulles, chaînes non vides, nombres non nuls, collections non vides
...
- `meta.depth >= 25000000` garantit une profondeur de séquençage suffisante avec une comparaison explicite
...
Notre pipeline route maintenant intelligemment les échantillons à travers les processes appropriés, mais les workflows de production doivent gérer les données invalides avec élégance. Rendons notre workflow robuste contre les valeurs manquantes ou nulles.
...
Supposons que nous voulions ajouter la prise en charge d'informations de séquençage optionnelles. Dans certains laboratoires, les échantillons peuvent avoir un champ supplémentaire pour l'ID de séquençage ou le numéro de lot, mais notre CSV actuel n'a pas cette colonne. Essayons d'y accéder quand même.
...
Pas de plantage ! Le workflow gère maintenant le champ manquant avec élégance. Lorsque `row.run_id` est `null`, l'opérateur `?.` empêche l'appel `.toUpperCase()`, et `run_id` devient `null` au lieu de provoquer une exception.

### 6.3. Opérateur Elvis (`?:`) pour les Valeurs par Défaut

L'opérateur Elvis (`?:`) fournit des valeurs par défaut lorsque le côté gauche est "faux" (comme expliqué précédemment). Il est nommé d'après Elvis Presley car `?:` ressemble à ses célèbres cheveux et yeux vus de côté !
...
Utilisez ces opérateurs de manière cohérente dans les fonctions, les closures d'opérateurs (`.map{}`, `.filter{}`), les scripts de process et les fichiers de configuration. Ils empêchent les plantages lors de la gestion de données du monde réel.

### Récapitulatif

- **Navigation sûre (`?.`)** : Empêche les plantages sur les valeurs nulles - retourne null au lieu de lever une exception
...
Jusqu'à présent, nous avons écrit du code dans nos scripts de workflow et nos définitions de process. Mais il y a une autre fonctionnalité importante que vous devriez connaître : les gestionnaires d'événements de workflow.
...
Vous pouvez également écrire le résumé dans un fichier en utilisant des opérations sur fichiers :
...
Tout au long de cette quête secondaire, vous avez construit un pipeline complet de traitement d'échantillons qui a évolué de la gestion de base des métadonnées à un workflow sophistiqué et prêt pour la production.
...
1.  **Dataflow vs Scripting :** Vous avez appris à distinguer entre les opérations de dataflow (orchestration de channel) et le scripting (code qui manipule les données), y compris les différences cruciales entre les opérations sur différents types comme `collect` sur Channel vs List.

    - Dataflow : orchestration de channel
...
2.  **Traitement Avancé de Chaînes** : Vous avez maîtrisé les expressions régulières pour analyser les noms de fichiers, la génération dynamique de scripts dans les processes et l'interpolation de variables (Nextflow vs Bash vs Shell).
    ...
    - Collection de fichiers vers arguments de commande (dans le bloc script de process)
...
3.  **Créer des Fonctions Réutilisables** : Vous avez appris à extraire une logique complexe dans des fonctions nommées qui peuvent être appelées depuis les opérateurs de channel, rendant les workflows plus lisibles et maintenables.
...
4.  **Directives de Ressources Dynamiques avec Closures** : Vous avez exploré l'utilisation de closures dans les directives de process pour une allocation adaptative de ressources basée sur les caractéristiques d'entrée.
...
5.  **Logique Conditionnelle et Contrôle de Process** : Vous avez ajouté un routage intelligent en utilisant les opérateurs `.branch()` et `.filter()`, en tirant parti de la véracité pour des expressions conditionnelles concises.
