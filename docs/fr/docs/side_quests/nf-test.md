...
Être capable de tester systématiquement que chaque partie de votre workflow fait ce qu'elle est censée faire est essentiel pour la reproductibilité et la maintenance à long terme, et peut être d'une aide précieuse pendant le processus de développement.

Prenons un moment pour parler de l'importance des tests. Si vous développez un workflow, l'une des premières choses que vous ferez sera de récupérer des données de test dont vous savez qu'elles sont valides et devraient produire un résultat. Vous ajoutez le premier processus au pipeline et le connectez à vos entrées pour le faire fonctionner. Ensuite, pour vérifier que tout fonctionne, vous l'exécutez sur les données de test. En supposant que cela fonctionne, vous passez au processus suivant et exécutez à nouveau les données de test. Vous répétez ce processus jusqu'à obtenir un pipeline qui vous satisfait.

Ensuite, vous ajoutez peut-être un simple paramètre vrai ou faux tel que `--skip_process`. Maintenant, vous devez exécuter le pipeline deux fois, une fois avec chaque paramètre pour vous assurer qu'il fonctionne comme prévu. Mais attendez, comment vérifier si `--skip_process` saute réellement le processus ? Nous devons fouiller dans les sorties ou vérifier les fichiers de log ! C'est pénible et sujet aux erreurs.

Au fur et à mesure que vous développez votre pipeline, il deviendra rapidement si complexe que tester manuellement chaque itération sera lent et sujet aux erreurs. De plus, si vous trouvez une erreur, il sera très difficile de déterminer exactement d'où vient l'erreur dans votre pipeline. C'est là que les tests interviennent.

Les tests vous permettent de vérifier systématiquement que chaque partie de votre pipeline fonctionne comme prévu. Les avantages pour un·e développeur·se de tests bien écrits sont énormes :

- **Confiance** : Parce que les tests couvrent l'ensemble du pipeline, vous pouvez être confiant·e que modifier quelque chose n'affecte rien d'autre
- **Fiabilité** : Lorsque plusieurs développeur·ses travaillent sur le pipeline, ils·elles savent que les autres développeur·ses n'ont pas cassé le pipeline et chaque composant.
...
5. **Tests de stress** : Évaluation des performances du pipeline dans des conditions extrêmes pour déterminer ses limites
...
- Comprendre quand utiliser des snapshots vs. des assertions de contenu
...
??? example "Que fait le workflow Hello Nextflow ?"
    ...
    Si vous n'avez pas suivi la formation [Hello Nextflow](../hello_nextflow/index.md), voici un aperçu rapide de ce que fait ce simple workflow.

    Le workflow prend un fichier CSV contenant des messages d'accueil, exécute quatre étapes de transformation consécutives sur eux, et produit un seul fichier texte contenant une image ASCII d'un personnage amusant disant les messages d'accueil.
    ...
    3. **`collectGreetings` :** Collecte tous les messages d'accueil en majuscules dans un seul fichier batch
    4. **`cowpy` :** Génère de l'art ASCII en utilisant l'outil `cowpy`

    Les résultats sont publiés dans un répertoire appelé `results/`, et la sortie finale du pipeline (lorsqu'il est exécuté avec les paramètres par défaut) est un fichier texte brut contenant de l'art ASCII d'un personnage disant les messages d'accueil en majuscules.
...
??? example "Code du workflow"
    ...
    */
    ...
    /*
    * Utilise un utilitaire de remplacement de texte pour convertir le message d'accueil en majuscules
    */
...
"Attendez, quoi ? Je viens juste d'exécuter le workflow et ça a marché ! En quoi est-ce un test ?"
...
Cela crée également un répertoire `tests` contenant un fichier de configuration de base.

### 1.1. Générer un nf-test

`nf-test` est fourni avec un ensemble d'outils pour construire des fichiers nf-test, nous épargnant la majorité du travail. Ceux-ci se trouvent sous la sous-commande `generate`. Générons un test pour le pipeline :
...
Le bloc `nextflow_pipeline` est le point d'entrée pour tous les tests au niveau pipeline. Il contient les éléments suivants :
...
En français simple, la logique du test se lit comme suit :
"**Lorsque** ces _paramètres_ sont fournis à ce _pipeline_, **alors** nous nous attendons à voir ces résultats."

Ce n'est pas un test fonctionnel, nous allons démontrer comment le transformer en un dans la section suivante.
...
Au fur et à mesure que nous ajouterons plus d'assertions et de cas de test spécifiques plus tard, nous utiliserons ces noms plus descriptifs pour clarifier ce que chaque test vérifie.
...
Exécutons à nouveau le test pour voir si cela fonctionne.
...
Par défaut, la sortie Nextflow est masquée, mais pour vous convaincre que nf-test exécute définitivement le workflow, vous pouvez utiliser le flag `--verbose` :
...
Une vérification simple consiste à s'assurer que notre pipeline exécute tous les processus attendus et n'en saute aucun silencieusement. Rappelez-vous que notre pipeline exécute 6 processus, un appelé `sayHello` et un appelé `convertToUpper` pour chacun des 3 messages d'accueil.
...
Exécutez à nouveau le test pour voir si cela fonctionne.
...
Ce n'est que la surface, nous pouvons continuer à écrire des assertions pour vérifier les détails du pipeline, mais pour l'instant passons au test des éléments internes du pipeline.
...
Exécutons le test pour voir si cela fonctionne.
...
Cela représente les sorties créées par le processus `sayHello`, que nous testons explicitement. Si nous ré-exécutons le test, le programme vérifiera que la nouvelle sortie correspond à la sortie qui a été enregistrée à l'origine. C'est un moyen rapide et simple de tester que les sorties du processus ne changent pas, c'est pourquoi nf-test le fournit par défaut.

!!!warning

    Cela signifie que nous devons être sûrs que la sortie que nous enregistrons lors de l'exécution originale est correcte !
...
- S'il s'avère que quelque chose dans le code s'est cassé, nous devrons le corriger, en nous attendant à ce que le code corrigé réussisse le test.
- S'il s'agit d'un changement attendu (par exemple, l'outil a été amélioré et les résultats sont meilleurs) alors nous devrons mettre à jour le snapshot pour accepter la nouvelle sortie comme référence à faire correspondre. nf-test a un paramètre `--update-snapshot` à cet effet.

Nous pouvons exécuter à nouveau le test et voir que le test devrait réussir :
...
Succès ! Le test réussit parce que le processus `sayHello` s'est exécuté avec succès et la sortie correspond au snapshot.

### 2.3. Alternative aux snapshots : Assertions de contenu directes

Bien que les snapshots soient excellents pour détecter tout changement dans la sortie, parfois vous voulez vérifier un contenu spécifique sans être aussi strict sur la correspondance de l'ensemble du fichier. Par exemple :

- Lorsque des parties de la sortie peuvent changer (horodatages, ID aléatoires, etc.) mais que certains contenus clés doivent être présents
- Lorsque vous voulez vérifier des motifs ou des valeurs spécifiques dans la sortie
...
Notez que nf-test voit les sorties du processus comme une liste de listes, donc `process.out[0][0]` récupère la première partie du premier élément de canal (ou 'émission') de ce processus.

Cette approche :

- Clarifie exactement ce que nous attendons dans la sortie
...
C'est un test similaire au processus `sayHello`, mais il teste le processus `convertToUpper`. Nous savons que celui-ci échouera parce que tout comme avec `sayHello`, le processus `convertToUpper` prend une seule entrée de chemin, mais nous n'en avons pas spécifié.
...
Notez que nous avons créé un fichier snapshot pour le processus `convertToUpper` à `tests/main.converttoupper.nf.test.snap`. Si nous exécutons à nouveau le test, nous devrions voir que nf-test réussit à nouveau.
...
Exécuter nf-test sur chaque composant est bien, mais laborieux et sujet aux erreurs. Ne pouvons-nous pas simplement tout tester en une seule fois ?
...
Vous savez également comment exécuter des tests soit un par un, soit pour un projet entier.
...
3. Deux approches de validation de sortie :
   ...
   - Utilisation d'assertions de contenu directes pour des vérifications de contenu spécifiques
...
**Rappelez-vous :** Les tests sont une documentation vivante de la façon dont votre code devrait se comporter. Plus vous écrivez de tests, et plus vos assertions sont spécifiques, plus vous pouvez être confiant·e dans la fiabilité de votre pipeline.
