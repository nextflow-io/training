...
Im fünften Teil des Hello nf-core Trainingskurses zeigen wir dir, wie du das nf-schema Plugin verwendest, um Pipeline-Eingaben und Parameter zu validieren.

??? info "Wie du von diesem Abschnitt aus beginnst"

    Dieser Abschnitt setzt voraus, dass du [Teil 4: Ein nf-core Modul erstellen](./04_make_module.md) abgeschlossen und das `COWPY` Prozessmodul auf nf-core Standards aktualisiert hast.
    ...
    Damit erhältst du eine Pipeline mit dem bereits auf nf-core Standards aktualisierten `COWPY` Modul.
...
Stell dir vor, du lässt deine Pipeline zwei Stunden laufen, nur damit sie abstürzt, weil ein\*e Nutzer\*in eine Datei mit der falschen Endung angegeben hat. Oder du verbringst Stunden mit dem Debuggen kryptischer Fehlermeldungen, nur um herauszufinden, dass ein Parameter falsch geschrieben war. Ohne Eingabevalidierung sind solche Szenarien alltäglich.
...
Die Pipeline hat ungültige Eingaben akzeptiert und ist stundenlang gelaufen, bevor sie fehlschlug. Mit ordentlicher Validierung:
...
Das [nf-schema Plugin](https://nextflow-io.github.io/nf-schema/latest/) ist ein Nextflow Plugin, das umfassende Validierungsfunktionen für Nextflow Pipelines bereitstellt.
...
nf-schema bietet mehrere Kernfunktionen:
...
- **Kanalkonvertierung**: Konvertiert validierte Sample Sheets in Nextflow Kanäle
...
nf-schema ist der Nachfolger des veralteten nf-validation Plugins und verwendet den Standard [JSON Schema Draft 2020-12](https://json-schema.org/) für die Validierung.
...
??? info "Was sind Nextflow Plugins?"
    ...
    Nach der Installation kannst du Funktionen aus Plugins mit der Syntax `include { functionName } from 'plugin/plugin-name'` importieren.

### 0.3. Zwei Schema-Dateien für zwei Validierungstypen

Eine nf-core Pipeline nutzt zwei separate Schema-Dateien, die zwei Validierungstypen entsprechen:
...
Beide Schemas verwenden das JSON Schema Format, einen weit verbreiteten Standard zur Beschreibung und Validierung von Datenstrukturen.

**Parametervalidierung** validiert Kommandozeilen-Parameter (Flags wie `--outdir`, `--batch`, `--input`):
...
!!! warning "Was Eingabedatenvalidierung NICHT tut"
    ...
    Eingabedatenvalidierung prüft die Struktur von *Manifest-Dateien* (Sample Sheets, CSV Dateien), NICHT die Inhalte deiner eigentlichen Datendateien (FASTQ, BAM, VCF, etc.).

    Bei großen Datenmengen sollte die Validierung von Dateiinhalten (wie die Prüfung der BAM-Integrität) in Pipeline-Prozessen auf Worker-Knoten erfolgen, nicht während der Validierungsphase auf der orchestrierenden Maschine.

### 0.4. Wann sollte Validierung erfolgen?
...
Validierung sollte **vor** der Ausführung jeglicher Pipeline-Prozesse erfolgen, um schnelles Feedback zu geben und verschwendete Rechenzeit zu vermeiden.

Lass uns diese Prinzipien nun in der Praxis anwenden, beginnend mit der Parametervalidierung.

---

## 1. Parametervalidierung (nextflow_schema.json)

Beginnen wir damit, Parametervalidierung zu unserer Pipeline hinzuzufügen. Diese validiert Kommandozeilen-Flags wie `--input`, `--outdir` und `--batch`.

### 1.1. Validierung konfigurieren, um Eingabedatei-Validierung zu überspringen
...
Da wir zunächst an der Parametervalidierung arbeiten (dieser Abschnitt) und das Eingabedaten-Schema erst in Abschnitt 2 konfigurieren, müssen wir nf-schema vorübergehend anweisen, die Validierung der Dateiinhalte des `input` Parameters zu überspringen.
...
- **`defaultIgnoreParams`**: Validierung komplexer Parameter wie `genomes` überspringen (von Template-Entwickler\*innen festgelegt)
- **`ignoreParams`**: Validierung der Dateiinhalte des `input` Parameters überspringen (vorübergehend; wir aktivieren dies in Abschnitt 2 wieder)
...
!!! note "Warum den input Parameter ignorieren?"
    ...
    Wir entfernen diese Einstellung in Abschnitt 2, nachdem wir das Eingabedaten-Schema konfiguriert haben.
...
Jede hier beschriebene Eingabe hat folgende Schlüsseleigenschaften, die validiert werden können:
...
2. Verwende den Zieh-Griff (⋮⋮), um den neuen Parameter in die "Input/output options" Gruppe zu verschieben, unterhalb des `input` Parameters
...
![Den batch Parameter hinzufügen](./img/schema_add.png)
...
Versuche zunächst, ohne den erforderlichen `input` Parameter auszuführen:
...
Perfekt! Die Validierung erkennt den fehlenden erforderlichen Parameter, bevor die Pipeline läuft.

Versuche es nun mit einem gültigen Parametersatz:
...
Die Web-Oberfläche handhabt die gesamte JSON Schema Syntax für dich und macht es einfach, komplexe Parameter-Schemas ohne fehleranfällige manuelle JSON-Bearbeitung zu verwalten.

### Wie geht es weiter?

Jetzt, da die Parametervalidierung funktioniert, fügen wir Validierung für die Eingabedatei-Inhalte hinzu.
...
Während Parametervalidierung Kommandozeilen-Flags prüft, stellt Eingabedatenvalidierung sicher, dass die Daten innerhalb der CSV-Datei korrekt strukturiert sind.
...
- Drei Spalten (ohne Kopfzeile)
- In jeder Zeile: eine Begrüßung, eine Sprache und ein Score
- Die ersten beiden Spalten sind Textstrings ohne spezielle Formatanforderungen
...
Für unseren Anwendungsfall möchten wir:
...
4. Sicherstellen, dass das Sprachfeld einem der unterstützten Sprachcodes entspricht (en, fr, es, it, de)
5. Sicherstellen, dass das Score-Feld eine Ganzzahl mit einem Wert zwischen 0 und 100 ist
...
Das nf-core Pipeline Template enthält ein Standard-`assets/schema_input.json`, das für Paired-End Sequenzierungsdaten konzipiert ist.
...
- **`properties`**: `sample`, `fastq_1` und `fastq_2` durch `greeting`, `language` und `score` ersetzt
  - **`type:`** Erzwingt entweder string (`greeting`, `language`) oder integer (`score`)
  - **`pattern: "^\\S.*$"`**: Begrüßung muss mit einem Nicht-Leerzeichen beginnen (kann danach aber Leerzeichen enthalten)
  - **`"enum": ["en", "fr", "es", "it", "de"]`**: Sprachcode muss in der unterstützten Menge sein
  - **`"minimum": 0` und `"maximum": 100`**: Score-Wert muss zwischen 0 und 100 liegen
...
Wenn nf-schema eine CSV-Datei liest, erwartet es, dass die erste Zeile Spaltenüberschriften enthält, die den Feldnamen im Schema entsprechen.

Für unseren einfachen Fall müssen wir eine Kopfzeile zu unserer Begrüßungsdatei hinzufügen:
...
Nun hat die CSV-Datei eine Kopfzeile, die den Feldnamen in unserem Schema entspricht.

Der letzte Schritt ist die Implementierung der Validierung im Pipeline-Code mit `samplesheetToList`.

### 2.5. Validierung in der Pipeline implementieren

Jetzt müssen wir unser einfaches CSV-Parsing durch die `samplesheetToList` Funktion von nf-schema ersetzen, die das Samplesheet validiert und parst.
...
3. Gibt eine Groovy-Liste zurück, wobei jeder Eintrag einer Zeile entspricht
4. Wirft hilfreiche Fehlermeldungen, wenn die Validierung fehlschlägt

Lass uns den Code für die Eingabeverarbeitung aktualisieren:

Öffne `subworkflows/local/utils_nfcore_hello_pipeline/main.nf` und finde den Abschnitt, wo wir den Eingabekanal erstellen (um Zeile 80).
...
Aktualisiere nun den Code zur Kanalerstellung:
...
2. **`Channel.fromList(...)`**: Konvertiert die Liste in einen Nextflow Kanal

Dies vervollständigt die Implementierung der Eingabedatenvalidierung mit `samplesheetToList` und JSON Schemas.

Jetzt, da wir das Eingabedaten-Schema konfiguriert haben, können wir die vorübergehende Ignore-Einstellung entfernen, die wir früher hinzugefügt haben.
...
Nun validiert nf-schema sowohl Parametertypen ALS AUCH die Eingabedatei-Inhalte.
...
Bestätige zunächst, dass die Pipeline mit gültiger Eingabe erfolgreich läuft.
...
Großartig! Die Pipeline läuft erfolgreich und die Validierung verläuft stillschweigend.
Die Warnung zu `--character` ist nur informativ, da er nicht im Schema definiert ist.
Wenn du möchtest, nutze das Gelernte, um auch für diesen Parameter Validierung hinzuzufügen!

#### 2.7.2. Test mit ungültiger Eingabe

Bestandene Validierung ist immer ein gutes Gefühl, aber lass uns sicherstellen, dass die Validierung tatsächlich Fehler erkennt.
...
Dies entspricht nicht unserem Schema, daher sollte die Validierung einen Fehler werfen.
...
Perfekt! Die Validierung hat den Fehler erkannt und eine klare, hilfreiche Fehlermeldung bereitgestellt, die auf Folgendes hinweist:

- Welche Datei die Validierung nicht bestanden hat
...
Wenn du möchtest, kannst du gerne weitere Begrüßungs-Eingabedateien erstellen, die das Schema auf andere lustige Weisen verletzen.

### Fazit

Du hast sowohl Parametervalidierung als auch Eingabedatenvalidierung implementiert und getestet. Deine Pipeline validiert nun Eingaben vor der Ausführung und liefert schnelles Feedback mit klaren Fehlermeldungen.

!!! tip "Weiterführende Literatur"

    Um mehr über erweiterte Validierungsfunktionen und -muster zu erfahren, schau dir die [nf-schema Dokumentation](https://nextflow-io.github.io/nf-schema/latest/) an. Der `nf-core pipelines schema build` Befehl bietet eine interaktive GUI zur Verwaltung komplexer Schemas.
