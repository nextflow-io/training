...
Wir werden diese Informationen nehmen und in drei Stufen in Nextflow einbinden:
...
Füge einen `profiles`-Block zu `nextflow.config` mit einem `test`-Profil hinzu, das den `reads_bam`-Parameter auf eine der Test-BAM-Dateien setzt.
...
Der Block `output {}` befindet sich außerhalb des Workflows und gibt an, wohin jedes benannte Ziel veröffentlicht wird.
...
??? abstract "Ergebnisverzeichnis-Inhalt"
...
Mehr dazu erfährst du in der Side Quest [Working with files](../../side_quests/working_with_files.md).
...
Du weißt, wie man einen zweistufigen modularen Workflow erstellt, der echte Analysearbeit leistet und mit Eigenheiten genomischer Dateiformate wie den Hilfsdateien umgehen kann.

### Wie geht es weiter?

Passe den Workflow an, um mehrere Proben in großen Mengen zu verarbeiten.
...
Musst du ein Bash-Skript schreiben, das durch alle deine Proben iteriert?

Nein, zum Glück nicht! Nimm einfach eine kleine Anpassung am Code vor und Nextflow wird das auch für dich erledigen.
...
Dies ist eine schnelle Möglichkeit, die Ausführung mit mehreren Proben zu testen; im nächsten Schritt wechseln wir zu einem skalierbareren Ansatz mit einer Datei von Eingaben.
...
Wir werden die Arbeitsverzeichnisse inspizieren und den Operator `view()` verwenden, um herauszufinden, was schiefgelaufen ist.

#### 3.3.1. Die Arbeitsverzeichnisse für die relevanten Aufrufe überprüfen

Schau dir das Arbeitsverzeichnis für den fehlgeschlagenen `GATK_HAPLOTYPECALLER`-Prozessaufruf an, der in der Konsolenausgabe aufgeführt ist.
...
Wir müssen dem `GATK_HAPLOTYPECALLER`-Prozess nicht mehr den ursprünglichen `reads_ch` bereitstellen, da die BAM-Datei jetzt in den Kanal gebündelt ist, der von `SAMTOOLS_INDEX` ausgegeben wird.
...
Der Workflow verarbeitet jetzt zuverlässig eine beliebige Anzahl von Proben, aber sie einzeln in der Konfiguration aufzulisten, ist nicht sehr skalierbar.
Im nächsten Schritt wechseln wir zum Lesen von Eingaben aus einer Datei.

### Fazit

Du weißt, wie du deinen Workflow auf mehreren Proben (unabhängig) laufen lassen kannst.
...
Es kann so einfach sein wie eine Textdatei, die einen Dateipfad pro Zeile auflistet und nichts anderes, oder die Datei kann zusätzliche Metadaten enthalten, in diesem Fall wird sie oft als Samplesheet bezeichnet.
...
Ändere den `reads_bam`-Parameter so, dass er auf die Datei `sample_bams.txt` zeigt, anstatt einzelne Proben aufzulisten.
...
Indem wir jedoch den vielseitigeren `splitCsv`-Operator (ergänzt durch `map`) verwenden, können wir unseren Workflow zukunftssicher machen, falls wir uns entscheiden, Metadaten zur Datei mit Dateipfaden hinzuzufügen.
...
Führe den Workflow noch einmal aus. Dies sollte dasselbe Ergebnis wie zuvor liefern, richtig?
...
Ja! Tatsächlich erkennt Nextflow korrekt, dass die Prozessaufrufe genau gleich sind, und macht sich nicht einmal die Mühe, alles erneut auszuführen, da wir mit `-resume` gelaufen sind.
...
Allgemeiner hast du gelernt, wie man wesentliche Nextflow-Komponenten und -Logik verwendet, um eine einfache Genomik-Pipeline zu erstellen, die echte Arbeit leistet und die Eigenheiten genomischer Dateiformate und Tool-Anforderungen berücksichtigt.
