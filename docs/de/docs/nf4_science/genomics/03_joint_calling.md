...
In Teil 2 hast du eine Pipeline für die Variantenerkennung pro Probe erstellt, die die Daten jeder Probe unabhängig verarbeitet hat.
...
2. GATK HaplotypeCaller auf jeder BAM-Eingabedatei ausführen, um ein GVCF mit Variantenaufrufen pro Probe zu generieren
...
??? info "Wie du von diesem Abschnitt aus beginnst"
    ...
    Dieser Abschnitt des Kurses setzt voraus, dass du [Teil 2: Variantenerkennung pro Probe](./02_per_sample_variant_calling.md) abgeschlossen hast und eine funktionierende `genomics.nf`-Pipeline besitzt.
    ...
    Damit erhältst du einen vollständigen Workflow für die Variantenerkennung pro Probe.
...
1. **Den Schritt zur Variantenerkennung pro Probe so ändern, dass ein GVCF erzeugt wird.**
...
Wir müssen den GVCF-Variantenerkennungsmodus aktivieren und die Ausgabedateierweiterung aktualisieren.

Erinnere dich an den GVCF-Variantenerkennungsbefehl aus [Teil 1](01_method.md):
...
Wie in [Teil 1](01_method.md) behandelt, ist dies eine Zwei-Tool-Operation: GenomicsDBImport kombiniert die GVCFs, dann erzeugt GenotypeGVCFs die finalen Variantenaufrufe.
...
#### 2.1.2. Die HaplotypeCaller-Ausgaben über alle Proben sammeln
...
Das können wir mit dem `collect()`-Kanaloperator tun.
...
Bevor wir mit dem Ausfüllen der Prozessdefinition beginnen, gibt es eine Sache zu klären.
...
1. `all_gvcfs.collect { gvcf -> "-V ${gvcf}" }` iteriert über jeden Dateipfad und stellt `-V ` voran, was `["-V A.g.vcf", "-V B.g.vcf", "-V C.g.vcf"]` erzeugt.
2. `.join(' ')` verkettet sie mit Leerzeichen: `"-V A.g.vcf -V B.g.vcf -V C.g.vcf"`.
3. Das Ergebnis wird einer lokalen Variable `gvcfs_line` zugewiesen (definiert mit `def`), die wir in die Befehlsvorlage interpolieren können.
...
Jetzt können wir uns dem Schreiben des vollständigen Prozesses widmen.
...
Führe den Workflow aus, um zu überprüfen, ob alles funktioniert.
...
Feiere deinen Erfolg und gönne dir eine wohlverdiente Pause.

Im nächsten Teil dieses Kurses lernst du, wie du eine produktionsreife Variant-Calling-Pipeline von nf-core ausführst und sie mit der Pipeline vergleichst, die du manuell erstellt hast.
