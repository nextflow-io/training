...
=== "Danach"
    ...
    workflow {
        ...
        main:
        // Kanal aus dem Inhalt einer CSV-Datei erstellen
...
=== "Vorher"
    ...
    workflow {
        ...
        main:
        // Kanal aus einem Dateipfad erstellen
        read_ch = channel.fromPath(params.input)
    ```
...
Das erlaubt es uns, Spalten nach Namen in der `map`-Operation zu referenzieren: `#!groovy row.fastq_path` extrahiert den Dateipfad aus der Spalte `fastq_path` jeder Zeile.
...
Füge diese Zeilen nach dem Aufruf von `HISAT2_ALIGN` zum Workflow-Body hinzu:
...
- Die Ausgabe ändert sich, um die Paired-End-Benennungskonventionen von Trim Galore zu reflektieren, und erzeugt separate FastQC-Berichte für jede Read-Datei
...
Der Abschnitt `publish:` und der Block `output {}` müssen ebenfalls die zwei separaten FastQC-Berichtskanäle aus dem Paired-End-Prozess `TRIM_GALORE` reflektieren.
