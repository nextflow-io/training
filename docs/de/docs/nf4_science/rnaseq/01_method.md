...
Unser Ziel ist es, einen Workflow zu entwickeln, der die folgenden Verarbeitungsschritte implementiert: initiale Qualitätskontrolle der Reads in einer Bulk-RNAseq-Probe durchführen, Adaptersequenzen von den Reads trimmen, die Reads an ein Referenzgenom alignieren und einen umfassenden Qualitätskontrollbericht (QC) erstellen.
...
- **HISAT2_ALIGN:** Reads mit Hisat2 an das Referenzgenom alignieren
- **MULTIQC:** Einen umfassenden QC-Bericht mit MultiQC erstellen

Bevor wir jedoch mit dem Schreiben von Workflow-Code beginnen, werden wir die Befehle manuell mit einigen Testdaten ausprobieren.
Die benötigten Tools sind nicht in der GitHub Codespaces-Umgebung installiert, daher verwenden wir sie über Container (siehe [Hello Containers](../../hello_nextflow/05_hello_containers.md)).
...
Dein Prompt ändert sich zu etwas wie `(base) root@b645838b3314:/tmp#`, was anzeigt, dass du dich jetzt im Container befindest.

Der Teil `-v ./data:/data` des Befehls ermöglicht es uns, auf den Inhalt des `data/`-Verzeichnisses von innerhalb des Containers zuzugreifen.
...
### 1.3. Den ersten `fastqc`-Befehl ausführen

Lass uns `fastqc` ausführen, um Qualitätskontrollmetriken für die Read-Daten zu sammeln.
...
Dies sollte sehr schnell laufen.
Du findest die Ausgabedateien im selben Verzeichnis wie die Originaldaten:
...
Jetzt führen wir `trim_galore` aus, das Cutadapt und FastQC bündelt, um die Adaptersequenzen zu trimmen und QC-Metriken nach dem Trimmen zu sammeln.
...
Alles, was im Container verbleibt, ist für zukünftige Arbeiten nicht zugänglich, also verschieben wir diese in ein neues Verzeichnis.
...
## 2. Reads an das Referenzgenom alignieren

Wir werden ein Container-Image herunterladen, das `hisat2` installiert hat, es interaktiv starten und den Alignment-Befehl ausführen, um die RNAseq-Daten an ein Referenzgenom zu alignieren.

### 2.1. Den `hisat2`-Container herunterladen
...
### 2.2. Den `hisat2`-Container interaktiv starten
...
### 2.3. Die Hisat2-Genomindexdateien erstellen

Hisat2 benötigt die Genomreferenz in einem sehr spezifischen Format und kann nicht einfach die `genome.fa` FASTA-Datei verwenden, die wir bereitstellen. Daher nutzen wir diese Gelegenheit, um die relevanten Ressourcen zu erstellen.
...
Dies erstellt mehrere Genomindexdateien, die du im Arbeitsverzeichnis findest.
...
Wir werden diese gleich verwenden, aber zuerst erstellen wir ein gezipptes Tarball mit diesen Genomindexdateien; wir werden sie später benötigen und das Generieren dieser Dateien ist normalerweise nichts, was wir als Teil eines Workflows tun möchten.

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

Dies speichert ein `genome_index.tar.gz` Tarball mit den Genomindexdateien im `data/`-Verzeichnis auf unserem Dateisystem, was in Teil 2 dieses Kurses nützlich sein wird.

### 2.4. Den `hisat2`-Befehl ausführen
...
Dies läuft fast sofort, da es sich um eine sehr kleine Testdatei handelt.
In realer Größenordnung könnte dies viel länger dauern.

Wieder findest du die Ausgabedateien im Arbeitsverzeichnis:
...
## 3. Einen umfassenden QC-Bericht erstellen

Wir werden ein Container-Image herunterladen, das `multiqc` installiert hat, es interaktiv starten und einen Berichtserstellungsbefehl auf den Vorher/Nachher-FastQC-Berichtsdateien ausführen.

### 3.1. Den `multiqc`-Container herunterladen
...
Hier sehen wir, dass das Tool alle drei QC-Berichte gefunden hat, die wir generiert haben: die initiale QC, die wir mit `fastqc` durchgeführt haben, den Bericht nach dem Trimmen von `cutadapt` (erstellt über `trim_galore`) und die QC nach dem Alignment, die von `hisat2` produziert wurde.
...
Lerne, wie du dieselben Befehle in einen mehrstufigen Workflow verpackst, der Container verwendet, um die Arbeit auszuführen.
