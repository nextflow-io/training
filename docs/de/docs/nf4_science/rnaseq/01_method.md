...
Zuerst beginnen wir mit der **Einzelproben-Verarbeitung**, die die QC-, Trimming- und Alignment-Tools auf eine Probe anwendet.
Dann erweitern wir zur **Mehrproben-Verarbeitung**, die dieselben Tools auf mehrere Proben anwendet und einen aggregierten Qualitätskontroll-Bericht generiert.
...
Damit ist der Testlauf der Einzelproben-Verarbeitung abgeschlossen.
...
## 2. Mehrproben-QC-Aggregation

Die Befehle, die wir gerade getestet haben, verarbeiten jeweils eine Probe.
In der Praxis müssen wir normalerweise viele Proben verarbeiten und dann die QC-Ergebnisse über alle hinweg aggregieren, um die Qualität des gesamten Datensatzes zu bewerten.
...
Hier verarbeiten wir zwei zusätzliche Proben durch dieselben probenspezifischen Tools und verwenden dann MultiQC, um QC-Berichte über alle drei Proben hinweg zu aggregieren.
...
Die probenspezifischen QC- und Trimming-Befehle sind identisch mit denen, die wir in Abschnitt 1.1 ausgeführt haben.
...
Hier sehen wir, dass das Tool alle drei QC-Berichte gefunden hat, die wir generiert haben: die initiale QC von `fastqc`, die Post-Trimming-Berichte von `cutadapt` (über `trim_galore`) und die Alignment-Zusammenfassungen von `hisat2`.
