...
| `{TOOL_A_PROCESS_NAME}`               | Prozessname in GROSSBUCHSTABEN     | SAMTOOLS_INDEX                                      |
| `{TOOL_A_COMMAND}`                    | Der auszuführende Shell-Befehl     | samtools index '<input_bam>'                        |

### Input/Output-Platzhalter
...
3. **Teil 3 (03_multi_sample.md)**: Multi-Sample-Aggregation mit Channel-Operatoren hinzufügen

### Wichtige Konventionen

- Verwende **Before/After-Codeblöcke mit Tabs** und `hl_lines`, um Codeänderungen zu zeigen
...
- Beende jeden Hauptabschnitt mit den Unterabschnitten **Fazit** und **Wie geht es weiter?**
- Verwende `---` horizontale Linien zur Trennung der nummerierten Hauptabschnitte
- Stelle Gerüstdateien (Workflow + Module) bereit, die die Lernenden ausfüllen können
