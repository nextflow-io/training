...
Sobald dein Codespace läuft, musst du zwei Dinge tun, bevor du ins Training einsteigst: Setze dein Arbeitsverzeichnis für diesen spezifischen Kurs und schaue dir die bereitgestellten Materialien an.

### Das Arbeitsverzeichnis festlegen

Standardmäßig öffnet sich der Codespace mit dem Arbeitsverzeichnis im Stammverzeichnis aller Trainingskurse, aber für diesen Kurs arbeiten wir im Verzeichnis `nf4-science/{DOMAIN_DIR}/`.
...
Du kannst VSCode so einstellen, dass es sich auf dieses Verzeichnis fokussiert, sodass nur die relevanten Dateien in der Dateisystem-Explorer-Seitenleiste angezeigt werden:
...
!!! tip "Tipp"
    ...
    Falls du aus irgendeinem Grund aus diesem Verzeichnis herauskommst (z. B. wenn dein Codespace in den Ruhezustand geht), kannst du jederzeit den vollständigen Pfad verwenden, um dorthin zurückzukehren, vorausgesetzt, du arbeitest in der GitHub Codespaces Trainingsumgebung:

    ```bash
    cd /workspaces/training/nf4-science/{DOMAIN_DIR}
    ```

Schauen wir uns nun die Inhalte an.

### Die bereitgestellten Materialien erkunden

Du kannst die Inhalte dieses Verzeichnisses über den Datei-Explorer auf der linken Seite des Trainingsarbeitsbereichs erkunden.
Alternativ kannst du den Befehl `tree` verwenden.

Im gesamten Kurs verwenden wir die Ausgabe von `tree`, um die Verzeichnisstruktur und -inhalte in lesbarer Form darzustellen, manchmal mit kleineren Anpassungen zur besseren Übersichtlichkeit.
...
- **Die Datei `{DOMAIN_DIR}.nf`** ist ein Workflow-Skript, das du im Laufe des Kurses aufbauen wirst.
...
- **Das Verzeichnis `solutions`** enthält fertige Moduldateien und teilspezifische Lösungen, die als Ausgangspunkt für den nächsten Teil dienen können.
