...
Um die von uns bereitgestellte Umgebung auf GitHub Codespaces zu nutzen, klicke auf den Button „Open in GitHub Codespaces" unten. Weitere Optionen findest du unter [Umgebungsoptionen](../envsetup/index.md).
...
Der Codespace ist mit einer VSCode-Oberfläche eingerichtet, die einen Datei-Explorer, einen Code-Editor und ein Terminal umfasst.
...
Sobald dein Codespace läuft, gibt es zwei Dinge zu tun, bevor es losgeht: das Arbeitsverzeichnis festlegen und einen Blick auf die bereitgestellten Materialien werfen.

### Arbeitsverzeichnis festlegen

Standardmäßig öffnet sich der Codespace im Stammverzeichnis aller Trainingskurse.
Wechsle für diesen Kurs in das Verzeichnis `seqera-scale/`:
...
!!! tip "Tipp"
    ...
    Falls du dieses Verzeichnis aus irgendeinem Grund verlässt (z. B. weil dein Codespace in den Ruhezustand wechselt), kannst du jederzeit den vollständigen Pfad verwenden, um dorthin zurückzukehren – vorausgesetzt, du arbeitest in der GitHub Codespaces-Trainingsumgebung:
...
Die Datei **`.seqera_config`** ist ein Platzhalter, den du in Abschnitt 3 ausfüllst, um die `tw` CLI mit deinem Seqera-Zugriffstoken und deinem Workspace zu konfigurieren.
...
**Um zu [Teil 1: Pipelines über die Web-Oberfläche starten](./01_run_with_seqera.md) zu gelangen, klicke auf den Pfeil in der unteren rechten Ecke dieser Seite.**
