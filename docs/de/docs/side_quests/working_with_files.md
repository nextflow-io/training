...
Nextflow bietet leistungsstarke Werkzeuge, um Dateien effizient zu handhaben und hilft dir, deine Daten mit minimalem Code zu organisieren und zu verarbeiten.
...
Die Sequenzierungsdateien sind mit einer typischen `_R1_`- und `_R2_`-Konvention für sogenannte 'Forward Reads' und 'Reverse Reads' benannt.
...
??? info "Was ist `.class`?"
    ...
    In Nextflow sagt uns `.class`, mit welcher Art von Objekt wir arbeiten. Es ist wie zu fragen "was für eine Art von Ding ist das?", um herauszufinden, ob es ein String, eine Zahl, eine Datei oder etwas anderes ist.
...
Wir haben auch bestätigt, dass dies aus Nextflows Sicht nur ein String ist (der Klasse `java.lang.String`).
...
Dieses Mal siehst du den vollständigen absoluten Pfad anstelle des relativen Pfads, den wir als Eingabe angegeben haben.
...
Der Unterschied zwischen Strings und Path-Objekten wird entscheidend, wenn du anfängst, tatsächliche Workflows mit Prozessen zu erstellen.
...
Wie du sehen kannst, ist es ein ziemlich einfaches kleines Skript, das die Datei entpackt und zählt, wie viele Zeilen sie enthält.
...
Dies besagt, dass das System die Datei nicht finden konnte; wenn du jedoch den Pfad nachschlägst, gibt es eine Datei mit diesem Namen an diesem Ort.
...
- Die `file()`-Methode wandelt einen Pfad-String in ein Path-Objekt um, mit dem Nextflow arbeiten kann
...
Wenn du es richtig machst, solltest du niemals die Logik deines Workflows ändern müssen, um Dateien von verschiedenen Standorten zu berücksichtigen.
...
Zum Beispiel hat `/path/to/data` kein Präfix, was anzeigt, dass es sich um einen 'normalen' lokalen Dateipfad handelt, während `s3://path/to/data` das Präfix `s3://` enthält, was anzeigt, dass es sich in Amazons S3-Objektspeicher befindet.
...
Lass uns das testen, indem wir den lokalen Pfad, den wir unserem Workflow zur Verfügung stellen, durch einen HTTPS-Pfad ersetzen, der auf eine Kopie derselben Daten zeigt, die in Github gespeichert sind.
...
Du musst dir diese Klassen nicht merken; wir erwähnen dies nur, um zu demonstrieren, dass Nextflow die verschiedenen Standorte identifiziert und angemessen behandelt.
...
Du kannst überprüfen, dass dies geschehen ist, indem du dir den Inhalt des Arbeitsverzeichnisses ansiehst, das sich am Hash-Wert des Prozesses befindet.

??? abstract "Arbeitsverzeichnis-Inhalt"
...
!!! note
    ...
    Die eine wichtige Ausnahme von diesem Prinzip ist, dass du keine Glob-Muster oder Verzeichnispfade mit HTTPS verwenden kannst, da HTTPS nicht mehrere Dateien auflisten kann, sodass du genaue Datei-URLs angeben musst.
...
Bisher haben wir mit jeweils einer Datei gearbeitet, aber in Nextflow möchten wir normalerweise einen Eingabekanal mit mehreren zu verarbeitenden Eingabedateien erstellen.

Eine naive Möglichkeit, dies zu tun, wäre, die `file()`-Methode mit [`channel.of()`](https://www.nextflow.io/docs/latest/reference/channel.html#of) wie folgt zu kombinieren:
...
Hier kommt [`channel.fromPath()`](https://www.nextflow.io/docs/latest/reference/channel.html#frompath) ins Spiel: eine praktische Channel-Factory, die alle Funktionen bündelt, die wir benötigen, um einen Kanal aus einem oder mehreren statischen Datei-Strings sowie Glob-Mustern zu generieren.
...
Wie du sehen kannst, wird der Dateipfad als `Path`-Typ-Objekt im Kanal geladen.
Dies ist ähnlich wie das, was `file()` getan hätte, außer dass wir jetzt einen Kanal haben, in den wir bei Bedarf mehr Dateien laden können.
...
Bei unserem ersten Versuch mit der Channel-Factory haben wir den Code vereinfacht und nur den Dateinamen ausgegeben.
...
Es gibt mehrere Möglichkeiten, wie wir mehr Dateien in den Kanal laden könnten.
...
Wie du sehen kannst, haben wir jetzt zwei Path-Objekte in unserem Kanal, was zeigt, dass Nextflow die Dateinamen-Expansion korrekt durchgeführt und beide Dateien wie erwartet geladen und verarbeitet hat.
...
- **Destructuring Assignment**: `def (patient, replicate, type, readNum) = ...` extrahiert die tokenisierten Werte in einer Zeile in benannte Variablen
...
Wir haben auch ein paar der Metadaten-Strings mit einer String-Ersetzungsmethode namens `replace()` vereinfacht, um einige unnötige Zeichen zu entfernen (_z.B._ `replicate.replace('rep', '')`, um nur die Nummer aus den Replikat-IDs zu behalten).
...
Viele experimentelle Designs erzeugen gepaarte Datendateien, die davon profitieren, auf explizit gepaarte Weise gehandhabt zu werden.
In der Bioinformatik werden beispielsweise Sequenzierungsdaten oft in Form von gepaarten Reads erzeugt, was Sequenzstrings bedeutet, die vom selben DNA-Fragment stammen (oft als 'Forward' und 'Reverse' bezeichnet, weil sie von entgegengesetzten Enden gelesen werden).

Das ist der Fall bei unseren Beispieldaten, wo R1 und R2 sich auf die beiden Read-Sets beziehen.
...
Nextflow bietet eine spezialisierte Channel-Factory für die Arbeit mit gepaarten Dateien wie dieser namens `channel.fromFilePairs()`, die automatisch Dateien basierend auf einem gemeinsamen Benennungsmuster gruppiert. Das ermöglicht es dir, die gepaarten Dateien mit weniger Aufwand enger zu verknüpfen.
...
### 5.1. Stelle die Channel-Factory auf `channel.fromFilePairs()` um

Um `channel.fromFilePairs` zu verwenden, müssen wir das Muster angeben, das Nextflow verwenden soll, um die beiden Mitglieder in einem Paar zu identifizieren.
...
Dies ist ähnlich wie das Glob-Muster, das wir zuvor verwendet haben, außer dass dies speziell die Teilstrings (entweder `1` oder `2` direkt nach dem R) aufzählt, die die beiden Mitglieder des Paares identifizieren.
...
Wir haben die Channel-Factory umgestellt und das Datei-Matching-Muster angepasst, und dabei haben wir die Map-Operation auskommentiert.
...
Alle Metadaten-Manipulationen, die wir durchgeführt haben, haben den Kanalinhalt nicht wirklich beeinflusst.

Jetzt, da wir die `.fromFilePairs`-Channel-Factory verwenden, ist der Inhalt des resultierenden Kanals anders.
...
Aber das werden wir nicht tun, weil unser ultimatives Ziel darin besteht, einen anderen Prozess, `ANALYZE_READS`, zu verwenden, der Dateipaare angemessen handhabt.
...
Juhu, dieses Mal ist der Workflow erfolgreich!
...
Wir haben bereits Zugriff auf den tatsächlichen Patient\*innen-Identifikator in dem String, den `fromFilePairs()` als Identifikator verwendet hat, sodass wir das verwenden können, um die Metadaten zu extrahieren, ohne den `simpleName` vom Path-Objekt zu holen, wie wir es zuvor getan haben.
...
Wenn du mit der Verarbeitung mehrerer Paare experimentieren möchtest, kannst du versuchen, Wildcards in das Eingabemuster einzufügen und zu sehen, was passiert.
...
Wir könnten uns vorstellen, dass dies Sequenz-Alignment, Variant Calling oder einen anderen Schritt durchführt, der für diesen Datentyp sinnvoll ist.
...
??? abstract "Verzeichnis- und Dateiinhalt"
...
### 6.3. Beziehe viel mehr Patient\*innen ein
...
Lass uns so tun, als wollten wir so gierig wie möglich sein.
Nimm die folgenden Änderungen am Workflow vor:
...
3.  **Laden von Dateien mit der `fromPath()`-Channel-Factory:** Wir haben Kanäle aus Dateimustern mit `channel.fromPath()` erstellt und ihre Dateiattribute einschließlich Objekttypen angezeigt.
