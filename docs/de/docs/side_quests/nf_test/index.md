...
1. **Tests auf Prozessebene**: Für einzelne Prozesse
...
5. **Stresstests**: Bewertung der Leistung der Pipeline unter extremen Bedingungen, um ihre Grenzen zu bestimmen
...
[**nf-test**](https://www.nf-test.com/) ist ein Tool, mit dem du Tests auf Prozess-, Workflow- und Pipeline-Ebene schreiben kannst. Kurz gesagt: Es ermöglicht dir, systematisch zu überprüfen, ob jeder einzelne Teil der Pipeline wie erwartet funktioniert – _isoliert_.

### Lernziele

In dieser Side Quest lernst du, nf-test zu verwenden, um einen Test auf Workflow-Ebene für die Pipeline sowie Tests auf Prozessebene für die zwei aufgerufenen Prozesse zu schreiben.

Am Ende dieser Side Quest wirst du folgende Techniken effektiv einsetzen können:

- nf-test in deinem Projekt initialisieren
- Tests auf Prozess- und Workflow-Ebene generieren
...
!!! warning "nf-test Versionsanforderung"

    Tests auf Prozessebene erfordern **nf-test 0.9.3 oder höher**. Ältere Versionen (einschließlich 0.9.2) generieren Test-Harness-Code, der nicht mit dem strikten Syntax-Parser kompatibel ist, den Nextflow ab Version 26.04 standardmäßig verwendet. Das führt zu einem `Script compilation failed`-Fehler anstelle des erwarteten Testergebnisses.

    Prüfe deine Version mit `nf-test version`. Falls du ein Upgrade benötigst:
...
Eine detaillierte Beschreibung der Dateien findest du im [Aufwärmen aus Hello Nextflow](../../hello_nextflow/00_orientation.md).
...
??? example "Was macht der Hello Nextflow Workflow?"
    ...
    Falls du das [Hello Nextflow](../../hello_nextflow/index.md)-Training nicht gemacht hast, hier ein kurzer Überblick darüber, was dieser einfache Workflow tut.
    ...
    In dieser Side Quest verwenden wir eine Zwischenform des Hello-Workflows, die nur die ersten beiden Prozesse enthält. <!-- TODO: change this to use the full finished workflow as suggested in https://github.com/nextflow-io/training/issues/735 -->
...
- `when`: Die Bedingungen, unter denen der Test ausgeführt werden soll. Dazu gehören die Parameter, die zum Ausführen der Pipeline verwendet werden.
- `then`: Die Assertions, die gemacht werden sollen. Dazu gehören die erwarteten Ergebnisse der Pipeline.

In einfachen Worten lautet die Logik des Tests wie folgt:
„**Wenn** diese _Parameter_ für diese _Pipeline_ angegeben werden, **dann** erwarten wir diese Ergebnisse."
...
2. Die spezifische Funktionalität oder das getestete Szenario beschreiben
3. Klar genug sein, dass du weißt, welche Funktionalität defekt ist, wenn der Test fehlschlägt
...
1. nf-test hat versucht, die Pipeline so auszuführen, wie sie ist, mit den Einstellungen im `when`-Block:
...
Du fragst dich vielleicht, wie wir im Test auf das Stammverzeichnis der Pipeline zeigen sollen. Da dies eine häufige Situation ist, hat nf-test eine Reihe von globalen Variablen, die wir verwenden können, um uns das Leben zu erleichtern. Die vollständige Liste findest du [hier](https://www.nf-test.com/docs/testcases/global_variables/), aber vorerst verwenden wir die Variable `projectDir`, die das Stammverzeichnis des Pipeline-Projekts bezeichnet.
...
Der Test schlägt fehl, weil der `sayHello`-Prozess 1 Eingabe deklariert, aber mit 0 Argumenten aufgerufen wurde. Beheben wir das, indem wir eine Eingabe für den Prozess hinzufügen. Denk daran aus [Hello Workflow](../../hello_nextflow/03_hello_workflow.md) (und dem Aufwärm-Abschnitt oben), dass unser `sayHello`-Prozess eine einzelne Wert-Eingabe nimmt, die wir angeben müssen. Wir sollten auch den Test-Namen korrigieren, um besser widerzuspiegeln, was wir testen.
...
- Wenn es eine erwartete Änderung ist (z. B. das Tool wurde verbessert und die Ergebnisse sind besser), müssen wir den Snapshot aktualisieren, um die neue Ausgabe als Referenz zu akzeptieren. nf-test hat dafür den Parameter `--update-snapshot`.
...
- Wir könnten die vorhandene `greetings.csv`-Datei wiederverwenden
- Wir könnten sie im Test spontan erstellen

Für jetzt verwenden wir die vorhandene `greetings.csv`-Datei mit dem Beispiel, das wir beim Test auf Pipeline-Ebene verwendet haben. Wie zuvor können wir den Test benennen, um besser widerzuspiegeln, was wir testen, aber dieses Mal lassen wir es den Inhalt per Snapshot erfassen, anstatt nach bestimmten Strings zu suchen (wie wir es beim anderen Prozess getan haben).
...
Du kennst jetzt die beiden Hauptansätze zur Ausgabevalidierung – Snapshots und direkte Inhalts-Assertions – und weißt, wann du welchen verwenden solltest.
Du weißt auch, wie man Tests einzeln oder für ein gesamtes Projekt ausführt.
...
Schau dir die [nf-test-Dokumentation](https://www.nf-test.com/) für erweiterte Test-Funktionen und Best Practices an. Vielleicht möchtest du:
...
- Mehr über andere Testtypen wie Workflow-, Performance- und Stresstests erfahren
