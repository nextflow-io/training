...
**Optional:** Wir empfehlen, zuerst die Side Quest [IDE Features for Nextflow Development](../dev_environment/index.md) abzuschließen.
...
Die Fehlermeldung zeigt an, dass die Variable im Skript-Template nicht erkannt wird – und tatsächlich siehst du `#!groovy ${undefined_var}` im script-Block, aber nirgendwo definiert.
...
Der Fehler zeigt auf Zeile 13, wo `#!groovy ${prefix}` verwendet wird. Schauen wir uns `bad_bash_var.nf` an, um zu sehen, was das Problem verursacht:
...
In diesem Beispiel definieren wir die Variable `prefix` in Bash, aber in einem Nextflow-Prozess wird die `$`-Syntax, die wir verwenden, um darauf zu verweisen (`#!groovy ${prefix}`), als Groovy-Variable interpretiert, nicht als Bash. Die Variable existiert nicht im Groovy-Kontext, daher erhalten wir einen 'no such variable'-Fehler.
...
Probiere eine der obigen Korrekturen aus und führe den Workflow erneut aus (die Ausgabe unten verwendet Option 1a):
...
Wähle eine der Lösungen und führe den Workflow erneut aus (die Ausgabe unten verwendet Option 1):
...
!!! tip "Resume für effizientes Debugging nutzen"
    ...
    Du bist `-resume` begegnet, wenn du [Hello Nextflow](../../hello_nextflow/index.md) durchgearbeitet hast, und es ist wichtig, dass du es beim Debugging gut nutzt, um nicht warten zu müssen, während die Prozesse vor deinem Problemprozess erneut ausgeführt werden.
    ...
    **Verfügbare Debugging-Tools:**
    ...
    ??? solution "Lösung"
        ...
        **Korrektur:** Abschließendes Komma entfernen
        ...
        Sobald das Komma entfernt ist, läuft der Parser bis zum Ende der Datei und sucht nach der schließenden geschweiften Klammer für `processFiles` – und meldet `Unexpected input: '<EOF>'`.
        ...
        **Fehler 4: Undefinierter Variablenfehler**
        ...
        ```
        **Korrektur:** Korrekten Kanal verwenden
        ```groovy linenums="87"
        ...
        **Fehler 5: Bash-Variablen-Escaping-Fehler**
        ```groovy linenums="48"
        echo "Heavy computation $i for ${sample_id}"  // FEHLER: $i sieht aus wie eine undefinierte Nextflow-Variable
        ```
        **Korrektur:** Bash-Variable escapen, damit Nextflow sie der Shell überlässt
        ```groovy linenums="48"
        ...
        **Fehler 7: Fehlerhafte Kanalstruktur für heavyProcess**
        ```groovy linenums="87"
        heavy_ch = heavyProcess(input_ch)  // FEHLER: input_ch gibt jetzt ein 2-Element-Tupel aus; heavyProcess braucht nur das erste Element
        ```
        **Korrektur:** Nur die Proben-IDs übergeben
        ```groovy linenums="87"
