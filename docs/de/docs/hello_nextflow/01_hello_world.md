...
:green_book: Das Video-Transkript ist [hier](./transcripts/01_hello_world.md) verfügbar.
///

In diesem ersten Teil des Hello Nextflow Trainingskurses steigen wir mit einem sehr einfachen, domänenunabhängigen Hello World Beispiel ein, das wir schrittweise erweitern, um die Verwendung grundlegender Nextflow-Logik und -Komponenten zu demonstrieren.
...
??? info "Was ist ein Hello World Beispiel?"
    ...
    Das Beispiel besteht typischerweise darin, die Phrase "Hello, World!" auf einem Ausgabegerät wie der Konsole oder dem Terminal auszugeben oder sie in eine Datei zu schreiben.
...
Das Ausführen von Pipelines beinhaltet hauptsächlich das Lesen von Daten aus Dateien und das Schreiben von Ergebnissen in andere Dateien. Lass uns den Befehl also so ändern, dass die Textausgabe in eine Datei geschrieben wird, um das Beispiel etwas relevanter zu machen.
...
Du kannst sie im Datei-Explorer öffnen oder von der Befehlszeile aus mit dem `cat`-Dienstprogramm, zum Beispiel.
...
Das ist es, was wir mit unserem allerersten Nextflow-Workflow nachbilden werden.
...
Wir stellen dir ein voll funktionsfähiges, wenn auch minimalistisches Workflow-Skript namens `hello-world.nf` zur Verfügung, das dasselbe tut wie zuvor (schreibt 'Hello World!' aus), aber mit Nextflow.
...
Wir werden uns zuerst den **process**-Block genauer ansehen, dann schauen wir uns den **workflow**-Block an.
...
Der Prozess-Body muss einen Script-Block enthalten, der den auszuführenden Befehl angibt, was alles sein kann, was du in einem Befehlszeilen-Terminal ausführen könntest.
...
Dies ist notwendig, um zu überprüfen, dass der Befehl erfolgreich ausgeführt wurde, und um die Ausgabe bei Bedarf an nachgelagerte Prozesse weiterzugeben. Ausgaben, die nicht mit dem übereinstimmen, was im Output-Block deklariert ist, werden nicht an nachgelagerte Prozesse weitergegeben.
...
Du wirst später in diesem Trainingsmodul lernen, wie du variable Eingaben hinzufügst; und du wirst in Teil 3 dieses Kurses lernen, wie du weitere Prozesse hinzufügst und sie durch Kanäle verbindest.
...
!!! tip
    ...
    Aber wir werden sie brauchen, um Workflow-Level-Ausgaben nutzen zu können, also können wir sie auch von Anfang an einbeziehen.
...
Für jeden Prozessaufruf erstellt Nextflow ein verschachteltes Unterverzeichnis, das mit einem Hash benannt ist, um es eindeutig zu machen, wo es alle notwendigen Eingaben bereitstellt (standardmäßig mit Symlinks), Hilfsdateien schreibt und Logs und alle Ausgaben des Prozesses ausgibt.
...
In diesem Fall ist es sehr einfach, weil der Prozessbefehl fest codiert war, aber später im Kurs wirst du Prozessbefehle sehen, die eine gewisse Interpolation von Variablen beinhalten.
...
Dies zeigt dir, dass das mehrmalige Ausführen desselben Workflows die Ergebnisse früherer Läufe nicht überschreibt.
...
Jetzt müssen wir nur noch den `output:`-Block hinzufügen, in dem der Ausgabeverzeichnispfad angegeben wird. Beachte, dass dieser neue Block **außerhalb** und **unterhalb** des `workflow`-Blocks innerhalb des Skripts steht.
...
Die Terminal-Ausgabe sollte vertraut aussehen. Äußerlich hat sich nichts geändert.

Überprüfe jedoch deinen Datei-Explorer: Dieses Mal hat Nextflow ein neues Verzeichnis namens `results/` erstellt.
...
Zum Beispiel möchtest du vielleicht deine Ausgaben in Unterverzeichnissen organisieren.
Der einfachste Weg, dies zu tun, ist, für jede Ausgabe einen spezifischen Ausgabepfad zuzuweisen.

#### 2.2.1. Den Ausgabepfad ändern

Auch hier ist das Ändern des Veröffentlichungsverhaltens für eine bestimmte Ausgabe wirklich einfach.
...
Dieses Mal wird das Ergebnis unter dem angegebenen Unterverzeichnis geschrieben.
...
Dieses Mal ist die Datei, wenn du dir die Ergebnisse ansiehst, eine richtige Kopie anstatt nur ein Symlink.
...
Wir müssen die Prozess-Definition bearbeiten, um (1) eine Eingabevariable zu akzeptieren und (2) diese Variable in der Befehlszeile zu verwenden.
...
Wenn wir jedoch echte Arbeit mit unserem Workflow erledigen, werden wir seine Eingaben von der Befehlszeile aus steuern wollen, damit wir so etwas tun können:
...
Prinzipiell können wir es überall schreiben; aber da wir es dem `sayHello()`-Prozessaufruf geben wollen, können wir es dort direkt einfügen, indem wir `sayHello(params.input)` schreiben.
...
Die Ergebnisse der vorherigen Läufe sind jedoch immer noch in den Aufgabenverzeichnissen unter `work` erhalten.
...
Ok, das war praktisch, aber in vielen Fällen macht es Sinn, einen Standardwert für einen bestimmten Parameter anzugeben, damit du ihn nicht für jeden Lauf angeben musst.
...
Manchmal wirst du eine Pipeline erneut ausführen wollen, die du bereits zuvor gestartet hast, ohne Schritte zu wiederholen, die bereits erfolgreich abgeschlossen wurden.
...
- Wenn du eine Pipeline in Produktion ausführst und etwas schief geht, kannst du in vielen Fällen das Problem beheben und die Pipeline erneut starten, und sie wird vom Punkt des Fehlers aus weiterlaufen, was dir viel Zeit und Rechenleistung sparen kann.
...
Die Konsolenausgabe sollte vertraut aussehen, aber es gibt eine Sache, die ein wenig anders ist als zuvor.

Suche nach dem `cached:`-Teil, der in der Prozess-Statuszeile (Zeile 5) hinzugefügt wurde, was bedeutet, dass Nextflow erkannt hat, dass es diese Arbeit bereits erledigt hat und einfach das Ergebnis des vorherigen erfolgreichen Laufs wiederverwendet hat.
...
Diese Datei gibt dir den Zeitstempel, Laufnamen, Status, Revisions-ID, Sitzungs-ID und die vollständige Befehlszeile für jeden Nextflow-Lauf, der vom aktuellen Arbeitsverzeichnis aus gestartet wurde.
...
Du kannst auch das Nextflow-Log verwenden, um einen Lauf anhand seines Zeitstempels und/oder seiner Befehlszeile nachzuschlagen.

#### 4.3.2. Einen Testlauf durchführen

Zuerst verwenden wir das Dry-Run-Flag `-n`, um zu überprüfen, was bei dem Befehl gelöscht wird:
...
Du weißt, wie du Ausgaben in einem bestimmten Verzeichnis veröffentlichst, eine Pipeline erneut startest, ohne Schritte zu wiederholen, die bereits auf identische Weise ausgeführt wurden, und den Befehl `nextflow clean` verwendest, um alte Work-Verzeichnisse zu bereinigen.
...
<quiz>
Welche Datei im Work-Verzeichnis einer Aufgabe enthält den tatsächlich ausgeführten Befehl?
...
<quiz>
Was ist der Standardmodus für das Veröffentlichen von Workflow-Ausgaben?
...
Mehr erfahren: [2.3. Den Veröffentlichungsmodus auf Kopieren setzen](#23-den-veroffentlichungsmodus-auf-kopieren-setzen)
</quiz>

<quiz>
Wie übergibt man einen Parameterwert von der Befehlszeile an einen Nextflow-Workflow?
- [ ] `-parameter value`
- [ ] `--parameter:value`
- [x] `--parameter value`
- [ ] `-p parameter=value`
...
<quiz>
Wie referenziert man eine Variable innerhalb eines Nextflow-Script-Blocks?
