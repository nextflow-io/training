...
Im ersten Teil des Hello Nextflow Trainings steigen wir mit einem sehr einfachen, domänenunabhängigen Hello World Beispiel ein. Wir bauen es schrittweise aus, um die Verwendung grundlegender Nextflow-Logik und -Komponenten zu demonstrieren.
...
Pipelines lesen hauptsächlich Daten aus Dateien und schreiben Ergebnisse in andere Dateien. Lass uns den Befehl also so ändern, dass er die Textausgabe in eine Datei schreibt, um das Beispiel etwas relevanter zu machen.
...
Du kannst sie im Datei-Explorer öffnen oder über die Befehlszeile mit dem `cat`-Dienstprogramm, zum Beispiel.
...
Wir stellen dir ein voll funktionsfähiges, wenn auch minimalistisches Workflow-Skript namens `hello-world.nf` zur Verfügung, das dasselbe tut wie vorher (schreibt 'Hello World!' aus), aber mit Nextflow.
...
Der Prozess-Body muss einen Script-Block enthalten, der den auszuführenden Befehl angibt – das kann alles sein, was du in einem Befehlszeilen-Terminal ausführen könntest.
...
!!! warning "Warnung"
    ...
    Wenn wir das eine ändern, aber nicht das andere, wird das Skript nicht funktionieren.
...
!!! tip "Tipp"
    ...
    Aber wir werden sie brauchen, um Workflow-Level-Ausgaben nutzen zu können, also können wir sie auch von Anfang an einbinden.
...
Für jeden Prozessaufruf erstellt Nextflow ein verschachteltes Unterverzeichnis, das mit einem Hash benannt ist, um es eindeutig zu machen. Dort werden alle notwendigen Eingaben bereitgestellt (standardmäßig mit Symlinks), Hilfsdateien geschrieben und Logs sowie alle Ausgaben des Prozesses geschrieben.

Der Pfad zu diesem Unterverzeichnis wird in gekürzter Form in eckigen Klammern in der Konsolenausgabe angezeigt.
Wenn wir uns ansehen, was wir für die oben gezeigte Ausführung erhalten haben, beginnt die Konsolenlog-Zeile für den sayHello-Prozess mit `[65/7be2fa]`. Das entspricht dem folgenden Verzeichnispfad: `work/65/7be2fa7be2fad5e71e5f49998f795677fd68`
...
??? question "Siehst du nicht dasselbe?"
    ...
    Die exakten Unterverzeichnisnamen werden auf deinem System anders sein.

    Wenn du den Inhalt des Aufgaben-Unterverzeichnisses im VSCode-Datei-Explorer durchsuchst, siehst du alle Dateien sofort.
    Die Log-Dateien sind jedoch so eingestellt, dass sie im Terminal unsichtbar sind. Wenn du also `ls` oder `tree` verwenden möchtest, um sie anzuzeigen, musst du die entsprechende Option zum Anzeigen unsichtbarer Dateien setzen.
...
Zugegeben, das mag nach viel Wrapper-Code für ein so kleines Ergebnis aussehen, aber der Wert all dieses Wrapper-Codes wird deutlicher, sobald wir anfangen, Eingabedateien zu lesen und mehrere Schritte miteinander zu verbinden.

Trotzdem schauen wir uns auch die anderen Dateien in diesem Verzeichnis an. Das sind Hilfs- und Log-Dateien, die von Nextflow als Teil der Aufgabenausführung erzeugt wurden.
...
Das macht es besonders wertvoll, genau sehen zu können, wie Nextflow den Code interpretiert hat und welcher Befehl erzeugt wurde, wenn du eine fehlgeschlagene Ausführung debuggst.

### 1.3. Den Workflow erneut ausführen

Versuche, den Workflow ein paar Mal erneut auszuführen, und schaue dir dann die Aufgabenverzeichnisse unter `work/` an.
...
Du siehst, dass für jede Ausführung ein neues Unterverzeichnis mit einem vollständigen Satz von Ausgabe- und Log-Dateien erstellt wurde.
Dies zeigt dir, dass das mehrfache Ausführen desselben Workflows die Ergebnisse früherer Ausführungen nicht überschreibt.
...
1. Einen `publish:`-Block innerhalb des `workflow`-Bodys, der Prozessausgaben deklariert.
...
Wir können dies verwenden, um spezifische Pfade für alle im `workflow`-Block deklarierten Prozessausgaben zuzuweisen.
...
Da dies auf der Ebene der einzelnen Ausgabe festgelegt wird, kannst du verschiedene Speicherorte und Unterverzeichnisse angeben, um deinen Anforderungen gerecht zu werden.
...
Eine einfache Option besteht darin, den Veröffentlichungsmodus für die Ausgaben, die dir wichtig sind, auf Kopieren umzustellen.
...
Da auch dies auf der Ebene der einzelnen Ausgabe festgelegt wird, kannst du den Veröffentlichungsmodus granular festlegen.
Dies wird später besonders praktisch sein, wenn wir zu mehrstufigen Pipelines übergehen, wo du beispielsweise nur finale Ausgaben kopieren und Zwischenausgaben als Symlinks belassen möchtest.
...
### 2.4. Hinweis zu prozesslevel `publishDir`-Direktiven
...
Lerne, eine variable Eingabe über einen Befehlszeilenparameter bereitzustellen und Standardwerte effektiv zu nutzen.
...
Die Variable `greeting` ist mit `val` vorangestellt, um Nextflow mitzuteilen, dass es sich um einen Wert handelt (nicht um einen Pfad).
...
Wenn wir jedoch echte Arbeit mit unserem Workflow erledigen, möchten wir seine Eingaben von der Befehlszeile aus steuern können, damit wir so etwas tun können:
...
Die allgemeine Syntax besteht darin, `params.<parameter_name>` zu deklarieren, um Nextflow mitzuteilen, dass ein `--<parameter_name>`-Parameter auf der Befehlszeile erwartet wird.
...
Die Ergebnisse der vorherigen Ausführungen sind jedoch weiterhin in den Aufgabenverzeichnissen unter `work` erhalten.
...
Ok, das war praktisch, aber in vielen Fällen macht es Sinn, einen Standardwert für einen bestimmten Parameter anzugeben, damit du ihn nicht bei jeder Ausführung angeben musst.
...
Die Syntax lautet `name: Type = default_value`.
...
Die Ausgabe befindet sich am selben Ort wie zuvor, aber der Inhalt sollte mit dem neuen Text aktualisiert sein.
...
Du weißt, wie du eine einfache variable Eingabe verwendest, die zur Laufzeit über einen Befehlszeilenparameter bereitgestellt wird, sowie wie du Standardwerte einrichtest, verwendest und überschreibst.
...
Das bedeutet, dass Nextflow nur Prozesse ausführt, die du seit der letzten Ausführung hinzugefügt oder geändert hast, oder denen du neue Einstellungen oder Eingaben bereitstellst.
...
Suche nach dem `cached:`-Teil, der in der Prozessstatuszeile (Zeile 5) hinzugefügt wurde, was bedeutet, dass Nextflow erkannt hat, dass es diese Arbeit bereits erledigt hat und einfach das Ergebnis der vorherigen erfolgreichen Ausführung wiederverwendet.

Du kannst auch sehen, dass der Hash des Work-Unterverzeichnisses derselbe ist wie bei der vorherigen Ausführung.
...
Ob du eine neue Pipeline entwickelst oder Pipelines in Produktion ausführst, irgendwann wirst du wahrscheinlich Informationen über vergangene Ausführungen nachschlagen müssen.
So machst du das.
...
Diese Datei gibt dir den Zeitstempel, den Ausführungsnamen, den Status, die Revisions-ID, die Sitzungs-ID und die vollständige Befehlszeile für jede Nextflow-Ausführung, die aus dem aktuellen Arbeitsverzeichnis gestartet wurde.
...
Nextflow verwendet die Sitzungs-ID, um Ausführungs-Caching-Informationen unter dem `cache`-Verzeichnis zu gruppieren, das sich ebenfalls unter `.nextflow` befindet.

### 4.3. Ältere Work-Verzeichnisse löschen

Während des Entwicklungsprozesses führst du deine Entwurfs-Pipeline typischerweise sehr oft aus, was zu einer Ansammlung vieler Dateien über viele Unterverzeichnisse hinweg führen kann.

Glücklicherweise enthält Nextflow einen hilfreichen `clean`-Unterbefehl, der automatisch die Work-Unterverzeichnisse für vergangene Ausführungen löschen kann, die dich nicht mehr interessieren.
...
Hier zeigen wir dir ein Beispiel, das alle Unterverzeichnisse von Ausführungen vor einer bestimmten Ausführung löscht, die über ihren Ausführungsnamen angegeben wird.

Suche die letzte erfolgreiche Ausführung, bei der du `-resume` nicht verwendet hast; in unserem Fall war der Ausführungsname `golden_cantor`.

Der Ausführungsname ist die maschinengenerierte zweiteilige Zeichenfolge, die in eckigen Klammern in der Konsolenausgabezeile `Launching (...)` angezeigt wird.
Du kannst auch das Nextflow-Log verwenden, um eine Ausführung anhand ihres Zeitstempels und/oder ihrer Befehlszeile nachzuschlagen.
...
Wenn du keine Zeilen ausgegeben siehst, hast du entweder keinen gültigen Ausführungsnamen angegeben oder es gibt keine vergangenen Ausführungen zum Löschen. Stelle sicher, dass du `golden_cantor` im Beispielbefehl durch den entsprechenden neuesten Ausführungsnamen in deinem Log ersetzt.
...
!!! Warning "Warnung"
    ...
    Das Löschen von Work-Unterverzeichnissen aus vergangenen Ausführungen entfernt sie aus dem Cache von Nextflow und löscht alle Ausgaben, die in diesen Verzeichnissen gespeichert waren.
...
Du weißt, wie du Ausgaben in einem bestimmten Verzeichnis veröffentlichst, eine Pipeline erneut startest, ohne Schritte zu wiederholen, die bereits auf identische Weise ausgeführt wurden, und wie du den Befehl `nextflow clean` verwendest, um alte Work-Verzeichnisse aufzuräumen.
...
Wenn du bereit bist, gehe weiter zu [**Teil 2: Hello Channels**](./02_hello_channels.md), um zu lernen, wie du Kanäle verwendest, um Eingaben in deinen Workflow einzuspeisen. Dies ermöglicht es dir, Nextflows eingebauten Datenfluss-Parallelismus und andere leistungsstarke Funktionen zu nutzen.
...
<quiz>
Wie übergibst du einen Parameterwert von der Befehlszeile an einen Nextflow-Workflow?
- [ ] `-parameter value`
- [ ] `--parameter:value`
- [x] `--parameter value`
- [ ] `-p parameter=value`

Mehr erfahren: [3.2. Einen Befehlszeilenparameter einrichten, um Benutzereingaben zu erfassen](#32-set-up-a-command-line-parameter-to-capture-user-input)
</quiz>
