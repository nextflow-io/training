...
2. Ausgaben von mehreren Prozess-Aufrufen in einen einzelnen Prozess-Aufruf sammeln
3. Zusätzliche Parameter an einen Prozess übergeben
4. Mehrere Ausgaben aus einem Prozess handhaben
...
Es sollte vertraut aussehen, außer dass wir jetzt explizit zeigen, dass die Ausgaben des Prozesses in einen Kanal verpackt werden, genau wie die Eingaben.
...
- Einen neuen Prozess schreiben, der den Großbuchstaben-Befehl umhüllt.
...
Wir können unseren neuen Prozess nach dem ersten modellieren, da wir alle dieselben Komponenten verwenden wollen.
...
Jetzt müssen wir Nextflow sagen, dass es den Prozess, den wir gerade definiert haben, tatsächlich aufrufen soll.
...
Dies ist noch nicht funktionsfähig, weil wir nicht angegeben haben, was in den `convertToUpper()`-Prozess eingegeben werden soll.
...
Praktischerweise verpackt Nextflow automatisch die Ausgabe eines Prozesses in einen Kanal, wie im Diagramm im Aufwärmabschnitt gezeigt.
...
Zum Schluss aktualisieren wir die Workflow-Ausgaben, um auch die Ergebnisse des zweiten Prozesses zu veröffentlichen.
...
Versuche gerne, die Pfade oder den Veröffentlichungsmodus für einen der Prozesse zu ändern, um zu sehen, was passiert.
...
In der Konsolenausgabe gibt es jetzt eine zusätzliche Zeile, die dem neuen Prozess entspricht, den wir gerade hinzugefügt haben.
...
Das ist praktisch! Aber es lohnt sich trotzdem, einen Blick in das Work-Verzeichnis eines der Aufrufe des zweiten Prozesses zu werfen.
...
Die Ausgabe des ersten Prozesses ist dort, weil Nextflow sie dort **gestaged** hat, um alles Notwendige für die Ausführung innerhalb desselben Unterverzeichnisses zu haben.

Es handelt sich jedoch tatsächlich um einen symbolischen Link, der auf die Originaldatei im Unterverzeichnis des ersten Prozess-Aufrufs zeigt.
...
Bevor du weitermachst, denke darüber nach, wie wir nur die Ausgabe von `sayHello` mit der Eingabe von `convertToUpper` verbunden haben und die beiden Prozesse nacheinander ausgeführt werden konnten.
...
Du weißt, wie du Prozesse verketten kannst, indem du die Ausgabe eines Schritts als Eingabe für den nächsten Schritt bereitstellst.

### Wie geht es weiter?

Lerne, wie du Ausgaben von gestapelten Prozess-Aufrufen sammelst und sie in einen einzelnen Prozess einspeist.

---

## 2. Einen dritten Schritt hinzufügen, um alle Begrüßungen zu sammeln

Wenn wir einen Prozess verwenden, um eine Transformation auf jedes der Elemente in einem Kanal anzuwenden, wie wir es hier mit den mehreren Begrüßungen tun, wollen wir manchmal Elemente aus dem Ausgabekanal dieses Prozesses sammeln und sie in einen anderen Prozess einspeisen, der eine Art Analyse oder Zusammenfassung durchführt.

Zur Demonstration werden wir einen neuen Schritt zu unserer Pipeline hinzufügen, der alle vom `convertToUpper`-Prozess produzierten Großbuchstaben-Begrüßungen sammelt und sie in eine einzelne Datei schreibt.
...
Nimm im Prozess-Block die folgende Codeänderung vor:
...
Konkret können wir den Befehl nicht im Voraus schreiben, also müssen wir Nextflow sagen, wie es ihn zur Laufzeit basierend auf den Eingaben, die in den Prozess fließen, zusammenstellen soll.
...
Jetzt sollten wir nur noch den Sammelprozess auf der Ausgabe des Großbuchstaben-Schritts aufrufen müssen.
...
#### 2.3.1. Die Prozess-Aufrufe verbinden
...
Wir können das direkt in den `collectGreetings()`-Prozess-Aufruf einstecken.
...
Schließlich kannst du dir den Inhalt der Ausgabedatei anschauen, um dich zu überzeugen, dass alles korrekt funktioniert hat.
...
!!! note "Hinweis"
    ...
    Dies zeigt dir, dass die Reihenfolge, in der Elemente durch Prozess-Aufrufe fließen, nicht garantiert konsistent ist.
...
Du weißt, wie du Ausgaben aus einem Stapel von Prozess-Aufrufen sammelst und sie in einen gemeinsamen Analyse- oder Zusammenfassungsschritt einspeist.

Zusammenfassend hast du bis hierhin Folgendes erstellt:
...
- Den Sammelprozess so modifizieren, dass er einen benutzerdefinierten Namen für die Ausgabedatei akzeptiert (`batch_name`)
...
### 3.1. Den Sammelprozess modifizieren
...
Gute Nachricht: Wir können so viele Eingabevariablen deklarieren, wie wir wollen, in der Prozessdefinition.
...
Du lernst später in deiner Nextflow-Reise, wie du erforderliche und optionale Eingaben verwaltest.
...
Dies richtet den Prozess ein, den `batch_name`-Wert zu verwenden, um einen spezifischen Dateinamen für die endgültige Ausgabe des Workflows zu generieren.

### 3.2. Einen `batch`-Kommandozeilenparameter hinzufügen

Jetzt brauchen wir einen Weg, den Wert für `batch_name` zu liefern und ihn an den Prozess-Aufruf zu übergeben.
...
Um den Wert des Parameters an den Prozess zu übergeben, müssen wir ihn im Prozess-Aufruf hinzufügen.
...
Du siehst, dass um mehrere Eingaben an einen Prozess zu übergeben, du sie einfach in den Aufruf-Klammern auflistest, durch Kommas getrennt.

!!! warning "Warnung"

    Du MUSST die Eingaben an den Prozess in der EXAKT GLEICHEN REIHENFOLGE übergeben, wie sie im Eingabedefinitionsblock des Prozesses aufgelistet sind.
...
Bisher haben wir Prozesse verwendet, die jeweils nur eine Ausgabe produzierten.
...
Was passiert, wenn ein Prozess mehr als eine Ausgabe produziert?
...
### 4.1. Den Prozess so modifizieren, dass er die Anzahl der Begrüßungen zählt und ausgibt

Dies erfordert zwei wichtige Änderungen an der Prozessdefinition: wir brauchen einen Weg, die Begrüßungen zu zählen und eine Berichtsdatei zu schreiben, dann müssen wir diese Berichtsdatei zum `output`-Block des Prozesses hinzufügen.

#### 4.1.1. Die Anzahl der gesammelten Begrüßungen zählen

Praktischerweise erlaubt Nextflow uns, beliebigen Code im `script:`-Block der Prozessdefinition hinzuzufügen, was wirklich praktisch ist, um solche Dinge zu tun.
...
Nimm im `collectGreetings`-Prozess-Block die folgenden Codeänderungen vor:
...
!!! tip "Tipp"
    ...
    Wenn du die Ausgaben eines Prozesses nicht mit `emit:` benennst, kannst du trotzdem einzeln auf sie zugreifen, indem du ihren jeweiligen (nullbasierten) Index verwendest.
    ...
    Wir bevorzugen es, Ausgaben zu benennen, weil es sonst zu einfach ist, versehentlich den falschen Index zu greifen, besonders wenn der Prozess viele Ausgaben produziert.
...
Wie du sehen kannst, ist das Referenzieren spezifischer Prozess-Ausgaben jetzt trivial.
Wenn wir in Teil 5 (Containers) einen weiteren Schritt zu unserer Pipeline hinzufügen, werden wir in der Lage sein, leicht auf `collectGreetings.out.outfile` zu verweisen und es dem neuen Prozess zu übergeben (Spoiler: der neue Prozess heißt `cowpy`).
...
Du weißt, wie du einen Prozess mehrere benannte Ausgaben ausgeben lässt und wie du sie auf Workflow-Ebene angemessen handhabst.

Allgemeiner verstehst du die wichtigsten Prinzipien, die beim Verbinden von Prozessen auf gängige Weise involviert sind.
...
<quiz>
Was bestimmt die Reihenfolge der Prozess-Ausführung in Nextflow?
- [ ] Die Reihenfolge, in der Prozesse im Workflow-Block geschrieben sind
- [ ] Alphabetische Reihenfolge nach Prozess-Namen
...
<quiz>
Welcher Operator sollte `???` ersetzen, um alle Ausgaben in eine einzelne Liste für den nachfolgenden Prozess zu sammeln?
...
- [x] Wenn ein nachfolgender Prozess alle Elemente von einem vorgelagerten Prozess benötigt
- [ ] Wenn du Daten auf mehrere Prozesse aufteilen möchtest

Mehr erfahren: [2.4. Einen Operator verwenden, um die Begrüßungen in eine einzelne Eingabe zu sammeln](#24-einen-operator-verwenden-um-die-begrussungen-in-eine-einzelne-eingabe-zu-sammeln)
</quiz>

<quiz>
Wie greifst du auf eine benannte Ausgabe von einem Prozess zu?
...
<quiz>
Was ist die korrekte Syntax, um eine Ausgabe in einem Prozess zu benennen?
- [ ] `name: outputName`
- [ ] `output: outputName`
- [x] `emit: outputName`
- [ ] `label: outputName`
...
<quiz>
Was muss bei der Übergabe mehrerer Eingaben an einen Prozess zutreffen?
...
Mehr erfahren: [3. Zusätzliche Parameter an einen Prozess übergeben](#3-zusatzliche-parameter-an-einen-prozess-ubergeben)
</quiz>
