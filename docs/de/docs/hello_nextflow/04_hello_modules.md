...
Das ermöglicht es, Prozess-Definitionen in mehreren Workflows wiederzuverwenden, ohne mehrere Kopien des Codes zu erstellen.
...
Das gibt uns einen Platz, um den Prozess-Code einzufügen.
...
include { <PROZESS_NAME> } from '<pfad_zum_modul>'
```
...
Du siehst, wir haben den Prozess-Namen `sayHello` und den Pfad zur Datei mit dem Modul-Code `./modules/sayHello.nf` eingetragen.

### 2.4. Führe den Workflow aus

Wir führen den Workflow mit im Wesentlichen dem gleichen Code und den gleichen Eingaben wie zuvor aus. Lass uns also mit dem `-resume`-Flag ausführen und sehen, was passiert.
...
/*
 * Verwende ein Textersetzungs-Tool, um die Begrüßung in Großbuchstaben umzuwandeln
 */
...
Glückwunsch, du hast all diese Arbeit geleistet und absolut nichts hat sich daran geändert, wie die Pipeline funktioniert!
...
Mehr erfahren: [2. Erstelle ein Modul für `sayHello()`](#2-erstelle-ein-modul-fur-sayhello)
</quiz>
...
Mehr erfahren: [1. Erstelle ein Verzeichnis zum Speichern von Modulen](#1-erstelle-ein-verzeichnis-zum-speichern-von-modulen)
</quiz>
...
Mehr erfahren: [2.3. Füge eine include-Deklaration hinzu](#23-fuge-eine-include-deklaration-vor-dem-workflow-block-hinzu)
</quiz>
