# Teil 3: Ein nf-core-Modul erstellen
...
??? info "Wie du von diesem Abschnitt aus beginnst"
    ...
    Dieser Abschnitt setzt voraus, dass du [Teil 2: Ein nf-core-Modul verwenden](./02_use_module.md) abgeschlossen hast und das `FIND_CONCATENATE`-Modul in deine Pipeline integriert hast.

    Wenn du Teil 2 nicht abgeschlossen hast oder für diesen Teil neu beginnen möchtest, kannst du die Lösung `core-hello-part2` als Ausgangspunkt verwenden.
    Führe diese Befehle im Verzeichnis `nfcore-build/` aus:
    ...
    Dies gibt dir eine Pipeline mit dem bereits integrierten `FIND_CONCATENATE`-Modul.
...
In der aktuellen Version der `core-hello`-Pipeline extrahieren wir die Datei aus dem Ausgabe-Tupel von `FIND_CONCATENATE`, um sie an `COWPY` zu übergeben, wie in der oberen Hälfte des Diagramms unten gezeigt.
...
Wie du sehen kannst, haben wir sowohl die **Haupteingabe** als auch die **Ausgabe** in ein Tupel geändert, das dem Muster `tuple val(meta), path(input_file)` folgt, das in Teil 2 dieses Trainings eingeführt wurde.
...
Da die Ausgabe von `FIND_CONCATENATE` und die Eingabe von `COWPY` jetzt die gleiche 'Form' haben, d.h. beide aus einer `tuple val(meta), path(input_file)`-Struktur bestehen, können wir sie einfach direkt verbinden, anstatt die Datei explizit aus der Ausgabe des `FIND_CONCATENATE`-Prozesses extrahieren zu müssen.
...
Wir rufen jetzt `COWPY` direkt mit `FIND_CONCATENATE.out.file_out` auf.
...
Die Pipeline sollte erfolgreich laufen, wobei Metadaten nun von `FIND_CONCATENATE` durch `COWPY` fließen.
...
Die `withName:`-Syntax weist diese Konfiguration nur dem `COWPY`-Prozess zu, und `#!groovy ext.args = { "-c ${params.character}" }` komponiert einfach eine Zeichenkette, die den Wert des `character`-Parameters enthält.
...
1. **Im `script:`-Block haben wir die Zeile `#!groovy prefix = task.ext.prefix ?: "${meta.id}"` hinzugefügt.**
...
2. **In der Befehlszeile haben wir `#!groovy cowpy-${input_file}` durch `#!groovy ${prefix}.txt` ersetzt.**
   Hier fügt Nextflow den durch die obige Zeile bestimmten Wert von `prefix` ein.

3. **Im `output:`-Block haben wir `#!groovy path("cowpy-${input_file}")` durch `#!groovy path("${prefix}.txt")` ersetzt.**
...
- **`core-hello-results`** — Das Ausgabeverzeichnis, das mit `--outdir` auf der Befehlszeile gesetzt wurde, das die nf-core-Logs und die von `FIND_CONCATENATE` veröffentlichten Ergebnisse erhalten hat.
...
"Hey", könntest du sagen, "`FIND_CONCATENATE` sendet seine Ausgaben an `--outdir`. Vielleicht sollten wir dessen `publishDir`-Direktive kopieren?"
...
!!! info "Info"
    ...
    Du wirst vielleicht `hello_software_versions.yml` in `pipeline_info/` bemerken.
    Diese Datei enthält derzeit nur Versionsinformationen von `FIND_CONCATENATE`, weil `COWPY` seine Version noch nicht meldet.
    Abschnitt 1.6 erklärt, wie man das hinzufügt.
...
- **`path:`** Bestimmt das Ausgabeverzeichnis basierend auf dem Prozessnamen.
  Der vollständige Name eines in `task.process` enthaltenen Prozesses umfasst die Hierarchie von Workflow- und Modulimporten (wie `CORE_HELLO:HELLO:FIND_CONCATENATE`).
  ...
  Dies bestimmt, dass die Ergebnisse von `FIND_CONCATENATE` nach `#!groovy ${params.outdir}/find/` veröffentlicht werden.
...
Allerdings kannst du entscheiden, dass du deine Ausgaben anders organisieren möchtest, und die gute Nachricht ist, dass dies einfach zu tun ist.
...
### 1.6. Versionserfassung mit topic channels

Reproduzierbarkeit ist ein Kernprinzip von nf-core: Jeder Pipeline-Lauf zeichnet genau auf, welche Software-Versionen verwendet wurden, damit Ergebnisse reproduziert und über Läufe hinweg verglichen werden können.
Der Bericht `hello_software_versions.yml` in `pipeline_info/` ist die Art und Weise, wie nf-core-Pipelines diese Anforderung erfüllen.

Der zugrundeliegende Mechanismus sind **topic channels**: eine Nextflow-Broadcast-Funktion, bei der jeder Prozess Daten zu einem benannten Topic veröffentlichen kann und jeder Subscriber alles empfängt, was zu diesem Topic veröffentlicht wurde — unabhängig davon, woher es in der Pipeline stammt.
nf-core führt dies 2026 in allen Pipelines ein.
Module veröffentlichen ihre Version als Tupel direkt im output-Block mit dem Schlüsselwort `topic:`, und der Workflow abonniert einmal, um sie alle zu sammeln.

#### 1.6.1. Versionsberichterstattung zu `COWPY` hinzufügen

Öffne die Moduldatei `cowpy.nf` und füge eine Versionsausgabe-Zeile wie unten gezeigt hinzu.
...
Die neue Zeile veröffentlicht ein `[process_name, tool_name, version]`-Tupel im topic channel `"versions"`.
Am script-Block sind keine Änderungen nötig — die Version wird statisch im output-Block deklariert.

#### 1.6.2. Die Pipeline ausführen und den Versionsbericht prüfen
...
Öffne `core-hello-results/pipeline_info/hello_software_versions.yml` und du siehst jetzt beide Module:
...
Die Sammlung auf Workflow-Seite — der `channel.topic("versions")`-Block, den du im Platzhalter-Workflow in Teil 1 gesehen hast — abonniert das Topic und schreibt diesen kombinierten Bericht automatisch.

!!! info "Rückwärtskompatibilität"

    Der `versions_file`-Branch im topic channel-Block des Workflows existiert, um Module zu unterstützen, die noch nicht auf `topic: versions` aktualisiert wurden und weiterhin eine `versions.yml`-Datei im script-Block mit `emit: versions` schreiben.
    Beide Stile werden während der Übergangsphase gleichzeitig unterstützt.
...
- Verstehe, wie topic channels Software-Versionen aller Module automatisch sammeln.

Für weitere Informationen zu Modul-Konventionen, siehe die [nf-core Modul-Spezifikationen](https://nf-co.re/docs/guidelines/components/modules).
...
!!! tip "Mehr über Tests erfahren"
    ...
    Die generierte Testdatei verwendet nf-test, ein Test-Framework für Nextflow-Pipelines und -Module. Um zu lernen, wie man diese Tests schreibt und ausführt, siehe die [nf-test-Nebenquest](../side_quests/nf_test/index.md).
...
- **`topic: versions`-Ausgabe**: Versionsberichterstattung über topic channel, wie in Abschnitt 1.6 beschrieben
...
- **`script:`-Block**: Leer — wir fügen hier den `cowpy`-Befehl ein
- **`stub:`-Block**: Wir aktualisieren ihn, um die korrekten Ausgaben zu erzeugen
...
Für die Conda-Umgebung gibt der Modulcode `#!groovy conda "${moduleDir}/environment.yml"` an, was bedeutet, dass sie in der Datei `environment.yml` konfiguriert werden sollte.
...
- Den Ausgabedateinamen mit dem konfigurierbaren Präfix-Muster (`#!groovy ${prefix}.txt` statt Wildcard `*`)
- Einen beschreibenden Emit-Namen (`cowpy_output` statt dem generischen `output`)
- Einen statischen Versionsstring (`#!groovy val("1.1.5")`) anstelle des `#!groovy eval("cowpy --version")` der Vorlage, passend zum manuellen Modul aus Abschnitt 1.6 (das `cowpy`-Tool stellt kein `--version`-Flag bereit)

Wenn du den Nextflow Language Server zur Syntaxvalidierung verwendest, wird der Teil `#!groovy ${prefix}` in dieser Phase als Fehler markiert, weil wir ihn noch nicht zum Script-Block hinzugefügt haben.
Kommen wir jetzt dazu.

#### 2.3.2. Der Script-Block

Die Vorlage generiert einen leeren Script-Block, den du mit dem eigentlichen Tool-Befehl füllen musst.
...
- Füge den eigentlichen `cowpy`-Befehl ein, der sowohl `$args` als auch `#!groovy ${prefix}.txt` verwendet
...
- Der Stub erstellt eine leere `#!groovy ${prefix}.txt`-Datei, die dem entspricht, was der Script-Block erzeugt
...
Wenn du bereit bist, fahre mit [Teil 4: Eingabevalidierung](./04_input_validation.md) fort, um zu lernen, wie du Schema-basierte Eingabevalidierung zu deiner Pipeline hinzufügst.
