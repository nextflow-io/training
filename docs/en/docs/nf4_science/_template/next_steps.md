...
Congratulations on completing the Nextflow for {DOMAIN} training course!

## Your journey

You started by running {METHOD} tools manually in the terminal to understand the methodology.
Then you built a single-sample Nextflow pipeline to automate the process, scaled it to handle multiple samples in parallel, and added multi-sample {AGGREGATION_SUMMARY}.

### What you built

- A {METHOD} pipeline that takes {PRIMARY_INPUT_TYPE}s as input and produces {FINAL_OUTPUT_TYPE}s as output.
- {N} processes ({PROCESS_LIST}) stored in separate module files.
...
You're now equipped to start applying Nextflow to {DOMAIN} analysis workflows in your own work.
...
- Get started with nf-core with [Build with nf-core](../../hello_nf-core/index.md)
