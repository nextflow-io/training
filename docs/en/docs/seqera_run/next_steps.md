...
- The `tw` CLI provides the same capabilities as the web interface, useful for scripting and automation.
- Any GitHub-hosted pipeline with a `main.nf` and `nextflow.config` can be registered with `tw pipelines add`.
- `tw launch` takes parameters through profiles and `--params-file`, rather than individual flags.
...
- Authenticate and use the `tw` CLI to manage and launch pipelines from the terminal
- Register a GitHub-hosted pipeline and launch it with a parameter file
