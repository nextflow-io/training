...
Na Parte 1 deste curso (Hello World), mostramos como fornecer uma entrada variável a um processo fornecendo a entrada diretamente na chamada do processo: `sayHello(params.input)`.
Essa foi uma abordagem deliberadamente simplificada.
Na prática, essa abordagem tem limitações importantes; ou seja, ela só funciona para casos muito simples onde queremos executar o processo apenas uma vez, em um único valor.
...
Canais são filas projetadas para lidar com entradas de forma eficiente e transportá-las de uma etapa para outra em fluxos de trabalho de múltiplas etapas, ao mesmo tempo que fornecem paralelismo integrado e muitos benefícios adicionais.
...
??? info "Como começar a partir desta seção"
    ...
    Esta seção do curso assume que você completou a Parte 1 do curso [Hello Nextflow](./index.md), mas se você está confortável com os conceitos básicos cobertos naquela seção, pode começar daqui sem fazer nada especial.
...
Ele é equivalente ao script produzido ao trabalhar na Parte 1 deste curso de treinamento, exceto que mudamos o destino de saída:
...
Como anteriormente, você encontrará o arquivo de saída chamado `output.txt` no diretório `results/hello_channels` (conforme especificado no bloco `output` do script de fluxo de trabalho, mostrado acima).
...
Se isso funcionou para você, você está pronto para aprender sobre canais.

---

## 1. Forneça entradas variáveis via um canal explicitamente

Vamos criar um **canal** para passar a entrada variável ao processo `sayHello()` em vez de depender do tratamento implícito, que tem certas limitações.
...
Isso cria um canal chamado `greeting_ch` usando a fábrica de canais `channel.of()`, que configura um canal de fila simples, e carrega a string `'Hello Channels!'` para usar como o valor de saudação.
...
!!! note
    ...
    Estamos temporariamente voltando a usar strings codificadas em vez de usar um parâmetro CLI por questão de legibilidade. Voltaremos a usar parâmetros CLI assim que tivermos coberto o que está acontecendo no nível do canal.
...
Agora precisamos realmente conectar nosso canal recém-criado na chamada do processo `sayHello()`, substituindo o parâmetro CLI que estávamos fornecendo diretamente antes.
...
Isso diz ao Nextflow para executar o processo `sayHello` no conteúdo do canal `greeting_ch`.
...
Então aumentamos a flexibilidade do nosso fluxo de trabalho enquanto alcançamos o mesmo resultado final.
...
Como uma prévia disso, vamos olhar mais uma coisa antes de seguir em frente: um pequeno mas conveniente benefício de usar um canal explícito para gerenciar a entrada de dados.
...
Os fluxos de trabalho normalmente executam em lotes de entradas que devem ser processadas em massa, então queremos atualizar o fluxo de trabalho para aceitar múltiplos valores de entrada.

### 2.1. Carregue múltiplas saudações no canal de entrada

Convenientemente, a fábrica de canais `channel.of()` que temos usado está perfeitamente feliz em aceitar mais de um valor, então não precisamos modificar isso de forma alguma.
Podemos simplesmente carregar múltiplos valores no canal.
...
A documentação nos diz que isso deve funcionar. Pode realmente ser tão simples?
...
O monitor de execução mostra que `3 of 3` chamadas foram feitas para o processo `sayHello`, e vemos as três saudações enumeradas pela instrução `view()`, uma por linha conforme prometido.
...
Você deve ver uma das três saudações lá, embora a que você obteve possa ser diferente do que é mostrado aqui.
Você consegue pensar por que isso pode ser?

<figure class="excalidraw">
--8<-- "docs/en/docs/hello_nextflow/img/hello-pipeline-channel-multi.svg"
</figure>

_No diagrama, o canal é representado em verde, e a ordem dos elementos é representada como bolinhas em um tubo: a primeira carregada está à direita, depois a segunda no meio, depois a terceira está à esquerda._
...
Neste ponto, precisamos dizer que, por padrão, o sistema de log ANSI escreve o log de múltiplas chamadas ao mesmo processo na mesma linha.
Então o status de todas as três chamadas ao processo sayHello() estão caindo no mesmo lugar.
...
Para um fluxo de trabalho complexo, ou um grande número de entradas, ter a lista completa exibida no terminal ficaria um pouco esmagador.
...
Isso mostra que todos os três processos executaram com sucesso (eba).

Dito isso, ainda temos o problema de que há apenas um arquivo de saída no diretório de resultados.

Você pode se lembrar que codificamos o nome do arquivo de saída para o processo `sayHello`, então todas as três chamadas produziram um arquivo chamado `output.txt`.
...
Mas quando são publicados no mesmo diretório de resultados, o que foi copiado primeiro é sobrescrito pelo próximo, e assim por diante.

### 2.2. Garanta que os nomes dos arquivos de saída serão únicos
...
Uma maneira comum de fazer isso é usar alguma informação única de metadados das entradas (recebidas do canal de entrada) como parte do nome do arquivo de saída.
Aqui, por conveniência, vamos apenas usar a própria saudação, já que é apenas uma string curta, e adicioná-la ao nome base do arquivo de saída.
...
Revertendo de volta para a visualização resumida, a saída é resumida em uma linha novamente.
...
!!! tip
    ...
    Na prática, nomear arquivos com base nos próprios dados de entrada é quase sempre impraticável.
    ...
    Os metadados são normalmente fornecidos via uma 'planilha de amostras' ou equivalentes.
    Você aprenderá como fazer isso mais tarde em seu treinamento Nextflow (veja [Missão secundária de metadados](../side_quests/metadata.md)).
...
O senso comum sugere que devemos ser capazes de simplesmente passar um array de valores em vez de um único valor.
...
Olhe para a saída de `view()` e as mensagens de erro.

Parece que o Nextflow tentou executar uma única chamada de processo, usando `[Hello, Bonjour, Holà]` como um único valor de string, em vez de usar as três strings no array como valores separados.
...
Você já usou o operador `.view()`, que apenas olha o que está lá.
Agora vamos olhar para operadores que nos permitem agir sobre o conteúdo de um canal.

Se você percorrer a [lista de operadores](https://nextflow.io/docs/latest/reference/operator.html) na documentação do Nextflow, encontrará [`flatten()`](https://nextflow.io/docs/latest/reference/operator.html#flatten), que faz exatamente o que precisamos: desempacotar o conteúdo de um array e emiti-los como itens individuais.

#### 3.2.1. Adicione o operador `flatten()`

Para aplicar o operador `flatten()` ao nosso canal de entrada, o anexamos à declaração da fábrica de canais.
...
Poderíamos executar isso imediatamente para testar se funciona, mas já que estamos nisso, vamos refinar como inspecionamos o conteúdo do canal.

Queremos ser capazes de contrastar como o conteúdo se parece antes e depois que o operador `flatten()` é aplicado, então vamos adicionar um segundo, E vamos adicionar um pouco de código para rotulá-los mais claramente na saída.
...
!!! info
    ...
    Esta é uma variável _implícita_ que permite um acesso abreviado à variável interna,
...
Desta vez funciona E nos dá a visão adicional sobre como o conteúdo do canal se parece antes e depois de executarmos o operador `flatten()`.

- Uma única instrução `Before flatten:` porque naquele ponto o canal contém um item, o array original.
...
Aprenda como fazer o fluxo de trabalho pegar um arquivo como sua fonte de valores de entrada.

---

## 4. Leia valores de entrada de um arquivo CSV

Realisticamente, raramente ou nunca vamos começar de um array de valores.
...
Preparamos um arquivo CSV chamado `greetings.csv` que contém várias saudações de entrada, imitando o tipo de dados em colunas que você pode querer processar em uma análise de dados real, armazenado em `data/`.
...
Para começar, vamos precisar fazer duas mudanças principais no script:
...
Isso assume que o arquivo está co-localizado com o código do fluxo de trabalho.
Você aprenderá como lidar com outras localizações de dados mais tarde em sua jornada Nextflow.
...
Para aplicar o operador, o anexamos à linha da fábrica de canais como anteriormente.

No bloco workflow, faça a seguinte alteração de código para substituir `flatten()` por `splitcsv()` (sem comentário):
...
Como você pode ver, também atualizamos as instruções `view()` antes/depois.
...
Vamos tentar executar o fluxo de trabalho com a lógica de análise CSV adicionada.
...
Curiosamente, isso também falha, mas com um erro diferente.
...
Precisamos dizer para pegar apenas a primeira coluna em cada linha.
...
O operador [`map()`](https://nextflow.io/docs/latest/reference/operator.html#map) é uma pequena ferramenta muito útil que nos permite fazer todos os tipos de mapeamentos para o conteúdo de um canal.

Neste caso, vamos usá-lo para extrair aquele elemento que queremos de cada linha em nosso arquivo de dados.
...
Então vamos aplicar isso à nossa análise CSV.
...
Desta vez deve executar sem erro.

Olhando para a saída das instruções `view()`, você vê o seguinte:

- Uma única instrução `Before splitCsv:`: naquele ponto o canal contém um item, o caminho do arquivo original.
...
Faça uma grande pausa, você trabalhou duro nesta!

Quando estiver pronto, passe para [**Parte 3: Hello Workflow**](./03_hello_workflow.md) para aprender como adicionar mais etapas e conectá-las em um fluxo de trabalho adequado.
...
- [x] Uma estrutura semelhante a uma fila para passar dados entre processos
- [ ] Uma configuração
...
- [ ] Filtrar o conteúdo do canal
- [ ] Transformar elementos do canal
- [x] Inspecionar e depurar o conteúdo do canal
- [ ] Salvar o conteúdo do canal em um arquivo
...
- [ ] Filtrar elementos de um canal
- [ ] Combinar múltiplos canais
- [x] Transformar cada elemento em um canal
- [ ] Contar elementos em um canal
...
- [ ] Para habilitar a funcionalidade de retomada

Saiba mais: [2.2. Garanta que os nomes dos arquivos de saída serão únicos](#22-garanta-que-os-nomes-dos-arquivos-de-saida-serao-unicos)
</quiz>
