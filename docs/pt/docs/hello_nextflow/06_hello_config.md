...
Esta seção explorará como configurar e gerenciar a configuração do seu pipeline Nextflow para que você possa personalizar seu comportamento, adaptá-lo a diferentes ambientes e otimizar o uso de recursos _sem alterar uma única linha do código do fluxo de trabalho em si_.
...
Ao aprender a utilizar essas opções de configuração de forma eficaz, você pode melhorar a flexibilidade, escalabilidade e desempenho de seus pipelines.

??? info "Como começar a partir desta seção"

    Esta seção do curso pressupõe que você completou as Partes 1-5 do curso [Hello Nextflow](./index.md) e tem um pipeline completo e funcional.
    ...
    Se você não está familiarizado com o pipeline Hello ou precisa de uma revisão, veja [esta página de informações](../info/hello_pipeline.md).

---

## 0. Aquecimento: Execute `hello-config.nf`

Vamos usar o script de fluxo de trabalho `hello-config.nf` como ponto de partida.
...
Se funcionou para você, está pronto para aprender como configurar seus pipelines.
...
### 1.1. Mover valores padrão para `nextflow.config`
...
Você poderia modificar o personagem, por exemplo, e executar o fluxo de trabalho para se convencer de que o valor definido no arquivo de configuração substitui o definido no arquivo de fluxo de trabalho.
...
#### 1.1.2. Remover os valores do bloco `params` no arquivo de fluxo de trabalho
...
Isso é ótimo, mas às vezes você pode querer executar alguns experimentos temporários com valores padrão diferentes sem mexer no arquivo de configuração principal.
...
Há uma abordagem mais simples para quando você deseja executar seu pipeline com um conjunto específico de valores, ou permitir que outra pessoa o faça com o mínimo de esforço.

O Nextflow nos permite especificar parâmetros via um [arquivo de parâmetros](https://nextflow.io/docs/latest/config.html#params-file) em formato YAML ou JSON, o que torna muito conveniente gerenciar e distribuir conjuntos alternativos de valores padrão, por exemplo, bem como valores de parâmetros específicos da execução.
...
Você sabe como aproveitar as principais opções de configuração para gerenciar entradas de fluxo de trabalho.
...
### 2.1. Personalizar o diretório de saída com `-output-dir`
...
#### 2.1.1. Executar o pipeline com `-output-dir`
...
Isso publica as saídas em `custom-outdir-cli/` em vez de `results/`:
...
#### 2.1.2. Remover caminhos codificados do bloco de saída
...
Para saídas que não precisam de um subdiretório, podemos definir a diretiva `path` como uma string vazia, ou removê-la inteiramente.
...
Além de alterar o diretório de saída via CLI, também podemos definir um valor padrão personalizado no arquivo de configuração usando `outputDir`.
Isso nos permite definir o caminho do diretório dinamicamente - não apenas usando strings estáticas.

#### 2.2.1. Definir `outputDir` no arquivo de configuração
...
Isso define o diretório de saída como `custom-outdir-config/` mais o valor do parâmetro `batch` como um subdiretório.
Agora você pode alterar o local de saída definindo o parâmetro `--batch`:

```bash
nextflow run hello-config.nf --batch my_run
```

Isso publica as saídas em `custom-outdir-config/my_run/`.

!!! note

    A opção CLI `-output-dir` tem precedência sobre a configuração `outputDir`.
    Se estiver definida, a opção de configuração será completamente ignorada.

#### 2.2.2. Subdiretórios com nomes de lote e processo

Também podemos definir declarações de `path` de saída de subdiretório dinamicamente, por saída.

Por exemplo, podemos organizar nossas saídas por processo referenciando `<process>.name` na declaração de caminho de saída:
...
Podemos ir além e compor caminhos de subdiretório mais complexos.
...
!!! tip
    ...
    Incluir `params.batch` no `path` do bloco de saída, em vez do `outputDir` de configuração, significa que não será sobrescrito com `-output-dir` na CLI.

Primeiro, atualize o arquivo de configuração para remover `${params.batch}` de `outputDir` (já que estamos movendo-o para as declarações de caminho):
...
Isso publica as saídas em `custom-outdir-config-2/rep2/`, com o caminho base especificado _e_ o subdiretório do nome do lote _e_ resultados agrupados por processo:
...
#### 2.3.2. Remover o modo de saída do arquivo de fluxo de trabalho
...
Isso publica as saídas em `config-output-mode/`, e todas ainda são cópias adequadas, não links simbólicos.
...
Até agora temos olhado para elementos de configuração que controlam como as entradas entram e onde as saídas saem. Agora é hora de focar mais especificamente em adaptar a configuração do seu fluxo de trabalho ao seu ambiente de computação.
...
Vamos fingir que estamos trabalhando em um cluster HPC e o administrador não permite o uso de Docker por razões de segurança.
Felizmente para nós, o Nextflow suporta múltiplas outras tecnologias de contêiner, incluindo Singularity (que é mais amplamente usado em HPC), e gerenciadores de pacotes de software como Conda.

Podemos alterar nosso arquivo de configuração para usar [Conda](https://nextflow.io/docs/latest/conda.html) em vez de Docker.
...
Nos bastidores, o Nextflow recuperou os pacotes Conda e criou o ambiente, o que normalmente leva um pouco de trabalho; então é bom que não tenhamos que fazer nada disso nós mesmos!
...
Do nosso ponto de vista, parece que funciona exatamente da mesma forma que executar com Docker, mesmo que no backend a mecânica seja um pouco diferente.
...
Por padrão, está definida como `local`, então a seguinte configuração está implícita:
...
Para definir o executor para direcionar um backend diferente, você simplesmente especificaria o executor que deseja usando sintaxe similar à descrita acima para alocações de recursos (veja [documentação de executor](https://nextflow.io/docs/latest/executor.html) para todas as opções).
...
A maioria das plataformas de computação de alto desempenho permite (e às vezes requer) que você especifique certos parâmetros, como solicitações e limitações de alocação de recursos (por exemplo, número de CPUs e memória) e nome da fila de trabalho a usar.
...
Veja [Reports](https://nextflow.io/docs/latest/reports.html) para documentação sobre todos os recursos disponíveis.
...
É muito útil quando seus processos têm requisitos de recursos diferentes. Isso permite que você dimensione corretamente as alocações de recursos que você configura para cada processo com base em dados reais, não em suposições.
...
??? info "Configurações de referência institucionais"
    ...
    Essas configurações compartilhadas são valiosas tanto para pessoas que trabalham lá e podem, portanto, apenas utilizar a configuração de sua instituição pronta para uso, quanto como um modelo para pessoas que estão procurando desenvolver uma configuração para sua própria infraestrutura.
...
Você pode querer alternar entre configurações alternativas dependendo de qual infraestrutura de computação está usando. Por exemplo, você pode querer desenvolver e executar testes em pequena escala localmente em seu laptop, e então executar cargas de trabalho em escala completa em HPC ou nuvem.

O Nextflow permite que você configure qualquer número de [perfis](https://nextflow.io/docs/latest/config.html#config-profiles) que descrevem diferentes configurações, que você pode então selecionar em tempo de execução usando um argumento de linha de comando, em vez de ter que modificar o próprio arquivo de configuração.

### 6.1. Criar perfis para alternar entre desenvolvimento local e execução em HPC

Vamos configurar dois perfis alternativos; um para executar cargas em pequena escala em um computador comum, onde usaremos contêineres Docker, e um para executar em um HPC universitário com um agendador Slurm, onde usaremos pacotes Conda.
...
Para especificar um perfil em nossa linha de comando do Nextflow, usamos o argumento `-profile`.
...
Se no futuro encontrarmos outros elementos de configuração que sempre co-ocorrem com estes, podemos simplesmente adicioná-los ao(s) perfil(is) correspondente(s).
...
Também podemos usá-los para definir valores padrão para parâmetros de fluxo de trabalho, para facilitar que outros experimentem o fluxo de trabalho sem ter que reunir valores de entrada apropriados eles mesmos.
...
Convenientemente, os perfis não são mutuamente exclusivos, então podemos especificar múltiplos perfis em nossa linha de comando usando a seguinte sintaxe `-profile <profile1>,<profile2>` (para qualquer número de perfis).
...
Se as configurações conflitantes forem definidas em diferentes fontes de configuração, a [ordem de precedência](https://nextflow.io/docs/latest/config.html) padrão se aplica.
...
Isso usará Docker onde possível e produzirá saídas em `custom-outdir-config/test`, e desta vez o personagem é a dupla cômica `dragonandcow`.
...
O Nextflow aplica uma [ordem de precedência](https://nextflow.io/docs/latest/config.html) definida para resolver quaisquer conflitos, mas isso pode ser complicado de determinar você mesmo.
E mesmo que nada esteja conflitando, pode ser tedioso procurar todos os lugares possíveis onde as coisas poderiam ser configuradas.
...
Se você fornecer parâmetros de linha de comando, por exemplo, habilitando um ou mais perfis ou carregando um arquivo de parâmetros, o comando também os levará em consideração.
...
Saiba mais: [1.1. Mover valores padrão para `nextflow.config`](#11-mover-valores-padrão-para-nextflowconfig)
</quiz>
...
Saiba mais: [3. Selecionar uma tecnologia de empacotamento de software](#3-selecionar-uma-tecnologia-de-empacotamento-de-software)
</quiz>
...
Saiba mais: [5.3. Definir alocações de recursos para um processo específico](#53-definir-alocações-de-recursos-para-um-processo-específico)
</quiz>
...
Saiba mais: [5.1. Executar o fluxo de trabalho para gerar um relatório de utilização de recursos](#51-executar-o-fluxo-de-trabalho-para-gerar-um-relatório-de-utilização-de-recursos)
</quiz>
...
Saiba mais: [5.5. Adicionar limites de recursos](#55-adicionar-limites-de-recursos)
</quiz>
...
Saiba mais: [4. Selecionar uma plataforma de execução](#4-selecionar-uma-plataforma-de-execução)
</quiz>
...
Saiba mais: [1.3. Usar um arquivo de parâmetros](#13-usar-um-arquivo-de-parâmetros)
</quiz>
...
Saiba mais: [6. Usar perfis para alternar entre configurações predefinidas](#6-usar-perfis-para-alternar-entre-configurações-predefinidas)
</quiz>
