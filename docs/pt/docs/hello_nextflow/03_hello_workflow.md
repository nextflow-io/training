...
Para demonstrar, continuaremos construindo sobre o exemplo Hello World independente de domínio das Partes 1 e 2.
...
Para testá-lo, podemos executar o comando `echo 'Hello World'` e canalizar sua saída para o comando `tr`:
...
Neste, componemos o segundo nome de arquivo de saída com base no nome do arquivo de entrada, de forma similar ao que fizemos originalmente para a saída do primeiro processo.
...
Esta é uma das razões pelas quais os canais Nextflow são tão poderosos: eles cuidam do trabalho braçal envolvido em conectar etapas de fluxo de trabalho.
...
As instruções `view()` podem ir onde você quiser; colocamos logo após a chamada para legibilidade.
...
Ele executa com sucesso, embora a saída do log possa parecer um pouco mais bagunçada do que isso (limpamos para legibilidade).

Desta vez a terceira etapa foi chamada apenas uma vez!
Olhando a saída das instruções `view()`, vemos o seguinte:
...
Agora, contanto que especifiquemos o parâmetro apropriadamente, execuções subsequentes em outros lotes de entradas não sobrescreverão resultados anteriores.
...
Podemos escolher dar nomes a esses canais de saída, o que facilita referenciá-los individualmente mais tarde, ou podemos nos referir a eles por índice.
...
Quando formos adicionar mais uma etapa ao nosso pipeline na Parte 5 (Contêineres), poderemos facilmente nos referir a `collectGreetings.out.outfile` e passá-lo para o novo processo (spoiler: o novo processo se chama `cowpy`).
