...
Estou muito ampliado neste VS Code, então para manter as coisas limpas e organizadas, removi todos os arquivos _.nextflow\*_ e o diretório _work/_ e o _results/_ e tudo do Capítulo Um. E estou apenas começando do zero aqui. Mas não se preocupe muito com isso. Se você não quiser, pode deixar esses arquivos por aí. Eles não causarão problemas.
...
Isso é legal. Mas é, é bem simplista. Temos uma variável neste parâmetro, que vai para um processo que executa uma vez, e não escala realmente. E não podemos dar a ele muitos arquivos diferentes para criar aqui. Não podemos dar a ele muitas saudações diferentes. Temos apenas uma.
...
Canais são um conceito um pouco único para muitas pessoas que estão começando com Nextflow. Vem desses tipos de conceitos de programação funcional, e pode levar um pouco de tempo para entender, mas uma vez que você entende, eles realmente desbloqueiam o poder do Nextflow e é fundamental para como você escreve seus fluxos de trabalho.
...
E vou chamá-lo de "_greeting_ch"_. Esta é a convenção de fazer "_\_ch"_ assim, apenas para que você possa lembrar que esta variável é um canal. Mas você pode chamá-lo do que quiser.

E então vou dizer igual, e vou fazer _"channel.of"._

Channel é como o namespace para tudo relacionado a canais. "c" minúsculo se você já usou Nextflow antes. E o _".of"_ é algo chamado de Channel factory, que é basicamente uma maneira de criar um canal.

Existem muitas channel factories diferentes. Se eu fizer apenas "." aqui, você pode ver que o VS Code está sugerindo várias delas, mas _".of"_ é a mais simples e apenas recebe uma entrada aqui.
...
Deve executar novamente. E imprimimos agora. _"Hello", "Bonjour"_ e _"Hej"_ no terminal com nossa declaração view. Fantástico.
...
E a chave é que temos três tarefas que executaram. A saída do Nextflow tenta resumir isso conforme o processamento continua, para que não tome completamente todo o seu terminal, e esse ANSI Logging usa códigos de escape ANSI, basicamente sobrescreveu as outras tarefas. Então apenas mostra a última que aconteceu de ser atualizada.

## 2.1.3. Executar o comando novamente com a opção -ansi-log false

Há algumas coisas que podemos fazer para realmente entender isso um pouco melhor. Podemos olhar no próprio diretório work e você pode ver todos os diferentes diretórios work lá, mas isso é um pouco confuso porque será misturado com diferentes execuções do Nextflow.

Ou podemos dizer ao Nextflow para não usar os códigos de escape ANSI.

Então se eu executar o comando novamente, mas desta vez eu digo _"-ansi-log false"_ para desligá-lo, eu também poderia usar as variáveis de ambiente _$NO_COLOR_ ou _"$NXF_ANSI_LOG=false"_. Então ele usa o tipo de estilo mais antigo de logging do Nextflow sem nenhum desses códigos de escape. Apenas imprime diretamente no terminal sem atualizações inteligentes.
...
No entanto, isso está corrigido para um problema com os diretórios work, mas não corrigiu um problema com o arquivo de saída. Ainda temos apenas um arquivo de saída que diz _"Hello"_.
...
No entanto, cada três vezes que este processo executa essas três tarefas diferentes. Todos eles geram um arquivo chamado _"output.txt"_, todos esses arquivos de saída são publicados no diretório results, e todos eles sobrescrevem uns aos outros. Então qualquer arquivo de resultado que você obtenha lá é apenas o último que foi gerado, mas destruiu todos os outros. Isso não é realmente o que queremos.
...
Fazemos isso exatamente da mesma maneira. Podemos usar esta variável em qualquer lugar dentro do bloco script e podemos usá-la várias vezes.
...
Ok, vamos tentar e ver se funcionou. Vamos executar o fluxo de trabalho novamente. Esperançosamente mostrará as três tarefas diferentes dentro dos três diretórios work diferentes. E com certeza, você pode ver aqui na pasta results aqui à esquerda. Agora temos três arquivos diferentes com três nomes de arquivo diferentes e cada um com o conteúdo diferente que esperamos. Então os arquivos não estão mais destruindo uns aos outros, e tudo está lá como esperamos.
...
Vale a pena notar também que o que fizemos aqui é um pouco impraticável em situações da vida real. Pegamos alguns dados de entrada e estamos usando esses dados, mas também estamos nomeando o arquivo com base nesses dados, o que você geralmente não pode fazer.
...
Se você está interessado em como fazer isso com as melhores práticas, há uma missão secundária em _training.nextflow.io_, que é toda sobre especificamente metadados e meta maps, então você pode se aprofundar lá para mais detalhes.
...
Posso usar um array em um canal? Vamos tentar. Vou criar um array, e copiei isso dos docs, _"greetings_array"_ e _"Hello", "Bonjour"_ e _"Holà"_. E então vou colocar isso aqui em vez das minhas strings codificadas. Então vou dizer "channel.of" _"greetings_array"_, passando este array para um canal. Vamos tentar.

Abrir o terminal, e executar o pipeline.

Ok. Você pode ver que a declaração view aqui imprimiu nosso array como esperado, mas então todo esse texto vermelho, ou não será vermelho se você ainda tiver _"-ansi-log"_ desligado, mas todo esse texto vermelho está nos dizendo que algo deu errado.

Não temos mais um tique verde aqui. Temos uma cruz vermelha, e se eu apenas tornar isso um pouco mais largo para que seja mais fácil de ler, o Nextflow está nos dizendo o que deu errado.

Então vamos quebrar isso seção por seção. Diz que o erro foi causado por, e então a razão do erro, que são arquivos de saída faltando. Então basicamente aquele bloco de saída disse que este arquivo deveria ser criado e não foi. Em seguida, diz que este é o comando que foi executado. Então isso é basicamente o conteúdo daquele arquivo _.command.sh_. É assim que parecia depois que todas aquelas variáveis foram colocadas.
...
E então o comando saiu assim, e esse foi o diretório work onde podemos ir e ver os arquivos para entender um pouco mais.
...
Ótimo. Então primeiro de tudo é que o pipeline não travou desta vez. Tivemos três processos que executaram corretamente e temos uma marquinha de tique. E então podemos ver que nossas declarações view funcionaram.
...
Então a próxima coisa que vamos fazer é vamos replicar isso, mas em vez de pegar os dados de um único parâmetro CLI ou de uma string ou array codificado, vamos pegá-los de um arquivo.

Então vamos nos livrar do nosso greetings away. E agora vamos mudar esta channel factory novamente. Eu acabei de dizer que havia um monte para escolher e há uma chamada _".fromPath"_. E vou dizer a ela para, neste caso, pegar _params.input_, que está voltando ao nosso input que estávamos usando anteriormente.
...
Então posso apenas definir o padrão aqui para _"data/greetings.csv"_. Agora, quando eu executar este pipeline novamente sem nenhuma opção de linha de comando, ele usará este valor padrão. Ele sabe que é um path, então sabe que deve lidar com isso como um path e não uma string.

E então vai passar isso para uma channel factory deste _params.input_ e criar nosso canal, que então vai ser usado neste processo chamado _sayHello_. Vamos tentar.
...
E então depois de _splitCsv_, vou colocar outra declaração _view_ apenas para que possamos ver como fica depois. Vamos executar o pipeline e ver o que temos.
...
Com nosso logging do operador view, podemos ver antes de _splitCsv_ era o path. E com certeza, depois de _splitCsv_, temos três saídas diferentes e cada uma dessas saídas se parece muito com cada uma das linhas do arquivo _greetings.csv_, o que faz sentido.
...
Para fazer isso, vamos usar _map_. Basicamente _map_ apenas diz, execute algum código, alguma função sobre cada elemento que eu recebo e faça algum tipo de transformação nele. E porque é tão flexível, você verá aparecer no código Nextflow o tempo todo.
...
Arrays no Nextflow são baseados em zero, então vamos dizer apenas o primeiro elemento, que é row zero. Se quiséssemos a segunda coluna, eu poderia ser um ou a terceira coluna ser dois, e assim por diante. Podemos retornar o que quisermos aqui, mas vou retornar apenas o primeiro valor.
...
Terminamos nossa espécie de depuração e este é o código com que terminamos. Pegando nosso parâmetro CLI chamado _input_, que é classificado como um _Path_. O Nextflow encontra o path, carrega-o, e entende o arquivo CSV. Retorna todas as diferentes linhas. E então mapeamos apenas o primeiro elemento daquela linha no canal que meio que nos dá o conteúdo do canal, que é passado para o processo.
...
Canais não são o conceito mais fácil de entender. Então faça uma pausa, pense um pouco sobre isso, talvez leia o material novamente, e realmente certifique-se de que você entendeu esses conceitos porque isso é fundamental para sua compreensão do Nextflow e quanto melhor você entender canais e os diferentes operadores de canal e as diferentes channel factories. Mais diversão você terá escrevendo Nextflow e mais poderosos seus pipelines serão.

Isso não é o mesmo que programação regular em Python ou outras linguagens. Não estamos usando declarações _if_ aqui, isso é programação de fluxo funcional usando canais e operadores. Então é um pouco diferente, mas também é super poderoso.
