...
Nesta primeira parte do curso de treinamento Hello Nextflow, começamos com um exemplo básico Hello World independente de domínio, que vamos construir progressivamente para demonstrar o uso de lógica e componentes fundamentais do Nextflow.

??? info "O que é um exemplo Hello World?"

    Um "Hello World!" é um exemplo minimalista que tem como objetivo demonstrar a sintaxe básica e a estrutura de uma linguagem de programação ou framework de software.
    O exemplo normalmente consiste em imprimir a frase "Hello, World!" no dispositivo de saída, como o console ou terminal, ou escrevê-la em um arquivo.

---

## 0. Aquecimento: Execute um exemplo Hello World diretamente

Vamos demonstrar isso com um comando simples que executamos diretamente no terminal, para mostrar o que ele faz antes de envolvê-lo no Nextflow.
...
Isso exibe o texto 'Hello World' ali mesmo no terminal.

### 0.2. Escreva a saída em um arquivo

Executar pipelines envolve principalmente ler dados de arquivos e escrever resultados em outros arquivos, então vamos modificar o comando para escrever a saída de texto em um arquivo para tornar o exemplo um pouco mais relevante.
...
Você agora sabe como executar um comando simples no terminal que exibe algum texto e, opcionalmente, como fazê-lo escrever a saída em um arquivo.
...
Fornecemos a você um script de fluxo de trabalho totalmente funcional, embora minimalista, chamado `hello-world.nf` que faz a mesma coisa que antes (escrever 'Hello World!'), mas com Nextflow.

Para começar, vamos abrir o script do fluxo de trabalho para que você possa ter uma noção de como ele está estruturado.
...
??? full-code "Arquivo de código completo"
    ...
    /*
    * Use echo para imprimir 'Hello World!' em um arquivo
    */
    process sayHello {
...
Vamos dar uma olhada mais de perto no bloco **process** primeiro, depois vamos olhar o bloco **workflow**.

#### 1.1.1. A definição do `process`
...
O corpo do processo deve conter um bloco script que especifica o comando a ser executado, que pode ser qualquer coisa que você conseguiria executar em um terminal de linha de comando.
...
Aqui temos um **process** chamado `sayHello` que escreve sua **output** em um arquivo chamado `output.txt`.
...
Importante: a definição de output não _determina_ qual saída será criada.
...
Isso é necessário para verificar se o comando foi executado com sucesso e para passar a saída para processos downstream, se necessário. Saídas produzidas que não correspondem ao que está declarado no bloco de output não serão passadas para processos downstream.

!!! warning

    Este exemplo é frágil porque codificamos o nome do arquivo de saída em dois lugares separados (os blocos script e output).
    Se mudarmos um mas não o outro, o script vai quebrar.
    Mais tarde, você aprenderá maneiras de usar variáveis para mitigar esse problema.

Em um pipeline do mundo real, um processo geralmente contém blocos adicionais como diretivas e entradas, que apresentaremos daqui a pouco.

#### 1.1.2. A definição do `workflow`

O segundo bloco de código descreve o **workflow** em si.
A definição do workflow começa com a palavra-chave `workflow`, seguida por um nome opcional, depois o corpo do workflow delimitado por chaves.

Aqui temos um **workflow** que consiste em um bloco `main:` (que diz 'este é o corpo principal do workflow') contendo uma chamada ao processo `sayHello`.
...
Em um pipeline do mundo real, o workflow normalmente contém múltiplas chamadas a **processes** conectados por **channels**, e os processos esperam uma ou mais **input(s)** variáveis.
...
!!! tip
    ...
    Tecnicamente a linha `main:` não é necessária para workflows simples como este, então você pode encontrar workflows que não a têm.
    Mas vamos precisar dela para aproveitar as saídas no nível do workflow, então é melhor incluí-la desde o início.

### 1.2. Execute o fluxo de trabalho

Olhar o código não é nem de longe tão divertido quanto executá-lo, então vamos experimentar isso na prática.

#### 1.2.1. Inicie o fluxo de trabalho e monitore a execução
...
Se a saída do seu console se parecer com isso, então parabéns, você acabou de executar seu primeiro fluxo de trabalho Nextflow!
...
Importante: esta linha também diz onde encontrar a saída da chamada do processo `sayHello`.
Vamos olhar isso agora.

#### 1.2.2. Encontre a saída e os logs no diretório `work`

Quando você executa o Nextflow pela primeira vez em um determinado diretório, ele cria um diretório chamado `work` onde escreverá todos os arquivos (e quaisquer links simbólicos) gerados no curso da execução.

Dentro do diretório `work`, o Nextflow organiza saídas e logs por chamada de processo.
Para cada chamada de processo, o Nextflow cria um subdiretório aninhado, nomeado com um hash para torná-lo único, onde ele preparará todas as entradas necessárias (usando links simbólicos por padrão), escreverá arquivos auxiliares e escreverá logs e quaisquer saídas do processo.

O caminho para esse subdiretório é mostrado de forma truncada entre colchetes na saída do console.
Olhando o que obtivemos para a execução mostrada acima, a linha de log do console para o processo sayHello começa com `[65/7be2fa]`. Isso corresponde ao seguinte caminho de diretório: `work/65/7be2fa7be2fad5e71e5f49998f795677fd68`
...
A primeira coisa que você quer olhar é a saída real do fluxo de trabalho, ou seja, o arquivo `output.txt` produzido pelo processo `sayHello`.
...
Concedido, isso pode parecer muito código de wrapper para um resultado tão pequeno, mas o valor de todo esse código de wrapper ficará mais óbvio quando começarmos a ler arquivos de entrada e encadear várias etapas.

Dito isso, vamos também olhar os outros arquivos nesse diretório. Esses são arquivos auxiliares e de log produzidos pelo Nextflow como parte da execução da tarefa.
...
O arquivo `.command.sh` é especialmente útil porque ele diz o comando principal que o Nextflow executou, não incluindo toda a contabilidade e configuração de tarefa/ambiente.
...
Neste caso é muito direto porque o comando do processo foi codificado, mas mais tarde no curso você verá comandos de processo que envolvem alguma interpolação de variáveis.
Isso torna especialmente valioso poder ver exatamente como o Nextflow interpretou o código e qual comando foi produzido quando você está solucionando problemas de uma execução que falhou.

### 1.3. Execute o fluxo de trabalho novamente

Tente executar novamente o fluxo de trabalho algumas vezes, depois olhe os diretórios de tarefa em `work/`.
...
Você sabe como decifrar um script Nextflow simples, executá-lo e encontrar a saída e os arquivos de log relevantes no diretório work.
...
Isso é feito de propósito; o Nextflow está no controle deste diretório e não devemos interagir com ele.
No entanto, isso torna inconveniente recuperar saídas que nos interessam.

Felizmente, o Nextflow fornece uma maneira de publicar saídas em um diretório designado usando [definições de saída de workflow](https://nextflow.io/docs/latest/workflow.html#workflow-outputs).
...
Precisamos adicionar um bloco `publish:` ao corpo do workflow (mesmo tipo de elemento de código que o bloco `main:`) e listar a saída do processo `sayHello()`.
...
Mais tarde, você aprenderá sobre maneiras de gerar estruturas de diretório de saída sofisticadas, mas por enquanto, estamos apenas codificando um caminho mínimo por simplicidade.
...
Para definir um local personalizado, basta editar o `path` adequadamente:
...
Você pode usar quantos níveis de aninhamento quiser.
...
Isso é ótimo quando você está lidando com arquivos muito grandes, para os quais você não quer armazenar múltiplas cópias.
No entanto, se você deletar o diretório work em algum momento (cobriremos operações de limpeza em breve), você perderá o acesso ao arquivo.
Então você precisa ter um plano para salvar cópias de quaisquer arquivos importantes em um lugar seguro.
...
Basta adicionar `mode 'copy'` à definição de saída relevante no nível do workflow:
...
Isso será especialmente útil mais tarde quando passarmos para pipelines de múltiplas etapas, onde você pode querer copiar apenas saídas finais e deixar saídas intermediárias como links simbólicos, por exemplo.

Como observado anteriormente, existem outras opções mais sofisticadas para controlar como as saídas são publicadas.
Mostraremos como usá-las no devido tempo em sua jornada com Nextflow.

### 2.4. Nota sobre diretivas `publishDir` no nível do processo

Até muito recentemente, a maneira estabelecida de publicar saídas era fazê-lo no nível de cada processo individual usando uma diretiva `publishDir`.

Para alcançar o que acabamos de fazer para as saídas do processo `sayHello`, teríamos adicionado a seguinte linha à definição do processo:
...
Você ainda encontrará esse padrão de código em todos os lugares em pipelines Nextflow mais antigos e módulos de processo, então é importante estar ciente disso.
No entanto, não recomendamos usá-lo em nenhum trabalho novo, pois eventualmente será proibido em versões futuras da linguagem Nextflow.
...
No seu estado atual, nosso fluxo de trabalho usa uma saudação codificada no comando do processo.
...
Isso requer que façamos três conjuntos de mudanças no nosso script:

1. Mudar o processo para esperar uma entrada variável
2. Configurar um parâmetro de linha de comando para capturar entrada do usuário
3. Passar a entrada para o processo no corpo do workflow

Vamos fazer essas mudanças uma de cada vez.

### 3.1. Mude o processo `sayHello` para esperar uma entrada variável
...
No bloco do processo, faça a seguinte mudança de código:
...
Agora trocamos o valor original codificado pelo valor da variável de entrada que esperamos receber.
...
!!! tip
    ...
    As chaves (`{ }`) eram tecnicamente opcionais em versões anteriores do Nextflow, então você pode ver fluxos de trabalho mais antigos onde isso é escrito como `echo '$greeting' > output.txt`.

Agora que o processo `sayHello()` está pronto para aceitar uma entrada variável, precisamos de uma maneira de fornecer um valor de entrada para a chamada do processo no nível do workflow.
...
Felizmente, o Nextflow tem um sistema de parâmetros de workflow integrado chamado [`params`](https://nextflow.io/docs/latest/config.html#params) que torna fácil declarar e usar parâmetros CLI.
...
Aqui, queremos criar um parâmetro chamado `--input`, então precisamos declarar `params.input` em algum lugar no workflow.
...
No bloco do workflow, faça a seguinte mudança de código:
...
Na verdade, realizamos as etapas (2) e (3) descritas no início da seção de uma só vez.
...
Certifique-se de abrir o arquivo de saída para verificar que agora você tem a nova versão da saudação.
...
!!! tip
    ...
    Você pode distinguir prontamente parâmetros no nível do Nextflow de parâmetros no nível do pipeline.

    - Parâmetros que se aplicam a um pipeline sempre levam um hífen duplo (`--`).
    - Parâmetros que modificam uma configuração do Nextflow, _por exemplo_ o recurso `-resume` que usamos anteriormente, levam um único hífen (`-`).

### 3.4. Use valores padrão para parâmetros de linha de comando

Ok, isso foi conveniente, mas em muitos casos, faz sentido fornecer um valor padrão para um determinado parâmetro para que você não tenha que especificá-lo para cada execução.

#### 3.4.1. Defina um valor padrão para o parâmetro CLI

Vamos dar ao parâmetro `input` um valor padrão declarando-o antes da definição do workflow.
...
Como você vê, podemos especificar o tipo de entrada que o workflow espera (Nextflow 25.10.2 e posterior).
A sintaxe é `nome: Tipo = valor_padrão`.
Tipos suportados incluem `String`, `Integer`, `Float`, `Boolean` e `Path`.

!!! info

    Em workflows mais antigos, você pode ver todo esse bloco `params` escrito como apenas `input = 'Holà mundo!'`.
...
Isso facilitará encontrar todos os parâmetros configuráveis rapidamente.
...
A saída estará no mesmo lugar de antes, mas o conteúdo deve ser atualizado com o novo texto.
...
O Nextflow usou o valor padrão do parâmetro greeting para criar a saída.

#### 3.4.3. Sobrescreva o valor padrão

Se você fornecer o parâmetro na linha de comando, o valor CLI sobrescreverá o valor padrão.
...
!!! note
    ...
    No Nextflow, existem vários lugares onde você pode especificar valores para parâmetros.
    Se o mesmo parâmetro for definido com valores diferentes em vários lugares, o Nextflow determinará qual valor usar com base na ordem de precedência que está descrita [aqui](https://www.nextflow.io/docs/latest/config.html).

    Cobriremos isso com mais detalhes na Parte 6 (Configuração).

### Conclusão

Você sabe como usar uma entrada variável simples fornecida em tempo de execução via parâmetro de linha de comando, bem como configurar, usar e sobrescrever valores padrão.
...
Saber como iniciar fluxos de trabalho e recuperar saídas é ótimo, mas você rapidamente descobrirá que existem alguns outros aspectos do gerenciamento de fluxo de trabalho que tornarão sua vida mais fácil, especialmente se você estiver desenvolvendo seus próprios fluxos de trabalho.

Aqui mostramos como usar o recurso [`-resume`](https://nextflow.io/docs/latest/cache-and-resume.html) para quando você precisar relançar o mesmo fluxo de trabalho, como inspecionar o log de execuções passadas com [`nextflow log`](https://nextflow.io/docs/latest/reference/cli.html#log), e como deletar diretórios work mais antigos com [`nextflow clean`](https://nextflow.io/docs/latest/reference/cli.html#clean).
...
- Se você está no meio do desenvolvimento do seu pipeline, pode iterar mais rapidamente, pois só precisa executar o(s) processo(s) em que está trabalhando ativamente para testar suas mudanças.
...
Procure pela parte `cached:` que foi adicionada na linha de status do processo (linha 5), o que significa que o Nextflow reconheceu que já fez esse trabalho e simplesmente reutilizou o resultado da execução bem-sucedida anterior.
...
Seja você desenvolvendo um novo pipeline ou executando pipelines em produção, em algum momento você provavelmente precisará procurar informações sobre execuções passadas.
...
Este arquivo fornece o timestamp, nome da execução, status, ID de revisão, ID de sessão e linha de comando completa para cada execução do Nextflow que foi lançada de dentro do diretório de trabalho atual.
...
Felizmente, o Nextflow inclui um subcomando `clean` útil que pode deletar automaticamente os subdiretórios work de execuções passadas que você não se importa mais.

#### 4.3.1. Determine critérios de exclusão

Existem múltiplas [opções](https://www.nextflow.io/docs/latest/reference/cli.html#clean) para determinar o que deletar.

Aqui mostramos um exemplo que deleta todos os subdiretórios de execuções antes de uma determinada execução, especificada usando seu nome de execução.
...
Primeiro usamos a flag de execução de teste `-n` para verificar o que será deletado dado o comando:
...
Se você não vir nenhuma linha de saída, você não forneceu um nome de execução válido ou não há execuções passadas para deletar. Certifique-se de mudar `golden_cantor` no comando de exemplo para o que for o nome de execução mais recente correspondente no seu log.
...
!!! Warning
    ...
    Deletar subdiretórios work de execuções passadas os remove do cache do Nextflow e deleta quaisquer saídas que foram armazenadas nesses diretórios.
...
Quando estiver pronto, passe para [**Parte 2: Hello Channels**](./02_hello_channels.md) para aprender como usar canais para alimentar entradas no seu fluxo de trabalho, o que permitirá que você aproveite o paralelismo de fluxo de dados integrado do Nextflow e outros recursos poderosos.
...
- [ ] Apenas um bloco de script

Saiba mais: [1.1.1. A definição do processo](#111-a-definição-do-process)
</quiz>

<quiz>
Qual é o propósito do bloco de output em um processo?
...
<quiz>
Olhando o diretório work de uma tarefa, qual arquivo contém o comando real que foi executado?
...
- [x] Pula processos que já foram concluídos com sucesso
...
Saiba mais: [3.2. Configure um parâmetro de linha de comando para capturar entrada do usuário](#32-configure-um-parâmetro-de-linha-de-comando-para-capturar-entrada-do-usuário)
</quiz>

<quiz>
Como você referencia uma variável dentro de um bloco de script Nextflow?
