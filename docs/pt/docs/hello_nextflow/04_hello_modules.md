...
Para usar um módulo em um fluxo de trabalho, você apenas adiciona uma única linha com a declaração `include` ao arquivo de código do seu fluxo de trabalho; então você pode integrar o processo no fluxo de trabalho da mesma forma que normalmente faria.
...
Isso tornará nosso código mais compartilhável, flexível e sustentável.

??? info "Como começar a partir desta seção"

    Esta seção do curso assume que você completou as Partes 1-3 do curso [Hello Nextflow](./index.md), mas se você está confortável com os conceitos básicos cobertos nessas seções, pode começar daqui sem fazer nada especial.
...
Se isso funcionou para você, você está pronto para aprender como modularizar o código do seu fluxo de trabalho.
...
Você pode chamar esse diretório do que quiser, mas a convenção é chamá-lo de `modules/`.
...
Vamos criar um esboço de arquivo para o módulo, copiar o código relevante e então deletá-lo do arquivo principal do fluxo de trabalho.

Então tudo o que precisaremos fazer é adicionar uma declaração `include` para que o Nextflow saiba buscar o código relevante em tempo de execução.
...
Copie toda a definição do processo do arquivo do fluxo de trabalho para o arquivo do módulo.
...
Uma vez feito isso, delete a definição do processo do arquivo do fluxo de trabalho.

### 2.3. Adicione uma declaração include antes do bloco workflow

A sintaxe para incluir um processo de um módulo é bastante direta:

```groovy title="Sintaxe: declaração include"
include { <PROCESS_NAME> } from '<caminho_para_o_módulo>'
```

Vamos inserir isso acima do bloco `params` e preenchê-lo apropriadamente.
...
Isso deve executar muito rapidamente porque tudo está em cache.
...
Você sabe como extrair um processo para um módulo local e sabe que fazer isso não quebra a capacidade de retomada do fluxo de trabalho.

### O que vem a seguir?

Pratique fazendo mais módulos.
Uma vez que você fez um, pode fazer um milhão mais...
...
### 3.3. Adicione uma declaração include antes do bloco `params`

Insira a declaração include acima do bloco `params` e preencha-a apropriadamente.
...
### 4.3. Adicione uma declaração include antes do bloco `params`
...
Parabéns, você fez todo esse trabalho e absolutamente nada mudou em como o pipeline funciona!
...
Faça uma pequena pausa se sentir vontade.

Quando estiver pronto, prossiga para [**Parte 5: Hello Containers**](./05_hello_containers.md) para aprender como usar contêineres para gerenciar dependências de software de forma mais conveniente e reproduzível.
...
<quiz>
Qual convenção é tipicamente usada para armazenar arquivos de módulos?
...
Saiba mais: [2.3. Adicione uma declaração include](#23-adicione-uma-declaracao-include-antes-do-bloco-workflow)
</quiz>
...
- [ ] Ela só funciona para módulos locais
</quiz>
