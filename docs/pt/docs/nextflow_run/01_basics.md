...
Nesta primeira parte do curso de treinamento Nextflow Run, começamos o tópico com um exemplo básico Hello World independente de domínio, que usaremos para demonstrar operações essenciais e apontar os componentes de código Nextflow correspondentes.
...
## 1. Executar um Hello World diretamente

Vamos demonstrar este conceito com um comando simples que executamos diretamente no terminal, para mostrar o que ele faz antes de envolvê-lo em Nextflow.

!!! tip

    Lembre-se de que você deve estar dentro do diretório `nextflow-run/` conforme descrito na página [Primeiros Passos](00_orientation.md).

### 1.1. Fazer o terminal dizer olá
...
### 1.2. Escrever a saída em um arquivo
...
Agora você sabe como executar um comando simples no terminal que exibe algum texto e, opcionalmente, como fazê-lo escrever a saída em um arquivo.
...
Fornecemos a você um script de fluxo de trabalho chamado `1-hello.nf` que recebe uma saudação de entrada via argumento de linha de comando chamado `--input` e produz um arquivo de texto contendo essa saudação.

Não vamos olhar o código ainda; primeiro vamos ver como é executá-lo.

### 2.1. Iniciar o fluxo de trabalho e monitorar a execução
...
### 2.2. Encontrar o arquivo de saída no diretório `results`

Este fluxo de trabalho está configurado para publicar sua saída em um diretório de resultados.
Se você olhar para o seu diretório atual, verá que quando você executou o fluxo de trabalho, o Nextflow criou um novo diretório chamado `results`, bem como um subdiretório chamado `1-hello` dentro dele, contendo um arquivo chamado `output.txt`.
...
### 2.3. Salvar os resultados em um diretório diferente

Por padrão, o Nextflow salvará as saídas do pipeline em um diretório chamado `results` no seu caminho atual.
Para alterar onde seus arquivos são publicados, use a flag CLI `-output-dir` (ou `-o` de forma abreviada)

!!! danger

    Note que `--input` tem dois hífens e `-output-dir` tem um!
...
Os arquivos dentro deste diretório são exatamente os mesmos de antes, é apenas o diretório de nível superior que é diferente.
No entanto, esteja ciente de que em ambos os casos o resultado 'publicado' é uma cópia (ou em alguns casos um link simbólico) da saída real produzida pelo Nextflow quando ele executou o fluxo de trabalho.

Então agora, vamos dar uma olhada por baixo do capô para ver onde o Nextflow realmente executou o trabalho.

!!! Warning

    Nem todos os fluxos de trabalho serão configurados para publicar saídas em um diretório de resultados, e/ou os nomes e estrutura dos diretórios podem ser diferentes.
    Um pouco mais adiante nesta seção, mostraremos como descobrir onde esse comportamento está especificado.

### 2.4. Encontrar a saída original e os logs no diretório `work/`
...
Para cada um, ele preparará as entradas necessárias, executará a(s) instrução(ões) relevante(s) e escreverá saídas e arquivos de log dentro daquele único diretório, que é nomeado automaticamente usando um hash para torná-lo único.
...
Isso deve fornecer o caminho completo do diretório: `work/a3/1e153543b0a7f9d2c4735ddb4ab231`
...
Existem dois conjuntos de diretórios em `work/`, das duas execuções diferentes do pipeline que fizemos.
Cada execução de tarefa obtém seu próprio diretório isolado para trabalhar.
Neste caso, o pipeline fez a mesma coisa nas duas vezes, então o conteúdo de cada diretório de tarefa é idêntico
...
Então isso confirma que o fluxo de trabalho compôs o mesmo comando que executamos diretamente na linha de comando anteriormente.
...
### 2.5. Re-executar o fluxo de trabalho com diferentes saudações

Tente re-executar o fluxo de trabalho algumas vezes com valores diferentes para o argumento `--input`, depois olhe os diretórios de tarefa.
...
## 3. Examinar o script inicial do fluxo de trabalho Hello World

O que fizemos ali foi basicamente tratar o script do fluxo de trabalho como uma caixa preta.
Agora que vimos o que ele faz, vamos abrir a caixa e olhar dentro.

Nosso objetivo aqui não é memorizar a sintaxe do código Nextflow, mas formar alguma intuição básica sobre quais são os principais componentes e como eles são organizados.

### 3.1. Examinar a estrutura geral do código
...
Um script de fluxo de trabalho Nextflow normalmente inclui uma ou mais definições de **process**, o **workflow** em si, e alguns blocos opcionais como **params** e **output**.

Cada **process** descreve qual(is) operação(ões) a etapa correspondente no pipeline deve realizar, enquanto o **workflow** descreve a lógica de fluxo de dados que conecta as várias etapas.
...
O primeiro bloco de código descreve um [**process**](https://nextflow.io/docs/latest/process.html).
...
/*
* Use echo para imprimir uma saudação em um arquivo
*/
process sayHello {
...
Aqui temos um **process** chamado `sayHello` que recebe uma variável de **input** chamada `greeting` e escreve sua **output** em um arquivo chamado `output.txt`.

<figure class="excalidraw">
--8<-- "docs/en/docs/nextflow_run/img/sayhello_with_input.svg"
</figure>

Esta é uma definição de processo muito mínima que contém apenas uma definição de `input`, uma definição de `output` e o `script` a ser executado.
...
O segundo bloco de código descreve o [**workflow**](https://nextflow.io/docs/latest/workflow.html) em si.
...
Aqui temos um **workflow** que consiste em um bloco `main:` e um bloco `publish:`.
...
Como discutiremos em mais detalhes em um momento, `params.input` contém o valor que demos ao parâmetro `--input` em nossa linha de comando.

O bloco `publish:` lista a saída da chamada do processo `sayHello()`, que ele refere como `sayHello.out` e dá o nome `first_output` (isso pode ser qualquer coisa que o autor do fluxo de trabalho queira).

Esta é uma definição de **workflow** muito mínima.
Em um pipeline do mundo real, o fluxo de trabalho normalmente contém múltiplas chamadas a **processes** conectados por **channels**, e pode haver valores padrão configurados para as entradas de variáveis.

Vamos entrar nisso na Parte 2 do curso.
Por enquanto, vamos dar uma olhada mais de perto em como nosso fluxo de trabalho está lidando com entradas e saídas.

### 3.4. O sistema `params` de parâmetros de linha de comando

O `params.input` que fornecemos à chamada do processo `sayHello()` é um pedaço interessante de código Nextflow e vale a pena gastar um minuto extra nisso.
...
Para saber mais, consulte [Workflow parameters](https://nextflow.io/docs/latest/config.html#workflow-parameters) na documentação de referência do Nextflow.

!!! tip

    Lembre-se de que parâmetros de _fluxo de trabalho_ declarados usando o sistema `params` sempre levam dois hífens na linha de comando (`--`).
    Isso os distingue das flags CLI de _nível Nextflow_, que levam apenas um hífen (`-`).

### 3.5. A diretiva `publish`

Na outra extremidade do fluxo de trabalho, já demos uma olhada no bloco `publish:`.
Essa é uma metade do sistema de manipulação de saída; a outra metade é o bloco `output` localizado abaixo.
...
A linha `mode 'copy'` substitui o comportamento padrão do sistema, que é fazer um link simbólico (ou symlink) para o arquivo original no diretório `work/` em vez de uma cópia adequada.

Existem mais opções do que as exibidas aqui para controlar o comportamento de publicação; cobriremos algumas mais tarde.
Você também verá que quando um fluxo de trabalho gera múltiplas saídas, cada uma é listada desta forma no bloco `output`.

Para saber mais, consulte [Publishing outputs](https://nextflow.io/docs/latest/workflow.html#publishing-outputs) na documentação de referência do Nextflow.
...
## 4. Gerenciar execuções de fluxo de trabalho
...
### 4.1. Relançar um fluxo de trabalho com `-resume`

Às vezes, você vai querer re-executar um pipeline que já lançou anteriormente sem refazer nenhum trabalho que já foi concluído com sucesso.
...
- Se você está no meio do desenvolvimento de um pipeline, pode iterar mais rapidamente, pois só precisa executar o(s) processo(s) em que está trabalhando ativamente para testar suas alterações.
...
Procure o bit `cached:` que foi adicionado na linha de status do processo (linha 5), o que significa que o Nextflow reconheceu que já fez este trabalho e simplesmente reutilizou o resultado da execução bem-sucedida anterior.
...
!!! tip
    ...
    Para saber mais, consulte [Cache and resume](https://nextflow.io/docs/latest/cache-and-resume.html) na documentação de referência do Nextflow.

### 4.2. Inspecionar o log de execuções passadas
...
Este arquivo fornece o timestamp, nome da execução, status, ID de revisão, ID de sessão e linha de comando completa para cada execução Nextflow que foi iniciada de dentro do diretório de trabalho atual.

Uma maneira mais conveniente de acessar essas informações é usar o comando [`nextflow log`](https://nextflow.io/docs/latest/reference/cli.html#log).
...
### 4.3. Excluir diretórios de trabalho mais antigos

Se você executar muitos pipelines, pode acabar acumulando muitos arquivos em muitos subdiretórios.
Como os subdiretórios são nomeados aleatoriamente, é difícil dizer pelos seus nomes quais são execuções mais antigas vs. mais recentes.

Felizmente, o Nextflow inclui um comando útil chamado [`nextflow clean`](https://www.nextflow.io/docs/latest/reference/cli.html#clean) que pode excluir automaticamente os subdiretórios de trabalho de execuções passadas com as quais você não se importa mais.

#### 4.3.1. Determinar critérios de exclusão

Existem múltiplas opções para determinar o que excluir, que você pode explorar na documentação vinculada acima.
Aqui mostramos um exemplo que exclui todos os subdiretórios de execuções anteriores a uma determinada execução, especificada usando seu nome de execução.

Procure a execução bem-sucedida mais recente em que você não usou `-resume`; no nosso caso, o nome da execução foi `backstabbing_swartz`.
...
#### 4.3.2. Fazer uma execução de teste
...
Se você não vir nenhuma linha de saída, você não forneceu um nome de execução válido ou não há execuções passadas para excluir. Certifique-se de alterar `backstabbing_swartz` no comando de exemplo para o que for o nome de execução mais recente correspondente no seu log.

#### 4.3.3. Prosseguir com a exclusão
...
!!! Warning
    ...
    Excluir subdiretórios de trabalho de execuções passadas os remove do cache do Nextflow e exclui quaisquer saídas que foram armazenadas nesses diretórios.
    Isso significa que quebra a capacidade do Nextflow de retomar a execução sem re-executar os processos correspondentes.

    Você é responsável por salvar quaisquer saídas com as quais se importa! Essa é a principal razão pela qual preferimos usar o modo `copy` em vez do modo `symlink` para a diretiva `publish`.
...
Saiba mais: [2.3. Encontrar a saída original e os logs no diretório `work/`](#23-find-the-original-output-and-logs-in-the-work-directory)
</quiz>
...
- [ ] Ele lista arquivos de entrada preparados para a tarefa
...
Saiba mais: [2.4. Re-executar o fluxo de trabalho com diferentes saudações](#24-re-run-the-workflow-with-different-greetings)
</quiz>
...
Saiba mais: [4.1. Relançar um fluxo de trabalho com `-resume`](#41-re-launch-a-workflow-with--resume)
</quiz>
...
Saiba mais: [4.2. Inspecionar o log de execuções passadas](#42-inspect-the-log-of-past-executions)
</quiz>
...
- [ ] Copia o script do fluxo de trabalho para os resultados
...
- [x] `-n` (execução de teste) para visualizar o que seria excluído
- [ ] `-v` (verbose) para ver saída detalhada
- [ ] `-a` (all) para selecionar todos os diretórios
- [ ] `-q` (quiet) para suprimir avisos

Saiba mais: [4.3. Excluir diretórios de trabalho mais antigos](#43-delete-older-work-directories)
</quiz>
