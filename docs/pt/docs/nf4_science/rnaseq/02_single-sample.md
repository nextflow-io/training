...
1. Escrever um fluxo de trabalho de estágio único que executa a etapa inicial de QC
2. Adicionar o corte de adaptadores e QC pós-corte
...
Fornecemos um arquivo de fluxo de trabalho, `rnaseq.nf`, que descreve as partes principais do fluxo de trabalho.
...
Vamos criar módulos independentes para cada processo para facilitar o gerenciamento e a reutilização, então vamos criar um diretório para armazená-los.
...
Se você pulou, o Nextflow baixará o contêiner para você; você não precisa fazer nada para que isso aconteça, mas pode precisar esperar até um minuto.
...
## 2. Adicionar o corte de adaptadores e controle de qualidade pós-corte

Vamos usar o wrapper Trim_Galore, que agrupa o Cutadapt para o corte em si e o FastQC para o controle de qualidade pós-corte.

### 2.1. Criar um módulo para o processo de corte e QC
...
### 3.4. Chamar o processo `HISAT2_ALIGN` nos reads cortados produzidos por `TRIM_GALORE`

Os reads cortados estão no canal de saída `TRIM_GALORE.out.trimmed_reads` produzido pela etapa anterior.

Além disso, usamos `file (params.hisat2_index_zip)` para fornecer à ferramenta Hisat2 o tarball compactado do índice do genoma.
...
_Adicionaremos a agregação de relatórios MultiQC na Parte 2, depois de fazer o fluxo de trabalho aceitar várias amostras por vez._
...
Aprenda como modificar o fluxo de trabalho para processar várias amostras em paralelo, agregar relatórios de QC em todas as etapas para todas as amostras e habilitar a execução do fluxo de trabalho em dados de RNAseq paired-end.
