# Parte 3: Implementação multi-amostra com dados paired-end

Nesta parte final do curso, vamos levar nosso fluxo de trabalho simples para o próximo nível, transformando-o em uma poderosa ferramenta de automação em lote para lidar com números arbitrários de amostras.
E enquanto fazemos isso, também vamos modificá-lo para esperar dados paired-end, que são mais comuns em estudos mais recentes.

Faremos isso em três etapas:

1. Fazer o fluxo de trabalho aceitar múltiplas amostras de entrada e paralelizar a execução
2. Adicionar geração abrangente de relatórios de QC
...
Fornecemos um arquivo CSV contendo IDs de amostras e caminhos de arquivos FASTQ no diretório `data/`.
...
Vamos renomear o parâmetro de entrada primário para `input_csv` e mudar o padrão para ser o caminho para o arquivo `single-end.csv`.
...
### 1.2. Atualizar a factory do canal de entrada para lidar com um CSV como entrada
...
Este é o ponto perfeito para colocar uma etapa de agregação de relatórios MultiQC!
...
Adicione a declaração `include { MULTIQC } from './modules/multiqc.nf'` ao arquivo `rnaseq.nf`:
...
Se tivéssemos quatro processos chamados A, B, C e D com um canal simples `.out` cada, a sintaxe ficaria assim: `A.out.mix( B.out, C.out, D.out )`. Como você pode ver, você o aplica ao primeiro dos canais que deseja combinar (não importa qual) e apenas adiciona todos os outros, separados por vírgulas, nos parênteses que seguem.
...
Mas como queremos agregá-los em todas as amostras, precisamos adicionar o operador `collect()` para reunir os relatórios de todas as amostras em uma única chamada para `MULTIQC`.
...
No contexto do bloco de fluxo de trabalho completo, fica assim:
...
Desta vez vemos uma única chamada para MULTIQC adicionada após as chamadas de processo em cache:
...
## 3. Habilitar processamento de dados de RNAseq paired-end
...
Fazer o fluxo de trabalho completamente agnóstico do tipo de dados exigiria usar recursos de linguagem Nextflow um pouco mais avançados, então não vamos fazer isso aqui, mas podemos fazer uma versão de processamento paired-end para demonstrar o que precisa ser adaptado.
...
Fornecemos um segundo arquivo CSV contendo IDs de amostras e caminhos de arquivos FASTQ pareados no diretório `data/`
...
### 3.3. Atualizar a factory do canal

Precisamos dizer ao operador `.map()` para pegar ambos os caminhos de arquivos FASTQ agora.
...
Finalmente, atualize a declaração de importação do módulo para usar a versão paired-end do módulo.
...
### 3.6. Atualizar a chamada ao processo MULTIQC para esperar dois relatórios do TRIM_GALORE

O processo `TRIM_GALORE` agora produz um canal de saída adicional, então precisamos alimentá-lo ao MultiQC.
...
- Substitua todas as instâncias de `${reads.simpleName}` por `${read1.simpleName}` no comando no bloco `script` assim como nas declarações de saída.
...
Não usamos `-resume` já que isso não seria cacheado, e há o dobro de dados para processar do que antes, mas ainda deve completar em menos de um minuto.
...
O próximo passo lógico seria fazer o fluxo de trabalho aceitar qualquer tipo de dados dinamicamente, o que está fora do escopo deste curso, mas podemos abordar isso em um acompanhamento.
...
Parabéns, você completou o mini-curso Nextflow For RNAseq! Celebre seu sucesso e faça uma pausa bem merecida!

Em seguida, pedimos que você complete uma pesquisa muito curta sobre sua experiência com este curso de treinamento, então vamos levá-lo a uma página com links para recursos de treinamento adicionais e links úteis.
