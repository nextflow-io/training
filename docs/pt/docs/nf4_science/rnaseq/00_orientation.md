...
Ao longo deste curso de treinamento, trabalharemos no diretório `nf4-science/rnaseq/`, para o qual você precisa navegar quando abrir o espaço de trabalho de treinamento.
...
- **O arquivo `rnaseq.nf`** é o esboço do script de fluxo de trabalho que desenvolveremos.
...
- **O diretório `data`** contém dados de entrada e recursos relacionados:
  ...
  - _Um genoma de referência_ chamado `genome.fa` consistindo de uma pequena região do cromossomo 20 humano (de hg19/b37).
  - _Dados de RNAseq_ que foram reduzidos a uma pequena região para manter os tamanhos de arquivo pequenos, no diretório `reads/`.
