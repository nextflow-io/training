...
Existem múltiplos métodos válidos para processar e analisar dados de RNAseq em massa.
Para este treinamento, estamos seguindo o método descrito [aqui](https://www.bioinformatics.babraham.ac.uk/training/RNASeq_Course/Analysing%20RNA-Seq%20data%20Exercise.pdf) pelos Drs. Simon Andrews e Laura Biggins no [Babraham Institute](https://www.babraham.ac.uk/).

Nosso objetivo é desenvolver um fluxo de trabalho que implemente as seguintes etapas de processamento: executar controle de qualidade inicial nas leituras em uma amostra de RNAseq em massa, aparar sequências adaptadoras das leituras, alinhar as leituras a um genoma de referência e produzir um relatório abrangente de controle de qualidade (QC).

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/rnaseq/img/preprocess.svg"
</figure>

- **FASTQC:** Realizar QC nos dados de leitura antes do aparamento usando FastQC
- **TRIM_GALORE:** Aparar sequências adaptadoras e realizar QC após o aparamento usando Trim Galore (agrupa Cutadapt e FastQC)
...
## 1. QC inicial e aparamento de adaptadores

Vamos baixar uma imagem de contêiner que tem tanto `fastqc` quanto `trim_galore` instalados, executá-la interativamente e rodar os comandos de aparamento e QC em um dos arquivos de dados de exemplo.
...
Isso fornece a seguinte saída no console conforme o sistema baixa a imagem:
...
### 1.2. Executar o contêiner interativamente
...
Você pode encontrar os arquivos de saída no mesmo diretório dos dados originais:
...
### 1.4. Aparar sequências adaptadoras com `trim_galore`

Agora vamos executar `trim_galore`, que agrupa Cutadapt e FastQC, para aparar as sequências adaptadoras e coletar métricas de QC pós-aparamento.

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

A flag `--fastqc` faz com que o comando execute automaticamente uma etapa de coleta de QC após a conclusão do aparamento.

_A saída é muito verbosa, então o que segue está abreviado._
...
Vamos baixar uma imagem de contêiner que tem `hisat2` instalado, executá-la interativamente e rodar o comando de alinhamento para alinhar os dados de RNAseq a um genoma de referência.
...
### 2.2. Executar o contêiner `hisat2` interativamente
...
A saída é muito verbosa, então o seguinte está abreviado:
...
Vamos usá-los em um momento, mas primeiro vamos gerar um tarball compactado com estes arquivos de índice do genoma; vamos precisar deles mais tarde e gerar estes não é tipicamente algo que queremos fazer como parte de um fluxo de trabalho.

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

Isso armazena um tarball `genome_index.tar.gz` contendo os arquivos de índice do genoma no diretório `data/` em nosso sistema de arquivos, o que será útil na Parte 2 deste treinamento.

### 2.4. Executar o comando `hisat2`

Agora podemos executar o comando de alinhamento, que realiza a etapa de alinhamento com `hisat2` e então canaliza a saída para `samtools` para escrever a saída como um arquivo BAM.
...
Vamos baixar uma imagem de contêiner que tem `multiqc` instalado, executá-la interativamente e rodar um comando de geração de relatório nos arquivos de relatório FastQC antes/depois.
...
### 3.2. Executar o contêiner `multiqc` interativamente
...
MultiQC é capaz de pesquisar através de diretórios por relatórios de QC compatíveis e agregará tudo que encontrar.

Aqui vemos que a ferramenta encontrou todos os três relatórios de QC que geramos: o QC inicial que fizemos com `fastqc`, o relatório pós-aparamento do `cutadapt` (feito via `trim_galore`) e o QC pós-alinhamento produzido por `hisat2`.
...
Aprenda como envolver esses mesmos comandos em um fluxo de trabalho de múltiplas etapas que usa contêineres para executar o trabalho.
