...
É isso! O arquivo de parâmetros documenta sua configuração exata e facilita a reexecução ou o compartilhamento.

### 1.3. Sobrescrevendo parâmetros

Você ainda pode sobrescrever parâmetros específicos pela linha de comando:
...
Cada linha é uma amostra, e as colunas especificam os arquivos e metadados para essa amostra.

Vamos ver a planilha de amostras que estivemos usando:
...
- `spot_table`: Pontos de transcritos (TXT)
- `membrane_image`: Imagem de coloração de membrana (TIFF, opcional)

### 2.2. Caminhos de arquivos

Planilhas de amostras aceitam múltiplos tipos de caminhos:
...
Você pode misturar tipos de caminhos na mesma planilha de amostras.
...
Como você pode ver, o Nextflow executa esta execução de forma similar a quando os arquivos foram baixados do Github. Este é um dos grandes recursos do Nextflow, ele prepara os dados adequadamente para você, independentemente de onde estão localizados.
...
A maioria dos pipelines nf-core usa esse padrão.

### O que vem a seguir?

Agora que cobrimos as entradas, vamos explorar como configurar pipelines Nextflow para diferentes ambientes de computação.
