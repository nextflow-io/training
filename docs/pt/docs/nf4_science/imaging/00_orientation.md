...
Esta orientação assume que você já abriu o ambiente de treinamento clicando no botão "Open in GitHub Codespaces".
Se não o fez, faça isso agora, idealmente em uma segunda janela ou aba do navegador para que você possa consultar estas instruções.
...
No entanto, você precisa de uma conta GitHub (gratuita) para fazer login e, se não estiver familiarizado com a interface, deve dedicar alguns minutos para se familiarizar com ela completando o mini-curso [Orientação do GitHub Codespaces](../../envsetup/index.md).
...
Você pode continuar com o resto da orientação enquanto isso é executado.
...
Mude para este diretório agora executando este comando no terminal:
...
!!!tip
    ...
    Se por algum motivo você sair deste diretório, sempre pode usar o caminho completo para retornar a ele, assumindo que você está executando isso dentro do ambiente de treinamento GitHub Codespaces:
