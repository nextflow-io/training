...
Antes de executarmos o pipeline, vamos entender o que é o nf-core e por que isso é importante para executar fluxos de trabalho.

### 1.1. O que é nf-core?

[nf-core](https://nf-co.re/) é uma coleção orientada pela comunidade de pipelines Nextflow de alta qualidade.
Todos os pipelines nf-core seguem a mesma estrutura e convenções, o que significa que, uma vez que você aprende a executar um, pode executar qualquer um deles.

Características principais dos pipelines nf-core:
...
!!! tip "Quer aprender mais sobre nf-core?"
    ...
    Ele cobre como criar e personalizar pipelines nf-core do zero.

### 1.2. O pipeline molkart

![Pipeline nf-core/molkart](img/molkart.png)

O pipeline [nf-core/molkart](https://nf-co.re/molkart) processa dados de imagem de transcriptômica espacial através de várias etapas:

1. **Pré-processamento de imagem**: Preenchimento de padrão de grade e realce de contraste opcional
2. **Segmentação celular**: Múltiplas opções de algoritmo (Cellpose, Mesmer, ilastik, Stardist)
3. **Atribuição de pontos**: Atribuir pontos de transcrição às células segmentadas
4. **Controle de qualidade**: Gerar relatórios de QC abrangentes

As principais saídas são:

- Tabelas de contagem célula-por-transcrição
...
Vamos tentar executar o pipeline usando o conjunto de dados de teste e parâmetros da configuração de teste do molkart:
...
- `--input`: Caminho para a planilha de amostras contendo metadados das amostras
- `--mindagap_tilesize`, `--mindagap_boxsize`, `--mindagap_loopnum`: Parâmetros para preenchimento de padrão de grade
- `--clahe_pyramid_tile`: Tamanho do kernel para realce de contraste
...
!!! Warning "Este comando falhará - isso é intencional!"
    ...
    Estamos deliberadamente executando isso sem contêineres para demonstrar por que eles são necessários.
...
2. Essas ferramentas (como `duplicate_finder.py`, `apply_clahe.dask.py`, etc.) não fazem parte das distribuições Linux padrão
...
No entanto, o Nextflow só usa esses contêineres se você disser a ele para fazer isso!
...
Mude `docker.enabled = false` para `docker.enabled = true`:
...
!!! Tip "Por que os contêineres são importantes"
    ...
    Para mais detalhes sobre contêineres no Nextflow, veja [Hello Containers](../../hello_nextflow/05_hello_containers.md) do treinamento Hello Nextflow.
...
- O pipeline mostra sua versão e logo
- Os parâmetros de configuração são exibidos
- Múltiplos processos são executados em paralelo (indicado por múltiplas linhas de processo)
- Os nomes dos processos incluem o caminho completo do módulo (por exemplo, `NFCORE_MOLKART:MOLKART:MINDAGAP_MINDAGAP`)
...
- **Progresso**: Porcentagem completa e contagem (por exemplo, `1 of 1 ✔`)
...
Por padrão, os pipelines nf-core escrevem saídas em um diretório especificado pelo parâmetro `outdir`, que definimos como `results/`.
...
Cada subdiretório contém saídas de uma etapa específica do pipeline:

- **mindagap/**: Imagens preenchidas com grade da etapa de pré-processamento MindaGap
- **clahe/**: Imagens com contraste realçado do pré-processamento CLAHE
...
- **spot2cell/**: Tabelas de contagem célula-por-transcrição
- **anndata/**: Objetos AnnData contendo matrizes célula-por-transcrição e coordenadas espaciais
- **molkartqc/**: Métricas de controle de qualidade para atribuição de pontos
- **multiqc/**: Relatório de controle de qualidade abrangente
...
Abra o relatório no explorador de arquivos e clique no botão "Show Preview" para vê-lo renderizado diretamente no VS Code.
...
### 3.3. Examinar as tabelas célula-por-transcrição

A saída científica mais importante é a tabela de contagem célula-por-transcrição.
Isso informa quantos de cada transcrição foram detectados em cada célula.
...
- `cellxgene_mem_only_cellpose.csv`: Tabela célula-por-transcrição usando segmentação Cellpose
- `cellxgene_mem_only_mesmer.csv`: Tabela célula-por-transcrição usando segmentação Mesmer
- `cellxgene_mem_only_stardist.csv`: Tabela célula-por-transcrição usando segmentação Stardist
...
Observe como o Nextflow é capaz de processar múltiplos métodos de segmentação em paralelo, facilitando a comparação de resultados.
...
- **pipeline_dag.html**: Grafo acíclico direcionado mostrando a estrutura do fluxo de trabalho
...
!!! Tip
    ...
    Esses relatórios são incrivelmente úteis para otimizar a alocação de recursos e solucionar problemas de desempenho.
...
Escolha um dos hashes de processo de segmentação da saída do console (por exemplo, `[3m/4n5o6p]`) e olhe dentro:
...
- **Arquivos .command.\***: Scripts de execução e logs do Nextflow (como antes)
...
A principal diferença do Hello World:

- Pipelines reais preparam arquivos de entrada grandes (imagens, dados de referência)
- Os arquivos de saída podem ser bastante grandes (máscaras de segmentação, imagens processadas)
...
2. Se as entradas, código e parâmetros forem idênticos, reutiliza o resultado em cache
3. Apenas reexecuta tarefas que mudaram ou falharam

Isso é essencial para pipelines de longa duração onde falhas podem ocorrer tarde na execução.

### 5.2. Tentar retomar com molkart
...
Você deve ver uma saída como: <!-- TODO: saída completa -->
...
Observe `cached: 2` ou `cached: 1` para cada processo - nada foi reexecutado!

### 5.3. Quando retomar é útil

Retomar é particularmente valioso quando:

- Um pipeline falha devido a limites de recursos (falta de memória, limite de tempo excedido)
- Você precisa modificar processos downstream sem reexecutar etapas upstream
- Sua conexão de rede cai durante o download de dados
- Você quer adicionar saídas adicionais sem refazer a computação

!!! Warning

    Retomar só funciona se você não mudou os dados de entrada, código do pipeline ou parâmetros.
    Se você mudar qualquer um desses, o Nextflow corretamente reexecutará as tarefas afetadas.

### Conclusão

Você sabe como usar `-resume` para reexecutar pipelines de forma eficiente sem repetir tarefas bem-sucedidas.

### O que vem a seguir?

Agora que você pode executar nf-core/molkart com dados de teste, está pronto para aprender como configurá-lo para seus próprios conjuntos de dados.
