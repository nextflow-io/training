...
Nesta primeira parte do curso de treinamento Nextflow para Bioimagem, usaremos um exemplo muito básico de Hello World, independente de domínio, para demonstrar operações essenciais e apontar os componentes de código Nextflow correspondentes.
...
Se você olhar seu diretório atual, verá que quando executou o fluxo de trabalho, o Nextflow criou um novo diretório chamado `results`, que contém um arquivo chamado `output.txt`.
...
!!! warning "Aviso"
    ...
    Nem todos os fluxos de trabalho estarão configurados para publicar saídas em um diretório de resultados, e/ou o nome do diretório pode ser diferente.
    Um pouco mais adiante nesta seção, mostraremos como descobrir onde esse comportamento está especificado.

### 1.3. Encontrar a saída original e os logs no diretório `work/`
...
Essa é uma forma truncada do caminho do diretório de tarefa para aquela chamada de processo, e informa onde encontrar a saída da chamada do processo `sayHello` dentro do caminho do diretório `work/`.
...
Isso deve produzir o caminho completo do diretório: `work/a3/7be2fa7be2fad5e71e5f49998f795677fd68`
...
Observe como as saídas e logs dos diretórios de tarefa isolados são preservados, enquanto o conteúdo do diretório `results` é sobrescrito pela saída de execuções subsequentes.
...
_O objetivo aqui não é memorizar a sintaxe do código Nextflow, mas formar alguma intuição básica sobre quais são os principais componentes e como eles são organizados._
...
!!! Tip "Dica"
    ...
    Isso é necessário para verificar se o comando foi executado com sucesso e para passar a saída para processos downstream, se necessário.
    Saídas produzidas que não correspondem ao que está declarado no bloco de saída não serão passadas para processos downstream.

Em um pipeline do mundo real, um processo geralmente contém informações adicionais, como diretivas de processo, que apresentaremos daqui a pouco.

### 2.3. A definição do `workflow`
...
O `params.greeting` que fornecemos à chamada do processo `sayHello()` é um pedaço interessante de código Nextflow e vale a pena gastar um minuto extra nisso.
...
Às vezes, você vai querer executar novamente um pipeline que já lançou anteriormente sem refazer nenhum trabalho que já foi concluído com sucesso.
...
- Se você está executando um pipeline em produção e algo dá errado, em muitos casos você pode corrigir o problema e relançar o pipeline, e ele retomará a execução a partir do ponto de falha, o que pode economizar muito tempo e computação.
...
Procure pelo bit `cached:` que foi adicionado na linha de status do processo (linha 5), o que significa que o Nextflow reconheceu que já fez este trabalho e simplesmente reutilizou o resultado da execução bem-sucedida anterior.
...
!!! Tip "Dica"
    ...
    Quando você executa novamente um pipeline com `resume`, o Nextflow não sobrescreve nenhum arquivo escrito em um diretório `publishDir` por qualquer chamada de processo que foi executada anteriormente com sucesso.
...
Isso produzirá o conteúdo do arquivo de log no terminal, mostrando o timestamp, nome da execução, status e linha de comando completa para cada execução Nextflow que foi iniciada de dentro do diretório de trabalho atual.

### 3.3. Excluir diretórios de trabalho mais antigos

Durante o processo de desenvolvimento, você normalmente executará seus rascunhos de pipelines um grande número de vezes, o que pode levar a um acúmulo de muitos arquivos em muitos subdiretórios.
...
!!! Warning "Aviso"
    ...
    Você é responsável por salvar quaisquer saídas que você se importa ou planeja confiar! Se você está usando a diretiva `publishDir` para esse propósito, certifique-se de usar o modo `copy`, não o modo `symlink`.
...
Agora que você entende as operações básicas do Nextflow, está pronto para executar um pipeline de bioimagem real com nf-core/molkart.
