# Metadados e mapas de metadados
...
Pense nisso como um catálogo de biblioteca: enquanto os livros contêm o conteúdo real (dados brutos), as fichas do catálogo fornecem informações essenciais sobre cada livro—quando foi publicado, quem escreveu, onde encontrá-lo (metadados).
Em pipelines Nextflow, metadados podem ser usados para:
...
Começando com uma planilha de dados simples (frequentemente chamada de samplesheet em bioinformática) contendo informações básicas de arquivos, você aprenderá como:
...
Essas habilidades ajudarão você a construir pipelines mais robustos e flexíveis que podem lidar com relacionamentos complexos de arquivos e requisitos de processamento.
...
4. **Organizar** saídas por grupo de idiomas

Isso representa um padrão típico de fluxo de trabalho onde metadados específicos de arquivos orientam decisões de processamento; exatamente o tipo de problema que mapas de metadados resolvem elegantemente.
...
Precisamos escolher um operador que analisará o conteúdo do arquivo apropriadamente com o mínimo de esforço de nossa parte.
Como nossa planilha de dados está em formato CSV, este é um trabalho para o operador [`splitCsv`](https://www.nextflow.io/docs/latest/reference/operator.html#splitcsv), que carrega cada linha no arquivo como um elemento no canal.

Faça as seguintes alterações para adicionar uma operação `splitCsv()` ao código de construção do canal, mais uma operação `view()` para verificar se o conteúdo do arquivo está sendo carregado no canal corretamente.
...
Note que estamos usando a opção `header: true` para dizer ao Nextflow para ler a primeira linha do arquivo CSV como a linha de cabeçalho.
...
??? info "(Opcional) Mais sobre mapas"
    ...
    Em Groovy, a linguagem de programação na qual o Nextflow é construído, um mapa é uma estrutura de dados chave-valor semelhante a dicionários em Python, objetos em JavaScript ou hashes em Ruby.
...
### 1.3. Organizar os metadados em um 'mapa de metadados'
...
A consequência prática é que todo processo que consome este canal precisaria ser configurado com esta estrutura em mente:
...
Também torna o processo difícil de compartilhar com outros que podem ter dados de entrada ligeiramente diferentes, e você pode acabar tendo que codificar variáveis no processo que não são necessárias pelo bloco de script.

Para evitar este problema, precisamos encontrar uma maneira de manter a estrutura do canal consistente independentemente de quantas colunas a planilha de dados contém.
...
Agora, cada elemento no canal contém o mapa de metadados primeiro e o objeto de arquivo correspondente em segundo:
...
- **Por que metadados são importantes:** Manter metadados com seus dados preserva informações importantes de arquivos ao longo do fluxo de trabalho.
...
A ferramenta vem pré-treinada em um conjunto de idiomas e, dado um trecho de texto, ela produzirá uma previsão de idioma e uma pontuação de probabilidade associada, ambos para `stdout`.

### 2.1. Importar o processo e examinar o código

Fornecemos um módulo de processo pré-escrito chamado `IDENTIFY_LANGUAGE` que envolve a ferramenta `langid`, então você só precisa adicionar uma declaração de inclusão antes do bloco de fluxo de trabalho.
...
A definição de saída é estruturada como uma tupla com uma estrutura semelhante à da entrada, exceto que também contém `stdout` como um terceiro elemento.
...
Note que estamos usando o qualificador de saída [`stdout`](https://www.nextflow.io/docs/latest/process.html#outputs) do Nextflow aqui porque a ferramenta imprime sua saída diretamente no console em vez de escrever um arquivo; e usamos `sed` na linha de comando para remover a pontuação de probabilidade, limpar a string removendo caracteres de nova linha e retornar apenas a previsão de idioma.

### 2.2. Adicionar uma chamada para `IDENTIFY_LANGUAGE`

Agora que o processo está disponível para o fluxo de trabalho, podemos adicionar uma chamada ao processo `IDENTIFY_LANGUAGE` para executá-lo nos dados do canal.
...
Note que removemos a operação `.view()` original na construção do canal.

Podemos agora executar o fluxo de trabalho:
...
E como observado anteriormente, também incluímos o arquivo de entrada e o mapa meta na saída, o que significa que ambos permanecem associados à nova informação que acabamos de produzir.
Isso será útil no próximo passo.

!!! note

    De forma mais geral, este padrão de manter o mapa meta associado aos resultados facilita a associação de resultados relacionados que compartilham os mesmos identificadores.

    Como você já deve ter aprendido, você não pode confiar na ordem dos itens nos canais para corresponder resultados entre eles.
    Em vez disso, você deve usar chaves para associar dados corretamente, e mapas meta fornecem uma estrutura ideal para este propósito.

    Exploramos este caso de uso em detalhes na missão secundária [Splitting & Grouping](./splitting_and_grouping.md).

### 2.3. Aumentar metadados com saídas de processos

Dado que os resultados que acabamos de produzir são em si uma forma de metadados sobre o conteúdo dos arquivos, seria útil adicioná-los ao nosso mapa meta.

No entanto, não queremos modificar o mapa meta existente no local.
De um ponto de vista técnico, é _possível_ fazer isso, mas não é seguro.
...
Se você ainda não está familiarizado com o operador `+`, ou se isso parece confuso, reserve alguns minutos para passar pela explicação detalhada abaixo.
...
??? info "Criação do novo mapa meta usando o operador `+`"
    ...
    **Mas e se você precisar adicionar um campo que ainda não faz parte de um mapa?**
    ...
    Em vez disso, ela está contida em uma variável chamada `lang_id`, e você sabe que quer armazenar seu valor (`'fr'`) com a chave `lang`.
    ...
    Aqui, `[lang: new_info]` cria um novo mapa sem nome instantaneamente, e `map1 + ` mescla `map1` com o novo mapa sem nome, produzindo o mesmo conteúdo de `new_map` de antes.
    ...
    **Agora vamos transpor isso para o contexto de uma operação `channel.map()` do Nextflow.**
    ...
    A saída é um único mapa sem nome com o mesmo conteúdo que `new_map` em nosso exemplo acima.
    ...
    Esperançosamente você pode ver que se mudarmos `map1` para `meta`, isso é basicamente tudo que precisamos para adicionar a previsão de idioma ao nosso mapa meta em nosso fluxo de trabalho.

    Exceto por uma coisa!

    No caso do nosso fluxo de trabalho, **também precisamos considerar a presença do objeto `file` na tupla**, que é composta de `meta, file, lang_id`.
    ...
    Isso deve deixar mais claro que estamos simplesmente deixando o `file` em sua posição original em segunda posição na tupla. Apenas pegamos o valor `new_info` e o incorporamos ao mapa que está em primeira posição.
...
Sim, isso confere!
Reorganizamos cuidadosamente a saída do processo de `meta, file, lang_id` para que `lang_id` agora seja uma das chaves no mapa meta, e as tuplas do canal se encaixem no modelo `meta, file` mais uma vez.
...
Agora que temos nossas previsões de idioma, vamos usar a informação para atribuir alguns novos agrupamentos.
...
Pode ser útil ter essa classificação prontamente disponível em algum lugar mais tarde no pipeline, então vamos adicionar essa informação no mapa meta.
...
Você pode ver que isso é muito semelhante à operação de mesclagem de mapa instantânea que usamos no passo anterior.
...
Tente escrever você mesmo se já sabe como escrever declarações condicionais em Nextflow.
...
Uma vez que tudo faça sentido, execute o fluxo de trabalho novamente para ver o resultado:
...
- **Aplicar metadados de entrada a canais de saída**: Copiar metadados desta forma nos permite associar resultados posteriormente com base no conteúdo dos metadados.
...
Isso permite que você associe metadados novos e existentes com arquivos conforme progride pelo seu pipeline.
Mesmo que você não esteja usando metadados como parte de um processo, manter o mapa meta associado aos dados desta forma facilita manter todas as informações relevantes juntas.
...
??? info "O que o `cowpy` faz?"
    ...
    `cowpy` é uma ferramenta de linha de comando que gera arte ASCII para exibir entradas de texto arbitrárias de uma forma divertida.
    É uma implementação em python da ferramenta clássica [cowsay](https://en.wikipedia.org/wiki/Cowsay) de Tony Monroe.
...
Se você trabalhou no curso Hello Nextflow, já viu esta ferramenta em ação.
Se não, não se preocupe; cobriremos tudo que você precisa saber conforme avançamos.

### 3.1. Importar o processo e examinar o código

Fornecemos um módulo de processo pré-escrito chamado `COWPY` que envolve a ferramenta `cowpy`, então você só precisa adicionar uma declaração de inclusão antes do bloco de fluxo de trabalho.
...
Boas notícias: temos uma coluna `character` em nossa planilha de dados e, portanto, em nosso mapa meta.
...
1. Dar um nome ao canal de saída vindo do processo anterior para que possamos operar nele mais convenientemente.
2. Determinar como acessar a informação de interesse
3. Adicionar uma chamada ao segundo processo e alimentar a informação apropriadamente.
...
Para alimentar o conteúdo desse canal ao próximo processo (e fazer isso de uma forma que seja clara e fácil de ler) queremos dar a ele seu próprio nome, `ch_languages`.
...
Como frequentemente é o caso, a maneira mais simples de fazer isso é usar uma operação `map`.
...
Note que estamos usando closures (como `{ file -> "File: " + file }`) para tornar a saída das operações `.view` mais legível.
...
_Os caminhos de arquivo e valores de personagem podem sair em uma ordem diferente em sua saída._
...
Você vê que simplesmente copiamos as duas operações map (menos as declarações `.view()`) como as entradas para a chamada do processo.
...
Se você olhar no diretório de resultados, deve ver os arquivos individuais contendo a arte ASCII de cada saudação falada pelo personagem correspondente.
...
Isso mostra que conseguimos usar a informação no mapa meta para parametrizar o comando no segundo passo do pipeline.

No entanto, como observado acima, parte do código envolvido foi um pouco desajeitado, já que tivemos que desempacotar metadados ainda no contexto do corpo do fluxo de trabalho.
...
Há outro operador chamado `multiMap()` que nos permite simplificar isso um pouco, mas mesmo assim não é ideal.
...
??? info "(Opcional) Versão alternativa com `multiMap()`"
    ...
    Tecnicamente há outra maneira de fazer isso através de uma única operação de mapeamento, usando o operador `multiMap()`, que é capaz de emitir múltiplos canais.
    Por exemplo, você poderia substituir a chamada para `COWPY` acima com o seguinte código:
...
Em qualquer caso, é estranho que tenhamos que fazer algum desempacotamento no nível do fluxo de trabalho.

Seria melhor se pudéssemos alimentar o mapa meta inteiro no processo e escolher o que precisamos uma vez lá.
...
Note que não atualizamos a definição de saída do processo para produzir o mapa meta, a fim de manter o tutorial simplificado, mas sinta-se livre para fazer isso você mesmo como um exercício seguindo o modelo do processo `IDENTIFY_LANGUAGE`.

#### 3.3.2. Atualizar o comando para usar o campo do mapa meta

O mapa meta inteiro está agora disponível dentro do processo, então podemos nos referir à informação que ele contém diretamente de dentro do bloco de comando.
...
Substituímos a referência ao valor `character` anteriormente passado como uma entrada independente pelo valor mantido no mapa meta, ao qual nos referimos usando `meta.character`.
...
Vamos deletar os resultados da execução anterior e executá-lo:
...
Então isso produz os mesmos resultados de antes com código mais simples.

Claro, isso assume que você é capaz de modificar o código do processo.
Em alguns casos, você pode ter que confiar em processos existentes que você não tem liberdade para modificar, o que limita suas opções.
...
### 3.4. Solução de problemas de entradas obrigatórias ausentes

O valor `character` é obrigatório para o processo `COWPY` ser executado com sucesso.
...
Digamos que deletamos o valor de character para uma das entradas em nossa planilha de dados para simular um erro de coleta de dados:
...
Para qualquer versão do fluxo de trabalho que usamos acima, a chave `character` será criada para todas as entradas quando a planilha de dados for lida, mas para `sampleA` o valor será uma string vazia.
...
Quando o Nextflow executa a linha de comando `cowpy` para aquela amostra, `${meta.character}` é preenchido com uma string vazia na linha de comando `cowpy`, então a ferramenta `cowpy` lança um erro dizendo que nenhum valor foi fornecido para o argumento `-c`.

#### 3.4.2. A coluna character não existe na planilha de dados

Agora digamos que deletamos a coluna `character` inteiramente de nossa planilha de dados:
...
Neste caso a chave `character` não será criada de forma alguma quando a planilha de dados for lida.
...
Isso causará um erro, mas um diferente comparado ao primeiro caso.
...
Isso acontece porque `meta.character` não existe, então nossa tentativa de acessá-lo retorna `null`. Como resultado, o Nextflow literalmente conecta `null` na linha de comando, que é claro não é reconhecido pela ferramenta `cowpy`.
...
1. Implementar validação de entrada em seu fluxo de trabalho para garantir que a planilha de dados contenha todas as informações necessárias. Você pode encontrar uma [introdução à validação de entrada](../hello_nf-core/05_input_validation.md) no curso de treinamento Hello nf-core. <!-- TODO (future) pending a proper Validation side quest -->

2. Se você quiser ter certeza de que qualquer pessoa que use seu módulo de processo possa identificar imediatamente as entradas obrigatórias, você também pode tornar a propriedade de metadados obrigatória uma entrada explícita.
...
Então, no nível do fluxo de trabalho, use uma operação de mapeamento para extrair a propriedade `character` dos metadados e torná-la um componente explícito da tupla de entrada:
...
Esta abordagem tem a vantagem de mostrar explicitamente que `character` é obrigatório, e torna o processo mais fácil de reimplantar em outros contextos.

Isso destaca um princípio de design importante:

**Use o mapa meta para informações opcionais e descritivas, mas extraia valores obrigatórios como entradas explícitas.**

O mapa meta é excelente para manter estruturas de canal limpas e prevenir estruturas de canal arbitrárias, mas para elementos obrigatórios que são diretamente referenciados em um processo, extraí-los como entradas explícitas cria código mais robusto e sustentável.

### Conclusão

Nesta seção, você aprendeu como utilizar metadados para personalizar a execução de um processo, acessando-os seja no nível do fluxo de trabalho ou no nível do processo.

---

## Exercício suplementar

Se você gostaria de praticar o uso de informações do mapa meta de dentro de um processo, tente usar outras peças de informação do mapa meta como `lang` e `lang_group` para personalizar como as saídas são nomeadas e/ou organizadas.
...
Este padrão de manter metadados explícitos e anexados aos dados é uma prática recomendada central no Nextflow, oferecendo várias vantagens sobre codificar informações de arquivo:

- Metadados de arquivo permanecem associados aos arquivos ao longo do fluxo de trabalho
- O comportamento do processo pode ser personalizado por arquivo
- A organização de saída pode refletir metadados de arquivo
- Informações de arquivo podem ser expandidas durante a execução do pipeline

Aplicar este padrão em seu próprio trabalho permitirá que você construa fluxos de trabalho de bioinformática robustos e sustentáveis.

### Padrões-chave

1.  **Lendo e Estruturando Metadados:** Lendo arquivos CSV e criando mapas de metadados organizados que permanecem associados aos seus arquivos de dados.
...
2.  **Expandindo Metadados Durante o Fluxo de Trabalho** Adicionando novas informações aos seus metadados conforme seu pipeline progride, adicionando saídas de processos e derivando valores através de lógica condicional.

    - Adicionando novas chaves baseadas na saída do processo
...
3.  **Personalizando Comportamento do Processo:** Usando metadados dentro do processo.
...
Retorne ao [menu de Missões Secundárias](./index.md) ou clique no botão no canto inferior direito da página para passar para o próximo tópico na lista.
