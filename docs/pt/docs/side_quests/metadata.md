...
Se você olhar no diretório de resultados, você deve ver as mesmas saídas de antes, _ou seja_, arquivos individuais contendo a arte ASCII de cada saudação falada pelo personagem correspondente.
...
Quando o Nextflow executa a linha de comando `cowpy` para essa amostra, `${meta.character}` é preenchido com uma string vazia na linha de comando `cowpy`, então a ferramenta `cowpy` lança um erro dizendo que nenhum valor foi fornecido para o argumento `-c`.
...
Até onde o Nextflow está preocupado, este fluxo de trabalho foi executado com sucesso!
...
Isso acontece porque `meta.character` não existe, então nossa tentativa de acessá-lo retorna `null`. Como resultado, o Nextflow literalmente insere `null` na linha de comando, que é claro não é reconhecido pela ferramenta `cowpy`.
...
1. Implementar validação de entrada em seu fluxo de trabalho para garantir que a planilha de dados contenha todas as informações necessárias. Você pode encontrar uma [introdução à validação de entrada](../hello_nf-core/05_input_validation.md) no curso de treinamento Hello nf-core.
...
O mapa meta é excelente para manter as estruturas de canal limpas e prevenir estruturas de canal arbitrárias, mas para elementos obrigatórios que são diretamente referenciados em um processo, extraí-los como entradas explícitas cria código mais robusto e sustentável.
...
2.  **Expandindo Metadados Durante o Fluxo de Trabalho** Adicionando novas informações aos seus metadados conforme seu pipeline progride, adicionando saídas de processo e derivando valores através de lógica condicional.

    - Adicionando novas chaves baseadas em saída de processo
...
3.  **Personalizando Comportamento de Processo:** Usando metadados dentro do processo.
