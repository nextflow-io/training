...
Vamos mostrar como usar essas ferramentas para criar um esqueleto de pipeline e depois adaptar o código de pipeline 'regular' existente para o esqueleto nf-core.
...
Primeiro, criamos o esqueleto para o novo pipeline.
...
Isso criará um novo esqueleto de pipeline usando o template base nf-core, personalizado com um nome de pipeline, descrição e autor.
...
Esta TUI pedirá que você forneça informações básicas sobre seu pipeline e oferecerá uma escolha de recursos para incluir ou excluir no esqueleto do seu pipeline.

- Na tela de boas-vindas, clique em **Let's go!**.
- Na tela `Choose pipeline type`, clique em **Custom**.
- Insira os detalhes do seu pipeline da seguinte forma (substituindo `< SEU NOME >` pelo seu próprio nome), depois clique em **Next**.
...
- Na tela Template features, defina `Toggle all features` como **off**, depois **habilite** seletivamente os seguintes. Verifique suas seleções e clique em **Continue**.
...
- Na tela `Final details`, clique em **Finish**. Aguarde a criação do pipeline, depois clique em **Continue**.
...
!!! note "Nota"
    ...
    Isso ocorre porque não optamos por incluir nenhum dos módulos padrão do nf-core.

### 1.2. Testar se o esqueleto é funcional

Acredite ou não, mesmo que você ainda não tenha adicionado nenhum módulo para fazer trabalho real, o esqueleto do pipeline pode realmente ser executado usando o perfil de teste, da mesma forma que executamos o pipeline `nf-core/demo`.
...
Isso mostra que toda a estrutura básica está no lugar.
...
![relatório de linha do tempo de execução vazio](./img/execution_timeline_empty.png)
...
Comparado a um fluxo de trabalho básico do Nextflow como o desenvolvido em [Hello Nextflow](../hello_nextflow/index.md), você notará algumas coisas que são novas aqui (linhas destacadas acima):
...
Você agora sabe como criar um esqueleto de pipeline usando as ferramentas nf-core.
...
Agora é hora de começar a trabalhar integrando nosso fluxo de trabalho no esqueleto nf-core.
Como lembrete, estamos trabalhando com o fluxo de trabalho apresentado em nosso curso de treinamento [Hello Nextflow](../hello_nextflow/index.md).

!!! tip "Dica"

    Se você não está familiarizado com esse pipeline ou precisa de uma revisão, consulte [O pipeline Hello](../info/hello_pipeline.md).
...
Sinta-se à vontade para executá-lo para se certificar de que funciona:
...
Vamos abrir o arquivo de fluxo de trabalho `hello.nf` para inspecionar o código, que é mostrado na íntegra abaixo (sem contar os processos, que estão em módulos):
...
Vamos percorrer as mudanças necessárias uma por uma.

### 2.1. Nomear o fluxo de trabalho

Primeiro, vamos dar um nome ao fluxo de trabalho para que possamos nos referir a ele a partir de um fluxo de trabalho pai.
...
As mesmas convenções se aplicam aos nomes de fluxos de trabalho como aos nomes de módulos.

### 2.2. Substituir a construção de canal por `take`

Agora, substitua a construção de canal por uma simples declaração `take` declarando as entradas esperadas.
...
Em seguida, adicione uma declaração `main` antes do restante das operações chamadas no corpo do fluxo de trabalho.
...
Isso basicamente diz 'isso é o que este fluxo de trabalho _faz_'.
...
Esta é uma adição totalmente nova ao código em comparação com o fluxo de trabalho original.

### 2.5. Recapitulação das mudanças concluídas

Se você fez todas as mudanças conforme descrito, seu fluxo de trabalho agora deve se parecer com isto:
...
### 2.6. Fazer um fluxo de trabalho entrypoint dummy

Antes de integrar nosso fluxo de trabalho componível no complexo esqueleto nf-core, vamos verificar se ele funciona corretamente.
Podemos fazer um fluxo de trabalho entrypoint dummy simples para testar o fluxo de trabalho componível isoladamente.
...
// importa o código do fluxo de trabalho do arquivo hello.nf
include { HELLO } from './hello.nf'

// declara o parâmetro de entrada
params.greeting = 'greetings.csv'
...
workflow {
  ...
  // chama o fluxo de trabalho importado no canal de saudações
  HELLO(greeting_ch)

  // visualiza as saídas emitidas pelo fluxo de trabalho
...
- Tudo que está relacionado a puxar as entradas para o fluxo de trabalho (parâmetro de entrada e construção de canal) agora está declarado neste fluxo de trabalho pai.
...
### 2.7. Testar se o fluxo de trabalho executa

Finalmente temos todas as peças que precisamos para verificar se o fluxo de trabalho componível funciona.
Vamos executá-lo!
...
Se tivéssemos nomeado o fluxo de trabalho entrypoint `something_else.nf`, teríamos que fazer `nextflow run original-hello/something_else.nf`.
...
Aprenda como enxertar um fluxo de trabalho componível básico no esqueleto nf-core.

---

## 3. Ajustar a lógica do fluxo de trabalho atualizado no fluxo de trabalho placeholder

Agora que verificamos que nosso fluxo de trabalho componível funciona corretamente, vamos retornar ao esqueleto do pipeline nf-core que criamos na seção 1.
...
Então, como fazemos isso acontecer? Vamos dar uma olhada no conteúdo atual do fluxo de trabalho `HELLO` em `core-hello/workflows/hello.nf` (o esqueleto nf-core).
...
No geral, este código faz muito pouco além de alguma manutenção que tem a ver com capturar a versão de quaisquer ferramentas de software que sejam executadas no pipeline.
...
1. Copiar os módulos e configurar as importações de módulos
...
!!! note "Nota"
    ...
    Vamos ignorar a captura de versão por enquanto e veremos como conectar isso em uma parte posterior deste treinamento.

### 3.1. Copiar os módulos e configurar as importações de módulos

Os quatro processos do nosso fluxo de trabalho Hello Nextflow estão armazenados como módulos em `original-hello/modules/`.
Precisamos copiar esses módulos para a estrutura do projeto nf-core (sob `core-hello/modules/local/`) e adicionar declarações de importação ao arquivo de fluxo de trabalho nf-core.
...
Você agora deve ver o diretório de módulos listado sob `core-hello/`.
...
O tratamento de entrada será feito upstream deste fluxo de trabalho (não neste arquivo de código).
...
Já existe algum código lá que tem a ver com capturar as versões das ferramentas que são executadas pelo fluxo de trabalho. Vamos deixar isso em paz por enquanto (lidaremos com as versões das ferramentas mais tarde).
Vamos manter a inicialização `ch_versions = channel.empty()` no topo, depois inserir nossa lógica de fluxo de trabalho, mantendo o código de coleta de versão no final.
Esta ordenação faz sentido porque em um pipeline real, os processos emitiriam informações de versão que seriam adicionadas ao canal `ch_versions` conforme o fluxo de trabalho é executado.
...
=== "Depois"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26"
        // emite uma saudação (atualizado para usar a convenção nf-core para samplesheets)
...
Aprenda como adaptar o tratamento de entradas no esqueleto do pipeline nf-core.

---

## 4. Adaptar o tratamento de entrada

Agora que integramos com sucesso nossa lógica de fluxo de trabalho no esqueleto nf-core, precisamos abordar mais uma peça crítica: garantir que nossos dados de entrada sejam processados corretamente.
O template nf-core vem com tratamento de entrada sofisticado projetado para conjuntos de dados genômicos complexos, então precisamos adaptá-lo para funcionar com nosso arquivo `greetings.csv` mais simples.

### 4.1. Identificar onde as entradas são tratadas

O primeiro passo é descobrir onde o tratamento de entrada é feito.

Você pode se lembrar de que quando reescrevemos o fluxo de trabalho Hello Nextflow para ser componível, movemos a declaração do parâmetro de entrada um nível acima, no fluxo de trabalho entrypoint `main.nf`.
Então vamos dar uma olhada no fluxo de trabalho entrypoint `main.nf` de nível superior que foi criado como parte do esqueleto do pipeline:
...
O projeto nf-core faz uso pesado de subfluxos de trabalho aninhados, então esta parte pode ser um pouco confusa na primeira abordagem.
...
- Um fluxo de trabalho sem nome que chama `CORE_HELLO` assim como dois outros subfluxos de trabalho, `PIPELINE_INITIALISATION` e `PIPELINE_COMPLETION`.

Aqui está um diagrama de como eles se relacionam entre si:

<figure class="excalidraw">
--8<-- "docs/en/docs/hello_nf-core/img/hello-nested-workflows.svg"
</figure>

Importante, não podemos encontrar nenhum código construindo um canal de entrada neste nível, apenas referências a um samplesheet fornecido via parâmetro `--input`.
...
Esta é a fábrica de canais que analisa o samplesheet e o passa em uma forma que está pronta para ser consumida pelo fluxo de trabalho HELLO.
...
Este código envolve algumas etapas de análise e validação que são altamente específicas para o samplesheet de exemplo incluído com o template do pipeline nf-core, que no momento da escrita é muito específico do domínio e não adequado para nosso projeto de pipeline simples.
...
Como lembrete, é assim que a construção do canal se parecia (como visto no diretório de soluções):
...
Em sua forma atual, isso não nos permitirá aproveitar as capacidades integradas do nf-core para validação de esquema, mas podemos adicionar isso mais tarde.
...
Falando em dados de teste e parâmetros, vamos atualizar o perfil de teste para este pipeline para usar o mini-samplesheet `greetings.csv` em vez do samplesheet de exemplo fornecido no template.

Sob `core-hello/conf`, encontramos dois perfis de teste do template: `test.config` e `test_full.config`, que são destinados a testar uma pequena amostra de dados e uma de tamanho completo.
Dado o propósito do nosso pipeline, não há realmente um ponto em configurar um perfil de teste de tamanho completo, então sinta-se à vontade para ignorar ou excluir `test_full.config`.
Vamos nos concentrar em configurar `test.config` para executar em nosso arquivo `greetings.csv` com alguns parâmetros padrão.

#### 4.3.1. Copiar o arquivo `greetings.csv`

Primeiro precisamos copiar o arquivo `greetings.csv` para um lugar apropriado em nosso projeto de pipeline.
...
- **Caminhos absolutos**: Ao usar `${projectDir}`, criamos um caminho absoluto, o que é importante para dados de teste que acompanham o pipeline.
- **Localização de dados de teste**: Os pipelines nf-core normalmente armazenam dados de teste no diretório `assets/` dentro do repositório do pipeline para arquivos de teste pequenos, ou referenciam conjuntos de dados de teste externos para arquivos maiores.
...
Como você pode ver, isso produziu o resumo típico do nf-core no início graças ao subfluxo de trabalho de inicialização, e as linhas para cada módulo agora mostram os nomes completos PIPELINE:WORKFLOW:module.

### 4.5. Encontrar as saídas do pipeline

A questão agora é: onde estão as saídas do pipeline?
E a resposta é bastante interessante: agora existem dois lugares diferentes para procurar os resultados.

Como você pode se lembrar de antes, nossa primeira execução do fluxo de trabalho recém-criado produziu um diretório chamado `core-hello-results/` que continha vários relatórios de execução e metadados.
...
Desta vez você vê todas as tarefas que foram executadas conforme esperado.

![relatório de linha do tempo de execução para o pipeline Hello](./img/execution_timeline_hello.png)
...
Aqui está o que aconteceu: não mudamos nada nos próprios módulos, então as saídas tratadas pelas diretivas `publishDir` no nível do módulo ainda vão para um diretório `results` conforme especificado no pipeline original.
...
Ah, lá estão eles, misturados com as saídas de execuções anteriores do pipeline Hello original.

Se quisermos que eles sejam organizados de forma organizada como as saídas do pipeline demo estavam, precisaremos mudar como configuramos as saídas para serem publicadas.
...
E aí está! Pode parecer muito trabalho para realizar o mesmo resultado que o pipeline original, mas você obtém todos aqueles relatórios adoráveis gerados automaticamente, e agora tem uma base sólida para aproveitar recursos adicionais do nf-core, incluindo validação de entrada e algumas capacidades interessantes de tratamento de metadados que cobriremos em uma seção posterior.

---

### Conclusão

Você sabe como converter um pipeline Nextflow regular em um pipeline estilo nf-core usando o template nf-core.
Como parte disso, você aprendeu como tornar um fluxo de trabalho componível e como identificar os elementos do template nf-core que mais comumente precisam ser adaptados ao desenvolver um pipeline estilo nf-core personalizado.

### O que vem a seguir?

Faça uma pausa, foi um trabalho duro! Quando estiver pronto, passe para [Parte 3: Usar um módulo nf-core](./03_use_module.md) para aprender como aproveitar módulos mantidos pela comunidade do repositório nf-core/modules.
