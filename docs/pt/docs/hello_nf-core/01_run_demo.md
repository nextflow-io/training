...
Nesta primeira parte do curso de treinamento Hello nf-core, mostramos como encontrar e experimentar um pipeline nf-core, entender como o código é organizado e reconhecer como ele difere do código Nextflow simples, conforme mostrado em [Hello Nextflow](../hello_nextflow/index.md).

Vamos usar um pipeline chamado nf-core/demo que é mantido pelo projeto nf-core como parte de seu inventário de pipelines para demonstrar a estrutura do código e as operações das ferramentas.
...
Uma vez que determinamos que o pipeline parece ser adequado para nossos propósitos, vamos experimentá-lo.
Felizmente, o Nextflow facilita a recuperação de pipelines de repositórios formatados corretamente sem ter que baixar nada manualmente.
...
O Nextflow faz um `pull` do código do pipeline, ou seja, baixa o repositório completo para sua unidade local.

Para ser claro, você pode fazer isso com qualquer pipeline Nextflow que esteja configurado adequadamente no GitHub, não apenas pipelines nf-core.
No entanto, o nf-core é a maior coleção de código aberto de pipelines Nextflow.

Você pode fazer o Nextflow fornecer uma lista de quais pipelines você recuperou dessa forma:
...
!!! note
    ...
    O caminho completo pode diferir no seu sistema se você não estiver usando nosso ambiente de treinamento.
...
Agora podemos mais facilmente dar uma olhada no código-fonte conforme necessário.

Mas primeiro, vamos tentar executar nosso primeiro pipeline nf-core!

### Conclusão

Agora você sabe como encontrar um pipeline através do site nf-core e recuperar uma cópia local do código-fonte.

### O que vem a seguir?

Aprenda como experimentar um pipeline nf-core com o mínimo de esforço.
...
O perfil `test` para `nf-core/demo` está localizado no arquivo de configuração `conf/test.config` e é mostrado abaixo.
...
As únicas coisas que precisamos fornecer são o que é mostrado entre sinais de maior e menor no comando de exemplo: `<docker/singularity>` e `<OUTDIR>`.

Como lembrete, `<docker/singularity>` refere-se à escolha do sistema de contêiner. Todos os pipelines nf-core são projetados para serem utilizáveis com contêineres (Docker, Singularity, etc.) para garantir reprodutibilidade e eliminar problemas de instalação de software.
...
Se ele ainda não existir, o Nextflow o criará para nós em tempo de execução.

Passando para a seção após o bloco de comentário, o perfil de teste nos mostra o que foi pré-configurado para teste: mais notavelmente, o parâmetro `input` já está definido para apontar para um conjunto de dados de teste, então não precisamos fornecer nossos próprios dados.
...
Se sua saída corresponder a isso, parabéns! Você acabou de executar seu primeiro pipeline nf-core.
...
!!! note
    ...
    Sua saída mostrará diferentes carimbos de data/hora, nomes de execução e caminhos de arquivo, mas a estrutura geral e a execução do processo devem ser semelhantes.
...
Isso nos diz que três processos foram executados, correspondendo às três ferramentas mostradas na página de documentação do pipeline no site nf-core: FASTQC, SEQTK_TRIM e MULTIQC.

Os nomes completos dos processos conforme mostrados aqui, como `NFCORE_DEMO:DEMO:MULTIQC`, são mais longos do que o que você pode ter visto no material introdutório Hello Nextflow.
Estes incluem os nomes de seus fluxos de trabalho pai e refletem a modularidade do código do pipeline.
Entraremos em mais detalhes sobre isso daqui a pouco.
...
Nesta etapa, o que é importante observar é que os resultados são organizados por módulo, e há adicionalmente um diretório chamado `pipeline_info` contendo vários relatórios com carimbo de data/hora sobre a execução do pipeline.
...
Agora que executamos com sucesso o pipeline como usuários, vamos mudar nossa perspectiva para ver como os pipelines nf-core são estruturados internamente.
...
Entender como tudo isso é organizado é o primeiro passo para desenvolver seus próprios pipelines compatíveis com nf-core, o que abordaremos na Parte 2 deste curso.

Vamos dar uma olhada em como o código do pipeline é organizado no repositório `nf-core/demo`, usando o link simbólico `pipelines` que criamos anteriormente.
...
Primeiro, vamos notar que no nível superior, você pode encontrar um arquivo README com informações resumidas, bem como arquivos acessórios que resumem informações do projeto, como licenciamento, diretrizes de contribuição, citação e código de conduta.
A documentação detalhada do pipeline está localizada no diretório `docs`.
Todo esse conteúdo é usado para gerar as páginas web no site nf-core programaticamente, então elas estão sempre atualizadas com o código.

Agora, para o resto, vamos dividir nossa exploração em três etapas:

1. Componentes do código do pipeline (`main.nf`, `workflows`, `subworkflows`, `modules`)
2. Configuração do pipeline
3. Entradas e validação

Vamos começar com os componentes do código do pipeline.
Vamos nos concentrar na hierarquia de arquivos e organização estrutural, em vez de mergulhar no código dentro de arquivos individuais.

### 3.1. Componentes do código do pipeline

A organização padrão do código do pipeline nf-core segue uma estrutura modular que é projetada para maximizar a reutilização de código, conforme introduzido em [Hello Modules](../hello_nextflow/04_hello_modules.md), Parte 4 do curso [Hello Nextflow](../hello_nextflow/index.md), embora no verdadeiro estilo nf-core, isso seja implementado com um pouco de complexidade adicional.
Especificamente, os pipelines nf-core fazem uso abundante de subworkflows, ou seja, scripts de fluxo de trabalho que são importados por um fluxo de trabalho pai.

Isso pode soar um pouco abstrato, então vamos dar uma olhada em como isso é usado na prática no pipeline `nf-core/demo`.

!!! note

    Não vamos revisar o código real de _como_ esses componentes modulares são conectados, porque há alguma complexidade adicional associada ao uso de subworkflows que pode ser confusa, e entender isso não é necessário nesta etapa do treinamento.
    Por enquanto, vamos nos concentrar na organização geral e na lógica.

#### 3.1.1. Visão geral

Aqui está como as relações entre os componentes de código relevantes se parecem para o pipeline `nf-core/demo`:
...
!!! note
    ...
    Os subworkflows não estão limitados a funções de manutenção, e eles podem fazer uso de módulos de processo.

    O pipeline `nf-core/demo` mostrado aqui está no lado mais simples do espectro, mas outros pipelines nf-core (como `nf-core/rnaseq`) utilizam subworkflows que estão envolvidos na análise real.
...
Isso funciona para qualquer pipeline Nextflow que siga essa nomenclatura e estrutura convencionais, não apenas pipelines nf-core.

Usar um script de ponto de entrada facilita a execução de subworkflows de 'manutenção' padronizados antes e depois que o script de análise real seja executado.
Vamos revisar esses depois de termos revisado o fluxo de trabalho de análise real e seus módulos.
...
Cobriremos as diferenças relevantes na próxima parte deste curso, quando abordarmos a conversão do pipeline Hello simples do Hello Nextflow em uma forma compatível com nf-core.

O fluxo de trabalho `demo.nf` chama **módulos** localizados em `modules/`, que revisaremos a seguir.

!!! note

    Alguns fluxos de trabalho de análise nf-core exibem níveis adicionais de aninhamento ao chamar subworkflows de nível inferior.
    ...
    Você pode ver alguns exemplos navegando pelos [subworkflows nf-core](https://nf-co.re/subworkflows/) disponíveis no site nf-core.

    Quando o script de análise usa subworkflows, esses são armazenados no diretório `subworkflows/`.
...
No nível superior, os módulos são diferenciados como `nf-core` ou `local` (não parte do projeto nf-core), e então colocados em um diretório nomeado após a(s) ferramenta(s) que eles envolvem.
Se a ferramenta pertence a um toolkit (ou seja, um pacote contendo várias ferramentas), então há um nível de diretório intermediário nomeado após o toolkit.
...
Aqui você vê que os módulos `fastqc` e `multiqc` ficam no nível superior dentro dos módulos `nf-core`, enquanto o módulo `trim` fica sob o toolkit ao qual pertence, `seqtk`.
Neste caso, não há módulos `local`.

O arquivo de código do módulo descrevendo o processo é sempre chamado `main.nf`, e é acompanhado por testes e arquivos `.yml` que vamos ignorar por enquanto.

Juntos, o fluxo de trabalho de ponto de entrada, fluxo de trabalho de análise e módulos são suficientes para executar as partes 'interessantes' do pipeline.
...
Como os módulos, os subworkflows são diferenciados em diretórios `local` e `nf-core`, e cada subworkflow tem sua própria estrutura de diretório aninhada com seu próprio script `main.nf`, testes e arquivo `.yml`.
...
Conforme observado acima, o pipeline `nf-core/demo` não inclui nenhum subworkflow específico de análise, então todos os subworkflows que vemos aqui são chamados de fluxos de trabalho de 'manutenção' ou 'utilitários', conforme indicado pelo prefixo `utils_` em seus nomes.
Esses subworkflows são o que produz o cabeçalho nf-core sofisticado na saída do console, entre outras funções acessórias.
...
O projeto nf-core aplica diretrizes altamente padronizadas para configuração de pipeline que visam construir sobre as opções de personalização flexíveis do Nextflow de uma forma que forneça maior consistência e manutenibilidade entre pipelines.
...
- `base.config`: Um arquivo de configuração 'em branco', apropriado para uso geral na maioria dos ambientes de computação de alto desempenho. Isso define amplas categorias de uso de recursos, por exemplo, que são convenientes para aplicar aos módulos.
- `modules.config`: Diretivas e argumentos adicionais de módulo.
...
Como observamos anteriormente, quando examinamos o perfil de teste do pipeline `nf-core/demo`, ele é projetado para receber como entrada um samplesheet contendo caminhos de arquivo e identificadores de amostra.
Os caminhos de arquivo vinculados a dados reais localizados no repositório `nf-core/test-datasets`.

Um exemplo de samplesheet também é fornecido no diretório `assets`, embora os caminhos neste não sejam reais.
...
Este samplesheet em particular é bastante simples, mas alguns pipelines são executados em samplesheets que são mais complexos, com muito mais metadados associados às entradas primárias.

Infelizmente, como esses arquivos podem ser difíceis de verificar visualmente, a formatação inadequada dos dados de entrada é uma fonte muito comum de falhas de pipeline.
...
Cobriremos este tópico com mais detalhes na Parte 5 deste curso.
...
Isso é usado para vários propósitos, incluindo validação automática de parâmetros, geração de texto de ajuda e renderização de formulário de parâmetro interativo em interfaces de UI.

O `schema_input.json` é um arquivo usado para definir a estrutura do samplesheet de entrada.
Cada coluna pode ter um tipo, padrão, descrição e texto de ajuda em um formato legível por máquina.
O esquema é usado para vários propósitos, incluindo validação automatizada e fornecimento de mensagens de erro úteis.

### Conclusão

Você sabe quais são os principais componentes de um pipeline nf-core e como o código é organizado; onde os principais elementos de configuração estão localizados; e você está ciente do que é a validação de entrada.

### O que vem a seguir?

Faça uma pausa! Isso foi muito. Quando você estiver se sentindo revigorado e pronto, passe para a próxima seção para aplicar o que aprendeu para escrever um pipeline compatível com nf-core.
