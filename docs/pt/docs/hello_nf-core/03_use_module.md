...
Nesta terceira parte do curso de treinamento Hello nf-core, mostramos como encontrar, instalar e usar um módulo nf-core existente em seu pipeline.
...
Para demonstrar como isso funciona, vamos substituir o módulo personalizado `collectGreetings` pelo módulo `cat/cat` do nf-core/modules no pipeline `core-hello`.

??? info "Como começar a partir desta seção"

    Esta seção do curso pressupõe que você completou a [Parte 2: Reescrever Hello para nf-core](./02_rewrite_hello.md) e tem um pipeline `core-hello` funcionando.

    Se você não completou a Parte 2 ou quer começar do zero para esta parte, pode usar a solução `core-hello-part2` como ponto de partida.
    Execute este comando de dentro do diretório `hello-nf-core/`:
...
Primeiro, vamos aprender como encontrar um módulo nf-core existente e instalá-lo em nosso pipeline.

Vamos buscar substituir o processo `collectGreetings`, que usa o comando Unix `cat` para concatenar vários arquivos de saudação em um só.
Concatenar arquivos é uma operação muito comum, então é razoável supor que já possa existir um módulo no nf-core projetado para esse propósito.
...
Navegue até a página de módulos em seu navegador web e use a barra de pesquisa para procurar por 'concatenate'.

![resultados da pesquisa de módulos](./img/module-search-results.png)

Como você pode ver, há vários resultados, muitos deles módulos projetados para concatenar tipos muito específicos de arquivos.
Entre eles, você deve ver um chamado `cat_cat` que é de propósito geral.

!!! note "Convenção de nomenclatura de módulos"

    O sublinhado (`_`) é usado como substituto do caractere barra (`/`) nos nomes dos módulos.
...
Alternativamente, você também pode procurar módulos diretamente pela linha de comando usando as ferramentas nf-core.

```bash
nf-core modules list remote
```

Isso exibirá uma lista de todos os módulos disponíveis no repositório nf-core/modules, embora seja um pouco menos conveniente se você ainda não conhece o nome do módulo que está procurando.
No entanto, se você conhece, pode canalizar a lista para `grep` para encontrar módulos específicos:
...
Apenas tenha em mente que a abordagem com `grep` só vai extrair resultados com o termo de pesquisa em seu nome, o que não funcionaria para `cat_cat`.

### 1.3. Obter informações detalhadas sobre o módulo

Para ver informações detalhadas sobre um módulo específico pela linha de comando, use o comando `info`:
...
A boa notícia é que o projeto nf-core inclui algumas ferramentas para tornar esta parte fácil.
Especificamente, o comando `nf-core modules install` torna possível automatizar a recuperação do código e disponibilizá-lo para seu projeto em uma única etapa.
...
Se for o caso, pressione enter para aceitar a resposta padrão (`Pipeline`) e continuar.

A ferramenta então oferecerá alterar a configuração do seu projeto para evitar este prompt no futuro.
...
Pode ser útil aproveitar esta ferramenta conveniente!
...
- Fornece a você a declaração `include` correta para usar em seu fluxo de trabalho

!!! tip

    Sempre certifique-se de que seu diretório de trabalho atual é a raiz do seu projeto de pipeline antes de executar o comando de instalação do módulo.
...
No entanto, para realmente usar o novo módulo, precisamos importá-lo para nosso pipeline.

### 1.5. Atualizar as importações de módulos

Vamos substituir a declaração `include` para o módulo `collectGreetings` pela do `CAT_CAT` na seção de importações do fluxo de trabalho `workflows/hello.nf`.

Como lembrete, a ferramenta de instalação do módulo nos deu a declaração exata para usar:

```groovy title="Declaração de importação produzida pelo comando install"
include { CAT_CAT } from '../modules/nf-core/cat/cat/main'`
```

Note que a convenção nf-core é usar maiúsculas para nomes de módulos ao importá-los.
...
- **Módulo local**: `'../modules/local/collectGreetings.nf'` (referência a arquivo único)

O módulo agora está disponível para o fluxo de trabalho, então tudo que precisamos fazer é trocar a chamada para `collectGreetings` para usar `CAT_CAT`. Certo?
...
Você sabe como encontrar um módulo nf-core e disponibilizá-lo para seu projeto.
...
(Para ser justo, existe um comando `uninstall` para se livrar de módulos que você decide que não quer mais.)

!!! note

    O processo CAT_CAT inclui um tratamento bastante inteligente de diferentes tipos de compressão, extensões de arquivo e assim por diante que não são estritamente relevantes para o que estamos tentando mostrar aqui, então vamos ignorar a maior parte disso e focar apenas nas partes que são importantes.

### 2.1. Comparar as interfaces dos dois módulos

Como lembrete, é assim que a interface do nosso módulo `collectGreetings` se parece:
...
Ao completar, `collectGreetings` produz um único caminho de arquivo, emitido com a tag `outfile`.
...
- `meta` é uma estrutura contendo metadados, chamada de metamap;
- `files_in` contém um ou mais arquivos de entrada para processar, equivalente ao `input_files` de `collectGreetings`.

Ao completar, CAT_CAT entrega suas saídas em duas partes:

- Outra tupla contendo o metamap e o arquivo de saída concatenado, emitido com a tag `file_out`;
- Um arquivo `versions.yml` que captura informações sobre a versão do software que foi usada, emitido com a tag `versions`.

Note também que, por padrão, o arquivo de saída será nomeado com base em um identificador que faz parte dos metadados (código não mostrado aqui).

Isso pode parecer muito para acompanhar apenas olhando o código, então aqui está um diagrama para ajudá-lo a visualizar como tudo se encaixa.
...
Ignorando o arquivo de versões por enquanto, sua saída principal também é equivalente (um arquivo concatenado), exceto que CAT_CAT também emite o metamap em conjunto com o arquivo de saída.

As diferenças de empacotamento serão razoavelmente fáceis de lidar, como você verá daqui a pouco.
No entanto, para entender a parte do metamap, precisamos apresentar algum contexto adicional.

### 2.2. Entendendo metamaps
...
O **mapa de metadados**, frequentemente chamado de **metamap** para abreviar, é um mapa no estilo Groovy contendo informações sobre unidades de dados.
...
Por convenção, um metamap nf-core é nomeado `meta` e contém o campo obrigatório `id`, que é usado para nomear saídas e rastrear unidades de dados.

Por exemplo, um mapa de metadados típico pode se parecer com isto:

```groovy title="Exemplo de metamap no nível de amostra"
[id: 'sample1', single_end: false, strandedness: 'forward']
```

Ou em um caso onde os metadados estão anexados no nível de lote:

```groovy title="Exemplo de metamap no nível de lote"
[id: 'batch1', date: '25.10.01']
```

Agora vamos colocar isso no contexto do processo `CAT_CAT`, que espera que os arquivos de entrada sejam empacotados em uma tupla com um metamap, e também produz o metamap como parte da tupla de saída.
...
Lembra quando dissemos que o arquivo produzido por `CAT_CAT` será nomeado com base em um identificador que faz parte dos metadados?
...
Isso se traduz aproximadamente da seguinte forma: se um `prefix` for fornecido via sistema de parâmetros de tarefa externo (`task.ext`), use-o para nomear o arquivo de saída; caso contrário, crie um usando `${meta.id}`, que corresponde ao campo `id` no metamap.
...
Como mencionado anteriormente, a configuração de entrada `tuple val(meta), path(files_in)` é um padrão padrão usado em todos os módulos nf-core.

Esperamos que você possa começar a ver como isso pode ser útil.
Não apenas permite que você nomeie saídas com base em metadados, mas você também pode fazer coisas como usá-lo para aplicar diferentes valores de parâmetros e, em combinação com operadores específicos, você pode até agrupar, ordenar ou filtrar dados conforme eles fluem pelo pipeline.

!!! note "Saiba mais sobre metadados"

    Para uma introdução abrangente ao trabalho com metadados em fluxos de trabalho Nextflow, incluindo como ler metadados de planilhas de amostras e usá-los para personalizar o processamento, consulte a missão secundária [Metadados em fluxos de trabalho](../side_quests/metadata).

### 2.3. Resumir as mudanças a serem feitas

Com base no que revisamos, estas são as principais mudanças que precisamos fazer em nosso pipeline para utilizar o módulo `cat/cat`:

- Criar um metamap contendo o nome do lote;
- Empacotar o metamap em uma tupla com o conjunto de arquivos de entrada para concatenar (vindo de `convertToUpper`);
...
Você sabe como avaliar a interface de entrada e saída de um novo módulo para identificar seus requisitos, e aprendeu como metamaps são usados por pipelines nf-core para manter metadados intimamente associados aos dados conforme eles fluem através de um pipeline.
...
Agora que você sabe tudo sobre metamaps (ou o suficiente para os propósitos deste curso, de qualquer forma), é hora de realmente implementar as mudanças que delineamos acima.

Para maior clareza, vamos dividir isso e cobrir cada etapa separadamente.
...
Primeiro, precisamos criar um mapa de metadados para `CAT_CAT`, tendo em mente que os módulos nf-core requerem que o metamap tenha pelo menos um campo `id`.

Como não precisamos de nenhum outro metadado, podemos manter simples e usar algo assim:

```groovy title="Exemplo de sintaxe"
def cat_meta = [id: 'test']
```

Exceto que não queremos codificar o valor de `id`; queremos usar o valor do parâmetro `params.batch`.
Então o código se torna:

```groovy title="Exemplo de sintaxe"
def cat_meta = [id: params.batch]
```

Sim, é literalmente tão simples criar um metamap básico.

Vamos adicionar essas linhas após a chamada `convertToUpper`, removendo a chamada `collectGreetings`:
...
=== "Depois"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="7-8"
        ...
        // cria mapa de metadados com o nome do lote como ID
        def cat_meta = [ id: params.batch ]
...
=== "Depois"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="10-11"
        ...
        // cria um canal com metadados e arquivos em formato de tupla
...
=== "Depois"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="13-14"
        ...
        // concatena arquivos usando o módulo nf-core cat/cat
        CAT_CAT(ch_for_cat)
...
Anteriormente, o processo `collectGreetings` apenas produzia um arquivo que podíamos passar para `cowpy` diretamente.
No entanto, o processo `CAT_CAT` produz uma tupla que inclui o metamap além do arquivo de saída.

Como `cowpy` ainda não aceita tuplas de metadados (vamos corrigir isso na próxima parte do curso), precisamos extrair o arquivo de saída da tupla produzida por `CAT_CAT` antes de passá-lo para `cowpy`:
...
=== "Depois"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="26" hl_lines="16-17 20"
        ...
        // concatena as saudações
        CAT_CAT(ch_for_cat)

        // extrai o arquivo da tupla já que cowpy ainda não usa metadados
...
Então é apenas uma questão de passar `ch_for_cowpy` para `cowpy` em vez de `collectGreetings.out.outfile` naquela última linha.

!!! note

    Na próxima parte do curso, vamos atualizar `cowpy` para trabalhar diretamente com tuplas de metadados, então esta etapa de extração não será mais necessária.
...
E é isso! Agora estamos usando um módulo robusto curado pela comunidade em vez de código personalizado de nível de protótipo para essa etapa no pipeline.
...
- Criar um mapa de metadados simples para uso com um módulo nf-core
- Integrar um módulo nf-core em seu fluxo de trabalho

### O que vem a seguir?

Aprender a adaptar seus módulos locais para seguir as convenções nf-core.
Também mostraremos como criar novos módulos nf-core a partir de um template usando as ferramentas nf-core.
