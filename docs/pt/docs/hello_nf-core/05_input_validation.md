...
Imagine executar seu pipeline por duas horas, apenas para ele falhar porque um usuário forneceu um arquivo com a extensão errada. Ou passar horas depurando erros enigmáticos, apenas para descobrir que um parâmetro foi digitado incorretamente. Sem validação de entrada, esses cenários são comuns.
...
O [plugin nf-schema](https://nextflow-io.github.io/nf-schema/latest/) é um plugin do Nextflow que fornece capacidades abrangentes de validação para pipelines Nextflow.
...
- **Geração de texto de ajuda**: Gera automaticamente a saída `--help` a partir das definições do schema
...
??? info "O que são plugins do Nextflow?"

    Plugins são extensões que adicionam novas funcionalidades à própria linguagem Nextflow. Eles são instalados via um bloco `plugins{}` no `nextflow.config` e podem fornecer:

    - Novas funções e classes que podem ser importadas (como `samplesheetToList`)
    - Novos recursos DSL e operadores
    - Integração com serviços externos

    O plugin nf-schema é especificado no `nextflow.config`:
...
- Valida que caminhos de arquivo existam
- Definido em `nextflow_schema.json`
...
- Valida que caminhos de arquivo referenciados na planilha de amostras existam
- Garante que campos obrigatórios estejam presentes
- Definido em `assets/schema_input.json`

!!! warning "O que a validação de dados de entrada NÃO faz"

    A validação de dados de entrada verifica a estrutura de *arquivos de manifesto* (planilhas de amostras, arquivos CSV), NÃO o conteúdo de seus arquivos de dados reais (FASTQ, BAM, VCF, etc.).

    Para dados em larga escala, validar o conteúdo dos arquivos (como verificar a integridade do BAM) deve acontecer em processos do pipeline executando em nós de trabalho, não durante o estágio de validação na máquina orquestradora.
...
A validação deve acontecer **antes** de qualquer processo do pipeline ser executado, para fornecer feedback rápido e evitar desperdício de tempo computacional.
...
### 1.1. Configurar validação para pular validação de arquivo de entrada

O template de pipeline nf-core vem com o nf-schema já instalado e configurado:

- O plugin nf-schema é instalado via o bloco `plugins{}` no `nextflow.config`
- A validação de parâmetros é habilitada por padrão via `params.validate_params = true`
- A validação é realizada pelo subfluxo de trabalho `UTILS_NFSCHEMA_PLUGIN` durante a inicialização do pipeline

O comportamento de validação é controlado através do escopo `validation{}` no `nextflow.config`.

Como vamos trabalhar primeiro na validação de parâmetros (esta seção) e não configuraremos o schema de dados de entrada até a seção 2, precisamos temporariamente dizer ao nf-schema para pular a validação do conteúdo do arquivo do parâmetro `input`.

Abra `nextflow.config` e encontre o bloco `validation` (por volta da linha 246). Adicione `ignoreParams` para pular a validação de arquivo de entrada:
...
- **`defaultIgnoreParams`**: Pular validação de parâmetros complexos como `genomes` (definido pelos desenvolvedores do template)
- **`ignoreParams`**: Pular validação do conteúdo do arquivo do parâmetro `input` (temporário; vamos reabilitar isso na seção 2)
...
!!! note "Por que ignorar o parâmetro input?"
    ...
    Vamos remover essa configuração na seção 2 após configurar o schema de dados de entrada.
...
Se você tem um olhar aguçado, pode notar que o parâmetro de entrada `batch` que temos usado ainda não está definido no schema.
...
??? info "De onde vêm os parâmetros do schema?"
    ...
    Parâmetros declarados em outros lugares em seus scripts de fluxo de trabalho (como em `main.nf` ou arquivos de módulo) **não** são automaticamente capturados pelo validador de schema.
...
Embora o schema seja um arquivo JSON que pode ser editado manualmente, **a edição manual é propensa a erros e não é recomendada**.
Em vez disso, o nf-core fornece uma ferramenta GUI interativa que lida com a sintaxe JSON Schema para você e valida suas alterações:
...
3. Preencha os detalhes do parâmetro:
   ...
   - Opcionalmente, selecione um ícone do seletor de ícones (por exemplo, `fas fa-layer-group`)
...
A ferramenta agora atualizou seu arquivo `nextflow_schema.json` com o novo parâmetro `batch`, lidando com toda a sintaxe JSON Schema corretamente.
...
### 1.5. Testar validação de parâmetros
...
Perfeito! A validação captura o parâmetro obrigatório ausente antes do pipeline executar.
...
A interface web lida com toda a sintaxe JSON Schema para você, facilitando o gerenciamento de schemas de parâmetros complexos sem edição manual de JSON propensa a erros.
...
- As duas primeiras colunas são strings de texto sem requisitos de formato especial
...
- **`description`**: Atualizado para mencionar "greetings file"
- **`properties`**: Substituído `sample`, `fastq_1` e `fastq_2` por `greeting`, `language` e `score`
  - **`type:`** Impõe string (`greeting`, `language`) ou integer (`score`)
  - **`pattern: "^\\S.*$"`**: Saudação deve começar com um caractere não-espaço (mas pode conter espaços depois disso)
  - **`"enum": ["en", "fr", "es", "it", "de"]`**: Código de idioma deve estar no conjunto suportado
  - **`"minimum": 0` e `"maximum": 100`**: Valor de pontuação deve estar entre 0 e 100
...
Quando o nf-schema lê um arquivo CSV, ele espera que a primeira linha contenha cabeçalhos de coluna que correspondam aos nomes de campo no schema.
...
Agora o arquivo CSV tem uma linha de cabeçalho que corresponde aos nomes de campo em nosso schema.
...
Se quiser praticar isso, sinta-se à vontade para criar outros arquivos de entrada de saudações que violem o schema de outras maneiras divertidas.
...
!!! tip "Leitura adicional"
    ...
    Para aprender mais sobre recursos e padrões avançados de validação, confira a [documentação do nf-schema](https://nextflow-io.github.io/nf-schema/latest/). O comando `nf-core pipelines schema build` fornece uma GUI interativa para gerenciar schemas complexos.
