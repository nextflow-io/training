...
Todas as instruções dadas durante o curso (por exemplo, 'abra o arquivo', 'edite o código' ou 'execute este comando') referem-se a essas três partes da interface VSCode, salvo indicação em contrário.
...
Este treinamento foi desenvolvido para **Nextflow 25.10.2** ou posterior **com o analisador de sintaxe v2 DESABILITADO**.
...
Certifique-se de estar usando as configurações corretas conforme documentado [aqui](../info/nxf_versions.md).

O treinamento também requer **nf-core tools 3.4.1**.
Se você usar uma versão diferente das ferramentas nf-core, pode ter dificuldade em acompanhar.
...
Por padrão, o codespace abre com o diretório de trabalho definido na raiz de todos os cursos de treinamento, mas para este curso, trabalharemos no diretório `hello-nf-core/`.
...
!!! tip "Dica"
    ...
    Se por algum motivo você sair deste diretório (por exemplo, seu codespace entrar em modo de espera), você sempre pode usar o caminho completo para retornar a ele, assumindo que você está executando isso dentro do ambiente de treinamento do Github Codespaces:
...
- [ ] Certifiquei-me de que o analisador de sintaxe está definido como **v1**
