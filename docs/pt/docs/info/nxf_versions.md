...
A versão do Nextflow atualmente carregada por padrão em nosso ambiente de treinamento é o **Nextflow 26.04.4**.
...
O analisador V2 é o padrão a partir do Nextflow 26.04 em diante, portanto, na versão que carregamos, você não precisa habilitá-lo manualmente.
...
No entanto, se você está planejando trabalhar nos treinamentos em seu próprio ambiente ([Instalação manual](../envsetup/02_local.md)), você precisará garantir o uso do Nextflow versão 25.10.2 ou posterior e habilitar o analisador de sintaxe v2 caso esteja usando uma versão anterior à 26.04.

## Versões mais antigas dos materiais de treinamento

Nossos materiais de treinamento são versionados desde fevereiro de 2025.

Você pode acessar versões mais antigas dos materiais de treinamento que funcionam com versões do Nextflow **anteriores à 25.10.2** através do menu suspenso no topo de cada página que mostra a versão numerada dos materiais de treinamento.
...
**DSL1 vs DSL2** refere-se a formas fundamentalmente diferentes de escrever pipelines Nextflow.
DSL1 era a sintaxe original onde os processos eram conectados implicitamente através de canais.
DSL2, introduzido no Nextflow 20.07, adicionou recursos de modularidade: a capacidade de importar processos e fluxos de trabalho de outros arquivos, blocos `workflow` explícitos e saídas de processo nomeadas.
DSL1 foi descontinuado no Nextflow 22.03 e removido na versão 22.12.
...
O analisador v2 é o padrão a partir do Nextflow 26.04 em diante.
...
Você pode verificar qual versão do Nextflow está instalada no seu sistema usando o comando `nextflow --version`.
...
A partir do Nextflow 26.04 em diante, o analisador v2 é o padrão, portanto as etapas abaixo são necessárias apenas em versões anteriores ao 26.04.

Para **habilitar** o analisador de sintaxe v2 para sua sessão atual, execute o seguinte comando no seu terminal:

```bash
export NXF_SYNTAX_PARSER=v2
```

Para tornar isso permanente, adicione o comando export ao seu perfil de shell (`~/.bashrc`, `~/.zshrc`, etc.):
...
Note que em versões do Nextflow anteriores ao 26.04, a variável de ambiente `NXF_SYNTAX_PARSER=v2` é necessária para acessar os recursos v2 utilizados nestes cursos.

### Desabilitando o analisador de sintaxe v2

Para **desabilitar** o analisador de sintaxe v2 para sua sessão atual, execute o seguinte comando no seu terminal:
...
Para orientação sobre a migração de código existente para estar em conformidade com versões mais recentes do Nextflow, consulte as [Notas de Migração](https://www.nextflow.io/docs/latest/migrations/index.html) na documentação de referência.
...
Ambos esses recursos são abordados como parte do treinamento para iniciantes a partir da versão 3.0 dos materiais de treinamento.

Dependendo da geração do código Nextflow que você pretende migrar, você pode conseguir fazer a maior parte usando o linter do Nextflow com o comando `nextflow lint -format`.
