...
Como os cursos agora usam entradas tipadas no nível do fluxo de trabalho, bem como diretivas de saída no nível do fluxo de trabalho, eles exigem o uso do analisador de sintaxe V2.
...
## Versões anteriores dos materiais de treinamento

Nossos materiais de treinamento são versionados desde fevereiro de 2025.

Você pode acessar versões anteriores dos materiais de treinamento que funcionam com versões do Nextflow **anteriores à 25.10.2** através do menu suspenso no topo de cada página que mostra a versão numerada dos materiais de treinamento.
Quando você seleciona uma versão anterior dos materiais de treinamento, os links para o ambiente de treinamento especificarão automaticamente a versão correspondente do ambiente.
...
Para tornar isso permanente (até que v2 se torne o padrão no Nextflow 26.04), adicione o comando export ao seu perfil de shell (`~/.bashrc`, `~/.zshrc`, etc.):
...
A partir do Nextflow 26.04 em diante, o analisador v2 se tornará o padrão e essa configuração não será mais necessária.
