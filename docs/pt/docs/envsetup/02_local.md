...
É possível instalar tudo o que você precisa para executar o treinamento em seu próprio ambiente local manualmente.
...
O Nextflow pode ser usado em qualquer sistema compatível com POSIX (Linux, macOS, Windows Subsystem for Linux, etc.) com Java instalado.
Nossos cursos de treinamento têm alguns requisitos adicionais.

No total, você precisará ter os seguintes softwares instalados:
...
O aplicativo VSCode é tecnicamente opcional, mas recomendamos fortemente que você o use para trabalhar nos cursos, bem como para seu trabalho de desenvolvimento em Nextflow em geral.

O manual de documentação do Nextflow fornece instruções para instalar essas dependências em [Configuração do ambiente](https://www.nextflow.io/docs/latest/developer-env.html).

## Nextflow e ferramentas nf-core

Você precisará instalar o próprio Nextflow, além das ferramentas nf-core, conforme detalhado nos artigos vinculados abaixo:
...
Recomendamos usar a opção de auto-instalação para o Nextflow e a opção PyPI para as ferramentas nf-core.

!!! warning "Aviso"

    <!-- Any update to this content needs to be copied to the home page -->
    **A partir de janeiro de 2026, todos os nossos cursos de treinamento em Nextflow requerem a versão 25.10.2 ou posterior do Nextflow, com sintaxe v2 estrita ativada, salvo indicação em contrário.**
...
Para trabalhar em um curso, abra uma janela de terminal (idealmente, de dentro do aplicativo VSCode) e use `cd` para entrar no diretório relevante.
