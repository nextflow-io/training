...
Se você tem uma instalação local do Docker ou está disposto a instalar uma, a maneira mais fácil de trabalhar localmente com estes materiais é usar o recurso de devcontainer do Visual Studio Code. Esta abordagem fornece todas as ferramentas e dependências necessárias sem exigir instalação manual.
...
- Navegue até a pasta do repositório de treinamento que você acabou de clonar e selecione-a
- Clique em **Open**
...
![Prompt para reabrir no contêiner](img/reopen_prompt.png)

![Selecionando a configuração local](img/select_local_config.png)

Aguarde a construção do contêiner. Isso pode levar alguns minutos na primeira vez, pois baixa e configura todos os componentes necessários.
...
Isso deve exibir a versão do Nextflow e informações de tempo de execução, confirmando que seu ambiente está configurado adequadamente.
