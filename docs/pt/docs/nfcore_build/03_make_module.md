# Parte 3: Criar um módulo nf-core

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } Tradução assistida por IA - [saiba mais e sugira melhorias](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>

Nesta terceira parte do curso de treinamento Build with nf-core, mostramos como criar um módulo nf-core aplicando as convenções principais que tornam os módulos portáteis e de fácil manutenção.

O projeto nf-core fornece um comando (`nf-core modules create`) que gera templates de módulos estruturados corretamente de forma automática, semelhante ao que usamos para o fluxo de trabalho na Parte 1.
...
??? info "Como começar a partir desta seção"
    ...
    Esta seção pressupõe que você completou a [Parte 2: Usar um módulo nf-core](./02_use_module.md) e integrou o módulo `FIND_CONCATENATE` ao seu pipeline.

    Se você não completou a Parte 2 ou deseja começar do zero para esta parte, pode usar a solução `core-hello-part2` como ponto de partida.
    Execute estes comandos de dentro do diretório `nfcore-build/`:
    ...
    Isso fornece um pipeline com o módulo `FIND_CONCATENATE` já integrado.
...
=== "Depois"
    ...
    // extrai o arquivo da tupla pois cowpy ainda não usa metadata
...
Na versão atual do pipeline `core-hello`, estamos extraindo o arquivo da tupla de saída do `FIND_CONCATENATE` para passar ao `COWPY`, como mostrado na metade superior do diagrama abaixo.
...
Como você pode ver, mudamos tanto a **entrada principal** quanto a **saída** para uma tupla que segue o padrão `tuple val(meta), path(input_file)` introduzido na Parte 2 deste treinamento.
...
Agora que a saída do `FIND_CONCATENATE` e a entrada do `COWPY` têm a mesma 'forma', ou seja, ambas consistem em uma estrutura `tuple val(meta), path(input_file)`, podemos simplesmente conectá-las diretamente em vez de ter que extrair o arquivo explicitamente da saída do processo `FIND_CONCATENATE`.
...
Agora chamamos `COWPY` em `FIND_CONCATENATE.out.file_out` diretamente.
...
O pipeline deve executar com sucesso, com os metadata agora fluindo do `FIND_CONCATENATE` através do `COWPY`.
...
A sintaxe `withName:` atribui essa configuração apenas ao processo `COWPY`, e `#!groovy ext.args = { "-c ${params.character}" }` simplesmente compõe uma string que incluirá o valor do parâmetro `character`.
...
Encontre a saída no explorador de arquivos ou use o hash da tarefa (a parte `38/eb29ea` no exemplo acima) para visualizar o arquivo de saída:
...
1. **No bloco `script:`, adicionamos a linha `#!groovy prefix = task.ext.prefix ?: "${meta.id}"`.**
...
2. **Na linha de comando, substituímos `#!groovy cowpy-${input_file}` por `#!groovy ${prefix}.txt`.**
   É aqui que o Nextflow injetará o valor de `prefix` determinado pela linha acima.

3. **No bloco `output:`, substituímos `#!groovy path("cowpy-${input_file}")` por `#!groovy path("${prefix}.txt")`.**
...
- **`core-hello-results`** — O diretório de saída definido com `--outdir` na linha de comando, que tem recebido os logs nf-core e os resultados publicados pelo `FIND_CONCATENATE`.
...
"Ei," você pode dizer, "`FIND_CONCATENATE` está enviando suas saídas para o `--outdir`. Talvez devêssemos copiar sua diretiva `publishDir`?"
...
!!! info "Info"
    ...
    Você pode notar o arquivo `hello_software_versions.yml` em `pipeline_info/`.
    Atualmente ele contém apenas informações de versão do `FIND_CONCATENATE`, pois o `COWPY` ainda não reporta sua versão.
    A seção 1.6 explica como adicionar isso.
...
- **`path:`** Determina o diretório de saída com base no nome do processo.
  O nome completo de um processo contido em `task.process` inclui a hierarquia de importações de fluxo de trabalho e módulo (como `CORE_HELLO:HELLO:FIND_CONCATENATE`).
  ...
  Isso é o que determina que os resultados do `FIND_CONCATENATE` sejam publicados em `#!groovy ${params.outdir}/find/`.
...
Dito isso, você pode decidir que deseja organizar suas saídas de forma diferente, e a boa notícia é que é fácil fazer isso.
...
### 1.6. Captura de versões com topic channels

Reprodutibilidade é um princípio central do nf-core: cada execução de pipeline registra exatamente quais versões de software foram usadas, para que os resultados possam ser reproduzidos e comparados entre execuções.
O relatório `hello_software_versions.yml` em `pipeline_info/` é como os pipelines nf-core cumprem esse requisito.

O mecanismo por trás disso são os **topic channels**: um recurso de broadcast do Nextflow onde qualquer processo pode publicar dados em um tópico nomeado, e qualquer assinante recebe tudo o que foi publicado naquele tópico — independentemente de onde venha no pipeline.
O nf-core está implementando isso em todos os pipelines em 2026.
Os módulos publicam sua versão como uma tupla diretamente no bloco de saída usando a palavra-chave `topic:`, e o fluxo de trabalho assina uma vez para coletar todas elas.

#### 1.6.1. Adicionar reporte de versão ao `COWPY`

Abra o arquivo do módulo `cowpy.nf` e adicione uma linha de saída de versão como mostrado abaixo.
...
A nova linha publica uma tupla `[process_name, tool_name, version]` no canal de tópico `"versions"`.
Nenhuma alteração no bloco de script é necessária — a versão é declarada estaticamente no bloco de saída.

#### 1.6.2. Executar o pipeline e inspecionar o relatório de versões
...
Abra `core-hello-results/pipeline_info/hello_software_versions.yml` e você verá agora ambos os módulos:
...
A coleta no lado do fluxo de trabalho — o bloco `channel.topic("versions")` que você viu no fluxo de trabalho placeholder na Parte 1 — assina o tópico e escreve esse relatório combinado automaticamente.

!!! info "Compatibilidade retroativa"

    O branch `versions_file` no bloco de topic channel do fluxo de trabalho existe para lidar com módulos que ainda não foram atualizados para usar `topic: versions` e ainda escrevem um arquivo `versions.yml` no bloco de script com `emit: versions`.
    Ambos os estilos são suportados simultaneamente durante a transição.
...
- Entenda como os topic channels coletam versões de software de todos os módulos automaticamente.

Para leitura adicional sobre convenções de módulos, consulte as [especificações de módulos nf-core](https://nf-co.re/docs/guidelines/components/modules).
...
!!! tip "Saiba mais sobre testes"
    ...
    O arquivo de teste gerado usa nf-test, um framework de testes para pipelines e módulos Nextflow. Para aprender como escrever e executar esses testes, veja a [missão paralela nf-test](../side_quests/nf_test/index.md).
...
Observe como todos os padrões que você aplicou manualmente acima já estão presentes!

O template também inclui várias convenções adicionais do nf-core.
Algumas delas funcionam imediatamente, enquanto outras são marcadores de posição que precisaremos preencher, como descrito abaixo.

**Funcionalidades que funcionam como estão:**

- **`tag "$meta.id"`**: Adiciona o ID da amostra aos nomes dos processos nos logs para facilitar o rastreamento
- **`label 'process_single'`**: Rótulo de recursos para configurar requisitos de CPU/memória
- **Bloco `when:`**: Permite execução condicional via configuração `task.ext.when`
- **Saída `topic: versions`**: Reporte de versão via topic channel, conforme abordado na seção 1.6

Essas funcionalidades já são operacionais e tornam os módulos mais manuteníveis.

**Marcadores de posição que personalizaremos abaixo:**

- **Blocos `input:` e `output:`**: Declarações genéricas que atualizaremos para corresponder à nossa ferramenta
- **Bloco `script:`**: Vazio — adicionaremos o comando `cowpy` aqui
- **Bloco `stub:`**: Atualizaremos para produzir as saídas corretas
- **Container e ambiente**: Marcadores de posição que preencheremos com informações de pacotes

As próximas seções descrevem a conclusão dessas personalizações.

### 2.2. Configurar o container e o ambiente Conda

As diretrizes do nf-core exigem que especifiquemos tanto um container quanto um ambiente Conda como parte do módulo.

#### 2.2.1. Container

Para o container, você pode usar o [Seqera Containers](https://seqera.io/containers/) para construir automaticamente um container a partir de qualquer pacote Conda, incluindo pacotes conda-forge.
Neste caso, estamos usando o mesmo container pré-construído de antes.

O código padrão oferece alternar entre Docker e Singularity, mas vamos simplificar essa linha e especificar apenas o container Docker que obtivemos do Seqera Containers acima.
...
Para o ambiente Conda, o código do módulo especifica `#!groovy conda "${moduleDir}/environment.yml"`, o que significa que deve ser configurado no arquivo `environment.yml`.

A ferramenta de criação de módulos nos avisou que não conseguiu encontrar o pacote `cowpy` no Bioconda (o canal principal para ferramentas de bioinformática).
...
=== "Antes"
    ...
    channels:
      ...
      - bioconda
    dependencies:
      # TODO nf-core: Liste os pacotes Conda necessários.
      #               O software DEVE ser fixado ao canal (ex.: "bioconda"), versão (ex.: "1.10").
      #               Para Conda, o build (ex.: "h9402c20_2") deve ser EXCLUÍDO para suportar instalação em diferentes sistemas operacionais.
      - "YOUR-TOOL-HERE"
    ```
...
!!! tip "Pacotes Bioconda vs conda-forge"
    ...
    - **Pacotes Bioconda**: Obtêm automaticamente BioContainers construídos, fornecendo containers prontos para uso
    - **Pacotes conda-forge**: Podem usar o Seqera Containers para construir containers sob demanda a partir da receita Conda

    A maioria das ferramentas de bioinformática está no Bioconda, mas para ferramentas conda-forge, o Seqera Containers fornece uma solução fácil para containerização.

### 2.3. Inserir a lógica do `COWPY`

Agora vamos atualizar os elementos de código específicos do que o processo `COWPY` faz: as entradas e saídas, e o bloco de script.
...
Olhando para o nosso módulo manual `COWPY` da seção 1, podemos usá-lo como guia.
...
- O nome do parâmetro do arquivo de entrada (`input_file` em vez do genérico `input`)
- O nome do arquivo de saída usando o padrão de prefixo configurável (`#!groovy ${prefix}.txt` em vez do curinga `*`)
- Um nome de emissão descritivo (`cowpy_output` em vez do genérico `output`)
- Uma string de versão estática (`#!groovy val("1.1.5")`) no lugar do `#!groovy eval("cowpy --version")` do template, correspondendo ao módulo manual da seção 1.6 (a ferramenta `cowpy` não expõe uma flag `--version`)

Se você está usando o servidor de linguagem Nextflow para validar a sintaxe, a parte `#!groovy ${prefix}` será marcada como erro nesta fase porque ainda não a adicionamos ao bloco de script.
Vamos fazer isso agora.

#### 2.3.2. O bloco de script

O template gera um bloco de script vazio que você precisa preencher com o comando real da ferramenta.

Com base no módulo que escrevemos manualmente anteriormente, devemos fazer as seguintes edições:
...
- Alterar `def prefix` para apenas `prefix` (sem `def`) para torná-lo acessível no bloco de saída
- Preencher o comando real de `cowpy` que usa tanto `$args` quanto `#!groovy ${prefix}.txt`
...
No contexto do Nextflow, um bloco [stub](https://www.nextflow.io/docs/latest/process.html#stub) permite definir um script leve e fictício usado para prototipagem rápida e teste da lógica de um pipeline sem executar o comando real.
...
Não se preocupe muito se isso parece misterioso; incluímos isso por completude, mas você também pode simplesmente excluir a seção stub se não quiser lidar com ela, já que é completamente opcional.
...
- Alterar `def prefix` para apenas `prefix` para corresponder ao bloco de script
- Remover a linha `echo $args` (que era apenas código de marcador de posição do template)
- O stub cria um arquivo vazio `#!groovy ${prefix}.txt` correspondendo ao que o bloco de script produz

Isso permite testar a lógica do fluxo de trabalho e o manuseio de arquivos sem esperar pela execução da ferramenta real.

Uma vez que você tenha completado a configuração do ambiente (seção 2.2), entradas/saídas (seção 2.3.1), bloco de script (seção 2.3.2) e bloco stub (seção 2.3.3), o módulo está pronto para teste!

### 2.4. Substituir pelo novo módulo `COWPY` e executar o pipeline

Tudo o que precisamos fazer para experimentar esta nova versão do módulo `COWPY` é alterar a declaração de importação no arquivo de fluxo de trabalho `hello.nf` para apontar para o novo arquivo.
...
Saiba quais são os benefícios de contribuir módulos para o nf-core e quais são os principais passos e requisitos envolvidos.

---

## 3. Contribuir módulos de volta para o nf-core

O repositório [nf-core/modules](https://github.com/nf-core/modules) aceita contribuições de módulos bem testados e padronizados.
...
- Garante manutenção contínua da comunidade e melhorias
- Fornece garantia de qualidade através de revisão de código e testes automatizados
- Dá visibilidade e reconhecimento ao seu trabalho

### 3.2. Checklist do contribuidor

Para contribuir um módulo para o nf-core, você precisará seguir os seguintes passos:
...
4. Preencher a lógica do módulo e os testes
5. Testar com `nf-core modules test tool/subtool`
6. Verificar com `nf-core modules lint tool/subtool`
7. Enviar um pull request

Para instruções detalhadas, consulte o [tutorial de componentes nf-core](https://nf-co.re/docs/tutorials/nf-core_components/components).
...
- **Tuplas de metadata** propagam metadata através do fluxo de trabalho
...
- **Publicação centralizada** via `publishDir` configurado em `modules.config` em vez de codificado nos módulos

Ao transformar `COWPY` passo a passo, você desenvolveu uma compreensão profunda desses padrões, tornando-se capaz de trabalhar com, depurar e criar módulos nf-core.
Na prática, você usará `nf-core modules create` para gerar módulos corretamente estruturados com esses padrões integrados desde o início.
...
Quando estiver pronto, continue com a [Parte 4: Validação de entrada](./04_input_validation.md) para aprender como adicionar validação de entrada baseada em esquema ao seu pipeline.
