...
이는 [파트 1: 방법 개요](./01_method.md#1-single-sample-processing)의 첫 번째 섹션에서 컨테이너에서 수동으로 실행한 단계를 자동화합니다.
...
이 파일들은 기능적이지 않으며, 코드의 중요한 부분을 채울 수 있는 골격 역할만 하기 위한 것입니다.
...
2. **어댑터 트리밍 및 트리밍 후 QC 추가.**
   한 프로세스의 출력을 다른 프로세스의 입력에 연결하여 프로세스를 연결하는 방법을 소개합니다.
3. **참조 게놈에 대한 정렬 추가.**
...
[파트 1](01_method.md)의 `fastqc` 명령을 기억하세요:
...
위에 제공된 정보를 사용하여 프로세스 정의를 직접 채운 다음, 아래 "후" 탭의 솔루션과 비교하여 작업을 확인하세요.
...
`simpleName` 접근자는 파일 이름에서 모든 확장자를 제거하므로 `ENCSR000COQ1_1.fastq.gz`는 `ENCSR000COQ1_1`이 됩니다.
...
#### 1.3.2. `output {}` 블록에서 출력 대상 설정

`output {}` 블록은 workflow 외부에 위치하며 각 명명된 대상이 게시되는 위치를 지정합니다.
두 대상 모두 `fastqc/` 하위 디렉토리에 게시되도록 설정합니다.
...
!!! note "참고"
    ...
    심볼릭 링크는 `work` 디렉토리를 정리할 때 끊어지므로 프로덕션 워크플로우에서는 기본 게시 모드를 `'copy'`로 재정의할 수 있습니다.
...
[파트 1](01_method.md)의 `trim_galore` 명령을 기억하세요:
...
`output {}` 블록에 트리밍 대상에 대한 항목을 추가하여 `trimming/` 하위 디렉토리에 게시합니다:
...
이제 워크플로우에 초기 QC와 어댑터 트리밍이 모두 포함됩니다.
...
트리밍된 리드 출력에서 연결되는 정렬 단계를 추가합니다.
...
[파트 1](01_method.md)의 정렬 명령을 기억하세요:
...
`output {}` 블록에 정렬 대상에 대한 항목을 추가하여 `align/` 하위 디렉토리에 게시합니다:
...
이제 워크플로우에 세 가지 처리 단계가 모두 포함됩니다: QC, 트리밍, 정렬.
