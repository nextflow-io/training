...
<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } AI 기반 번역 - [자세히 알아보기 및 개선 제안하기](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>

대량 RNAseq 데이터를 처리하고 분석하는 여러 가지 유효한 방법이 있습니다.
이 과정에서는 [Babraham Institute](https://www.babraham.ac.uk/)의 Simon Andrews 박사와 Laura Biggins 박사가 [여기](https://www.bioinformatics.babraham.ac.uk/training/RNASeq_Course/Analysing%20RNA-Seq%20data%20Exercise.pdf)에 설명한 방법을 따릅니다.

우리의 목표는 다음 처리 단계를 구현하는 워크플로우를 개발하는 것입니다: 대량 RNAseq 샘플의 리드에 대한 초기 품질 관리를 실행하고, 리드에서 어댑터 서열을 트리밍하고, 리드를 참조 게놈에 정렬하고, 포괄적인 품질 관리(QC) 보고서를 생성합니다.

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/rnaseq/img/preprocess.svg"
</figure>

- **FASTQC:** FastQC를 사용하여 트리밍 전 리드 데이터에 대한 QC를 수행합니다
- **TRIM_GALORE:** Trim Galore를 사용하여 어댑터 서열을 트리밍하고 트리밍 후 QC를 수행합니다 (Cutadapt와 FastQC를 번들로 제공)
- **HISAT2_ALIGN:** Hisat2를 사용하여 리드를 참조 게놈에 정렬합니다
- **MULTIQC:** MultiQC를 사용하여 포괄적인 QC 보고서를 생성합니다

그러나 워크플로우 코드를 작성하기 전에, 먼저 테스트 데이터에서 명령을 수동으로 실행해 보겠습니다.
필요한 도구들이 GitHub Codespaces 환경에 설치되어 있지 않으므로, 컨테이너를 통해 사용하겠습니다 ([Hello Containers](../../hello_nextflow/05_hello_containers.md) 참조).

!!! note "참고"

     `nf4-science/rnaseq` 디렉토리에 있는지 확인하세요. `pwd`를 입력했을 때 표시되는 경로의 마지막 부분이 `rnaseq`여야 합니다.

---

## 1. 초기 QC 및 어댑터 트리밍

`fastqc`와 `trim_galore`가 모두 설치된 컨테이너 이미지를 가져와서 대화형으로 실행하고, 예제 데이터 파일 중 하나에서 트리밍 및 QC 명령을 실행하겠습니다.
...
시스템이 이미지를 다운로드하면서 다음과 같은 콘솔 출력이 표시됩니다:
...
프롬프트가 `(base) root@b645838b3314:/tmp#`와 같이 변경되며, 이는 컨테이너 내부에 있음을 나타냅니다.

명령의 `-v ./data:/data` 부분은 컨테이너 내부에서 `data/` 디렉토리의 내용에 접근할 수 있게 해줍니다.
...
리드 데이터에 대한 품질 관리 메트릭을 수집하기 위해 `fastqc`를 실행하겠습니다.
...
이 명령은 매우 빠르게 실행됩니다.
출력 파일은 원본 데이터와 동일한 디렉토리에서 찾을 수 있습니다:
...
이제 Cutadapt와 FastQC를 번들로 제공하는 `trim_galore`를 실행하여 어댑터 서열을 트리밍하고 트리밍 후 QC 메트릭을 수집하겠습니다.

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

`--fastqc` 플래그는 트리밍이 완료된 후 자동으로 QC 수집 단계를 실행하도록 합니다.

_출력이 매우 길기 때문에 다음은 축약된 내용입니다._
...
### 1.5. 출력 파일을 컨테이너 외부 파일 시스템으로 이동하기

컨테이너 내부에 남아있는 모든 것은 향후 작업에서 접근할 수 없으므로, 이것들을 새 디렉토리로 이동하겠습니다.
...
`hisat2`가 설치된 컨테이너 이미지를 가져와서 대화형으로 실행하고, RNAseq 데이터를 참조 게놈에 정렬하는 정렬 명령을 실행하겠습니다.
...
### 2.2. `hisat2` 컨테이너를 대화형으로 실행하기

```bash
docker run -it -v ./data:/data community.wave.seqera.io/library/hisat2_samtools:5e49f68a37dc010e
```

명령은 이전과 동일하며, 관련 컨테이너 URI만 교체되었습니다.

### 2.3. Hisat2 게놈 인덱스 파일 생성하기

Hisat2는 게놈 참조가 매우 특정한 형식으로 제공되어야 하며, 우리가 제공하는 `genome.fa` FASTA 파일을 그냥 사용할 수 없습니다. 따라서 이 기회에 관련 리소스를 생성하겠습니다.

```bash
hisat2-build /data/genome.fa genome_index
```

출력이 매우 길기 때문에 다음은 축약된 내용입니다:
...
이렇게 하면 여러 게놈 인덱스 파일이 생성되며, 작업 디렉토리에서 찾을 수 있습니다.
...
잠시 후 이것들을 사용하겠지만, 먼저 이 게놈 인덱스 파일들로 gzip 압축된 tarball을 생성하겠습니다. 나중에 필요하며, 이것들을 생성하는 것은 일반적으로 워크플로우의 일부로 하고 싶지 않은 작업입니다.

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

이렇게 하면 게놈 인덱스 파일을 포함하는 `genome_index.tar.gz` tarball이 파일 시스템의 `data/` 디렉토리에 저장되며, 이 과정의 파트 2에서 유용하게 사용됩니다.

### 2.4. `hisat2` 명령 실행하기

이제 정렬 명령을 실행할 수 있습니다. 이 명령은 `hisat2`로 정렬 단계를 수행한 다음 출력을 `samtools`로 파이프하여 BAM 파일로 출력을 작성합니다.
...
매우 작은 테스트 파일이기 때문에 거의 즉시 실행됩니다.
실제 규모에서는 훨씬 더 오래 걸릴 수 있습니다.

다시 한 번 출력 파일은 작업 디렉토리에서 찾을 수 있습니다:
...
### 2.5. 출력 파일을 컨테이너 외부 파일 시스템으로 이동하기
...
`multiqc`가 설치된 컨테이너 이미지를 가져와서 대화형으로 실행하고, 트리밍 전후 FastQC 보고서 파일에 대한 보고서 생성 명령을 실행하겠습니다.
...
MultiQC는 호환되는 QC 보고서를 찾기 위해 디렉토리를 검색할 수 있으며, 찾은 모든 것을 집계합니다.

여기서 도구가 우리가 생성한 세 가지 QC 보고서를 모두 찾은 것을 볼 수 있습니다: `fastqc`로 수행한 초기 QC, `cutadapt`의 트리밍 후 보고서(`trim_galore`를 통해 생성), 그리고 `hisat2`가 생성한 정렬 후 QC입니다.

출력 파일은 다시 한 번 작업 디렉토리에 있습니다:
...
### 3.4. 출력 파일을 컨테이너 외부 파일 시스템으로 이동하기
...
컨테이너를 사용하여 작업을 실행하는 다단계 워크플로우에 동일한 명령을 적용하는 방법을 학습합니다.
