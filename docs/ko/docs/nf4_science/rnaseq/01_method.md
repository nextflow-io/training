...
- **CSV 샘플시트** (`single-end.csv` 및 `paired-end.csv`): 예제 데이터 파일의 ID와 경로를 나열하는 파일.

### 소프트웨어

관련된 네 가지 주요 도구는 품질 관리 메트릭 수집을 위한 [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/), 어댑터 트리밍을 위한 [Trim Galore](https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) (Cutadapt 및 트리밍 후 QC를 위한 FastQC 번들), 참조 게놈에 대한 spliced 정렬을 위한 [HISAT2](http://daehwankimlab.github.io/hisat2/), 그리고 집계된 QC 보고서 생성을 위한 [MultiQC](https://multiqc.info/)입니다.
...
이전에 이 이미지를 다운로드하지 않았다면 완료하는 데 1분 정도 걸릴 수 있습니다.
...
`/data/reads` 아래에서 시퀀스 데이터 파일을 볼 수 있는지 확인합니다:
...
하나의 데이터 파일에 대해 `fastqc` 명령을 실행합니다:
...
위에서 언급했듯이 소프트웨어는 동일한 컨테이너에 포함되어 있으므로 변경이 필요하지 않습니다.
...
일부 레이어가 이전에 가져온 Trim Galore 컨테이너 이미지와 공유되므로 `Already exists`로 표시됩니다.
결과적으로 이 가져오기는 첫 번째 것보다 빠르게 진행됩니다.

#### 1.2.2. 대화형으로 컨테이너 실행하기

이전과 동일한 방법으로 관련 컨테이너 URI로 교체하여 컨테이너를 대화형으로 실행합니다.
...
이전과 마찬가지로 컨테이너를 종료한 후에도 액세스할 수 있도록 출력 파일을 마운트된 파일시스템의 디렉토리로 이동합니다.
...
!!! example "워크플로우로 작성하기!"
    ...
    바로 [파트 2](./02_single-sample.md)로 이동하여 이 분석을 Nextflow 워크플로우로 구현하기 시작하고 싶다면 그렇게 하셔도 됩니다.
    파트 3으로 이동하기 전에 두 번째 테스트 라운드를 완료하기 위해 돌아오기만 하면 됩니다.
...
실제로는 일반적으로 많은 샘플을 처리한 다음 전체 데이터셋의 품질을 평가하기 위해 모든 샘플에 걸쳐 QC 결과를 집계해야 합니다.
...
두 개의 추가 샘플에 대해 FastQC 및 Trim Galore를 차례로 실행합니다.
...
Trim Galore 출력 파일을 섹션 1에서 사용한 동일한 디렉토리로 이동합니다.
...
마운트된 파일시스템에 저장한 tarball에서 게놈 인덱스 파일을 추출합니다:
...
새로 트리밍된 두 샘플에 대해 HISAT2 정렬을 차례로 실행합니다.
...
정렬 출력 파일을 섹션 1에서 사용한 동일한 디렉토리로 이동합니다.
...
일부 레이어가 이전에 가져온 컨테이너 이미지와 공유되므로 `Already exists`로 표시됩니다.
결과적으로 이 가져오기는 이전 것들보다 빠르게 진행됩니다.

#### 2.3.2. 대화형으로 컨테이너 실행하기

이전과 마찬가지로 데이터 디렉토리가 마운트된 상태로 컨테이너를 대화형으로 실행합니다.
...
세 샘플 모두에 대한 QC 관련 출력 파일을 저장한 디렉토리를 가리키도록 `multiqc`를 실행합니다.
...
보고서와 데이터 디렉토리를 마운트된 파일시스템으로 이동합니다.
...
여러 샘플을 처리하고 QC 보고서를 집계하는 방법을 포함하여 각각의 컨테이너에서 FastQC, Trim Galore, HISAT2 및 MultiQC 명령을 실행하는 방법을 알게 되었습니다.
