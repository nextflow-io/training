...
이 과정의 마지막 파트에서는 간단한 워크플로우를 임의의 수의 샘플을 처리할 수 있는 강력한 배치 자동화 도구로 발전시킬 것입니다.
그리고 이와 함께 최신 연구에서 더 일반적으로 사용되는 paired-end 데이터를 처리하도록 전환할 것입니다.

이를 세 단계로 진행하겠습니다:

1. 워크플로우가 여러 입력 샘플을 받아들이고 실행을 병렬화하도록 만들기
2. 포괄적인 QC 보고서 생성 추가하기
3. paired-end RNAseq 데이터로 전환하기
...
### 1.1. 기본 입력을 단일 파일 대신 파일 경로의 CSV로 변경하기

`data/` 디렉토리에 샘플 ID와 FASTQ 파일 경로가 포함된 CSV 파일을 제공합니다.
이 CSV 파일에는 헤더 라인이 포함되어 있습니다.
...
기본 입력 매개변수의 이름을 `input_csv`로 변경하고 기본값을 `single-end.csv` 파일의 경로로 변경하겠습니다.
...
### 1.2. CSV를 입력으로 처리하도록 입력 채널 팩토리 업데이트하기

파일 경로 자체가 아닌 파일의 내용을 채널에 로드하려고 하므로, `.splitCsv()` 연산자를 사용하여 CSV 형식을 분석한 다음 `.map()` 연산자를 사용하여 원하는 특정 정보(FASTQ 파일 경로)를 가져옵니다.
...
이번에는 각 단계가 제공한 6개의 데이터 파일 각각에 대해 6번 실행되는 것을 볼 수 있습니다.

워크플로우가 여러 파일에서 실행되도록 하는 데 필요한 것은 이것뿐입니다!
Nextflow가 모든 병렬 처리를 자동으로 처리합니다.

---

## 2. 전처리 QC 메트릭을 단일 MultiQC 보고서로 집계하기

이 모든 과정에서 많은 QC 보고서가 생성되며, 개별 보고서를 일일이 확인하고 싶지 않습니다.
이것이 MultiQC 보고서 집계 단계를 추가하기에 완벽한 시점입니다!

### 2.1. QC 집계 프로세스를 위한 모듈 생성하기

`MULTIQC` 프로세스를 담을 `modules/multiqc.nf`라는 모듈 파일을 생성하겠습니다:
...
### 2.3. `report_id` 매개변수를 추가하고 적절한 기본값 지정하기
...
### 2.4. 이전 단계의 출력에 대해 프로세스 호출하기

`MULTIQC` 프로세스에 이전 단계의 모든 QC 관련 출력을 제공해야 합니다.

이를 위해 여러 채널을 하나로 집계하는 `.mix()` 연산자를 사용하겠습니다.

A, B, C, D라는 네 개의 프로세스가 각각 간단한 `.out` 채널을 가지고 있다면, 구문은 다음과 같습니다: `A.out.mix( B.out, C.out, D.out )`. 보시다시피, 결합하려는 첫 번째 채널에 적용하고(어느 것이든 상관없음) 나머지 모든 채널을 쉼표로 구분하여 괄호 안에 추가합니다.

워크플로우의 경우 다음 출력을 집계해야 합니다:
...
이렇게 하면 샘플별로 QC 보고서가 수집됩니다.
하지만 모든 샘플에 걸쳐 집계하려면 모든 샘플의 보고서를 `MULTIQC`에 대한 단일 호출로 가져오기 위해 `collect()` 연산자를 추가해야 합니다.
...
이번에는 캐시된 프로세스 호출 후에 MULTIQC에 대한 단일 호출이 추가된 것을 볼 수 있습니다:
...
마지막 `all_single-end.html` 파일은 전체 집계 보고서로, 탐색하기 쉬운 하나의 HTML 파일로 편리하게 패키징되어 있습니다.

---

## 3. paired-end RNAseq 데이터 처리 활성화하기

현재 워크플로우는 single-end RNAseq 데이터만 처리할 수 있습니다.
paired-end RNAseq 데이터를 보는 것이 점점 더 일반화되고 있으므로 이를 처리할 수 있어야 합니다.

워크플로우를 데이터 타입에 완전히 독립적으로 만들려면 약간 더 고급 Nextflow 언어 기능을 사용해야 하므로 여기서는 다루지 않겠지만, 무엇을 조정해야 하는지 보여주기 위해 paired-end 처리 버전을 만들 수 있습니다.
...
### 3.2. 기본 `input_csv`를 paired-end 데이터를 가리키도록 수정하기
...
이제 `.map()` 연산자가 두 FASTQ 파일 경로를 모두 가져오도록 지시해야 합니다.

따라서 `row -> file(row.fastq_path)`가 `row -> [file(row.fastq_1), file(row.fastq_2)]`로 변경됩니다
...
두 버전을 모두 보유할 수 있도록 모듈의 복사본을 만들겠습니다.

```bash
cp modules/fastqc.nf modules/fastqc_pe.nf
```

코드 편집기에서 새 `fastqc_pe.nf` 모듈 파일을 열고 다음 코드를 변경합니다:

- `script` 블록(17번째 줄)에서 `fastqc $reads`를 `fastqc ${reads}`로 변경하여 `reads` 입력이 단일 경로가 아닌 두 경로의 튜플이므로 압축 해제되도록 합니다.
...
기술적으로 이것은 `FASTQC` 프로세스를 single-end 또는 paired-end RNAseq 데이터를 모두 처리할 수 있도록 일반화합니다.

마지막으로, 모듈의 paired-end 버전을 사용하도록 모듈 가져오기 문을 업데이트합니다.
...
두 버전을 모두 보유할 수 있도록 모듈의 복사본을 만듭니다.

```bash
cp modules/trim_galore.nf modules/trim_galore_pe.nf
```

코드 편집기에서 새 `trim_galore_pe.nf` 모듈 파일을 열고 다음 코드를 변경합니다:

- 입력 선언을 `path reads`에서 `tuple path(read1), path(read2)`로 변경합니다
...
### 3.6. TRIM_GALORE에서 두 개의 보고서를 예상하도록 MULTIQC 프로세스 호출 업데이트하기

`TRIM_GALORE` 프로세스가 이제 추가 출력 채널을 생성하므로 이를 MultiQC에 제공해야 합니다.
...
MultiQC를 다루는 김에 `report_id` 매개변수 기본값도 `"all_single-end"`에서 `"all_paired-end"`로 업데이트하겠습니다.
...
코드 편집기에서 새 `hisat2_align_pe.nf` 모듈 파일을 열고 다음 코드를 변경합니다:
...
### 3.8. 워크플로우를 실행하여 작동하는지 테스트하기

캐시되지 않으므로 `-resume`을 사용하지 않으며, 이전보다 처리할 데이터가 두 배이지만 여전히 1분 이내에 완료되어야 합니다.
...
이것으로 끝입니다! 이제 워크플로우의 약간 다른 두 버전이 있습니다. 하나는 single-end 리드 데이터용이고 다른 하나는 paired-end 데이터용입니다.
다음 논리적 단계는 워크플로우가 두 데이터 타입을 즉시 받아들이도록 만드는 것인데, 이는 이 과정의 범위를 벗어나지만 후속 과정에서 다룰 수 있습니다.

---

### 핵심 정리

단일 샘플 워크플로우를 여러 샘플의 병렬 처리로 조정하고, 포괄적인 QC 보고서를 생성하며, 필요한 경우 paired-end 리드 데이터를 사용하도록 워크플로우를 조정하는 방법을 학습했습니다.

### 다음 단계

축하합니다, Nextflow For RNAseq 단기 과정을 완료하셨습니다! 성공을 축하하고 충분한 휴식을 취하세요!

다음으로, 이 교육 과정에 대한 경험에 관한 매우 짧은 설문조사를 완료해 주시기 바랍니다. 그런 다음 추가 교육 자료 및 유용한 링크가 있는 페이지로 안내해 드리겠습니다.
