...
이 과정의 이번 파트에서는 간단한 워크플로우를 한 단계 더 발전시켜 임의의 수의 샘플을 처리할 수 있는 강력한 배치 자동화 도구로 전환합니다.
그리고 그 과정에서 더 최근 연구에서 일반적으로 사용되는 paired-end 데이터를 처리하도록 업데이트합니다.
...
이 과정의 이번 파트에서는 워크플로우를 다음과 같이 확장합니다:
...
`HISAT2_ALIGN` 실행 후 워크플로우 본문에 다음 줄을 추가합니다:
...
`val output_name` 입력은 보고서 파일 이름을 제어하는 string입니다.
...
publish 선언에 MultiQC 출력을 추가하고 어디로 갈지 설정해야 합니다.
...
캐시된 프로세스 실행 후에 단일 MULTIQC 실행이 추가되었습니다.
...
입력 처리가 paired-end 데이터용으로 설정되었습니다.
...
- 입력이 단일 경로에서 두 경로의 튜플로 변경됩니다
- HISAT2 명령이 `-U`(unpaired)에서 `-1`과 `-2`(paired) 읽기 인자로 변경됩니다
...
이전 실행과 캐시가 공유되지 않으므로 `-resume`을 사용하지 않습니다. 처리할 데이터가 이전보다 두 배 더 많지만 1분 이내에 완료되어야 합니다.
