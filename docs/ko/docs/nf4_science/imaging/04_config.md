...
모든 nf-core 파이프라인에는 사용할 수 있는 여러 기본 프로파일이 함께 제공됩니다.
...
로컬 `nextflow.config` 파일에서 docker 구성을 비활성화하고 프로파일을 이해했으므로, `-profile` 플래그를 사용하여 파이프라인을 다시 실행합니다.
...
- nf-core 도구에 대해 더 깊이 이해하려면 [Build with nf-core](../../nfcore_build/index.md)를 살펴보십시오
- [Side Quests](../../side_quests/index.md)에서 더 심화된 주제를 살펴보십시오
