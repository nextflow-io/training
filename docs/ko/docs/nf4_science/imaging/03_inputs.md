...
파트 2에서는 명령줄에서 여러 매개변수를 사용하여 molkart를 실행했습니다.
...
다음 달에 동일한 분석을 다시 실행해야 한다면 어떻게 할까요?
협력자가 여러분의 정확한 설정을 사용하고 싶다면 어떻게 할까요?

### 1.2. 해결책: 매개변수 파일 사용

`params.yaml`이라는 파일을 생성하세요:
...
이것으로 끝입니다! 매개변수 파일은 정확한 구성을 문서화하고 재실행하거나 공유하기 쉽게 만듭니다.

### 1.3. 매개변수 재정의하기

명령줄에서 특정 매개변수를 재정의할 수 있습니다:
...
또한 `-resume` 플래그를 사용하여 이전 실행의 전처리 결과를 재사용하여 시간을 절약할 수 있습니다.
이 패턴을 사용하여 파이프라인의 다양한 변형을 빠르게 테스트할 수 있습니다.

### 핵심 정리

매개변수 파일은 분석을 재현 가능하고 공유하기 쉽게 만듭니다.
실제 분석 작업에는 매개변수 파일을 사용하세요.
...
- `membrane_image`: 막 염색 이미지 (TIFF, 선택 사항)

### 2.2. 파일 경로

샘플시트는 여러 경로 유형을 허용합니다:

- **URL**: Nextflow가 자동으로 다운로드합니다 (위에 표시된 것처럼)
...
이제 샘플시트를 수정하여 이러한 로컬 파일을 참조하도록 하겠습니다:
...
!!! warning "경고"
    ...
    샘플시트의 경로는 샘플시트가 위치한 곳이 아니라 Nextflow를 **실행**하는 위치를 기준으로 한다는 점에 유의하세요.

마지막으로 로컬 파일 경로가 있는 샘플시트로 nf-core/molkart를 한 번 더 실행하겠습니다:

`nextflow run ./molkart -params-file params.yaml -resume`

보시다시피 Nextflow는 파일이 Github에서 다운로드되었을 때와 유사하게 이 실행을 수행합니다. 이것은 Nextflow의 훌륭한 기능 중 하나로, 데이터가 어디에 있든 적절하게 스테이징합니다.
...
입력을 다루었으니, 이제 다양한 컴퓨팅 환경에 맞게 Nextflow 파이프라인을 구성하는 방법을 살펴보겠습니다.
