...
파트 1에서는 간단한 Hello World 워크플로우를 실행하여 Nextflow 실행의 기본 사항을 학습했습니다.
이제 실제 바이오이미징 파이프라인인 **nf-core/molkart**를 실행해 보겠습니다.

이 파이프라인은 Resolve Bioscience의 Molecular Cartography 공간 전사체학 데이터를 처리합니다.
하지만 여기서 학습할 Nextflow 패턴은 모든 nf-core 파이프라인이나 프로덕션 워크플로우에 적용됩니다.

## 1. nf-core 파이프라인 이해하기

파이프라인을 실행하기 전에 nf-core가 무엇이며 워크플로우 실행에 왜 중요한지 이해해 봅시다.

### 1.1. nf-core란 무엇인가요?

[nf-core](https://nf-co.re/)는 커뮤니티 주도의 고품질 Nextflow 파이프라인 모음입니다.
모든 nf-core 파이프라인은 동일한 구조와 규칙을 따르므로, 하나를 실행하는 방법을 배우면 모든 파이프라인을 실행할 수 있습니다.

nf-core 파이프라인의 주요 특징:

- **표준화된 구조**: 모든 파이프라인이 일관된 매개변수 이름과 사용 패턴을 가집니다
- **내장된 테스트 데이터**: 모든 파이프라인에 빠른 검증을 위한 테스트 프로필이 포함되어 있습니다
- **포괄적인 문서**: 상세한 사용 지침과 매개변수 설명을 제공합니다
- **품질 관리**: MultiQC를 사용한 자동화된 QC 리포트를 생성합니다
- **컨테이너 지원**: 재현성을 위해 사전 빌드된 컨테이너를 제공합니다

!!! tip "nf-core에 대해 더 알고 싶으신가요?"

    nf-core 파이프라인 개발에 대한 심화 학습은 [Hello nf-core](../../hello_nf-core/index.md) 교육 과정을 확인하세요.
    처음부터 nf-core 파이프라인을 생성하고 커스터마이징하는 방법을 다룹니다.
...
2. **세포 분할**: 여러 알고리즘 옵션 (Cellpose, Mesmer, ilastik, Stardist)
3. **스팟 할당**: 전사체 스팟을 분할된 세포에 할당
4. **품질 관리**: 포괄적인 QC 리포트 생성
...
시작하기 전에 molkart 저장소를 로컬에 클론하여 코드를 살펴볼 수 있도록 하겠습니다:
...
이 명령은 완전한 파이프라인 소스 코드가 포함된 `molkart/` 디렉토리를 생성합니다.

!!! note "왜 로컬에 클론하나요?"

    일반적으로는 `nextflow run nf-core/molkart -r 1.2.0`을 사용하여 GitHub에서 직접 nf-core 파이프라인을 실행합니다.
    Nextflow가 자동으로 요청한 파이프라인 버전을 `$HOME/.nextflow/assets/nf-core/molkart`에 다운로드하고 거기서 실행합니다.
    하지만 이 교육에서는 코드를 더 쉽게 살펴볼 수 있도록 파이프라인을 다른 로컬 디렉토리에 클론합니다.

### 2.1. 컨테이너 요구사항 이해하기

전체 파이프라인을 실행하기 전에 컨테이너가 nf-core 파이프라인에 왜 필수적인지 알아봅시다.
...
!!! Warning "이 명령은 실패합니다 - 의도적입니다!"

    컨테이너가 왜 필요한지 보여주기 위해 의도적으로 컨테이너 없이 실행합니다.
...
`command not found` 오류(종료 상태 127)는 Nextflow가 `duplicate_finder.py`를 실행하려 했지만 시스템에서 찾을 수 없다는 의미입니다.
이는 다음과 같은 이유 때문입니다:

1. 파이프라인은 특수한 바이오인포매틱스 소프트웨어가 설치되어 있을 것으로 예상합니다
2. 이러한 도구들(`duplicate_finder.py`, `apply_clahe.dask.py` 등)은 표준 Linux 배포판에 포함되어 있지 않습니다
3. 컨테이너가 없으면 Nextflow는 로컬 머신에서 직접 명령을 실행하려고 시도합니다

**이러한 도구들은 어디서 제공되어야 하나요?**

프로세스 모듈 중 하나를 살펴보고 소프트웨어 요구사항을 어떻게 선언하는지 확인해 봅시다.
...
!!! Tip "컨테이너가 중요한 이유"
    ...
    대부분의 nf-core 파이프라인은 컨테이너화(Docker, Singularity, Podman 등)를 **필수로** 요구합니다:

    - 표준 환경에서 사용할 수 없는 특수한 바이오인포매틱스 소프트웨어를 사용하기 때문입니다
    - 컨테이너는 재현성을 보장합니다 - 정확히 동일한 소프트웨어 버전이 어디서나 실행됩니다
...
파이프라인이 따르는 nf-core 규칙 덕분에 이 출력이 Hello World 예제보다 더 상세합니다:

- 파이프라인이 버전과 로고를 표시합니다
- 구성 매개변수가 표시됩니다
- 여러 프로세스가 병렬로 실행됩니다 (여러 프로세스 줄로 표시됨)
- 프로세스 이름에 전체 모듈 경로가 포함됩니다 (예: `NFCORE_MOLKART:MOLKART:MINDAGAP_MINDAGAP`)

### 2.4. 프로세스 실행 이해하기

executor 줄 `executor > local (22)`는 다음을 알려줍니다:
...
- **해시** (`[1a/2b3c4d]`): 작업 디렉토리 식별자 (이전과 동일)
- **프로세스 이름**: 전체 모듈 경로와 프로세스 이름
- **입력 식별자**: 괄호 안의 샘플 이름
- **진행률**: 완료 백분율과 개수 (예: `1 of 1 ✔`)
...
각 하위 디렉토리에는 파이프라인의 특정 단계에서 나온 출력이 포함됩니다:

- **mindagap/**: MindaGap 전처리 단계에서 그리드가 채워진 이미지
- **clahe/**: CLAHE 전처리에서 대비가 향상된 이미지
...
- **anndata/**: 세포별 전사체 행렬과 공간 좌표가 포함된 AnnData 객체
- **molkartqc/**: 스팟 할당에 대한 품질 관리 메트릭
- **multiqc/**: 포괄적인 품질 관리 리포트
- **pipeline_info/**: 실행 리포트 및 로그

### 3.2. MultiQC 리포트 검토하기

MultiQC 리포트는 모든 파이프라인 단계의 품질 메트릭을 집계한 포괄적인 HTML 파일입니다.

파일 브라우저에서 리포트를 열고 "Show Preview" 버튼을 클릭하여 VS Code에서 직접 렌더링된 내용을 확인하세요.
...
!!! Tip
    ...
    MultiQC 리포트는 일반적으로 모든 nf-core 파이프라인에 포함됩니다.
    항상 파이프라인 실행 및 데이터 품질에 대한 높은 수준의 개요를 제공합니다.

### 3.3. 세포별 전사체 테이블 검토하기

가장 중요한 과학적 출력은 세포별 전사체 카운트 테이블입니다.
...
이 테스트 데이터셋에서는 1개의 샘플만 실행했지만, 실제 실험에서는 각 샘플에 대해 이러한 테이블이 생성됩니다.
Nextflow가 여러 분할 방법을 병렬로 처리하여 결과를 쉽게 비교할 수 있게 해주는 것을 주목하세요.

### 3.4. 실행 리포트 보기

Nextflow는 여러 실행 리포트를 자동으로 생성합니다.
...
- **execution_timeline.html**: 프로세스 실행의 간트 차트
- **execution_trace.txt**: 상세한 작업 실행 메트릭
- **pipeline_dag.html**: 워크플로우 구조를 보여주는 방향성 비순환 그래프

실행 리포트를 열어 리소스 사용량을 확인하세요:
...
!!! Tip
    ...
    이러한 리포트는 리소스 할당을 최적화하고 성능 문제를 해결하는 데 매우 유용합니다.

### 핵심 정리

파이프라인 출력을 찾고, 품질 관리 리포트를 검토하고, 실행 메트릭에 접근하는 방법을 알게 되었습니다.

### 다음 단계

작업 디렉토리와 Nextflow가 중간 파일을 관리하는 방법을 학습합니다.
...
12개의 작업이 있는 이 파이프라인의 경우 12개의 작업 하위 디렉토리가 있습니다.
...
콘솔 출력에서 분할 프로세스 해시 중 하나(예: `[3m/4n5o6p]`)를 선택하고 내부를 살펴보세요:
...
- **.command.\* 파일**: Nextflow 실행 스크립트 및 로그 (이전과 동일)
...
- 작업당 여러 입력 및 출력 파일이 있습니다

!!! Tip

    프로세스가 실패하면 작업 디렉토리로 이동하여 `.command.err`에서 오류 메시지를 확인하고, 문제를 디버깅하기 위해 `.command.sh`를 수동으로 다시 실행할 수도 있습니다.

### 4.3. 작업 디렉토리 정리

작업 디렉토리는 여러 파이프라인 실행을 거치면서 상당히 커질 수 있습니다.
파트 1에서 학습한 것처럼 `nextflow clean`을 사용하여 이전 실행의 작업 디렉토리를 제거할 수 있습니다.

하지만 대용량 중간 파일이 있는 nf-core 파이프라인의 경우 정기적으로 정리하는 것이 특히 중요합니다.

### 핵심 정리

nf-core 파이프라인이 작업 디렉토리를 구성하는 방법과 디버깅을 위해 개별 작업을 검사하는 방법을 이해하게 되었습니다.

### 다음 단계

Nextflow 캐시와 실패한 파이프라인 실행을 재개하는 방법을 학습합니다.
...
2. 입력, 코드, 매개변수가 동일하면 캐시된 결과를 재사용합니다
3. 변경되거나 실패한 작업만 다시 실행합니다

이는 실행 후반부에 실패가 발생할 수 있는 장시간 실행 파이프라인에 필수적입니다.
...
- 리소스 제한으로 파이프라인이 실패한 경우 (메모리 부족, 시간 제한 초과)
- 업스트림 단계를 다시 실행하지 않고 다운스트림 프로세스를 수정해야 하는 경우
- 데이터 다운로드 중 네트워크 연결이 끊어진 경우
- 계산을 다시 수행하지 않고 추가 출력을 추가하려는 경우

!!! Warning

    재개는 입력 데이터, 파이프라인 코드 또는 매개변수를 변경하지 않은 경우에만 작동합니다.
    이 중 하나라도 변경하면 Nextflow는 영향을 받는 작업을 올바르게 다시 실행합니다.
...
이제 테스트 데이터로 nf-core/molkart를 실행할 수 있으므로, 자신의 데이터셋에 맞게 구성하는 방법을 학습할 준비가 되었습니다.
