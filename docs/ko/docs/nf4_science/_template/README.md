...
4. 문서와 스크립트에서 모든 `{PLACEHOLDER}` 값을 검색하여 콘텐츠로 교체하세요.
...
| `{TOOL_A_MODULE}` / `{TOOL_B_MODULE}` | 모듈 파일명 (.nf 제외)     | samtools_index / gatk_haplotypecaller               |
...
| 플레이스홀더            | 설명                             |
| ----------------------- | -------------------------------- |
| `{PART2_SUMMARY}`       | Part 2 범위의 한 줄 요약         |
| `{AGGREGATION_SUMMARY}` | 집계 단계의 한 줄 요약           |
...
2. **Part 2 (02_single_sample.md)**: 명령을 Nextflow로 적용; 단일 샘플, 그 다음 배치
3. **Part 3 (03_multi_sample.md)**: 채널 연산자를 사용하여 다중 샘플 집계 추가

### 주요 규칙

- `hl_lines`와 함께 **Before/After 탭 코드 블록**을 사용하여 코드 변경 사항 표시
...
- `---` 수평선을 사용하여 주요 번호 섹션 구분
- 학습자가 채울 수 있는 스켈레톤 파일 제공 (워크플로우 + 모듈)
