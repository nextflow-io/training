...
환경이 로드되는 동안 계속 읽을 수 있도록 새 브라우저 탭이나 창에서 교육 환경을 여는 것을 권장합니다(사용 중인 운영체제에 따라 마우스 오른쪽 버튼 클릭, ctrl-클릭 또는 cmd-클릭을 사용하십시오).
과정을 진행하려면 이 지침을 동시에 열어 두어야 합니다.
...
코드스페이스가 실행되면, 교육을 시작하기 전에 두 가지를 해야 합니다: 이 특정 과정의 작업 디렉토리를 설정하고 제공된 자료를 살펴보는 것입니다.
...
VSCode가 이 디렉토리에 집중하도록 설정하여 파일 탐색기(File Explorer) 사이드바에 관련 파일만 표시되도록 할 수 있습니다:
