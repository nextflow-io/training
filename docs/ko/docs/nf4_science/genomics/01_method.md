...
변이 호출(Variant calling)은 참조 게놈(reference genome)에 대한 게놈 서열의 변이를 식별하는 것을 목표로 하는 게놈 분석 방법입니다.
여기서는 전체 게놈 시퀀싱 데이터에서 짧은 생식세포 변이, 즉 SNP와 indel을 호출하기 위해 설계된 도구와 방법을 사용할 것입니다.

![GATK 파이프라인](img/gatk-pipeline.png)

완전한 변이 호출 파이프라인은 일반적으로 참조에 대한 매핑(때로는 게놈 정렬이라고도 함)과 변이 필터링 및 우선순위 지정을 포함하여 많은 단계를 포함합니다.
간단하게 하기 위해, 이 과정에서는 변이 호출 부분에만 집중할 것입니다.

### 방법

전체 게놈 시퀀싱 샘플에 변이 호출을 적용하여 생식세포 SNP와 indel을 식별하는 두 가지 방법을 보여드리겠습니다.
먼저 각 샘플에서 독립적으로 변이를 호출하는 간단한 **샘플별 접근 방식**부터 시작하겠습니다.
그런 다음 여러 샘플을 함께 분석하여 더 정확하고 유익한 결과를 생성하는 더 정교한 **공동 호출 접근 방식**을 보여드리겠습니다.

두 접근 방식 중 하나에 대한 워크플로우 코드를 작성하기 전에, 먼저 일부 테스트 데이터에서 명령을 수동으로 시도해 보겠습니다.

### 데이터셋

다음 데이터와 관련 리소스를 제공합니다:

- 인간 염색체 20의 작은 영역(hg19/b37에서)과 그 부속 파일(인덱스 및 서열 사전)로 구성된 **참조 게놈**.
- 가족 삼인조(어머니, 아버지, 아들)에 해당하는 **세 개의 전체 게놈 시퀀싱 샘플**로, 파일 크기를 작게 유지하기 위해 염색체 20의 작은 데이터 조각으로 부분 집합화되었습니다.
  이것은 이미 참조 게놈에 매핑된 Illumina 단일 리드 시퀀싱 데이터로, [BAM](https://samtools.github.io/hts-specs/SAMv1.pdf) 형식(Binary Alignment Map, SAM의 압축 버전, Sequence Alignment Map)으로 제공됩니다.
- **게놈 간격 목록**, 즉 샘플에 변이 호출에 적합한 데이터가 있는 게놈의 좌표로, BED 형식으로 제공됩니다.

### 소프트웨어

관련된 두 가지 주요 도구는 서열 정렬 파일을 조작하기 위해 널리 사용되는 툴킷인 [Samtools](https://www.htslib.org/)와 Broad Institute에서 개발한 변이 발견을 위한 도구 세트인 [GATK](https://gatk.broadinstitute.org/)(Genome Analysis Toolkit)입니다.

이러한 도구는 GitHub Codespaces 환경에 설치되어 있지 않으므로 컨테이너를 통해 사용할 것입니다([Hello Containers](../../hello_nextflow/05_hello_containers.md) 참조).

!!! note "참고"

     `pwd`를 입력할 때 표시되는 경로의 마지막 부분이 `genomics`가 되도록 `nf4-science/genomics` 디렉토리에 있는지 확인하세요.

---

## 1. 샘플별 변이 호출

샘플별 변이 호출은 각 샘플을 독립적으로 처리합니다: 변이 호출기는 한 번에 하나의 샘플에 대한 시퀀싱 데이터를 검사하고 샘플이 참조와 다른 위치를 식별합니다.

이 섹션에서는 샘플별 변이 호출 접근 방식을 구성하는 두 가지 명령을 테스트합니다: Samtools로 BAM 파일 인덱싱과 GATK HaplotypeCaller로 변이 호출.
이것들은 이 과정의 파트 2에서 Nextflow 워크플로우로 래핑할 명령입니다.
...
먼저 하나의 샘플에서만 두 명령을 테스트합니다.

### 1.1. Samtools로 BAM 입력 파일 인덱싱

인덱스 파일은 생물정보학 파일 형식의 일반적인 기능입니다. 인덱스 파일은 GATK와 같은 도구가 전체 파일을 읽지 않고도 데이터의 부분 집합에 액세스할 수 있도록 하는 메인 파일의 구조에 대한 정보를 포함합니다.
이것은 이러한 파일이 얼마나 커질 수 있는지 때문에 중요합니다.

BAM 파일은 종종 인덱스 없이 제공되므로 많은 분석 워크플로우의 첫 번째 단계는 `samtools index`를 사용하여 인덱스를 생성하는 것입니다.

Samtools 컨테이너를 가져와서 대화형으로 실행하고 BAM 파일 중 하나에서 `samtools index` 명령을 실행할 것입니다.

#### 1.1.1. Samtools 컨테이너 가져오기

`docker pull` 명령을 실행하여 Samtools 컨테이너 이미지를 다운로드합니다:
...
#### 1.1.2. Samtools 컨테이너를 대화형으로 실행

컨테이너를 대화형으로 실행하려면 `-it` 플래그와 함께 `docker run`을 사용합니다.
`-v ./data:/data` 옵션은 로컬 `data` 디렉토리를 컨테이너에 마운트하여 도구가 입력 파일에 액세스할 수 있도록 합니다.
...
이제 원본 BAM 입력 파일과 같은 디렉토리에 `reads_mother.bam.bai`라는 파일이 표시됩니다.
...
GATK 컨테이너를 가져와서 대화형으로 실행하고 방금 인덱싱한 BAM 파일에서 `gatk HaplotypeCaller` 명령을 실행할 것입니다.

#### 1.2.1. GATK 컨테이너 가져오기

`docker pull` 명령을 실행하여 GATK 컨테이너 이미지를 다운로드합니다:
...
??? success "명령 출력"
    ...
    일부 레이어는 이전에 가져온 Samtools 컨테이너 이미지와 공유되기 때문에 `Already exists`로 표시됩니다.
...
두 컨테이너 이미지가 대부분의 레이어를 공유하기 때문에 첫 번째 가져오기보다 빠를 것입니다.

#### 1.2.2. GATK 컨테이너를 대화형으로 실행

Samtools에서 했던 것처럼 데이터 디렉토리가 마운트된 상태로 GATK 컨테이너를 대화형으로 실행합니다.
...
그러나 인덱스 파일의 경로를 지정할 필요는 없습니다. 도구는 확립된 명명 및 공동 배치 규칙에 따라 같은 디렉토리에서 자동으로 찾습니다.
...
??? success "명령 출력"
    ...
    도구는 자세한 로깅 출력을 생성합니다. 강조 표시된 줄은 성공적인 완료를 확인합니다.
...
출력 파일 `reads_mother.vcf`는 컨테이너의 작업 디렉토리 내부에 생성되므로 출력 파일 경로를 변경하지 않는 한 VS Code 파일 탐색기에 표시되지 않습니다.
그러나 작은 테스트 파일이므로 `cat`을 사용하여 열고 내용을 볼 수 있습니다.
파일의 시작 부분까지 스크롤하면 많은 줄의 메타데이터로 구성된 헤더와 그 뒤에 한 줄에 하나씩 변이 호출 목록이 있습니다.
...
출력 VCF 파일에는 GATK에 의해 자동으로 생성된 `reads_mother.vcf.idx`라는 인덱스 파일이 함께 제공됩니다.
BAM 인덱스 파일과 동일한 기능을 가지며, 도구가 전체 파일을 로드하지 않고도 데이터의 부분 집합을 찾고 검색할 수 있도록 합니다.
...
방금 사용한 변이 호출 접근 방식은 샘플별로 변이 호출을 생성합니다.
각 샘플의 변이를 개별적으로 보는 것은 괜찮지만 제한된 정보를 제공합니다.
여러 샘플에서 변이 호출이 어떻게 다른지 보는 것이 종종 더 흥미롭습니다.
GATK는 이 목적을 위해 공동 변이 호출이라는 대체 방법을 제공합니다.

공동 변이 호출은 각 샘플에 대해 GVCF(Genomic VCF)라는 특수한 종류의 변이 출력을 생성한 다음 모든 샘플의 GVCF 데이터를 결합하고 '공동 유전자형 결정' 통계 분석을 실행하는 것을 포함합니다.

![공동 분석](img/joint-calling.png)

샘플의 GVCF의 특별한 점은 프로그램이 변이의 증거를 발견한 위치뿐만 아니라 게놈의 대상 영역에 있는 모든 위치에 대한 서열 데이터 통계를 요약하는 레코드를 포함한다는 것입니다.
이것은 공동 유전자형 결정 계산에 중요합니다([추가 읽기](https://gatk.broadinstitute.org/hc/en-us/articles/360035890431-The-logic-of-joint-calling-for-germline-short-variants)).

GVCF는 방금 테스트한 것과 동일한 도구인 GATK HaplotypeCaller에 의해 추가 매개변수(`-ERC GVCF`)와 함께 생성됩니다.
GVCF를 결합하는 것은 샘플별 호출을 데이터 저장소(데이터베이스와 유사)로 결합하는 GATK GenomicsDBImport로 수행됩니다.
실제 '공동 유전자형 결정' 분석은 GATK GenotypeGVCFs로 수행됩니다.

여기서는 GVCF를 생성하고 공동 유전자형 결정을 실행하는 데 필요한 명령을 테스트합니다.
이것들은 이 과정의 파트 3에서 Nextflow 워크플로우로 래핑할 명령입니다.
...
4. 결합된 GVCF 데이터 저장소에서 공동 유전자형 결정을 실행하여 코호트 수준 VCF 생성
...
이제 GATK HaplotypeCaller가 처리할 수 있도록 세 샘플을 모두 인덱싱해야 합니다.

#### 2.1.1. Samtools 컨테이너를 대화형으로 실행

이미 Samtools 컨테이너 이미지를 가져왔으므로 직접 실행할 수 있습니다:

```bash
docker run -it -v ./data:/data community.wave.seqera.io/library/samtools:1.20--b5dfbd93de237464
```

프롬프트가 변경되어 컨테이너 내부에 있음을 나타내며, 이전과 같이 데이터 디렉토리가 마운트됩니다.

#### 2.1.2. 세 샘플 모두에서 인덱싱 명령 실행

세 개의 BAM 파일 각각에서 인덱싱 명령을 실행합니다:
...
공동 유전자형 결정 단계를 실행하려면 세 샘플 모두에 대한 GVCF가 필요합니다.

#### 2.2.1. GATK 컨테이너를 대화형으로 실행

이미 GATK 컨테이너 이미지를 가져왔으므로 직접 실행할 수 있습니다:
...
내용을 보기 위해 `cat`을 실행하면 섹션 1에서 생성한 동등한 VCF보다 훨씬 길다는 것을 알 수 있습니다. 파일의 시작 부분까지 스크롤할 수도 없으며 대부분의 줄은 VCF에서 본 것과 상당히 다르게 보입니다.
...
이것들은 변이 호출기가 변이의 증거를 발견하지 못한 비변이 영역을 나타내므로 변이가 없다는 것에 대한 신뢰 수준을 설명하는 일부 통계를 캡처했습니다.
이것은 두 가지 매우 다른 경우를 구별할 수 있게 합니다: (1) 샘플이 동형접합-참조임을 보여주는 양질의 데이터가 있고, (2) 어느 쪽으로든 결정을 내리기에 충분한 양질의 데이터가 없습니다.

GVCF에는 일반적으로 이러한 비변이 줄이 많이 있으며, 그 사이에 소수의 변이 레코드가 뿌려져 있습니다.
실제 변이 호출을 찾기 위해 GVCF에서 `head -176`을 실행하여 파일의 처음 176줄만 로드해 보세요.
...
두 번째 줄은 파일의 첫 번째 변이 레코드를 보여주며, 이는 이전에 본 VCF 파일의 첫 번째 변이에 해당합니다.

원본 VCF와 마찬가지로 출력 GVCF 파일에도 `reads_mother.g.vcf.idx`라는 인덱스 파일이 함께 제공됩니다.

#### 2.2.3. 다른 두 샘플에서 프로세스 반복

아래 명령을 차례로 실행하여 나머지 두 샘플에 대한 GVCF를 생성합니다.
...
이제 모든 GVCF가 있으므로 샘플 코호트에 대한 변이 호출을 생성하는 공동 유전자형 결정 접근 방식을 시도할 수 있습니다.
모든 GVCF의 데이터를 데이터 저장소로 결합한 다음 공동 유전자형 결정 분석을 실행하여 공동 호출된 변이의 최종 VCF를 생성하는 2단계 방법입니다.

#### 2.3.1. 모든 샘플별 GVCF 결합

이 첫 번째 단계는 GenomicsDBImport라는 또 다른 GATK 도구를 사용하여 모든 GVCF의 데이터를 GenomicsDB 데이터 저장소로 결합합니다.
...
이 단계의 출력은 여러 다른 파일의 형태로 결합된 변이 데이터를 보유하는 추가 중첩 디렉토리 세트를 포함하는 디렉토리입니다.
둘러볼 수 있지만 이 데이터 저장소 형식은 사람이 직접 읽도록 의도되지 않았다는 것을 빠르게 알 수 있습니다.

!!! note "참고"

    GATK에는 필요에 따라 데이터 저장소에서 변이 호출 데이터를 검사하고 추출할 수 있는 도구가 포함되어 있습니다.

#### 2.3.2. 공동 유전자형 결정 분석 실행

이 두 번째 단계는 GenotypeGVCFs라는 또 다른 GATK 도구를 사용하여 코호트의 모든 샘플에서 사용 가능한 데이터를 고려하여 변이 통계와 개별 유전자형을 재계산합니다.
...
또 다른 합리적으로 작은 파일이므로 이 파일을 `cat`하여 내용을 보고 처음 몇 개의 변이 줄을 찾기 위해 위로 스크롤할 수 있습니다.
...
이것은 이전에 생성한 VCF와 유사해 보이지만 이번에는 세 샘플 모두에 대한 유전자형 수준 정보가 있습니다.
파일의 마지막 세 열은 알파벳 순서로 나열된 샘플의 유전자형 블록입니다.

테스트 가족 삼인조의 첫 번째 변이에 대해 호출된 유전자형을 보면 아버지는 이형접합-변이(`0/1`)이고 어머니와 아들은 모두 동형접합-변이(`1/1`)입니다.

이것이 궁극적으로 데이터셋에서 추출하려는 정보입니다!
...
컨테이너에서 Samtools 인덱싱 및 GATK 변이 호출 명령을 테스트하는 방법을 알게 되었습니다. 여기에는 GVCF를 생성하고 여러 샘플에서 공동 유전자형 결정을 실행하는 방법이 포함됩니다.

### 다음 단계

동일한 명령을 컨테이너를 사용하여 작업을 실행하는 워크플로우로 래핑하는 방법을 학습합니다.
