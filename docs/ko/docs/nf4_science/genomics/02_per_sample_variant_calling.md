...
시작점으로, 워크플로우 파일 `genomics.nf`와 두 개의 모듈 파일 samtools_index.nf 및 gatk_haplotypecaller.nf를 제공합니다. 이 파일들은 워크플로우의 주요 부분과 모듈의 구조를 개략적으로 보여줍니다.
...
이는 Nextflow 생태계에서 일반적인 관례입니다([Hello Config](../../hello_nextflow/06_hello_config.md) 참조).
