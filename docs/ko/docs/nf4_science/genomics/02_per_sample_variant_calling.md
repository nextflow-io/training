...
다음으로, 동일한 명령을 Nextflow 워크플로우로 적용하겠습니다.
...
이는 [파트 1: 방법 개요](./01_method.md#1-per-sample-variant-calling)의 첫 번째 섹션에서 컨테이너에서 수동으로 실행했던 단계를 자동화합니다.

시작점으로, 워크플로우의 주요 부분을 개요로 제시하는 워크플로우 파일 `genomics.nf`와 각 프로세스의 구조를 개요로 제시하는 두 개의 모듈 파일 `samtools_index.nf` 및 `gatk_haplotypecaller.nf`를 제공합니다.
...
메인 워크플로우 파일 `genomics.nf`의 `Pipeline parameters` 섹션 아래에 `input`이라는 CLI 매개변수를 선언하세요.
...
`nextflow.config`에 `profiles` 블록을 추가하고 `input` 매개변수를 테스트 BAM 파일 중 하나로 설정하는 `test` 프로파일을 추가하세요.
...
다음으로, 이 입력에 대해 인덱싱을 실행할 프로세스를 생성해야 합니다.
...
섹션 1.1.2에서 `input`에 대해 했던 것처럼, `nextflow.config`의 테스트 프로파일에 보조 파일에 대한 기본값을 추가하세요:
...
다음으로, 워크플로우에서 사용할 이러한 파일 경로를 로드하는 변수를 생성해야 합니다.
...
Hello Nextflow 교육 시리즈에서 `*.out` 구문을 인식할 수 있어야 합니다. Nextflow에 `SAMTOOLS_INDEX`가 출력한 채널을 가져와 `GATK_HAPLOTYPECALLER` process 실행에 연결하도록 지시하고 있습니다.
...
다음으로, Nextflow에 새 출력을 어디에 둘지 알려줘야 합니다.
...
콘솔 출력에 나열된 실패한 `GATK_HAPLOTYPECALLER` process 실행의 작업 디렉토리 내부를 살펴보세요.
...
뭐라고요? Nextflow가 이 process 실행의 작업 디렉토리에 인덱스 파일을 스테이징했지만 잘못된 것입니다. 어떻게 이런 일이 발생할 수 있을까요?

#### 3.3.2. [view() 연산자](https://www.nextflow.io/docs/latest/reference/operator.html#view)를 사용하여 채널 내용 검사

워크플로우 본문의 `GATK_HAPLOTYPECALLER` process 실행 전에 다음 두 줄을 추가하여 채널의 내용을 확인하세요:
...
다음으로, 프로세스 호출과 게시 대상에서 새 튜플 구조를 반영하도록 워크플로우를 업데이트해야 합니다.
...
이제 워크플로우는 임의의 수의 샘플을 안정적으로 처리하지만, 설정에서 개별적으로 나열하는 것은 확장성이 좋지 않습니다.
...
### 4.1. 입력 파일 경로를 나열하는 제공된 CSV 파일 검사

`data/` 디렉토리에서 찾을 수 있는 `samplesheet.csv`라는 입력 파일 경로를 나열하는 CSV 파일을 이미 만들었습니다.
...
!!! warning "경고"
    ...
    제공된 교육 환경에서 실행하지 않는 경우 시스템에 맞게 경로를 업데이트해야 합니다.

### 4.2. 매개변수 및 테스트 프로파일 업데이트

개별 샘플을 나열하는 대신 `samplesheet.csv` 파일을 가리키도록 `input` 매개변수를 전환하세요.
...
[Hello Nextflow의 파트 2](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file)에서 사용한 것과 동일한 패턴을 사용하여 이를 수행할 수 있습니다: [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) 연산자를 적용하여 파일을 분석한 다음 `map` 연산을 사용하여 각 행에서 파일 경로를 추출합니다.

=== "후"

    ```groovy title="genomics.nf" linenums="24" hl_lines="1-4"
        // CSV 파일에서 입력 채널 생성 (입력 파일 경로 목록)
        reads_ch = channel.fromPath(params.input)
...
이를 통해 `map` 연산에서 열을 이름으로 참조할 수 있습니다: `#!groovy row.reads_bam`은 각 행의 `reads_bam` 열에서 파일 경로를 추출합니다.
...
더 일반적으로, 유전체학 파일 형식과 도구 요구 사항의 특이성을 고려하여 실제 작업을 수행하는 간단한 유전체학 파이프라인을 구축하기 위해 필수 Nextflow 구성 요소와 로직을 사용하는 방법을 학습했습니다.

### 다음 단계

휴식을 취하세요! 많은 내용이었습니다.

기분이 상쾌해지면 [파트 3](./03_joint_calling.md)으로 이동하세요. 여기서는 이 간단한 샘플별 변이 호출 워크플로우를 데이터에 공동 변이 호출을 적용하도록 변환하는 방법을 학습합니다.
