...
개발 과정을 더욱 교육적으로 만들기 위해 이를 네 단계로 나누었습니다:

1. **BAM 파일에서 Samtools index를 실행하는 단일 단계 워크플로우 작성.**
   모듈 생성, 가져오기 및 워크플로우에서 호출하는 방법을 다룹니다.
2. **인덱싱된 BAM 파일에서 GATK HaplotypeCaller를 실행하는 두 번째 프로세스 추가.**
...
각 단계는 워크플로우 개발의 특정 측면에 중점을 둡니다.

---

## 1. BAM 파일에서 Samtools index를 실행하는 단일 단계 워크플로우 작성

이 첫 번째 단계는 기본 사항에 중점을 둡니다: BAM 파일을 로드하고 인덱스를 생성합니다.
...
`nextflow.config`에 `profiles` 블록을 추가하고 `reads_bam` 매개변수를 테스트 BAM 파일 중 하나로 설정하는 `test` 프로필을 추가하세요.
...
여기서는 워크플로우 스크립트가 위치한 디렉토리를 가리키는 내장 Nextflow 변수인 `${projectDir}`를 사용하고 있습니다.
...
이제 이 입력에서 인덱싱을 실행할 프로세스를 생성해야 합니다.

### 1.2. 인덱싱 프로세스 작성 및 워크플로우에서 호출

모듈 파일에 프로세스 정의를 작성하고, include 문을 사용하여 워크플로우로 가져오고, 입력에서 호출해야 합니다.
...
위에 제공된 정보를 사용하여 직접 프로세스 정의를 작성한 다음, 아래 "후" 탭의 해결책과 비교하여 작업을 확인하세요.
...
#### 1.2.3. 입력에서 인덱싱 프로세스 호출

이제 워크플로우 블록에 `SAMTOOLS_INDEX` 호출을 추가하고 입력 채널을 인수로 전달하겠습니다.
...
이제 워크플로우가 입력을 로드하고 그것에 대해 인덱싱 프로세스를 실행합니다.
다음으로 출력이 게시되는 방법을 구성해야 합니다.

### 1.3. 출력 처리 구성

게시할 프로세스 출력을 선언하고 어디로 가야 하는지 지정해야 합니다.

#### 1.3.1. `publish:` 섹션에서 출력 선언

워크플로우 블록 내부의 `publish:` 섹션은 어떤 프로세스 출력을 게시할지 선언합니다.
...
`bam/` 하위 디렉토리에 게시하는 `bam_index`에 대한 대상을 추가하겠습니다.
...
테스트 프로필에 설정된 기본값을 사용하고 명령줄에 경로를 작성하지 않도록 `-profile test`로 실행할 수 있습니다.
...
인덱싱 프로세스의 출력을 가져와서 변이 호출을 실행하는 데 사용하는 두 번째 단계를 추가합니다.
...
이들에 대한 매개변수를 선언하고, 테스트 프로필에 기본값을 추가하고, 이들을 로드할 변수를 생성해야 합니다.
...
이전과 마찬가지로 인라인이 아닌 테스트 프로필을 통해 기본값을 제공합니다.

#### 2.1.2. 테스트 프로필에 보조 파일 기본값 추가

섹션 1.1.2에서 `reads_bam`에 대해 했던 것처럼, `nextflow.config`의 테스트 프로필에 보조 파일에 대한 기본값을 추가하세요:
...
모듈 파일에 프로세스 정의를 작성하고, include 문을 사용하여 워크플로우로 가져오고, 입력 reads와 인덱싱 단계의 출력 및 보조 파일에서 호출해야 합니다.
...
마찬가지로, Nextflow가 후속 단계를 위해 추적하도록 출력 VCF의 인덱스 파일(`"${input_bam}.vcf.idx"`)을 명시적으로 나열합니다.
...
Hello Nextflow 교육 시리즈에서 `*.out` 구문을 인식할 수 있어야 합니다. Nextflow에게 `SAMTOOLS_INDEX`가 출력한 채널을 가져와서 `GATK_HAPLOTYPECALLER` 프로세스 호출에 연결하도록 지시하고 있습니다.
...
게시 선언에 새 출력을 추가하고 어디로 가는지 구성해야 합니다.
...
VCF와 그 인덱스는 둘 다 `vcf/` 하위 디렉토리로 가는 별도의 대상으로 게시됩니다.
...
단일 샘플에서 처리를 자동화할 수 있는 워크플로우를 갖는 것은 좋지만, 1000개의 샘플이 있다면 어떻게 될까요?
...
여러 샘플에서 실행하려면 단일 파일 경로 대신 파일 경로 배열을 제공하도록 테스트 프로필을 업데이트하세요.
이는 다중 샘플 실행을 테스트하는 빠른 방법입니다. 다음 단계에서는 입력 파일을 사용하는 더 확장 가능한 접근 방식으로 전환하겠습니다.

먼저, 배열은 타입이 지정된 선언을 사용할 수 없으므로 매개변수 선언에서 타입 주석을 주석 처리하세요:
...
그런 다음 세 개의 샘플을 모두 나열하도록 테스트 프로필을 업데이트하세요:
...
이제 세 개의 테스트 샘플 모두에서 실행되도록 플러밍이 설정되었으므로 워크플로우를 실행해 보세요.
...
음, 이상하네요. 워크플로우의 첫 번째 단계에서 BAM 파일을 명시적으로 인덱싱했는데 말이죠. 플러밍에 문제가 있을 수 있을까요?
...
이 디렉토리에 나열된 BAM 파일과 BAM 인덱스의 이름에 특히 주의를 기울이세요: `reads_son.bam`과 `reads_father.bam.bai`.
...
채널의 내용을 보기 위해 `GATK_HAPLOTYPECALLER` 프로세스 호출 전에 워크플로우 본문에 다음 두 줄을 추가하세요:
...
!!! note
    ...
    결과적으로 해당 출력이 원래 입력이 제공된 것과 다른 순서로 수집될 수 있습니다.

현재 작성된 대로 워크플로우 스크립트는 인덱스 파일이 입력이 제공된 것과 동일한 mother/father/son 순서로 인덱싱 단계에서 나올 것으로 가정합니다.
그러나 이것이 항상 그런 것은 아니므로 때때로(항상은 아니지만) 잘못된 파일이 두 번째 단계에서 쌍을 이룹니다.
...
수정 방법은 각 BAM 파일을 그 인덱스와 함께 튜플로 묶은 다음, 다운스트림 프로세스와 워크플로우 플러밍을 일치하도록 업데이트하는 것입니다.
...
BAM 파일을 포함하도록 `modules/samtools_index.nf`의 출력을 업데이트하세요:
...
#### 3.4.2. 튜플을 받아들이도록 GATK_HAPLOTYPECALLER 모듈의 입력 변경
...
SAMTOOLS_INDEX 출력이 이제 BAM 파일과 그 인덱스를 모두 포함하는 튜플이므로, 내용을 더 잘 반영하도록 게시 대상의 이름을 `bam_index`에서 `indexed_bam`으로 변경하세요:
...
워크플로우에 여러 데이터 입력 파일을 제공하는 매우 일반적인 방법은 파일 경로를 포함하는 텍스트 파일로 수행하는 것입니다.
한 줄에 하나의 파일 경로를 나열하고 다른 것은 없는 텍스트 파일처럼 간단할 수 있으며, 파일에 추가 메타데이터가 포함될 수 있으며, 이 경우 종종 샘플시트라고 합니다.
...
그런 다음 텍스트 파일을 가리키도록 테스트 프로필을 업데이트하세요:
...
[Hello Nextflow의 파트 2](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file)에서 사용한 것과 동일한 패턴을 사용하여 이를 수행할 수 있습니다: 파일을 분석하기 위해 [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) 연산자를 적용한 다음 각 줄의 첫 번째 필드를 선택하기 위해 `map` 연산을 적용합니다.
...
기술적으로 입력 파일이 현재 파일 경로만 포함하고 있으므로 [`.splitText()`](https://www.nextflow.io/docs/latest/reference/operator.html#operator-splittext) 연산자를 사용하여 더 간단하게 수행할 수 있습니다.
그러나 더 다재다능한 `splitCsv` 연산자를(`map`으로 보완하여) 사용함으로써 파일 경로를 포함하는 파일에 메타데이터를 추가하기로 결정하는 경우 워크플로우를 미래에 대비할 수 있습니다.

!!! tip

    연산자가 여기서 무엇을 하는지 확신이 서지 않는다면, 이것은 `.view()` 연산자를 사용하여 적용하기 전과 후에 채널 내용이 어떻게 보이는지 확인할 수 있는 또 다른 좋은 기회입니다.
...
더 일반적으로, 유전체학 파일 형식의 특이성과 도구 요구 사항을 고려하여 실제 작업을 수행하는 간단한 유전체학 파이프라인을 구축하기 위해 필수 Nextflow 구성 요소와 로직을 사용하는 방법을 학습했습니다.

### 다음 단계

성공을 축하하고 특별히 긴 휴식을 취하세요!

이 과정의 다음 파트에서는 이 간단한 샘플별 변이 호출 워크플로우를 데이터에 공동 변이 호출을 적용하도록 변환하는 방법을 학습하게 됩니다.
