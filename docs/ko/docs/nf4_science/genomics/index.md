...
먼저 터미널에서 변이 호출 도구를 수동으로 실행하여 방법론을 이해한 다음, 분석을 자동화하고 확장하는 Nextflow 파이프라인을 단계적으로 구축합니다.
...
| [Part 3: Joint calling on a cohort](./03_joint_calling.md)               | 채널 연산자를 사용하여 샘플별 출력을 결합하는 다중 샘플 공동 유전자형 분석 추가    | 45분           |
