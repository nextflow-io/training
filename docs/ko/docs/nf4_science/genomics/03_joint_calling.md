...
이제 [파트 1](01_method.md)(다중 샘플 사용 사례)에서 설명한 대로 조인트 변이 호출을 구현하도록 파이프라인을 확장하겠습니다.

??? info "이 섹션을 시작하는 방법"

    이 과정의 이 섹션은 [파트 1: 방법 개요](./01_method.md), [파트 2: 샘플별 변이 호출](./02_per_sample_variant_calling.md)을 완료하고 작동하는 `genomics.nf` 파이프라인이 있다고 가정합니다.
...
이는 [파트 1: 방법 개요](./01_method.md)(다중 샘플 사용 사례를 다루는 두 번째 섹션)에서 설명한 방법을 구현하며, [파트 2: 샘플별 변이 호출](./02_per_sample_variant_calling.md)에서 생성한 워크플로우를 직접 기반으로 합니다.
...
2. **샘플별 GVCF를 결합하고 유전자형을 분석하는 조인트 유전자형 분석 단계를 추가합니다.**
   `collect()` 연산자, 명령줄 구성을 위한 Groovy 클로저, 그리고 다중 명령 프로세스를 다룹니다.

이는 [파트 1: 방법 개요](./01_method.md#2-joint-calling-on-a-cohort)의 두 번째 섹션에서 컨테이너에서 수동으로 실행했던 단계를 자동화합니다.
...
파트 2에서 적용한 기본 HaplotypeCaller 명령과 비교하면, 차이점은 `-ERC GVCF` 매개변수와 `.g.vcf` 출력 확장자입니다.
...
또한 새 GVCF 출력을 반영하도록 워크플로우의 게시 및 출력 설정을 업데이트해야 합니다.
...
도구 명령의 출력 파일명을 변경하면, 프로세스 `output:` 블록과 게시/출력 설정을 일치하도록 업데이트해야 합니다.
...
두 도구를 `GATK_JOINTGENOTYPING`이라는 단일 프로세스로 적용하겠습니다.
...
교육 시리즈의 후반부에서 이러한 종류의 작업에 샘플 메타데이터를 사용하는 방법을 배우겠지만, 지금은 `params`를 사용하여 CLI 매개변수를 선언합니다.
...
#### 2.1.2. 테스트 프로필에 `cohort_name`의 기본값 추가
...
`GATK_HAPLOTYPECALLER`의 출력 채널을 새 프로세스에 직접 연결하면, Nextflow는 각 샘플 GVCF에 대해 프로세스를 개별적으로 실행합니다.
세 개의 GVCF(및 해당 인덱스 파일)를 모두 묶어서 Nextflow가 단일 process 실행에 모두 함께 전달하도록 하려고 합니다.

`collect()` 채널 연산자를 사용하여 이를 수행할 수 있습니다.
GATK_HAPLOTYPECALLER 실행 직후 `workflow` 본문에 다음 줄을 추가하세요:
...
### 2.2. 조인트 유전자형 분석 프로세스 작성 및 워크플로우에서 실행

파트 2에서 사용한 것과 동일한 패턴을 따라, 모듈 파일에 프로세스 정의를 작성하고, 워크플로우로 가져온 다음, 방금 준비한 입력에 대해 실행하겠습니다.

#### 2.2.1. 각 GVCF에 `-V` 인자를 제공하는 문자열 구성

프로세스 정의를 채우기 시작하기 전에 해결해야 할 한 가지가 있습니다.
GenomicsDBImport 명령은 각 GVCF 파일에 대해 별도의 `-V` 인자를 기대합니다:
...
`gvcfs_line` 변수는 위에서 설명한 Groovy 클로저를 사용하여 GenomicsDBImport에 대한 `-V` 인자를 구성합니다.
...
워크플로우에서 사용하려면 모듈을 가져오고 프로세스 실행을 추가해야 합니다.
...
`collect()` 줄 다음에 워크플로우 본문에 `GATK_JOINTGENOTYPING` 실행을 추가하세요:
