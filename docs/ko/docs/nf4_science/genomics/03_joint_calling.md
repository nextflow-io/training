...
이제 [파트 1](01_method.md)에서 다룬 공동 변이 호출을 구현하도록 파이프라인을 확장하겠습니다.

## 과제

이 과정의 이번 파트에서는 다음을 수행하도록 워크플로우를 확장하겠습니다:
...
4. 결합된 GVCF 데이터 저장소에 대해 공동 유전자형 분석을 실행하여 코호트 수준의 VCF 생성
...
1. **샘플별 변이 호출 단계를 수정하여 GVCF를 생성합니다.**
   프로세스 명령과 출력 업데이트를 다룹니다.
2. **샘플별 GVCF를 결합하고 유전자형을 분석하는 공동 유전자형 분석 단계를 추가합니다.**
   `collect()` 연산자, 명령줄 구성을 위한 Groovy 클로저, 다중 명령 프로세스를 소개합니다.
...
파트 2의 파이프라인은 VCF 파일을 생성하지만, 공동 호출에는 GVCF 파일이 필요합니다.
GVCF 변이 호출 모드를 활성화하고 출력 파일 확장자를 업데이트해야 합니다.

[파트 1](01_method.md)의 GVCF 변이 호출 명령을 상기해보세요:
...
`modules/gatk_haplotypecaller.nf` 모듈 파일을 열어 두 가지 변경을 수행하세요:

- GATK HaplotypeCaller 명령에 `-ERC GVCF` 매개변수 추가
- GATK 규칙에 따라 출력 파일 경로를 해당 `.g.vcf` 확장자를 사용하도록 업데이트
...
명령 출력을 `.vcf`에서 `.g.vcf`로 변경했으므로, 프로세스 `output:` 블록도 동일한 변경을 반영해야 합니다.
...
### 1.4. 새 디렉토리 구조에 대한 출력 블록 업데이트

또한 GVCF 파일을 `gvcf` 하위 디렉토리에 배치하도록 `output` 블록을 업데이트해야 합니다.
...
도구 명령의 출력 파일 이름을 변경하면, 프로세스 `output:` 블록과 게시/출력 구성을 일치하도록 업데이트해야 합니다.
...
이제 샘플별 GVCF를 수집하고, GenomicsDB 데이터 저장소로 결합한 다음, 공동 유전자형 분석을 실행하여 코호트 수준의 VCF를 생성해야 합니다.
[파트 1](01_method.md)에서 다룬 것처럼, 이는 두 도구 작업입니다: GenomicsDBImport가 GVCF를 결합하고, GenotypeGVCFs가 최종 변이 호출을 생성합니다.
두 도구를 `GATK_JOINTGENOTYPING`이라는 단일 프로세스로 래핑하겠습니다.

[파트 1](01_method.md)의 두 명령을 상기해보세요:
...
공동 유전자형 분석 프로세스에는 아직 없는 두 종류의 입력이 필요합니다: 임의의 코호트 이름과 모든 샘플의 수집된 GVCF 출력을 함께 묶은 것입니다.
...
세 개의 GVCF(및 해당 인덱스 파일)를 모두 묶어서 Nextflow가 모두 함께 단일 프로세스 호출에 전달하도록 하려고 합니다.
...
2. 섹션 1에서 `emit:`을 사용하여 출력 이름을 지정했기 때문에, `.vcf`로 GVCF를 선택하고 `.idx`로 인덱스 파일을 선택할 수 있습니다. 이름이 지정된 출력이 없으면 `.out[0]`과 `.out[1]`을 사용해야 합니다.
3. `collect()` 연산자는 모든 파일을 단일 요소로 묶으므로, `all_gvcfs_ch`에는 세 개의 GVCF가 모두 함께 포함되고, `all_idxs_ch`에는 세 개의 인덱스 파일이 모두 함께 포함됩니다.

Nextflow가 실행을 위해 모든 입력 파일을 함께 스테이징하므로 인덱스 파일이 GVCF와 함께 존재하기 때문에, GVCF와 해당 인덱스 파일을 (튜플로 함께 유지하는 대신) 별도로 수집할 수 있습니다.

!!! tip

    `view()` 연산자를 사용하여 채널 연산자를 적용하기 전후에 채널의 내용을 검사할 수 있습니다.

### 2.2. 공동 유전자형 분석 프로세스 작성 및 워크플로우에서 호출
...
`-V ${all_gvcfs_ch}`로 작성하면, Nextflow는 단순히 파일 이름을 연결하고 명령의 해당 부분은 다음과 같이 보일 것입니다:
...
#### 2.2.2. 공동 유전자형 분석 프로세스에 대한 모듈 작성

이제 전체 프로세스 작성을 다룰 수 있습니다.

`modules/gatk_jointgenotyping.nf`를 열고 프로세스 정의의 개요를 검토하세요.
...
=== "전"
    ...
    /*
     * GVCF를 GenomicsDB 데이터 저장소로 결합하고 공동 유전자형 분석을 실행하여 코호트 수준의 호출 생성
     */
...
이전과 마찬가지로, 명령이 직접 참조하지 않더라도 여러 입력이 나열됩니다: `all_idxs`, `ref_index`, `ref_dict`.
...
GenomicsDB 데이터 저장소(`${cohort_name}_gdb`)는 프로세스 내에서만 사용되는 중간 산출물이며, 출력 블록에 나타나지 않습니다.
...
공동 VCF 출력을 게시해야 합니다.
공동 유전자형 분석 결과에 대한 게시 대상과 출력 블록 항목을 추가하세요.

#### 2.3.1. 공동 VCF에 대한 게시 대상 추가

워크플로우의 `publish:` 섹션에 공동 VCF와 해당 인덱스를 추가하세요:
...
#### 2.3.2. 공동 VCF에 대한 출력 블록 항목 추가

공동 VCF 파일에 대한 항목을 추가하세요.
...
프로세스, 게시 대상, 출력 블록이 모두 준비되었으므로 전체 워크플로우를 테스트할 수 있습니다.
...
공동 VCF 파일을 열면 워크플로우가 예상된 변이 호출을 생성했는지 확인할 수 있습니다.
...
이제 자동화되고 완전히 재현 가능한 공동 변이 호출 워크플로우를 갖게 되었습니다!
...
성공을 축하하고 잘 받을 자격이 있는 휴식을 취하세요.

이 과정의 다음 파트에서는 nf-core의 프로덕션 준비 변이 호출 파이프라인을 실행하고 수동으로 구축한 파이프라인과 비교하는 방법을 학습합니다.
