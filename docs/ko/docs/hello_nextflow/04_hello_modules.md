...
워크플로우 개발을 시작했을 때, 우리는 모든 것을 하나의 단일 코드 파일에 작성했습니다.
...
시작점으로 워크플로우 스크립트 `hello-modules.nf`를 사용할 것입니다.
...
모듈에 대한 파일 스텁을 생성하고, 관련 코드를 복사한 다음 메인 워크플로우 파일에서 삭제할 것입니다.
...
이전과 본질적으로 동일한 코드와 입력으로 워크플로우를 실행하고 있으므로, `-resume` 플래그로 실행하고 어떻게 되는지 살펴봅시다.
