...
이 섹션에서는 파이프라인의 개발 및 유지보수를 더 효율적이고 지속 가능하게 만들기 위해 워크플로우 코드를 구성하는 방법을 다룹니다.
구체적으로, [**모듈**](https://nextflow.io/docs/latest/module.html)을 사용하는 방법을 시연합니다.

Nextflow에서 **모듈**은 단독 실행형 코드 파일이며, 종종 단일 process 정의를 캡슐화합니다.
워크플로우에서 모듈을 사용하려면 워크플로우 코드 파일에 한 줄의 `include` 문만 추가하면 됩니다. 그런 다음 일반적으로 하는 것과 같은 방식으로 process를 워크플로우에 통합할 수 있습니다.
이렇게 하면 코드의 여러 복사본을 생성하지 않고도 여러 워크플로우에서 process 정의를 재사용할 수 있습니다.

워크플로우 개발을 시작했을 때, 모든 것을 하나의 단일 코드 파일에 작성했습니다.
이제 process를 개별 모듈로 이동합니다.
...
시작점으로 워크플로우 스크립트 `hello-modules.nf`를 사용합니다.
...
모듈에 대한 파일 스텁을 생성하고, 관련 코드를 복사한 다음 메인 워크플로우 파일에서 삭제합니다.

그런 다음 `include` 문만 추가하면 Nextflow가 런타임에 관련 코드를 가져옵니다.

### 2.1. 새 모듈에 대한 파일 스텁 생성

`sayHello.nf`라는 모듈에 대한 빈 파일을 생성합니다.
...
전체 process 정의를 워크플로우 파일에서 모듈 파일로 복사합니다.
...
완료되면 워크플로우 파일에서 process 정의를 삭제하십시오.
...
`params` 블록 위에 이것을 삽입하고 적절하게 채웁니다.
...
이전과 본질적으로 동일한 코드와 입력으로 워크플로우를 실행하므로, `-resume` 플래그로 실행하고 어떻게 되는지 확인합니다.
...
Process를 로컬 모듈로 추출하는 방법과 이렇게 해도 워크플로우의 재개 가능성이 깨지지 않는다는 것을 알게 되었습니다.
...
워크플로우에서 여러 process를 모듈화하는 방법을 알게 되었습니다.

축하합니다, 이 모든 작업을 수행했지만 파이프라인 작동 방식에는 아무것도 변경되지 않았습니다!

농담은 제쳐두고, 이제 코드가 더 모듈화되었습니다. 해당 process 중 하나를 실행하는 다른 파이프라인을 작성하기로 결정하면 관련 모듈을 사용하기 위해 짧은 `include` 문 하나만 입력하면 됩니다.
...
- [ ] 설정 파일
- [x] Process 정의를 포함할 수 있는 단독 실행형 파일
- [ ] 워크플로우 정의
- [ ] 채널 연산자

자세히 알아보기: [2. `sayHello()`용 모듈 생성](#2-create-a-module-for-sayhello)
</quiz>
...
자세히 알아보기: [1. 모듈을 저장할 디렉토리 생성](#1-create-a-directory-to-store-modules)
</quiz>
...
자세히 알아보기: [2.3. include 선언 추가](#23-add-an-include-declaration-before-the-workflow-block)
</quiz>
