...
이번에는 사람들이 실제 워크플로우를 구축하는 방식을 더 잘 반영하기 위해 워크플로우에 다음과 같은 변경을 수행할 것입니다:
...
이제 프로세스의 출력이 입력과 마찬가지로 채널로 패키징되는 것을 명시적으로 보여주는 점을 제외하면 익숙해 보일 것입니다.
곧 이 출력 채널을 유용하게 사용할 것입니다.
...
이것이 정상적으로 작동했다면 다단계 워크플로우를 조립하는 방법을 배울 준비가 되었습니다.
...
편리하게도 Nextflow는 준비 운동 섹션의 다이어그램에 표시된 대로 프로세스의 출력을 자동으로 채널로 패키징합니다.
...
- 수집기 프로세스가 출력 파일에 대해 사용자 정의 이름(`batch_name`)을 허용하도록 수정
- 워크플로우에 명령줄 매개변수(`--batch`)를 추가하고 수집기 프로세스에 전달
