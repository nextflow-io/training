...
이것은 악센트가 있는 문자를 고려하지 않는 매우 단순한 텍스트 대체 한 줄 명령이지만, Nextflow 개념을 시연하기에는 충분히 작동하며 그것이 중요합니다.
...
이전에 했던 것처럼 터미널에서 명령 자체를 실행하여 예상대로 작동하는지 확인합니다.
...
!!! tip "팁"
    ...
    일부 명령줄 도구는 각 입력 파일에 대해 인자(예: `-input`)를 제공해야 합니다.
    그런 경우 명령을 구성하기 위해 약간의 추가 작업을 수행해야 합니다.
    [Nextflow for Genomics](../nf4_science/genomics/index.md) 교육 과정에서 이에 대한 예를 볼 수 있습니다.
...
이를 사용하여 `batch` 매개변수를 선언합시다(기본값과 함께).
...
자세히 알아보기: [1.4. 첫 번째 프로세스의 출력을 두 번째 프로세스로 전달](#14-pass-the-output-of-the-first-process-to-the-second-process)
</quiz>
...
자세히 알아보기: [2.4. 연산자를 사용하여 인사말을 단일 입력으로 수집](#24-use-an-operator-to-collect-the-greetings-into-a-single-input)
</quiz>
...
자세히 알아보기: [4.1.2. 보고 파일 내보내기 및 출력 이름 지정](#412-emit-the-report-file-and-name-outputs)
</quiz>
