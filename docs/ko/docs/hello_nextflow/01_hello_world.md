...
각 프로세스 호출에 대해 Nextflow는 고유하게 만들기 위해 해시로 이름이 지정된 내포된 하위 디렉토리를 생성하며, 여기에 필요한 모든 입력을 스테이징하고(기본적으로 심볼릭 링크 사용), 보조 파일을 작성하고, 프로세스의 로그와 출력을 기록합니다.
...
다행히 Nextflow는 [워크플로우 수준 출력 정의](https://nextflow.io/docs/latest/workflow.html#workflow-outputs)를 사용하여 출력을 지정된 디렉토리에 게시하는 방법을 제공합니다.
...
자세히 알아보기: [4.1. `-resume`으로 워크플로우 다시 실행](#41-re-launch-a-workflow-with--resume)
</quiz>
...
자세히 알아보기: [2.3. 게시 모드를 copy로 설정](#23-set-the-publish-mode-to-copy)
</quiz>
...
자세히 알아보기: [3.2. 사용자 입력을 캡처하기 위한 명령줄 매개변수 설정](#32-set-up-a-command-line-parameter-to-capture-user-input)
</quiz>
