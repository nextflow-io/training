...
이 과정의 파트 1(Hello World)에서는 process 실행에서 직접 입력을 제공하여 process에 변수 입력을 전달하는 방법을 보여드렸습니다: `sayHello(params.input)`.
이것은 의도적으로 단순화된 접근 방식이었습니다.
실제로 이 접근 방식에는 주요한 제한 사항이 있습니다. 즉, 단일 값에 대해 process를 한 번만 실행하려는 매우 간단한 경우에만 작동합니다.
대부분의 실제 워크플로우 사용 사례에서는 여러 값(예: 여러 샘플의 실험 데이터)을 처리하려고 하므로 입력을 처리하는 더 정교한 방법이 필요합니다.

이것이 바로 Nextflow [**channel**](https://nextflow.io/docs/latest/channel.html)의 역할입니다.
Channel은 입력을 효율적으로 처리하고 다단계 워크플로우에서 한 단계에서 다른 단계로 입력을 전달하도록 설계된 큐이며, 내장된 병렬 처리와 많은 추가적인 이점을 제공합니다.

이 파트에서는 다양한 소스에서 여러 입력을 처리하기 위해 channel을 사용하는 방법을 학습합니다.
또한 필요에 따라 channel 내용을 변환하기 위해 [**연산자**](https://nextflow.io/docs/latest/reference/operator.html)를 사용하는 방법도 학습합니다.
...
시작점으로 워크플로우 스크립트 `hello-channels.nf`를 사용할 것입니다.
...
아직 process 실행의 입력을 전환하지 않았으므로 아직 작동하지 않습니다.

### 1.2. Process 실행에 channel을 입력으로 추가

이제 새로 생성한 channel을 `sayHello()` process 실행에 연결하여 이전에 직접 제공하던 CLI 매개변수를 대체해야 합니다.
...
이제 워크플로우가 제대로 작동합니다. 이것은 `sayHello('Hello Channels!')`를 작성하는 것과 동일한 명시적 표현입니다.
...
동일한 최종 결과를 얻으면서 워크플로우의 유연성을 높였습니다.
...
Channel을 사용하여 워크플로우가 여러 입력 값을 반복 처리하도록 하는 방법을 학습합니다.

---

## 2. 워크플로우를 수정하여 여러 입력 값에서 실행

워크플로우는 일반적으로 대량으로 처리해야 하는 입력 배치에서 실행되므로, 워크플로우를 업그레이드하여 여러 입력 값을 허용하도록 해야 합니다.
...
확실히 잘 실행된 것 같습니다.
실행 모니터는 `sayHello` process에 대해 `3 of 3` 실행이 이루어졌음을 보여주며, 약속한 대로 `view()` 문에 의해 세 개의 인사말이 줄당 하나씩 나열되어 있습니다.
...
이 시점에서 기본적으로 ANSI 로깅 시스템은 동일한 process에 대한 여러 실행의 로깅을 같은 줄에 기록한다는 것을 알려드려야 합니다.
따라서 sayHello() process에 대한 세 번의 실행 상태가 모두 같은 위치에 표시됩니다.

다행히도 전체 process 실행 목록을 볼 수 있도록 해당 동작을 비활성화할 수 있습니다.

#### 2.1.3. `-ansi-log false` 옵션으로 명령을 다시 실행

로깅을 확장하여 process 실행당 한 줄씩 표시하려면 명령에 `-ansi-log false`를 추가하십시오.
...
훨씬 낫습니다. 적어도 간단한 워크플로우에서는요.
복잡한 워크플로우나 많은 수의 입력의 경우 전체 목록이 터미널에 출력되면 다소 압도적일 수 있습니다.
그래서 `-ansi-log false`가 기본 동작이 아닙니다.

!!! tip "팁"

    상태가 보고되는 방식은 두 로깅 모드 간에 약간 다릅니다.
    압축 모드에서 Nextflow는 실행이 성공적으로 완료되었는지 여부를 보고합니다.
    이 확장 모드에서는 제출되었다는 것만 보고합니다.

어쨌든 이제 각 process 실행의 하위 디렉토리가 있으므로 로그와 출력을 찾을 수 있습니다.
...
`sayHello` process의 출력 파일 이름을 하드코딩했기 때문에 세 번의 실행 모두 `output.txt`라는 파일을 생성했던 것을 기억하실 것입니다.
...
이렇게 하면 process가 실행될 때마다 고유한 출력 파일 이름이 생성되어 출력 디렉토리에서 동일한 process에 대한 다른 실행의 출력과 구별될 수 있습니다.

#### 2.2.2. 워크플로우 실행

실행합니다. 기본 ANSI 로그 설정으로 다시 실행합니다.
...
!!! tip "팁"
    ...
    나중에 Nextflow 교육에서 이를 수행하는 방법을 학습합니다([메타데이터 사이드 퀘스트](../side_quests/metadata/index.md) 참조).
...
연산자를 사용하여 channel의 내용을 변환하는 방법을 학습합니다.
...
이제 channel 팩토리에 현재 하드코딩된 값 `'Hello','Bonjour','Hola'`를 방금 생성한 `greetings_array`로 교체할 것입니다.
...
Nextflow가 배열의 세 문자열을 별도의 값으로 사용하는 대신 `[Hello, Bonjour, Hola]`를 단일 문자열 값으로 사용하여 단일 process 실행을 시도한 것 같습니다.
...
Nextflow 문서의 [연산자 목록](https://nextflow.io/docs/latest/reference/operator.html)을 훑어보면, 정확히 필요한 것을 수행하는 [`flatten()`](https://nextflow.io/docs/latest/reference/operator.html#flatten)을 찾을 수 있습니다. 배열의 내용을 풀고 개별 항목으로 내보냅니다.
...
두 번째 `.view` 문을 추가했으며, 각각에 대해 빈 괄호(`()`)를 `#!groovy { greeting -> "Before flatten: $greeting" }`와 같은 코드가 포함된 중괄호로 교체했습니다.
...
중요한 것은 이제 각 항목이 워크플로우에서 별도로 처리될 수 있다는 것입니다.
...
워크플로우가 파일을 입력 값의 소스로 사용하도록 하는 방법을 학습합니다.
...
나중에 Nextflow 여정에서 다른 데이터 위치를 다루는 방법을 학습합니다.
...
새 channel 팩토리와 입력 파일로 워크플로우를 실행합니다.
...
약간 익숙해 보일 수 있습니다.
Nextflow가 파일 경로 자체를 문자열 값으로 사용하여 단일 process 실행을 시도한 것 같습니다.
파일 경로를 올바르게 확인했지만 실제로 원하는 내용을 파싱하지 않았습니다.
...
연산자 목록을 다시 살펴보면, CSV 형식 텍스트를 파싱하고 분할하도록 설계된 [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv)를 찾을 수 있습니다.
...
이번에 Nextflow는 파일 내용을 파싱했지만(야호!) 각 행을 배열로 로드했으며 각 배열이 channel의 요소입니다.
...
또한 channel이 암묵적으로 병렬 실행을 처리하는 방법도 확인했습니다.
...
준비가 되면 [**파트 3: Hello Workflow**](./03_hello_workflow.md)로 이동하여 더 많은 단계를 추가하고 적절한 워크플로우로 연결하는 방법을 학습하십시오.
...
자세히 알아보기: [1.1. 입력 channel 생성](#11-create-an-input-channel)
</quiz>
...
자세히 알아보기: [2. 워크플로우를 수정하여 여러 입력 값에서 실행](#2-modify-the-workflow-to-run-on-multiple-input-values)
</quiz>
...
자세히 알아보기: [3.2.1. `flatten()` 연산자 추가](#321-add-the-flatten-operator)
</quiz>
...
자세히 알아보기: [1.4. `view()`를 사용하여 channel 내용 검사](#14-use-view-to-inspect-the-channel-contents)
</quiz>
...
- [ ] 문자열을 쉼표로 분할
- [x] CSV 파일을 각 행을 나타내는 배열로 파싱
- [ ] 여러 CSV 파일 병합

자세히 알아보기: [4.2. `splitCsv()` 연산자를 사용하여 파일 파싱](#42-use-the-splitcsv-operator-to-parse-the-file)
</quiz>
...
자세히 알아보기: [4.3. `map()` 연산자를 사용하여 인사말 추출](#43-use-the-map-operator-to-extract-the-greetings)
</quiz>
...
자세히 알아보기: [2.2. 출력 파일 이름이 고유하도록 보장](#22-ensure-the-output-file-names-will-be-unique)
</quiz>
