...
이것이 정상적으로 작동했다면 channel에 대해 학습할 준비가 되었습니다.
...
!!! tip "팁"
    ...
    나중에 Nextflow 교육에서 이를 수행하는 방법을 학습하게 됩니다([메타데이터 사이드 퀘스트](../side_quests/metadata.md) 참조).
...
나중에 Nextflow 여정에서 다른 데이터 위치를 다루는 방법을 학습하게 됩니다.
...
새 channel 팩토리와 입력 파일로 워크플로우를 실행해 봅시다.
...
추가된 CSV 구문 분석 로직으로 워크플로우를 실행해 봅시다.
...
나중 교육에서 복잡한 입력을 처리하는 더 정교한 접근 방식을 학습하게 됩니다.
...
자세히 알아보기: [2. Workflow를 수정하여 여러 입력 값에서 실행](#2-modify-the-workflow-to-run-on-multiple-input-values)
</quiz>
...
자세히 알아보기: [4.2. `splitCsv()` 연산자를 사용하여 파일 구문 분석](#42-use-the-splitcsv-operator-to-parse-the-file)
</quiz>
