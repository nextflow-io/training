...
동일한 방식으로 실행되어야 합니다. 이전에 계산된 결과를 사용하고 조금 더 빠르게 만들기 위해 이 모든 것에 대시 resume을 사용할 수 있었습니다. 이제 tree custom-outdir-config를 수행하면 results에 있지 않고 batch 이름이 있는 기본 디렉토리에 있는 것을 볼 수 있습니다. 그리고 모든 결과가 이제 프로세스 이름을 따서 명명된 하위 디렉토리 내에 구성된 것을 볼 수 있습니다. 따라서 cowpy와 collectGreetings 두 개가 있습니다. 여기서 두 가지 다른 위치에서 동적 출력 경로를 정의하고 있습니다.

좋습니다. 마지막으로, 이전에 있던 중간 폴더를 다시 추가하겠습니다. 꽤 좋았기 때문입니다. Intermediates.

그리고 이 params.batch에 대해서도 조금 생각해 볼 수 있습니다. 파이프라인 개발자로서 저는 하위 디렉토리에 그것이 있는 것을 정말 좋아했지만, 파이프라인의 최종 사용자가 CLI에서 "-o" 또는 -output-dir을 설정하면 이 전체 문장을 완전히 덮어쓰고 해당 하위 디렉토리를 잃게 됩니다.

따라서 우리가 할 수 있는 것은 덮어쓰여질 outputDir 설정에서 해당 동적 경로를 가져와서 덮어쓰여지지 않는 output 경로에 넣는 것입니다.

따라서 params.batch 슬래시 intermediates 슬래시 sayHello.name을 수행할 수 있으며, Nextflow에 의해 보간되도록 이 모든 것을 큰따옴표 문자열로 수행합니다.

이제 복사할 수 있습니다, 앗. 이것들을 다른 프로세스에 복사합니다. 모두 따옴표로 묶는 것을 기억하세요. 그리고 이러한 특정 출력에서 intermediates를 제거합니다.

좋습니까? 이제 조금 더 복잡해 보이지만 코드에서 정말 잘 구성된 출력 디렉토리 구조를 구축하기 시작하고 있는 것을 볼 수 있습니다.

그리고 정말 좋은 점은 CLI로 전달되지 않는 코드의 이러한 추가 복잡성입니다. 따라서 -output-dir과 어떤 batch 변수로든 명령을 실행할 수 있으며, 파이프라인을 실행하는 방법에 대해서만 생각하고 코드에 무엇이 있는지에 대해서는 너무 많이 생각하지 않습니다. 그리고 출력 파일은 매우 잘 구성된 방식으로 정말 멋지게 구성될 것이며, 이는 기본적으로 파이프라인을 사용하는 사람들에게 좋습니다.

좋습니다. 이것을 작성하면서 실수를 했다는 것을 깨달았습니다. 여기서 누가 저를 잡았는지 보세요. collectGreetings.name이 있으므로 뭔가 잘못되었습니다. 그리고 확실히, 실수로 이것들을 중괄호에 넣는 것을 잊었습니다.

따라서 코드를 작성할 때 주의하고 무엇이 변수이고 무엇이 단순한 문자열인지 Nextflow에 알려야 합니다. 왜냐하면 Nextflow는 여러분이 말한 대로 정확히 수행할 것이기 때문입니다. 그 이상도 이하도 아닙니다. 모든 좋은 컴퓨터처럼. 좋습니다, 이것으로 수정되어야 합니다.

## 2.3. 워크플로우 레벨에서 게시 모드 설정

이 스크립트에서 여전히 마음에 들지 않는 부분이 하나 있는데, mode copy를 계속 반복해서 작성하고 있다는 사실입니다. 그리고 우리가 싫어하는 것이 하나 있다면 그것은 반복하는 것입니다.

따라서 이것을 가져와서 설정으로 이동하여 조금 정리할 수 있습니다. 그리고 실제로 전체 파이프라인에 대해 한 번에 설정할 수 있습니다. 따라서 여러 번 말할 필요가 없습니다.

설정 파일로 이동하면 workflow라는 새로운 범위가 있습니다. 그리고 중괄호를 사용하거나 점 표기법을 사용할 수 있습니다. 차이가 없습니다. 교육 자료 웹사이트는 점 표기법을 사용합니다. output이라고 할 수 있고 혼합할 수 있으므로 mode equals copy입니다. 좋습니다.

이제 여기로 돌아가서 이것들을 삭제할 수 있습니다. 그대로 둘 수도 있습니다. 설정이 기본적으로 여기에 작성된 것을 덮어쓰지만, 파이프라인 레벨 설정에 있고 이 두 파일이 함께 제공되므로 실제로 두 번 수행할 이유가 없습니다.

좋습니다. 우리 자신을 확인해 봅시다. 분명히 우리는 실수를 하기 때문입니다. 다시 실행하고 파일 게시에 copy 모드를 올바르게 사용하고 있는지 확인하겠습니다. 따라서 스크립트를 다시 실행하고 이번에는 결과를 config-output-mode라는 디렉토리에 넣었습니다. 거기에 있는 파일이 어떻게 보이는지 보겠습니다.

그런 다음 "ls -l"을 수행하여 batch를 보고 예를 들어 cowpy를 볼 수 있습니다. 그리고 이것이 소프트 링크가 아닌 적절한 파일임을 볼 수 있어야 하므로 해당 설정 속성이 제대로 적용되었습니다.

## 3. 소프트웨어 패키징 기술 선택

좋습니다. 지금까지 워크플로우가 실행되는 파일인 입력과 출력에 집중했습니다. 하지만 인프라는 어떨까요? 처음에 Nextflow를 사용하면 다양한 컴퓨팅 설정에서 동일한 파이프라인을 실행할 수 있다고 말했습니다. 그것은 어떻게 보일까요?

이를 보여주기 위해 cowpy를 실행하기 위해 Docker를 사용하는 것에서 전환하고 대신 Conda를 사용하여 동일한 작업을 수행할 것입니다.

매우 간단하게 할 수 있습니다. code, "nextflow.config"로 이동하면. 상단에서 이전에 docker.enabled를 정의했던 것을 기억하신다면, 마지막 장에서 cowpy와 함께 컨테이너를 사용할 수 있도록 했습니다.

Nextflow에게 Docker를 사용하지 말라고 말하겠습니다. 그것을 false로 설정합니다. 그리고 Conda enabled equals true라고 말하겠습니다. 따라서 Nextflow에게 Conda를 사용하라고 말합니다.

이제 Conda를 활성화하는 것만으로는 충분하지 않습니다. Docker에서 했던 것처럼 Nextflow에게 필요한 소프트웨어를 어디서 얻을 수 있는지 알려줘야 합니다.

따라서 여기 모듈로 이동하면. 그리고 cowpy 스크립트를 엽니다. 상단에 container 선언이 있는 것을 볼 수 있습니다. 그리고 컨테이너는 Docker뿐만 아니라 Singularity, Apptainer 및 다른 많은 소프트웨어 도구에서도 사용됩니다.

하지만 Conda에는 사용할 수 없으므로 "conda"라는 별도의 선언이 있으며 "cowpy"라고만 작성할 수 있습니다. 그리고 그것은 로컬 conda 환경에 따라 해결하는 최선의 방법을 알아내기 위해 conda 패키지 해상도에 맡길 것입니다.

또는 교육 자료 웹사이트가 말하는 것을 수행하는 것이 좋은 관행입니다. 이중 콜론 표기법으로 특정 conda 채널을 정의하고 모든 사람이 파이프라인을 실행할 때 동일한 버전을 얻을 수 있도록 소프트웨어의 특정 버전을 확실히 정의하는 것입니다.

컨테이너는 이 점에서 약간 우수합니다. Conda로 무언가를 설치할 때 여전히 해당 패키지에 대한 모든 의존성을 파악할 것이고 시간이 지남에 따라 변경될 수 있기 때문입니다. 의존성 드리프트라고 합니다.

그러나 컨테이너는 전체 소프트웨어 의존성 스택을 끝까지 잠그므로 A, 작동할 것이고 B, 재현 가능할 것이라는 것을 조금 더 확신할 수 있습니다.

따라서 Docker 또는 Singularity 또는 Apptainer를 사용할 수 있다면 확실히 권장합니다.

이제 좋은 점은 파이프라인 개발자가 작성한 모듈 파일에 이제 Container와 Conda가 모두 있으며, 따라서 이 파이프라인을 실행하는 사람에게 어떤 소프트웨어 패키징 솔루션을 사용하든 상관없다고 말하고 있습니다. Docker와 Conda 모두에서 작동할 것이며 두 경우 모두 소프트웨어를 얻을 수 있는 위치입니다.

터미널을 열고 시도해 보겠습니다. Nextflow run hello config --batch conda. 그리고 conda로 처음 실행할 때 해당 특정 프로세스에 도달하면 "conda install"을 실행해야 하기 때문에 조금 느릴 것입니다.

그리고 이 하나의 프로세스만을 위한 특별한 conda 환경을 만들고 있습니다. 따라서 터미널에 있는 전역 conda 환경을 사용하는 것이 아닙니다. 해당 하나의 프로세스만을 위한 것을 만들고 있습니다. 이것은 워크플로우의 다른 프로세스 간의 의존성 충돌과 같은 것을 피하기 때문에 좋습니다. 프로세스에 다른 버전의 Python이 필요한 도구가 있거나 그런 것들이 있다면 다른 conda 환경을 사용하고 있기 때문에 괜찮습니다.

Nextflow는 이러한 conda 환경을 로컬로 캐시하며, 해당 경로가 어디인지 알려줍니다. 여기 work 디렉토리에 있습니다. 따라서 다음에 Conda로 이 스크립트를 실행하면 기존 conda 환경을 찾아 재사용하기 때문에 훨씬 빠를 것입니다. 하지만 처음 수행할 때는 가져와서 해결하고 모든 의존성을 다운로드하고 모든 것을 설정해야 합니다.

좋습니다, 훌륭합니다, 실행되었습니다. 파이프라인이 현재 사용하도록 구성된 것이 무엇인지 빠르게 상기시켜 봅시다. 설정 파일을 보면 지금 저에게는 "custom-outdir-config"였습니다. 해당 기본 디렉토리로 올라가면. 그리고 --batch conda를 수행했습니다. conda 하위 디렉토리가 있습니다. 따라서 작동했고 cowpy 출력이 있습니다.

따라서 cowpy를 가져와서 conda를 사용하여 로컬 시스템에 설치하고 프로세스를 실행했습니다. 그리고 좋은 점은 최종 사용자로서 거기에서 소프트웨어 관리에 대해 전혀 생각할 필요가 없었다는 것입니다. Nextflow가 저를 위해 정리했습니다. 이 시스템에서 conda를 사용해야 한다고 말했습니다. 파이프라인 개발자는 필요한 패키지를 말했습니다. 그리고 Nextflow가 나머지를 수행했습니다. 매우 강력합니다.

실제로 다양한 기술의 혼합을 사용할 수 있습니다. 따라서 특정 프로세스에 대해 Docker를 활성화하고 다른 프로세스에 대해 conda를 활성화하거나 일부 프로세스가 로컬로 설치한 소프트웨어를 사용해야 한다고 말할 수 있습니다. 이것은 꽤 드물지만 가능하며, 예를 들어 Docker에서 패키징하기 어려울 수 있는 특정 소프트웨어를 사용하는 경우 탈출구가 있습니다.

## 4. 실행 플랫폼 선택

따라서 그것이 소프트웨어 패키징입니다. 다른 시스템으로의 이식성의 다른 부분은 작업이 실제로 실행되는 위치입니다. 현재 저는 기본적으로 my_laptop 또는 이 Codespaces에서 실행하고 있으며, 이는 단일 컴퓨터입니다. 특별한 것은 없습니다. Nextflow는 최선을 다해 작업을 병렬화하는 데 약간 영리하지만 모두 하나의 시스템에 있습니다.

이제 HPC에서 실행하는 경우 SLURM 또는 PBS 또는 무언가와 같은 일종의 작업 스케줄러가 있을 것이며 해당 스케줄러에 작업을 제출하면 모든 작업을 다른 컴퓨팅 노드로 분산시킬 것입니다.

실행하는 또 다른 방법은 클라우드에서입니다. 따라서 AWS Batch 또는 Azure Cloud 또는 Google을 사용하고 있을 수 있습니다. 그리고 이것들은 모두 스케줄러가 있고 작업을 제출하면 계산을 위해 다른 장소로 제출되는 유사한 시스템에서 작동합니다.

이제 제가 생물정보학을 시작했던 먼 과거에는 분석을 실행하기 위한 모든 사람의 소프트웨어가 컴퓨팅 인프라에 매우 밀접하게 연결되어 있어 복제가 거의 불가능했습니다.

하지만 Nextflow의 이러한 설정 분리와 Nextflow가 매우 다양한 컴퓨팅 인프라 백엔드와 상호 작용할 수 있는 능력으로 파이프라인 코드를 전혀 수정하지 않고 파이프라인을 가져와서 전환하는 것이 매우 간단합니다.

## 4.1. 다른 백엔드 타겟팅

따라서 "nextflow.config" 파일로 이동하면 이제 일부 프로세스 레벨 설정을 넣을 수 있습니다. 상단에 process 범위를 넣으면 executor를 설정할 수 있으며 여기서는 기본값인 local로 설정되어 있습니다.

이것이 프로세스 레벨이기 때문에 다른 프로세스에 대해 타겟팅할 수 있습니다. 따라서 실제로 executor를 프로세스별로 설정하고 하이브리드 실행을 할 수 있습니다. 여기서 일부 작업은 Nextflow 작업이 실행되는 곳이면 어디든 로컬로 실행될 수 있습니다. 일부는 다른 HPC에 제출되고 일부는 클라우드에 제출될 수 있습니다. 원하는 만큼 영리할 수 있습니다.

이제 제출할 HPC가 없기 때문에 이와 같은 교육 환경에서 이것을 시연하기는 매우 어렵습니다. 하지만 제가 할 수 있는 것은 slurm을 입력하면 조금 속일 수 있고 이것에 대한 느낌을 얻을 수 있습니다.

그리고 이것은 SLURM에서 실행하는 데 익숙하고 SLURM 헤더가 어떻게 보이는지 아는 사람들에게 가장 흥미롭습니다. 하지만 Nextflow run, hello config를 수행하면. 존재하지 않는 클러스터에 작업을 제출하려고 하기 때문에 실패할 것입니다. 따라서 sbatch를 사용할 수 없다는 오류가 발생할 것입니다.

네, 작성되었습니다. 그것이 도구입니다. 그것은 slurm 클러스터에 작업을 제출하는 데 사용하는 CLI 도구입니다. 하지만 우리가 할 수 있는 것은 명령을 클릭하여 work 디렉토리로 이동하여 해당 디렉토리를 열고 .command.run을 보는 것입니다. 그리고 .command.run 파일의 상단에 이론적인 SLURM 클러스터에 이 작업 제출을 처리하는 방법을 알려주는 sbatch 헤더가 있는 것을 볼 수 있습니다.

따라서 Nextflow가 영리하게 모든 올바른 작업을 수행하고 있음을 알 수 있습니다. 단지 제출할 클러스터가 없었을 뿐입니다.

## 5. 컴퓨팅 리소스 할당 제어

다른 컴퓨팅 인프라 간에 다른 것은 무엇입니까? 또 다른 것은 사용 가능한 리소스가 얼마나 있는지이며, 실제로 많은 컴퓨팅 환경에서 작업에 필요한 CPU 수와 메모리 양을 지정해야 하는 요구 사항입니다.

다시 말하지만 Nextflow는 이것을 추상화하여 더 이상 단일 컴퓨팅 환경 유형에 특정하지 않으며 여기 프로세스 레벨 범위에 입력할 수 있습니다. CPUs equals one, memory equals two gigabytes. 우리 파이프라인은 그다지 까다롭지 않으므로 괜찮을 것입니다.

이제 여기서 이러한 숫자를 추측했지만 사용할 합리적인 리소스 양이 무엇인지 어떻게 알 수 있습니까? 많은 샘플의 큰 파이프라인의 이러한 모든 다른 프로세스를 살펴보고 리소스 활용이 무엇인지 이해하는 것은 꽤 어려운 작업입니다.

따라서 이에 대한 좋은 접근 방식은 이러한 값을 처음에 높은 숫자로 설정하여 파이프라인이 오류 없이 실행되도록 한 다음 Nextflow에게 사용 리포트를 생성하도록 요청하는 것입니다.

이것은 매우 쉽게 할 수 있으므로 터미널로 돌아가겠습니다. 오, 파이프라인이 실제로 실행되도록 그것을 local로 다시 설정하는 것을 기억해야 합니다. 그리고 Nextflow run을 말하고 명령줄 플래그 -with-report를 사용하겠습니다.

그리고 그것을 비워두면 기본 파일 이름을 제공하지만 특정 위치에 저장되도록 특정 파일 이름을 제공하겠습니다.

Enter를 누르면 파이프라인이 정상적으로 실행되지만 완료되면 멋진 HTML 리포트를 생성할 것입니다.
...
Nextflow는 모든 파이프라인에 대해 이와 같은 리포트를 생성할 수 있으며 정말 좋은 정보가 있습니다. 따라서 항상 이것들을 저장하는 것이 좋은 관행입니다. 언제 실행했는지, 어디서 실행했는지, 성공했는지 여부, 어떤 매개변수가 사용되었는지, CLI 명령이 무엇이었는지, 이런 것들을 알려줍니다.

그리고 리소스 사용에 대한 이러한 플롯도 있습니다. 따라서 각 프로세스에 대해 사용된 CPU 호출의 백분율을 상자 플롯으로 알려줍니다. 각 프로세스에 대해 많은 작업이 있기 때문에 분포를 볼 수 있습니다.

여기 프로세스인 cowpy와 collectGreetings는 단일 작업만 있었으므로 단일 라인일 뿐입니다. 그리고 CPU와 메모리 및 작업 지속 시간이 있으며 매우 빨랐습니다.
