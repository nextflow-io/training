...
그리고 실제로 우리가 찾고 있는 중요한 것은 *여기*인데, 실행된 다양한 작업의 요약입니다.
...
work 디렉토리의 다른 파일들을 빠르게 살펴봅시다. 위에서 아래로, *.command.begin*이라는 파일이 있습니다. 이것은 비어 있습니다. Nextflow가 "작업을 시작하겠습니다"라고 말하며 생성한 센티널 파일일 뿐입니다. 거기에 흥미로운 것은 없습니다.

그런 다음 _.command.error_, _.command.log_ 및 *.command.out*이 있습니다. 이것들은 모두 실행된 bash 명령 또는 이 스크립트의 출력입니다. 이것은 표준 에러입니다. 이것은 표준 출력이고, 이것은 나온 대로 두 개를 결합한 것입니다. 따라서 논리적 순서를 얻습니다.

좋습니다, 이것들도 모두 비어 있어서 별로 흥미롭지 않지만, *.command.run*에 도착하면 더 흥미로워집니다.
...
다음 것은 훨씬 더 흥미롭습니다. *.command.sh*는 프로세스에서 나온 생성된 스크립트이며, 여기서 Nextflow가 Bash 헤더를 추가한 다음 스크립트 블록에 있던 명령을 실행한 것을 볼 수 있습니다.
...
마지막으로 *.exitcode*라는 파일이 있는데, 이것은 작업의 종료 코드를 캡처하는 것으로, 이 경우 성공했으므로 종료 코드가 0이었습니다.

메모리가 부족하거나 다른 문제가 발생하여 실패하면 무엇이 잘못되었는지 이해하는 데 매우 유용합니다.

## 1.3. 워크플로우 다시 실행하기

work 디렉토리에 대해 이해해야 할 한 가지가 더 있는데, 이 파이프라인을 반복적으로 계속 실행하면, *"nextflow run hello-world.nf"*를 실행하면 정확히 같은 작업을 수행하지만, 이번에는 새로운 작업 ID를 갖게 됩니다. 여기 이 해시가 다른 것을 볼 수 있고, 이제 work를 보면 두 개의 해시 디렉토리가 있습니다. 그리고 이것들은 다시 서로 분리되어 있습니다.
...
이 디렉토리에 들어가면 동일한 모든 파일과 처음부터 재생성된 동일한 *output.txt*를 볼 수 있습니다.
...
main 블록이 있고, 다음으로 publish라고 말하고 퍼블리싱에서 무엇을 기대하는지 Nextflow에 알려줄 것입니다. *first_output*이라고 부르고, *sayHello.out*이라고 부르겠습니다.
...
워크플로우를 다시 실행해 봅시다. *"nextflow run hello-world.nf"*를 실행하면 기본적으로 정확히 동일하게 보일 것입니다. Nextflow와 관련하여 실제로 변경된 것은 없습니다. 같은 작업을 수행하고 있습니다. work 디렉토리에서 다시 수행하고 있을 뿐입니다.

하지만 이제 *"ls results/"*를 실행하면 results라는 새 디렉토리가 생성된 것을 볼 수 있는데, 이것은 워크플로우 퍼블리싱의 기본 기본 디렉토리입니다. 그리고 거기에 *output.txt*라는 파일이 있습니다.

*"ls -l results"*를 실행하면 이것이 실제로 work 디렉토리에 소프트 링크되어 있음을 알 수 있습니다. 따라서 이것은 실제 파일이 아니며 work 디렉토리에 링크되어 있으며 거기에서 모든 파일을 수집했습니다.

## 2.2. 사용자 정의 위치 설정하기

"Results"는 이 경로의 기본 이름입니다. 워크플로우를 다시 실행하고, 이번에는 _단일_ 하이픈으로 *"-"*를 수행합니다. 이것은 핵심 Nextflow 옵션이기 때문입니다. _"-output-dir **my**results"_. 줄여서 *"-o"*라고도 할 수 있습니다. 그러면 파일이 저장되는 위치에 대한 다른 기본 디렉토리가 설정되고, 다시 한 번 여기 _myresults/_ 위에 *output.txt*가 있습니다.

좋습니다만, 모든 파일을 루트에만 두고 싶지는 않을 것입니다. 약간의 구조가 필요하므로 여기에 원하는 대로 호출할 수 있는 하위 디렉토리를 만들 수도 있습니다. *"path 'hello_world'"*라고 말하고, 이것을 다시 실행하겠습니다. _"nextflow run hello-world.nf"_. results 디렉토리의 하위 디렉토리로 들어가야 하고, 확실히 이제 상단의 results 아래에 *hello_world/*가 있고 *output.txt*가 있습니다.
...
좋습니다, 이 파일들이 소프트 링크라고 언급했으므로, *"ls -l results/hello_world/"*를 실행하면 work 디렉토리에 소프트 링크되어 있는 것을 볼 수 있습니다. 일반적으로 HPC 같은 곳에서 작업하고 있고 이것들이 정말 거대한 파일이며 중복하고 싶지 않다면 좋은 일입니다. 파일이 파일 시스템에 한 번만 저장되기 때문입니다.

그러나 work 디렉토리를 삭제하면: *"rm -r work"*를 실행하고 생성된 모든 중간 파일을 지우면. 이제 이 파일 *"results/hello_world/"*를 읽으려고 하면. 더 이상 존재하지 않는 파일에 대한 소프트 링크로 가리키고 있고 데이터가 영원히 사라져 복구할 수 없는데, 이는 그다지 좋지 않을 수 있습니다.
...
이제 *"ls -l results/hello_world/"*로 가면 이것이 실제 파일이고 더 이상 소프트 링크가 아니며, Nextflow가 그것을 복사한 것을 볼 수 있습니다. 알아두면 좋습니다. 따라서 path와 mode는 꽤 자주 작성하게 될 것입니다.
...
이것은 모두 문서화되어 있으며, 이것을 수행하고 이전에 Nextflow를 사용한 적이 있다면, 여기 Nextflow 문서 nextflow.io/docs/로 갈 수 있습니다. 튜토리얼로 스크롤하면 *Migrating to Workflow Outputs*라는 튜토리얼이 있습니다.
...
첫째, 이 프로세스가 입력 변수를 받을 수 있다고 알려주어야 하며 여기에 *input*을 새로운 선언 블록으로 입력합니다. 그리고 *"val greeting"*이라고 부르겠습니다.
...
좋습니다. 따라서 *"input greeting"*이 *$\{greeting\}*으로 들어갑니다. 마지막으로 워크플로우 수준에서 Nextflow에게 이 프로세스가 이제 입력을 받는다고 알려주어야 합니다. 그리고 그렇게 하기 위해 기본적으로 변수를 제공할 것입니다.
...
그리고 그렇게 하는 방법은 *params*라는 특별한 Nextflow 개념입니다. *params.input*이라고 부르겠습니다.

이것이 하는 일은 CLI에서 이 input 변수를 노출하는 것이며, Nextflow를 시작할 때 이중 대시를 사용하는 곳입니다.

이것을 원하는 대로 *hello, greeting*이라고 부를 수 있습니다. 상관없습니다. 거기서 무엇을 하든 파이프라인을 시작할 때 CLI 옵션으로 노출됩니다. 그리고 이것은 Nextflow의 진정한 마법인데, 이러한 매개변수로 워크플로우 스크립트를 매우 빠르게 구축할 수 있고, 본질적으로 파이프라인에 대한 사용자 정의 CLI를 구축하여 시작할 때 다양한 옵션을 즉석에서 매우 쉽게 사용자 정의할 수 있기 때문입니다.

그래서. 시도해 봅시다. 터미널로 돌아갑니다. 여기에 _"nextflow run"_ 명령이 있습니다. 그리고 이제 *"--input"*을 수행하겠습니다. 이것은 앞서 본 *"params.input"*과 일치합니다. 문서에서는 프랑스어로 되어 있다고 생각합니다. Geraldine은 프랑스어를 좋아합니다. 저는 스웨덴에 살기 때문에 스웨덴어로 하겠습니다. "_Hej Världen_"이라고 하고 Enter를 누르겠습니다.

작은따옴표나 큰따옴표를 사용할 수 있으며, Bash가 어떻게 해석하는지에만 영향을 미칩니다.

Nextflow 파이프라인을 정확히 같은 방식으로 실행합니다. 작업 디렉토리와 모든 것이 동일한 것을 볼 수 있습니다. 하지만 이제 *"results/hello_world/output"*으로 가면. 대신 멋진 스웨덴어를 볼 수 있습니다.
...
이것을 작성할 때 *params.input*을 사용하는 곳에 바로 넣었다는 것을 알 수 있습니다. 따라서 명백한 해결책은 기본값을 정의하는 것이며, 여기 워크플로우의 스크립트 상단에 있는 특별한 params 블록에서 수행합니다. 이것은 여기 workflow 스크립트에 있습니다.
...
그런 다음 그 다음에 등호 기호가 있고 여기에 기본값이 있습니다. Nextflow는 원래 바르셀로나에서 작성되었으므로, 기본값으로 *"Holà mundo!"*와 같은 스페인어가 있는 것이 적절해 보입니다.

그 스크립트를 저장하고, 돌아가서, _--input_ 없이 스크립트를 다시 실행하겠습니다. 그리고 이번에는 실행되어야 하며 *results*에 새 파일을 생성할 것입니다. 그리고 이 파일에는 이제 *"Holà mundo!"*라고 나옵니다.

그러나 이것은 단지 기본값일 뿐이므로, 이전과 같은 작업을 여전히 수행할 수 없다는 의미는 아닙니다. 돌아가서 여기 이전 스크립트 *"Hej Världen"*을 찾으면, 명령줄에서 *--input*을 수행하기 때문에 그 기본값을 덮어쓰고 output.txt 파일에서 다시 사용할 것입니다.
...
다행히 Nextflow는 이전에 실행된 것과 사용 가능한 것을 아는 데 정말 능숙하며, 이전 결과를 재사용하는 것은 매우 간단합니다. 명령 끝에 새 플래그 *"-resume"*을 추가하기만 하면 됩니다.
...
좋습니다, 이제 *-resume*을 수행하고 정확히 같은 워크플로우를 다시 실행합니다. 그리고 이번에는 한 가지 주요 차이점을 제외하고 거의 정확히 동일하게 보일 것입니다.
...
이제 *-resume*을 수행하면 작업 디렉토리에서 마지막 파이프라인 실행을 재개하는데, 그것이 무엇이든 간에. 하지만 실제로 거기에서 수행한 이전 실행 중 어느 것에서나 재개할 수 있습니다. 그리고 이제 꽤 많이 했습니다.

## 4.2. 과거 실행의 로그 검사하기

모두 살펴보려면 _"nextflow run"_ 대신 *"nextflow log"*를 수행할 수 있으며, 그러면 이러한 모든 다른.. 화면을 조금 작게 만들어야 볼 수 있는데, 실행했을 때 이러한 모든 다른 실행, 세션 ID, 명령 및 모든 것을 보여주는 멋진 출력을 제공합니다.

그리고 여기를 살펴보고 이들 중 아무거나의 실행 이름을 가져와서 해당 특정 것 중 하나를 재개할 수 있습니다. 돌아가서 *hungry_ekeblad*라는 것을 재개할 수 있습니다. 그리고 resume 뒤에 넣으면 됩니다.
...
그런 다음 여기서 하나를 선택하고 또 다른 Nextflow 명령인 *"nextflow clean"*을 사용할 수 있으며, *"nextflow clean"*을 수행할 수 있습니다. *"-before"*를 수행하고 특정 실행 이름을 사용하겠습니다. 이 경우 *reverent_pike*였고 *"-n"*을 수행하겠습니다. 이것은 Nextflow에게 드라이 런만 수행하라고 알려줍니다. 따라서 무엇을 삭제할지 알려줍니다. 실제로 아무것도 하지 않고, 이러한 work 디렉토리를 제거할 것입니다.

합리적으로 보입니다. 따라서 같은 명령을 다시 수행하겠지만, _"-n"_ 대신 *"-f"*를 수행하여 실제로 정리를 수행하겠습니다. 그리고 이번에는 실제로 이러한 모든 디렉토리를 제거했습니다. 그리고 work 디렉토리를 살펴보면 이제 훨씬 가벼워 보입니다. 훌륭합니다.

따라서 캐시를 완전히 파괴하지 않고 모든 로컬 work 디렉토리를 꽤 안전한 방식으로 정리하는 방법입니다. 따라서 원한다면 여전히 재개할 수 있습니다.

이러한 플래그가 무엇인지 잊어버린 경우 모든 Nextflow 명령에 대해 *"nextflow help"*를 수행한 다음 명령 이름을 수행할 수 있습니다. *"nextflow help clean"*을 수행하면 모든 다른 옵션을 볼 수 있습니다: _-after, -before, -but_, 이 정리 동작을 구성하는 모든 다른 방법. 정말 멋집니다.

## 핵심 정리

좋습니다, Hello Nextflow의 파트 1이 끝났습니다. 과정의 꽤 강도 높은 시작이지만, 이제 Nextflow 스크립트가 어떻게 생겼는지 꽤 잘 이해하고 있기를 바랍니다. 다양한 핵심 부분, 프로세스, 워크플로우, 출력 및 매개변수. 명령줄에서 기본 재정의로 구성하는 방법, 동적 스크립트로 동적 입력 블록을 만드는 방법을 알고 있으며, 모든 워크플로우 실행을 관리하는 방법을 알고 있습니다: 이미 실행한 것을 보고, 재개하고, 정리하는 것. 많은 것들이 있습니다. 먼 길을 왔습니다. 따라서 휴식을 취하고 빠르게 산책하고 차 한 잔을 마시고 싶다면, 지금이 아마 좋은 시기입니다. 자격이 있습니다.

여기서부터는 기본적으로 이 기초를 쌓고 있습니다. 이것을 어떻게 더 복잡하고 강력하게 만들 수 있을까요? 어떻게 더 유연하게 만들 수 있을까요? 대규모로 분석을 수행하고 싶은 작업을 수행합니다.

## 퀴즈

이제 웹페이지의 파트 1, hello world로 스크롤하면 작은 퀴즈를 볼 수 있으며, 이것은 이 버전의 Nextflow 교육을 위해 수행한 새로운 것입니다. 그리고 이 챕터에서 다룬 모든 자료를 이해했는지 확인하기 위해 스스로 퀴즈를 풀 수 있습니다.
...
저처럼 VS Code 인스턴스의 터미널에 머물고 싶다면, _quiz_ 명령을 입력한 다음 어느 챕터에 있는지 알려주기만 하면 됩니다. *"Hello World"*를 수행하면 웹 브라우저에 있는 것과 정확히 동일한 퀴즈 질문을 수행할 수 있지만, 터미널에서만 수행할 수 있습니다.
