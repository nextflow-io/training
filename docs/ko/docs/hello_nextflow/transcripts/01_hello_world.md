...
콜론(:) 문자 뒤에 변수의 타입을 정의합니다. 필수는 아니며 비워 둘 수도 있지만, 매우 유용합니다. Nextflow에게 string을 기대하고 그렇게 처리하라고 알려줍니다.
...
그 다음에는 등호(=)와 기본값이 있습니다. Nextflow는 원래 바르셀로나에서 개발되었으므로, 기본값으로 스페인어 *"Hola mundo!"*를 사용하는 것이 적절합니다.

스크립트를 저장하고 _--input_ 없이 다시 실행합니다. 이번에는 정상적으로 실행되어 *results*에 새 파일이 생성됩니다. 파일에는 *"Hola mundo!"*가 표시됩니다.
