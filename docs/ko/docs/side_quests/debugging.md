...
이 과정은 디버깅을 지원하는 IDE 기능(구문 강조, 오류 감지 등)을 포괄적으로 다루며, 여기서 많이 사용하게 됩니다.
...
5. 다음 섹션으로 이동하기 전에 **파일 초기화**하기 (`git checkout <filename>` 사용)

연습은 간단한 구문 오류에서 더 미묘한 런타임 문제로 진행됩니다.
해결책은 인라인으로 논의되지만, 앞으로 읽기 전에 각각을 직접 해결해 보세요.
...
구문 오류는 Nextflow 코드를 작성할 때 마주치게 될 가장 일반적인 오류 유형입니다. 코드가 Nextflow DSL의 예상 구문 규칙을 따르지 않을 때 발생합니다. 이러한 오류는 워크플로우가 전혀 실행되지 않도록 하므로, 빠르게 식별하고 수정하는 방법을 학습하는 것이 중요합니다.

### 1.1. 중괄호 누락

가장 일반적인 구문 오류 중 하나이며, 때로는 디버깅하기 더 복잡한 오류 중 하나는 **괄호 누락 또는 불일치**입니다.
...
이 예제의 목적상 오류가 어디에 있는지 보여주는 주석을 남겨두었습니다. Nextflow VSCode 확장 프로그램도 무엇이 잘못되었는지에 대한 힌트를 제공하여 일치하지 않는 중괄호를 빨간색으로 표시하고 파일의 조기 종료를 강조 표시합니다:

![Bad syntax](img/bad_syntax.png)

**괄호 오류에 대한 디버깅 전략:**

1. VS Code의 괄호 매칭 사용 (괄호 옆에 커서 배치)
2. 괄호 관련 메시지에 대한 문제 패널 확인
3. 각 여는 `{`에 해당하는 닫는 `}`가 있는지 확인

#### 코드 수정

주석을 누락된 닫는 중괄호로 교체하세요:
...
또 다른 일반적인 구문 오류는 **잘못된 프로세스 정의**입니다. 필수 블록을 정의하는 것을 잊거나 프로세스 정의에서 잘못된 지시문을 사용하면 발생할 수 있습니다.
...
[문서](https://www.nextflow.io/docs/latest/process.html#)를 참조하여 잘못된 키워드를 올바른 것으로 교체하세요:
...
스크립트 블록에서 사용하는 변수 이름은 입력에서 파생되거나 스크립트 전에 삽입된 groovy 코드에서 파생된 유효한 것이어야 합니다. 하지만 파이프라인 개발 초기에 복잡성을 다루다 보면 변수 이름에서 실수하기 쉽고, Nextflow는 빠르게 알려줄 것입니다.
...
process PROCESS_FILES {
    ...
    echo "Processing ${sample_name}" > ${prefix}.txt  # 오류: ${prefix}는 Groovy 구문이지 Bash가 아님
    """
}
```
...
Nextflow VSCode 확장 프로그램은 오류를 일으킬 코드 구조 문제를 강조 표시합니다. 일반적인 예는 `workflow {}` 블록 외부에서 채널을 정의하는 것입니다 - 이것은 이제 구문 오류로 적용됩니다.
...
오류 메시지는 문제를 명확하게 나타냅니다: 문(채널 정의와 같은)은 워크플로우 또는 프로세스 블록 외부의 스크립트 선언과 혼합될 수 없습니다.
...
VSCode 확장 프로그램도 워크플로우 블록 외부에서 정의된 `input_ch` 변수를 강조 표시합니다:

![Non-lethal syntax error](img/nonlethal.png)

#### 코드 수정

채널 정의를 워크플로우 블록 내부로 이동하세요:
...
워크플로우를 다시 실행하여 수정이 작동하는지 확인하세요:
...
Nextflow 오류 메시지와 IDE 시각적 표시기를 사용하여 구문 오류를 체계적으로 식별하고 수정할 수 있습니다. 일반적인 구문 오류에는 중괄호 누락, 잘못된 프로세스 키워드, 정의되지 않은 변수, Bash vs Nextflow 변수의 부적절한 사용이 포함됩니다. VSCode 확장 프로그램은 런타임 전에 많은 것을 포착하는 데 도움이 됩니다. 이러한 구문 디버깅 기술을 도구 상자에 넣으면 가장 일반적인 Nextflow 구문 오류를 빠르게 해결하고 더 복잡한 런타임 문제를 해결하는 데 집중할 수 있습니다.

### 다음 단계

구문이 올바른 경우에도 발생하는 더 복잡한 채널 구조 오류를 디버깅하는 방법을 학습하세요.
...
!!! tip "`.view()`로 채널 디버깅"
    ...
    이 섹션 전체에서 `.view()` 연산자를 사용하여 워크플로우의 어느 지점에서든 채널 내용을 검사할 수 있다는 것을 기억하세요. 이것은 채널 구조 문제를 이해하기 위한 가장 강력한 디버깅 도구 중 하나입니다. 섹션 2.4에서 이 기법을 자세히 학습하겠지만, 예제를 진행하면서 자유롭게 사용하세요.
...
오류 메시지는 호출이 1개의 인수를 예상했지만 2개를 받았다고 명확하게 말하며 23줄을 가리킵니다. `bad_number_inputs.nf`를 검토해 봅시다:
...
process PROCESS_FILES {
    input:
        val sample_name  // 프로세스는 1개의 입력만 예상
...
workflow {
    ...
    // 오류: 2개의 채널을 전달하지만 프로세스는 1개만 예상
    PROCESS_FILES(samples_ch, files_ch)
}
```

프로세스가 하나만 정의할 때 여러 입력 채널을 제공하는 일치하지 않는 `PROCESS_FILES` 호출을 볼 수 있습니다. VSCode 확장 프로그램도 프로세스 호출에 빨간색 밑줄을 표시하고 마우스를 올리면 진단 메시지를 제공합니다:
...
일부 채널 구조 오류는 훨씬 더 미묘하며 전혀 오류를 생성하지 않습니다. 아마도 이러한 것 중 가장 일반적인 것은 새로운 Nextflow 사용자가 큐 채널이 소진되어 항목이 부족해질 수 있다는 것을 이해하는 데 직면하는 문제를 반영하며, 이는 워크플로우가 조기에 완료됨을 의미합니다.
...
`exhausted.nf`를 검토하여 그것이 맞는지 봅시다:
...
**옵션 1**: 많이 재사용하는 단일 참조 파일이 있습니다. 반복해서 사용할 수 있는 값 채널 유형을 간단히 만들 수 있습니다. 이를 수행하는 세 가지 방법이 있습니다:
...
**옵션 2**: 더 복잡한 시나리오에서, 아마도 샘플 채널의 모든 샘플에 대한 여러 참조 파일이 있는 경우, `combine` 연산자를 사용하여 두 채널을 튜플로 결합하는 새 채널을 만들 수 있습니다:
...
이를 위해서는 프로세스 입력을 조정해야 합니다. 예제에서 프로세스 정의의 시작 부분을 다음과 같이 조정해야 합니다:
...
위의 수정 중 하나를 시도하고 워크플로우를 다시 실행하세요:
...
이제 하나가 아닌 세 개의 샘플이 모두 처리되는 것을 볼 수 있습니다.
...
튜플로 구성된 채널을 생성하고 있음을 볼 수 있습니다: `['sample1', 'file1.txt']`, 하지만 프로세스는 단일 값 `val sample_name`을 예상합니다. 실행된 명령은 프로세스가 `[sample3, file3.txt]_output.txt`라는 파일을 만들려고 시도하고 있음을 보여주며, 이것은 의도된 출력이 아닙니다.
...
해결책 중 하나를 선택하고 워크플로우를 다시 실행하세요:
...
채널을 위한 가장 강력한 디버깅 도구는 `.view()` 연산자입니다. `.view()`를 사용하면 모든 단계에서 채널의 형태를 이해하여 디버깅에 도움이 될 수 있습니다.

#### 파이프라인 실행

`bad_channel_shape_viewed.nf`를 실행하여 이것이 작동하는 것을 보세요:
...
프로세스 정의로 인해 생성된 오류에 대해 학습하세요.

---

## 3. 프로세스 구조 오류

프로세스와 관련하여 마주치게 될 대부분의 오류는 명령을 형성하는 데 실수를 했거나 기본 소프트웨어와 관련된 문제와 관련이 있습니다. 그렇긴 하지만 위의 채널 문제와 유사하게 구문 오류로 인정되지 않지만 런타임에 오류를 일으킬 프로세스 정의에서 실수를 할 수 있습니다.
...
`output:` 블록의 출력 파일 이름과 스크립트에서 사용된 것 사이에 불일치가 있음을 볼 수 있습니다. 이 불일치로 인해 프로세스가 실패합니다. 이러한 종류의 오류가 발생하면 돌아가서 프로세스 정의와 출력 블록 사이에 출력이 일치하는지 확인하세요.

문제가 여전히 명확하지 않으면 작업 디렉토리 자체를 확인하여 실제로 생성된 출력 파일을 식별하세요:
...
이 예제의 경우 `_output` 접미사가 `output:` 정의와 반대로 출력 파일 이름에 통합되고 있음을 강조합니다.

#### 코드 수정

출력 파일 이름을 일관되게 만들어 불일치를 수정하세요:
...
프로세스가 지정하는 명령에 액세스할 수 없습니다. 때로는 스크립트가 워크플로우 `bin` 디렉토리에 있지만 실행 가능하게 만들어지지 않았기 때문입니다. 다른 경우에는 워크플로우가 실행되는 컨테이너 또는 환경에 소프트웨어가 설치되지 않았기 때문입니다.
...
여기서 조금 부정직했고 실제로 코드에는 아무런 문제가 없습니다. 문제의 명령에 액세스할 수 있는 방식으로 프로세스를 실행하는 데 필요한 구성을 지정하기만 하면 됩니다. 이 경우 프로세스에 컨테이너 정의가 있으므로 Docker를 활성화하여 워크플로우를 실행하기만 하면 됩니다.

#### 파이프라인 실행

`nextflow.config`에 Docker 프로파일을 설정했으므로 다음과 같이 워크플로우를 실행할 수 있습니다:
...
프로덕션 사용에서는 프로세스에 리소스를 구성하게 됩니다. 예를 들어 `memory`는 프로세스에 사용 가능한 최대 메모리 양을 정의하며, 프로세스가 이를 초과하면 스케줄러는 일반적으로 프로세스를 종료하고 `137`의 종료 코드를 반환합니다. `local` 실행자를 사용하고 있기 때문에 여기서는 이를 시연할 수 없지만 `time`으로 유사한 것을 보여줄 수 있습니다.

#### 파이프라인 실행

`bad_resources.nf`는 1밀리초의 비현실적인 시간 제한이 있는 프로세스 구성을 가지고 있습니다:
...
프로세스가 실패하거나 예상치 못하게 동작할 때 무엇이 잘못되었는지 조사하기 위한 체계적인 기법이 필요합니다. 작업 디렉토리에는 프로세스 실행을 이해하는 데 필요한 모든 정보가 포함되어 있습니다.
...
이 오류가 발생하면 작업 디렉토리에 모든 디버깅 정보가 포함됩니다. 오류 메시지에서 작업 디렉토리 경로를 찾아 내용을 검사하세요:
...
출력 파일 누락으로 인해 프로세스가 실패하면 실제로 생성된 파일을 확인하세요:
...
Nextflow의 내장 디버깅 도구와 문제 해결에 대한 체계적 접근법에 대해 학습하세요.
...
앞서 예제의 `bad_channel_shape_viewed.nf`는 `.view()`를 사용하여 채널 내용을 출력했지만, `debug` 지시문을 사용하여 프로세스 자체 내에서 변수를 에코할 수도 있으며, 이를 `bad_channel_shape_viewed_debug.nf`에서 시연합니다. 워크플로우를 실행하세요:
...
`missing_software.nf`에 비해 이 프로세스는 Nextflow가 스텁 모드에서 실행되는 경우 `script:`에 지정된 것 대신 사용할 명령을 지정하는 `stub:` 지시문을 가지고 있습니다.

여기서 사용하는 `touch` 명령은 소프트웨어나 적절한 입력에 의존하지 않으며 모든 상황에서 실행되어 프로세스 내부에 대해 걱정하지 않고 워크플로우 로직을 디버깅할 수 있습니다.
...
이제 추적 파일과 작업 디렉토리에서 미리보기 모드, 스텁 실행, 리소스 모니터링에 이르기까지 개별 디버깅 기법을 학습했으므로 이를 체계적인 방법론으로 묶어봅시다. 구조화된 접근 방식을 갖추면 복잡한 오류에 압도되지 않고 중요한 단서를 놓치지 않을 수 있습니다.
...
!!! tip "효율적인 디버깅을 위한 Resume 사용"
    ...
    문제를 식별한 후에는 워크플로우의 성공적인 부분을 다시 실행하는 데 시간을 낭비하지 않고 수정을 테스트하는 효율적인 방법이 필요합니다. Nextflow의 `-resume` 기능은 디버깅에 매우 유용합니다.
    ...
    **사용 가능한 디버깅 도구:**
    ...
    ??? solution
        ...
        **오류 5: 채널 구조 오류 - 잘못된 맵 출력**
        ```groovy linenums="83"
        .map { row -> row.sample_id }  // 오류: processFiles는 튜플을 예상
        ```
        **수정:** processFiles가 예상하는 튜플 구조 반환
        ...
        하지만 이것은 위의 `heavyProcess()` 실행을 위한 수정을 깨뜨리므로 맵을 사용하여 해당 프로세스에 샘플 ID만 전달해야 합니다:

        **오류 6: heavyProcess에 대한 잘못된 채널 구조**
        ```groovy linenums="87"
        heavy_ch = heavyProcess(input_ch)  // 오류: input_ch는 이제 방출당 2개의 요소를 가짐 - heavyProcess는 1개만 필요(첫 번째)
        ```
        **수정:** 올바른 채널 사용 및 샘플 ID 추출
        ```groovy linenums="87"
        ...
        이제 조금 더 진행되지만 `No such variable: i`에 대한 오류를 받습니다. Bash 변수를 이스케이프하지 않았기 때문입니다.
...
- Nextflow 오류 메시지를 해석하고 문제 찾기
- 일반적인 구문 오류: 중괄호 누락, 잘못된 키워드, 정의되지 않은 변수
- Nextflow(Groovy)와 Bash 변수 구별
- 조기 오류 감지를 위한 VS Code 확장 기능 사용

```groovy
// 중괄호 누락 - IDE에서 빨간색 밑줄 찾기
process FOO {
    script:
    """
    echo "hello"
...
- [Nextflow 채널 이해하기](https://www.nextflow.io/docs/latest/channel.html): 채널 유형 및 동작에 대한 심층 분석
...
**기억하세요:** 효과적인 디버깅은 연습으로 향상되는 기술입니다. 여기서 습득한 체계적인 방법론과 포괄적인 도구 키트는 Nextflow 개발 여정 전반에 걸쳐 여러분에게 도움이 될 것입니다.
