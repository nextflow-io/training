...
구문이 올바른 경우에도 발생하는 더 복잡한 채널 구조 오류를 디버그하는 방법을 학습합니다.
...
!!! tip "`.view()`로 채널 디버깅"
    ...
    이 섹션 전체에서 `.view()` 연산자를 사용하여 워크플로우의 어느 지점에서든 채널 내용을 검사할 수 있다는 것을 기억하십시오. 이것은 채널 구조 문제를 이해하기 위한 가장 강력한 디버깅 도구 중 하나입니다. 섹션 2.4에서 이 기법을 자세히 다룰 것이지만, 예제를 작업하면서 자유롭게 사용하십시오.
...
```groovy title="exhausted.nf (수정됨 - 옵션 1a)" hl_lines="2" linenums="21"
workflow {
...
```groovy title="exhausted.nf (수정됨 - 옵션 1b)" hl_lines="2" linenums="21"
workflow {
...
```groovy title="exhausted.nf (수정됨 - 옵션 1c)" hl_lines="2" linenums="21"
workflow {
...
```groovy title="exhausted.nf (수정됨 - 옵션 2)" hl_lines="4" linenums="21"
workflow {
...
```groovy title="exhausted.nf (수정됨 - 옵션 2)" hl_lines="5" linenums="1"
#!/usr/bin/env nextflow
...
이것은 워크플로우가 복잡성이 증가하고 채널 구조가 더 불투명해질 때 더욱 중요해질 것입니다.
...
많은 채널 구조 오류는 유효한 Nextflow 구문으로 생성될 수 있습니다. 데이터 흐름을 이해하고, 검사를 위해 `.view()` 연산자를 사용하고, 예상치 못한 튜플 구조를 나타내는 대괄호와 같은 오류 메시지 패턴을 인식하여 채널 구조 오류를 디버그할 수 있습니다.

### 다음 단계

프로세스 정의로 인해 생성되는 오류에 대해 학습합니다.

---

## 3. 프로세스 구조 오류

프로세스와 관련하여 마주치게 될 대부분의 오류는 명령을 형성하는 데 실수를 하거나 기본 소프트웨어와 관련된 문제와 관련이 있을 것입니다. 그렇긴 하지만, 위의 채널 문제와 유사하게, 구문 오류로 분류되지 않지만 런타임에 오류를 일으킬 프로세스 정의에서 실수를 할 수 있습니다.
...
오류 메시지는 프로세스가 `sample3.txt`라는 출력 파일을 생성할 것으로 예상했지만 스크립트가 실제로 `sample3_output.txt`를 생성한다고 나타냅니다. `missing_output.nf`의 프로세스 정의를 살펴보겠습니다:
...
`output:` 블록의 출력 파일 이름과 스크립트에서 사용된 것 사이에 불일치가 있음을 볼 수 있어야 합니다. 이 불일치로 인해 프로세스가 실패합니다. 이러한 종류의 오류가 발생하면 프로세스 정의와 출력 블록 사이에 출력이 일치하는지 다시 확인하십시오.

문제가 여전히 명확하지 않으면 작업 디렉토리 자체를 확인하여 실제로 생성된 출력 파일을 식별하십시오:
...
이 예제의 경우 이것은 `output:` 정의와 반대로 출력 파일 이름에 `_output` 접미사가 포함되고 있음을 강조할 것입니다.
...
=== "수정 후"
    ...
    process PROCESS_FILES {
        ...
        output:
        path "${sample_name}_output.txt"  // 수정됨: 스크립트 출력과 일치
...
또 다른 종류의 오류는 소프트웨어 프로비저닝의 실수로 인해 발생합니다. `missing_software.nf`는 구문적으로 유효한 워크플로우이지만, 사용하는 `cowpy` 명령을 제공하는 일부 외부 소프트웨어에 의존합니다.
...
`127` 종료 코드를 주의하십시오 - 정확히 문제를 알려줍니다. `missing_software.nf`를 살펴보겠습니다:
...
여기서 우리는 약간 부정직했고, 실제로 코드에는 아무런 문제가 없습니다. 문제의 명령에 액세스할 수 있는 방식으로 프로세스를 실행하는 데 필요한 구성을 지정하기만 하면 됩니다. 이 경우 프로세스에 컨테이너 정의가 있으므로 Docker를 활성화하여 워크플로우를 실행하기만 하면 됩니다.
...
!!! note
    ...
    Nextflow가 컨테이너를 사용하는 방법에 대해 자세히 알아보려면 [Hello Nextflow](../hello_nextflow/05_hello_containers.md)를 참조하십시오.

### 3.3. 잘못된 리소스 구성

프로덕션 사용에서는 프로세스에 리소스를 구성하게 될 것입니다. 예를 들어 `memory`는 프로세스에 사용 가능한 최대 메모리 양을 정의하며, 프로세스가 이를 초과하면 스케줄러는 일반적으로 프로세스를 종료하고 `137`의 종료 코드를 반환합니다. `local` executor를 사용하고 있기 때문에 여기서는 이를 시연할 수 없지만, `time`으로 유사한 것을 보여줄 수 있습니다.
...
프로세스가 1초 이상 걸릴 것을 알고 있지만(확실히 하기 위해 sleep을 추가했습니다), 프로세스는 1밀리초 후에 시간 초과되도록 설정되어 있습니다. 누군가가 구성에서 약간 비현실적이었습니다!

#### 코드 수정

시간 제한을 현실적인 값으로 증가시킵니다:
...
앞서의 `missing_output.nf` 예제를 사용하여 작업 디렉토리 검사를 시연하겠습니다(필요한 경우 출력 이름 불일치를 다시 생성하십시오):
...
이 오류가 발생하면 작업 디렉토리에 모든 디버깅 정보가 포함됩니다. 오류 메시지에서 작업 디렉토리 경로를 찾아 내용을 검사하십시오:
...
- **누락된 따옴표**: 공백을 포함하는 변수는 적절한 따옴표가 필요함
...
누락된 출력 파일로 인해 프로세스가 실패하면 실제로 생성된 파일을 확인하십시오:
...
Nextflow는 워크플로우 실행을 디버깅하고 분석하기 위한 여러 강력한 내장 도구를 제공합니다. 이러한 도구는 무엇이 잘못되었는지, 어디서 잘못되었는지, 그리고 효율적으로 수정하는 방법을 이해하는 데 도움이 됩니다.
...
`bad_channel_shape_viewed_debug.nf`를 살펴보고 `debug` 지시문이 어떻게 작동하는지 확인하겠습니다:
...
때로는 프로세스가 실행되기 전에 문제를 포착하고 싶을 때가 있습니다. Nextflow는 이러한 종류의 사전 디버깅을 위한 플래그를 제공합니다: `-preview`.

#### 파이프라인 실행

미리보기 모드를 사용하면 명령을 실행하지 않고 워크플로우 로직을 테스트할 수 있습니다. 이는 워크플로우의 구조를 빠르게 확인하고 실제 명령을 실행하지 않고 프로세스가 올바르게 연결되었는지 확인하는 데 매우 유용할 수 있습니다.

!!! note

    앞서 `bad_syntax.nf`를 수정했다면, 이 명령을 실행하기 전에 스크립트 블록 뒤의 닫는 중괄호를 제거하여 구문 오류를 다시 도입하십시오.
...
때로는 명령이 너무 오래 걸리거나, 특수 소프트웨어가 필요하거나, 복잡한 이유로 실패하기 때문에 오류를 디버그하기 어렵습니다. Stub 실행을 사용하면 실제 명령을 실행하지 않고 워크플로우 로직을 테스트할 수 있습니다.
...
예를 들어, 앞서의 `missing_software.nf`를 기억하십니까? `-profile docker`를 추가할 때까지 워크플로우가 실행되지 않도록 하는 누락된 소프트웨어가 있었던 것? `missing_software_with_stub.nf`는 매우 유사한 워크플로우입니다. 같은 방식으로 실행하면 같은 오류가 생성됩니다:
...
그러나 이 워크플로우는 `docker` 프로필 없이도 `-stub-run`으로 실행하면 오류를 생성하지 않습니다:
...
```groovy title="missing_software.nf (stub 포함)" hl_lines="16-19" linenums="3"
process PROCESS_FILES {
...
`missing_software.nf`와 비교하여, 이 프로세스는 Nextflow가 stub 모드에서 실행되는 경우 `script:`에 지정된 것 대신 사용할 명령을 지정하는 `stub:` 지시문을 가지고 있습니다.

여기서 사용하는 `touch` 명령은 소프트웨어나 적절한 입력에 의존하지 않으며 모든 상황에서 실행되어 프로세스 내부에 대해 걱정하지 않고 워크플로우 로직을 디버그할 수 있습니다.

**Stub 실행이 디버그하는 데 도움이 되는 것:**

- 채널 구조 및 데이터 흐름
- 프로세스 연결 및 의존성
- 매개변수 전파
- 소프트웨어 의존성 없는 워크플로우 로직

### 4.4. 체계적 디버깅 접근법

이제 추적 파일과 작업 디렉토리에서 미리보기 모드, stub 실행, 리소스 모니터링에 이르기까지 개별 디버깅 기법을 배웠으므로, 이를 체계적인 방법론으로 묶어보겠습니다. 구조화된 접근 방식을 갖추면 복잡한 오류에 압도되지 않고 중요한 단서를 놓치지 않도록 할 수 있습니다.
...
!!! tip "효율적인 디버깅을 위한 Resume 사용"
    ...
    문제를 식별한 후에는 성공한 워크플로우 부분을 다시 실행하는 데 시간을 낭비하지 않고 수정 사항을 테스트하는 효율적인 방법이 필요합니다. Nextflow의 `-resume` 기능은 디버깅에 매우 유용합니다.

    [Hello Nextflow](../hello_nextflow/)를 진행했다면 `-resume`을 접했을 것이며, 디버깅할 때 문제 프로세스 이전의 프로세스가 실행되는 동안 기다리는 시간을 절약하기 위해 이를 잘 활용하는 것이 중요합니다.
...
```groovy title="nextflow.config (debug 프로필)" linenums="1"
profiles {
    debug {
        process {
            debug = true
...
이제 체계적인 디버깅 접근법을 실제로 적용할 시간입니다. 워크플로우 `buggy_workflow.nf`에는 실제 개발에서 마주치게 될 문제 유형을 나타내는 여러 일반적인 오류가 포함되어 있습니다.

!!! exercise

    체계적인 디버깅 접근법을 사용하여 `buggy_workflow.nf`의 모든 오류를 식별하고 수정하십시오. 이 워크플로우는 CSV 파일에서 샘플 데이터를 처리하려고 시도하지만 일반적인 디버깅 시나리오를 나타내는 여러 의도적인 버그를 포함하고 있습니다.

    워크플로우를 실행하여 첫 번째 오류를 확인하는 것으로 시작하십시오:
    ...
    **사용 가능한 디버깅 도구:**
    ...
    ??? solution
        `buggy_workflow.nf`에는 모든 주요 디버깅 범주를 다루는 9개 또는 10개의 별개 오류가 포함되어 있습니다(세는 방법에 따라). 다음은 각 오류와 수정 방법에 대한 체계적인 분석입니다.
        ...
        **오류 2: 구문 오류 - 닫는 중괄호 누락**
        ...
        """
        // 오류: processFiles process의 닫는 중괄호 누락
        ```
        **수정:** 누락된 닫는 중괄호 추가
        ```groovy linenums="29"
        """
        ...
        **오류 5: 채널 구조 오류 - 잘못된 Map 출력**
        ```groovy linenums="83"
        .map { row -> row.sample_id }  // 오류: processFiles는 튜플을 예상함
        ```
        **수정:** processFiles가 예상하는 튜플 구조 반환
        ...
        하지만 이것은 위의 `heavyProcess()` 실행을 중단시킬 것이므로 map을 사용하여 해당 프로세스에 샘플 ID만 전달해야 합니다:

        **오류 6: heavyProcess의 잘못된 채널 구조**
        ```groovy linenums="87"
        heavy_ch = heavyProcess(input_ch)  // 오류: input_ch는 이제 방출당 2개의 요소를 가짐 - heavyProcess는 1개만 필요함(첫 번째)
        ```
        **수정:** 올바른 채널 사용 및 샘플 ID 추출
        ```groovy linenums="87"
        ...
        * 이 워크플로우는 학습 목적으로 여러 의도적인 버그를 포함합니다
        */
...
이 사이드 퀘스트에서는 Nextflow 워크플로우를 디버깅하기 위한 체계적인 기법 세트를 학습했습니다.
자신의 작업에 이러한 기법을 적용하면 컴퓨터와 싸우는 데 소비하는 시간을 줄이고, 문제를 더 빠르게 해결하며, 미래의 문제로부터 자신을 보호할 수 있습니다.

### 주요 패턴

**1. 구문 오류를 식별하고 수정하는 방법**:

- Nextflow 오류 메시지 해석 및 문제 위치 찾기
...
- 누락된 출력 파일 오류 진단
- 종료 코드 이해 (누락된 소프트웨어는 127, 메모리 문제는 137)
- 작업 디렉토리 및 명령 파일 조사
- 리소스를 적절하게 구성
...
!!! tip "빠른 디버깅 참조"
    ...
    **채널 문제?** → 내용 검사를 위해 `.view()` 사용: `my_channel.view()`
    ...
    **이상한 동작?** → 워크플로우 로직을 테스트하기 위해 `-stub-run`으로 실행

    **수정했나요?** → 시간을 절약하기 위해 `-resume` 사용: `nextflow run workflow.nf -resume`
...
- [Nextflow 채널 이해하기](https://www.nextflow.io/docs/latest/channel.html): 채널 타입 및 동작에 대한 심층 분석
...
프로덕션 워크플로우의 경우 다음을 고려하십시오:

- 대규모 모니터링 및 디버깅을 위한 [Seqera Platform](https://seqera.io/platform/) 설정
- 재현 가능한 소프트웨어 환경을 위한 [Wave 컨테이너](https://seqera.io/wave/) 사용

**기억하십시오:** 효과적인 디버깅은 연습으로 향상되는 기술입니다. 여기서 습득한 체계적인 방법론과 포괄적인 도구 키트는 Nextflow 개발 여정 전반에 걸쳐 잘 활용될 것입니다.
