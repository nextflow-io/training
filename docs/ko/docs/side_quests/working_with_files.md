...
대부분의 과학 분야에서 데이터를 포함하는 파일 이름에는 메타데이터가 인코딩되어 있는 것이 매우 일반적입니다.
...
물론 이것은 큰 '만약'이며, 파일 이름 구조에 의존할 때마다 매우 주의해야 합니다. 그러나 현실은 이 접근 방식이 매우 널리 사용되므로 Nextflow에서 어떻게 수행되는지 살펴보겠습니다.

예제 데이터의 경우 파일 이름에 일관되게 구조화된 메타데이터가 포함되어 있습니다.
예를 들어 파일 이름 `patientA_rep1_normal_R2_001`은 다음을 인코딩합니다:

- 환자 ID: `patientA`
- 복제 ID: `rep1`
- 샘플 타입: `normal` (`tumor`와 대조)
- 리드 세트: `R1` (`R2`와 대조)

이 정보를 세 단계로 검색하도록 워크플로우를 수정하겠습니다:
...
!!! warning
    ...
    파일 이름에 환자 이름이나 기타 식별 특성과 같은 민감한 정보를 인코딩해서는 안 됩니다. 이는 환자 프라이버시 또는 기타 관련 보안 제한을 침해할 수 있기 때문입니다.
...
이것은 `simpleName`을 검색하고 `map()` 연산을 사용하여 전체 파일 객체와 연결합니다.

워크플로우를 실행하여 작동하는지 테스트하십시오:
...
이 시점에서 원하는 메타데이터가 `simplename`에 포함되어 있지만 개별 항목에 직접 접근할 수 없습니다.
따라서 `simplename`을 구성 요소로 분할해야 합니다.
다행히 이러한 구성 요소는 원본 파일 이름에서 단순히 밑줄로 구분되므로, 이 작업에 완벽한 `tokenize()`라는 일반적인 Nextflow 메서드를 적용할 수 있습니다.
...
`tokenize()` 메서드는 밑줄을 찾을 때마다 `simpleName` 문자열을 분할하고 하위 문자열을 포함하는 리스트를 반환합니다.
...
이제 채널의 각 요소에 대한 튜플에는 메타데이터 리스트(_예:_ `[patientA, rep1, normal, R1, 001]`)와 원본 파일 객체가 포함되어 있습니다.
...
사용하기에는 충분히 쉽지만 읽기는 어렵습니다.

```console
[patientA, rep1, normal, R1, 001]
```

인덱스 3의 항목은 무엇입니까? 메타데이터 구조의 원래 설명을 다시 참조하지 않고 말할 수 있습니까?

이것은 키-값 저장소를 사용할 좋은 기회입니다. 여기서 모든 항목에는 키 세트와 관련 값이 있어 각 키를 쉽게 참조하여 해당 값을 얻을 수 있습니다.
...
- **분해 할당**: `def (patient, replicate, type, readNum) = ...`은 토큰화된 값을 한 줄로 명명된 변수로 추출합니다
- **Map 리터럴 구문**: `[id: patient, replicate: ...]`은 각 키(예: `id`)가 값(예: `patient`)과 연결된 map을 생성합니다
- **중첩 구조**: 외부 리스트 `[..., myFile]`은 메타데이터 map을 원본 파일 객체와 쌍으로 만듭니다

또한 `replace()`라는 문자열 치환 메서드를 사용하여 일부 메타데이터 문자열을 단순화했습니다. 이는 불필요한 문자를 제거하기 위함입니다(_예:_ `replicate.replace('rep', '')`는 복제 ID에서 숫자만 유지).
...
이제 메타데이터가 깔끔하게 레이블이 지정되어 있어(_예:_ `[id:patientA, replicate:1, type:normal, readNum:2]`) 무엇이 무엇인지 구별하기가 훨씬 쉽습니다.

또한 워크플로우에서 메타데이터 요소를 실제로 사용하는 것이 훨씬 쉬워지고 코드가 더 읽기 쉽고 유지 관리하기 쉬워집니다.

### 핵심 정리

- Nextflow에서 완전한 프로그래밍 언어의 강력함으로 파일 이름을 처리할 수 있습니다
- 파일 이름을 문자열로 처리하여 관련 정보를 추출할 수 있습니다
- `tokenize()` 및 `replace()`와 같은 메서드를 사용하여 파일 이름의 문자열을 조작할 수 있습니다
- `.map()` 연산은 구조를 유지하면서 채널 요소를 변환합니다
...
많은 실험 설계에서는 명시적으로 페어드 방식으로 처리하면 이점이 있는 페어드 데이터 파일을 생성합니다.
예를 들어 생물정보학에서는 시퀀싱 데이터가 종종 페어드 리드 형태로 생성됩니다. 이는 동일한 DNA 단편에서 유래한 시퀀스 문자열을 의미합니다(종종 반대 끝에서 읽히기 때문에 '정방향'과 '역방향'이라고 불림).
...
Nextflow는 이와 같은 페어드 파일 작업을 위해 `channel.fromFilePairs()`라는 특수 채널 팩토리를 제공합니다. 이는 공유된 명명 패턴을 기반으로 파일을 자동으로 그룹화합니다. 이를 통해 더 적은 노력으로 페어드 파일을 더 밀접하게 연결할 수 있습니다.

이를 활용하도록 워크플로우를 수정하겠습니다.
두 단계가 필요합니다:
...
이것은 이전에 사용한 glob 패턴과 유사하지만, 쌍의 두 멤버를 식별하는 하위 문자열(R 바로 뒤에 오는 `1` 또는 `2`)을 구체적으로 열거합니다.
...
채널 팩토리를 전환하고 파일 매칭 패턴을 조정했으며, 그러는 동안 map 연산을 주석 처리했습니다.
...
우리가 해온 모든 메타데이터 조작은 실제로 채널 내용에 영향을 주지 않았습니다.

이제 `.fromFilePairs` 채널 팩토리를 사용하고 있으므로 결과 채널의 내용이 다릅니다.
두 파일이 공유하는 `simpleName`의 일부(식별자 역할을 하는)와 두 파일 객체를 포함하는 튜플, `id, [ file1, file2 ]` 형식으로 구성된 하나의 채널 요소만 표시됩니다.

이것은 훌륭합니다. Nextflow가 공유된 접두사를 검사하고 환자 식별자로 사용하여 환자 이름을 추출하는 어려운 작업을 수행했기 때문입니다.

그러나 현재 워크플로우가 중단됩니다.
process를 변경하지 않고 여전히 `COUNT_LINES`를 동일한 방식으로 실행하려면 파일 경로를 추출하기 위해 매핑 연산을 적용해야 합니다.
그러나 우리는 그렇게 하지 않을 것입니다. 최종 목표는 파일 쌍을 적절하게 처리하는 다른 process인 `ANALYZE_READS`를 사용하는 것이기 때문입니다.

따라서 `COUNT_LINES` 호출을 주석 처리(또는 삭제)하고 계속 진행합시다.
...
좋습니다, 이번에는 워크플로우가 성공합니다!

그러나 `id` 필드에서 나머지 메타데이터를 추출해야 합니다.

### 5.2. 파일 쌍에서 메타데이터 추출 및 구성

이전의 `map` 연산은 데이터 구조와 일치하지 않기 때문에 작동하지 않지만, 작동하도록 수정할 수 있습니다.

`fromFilePairs()`가 식별자로 사용한 문자열에서 실제 환자 식별자에 이미 접근할 수 있으므로, 이전에 했던 것처럼 Path 객체에서 `simpleName`을 가져오지 않고 메타데이터를 추출하는 데 사용할 수 있습니다.

워크플로우에서 map 연산의 주석을 해제하고 다음과 같이 편집하십시오:
...
이번에는 map이 단순히 `myFile` 대신 `id, files`에서 시작하며, `myFile.simpleName` 대신 `id`에 `tokenize()`가 적용됩니다.

또한 `tokenize()` 줄에서 `readNum`을 삭제했습니다. 왼쪽에서 시작하여 구체적으로 명명하지 않은 하위 문자열은 자동으로 삭제됩니다.
페어드 파일이 이제 밀접하게 연결되어 있으므로 메타데이터 map에서 `readNum`이 더 이상 필요하지 않기 때문에 이렇게 할 수 있습니다.
...
그리고 여기 있습니다: 의도한 대로 출력 튜플의 첫 번째 위치에 메타데이터 map(`[id:patientA, replicate:1, type:normal]`)이 있고, 그 뒤에 페어드 파일의 튜플이 있습니다.
...
- [`channel.fromFilePairs()` 자동으로 관련 파일을 찾아 페어링합니다](https://www.nextflow.io/docs/latest/reference/channel.html#fromfilepairs)
- 이것은 파이프라인에서 페어드 엔드 리드를 처리하는 것을 단순화합니다
...
메타데이터 튜플과 입력 파일 쌍을 받아 분석하는 `ANALYZE_READS`라는 미리 작성된 process 모듈을 제공합니다.
이것이 시퀀스 정렬, 변이 호출 또는 이 데이터 타입에 적합한 다른 단계를 수행한다고 상상할 수 있습니다.
...
```groovy title="modules/analyze_reads.nf - process 예제" linenums="1"
#!/usr/bin/env nextflow
...
!!! note
    ...
    관례상 메타데이터 map을 `meta`라고 부릅니다.
    메타 map에 대한 심층 분석은 [Metadata and meta maps](./metadata.md) 사이드 퀘스트를 참조하십시오.
...
#### 6.2.1. 재매핑된 입력 채널에 이름 지정

이전에는 입력 채널에 직접 매핑 조작을 적용했습니다.
재매핑된 내용을 `ANALYZE_READS` process에 공급하기 위해(명확하고 읽기 쉬운 방식으로) `ch_samples`라는 새 채널을 생성하려고 합니다.

[`set`](https://www.nextflow.io/docs/latest/reference/operator.html#set) 연산자를 사용하여 수행할 수 있습니다.
...
물론 이것은 단일 환자에 대한 단일 파일 쌍만 처리하는 것이므로 Nextflow로 기대하는 높은 처리량과는 거리가 멉니다.
한 번에 훨씬 더 많은 데이터를 처리하고 싶을 것입니다.

`channel.fromPath()`는 입력으로 *glob*을 허용하므로 패턴과 일치하는 파일을 원하는 만큼 받아들일 수 있다는 것을 기억하십시오.
따라서 모든 환자를 포함하려면 입력 문자열을 수정하여 더 많은 환자를 포함하면 됩니다. 이는 이전에 간단히 언급한 대로입니다.

가능한 한 탐욕스럽게 하고 싶다고 가정해 봅시다.
워크플로우를 다음과 같이 편집하십시오:
...
성공! 한 번에 모든 환자를 분석했습니다! 맞습니까?

아마 아닐 것입니다.
더 자세히 살펴보면 문제가 있습니다: patientA에 대해 두 개의 복제가 있지만 출력 파일은 하나뿐입니다!
매번 출력 파일을 덮어쓰고 있습니다.

### 6.4. 게시된 파일을 고유하게 만들기

환자 메타데이터에 접근할 수 있으므로 디렉토리 구조나 파일 이름 자체에 구별되는 메타데이터를 포함하여 게시된 파일을 고유하게 만들 수 있습니다.
...
여기서는 샘플 타입과 복제를 고려하기 위해 추가 디렉토리 수준을 사용하는 옵션을 보여주지만, 파일 이름 수준에서 수행하는 것을 실험할 수도 있습니다.

이제 파이프라인을 한 번 더 실행하되, 먼저 results 디렉토리를 제거하여 깨끗한 작업 공간을 제공하십시오:
...
그리고 여기 있습니다, 모든 메타데이터가 깔끔하게 정리되었습니다. 성공입니다!

이와 같은 map에 메타데이터를 로드한 후 수행할 수 있는 작업이 훨씬 더 많습니다:

1. 환자 속성을 기반으로 정리된 출력 디렉토리 생성
2. 환자 속성에 따라 process에서 결정 내리기
3. 메타데이터 값에 따라 데이터 분할, 결합 및 재결합

메타데이터를 명시적으로 유지하고 데이터에 첨부하는 이 패턴(파일 이름에 인코딩하는 대신)은 강력하고 유지 관리 가능한 분석 워크플로우를 구축할 수 있도록 하는 Nextflow의 핵심 모범 사례입니다.
[Metadata and meta maps](./metadata.md) 사이드 퀘스트에서 이에 대해 자세히 알아볼 수 있습니다.

### 핵심 정리

- `publishDir` 지시문은 메타데이터 값에 따라 출력을 구성할 수 있습니다
- 튜플의 메타데이터는 결과의 구조화된 구성을 가능하게 합니다
- 이 접근 방식은 명확한 데이터 출처를 가진 유지 관리 가능한 워크플로우를 생성합니다
- Process는 메타데이터와 파일의 튜플을 입력으로 받을 수 있습니다
- `tag` 지시문은 실행 로그에서 process 식별을 제공합니다
...
이 사이드 퀘스트에서는 기본 작업부터 파일 컬렉션을 처리하기 위한 고급 기술까지 Nextflow에서 파일을 다루는 방법을 배웠습니다.
...
2.  **원격 파일 사용**: URI를 사용하여 로컬 파일과 원격 파일 간을 투명하게 전환하는 방법을 배웠으며, 워크플로우 로직을 변경하지 않고 다양한 소스의 파일을 처리하는 Nextflow의 능력을 시연했습니다.
...
3.  **`fromPath()` 채널 팩토리를 사용한 파일 로드:** `channel.fromPath()`로 파일 패턴에서 채널을 생성하고 객체 타입을 포함한 파일 속성을 확인했습니다.
...
4.  **파일 이름에서 환자 메타데이터 추출:** `tokenize()` 및 `replace()`를 사용하여 파일 이름에서 메타데이터를 추출하고 구조화하여 정리된 map으로 변환했습니다.
...
6.  **Process에서 파일 작업 사용:** 적절한 입력 처리로 Nextflow process에 파일 작업을 통합하고 `publishDir`를 사용하여 메타데이터에 따라 출력을 구성했습니다.

    - 메타 map을 process 입력과 연결
