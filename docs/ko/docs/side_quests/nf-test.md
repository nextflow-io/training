...
테스트가 왜 그렇게 중요한지 잠시 이야기해 보겠습니다. 워크플로우를 개발할 때 가장 먼저 하는 일 중 하나는 유효하고 결과를 생성할 수 있는 테스트 데이터를 준비하는 것입니다. 파이프라인에 첫 번째 프로세스를 추가하고 입력과 연결하여 작동하도록 만듭니다. 그런 다음 모든 것이 작동하는지 확인하기 위해 테스트 데이터로 실행합니다. 이것이 작동한다고 가정하면 다음 프로세스로 넘어가서 테스트 데이터를 다시 실행합니다. 만족스러운 파이프라인이 완성될 때까지 이 과정을 반복합니다.

그런 다음 `--skip_process`와 같은 간단한 참/거짓 매개변수를 추가할 수 있습니다. 이제 파이프라인을 두 번 실행해야 합니다. 각 매개변수로 한 번씩 실행하여 예상대로 작동하는지 확인해야 합니다. 하지만 잠깐, `--skip_process`가 실제로 프로세스를 건너뛰는지 어떻게 확인할까요? 출력을 확인하거나 로그 파일을 확인해야 합니다! 이것은 번거롭고 오류가 발생하기 쉽습니다.

파이프라인을 개발하면서 매번 수동으로 테스트하기에는 너무 복잡해져서 느리고 오류가 발생하기 쉬워집니다. 게다가 오류를 발견하더라도 파이프라인의 어디에서 오류가 발생하는지 정확히 찾아내기가 매우 어려울 것입니다. 이것이 바로 테스트가 필요한 이유입니다.

테스트를 통해 파이프라인의 모든 부분이 예상대로 작동하는지 체계적으로 확인할 수 있습니다. 잘 작성된 테스트가 개발자에게 주는 이점은 엄청납니다:

- **확신**: 테스트가 전체 파이프라인을 다루기 때문에 무언가를 변경해도 다른 것에 영향을 미치지 않는다는 확신을 가질 수 있습니다
- **신뢰**: 여러 개발자가 파이프라인에서 작업할 때 다른 개발자가 파이프라인과 모든 구성 요소를 손상시키지 않았다는 것을 알 수 있습니다
- **투명성**: 테스트는 파이프라인이 어디에서 실패하는지 보여주고 문제를 추적하기 쉽게 만듭니다. 또한 프로세스나 워크플로우를 실행하는 방법을 보여주는 문서의 한 형태로도 기능합니다
- **속도**: 테스트가 자동화되어 있기 때문에 매우 빠르고 반복적으로 실행할 수 있습니다. 새로운 버그를 도입할 염려 없이 빠르게 반복할 수 있습니다

작성할 수 있는 다양한 유형의 테스트가 있습니다:

1. **모듈 수준 테스트**: 개별 프로세스용
2. **워크플로우 수준 테스트**: 단일 워크플로우용
3. **파이프라인 수준 테스트**: 파이프라인 전체용
4. **성능 테스트**: 파이프라인의 속도와 효율성용
5. **스트레스 테스트**: 극한 조건에서 파이프라인의 성능을 평가하여 한계를 결정

개별 프로세스를 테스트하는 것은 다른 언어의 단위 테스트와 유사합니다. 워크플로우나 전체 파이프라인을 테스트하는 것은 다른 언어에서 통합 테스트라고 하는 것과 유사하며, 구성 요소의 상호 작용을 테스트합니다.

[**nf-test**](https://www.nf-test.com/)는 모듈, 워크플로우 및 파이프라인 수준 테스트를 작성할 수 있는 도구입니다. 간단히 말해서, 파이프라인의 모든 개별 부분이 _독립적으로_ 예상대로 작동하는지 체계적으로 확인할 수 있습니다.

### 학습 목표

이 사이드 퀘스트에서는 nf-test를 사용하여 파이프라인에 대한 워크플로우 수준 테스트와 호출하는 세 가지 프로세스에 대한 모듈 수준 테스트를 작성하는 방법을 학습합니다.

이 사이드 퀘스트를 마치면 다음 기술을 효과적으로 사용할 수 있습니다:

- 프로젝트에서 nf-test 초기화
- 모듈 수준 및 워크플로우 수준 테스트 생성
- 일반적인 유형의 검증 추가
- 스냅샷과 콘텐츠 검증을 언제 사용할지 이해
- 전체 프로젝트에 대한 테스트 실행

이러한 기술은 파이프라인 프로젝트에서 포괄적인 테스트 전략을 구현하여 더 견고하고 유지보수 가능하도록 만드는 데 도움이 됩니다.
...
메인 워크플로우 파일과 파이프라인의 입력을 포함하는 `greetings.csv`라는 CSV 파일을 찾을 수 있습니다.
...
??? example "Hello Nextflow 워크플로우는 무엇을 하나요?"
    ...
    [Hello Nextflow](../hello_nextflow/index.md) 교육을 받지 않았다면 이 간단한 워크플로우가 무엇을 하는지 간단히 설명하겠습니다.

    워크플로우는 인사말이 포함된 CSV 파일을 받아 네 가지 연속적인 변환 단계를 실행하고 재미있는 캐릭터가 인사말을 하는 ASCII 그림이 포함된 단일 텍스트 파일을 출력합니다.
    ...
    결과는 `results/`라는 디렉토리에 게시되며, (기본 매개변수로 실행할 때) 파이프라인의 최종 출력은 대문자로 변환된 인사말을 하는 캐릭터의 ASCII 아트가 포함된 일반 텍스트 파일입니다.
...
작업할 부분은 두 개의 프로세스로 구성됩니다: `sayHello`와 `convertToUpper`.
아래에서 전체 워크플로우 코드를 볼 수 있습니다.
...
??? example "워크플로우 코드"
    ...
    */
    ...
    /*
    * echo를 사용하여 'Hello World!'를 표준 출력으로 출력
    */
    process sayHello {
...
"잠깐, 뭐라고요? 워크플로우를 실행했을 뿐인데 어떻게 테스트가 되나요?"

좋은 질문입니다!

방금 무슨 일이 일어났는지 분석해 보겠습니다.

기본 매개변수로 워크플로우를 실행하고 작동하는지 확인했으며 결과에 만족했습니다. 이것이 테스트의 본질입니다. Hello Nextflow 교육 과정을 진행했다면 모든 섹션을 시작할 때 항상 시작점으로 사용하는 워크플로우를 실행하여 모든 것이 올바르게 설정되었는지 확인했다는 것을 알 수 있습니다.

소프트웨어 테스트는 본질적으로 이 과정을 자동화합니다.

#### 과제 검토

여러분의 과제는 nf-test를 사용하여 이 워크플로우에 표준화된 테스트를 추가하여 추가 변경 사항이 있을 경우 모든 부분이 계속 예상대로 작동하는지 쉽게 확인할 수 있도록 하는 것입니다.
...
`nf-test` 패키지는 프로젝트에 대한 테스트 개발을 시작하기 위해 몇 가지를 설정하는 초기화 명령을 제공합니다.
...
`nf-test`는 nf-test 파일을 구축하기 위한 도구 세트와 함께 제공되어 대부분의 작업을 절약해 줍니다. 이들은 `generate` 하위 명령 아래에 있습니다. 파이프라인에 대한 테스트를 생성해 보겠습니다:
...
이것은 `tests` 디렉토리 내에 `main.nf.test` 파일을 생성합니다. 이것이 파이프라인 수준 테스트 파일입니다. `tree tests/`를 실행하면 다음과 같은 것을 볼 수 있습니다:
...
`main.nf.test` 파일이 파이프라인 수준 테스트 파일입니다. 파일을 열어 내용을 살펴보겠습니다.
...
테스트 파일의 구조를 이해하는 데 잠시 시간을 할애하겠습니다.
...
- `when`: 테스트를 실행해야 하는 조건. 파이프라인을 실행하는 데 사용될 매개변수를 포함합니다
- `then`: 수행해야 하는 검증. 파이프라인의 예상 결과를 포함합니다

평범한 언어로 테스트의 논리는 다음과 같이 읽힙니다:
"**이러한** _매개변수_가 이 _파이프라인_에 제공될 **때**, **그러면** 이러한 결과를 볼 것으로 예상합니다."

이것은 기능 테스트가 아니며, 다음 섹션에서 이를 기능 테스트로 전환하는 방법을 보여드리겠습니다.
...
2. 테스트되는 특정 기능이나 시나리오를 설명
3. 테스트가 실패할 경우 어떤 기능이 손상되었는지 알 수 있을 만큼 명확

나중에 더 많은 검증과 특정 테스트 케이스를 추가할 때 각 테스트가 무엇을 확인하는지 명확하게 하기 위해 이러한 더 설명적인 이름을 사용할 것입니다.

### 1.2. 테스트 실행

테스트를 실행하여 무슨 일이 일어나는지 봅시다.
...
nf-test가 파이프라인이 실패했다고 보고하고 Nextflow의 오류 메시지를 제공한 방법에 주목하세요:

```console title="Error"
ERROR ~ No such file or directory: /workspaces/training/side-quests/nf-test/.nf-test/tests/693ba951a20fec36a5a9292ed1cc8a9f/greetings.csv
```

그렇다면 문제가 무엇이었을까요? 파이프라인에는 프로젝트 디렉토리에 greetings.csv 파일이 있다는 것을 기억하세요. nf-test가 파이프라인을 실행할 때 이 파일을 찾지만 찾을 수 없습니다. 파일이 있는데 무슨 일이 일어나고 있을까요? 경로를 보면 테스트가 `./nf-test/tests/longHashString/` 경로에서 발생하고 있음을 알 수 있습니다. Nextflow와 마찬가지로 nf-test는 모든 것을 격리된 상태로 유지하기 위해 각 테스트에 대해 새 디렉토리를 생성합니다. 데이터 파일이 거기에 없으므로 원래 테스트에서 파일 경로를 수정해야 합니다.

테스트 파일로 돌아가서 `when` 블록에서 파일 경로를 변경하겠습니다.

테스트에서 파이프라인의 루트를 어떻게 가리킬지 궁금할 수 있습니다. 이것은 일반적인 상황이므로 nf-test에는 우리의 삶을 더 쉽게 만들기 위해 사용할 수 있는 다양한 전역 변수가 있습니다. 전체 목록은 [여기](https://www.nf-test.com/docs/testcases/global_variables/)에서 찾을 수 있지만 당분간은 파이프라인 프로젝트의 루트를 의미하는 `projectDir` 변수를 사용하겠습니다.
...
성공입니다! 파이프라인이 성공적으로 실행되고 테스트가 통과합니다. 원하는 만큼 실행하면 항상 같은 결과를 얻을 수 있습니다!
...
간단한 확인은 파이프라인이 예상하는 모든 프로세스를 실행하고 조용히 건너뛰지 않는지 확인하는 것입니다. 파이프라인이 6개의 프로세스를 실행한다는 것을 기억하세요. 3개의 인사말 각각에 대해 `sayHello`라는 하나와 `convertToUpper`라는 하나입니다.

파이프라인이 예상 수의 프로세스를 실행하는지 확인하기 위해 테스트에 검증을 추가하겠습니다. 또한 테스트하는 내용을 더 잘 반영하도록 테스트 이름을 업데이트하겠습니다.
...
이제 테스트 이름이 실제로 확인하는 내용을 더 잘 반영합니다 - 파이프라인이 실패 없이 실행되는 것뿐만 아니라 예상 수의 프로세스를 실행하는지 확인합니다.
...
성공입니다! 파이프라인이 성공적으로 실행되고 테스트가 통과합니다. 이제 전체 상태뿐만 아니라 파이프라인의 세부 사항도 테스트하기 시작했습니다.

### 1.4. 출력 테스트

출력 파일이 생성되었는지 확인하기 위해 테스트에 검증을 추가하겠습니다. 결과를 더 쉽게 해석할 수 있도록 정보를 제공하는 이름으로 별도의 테스트로 추가하겠습니다.
...
성공입니다! 파이프라인이 성공적으로 완료되고 올바른 수의 프로세스가 실행되었으며 출력 파일이 생성되었기 때문에 테스트가 통과합니다. 이것은 또한 테스트에 정보를 제공하는 이름을 제공하는 것이 얼마나 유용한지 보여줍니다.

이것은 표면에 불과하며 파이프라인의 세부 사항을 확인하기 위해 계속 검증을 작성할 수 있지만 지금은 파이프라인의 내부를 테스트하는 것으로 넘어가겠습니다.
...
Nextflow 프로세스를 테스트하는 방법을 학습합니다.

---

## 2. Nextflow 프로세스 테스트

파이프라인의 모든 부분에 대한 테스트를 작성할 필요는 없지만 테스트가 많을수록 파이프라인에 대해 더 포괄적으로 확인할 수 있고 예상대로 작동한다는 확신을 가질 수 있습니다. 이 섹션에서는 파이프라인의 두 프로세스를 개별 단위로 테스트할 것입니다.

### 2.1. `sayHello` 프로세스 테스트

`sayHello` 프로세스부터 시작하겠습니다.

`nf-test generate` 명령을 다시 사용하여 프로세스에 대한 테스트를 생성하겠습니다.
...
이전과 마찬가지로 테스트 세부 정보로 시작한 다음 `when` 및 `then` 블록이 있습니다. 그러나 프로세스에 대한 입력을 정의할 수 있는 추가 `process` 블록도 있습니다.
...
`sayHello` 프로세스가 1개의 입력을 선언했지만 0개의 인수로 호출되었기 때문에 테스트가 실패합니다. 프로세스에 입력을 추가하여 이를 수정하겠습니다. [Hello Workflow](../hello_nextflow/03_hello_workflow.md)(및 위의 준비 운동 섹션)에서 `sayHello` 프로세스가 제공해야 하는 단일 값 입력을 받는다는 것을 기억하세요. 또한 테스트하는 내용을 더 잘 반영하도록 테스트 이름을 수정해야 합니다.
...
성공입니다! `sayHello` 프로세스가 성공적으로 실행되고 출력이 생성되었기 때문에 테스트가 통과합니다.

### 2.2. 테스트로 생성된 스냅샷 확인

`tests/main.sayhello.nf.test` 파일을 보면 검증 블록에서 `snapshot()` 메서드를 사용하는 것을 볼 수 있습니다:

```groovy title="tests/main.sayhello.nf.test"
assert snapshot(process.out).match()
```

이것은 nf-test에게 `sayHello` 프로세스의 출력 스냅샷을 생성하도록 지시합니다. 스냅샷 파일의 내용을 살펴보겠습니다.

```console title="Snapshot file contents"
code tests/main.sayhello.nf.test.snap
```

여기에 출력하지는 않겠지만 프로세스 및 프로세스 출력의 세부 정보가 포함된 JSON 파일을 볼 수 있습니다. 특히 다음과 같은 줄을 볼 수 있습니다:
...
이것은 명시적으로 테스트하고 있는 `sayHello` 프로세스에서 생성된 출력을 나타냅니다. 테스트를 다시 실행하면 프로그램은 새 출력이 원래 기록된 출력과 일치하는지 확인합니다. 이것은 프로세스 출력이 변경되지 않았는지 테스트하는 빠르고 간단한 방법이며, 이것이 nf-test가 기본값으로 제공하는 이유입니다.

!!!warning

    즉, 원래 실행에서 기록한 출력이 올바른지 확인해야 합니다!

향후 개발 과정에서 코드의 무언가가 변경되어 출력이 달라지면 테스트가 실패하고 변경이 예상된 것인지 여부를 결정해야 합니다.

- 코드에서 무언가가 손상된 것으로 판명되면 수정해야 하며, 수정된 코드가 테스트를 통과할 것으로 예상됩니다
- 예상된 변경인 경우(예: 도구가 개선되어 결과가 더 나아진 경우) 새 출력을 일치시킬 참조로 받아들이도록 스냅샷을 업데이트해야 합니다. nf-test에는 이를 위한 `--update-snapshot` 매개변수가 있습니다
...
성공입니다! `sayHello` 프로세스가 성공적으로 실행되고 출력이 스냅샷과 일치했기 때문에 테스트가 통과합니다.

### 2.3. 스냅샷의 대안: 직접 콘텐츠 검증

스냅샷은 출력의 모든 변경 사항을 포착하는 데 유용하지만 때로는 전체 파일이 일치하는 것에 대해 그렇게 엄격하지 않고 특정 콘텐츠를 확인하고 싶을 때가 있습니다. 예를 들어:

- 출력의 일부가 변경될 수 있지만(타임스탬프, 무작위 ID 등) 특정 핵심 콘텐츠가 있어야 하는 경우
...
nf-test는 프로세스 출력을 리스트의 리스트로 보므로 `process.out[0][0]`은 이 프로세스의 첫 번째 채널 항목(또는 '방출')의 첫 번째 부분을 가져옵니다.
...
이것은 `sayHello` 프로세스와 유사한 테스트이지만 `convertToUpper` 프로세스를 테스트합니다. `sayHello`와 마찬가지로 `convertToUpper` 프로세스가 단일 경로 입력을 받지만 지정하지 않았기 때문에 이것이 실패할 것임을 알고 있습니다.
...
참고로 `tests/main.converttoupper.nf.test.snap`에 `convertToUpper` 프로세스에 대한 스냅샷 파일을 생성했습니다. 테스트를 다시 실행하면 nf-test가 다시 통과하는 것을 볼 수 있습니다.
...
각 구성 요소에서 nf-test를 실행하는 것은 괜찮지만 번거롭고 오류가 발생하기 쉽습니다. 모든 것을 한 번에 테스트할 수 없을까요?
...
참고로 현재 디렉토리에서 모든 것을 실행하기 위해 `.`만 사용하고 있습니다. 이것은 모든 테스트를 포함합니다!
...
확인해 보세요! 단일 명령으로 각 프로세스에 대해 1개, 전체 파이프라인에 대해 2개, 총 4개의 테스트를 실행했습니다. 대규모 코드베이스에서 이것이 얼마나 강력한지 상상해 보세요!

---

## 요약

이 사이드 퀘스트에서는 nf-test의 기능을 활용하여 개별 프로세스에 대한 테스트와 전체 파이프라인에 대한 엔드투엔드 테스트를 생성하고 실행하는 방법을 학습했습니다.
이제 출력 검증에 대한 두 가지 주요 접근 방식인 스냅샷과 직접 콘텐츠 검증을 알고 있으며 각각을 언제 사용할지 알고 있습니다.
또한 테스트를 하나씩 또는 전체 프로젝트에 대해 실행하는 방법도 알고 있습니다.

자신의 작업에 이러한 기술을 적용하면 다음을 보장할 수 있습니다:

- 코드가 예상대로 작동합니다
- 변경 사항이 기존 기능을 손상시키지 않습니다
- 다른 개발자가 확신을 가지고 기여할 수 있습니다
...
1. 파이프라인 수준 테스트:
   ...
   - 출력 파일 존재 확인
2. 프로세스 수준 테스트
3. 출력 검증에 대한 두 가지 접근 방식:
   - 완전한 출력 검증을 위한 스냅샷 사용
   - 특정 콘텐츠 확인을 위한 직접 콘텐츠 검증 사용
...
- 테스트를 자동으로 실행하도록 지속적 통합 설정
- 워크플로우 및 모듈 테스트와 같은 다른 유형의 테스트에 대해 학습
- 더 고급 콘텐츠 검증 기술 탐색

**기억하세요:** 테스트는 코드가 어떻게 동작해야 하는지에 대한 살아있는 문서입니다. 더 많은 테스트를 작성하고 검증이 더 구체적일수록 파이프라인의 신뢰성에 대해 더 확신을 가질 수 있습니다.
