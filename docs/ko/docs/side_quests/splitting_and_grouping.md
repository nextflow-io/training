...
간격 채널을 만드는 것부터 시작하겠습니다. 간단하게 유지하기 위해 수동으로 정의할 3개의 간격만 사용합니다. 실제 워크플로에서는 파일 입력에서 이를 읽거나 많은 간격 파일이 있는 채널을 생성할 수도 있습니다.
...
이제 각 간격에 대해 각 샘플을 반복하려고 합니다. 이것은 때때로 샘플과 간격의 데카르트 곱이라고 합니다. [`combine` 연산자](https://www.nextflow.io/docs/latest/operator.html#combine)를 사용하여 이를 달성할 수 있습니다. 이것은 채널 1의 모든 항목을 가져와 채널 2의 각 항목에 대해 반복합니다. 워크플로에 combine 연산자를 추가해 보겠습니다:
...
성공입니다! 3개의 간격 목록에서 모든 간격에 대해 모든 샘플을 반복했습니다. 채널의 항목 수를 효과적으로 3배로 늘렸습니다.

읽기가 조금 어렵기 때문에 다음 섹션에서 정리하겠습니다.

### 4.2. 채널 구성

`map` 연산자를 사용하여 샘플 데이터를 정리하고 리팩토링하여 이해하기 쉽게 만들 수 있습니다. 간격 문자열을 첫 번째 요소의 조인 맵으로 이동해 보겠습니다.
...
다음으로 `grouping_key`를 `interval` 필드와 결합합니다. `grouping_key`는 `id` 및 `repeat` 필드를 포함하는 맵입니다. `interval`로 새 맵을 만들고 Groovy의 맵 추가(`+`)를 사용하여 병합합니다:

```groovy
                grouping_key + [interval: interval],
```

마지막으로 이것을 결합된 메타데이터 맵, 정상 샘플 파일 및 종양 샘플 파일의 세 가지 요소가 있는 튜플로 반환합니다:
...
이제 모든 게놈 간격에 걸쳐 반복되는 모든 샘플이 있어 병렬로 처리할 수 있는 여러 독립적인 분석 단위가 생성됩니다. 그러나 관련 샘플을 다시 가져오려면 어떻게 해야 합니까? 다음 섹션에서는 공통 속성을 공유하는 샘플을 그룹화하는 방법을 학습합니다.
...
이전 섹션에서는 입력 파일에서 데이터를 분할하고 특정 필드(우리의 경우 정상 및 종양 샘플)로 필터링하는 방법을 학습했습니다. 그러나 이것은 단일 유형의 조인만 다룹니다. 특정 속성으로 샘플을 그룹화하려면 어떻게 해야 합니까? 예를 들어, 일치하는 정상-종양 쌍을 조인하는 대신 유형에 관계없이 "sampleA"의 모든 샘플을 함께 처리하려고 할 수 있습니다. 이 패턴은 효율성을 위해 관련 샘플을 별도로 처리한 후 마지막에 결과를 비교하거나 결합하려는 생물정보학 워크플로에서 일반적입니다.

Nextflow에는 이를 수행하는 내장 메서드가 포함되어 있으며 주로 살펴볼 것은 `groupTuple`입니다.

동일한 `id` 및 `interval` 필드가 있는 모든 샘플을 그룹화하는 것부터 시작하겠습니다. 이것은 기술적 복제를 그룹화하지만 의미 있게 다른 샘플을 분리하려는 분석의 전형입니다.
...
이전의 `subMap` 메서드를 재사용하여 맵에서 `id` 및 `interval` 필드를 분리할 수 있습니다. 이전과 마찬가지로 `map` 연산자를 사용하여 각 샘플의 튜플의 첫 번째 요소에 `subMap` 메서드를 적용합니다.
...
!!! note "참고"
    ...
    여기서 `replicate` 필드를 삭제하고 있습니다. 이는 추가 다운스트림 처리에 필요하지 않기 때문입니다. 이 튜토리얼을 완료한 후 나중 그룹화에 영향을 주지 않고 포함할 수 있는지 확인하십시오!

이제 [`groupTuple` 연산자](https://www.nextflow.io/docs/latest/operator.html#grouptuple)를 사용하여 이 새 그룹화 요소로 샘플을 그룹화해 보겠습니다.
...
그게 전부입니다! 단일 코드 줄만 추가했습니다. 실행할 때 어떻게 되는지 봅시다:
...
데이터가 구조를 변경했으며 각 채널 요소 내에서 파일이 이제 `[patientA_rep1_normal.bam, patientA_rep2_normal.bam]`과 같은 튜플에 포함되어 있습니다. 이는 `groupTuple`을 사용할 때 Nextflow가 그룹의 각 샘플에 대한 단일 파일을 결합하기 때문입니다. 이는 다운스트림에서 데이터를 처리하려고 할 때 기억해야 할 중요한 사항입니다.

!!! note "참고"

    [`transpose`](https://www.nextflow.io/docs/latest/reference/operator.html#transpose)는 groupTuple의 반대입니다. 채널의 항목을 풀고 평평하게 만듭니다. `transpose`를 추가하여 위에서 수행한 그룹화를 취소해 보십시오!
...
- **그룹화된 데이터 구조 처리**: `groupTuple`에 의해 생성된 중첩 구조로 작업하는 방법
- **기술적 복제 처리**: 동일한 실험 조건을 공유하는 샘플을 그룹화하는 방법
...
파이프라인을 통해 흐르는 데이터를 수정하여 루프 또는 while 문을 사용하지 않고 확장 가능한 파이프라인을 구축할 수 있으며, 이는 보다 전통적인 접근 방식에 비해 여러 가지 이점을 제공합니다:

- 추가 코드 없이 원하는 만큼 많거나 적은 입력으로 확장할 수 있습니다
- 반복 대신 파이프라인을 통한 데이터 흐름 처리에 집중합니다
- 필요에 따라 복잡하거나 간단할 수 있습니다
- 파이프라인은 어떻게 해야 하는지보다 무엇이 일어나야 하는지에 초점을 맞춰 더 선언적이 됩니다
- Nextflow는 독립적인 작업을 병렬로 실행하여 실행을 최적화합니다

이러한 채널 작업을 마스터하면 루프 또는 반복 프로그래밍에 의존하지 않고 복잡한 데이터 관계를 처리하는 유연하고 확장 가능한 파이프라인을 구축할 수 있으며, Nextflow가 실행을 최적화하고 독립적인 작업을 자동으로 병렬화할 수 있습니다.

### 주요 패턴

1.  **구조화된 입력 데이터 생성:** 메타 맵이 있는 CSV 파일에서 시작([워크플로의 메타데이터](./metadata.md)의 패턴 기반)
...
2.  **데이터를 별도의 채널로 분할:** `filter`를 사용하여 `type` 필드를 기반으로 데이터를 독립적인 스트림으로 나눴습니다

    ```groovy
    channel.filter { it.type == 'tumor' }
    ```

3.  **일치하는 샘플 조인:** `join`을 사용하여 `id` 및 `repeat` 필드를 기반으로 관련 샘플을 다시 결합했습니다

    - 키로 두 채널 조인(튜플의 첫 번째 요소)
...
4.  **간격에 분산:** `combine`을 사용하여 병렬 처리를 위해 샘플과 게놈 간격의 데카르트 곱을 생성했습니다.

    ```groovy
    samples_ch.combine(intervals_ch)
    ```

5.  **그룹화 키로 집계:** `groupTuple`을 사용하여 각 튜플의 첫 번째 요소로 그룹화하여 `id` 및 `interval` 필드를 공유하는 샘플을 수집하고 기술적 복제를 병합했습니다.

    ```groovy
    channel.groupTuple()
    ```

6.  **데이터 구조 최적화:** `subMap`을 사용하여 특정 필드를 추출하고 변환을 재사용 가능하게 만들기 위해 명명된 클로저를 생성했습니다.
