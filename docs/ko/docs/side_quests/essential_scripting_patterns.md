...
이번에는 데이터 구조를 변경하지 않았고, 리스트에 여전히 3개 항목이 있지만, List의 `collect` 메서드를 사용하여 각 항목을 변환하여 수정된 값이 있는 새로운 리스트를 생성했습니다. 이는 채널에서 `map` 연산자를 사용하는 것과 유사하지만 채널이 아닌 List 데이터 구조에서 작동합니다.
...
이제 워크플로는 단일 말단과 쌍 말단 읽기를 모두 우아하게 처리할 수 있습니다. 조건부 로직이 입력 파일 개수를 확인하고 `fastp`에 적절한 명령을 구성합니다. 작동하는지 확인해 봅시다:
...
좋아 보입니다! 실제로 실행된 명령을 확인하면(작업 해시에 맞게 조정):

```console title="실행된 명령 확인"
cat work/31/a8ad4d95749e685a6d842d3007957f/.command.sh
```

Nextflow가 단일 말단 읽기에 대해 올바른 명령을 선택한 것을 볼 수 있습니다:
...
동적 스크립트 로직의 또 다른 일반적인 사용 사례는 [Nextflow for Science Genomics 모듈](../../nf4science/genomics/02_joint_calling)에서 볼 수 있습니다. 해당 모듈에서 호출되는 GATK 프로세스는 여러 입력 파일을 받을 수 있지만, 각 파일은 올바른 명령줄을 형성하기 위해 `-V`로 접두사가 붙어야 합니다. 프로세스는 스크립팅을 사용하여 입력 파일 컬렉션(`all_gvcfs`)을 올바른 명령 인수로 변환합니다:

```groovy title="GATK를 위한 명령줄 조작" linenums="1" hl_lines="2 5"
    script:
...
프로세스 스크립트 블록에서 스크립팅을 사용하는 이러한 패턴은 매우 강력하며 다양한 시나리오에 적용할 수 있습니다 - 가변 입력 타입 처리부터 파일 컬렉션에서 복잡한 명령줄 인수 구축까지, 프로세스를 실제 데이터의 다양한 요구사항에 진정으로 적응할 수 있게 만듭니다.

### 2.3. 변수 보간: Nextflow와 Shell 변수

프로세스 스크립트는 Nextflow 변수, 셸 변수, 명령 치환을 혼합하며, 각각 다른 보간 구문을 가지고 있습니다. 잘못된 구문을 사용하면 오류가 발생합니다. 처리 보고서를 생성하는 프로세스로 이를 탐색해 봅시다.

모듈 파일 `modules/generate_report.nf`를 살펴봅니다:
...
이 프로세스는 샘플 ID와 파일 이름이 포함된 간단한 보고서를 작성합니다. 이제 실행하여 다양한 타입의 변수를 혼합해야 할 때 어떤 일이 일어나는지 봅시다.
...
이제 워크플로를 실행하고 `results/reports/`에서 생성된 보고서를 확인합니다. 각 샘플에 대한 기본 정보가 포함되어 있어야 합니다.
...
하지만 처리가 언제 어디서 발생했는지에 대한 정보를 추가하고 싶다면 어떻게 할까요? **셸** 변수와 약간의 명령 치환을 사용하여 현재 사용자, 호스트 이름, 날짜를 보고서에 포함하도록 프로세스를 수정해 봅시다:
...
이를 실행하면 오류가 발생합니다 - Nextflow가 `${USER}`를 존재하지 않는 Nextflow 변수로 해석하려고 시도합니다.
...
이 섹션에서 **문자열 처리** 기법을 배웠습니다:

- **파일 파싱을 위한 정규 표현식**: `=~` 연산자와 정규식 패턴(`~/pattern/`)을 사용하여 복잡한 파일 명명 규칙에서 메타데이터 추출
- **동적 스크립트 생성**: 조건부 로직(if/else, 삼항 연산자)을 사용하여 입력 특성에 따라 다양한 스크립트 문자열 생성
- **변수 보간**: Nextflow가 문자열을 해석하는 시점과 셸이 해석하는 시점 이해
  - `${var}` - Nextflow 변수 (워크플로 컴파일 시점에 Nextflow가 보간)
  - `\${var}` - 셸 환경 변수 (이스케이프되어 런타임에 bash로 전달)
  - `\$(cmd)` - 셸 명령 치환 (이스케이프되어 런타임에 bash가 실행)

이러한 문자열 처리 및 생성 패턴은 실제 생물정보학 워크플로에서 마주치게 될 다양한 파일 형식과 명명 규칙을 처리하는 데 필수적입니다.

---

## 3. 재사용 가능한 함수 생성

채널 연산자나 프로세스 정의에 복잡한 워크플로 로직을 인라인으로 작성하면 가독성과 유지보수성이 떨어집니다. **함수**를 사용하면 이 로직을 명명된 재사용 가능한 컴포넌트로 추출할 수 있습니다.

우리의 map 연산이 길고 복잡해졌습니다. `def` 키워드를 사용하여 재사용 가능한 함수로 추출해 봅시다.

기존 워크플로에서 어떻게 보이는지 설명하기 위해 아래 수정을 수행하여 `def`를 사용하여 `separateMetadata`라는 재사용 가능한 함수를 정의합니다:
...
이 로직을 함수로 추출함으로써 실제 워크플로 로직을 훨씬 더 깔끔하게 줄였습니다:

```groovy title="최소 워크플로"
    ch_samples = channel.fromPath("./data/samples.csv")
        .splitCsv(header: true)
        .map{ row -> separateMetadata(row) }
...
이렇게 하면 워크플로 로직을 한눈에 훨씬 쉽게 읽고 이해할 수 있습니다. `separateMetadata` 함수는 메타데이터를 파싱하고 풍부하게 하는 모든 복잡한 로직을 캡슐화하여 재사용 가능하고 테스트 가능하게 만듭니다.

워크플로를 실행하여 여전히 작동하는지 확인합니다:
...
출력은 두 프로세스가 모두 성공적으로 완료되었음을 보여줍니다. 이제 워크플로가 훨씬 깔끔하고 유지보수하기 쉬우며, 모든 복잡한 메타데이터 처리 로직이 `separateMetadata` 함수에 캡슐화되어 있습니다.

### 핵심 정리

이 섹션에서 **함수 생성**을 배웠습니다:

- **`def`로 함수 정의**: 명명된 함수를 생성하는 키워드 (Python의 `def`나 JavaScript의 `function`과 유사)
- **함수 스코프**: 스크립트 레벨에서 정의된 함수는 Nextflow 워크플로 전체에서 접근 가능
- **반환 값**: 함수는 자동으로 마지막 표현식을 반환하거나 명시적 `return` 사용
- **더 깔끔한 코드**: 복잡한 로직을 함수로 추출하는 것은 모든 언어에서 기본적인 소프트웨어 엔지니어링 관행

다음으로는 동적 리소스 할당을 위해 프로세스 지시문에서 클로저를 사용하는 방법을 탐색하겠습니다.

---

## 4. 클로저를 사용한 동적 리소스 지시문

지금까지 프로세스의 `script` 블록에서 스크립팅을 사용했습니다. 하지만 **클로저**(섹션 1.1에서 소개)는 프로세스 지시문, 특히 동적 리소스 할당에서도 매우 유용합니다. 샘플 특성에 따라 적응하는 리소스 지시문을 FASTP 프로세스에 추가해 봅시다.

### 4.1. 샘플별 리소스 할당

현재 FASTP 프로세스는 기본 리소스를 사용합니다. 고깊이 샘플에 더 많은 CPU를 할당하여 더 스마트하게 만들어 봅시다. `modules/fastp.nf`를 편집하여 동적 `cpus` 지시문과 정적 `memory` 지시문을 포함합니다:
...
클로저 `{ meta.depth > 40000000 ? 2 : 1 }`은 **삼항 연산자**(섹션 1.1에서 다룸)를 사용하며 각 작업에 대해 평가되어 샘플별 리소스 할당을 가능하게 합니다. 고깊이 샘플(>40M 읽기)은 2개의 CPU를 받고, 다른 샘플은 1개의 CPU를 받습니다.

!!! note "지시문에서 입력 변수 접근"

    클로저는 Nextflow가 각 작업 실행 컨텍스트에서 이러한 클로저를 평가하기 때문에 모든 입력 변수(여기서는 `meta`)에 접근할 수 있습니다.

작업 해시를 더 쉽게 볼 수 있도록 `-ansi-log false` 옵션으로 워크플로를 다시 실행합니다.
...
주어진 작업에 대한 CPU 할당을 확인하기 위해 실행된 정확한 `docker` 명령을 확인할 수 있습니다:
...
또 다른 강력한 패턴은 재시도 전략을 위해 `task.attempt`를 사용하는 것입니다. 이것이 왜 유용한지 보여주기 위해 먼저 FASTP에 대한 메모리 할당을 필요한 것보다 적게 줄여보겠습니다. `modules/fastp.nf`의 `memory` 지시문을 `1.GB`로 변경합니다:
...
이는 실제 워크플로에서 매우 일반적인 시나리오입니다 - 때로는 실행해 보기 전까지 작업에 얼마나 많은 메모리가 필요한지 알 수 없습니다.

워크플로를 더 견고하게 만들기 위해 각 시도마다 메모리 할당을 증가시키는 재시도 전략을 구현할 수 있습니다. 다시 한 번 Groovy 클로저를 사용합니다. `memory` 지시문을 수정하여 기본 메모리에 `task.attempt`를 곱하고, `errorStrategy 'retry'`와 `maxRetries 2` 지시문을 추가합니다:
...
이렇게 하면 워크플로가 더 효율적(과도한 할당 방지)이고 더 견고(더 많은 리소스로 자동 재시도)해집니다.

---

## 5. 조건부 로직과 프로세스 제어

이전에는 채널 데이터를 변환하기 위해 스크립팅과 함께 `.map()`을 사용했습니다. 이제는 데이터를 기반으로 어떤 프로세스가 실행될지 제어하기 위해 조건부 로직을 사용하겠습니다 - 이는 다양한 샘플 타입에 적응하는 유연한 워크플로에 필수적입니다.

Nextflow의 [데이터플로 연산자](https://www.nextflow.io/docs/latest/reference/operator.html)는 런타임에 평가되는 클로저를 받아 채널 내용을 기반으로 워크플로 결정을 내릴 수 있게 합니다.

### 5.1. `.branch()`로 라우팅

예를 들어, 시퀀싱 샘플이 특정 임계값 이상의 커버리지를 가진 인간 샘플인 경우에만 FASTP로 트리밍되어야 한다고 가정해 봅시다. 마우스 샘플이나 저커버리지 샘플은 대신 Trimgalore로 실행되어야 합니다(이것은 인위적인 예제이지만 요점을 설명합니다).

`modules/trimgalore.nf`에 간단한 Trimgalore 프로세스를 제공했습니다. 원한다면 살펴보세요. 하지만 세부 사항은 이 연습에 중요하지 않습니다. 핵심은 메타데이터를 기반으로 샘플을 라우팅하고 싶다는 것입니다.
...
... 그런 다음 `main.nf` 워크플로를 수정하여 메타데이터를 기반으로 샘플을 분기하고 적절한 트리밍 프로세스를 통해 라우팅합니다:
...
여기서 `.branch{}` 연산자 내부의 작지만 강력한 조건부 표현식을 사용하여 메타데이터를 기반으로 샘플을 라우팅했습니다. 높은 커버리지를 가진 인간 샘플은 `FASTP`를 통과하고, 다른 모든 샘플은 `TRIMGALORE`를 통과합니다.

### 5.2. Truthiness와 함께 `.filter()` 사용

워크플로 실행을 제어하는 또 다른 강력한 패턴은 `.filter()` 연산자입니다. 이는 클로저를 사용하여 어떤 항목이 파이프라인을 계속 진행해야 하는지 결정합니다. 필터 클로저 내부에서 어떤 항목이 통과할지 결정하는 **불린 표현식**을 작성합니다.

Nextflow(많은 동적 언어처럼)는 불린 컨텍스트에서 어떤 값이 `true` 또는 `false`로 평가되는지 결정하는 **"truthiness"** 개념을 가지고 있습니다:

- **Truthy**: null이 아닌 값, 비어있지 않은 문자열, 0이 아닌 숫자, 비어있지 않은 컬렉션
- **Falsy**: `null`, 빈 문자열 `""`, 0 `0`, 빈 컬렉션 `[]` 또는 `[:]`, `false`

이는 `meta.id` 단독으로(명시적 `!= null` 없이) ID가 존재하고 비어있지 않은지 확인한다는 의미입니다. 이를 사용하여 품질 요구사항을 충족하지 않는 샘플을 필터링해 봅시다.
...
- `meta.id && meta.organism`은 두 필드가 모두 존재하고 비어있지 않은지 확인합니다(truthiness 사용)
...
이 섹션에서는 Nextflow 연산자의 클로저 인터페이스(`.branch{}`와 `.filter{}` 같은)를 사용하여 워크플로 실행을 제어하는 조건부 로직을 사용하는 방법을 배웠고, truthiness를 활용하여 간결한 조건부 표현식을 작성했습니다.

이제 파이프라인이 적절한 프로세스를 통해 샘플을 지능적으로 라우팅하지만, 프로덕션 워크플로는 유효하지 않은 데이터를 우아하게 처리해야 합니다. 워크플로를 누락되거나 null 값에 대해 견고하게 만들어 봅시다.

---

## 6. 안전 탐색 및 Elvis 연산자

우리의 `separateMetadata` 함수는 현재 모든 CSV 필드가 존재하고 유효하다고 가정합니다. 하지만 불완전한 데이터가 있으면 어떻게 될까요? 알아봅시다.

### 6.1. 문제: 존재하지 않는 속성 접근

선택적 시퀀싱 실행 정보에 대한 지원을 추가하고 싶다고 가정해 봅시다. 일부 실험실에서는 샘플에 시퀀싱 실행 ID 또는 배치 번호에 대한 추가 필드가 있을 수 있지만, 현재 CSV에는 이 열이 없습니다. 어쨌든 접근해 봅시다.
...
문제는 `row.run_id`가 CSV에 `run_id` 열이 존재하지 않기 때문에 `null`을 반환한다는 것입니다. `null`에 대해 `.toUpperCase()`를 호출하려고 하면 충돌합니다. 이것이 안전 탐색 연산자가 구해주는 곳입니다.
...
충돌하지 않습니다! 이제 워크플로가 누락된 필드를 우아하게 처리합니다. `row.run_id`가 `null`일 때 `?.` 연산자는 `.toUpperCase()` 호출을 방지하고, `run_id`는 예외를 발생시키는 대신 `null`이 됩니다.

### 6.3. 기본값을 위한 Elvis 연산자 (`?:`)

Elvis 연산자(`?:`)는 왼쪽이 "falsy"(이전에 설명한 대로)일 때 기본값을 제공합니다. Elvis Presley의 유명한 머리와 눈처럼 보이기 때문에 `?:`라는 이름이 붙었습니다!

이제 안전 탐색을 사용하고 있으므로 해당 필드가 없는 샘플의 경우 `run_id`가 `null`이 됩니다. Elvis 연산자를 사용하여 기본값을 제공하고 `sample_meta` 맵에 추가해 봅시다:
...
또한 워크플로에 `view()` 연산자를 추가하여 결과를 확인합니다:
...
!!! tip "안전 탐색과 Elvis 결합"
    ...
    `value?.method() ?: 'default'` 패턴은 프로덕션 워크플로에서 일반적입니다:

    - `value?.method()` - 안전하게 메서드 호출, `value`가 `null`이면 `null` 반환
...
때로는 입력 매개변수가 유효하지 않으면 워크플로를 즉시 중지해야 합니다. Nextflow에서는 `error()`와 `log.warn` 같은 내장 함수와 `if` 문 및 불린 로직 같은 표준 프로그래밍 구조를 사용하여 검증 로직을 구현할 수 있습니다. 워크플로에 검증을 추가해 봅시다.

워크플로 블록 전에 검증 함수를 생성하고, 워크플로에서 호출하며, CSV 파일 경로에 대한 매개변수를 사용하도록 채널 생성을 변경합니다. 매개변수가 누락되었거나 파일이 존재하지 않으면 `error()`를 호출하여 명확한 메시지와 함께 실행을 중지합니다.
...
=== "변경 후"
    ...
    def validateInputs() {
        // 입력 매개변수가 제공되었는지 확인
        if (!params.input) {
            error("입력 CSV 파일 경로가 제공되지 않았습니다. --input <file.csv>를 지정하세요")
        }

        // CSV 파일이 존재하는지 확인
        if (!file(params.input).exists()) {
            error("입력 CSV 파일을 찾을 수 없습니다: ${params.input}")
        }
    }
    ...
    workflow {
...
??? failure "명령 출력"
    ...
    입력 CSV 파일 경로가 제공되지 않았습니다. --input <file.csv>를 지정하세요
    ```

워크플로가 나중에 신비롭게 실패하는 대신 명확한 오류 메시지와 함께 즉시 중지됩니다.
...
??? failure "명령 출력"
    ...
    입력 CSV 파일을 찾을 수 없습니다: ./data/nonexistent.csv
    ```
...
`separateMetadata` 함수 내에 검증을 추가할 수도 있습니다. 낮은 시퀀싱 깊이를 가진 샘플에 대해 경고를 발행하지만 워크플로가 계속 진행되도록 치명적이지 않은 `log.warn`을 사용해 봅시다:
...
=== "변경 후"
    ...
    ```groovy title="main.nf" linenums="1" hl_lines="3-6"
        ...
        if (sample_meta.depth < 30000000) {
            log.warn "${sample_meta.id}의 시퀀싱 깊이가 낮습니다: ${sample_meta.depth}"
        }
...
??? warning "명령 출력"
    ...
    WARN: sample_002의 시퀀싱 깊이가 낮습니다: 25000000
    ```
...
- **조기 검증**: 처리 전에 입력을 확인하여 유용한 오류와 함께 빠르게 실패
- **검증 함수**: 워크플로 시작 시 호출할 수 있는 재사용 가능한 검증 로직 생성

적절한 검증은 명확한 오류 메시지로 문제를 조기에 포착하여 워크플로를 더 견고하고 사용자 친화적으로 만듭니다.

---

## 8. 워크플로 이벤트 핸들러

지금까지 워크플로 스크립트와 프로세스 정의에 코드를 작성했습니다. 하지만 알아야 할 중요한 기능이 하나 더 있습니다: 워크플로 이벤트 핸들러입니다.

이벤트 핸들러는 워크플로 라이프사이클의 특정 시점에 실행되는 클로저입니다. 로깅, 알림 또는 정리 작업을 추가하는 데 완벽합니다. 이러한 핸들러는 워크플로 정의와 함께 워크플로 스크립트에 정의되어야 합니다.

### 8.1. `onComplete` 핸들러

가장 일반적으로 사용되는 이벤트 핸들러는 `onComplete`로, 워크플로가 완료될 때(성공 또는 실패 여부와 관계없이) 실행됩니다. 파이프라인 결과를 요약하는 핸들러를 추가해 봅시다.

`main.nf` 파일의 워크플로 정의 내부에 이벤트 핸들러를 추가합니다:
...
=== "변경 후"
    ...
    ```groovy title="main.nf" linenums="66" hl_lines="5-16"
        ...
        workflow.onComplete = {
            ...
            println "파이프라인 실행 요약:"
            println "=========================="
            println "완료 시각: ${workflow.complete}"
            println "소요 시간: ${workflow.duration}"
            println "성공 여부: ${workflow.success}"
            println "작업 디렉토리: ${workflow.workDir}"
            println "종료 상태: ${workflow.exitStatus}"
            println ""
        }
    }
    ```
...
워크플로를 실행하면 마지막에 이 요약이 나타나는 것을 볼 수 있습니다!
...
??? success "명령 출력"
    ...
    파이프라인 실행 요약:
    ==========================
    완료 시각: 2025-10-10T12:14:24.885384+01:00
    소요 시간: 2.9s
    성공 여부: true
    작업 디렉토리: /workspaces/training/side-quests/essential_scripting_patterns/work
    종료 상태: 0
    ```
...
=== "변경 후"
    ...
    ```groovy title="main.nf" linenums="66" hl_lines="5-22"
        ...
        workflow.onComplete = {
            ...
            if (workflow.success) {
                println "✅ 파이프라인이 성공적으로 완료되었습니다!"
            } else {
                println "❌ 파이프라인이 실패했습니다!"
                println "오류: ${workflow.errorMessage}"
            }
        }
    }
    ```
...
??? success "명령 출력"
    ...
    파이프라인 실행 요약:
    ==========================
    완료 시각: 2025-10-10T12:16:00.522569+01:00
    소요 시간: 3.6s
    성공 여부: true
    작업 디렉토리: /workspaces/training/side-quests/essential_scripting_patterns/work
    종료 상태: 0
...
```groovy title="main.nf - 요약을 파일에 작성"
workflow {
    // ... 워크플로 코드 ...
    ...
    workflow.onComplete = {
        ...
        파이프라인 실행 요약
        ===========================
        완료: ${workflow.complete}
        소요 시간: ${workflow.duration}
        성공: ${workflow.success}
        명령: ${workflow.commandLine}
        """
...
`onComplete` 외에도 사용할 수 있는 다른 이벤트 핸들러가 하나 더 있습니다: `onError`는 워크플로가 실패할 때만 실행됩니다:
...
workflow {
    ...
    workflow.onError = {
        println "="* 50
        println "파이프라인 실행이 실패했습니다!"
        println "오류 메시지: ${workflow.errorMessage}"
        println "="* 50
        ...
        =====================
        시각: ${new Date()}
        오류: ${workflow.errorMessage}
        오류 보고서: ${workflow.errorReport ?: '상세 보고서 없음'}
        """

        println "오류 세부 정보가 다음에 작성되었습니다: ${error_file}"
    }
}
```
...
workflow {
    ...
    workflow.onError = {
        println "워크플로 실패: ${workflow.errorMessage}"
    }

    workflow.onComplete = {
        def duration_mins = workflow.duration.toMinutes().round(2)
        def status = workflow.success ? "성공 ✅" : "실패 ❌"

        println """
        파이프라인 완료: ${status}
        소요 시간: ${duration_mins}분
        """
    }
}
```
...
- **이벤트 핸들러 클로저**: 다양한 라이프사이클 시점에 실행되는 워크플로 스크립트의 클로저
...
이 사이드 퀘스트를 통해 기본 메타데이터 처리에서 정교한 프로덕션 준비 워크플로로 발전한 포괄적인 샘플 처리 파이프라인을 구축했습니다.
각 섹션은 이전 섹션을 기반으로 구축되어 프로그래밍 구조가 간단한 워크플로를 강력한 데이터 처리 시스템으로 변환하는 방법을 보여주었으며, 다음과 같은 이점이 있습니다:

- **더 명확한 코드**: 데이터플로 vs 스크립팅을 이해하면 더 체계적인 워크플로를 작성할 수 있습니다
- **견고한 처리**: 안전 탐색 및 Elvis 연산자는 워크플로를 누락된 데이터에 대해 탄력적으로 만듭니다
- **유연한 처리**: 조건부 로직은 워크플로가 다양한 샘플 타입을 적절하게 처리할 수 있게 합니다
- **적응형 리소스**: 동적 지시문은 입력 특성에 따라 리소스 사용을 최적화합니다

이 진행은 실제 생물정보학 파이프라인의 진화를 반영합니다. 몇 개의 샘플을 처리하는 연구 프로토타입에서 실험실과 기관 전체에서 수천 개의 샘플을 처리하는 프로덕션 시스템까지.
해결한 모든 과제와 배운 모든 패턴은 개발자가 Nextflow 워크플로를 확장할 때 직면하는 실제 문제를 반영합니다.

자신의 작업에 이러한 패턴을 적용하면 견고한 프로덕션 준비 워크플로를 구축할 수 있습니다.

### 핵심 패턴

1.  **데이터플로 vs 스크립팅:** 데이터플로 작업(채널 오케스트레이션)과 스크립팅(데이터를 조작하는 코드)을 구분하는 방법을 배웠으며, Channel vs List의 `collect` 같은 다양한 타입에 대한 작업의 중요한 차이점을 포함합니다.
...
2.  **고급 문자열 처리**: 파일 이름 파싱을 위한 정규 표현식, 프로세스의 동적 스크립트 생성, 변수 보간(Nextflow vs Bash vs Shell)을 마스터했습니다.
...
3.  **재사용 가능한 함수 생성**: 복잡한 로직을 채널 연산자에서 호출할 수 있는 명명된 함수로 추출하여 워크플로를 더 읽기 쉽고 유지보수 가능하게 만드는 방법을 배웠습니다.

    - 명명된 함수 정의

    ```groovy
    def separateMetadata(row) {
        def sample_meta = [ /* 간결성을 위해 코드 숨김 */ ]
        def fastq_path = file(row.file_path)
        def m = (fastq_path.name =~ /^(.+)_S(\d+)_L(\d{3})_(R[12])_(\d{3})\.fastq(?:\.gz)?$/)
        def file_meta = m ? [ /* 간결성을 위해 코드 숨김 */ ] : [:]
        def priority = sample_meta.quality > 40 ? 'high' : 'normal'
...
4.  **클로저를 사용한 동적 리소스 지시문**: 입력 특성에 따라 적응형 리소스 할당을 위해 프로세스 지시문에서 클로저를 사용하는 방법을 탐색했습니다.
...
5.  **조건부 로직과 프로세스 제어**: `.branch()`와 `.filter()` 연산자를 사용하여 지능적인 라우팅을 추가하고, 간결한 조건부 표현식을 위해 truthiness를 활용했습니다.

    - `.branch()`를 사용하여 다양한 워크플로 분기를 통해 데이터 라우팅
    ...
    ```groovy
    if (sample.files) println "파일이 있습니다"
    ```

    - 'truthiness'로 데이터를 부분집합하기 위해 `filter()` 사용
...
6.  **안전 탐색 및 Elvis 연산자**: null-safe 속성 접근을 위해 `?.`를 사용하고 기본값 제공을 위해 `?:`를 사용하여 누락된 데이터에 대해 파이프라인을 견고하게 만들었습니다.
...
7.  **error()와 log.warn을 사용한 검증**: 입력을 조기에 검증하고 명확한 오류 메시지로 빠르게 실패하는 방법을 배웠습니다.
    ...
    try {
        ...
        if (errors) throw new RuntimeException("유효하지 않음: ${errors.join(', ')}")
    } catch (Exception e) {
        println "오류: ${e.message}"
    }
    ```

8.  **설정 이벤트 핸들러**: 로깅, 알림, 라이프사이클 관리를 위해 워크플로 이벤트 핸들러(`onComplete`와 `onError`)를 사용하는 방법을 배웠습니다.
...
더 고급 기능을 탐색해야 할 때 이러한 자료를 확인하세요.

다음을 위해 기술을 연습하고 확장하면 도움이 될 것입니다:

- 데이터플로와 스크립팅을 적절히 분리하여 더 깔끔한 워크플로 작성
- Nextflow, Bash, 셸 변수의 일반적인 함정을 피하기 위해 변수 보간 마스터
- 효율적이고 적응형 워크플로를 위한 동적 리소스 지시문 사용
...
- 검증, 오류 처리, 로깅을 추가하여 워크플로를 프로덕션 준비 상태로 만들기
- 이벤트 핸들러로 워크플로 라이프사이클 관리 구현
