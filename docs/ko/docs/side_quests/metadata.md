...
메타데이터는 파일과 실험 조건에 대한 중요한 세부 정보를 추적하고, 각 데이터셋의 고유한 특성에 맞게 분석을 조정하는 데 도움을 줍니다.

도서관 목록과 같다고 생각하면 됩니다: 책에는 실제 내용(원시 데이터)이 포함되어 있지만, 목록 카드는 각 책에 대한 필수 정보(언제 출판되었는지, 누가 썼는지, 어디에서 찾을 수 있는지 등의 메타데이터)를 제공합니다.
...
이 사이드 퀘스트에서는 워크플로우에서 메타데이터를 처리하는 방법을 학습합니다.
...
- `id`: 파일에 부여된 ID
- `character`: 나중에 다른 생물을 그리는 데 사용할 캐릭터 이름
...
이것은 파일별 메타데이터가 처리 결정을 주도하는 일반적인 워크플로우 패턴을 나타냅니다. 메타데이터 맵이 우아하게 해결하는 바로 그런 종류의 문제입니다.
...
먼저 그것을 추가하는 것부터 시작하겠습니다.

### 1.1. `splitCsv`로 내용 읽기

파일 내용을 최소한의 노력으로 적절하게 분석할 연산자를 선택해야 합니다.
...
Nextflow에게 CSV 파일의 첫 번째 행을 헤더 행으로 읽도록 지시하기 위해 `header: true` 옵션을 사용하고 있습니다.
...
??? info "(선택 사항) 맵에 대한 추가 정보"
    ...
    다음은 실제로 맵을 정의하고 그 내용에 접근하는 방법을 보여주는 실행 가능한 스크립트입니다:
...
또한 프로세스를 약간 다른 입력 데이터를 가진 다른 사람들과 공유하기 어렵게 만들고, 스크립트 블록에서 필요하지 않은 변수를 프로세스에 하드코딩해야 할 수도 있습니다.

이 문제를 피하려면 데이터시트에 포함된 열 수에 관계없이 채널 구조를 일관되게 유지하는 방법을 찾아야 합니다.

모든 메타데이터를 튜플 내의 항목으로 수집하여 이를 수행할 수 있으며, 이를 메타데이터 맵 또는 더 간단히 '메타 맵'이라고 부릅니다.
...
채널 요소를 메타 맵과 해당 파일 객체로 구성된 두 요소의 튜플로 재구성했습니다.
...
결과적으로 데이터시트에 더 많은 열을 추가하면 `meta` 맵에서 더 많은 메타데이터를 사용할 수 있게 되지만 채널 형태는 변경되지 않습니다.
...
도구가 파일을 작성하는 대신 출력을 콘솔에 직접 출력하므로 여기서 Nextflow의 [`stdout`](https://www.nextflow.io/docs/latest/process.html#outputs) 출력 한정자를 사용하고 있으며, 명령줄에서 `sed`를 사용하여 확률 점수를 제거하고 개행 문자를 제거하여 문자열을 정리하고 언어 예측만 반환합니다.
...
!!! note
    ...
    [Splitting & Grouping](./splitting_and_grouping.md) 사이드 퀘스트에서 이 사용 사례를 자세히 다룹니다.
...
기술적 관점에서 그렇게 하는 것이 _가능_하지만 안전하지 않습니다.
...
??? info "`+` 연산자를 사용한 새 메타 맵 생성"
    ...
    **이제 이것을 Nextflow `channel.map()` 연산의 맥락으로 옮겨 봅시다.**
    ...
    `map1`을 `meta`로 변경하면 워크플로우의 메타 맵에 언어 예측을 추가하는 데 필요한 기본적으로 모든 것이라는 것을 알 수 있기를 바랍니다.
    ...
    이렇게 하면 단순히 `file`을 튜플의 두 번째 위치에 원래 위치에 남겨두고 있다는 것이 더 명확해집니다. `new_info` 값을 가져와서 첫 번째 위치에 있는 맵에 접었을 뿐입니다.

    **그리고 이것은 우리를 `tuple val(meta), path(file)` 채널 구조로 다시 데려옵니다!**
...
더 구체적으로, 각 동물을 ASCII 아트로 그리고 말풍선에 녹음된 텍스트를 말하게 하는 두 번째 단계를 워크플로우에 추가할 것입니다.
[`cowpy`](https://github.com/jeffbuttars/cowpy)라는 도구를 사용하여 이를 수행할 것입니다.

??? info "`cowpy`는 무엇을 하나요?"

    `cowpy`는 임의의 텍스트 입력을 재미있는 방식으로 표시하기 위해 ASCII 아트를 생성하는 명령줄 도구입니다.
...
해당 채널의 내용을 다음 프로세스에 제공하기 위해(그리고 명확하고 읽기 쉬운 방식으로 수행하기 위해) 자체 이름인 `ch_languages`를 지정하려고 합니다.
...
??? info "(선택 사항) `multiMap()`을 사용한 대체 버전"
    ...
    `file`과 `character` 요소를 프로세스에 별도로 제공하기 위해 두 개의 별도 `map()` 연산을 작성해야 했습니다.
...
전체 메타 맵을 프로세스에 제공하고 거기에 도착하면 필요한 것을 선택할 수 있다면 더 좋을 것입니다.
...
워크플로우를 간소화할 수 있도록 첫 번째 프로세스에서 사용한 `[meta, file]` 튜플 구조를 수락하도록 `COWPY` 프로세스를 수정해 봅시다.
...
튜토리얼을 간소화하기 위해 프로세스 출력 정의를 업데이트하여 메타 맵을 출력하지 않았지만 `IDENTIFY_LANGUAGE` 프로세스의 모델을 따라 직접 수행해 보세요.
...
Nextflow에 관한 한 이 워크플로우는 성공적으로 실행되었습니다!
그러나 원하는 출력은 생성되지 않습니다.

##### 3.4.2.2. 프로세스 수준에서 접근되는 값
...
이것은 `meta.character`가 존재하지 않기 때문에 접근하려는 시도가 `null`을 반환하기 때문에 발생합니다. 결과적으로 Nextflow는 명령줄에 문자 그대로 `null`을 연결하며, 이는 물론 `cowpy` 도구에서 인식되지 않습니다.
...
이 사이드 퀘스트에서 Nextflow 워크플로우에서 메타데이터를 효과적으로 작업하는 방법을 학습했습니다.
