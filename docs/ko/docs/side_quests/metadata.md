...
이는 파일별 메타데이터가 처리 결정을 주도하는 일반적인 워크플로우 패턴을 나타냅니다. 메타데이터 맵이 우아하게 해결하는 문제 유형입니다.
...
먼저 이를 추가하는 것부터 시작하겠습니다.

### 1.1. `splitCsv`로 내용 읽기

최소한의 노력으로 파일 내용을 적절하게 분석할 연산자를 선택해야 합니다.
데이터시트가 CSV 형식이므로 파일의 각 행을 채널의 요소로 로드하는 [`splitCsv`](https://www.nextflow.io/docs/latest/reference/operator.html#splitcsv) 연산자를 사용하는 것이 적합합니다.

다음과 같이 변경하여 채널 구성 코드에 `splitCsv()` 연산과 파일 내용이 채널에 올바르게 로드되는지 확인하는 `view()` 연산을 추가하세요.
...
연산자가 CSV 파일의 각 행에 대해 키-값 쌍의 맵을 구성했으며, 열 헤더가 해당 값의 키로 사용되었음을 볼 수 있습니다.
...
훌륭합니다! 이를 통해 각 파일의 특정 필드에 쉽게 접근할 수 있습니다.
...
워크플로우의 현재 상태에서 입력 파일(`recording` 키 아래)과 관련 메타데이터(`id`, `character`)는 모두 동일한 수준에 있으며, 마치 모두 하나의 큰 가방에 들어 있는 것과 같습니다.
...
이는 Nextflow 워크플로우에서 메타데이터를 구성하는 데 널리 사용되는 규칙입니다.
...
여기서는 도구가 파일을 작성하는 대신 출력을 콘솔에 직접 출력하기 때문에 Nextflow의 [`stdout`](https://www.nextflow.io/docs/latest/process.html#outputs) 출력 한정자를 사용하고 있으며, 명령줄에서 `sed`를 사용하여 확률 점수를 제거하고 줄바꿈 문자를 제거하여 문자열을 정리하고 언어 예측만 반환합니다.
...
그리고 이를 [`map`](https://www.nextflow.io/docs/latest/operator.html#map) 연산과 결합하여 이전 맵을 새 맵으로 교체합니다.
...
??? info "`+` 연산자를 사용한 새 메타 맵 생성"
    ...
    **이제 이를 Nextflow `channel.map()` 연산의 맥락으로 옮겨 봅시다.**
    ...
    `map1`을 `meta`로 변경하면 워크플로우의 메타 맵에 언어 예측을 추가하는 데 필요한 기본적인 모든 것이 있다는 것을 알 수 있기를 바랍니다.

    한 가지를 제외하고!

    워크플로우의 경우 **튜플에 `file` 객체의 존재도 고려해야 합니다**. 튜플은 `meta, file, lang_id`로 구성됩니다.
...
<!-- TODO (future) subMap을 사용하여 키를 제거하는 방법도 보여줘야 할까요?! 또는 어디에서 찾을 수 있는지 메모. -->
...
이것이 이전 단계에서 사용한 즉석 맵 병합 연산과 매우 유사하다는 것을 알 수 있습니다.
조건문만 작성하면 됩니다.
...
<!-- TODO (future) 추가 리소스 섹션에 관련 문서에 대한 메모/링크 추가 -->
...
이를 통해 파이프라인을 진행하면서 파일과 새로운 메타데이터 및 기존 메타데이터를 연결할 수 있습니다.
...
[`cowpy`](https://github.com/jeffbuttars/cowpy)라는 도구를 사용하여 이를 수행할 것입니다.
...
해당 채널의 내용을 다음 프로세스에 제공하기 위해(그리고 명확하고 읽기 쉬운 방식으로) 자체 이름인 `ch_languages`를 지정하려고 합니다.
...
이를 통해 채널의 각 요소에 대한 파일과 캐릭터에 접근할 수 있음을 확인했습니다.
...
프로세스 호출의 입력으로 두 개의 맵 연산(`.view()` 문 제외)을 복사하기만 하면 됩니다.
그 사이에 쉼표를 잊지 마세요!
...
이는 메타 맵의 정보를 사용하여 파이프라인의 두 번째 단계에서 명령을 매개변수화할 수 있었음을 보여줍니다.

그러나 위에서 언급했듯이 관련된 일부 코드가 약간 투박했습니다. 워크플로우 본문의 컨텍스트에 있는 동안 메타 데이터의 압축을 풀어야 했기 때문입니다.
이 접근 방식은 메타 맵에서 소수의 필드를 사용하는 데는 잘 작동하지만 훨씬 더 많이 사용하려는 경우 확장성이 떨어집니다.

이를 약간 간소화할 수 있는 `multiMap()`이라는 다른 연산자가 있지만 그렇다고 해도 이상적이지는 않습니다.

??? info "(선택 사항) `multiMap()`을 사용한 대체 버전"

    궁금하실 경우를 대비해, `file`과 `character`를 모두 출력하는 단일 `map()` 연산을 작성할 수 없었습니다. 튜플로 반환되기 때문입니다.
    `file`과 `character` 요소를 프로세스에 별도로 제공하려면 두 개의 별도 `map()` 연산을 작성해야 했습니다.

    기술적으로 여러 채널을 내보낼 수 있는 `multiMap()` 연산자를 사용하여 단일 매핑 연산을 통해 이를 수행하는 다른 방법이 있습니다.
...
어느 경우든 워크플로우 수준에서 일부 압축 해제를 수행해야 한다는 것이 어색합니다.
...
위에서 그렇게 할 수 없었던 유일한 이유는 프로세스가 메타 맵을 받아들이도록 설정되지 않았기 때문입니다.
하지만 프로세스 코드를 제어하므로 변경할 수 있습니다.

워크플로우를 간소화할 수 있도록 첫 번째 프로세스에서 사용한 `[meta, file]` 튜플 구조를 받아들이도록 `COWPY` 프로세스를 수정해 봅시다.
...
튜토리얼을 간소화하기 위해 프로세스 출력 정의를 업데이트하여 메타 맵을 출력하지 않았지만, `IDENTIFY_LANGUAGE` 프로세스의 모델을 따라 직접 연습으로 수행해 보세요.
...
이전에 독립 실행형 입력으로 전달된 `character` 값에 대한 참조를 메타 맵에 저장된 값으로 교체했으며, `meta.character`를 사용하여 참조합니다.
...
따라서 더 간단한 코드로 이전과 동일한 결과를 생성합니다.
...
이는 `meta.character`가 존재하지 않기 때문에 접근하려는 시도가 `null`을 반환하기 때문에 발생합니다. 결과적으로 Nextflow는 명령줄에 문자 그대로 `null`을 연결하며, 이는 당연히 `cowpy` 도구에서 인식되지 않습니다.

#### 3.4.3. 해결책

워크플로우 구성의 일부로 기본값을 제공하는 것 외에도 이를 더 견고하게 처리하기 위해 할 수 있는 두 가지가 있습니다:

1. 워크플로우에 입력 검증을 구현하여 데이터시트에 필요한 모든 정보가 포함되어 있는지 확인합니다. Hello nf-core 교육 과정에서 [입력 검증 소개](../hello_nf-core/05_input_validation.md)를 찾을 수 있습니다. <!-- TODO (future) 적절한 검증 사이드 퀘스트 대기 중 -->
...
이 접근 방식은 `character`가 필수임을 명시적으로 보여주는 장점이 있으며 다른 컨텍스트에서 프로세스를 재배포하기 쉽게 만듭니다.
...
이 섹션에서 워크플로우 수준 또는 프로세스 수준에서 메타데이터에 접근하여 프로세스 실행을 사용자 정의하는 방법을 학습했습니다.
...
<!-- TODO (future) 작업된 해결책 제공 -->
<!-- 이름 변경은 프로세스 내부의 메타를 사용해야 합니다 -->
<!-- 출력 구성은 워크플로우 출력의 메타를 사용해야 합니다 -->
...
자신의 작업에 이 패턴을 적용하면 견고하고 유지 관리 가능한 생물정보학 워크플로우를 구축할 수 있습니다.
