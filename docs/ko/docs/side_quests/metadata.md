...
`main.nf` 파일의 워크플로는 점진적으로 완전히 작동하는 워크플로로 확장할 스텁입니다.
...
`langid` 도구를 래핑하는 `IDENTIFY_LANGUAGE`라는 사전 작성된 프로세스 모듈을 제공하므로 workflow 블록 앞에 include 문을 추가하기만 하면 됩니다.
...
`cowpy` 도구를 래핑하는 `COWPY`라는 사전 작성된 프로세스 모듈을 제공하므로 workflow 블록 앞에 include 문을 추가하기만 하면 됩니다.
