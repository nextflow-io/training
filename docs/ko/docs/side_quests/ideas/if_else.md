...
## 1. 유명한 과학자의 명언을 소가 말하게 만들기

이 섹션에는 지금까지 학습한 내용을 연습할 수 있는 심화 연습 문제가 포함되어 있습니다.
이러한 연습 문제는 교육의 후반부를 이해하는 데 _필수는 아니지만_, 유명한 과학자의 명언을 소가 말하게 만드는 방법을 알아내면서 학습 내용을 강화할 수 있는 재미있는 방법을 제공합니다.
...
`containers/data/pioneers.csv` 파일에는 컴퓨터 및 생물학 분야의 선구자 목록이 있습니다.
이 연습 문제를 완료하려면 다음 작업을 수행해야 합니다:
...
`quote` 컨테이너 이미지는 이전 심화 연습 문제에서 직접 빌드한 것을 사용하거나 Seqera Containers에서 가져온 것을 사용할 수 있습니다.
...
다음은 Nextflow 워크플로우에서 `if` 문을 사용하는 예제입니다:
...
Nextflow에서 컨테이너를 사용하여 프로세스를 실행하는 방법과 파이프라인에 분기 로직을 구축하는 방법을 학습했습니다!

### 다음 단계

축하합니다! 잠시 휴식을 취하고 물을 마시세요!

준비가 되면, 이 교육 시리즈의 파트 3으로 이동하여 지금까지 학습한 내용을 보다 현실적인 데이터 분석 사례에 적용하는 방법을 학습하세요.
