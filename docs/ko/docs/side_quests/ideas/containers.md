...
이 섹션 마지막의 연습에서 사용할 `quote` pip 패키지의 컨테이너 이미지를 얻거나 빌드하게 됩니다.
...
이 버전의 패키지에 대해 커뮤니티 컨테이너가 처음 빌드되는 경우, 완료되는 데 몇 분이 걸릴 수 있습니다.
생성된 컨테이너 이미지의 URI(예: `community.wave.seqera.io/library/pip_quote:ae07804021465ee9`)를 클릭하여 복사하세요.
...
먼저 살펴볼 항목은 `Dockerfile`입니다. 이는 컨테이너 이미지를 빌드하는 데 필요한 모든 명령을 포함하는 스크립트 파일의 일종입니다.
