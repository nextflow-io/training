...
`langid` 도구를 적용하는 `IDENTIFY_LANGUAGE`라는 사전 작성된 프로세스 모듈을 제공합니다. 워크플로우 블록 앞에 include 문만 추가하면 됩니다.
...
`cowpy` 도구를 적용하는 `COWPY`라는 사전 작성된 프로세스 모듈을 제공합니다. 워크플로우 블록 앞에 include 문만 추가하면 됩니다.
