...
이 과정은 실습 중심이며, 정보를 점진적으로 소개하는 목표 지향적인 연습으로 구성되어 있습니다.

텍스트 입력을 처리하는 Nextflow pipeline의 여러 버전을 실행합니다.
단일 단계로 구성된 간단한 버전부터 시작하여 결국 CSV 파일의 표 형식 텍스트 입력을 받아 몇 가지 변환 단계를 거친 후 변환된 텍스트를 말하는 캐릭터의 ASCII 그림을 포함하는 단일 텍스트 파일을 출력하는 다단계 버전으로 진행합니다.
...
| 과정 챕터                                      | 요약                                                                      | 예상 소요 시간 |
| ---------------------------------------------- | ------------------------------------------------------------------------- | -------------- |
| [Part 1: 기본 작업 실행](./01_basics.md)       | 간단한 workflow의 실행 및 관리                                            | 30분           |
| [Part 2: 실제 pipeline 실행](./02_pipeline.md) | 복잡한 입력 처리, 다단계 workflow 실행, 컨테이너 사용 및 손쉬운 병렬 실행 | 60분           |
| [Part 3: 실행 구성](./03_config.md)            | pipeline 동작 사용자 정의 및 다양한 컴퓨팅 환경에서의 사용 최적화         | 60분           |
