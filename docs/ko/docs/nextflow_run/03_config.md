...
이 섹션에서는 _workflow 코드 자체를 한 줄도 변경하지 않고_ pipeline의 동작을 맞춤화하고, 다양한 환경에 적응시키고, 리소스 사용을 최적화하기 위해 Nextflow pipeline의 설정을 관리하는 방법을 살펴봅니다.

이를 수행하는 여러 가지 방법이 있으며, 조합하여 사용할 수 있고 [Configuration](https://nextflow.io/docs/latest/config.html) 문서에 설명된 우선순위에 따라 해석됩니다.

과정의 이 부분에서는 Part 2의 컨테이너 섹션에서 이미 접한 `nextflow.config` 파일이라는 가장 간단하고 일반적인 설정 파일 메커니즘을 소개합니다.

process 지시문, executor, profile 및 매개변수 파일과 같은 Nextflow 설정의 필수 구성 요소를 살펴볼 것입니다.
이러한 설정 옵션을 효과적으로 활용하는 방법을 배우면 Nextflow pipeline의 유연성, 확장성 및 성능을 최대한 활용할 수 있습니다.

이러한 설정 요소를 연습하기 위해 이 교육 과정의 Part 2 끝에서 마지막으로 실행한 workflow의 새 복사본인 `3-main.nf`를 실행할 것입니다.
...
??? example "시나리오"
    ...
    또는 명령줄 인자에 익숙하지 않은 동료를 위해 pipeline을 설정하고 있을 수도 있습니다.

지금까지 작업해 온 것의 단순한 확장인 측면인 입력 매개변수 관리로 시작합니다.
...
이를 수행하는 여러 가지 방법이 있습니다. 매우 일반적으로 사용되는 세 가지 기본 방법을 소개합니다.

### 1.1. `nextflow.config`에서 값 설정

이것은 가장 간단한 접근 방식이지만, 기본 `nextflow.config` 파일은 모든 실행에 대해 편집하고 싶은 것이 아니기 때문에 가장 유연하지 않을 수 있습니다.
그러나 workflow에서 매개변수를 *선언*하는 것(확실히 거기에 속함)과 *기본값*을 제공하는 것(설정 파일에 더 적합함)의 관심사를 분리하는 이점이 있습니다.
...
workflow의 `params` 블록을 설정 파일에 단순히 복사하지 않았습니다.
이미 기본값이 선언된 `batch` 매개변수의 경우 구문이 약간 다릅니다.
workflow 파일에서는 타입이 지정된 선언입니다.
설정에서는 값 할당입니다.

기술적으로 이것은 workflow 파일에 여전히 지정된 기본값을 재정의하기에 충분합니다.
`batch`의 기본값을 수정하고 workflow를 실행하여 설정 파일에 설정된 값이 workflow 파일에 설정된 값을 재정의하는지 확인할 수 있습니다.

그러나 설정을 완전히 설정 파일로 이동하는 정신으로 workflow 파일에서 해당 기본값을 완전히 제거해 보겠습니다.
...
명령줄에서 매개변수를 지정하지 않고 올바르게 작동하는지 테스트합니다.
...
기능적으로 이 이동은 아무것도 변경하지 않았지만, 개념적으로 설정 파일에 기본값을 설정하는 것이 약간 더 깔끔합니다.

### 1.2. 실행별 설정 파일 사용

??? example "시나리오"

    기본 설정 파일을 수정하지 않고 다른 설정으로 실험하고 싶습니다.

실험을 위한 작업 디렉토리로 사용할 하위 디렉토리에 새 `nextflow.config` 파일을 생성하여 이를 수행할 수 있습니다.

#### 1.2.1. 빈 설정으로 작업 디렉토리 생성

새 디렉토리를 생성하고 그 안으로 이동합니다:
...
이렇게 하면 '일반' 설정을 수정하지 않고 실험할 수 있는 공간이 생겼습니다.
...
설정 파일과 구문을 비교할 때 등호(`=`) 대신 콜론(`:`)을 사용합니다.
설정 파일은 Groovy로 작성되고 매개변수 파일은 YAML로 작성됩니다.
...
workflow 입력 관리를 위한 주요 설정 옵션을 활용하는 방법을 알게 되었습니다.
...
대신 설정 파일에서 이를 구성하여 더 복잡한 동적 경로를 정의할 수 있습니다.
이를 위해 완전히 새로운 매개변수를 만들 수도 있지만, `batch` 매개변수가 바로 있으니 사용해 보겠습니다.

#### 2.1.1. 설정 파일에서 `outputDir` 값 설정

Nextflow가 출력을 게시하는 데 사용하는 경로는 `outputDir` 옵션으로 제어됩니다.
모든 출력의 경로를 변경하려면 `nextflow.config` 설정 파일에서 이 옵션의 값을 설정할 수 있습니다.
...
CLI 플래그를 사용하면 설정에 설정된 `outputDir`를 덮어씁니다.
...
명령줄에서 배치 이름을 `outdir`로 설정하여 올바르게 작동하는지 테스트합니다.
...
이전과 동일한 출력이 생성되지만, 이번에는 출력을 `results_config/outdir/` 아래에서 찾을 수 있습니다.
...
이것은 출력 경로 설정에서 남아 있는 하드코딩된 요소를 제거합니다.

#### 2.2.2. Pipeline 실행

명령줄에서 배치 이름을 `pnames`로 설정하여 올바르게 작동하는지 테스트합니다.
...
이전과 동일한 출력이 생성되지만, 이번에는 출력을 `results_config/pnames/` 아래에서 찾을 수 있으며 process별로 그룹화됩니다.
...
마지막으로 반복적인 코드의 양을 줄이는 정신으로 출력별 `mode` 선언을 설정의 단일 줄로 대체할 수 있습니다.
...
`outputDir` 옵션과 마찬가지로 설정 파일에서 `workflow.output.mode`에 값을 부여하면 workflow 파일에 설정된 것을 재정의하기에 충분하지만, 어쨌든 불필요한 코드를 제거해 보겠습니다.
...
명령줄에서 배치 이름을 `outmode`로 설정하여 올바르게 작동하는지 테스트합니다.
...
이전과 동일한 출력이 생성되지만, 이번에는 출력을 `results_config/outmode/` 아래에서 찾을 수 있습니다.
...
소프트웨어 패키징 기술부터 시작하여 컴퓨팅 환경에 맞게 workflow 설정을 조정하는 방법을 배웁니다.

---

## 3. 소프트웨어 패키징 기술 선택

지금까지 입력이 어떻게 들어가고 출력이 어디서 나오는지 제어하는 설정 요소를 살펴보았습니다. 이제 컴퓨팅 환경에 맞게 workflow 설정을 조정하는 데 더 구체적으로 집중할 때입니다.
...
문제없이 작동하고 이전과 동일한 출력을 `results_config/conda` 아래에 생성해야 합니다.
...
??? info "Docker와 Conda 혼합 사용"
    ...
    이러한 지시문은 process별로 할당되므로 '혼합하여 사용'할 수 있습니다. 즉, 사용 중인 컴퓨팅 인프라가 둘 다 지원하는 경우 workflow의 일부 process는 Docker로, 다른 일부는 Conda로 실행하도록 설정할 수 있습니다.
...
각 process가 사용해야 하는 소프트웨어 패키지를 설정하고 기술 간에 전환하는 방법을 알게 되었습니다.
...
불행히도 이러한 각 시스템은 작업이 정의되고 관련 스케줄러에 제출되는 방법을 정의하기 위해 다른 기술, 구문 및 설정을 사용합니다.
...
해당 process 지시문은 `cpus`와 `memory`라고 하므로 다음 설정이 암시됩니다:
...
설정 파일에서 추가 process 지시문을 사용하여 모든 process 또는 특정 명명된 process에 대해 이러한 값을 수정할 수 있습니다.
...
보고서는 브라우저에서 다운로드하여 열 수 있는 html 파일입니다. 교육 환경에서 보려면 왼쪽의 파일 탐색기(File Explorer)에서 마우스 오른쪽 버튼을 클릭하고 `Show preview`를 클릭할 수도 있습니다.
...
이 설정을 사용하면 모든 process는 1GB의 메모리와 단일 CPU(암시된 기본값)를 요청하지만, `cowpy` process는 2GB와 2개의 CPU를 요청합니다.

!!! info "정보"

    CPU가 적은 머신에서 process당 높은 수의 CPU를 할당하면 process 실행이 서로 뒤에 대기열에 들어가는 것을 볼 수 있습니다.
    이것은 Nextflow가 사용 가능한 것보다 더 많은 CPU를 요청하지 않도록 하기 때문입니다.

### 5.4. 업데이트된 설정으로 workflow 실행

설정 변경 전후의 성능을 비교할 수 있도록 프로파일링 보고서에 다른 파일 이름을 제공하여 시도해 보겠습니다.
...
??? info "기관 참조 설정"
    ...
    nf-core 프로젝트는 다양한 HPC 및 클라우드 executor를 포괄하는 전 세계 여러 기관에서 공유하는 [설정 파일 모음](https://nf-co.re/configs/)을 컴파일했습니다.

    이러한 공유 설정은 해당 기관의 설정을 바로 활용할 수 있는 해당 기관에서 근무하는 사람들과 자체 인프라에 대한 설정을 개발하려는 사람들의 모델로 모두 가치가 있습니다.
...
사전 설정 설정 프로필을 설정하고 런타임에 전환하는 방법을 배웁니다.
...
??? example "시나리오"
    ...
    환경을 전환할 때마다 설정을 수동으로 변경하는 것이 지겹습니다.

작업 중인 프로젝트나 사용 중인 컴퓨팅 환경에 따라 pipeline 설정을 사용자 정의할 수 있는 여러 가지 방법을 소개했습니다.

사용 중인 컴퓨팅 인프라에 따라 대체 설정 간에 전환하고 싶을 수 있습니다. 예를 들어 노트북에서 로컬로 개발하고 소규모 테스트를 실행한 다음 HPC 또는 클라우드에서 전체 규모 워크로드를 실행할 수 있습니다.

Nextflow를 사용하면 다양한 설정을 설명하는 [**profile**](https://nextflow.io/docs/latest/config.html#profiles)을 원하는 수만큼 설정한 다음 설정 파일 자체를 수정하지 않고 명령줄 인자를 사용하여 런타임에 선택할 수 있습니다.
...
`my_laptop` 설정으로 workflow를 실행합니다.
...
보시다시피 런타임에 설정 간에 매우 편리하게 전환할 수 있습니다.

!!! warning "경고"

    Slurm 스케줄러에 액세스할 수 없으므로 `univ_hpc` 프로필은 교육 환경에서 제대로 실행되지 않습니다.

향후 이와 항상 함께 발생하는 다른 설정 요소를 발견하면 해당 프로필에 간단히 추가할 수 있습니다.
함께 그룹화하려는 다른 설정 요소가 있는 경우 추가 프로필을 만들 수도 있습니다.
...
동일한 설정 요소에 대해 값을 설정하는 프로필을 결합하고 동일한 설정 파일에 설명되어 있는 경우 Nextflow는 마지막으로 읽은 값(즉, 파일에서 나중에 나오는 값)을 사용하여 충돌을 해결합니다.
충돌하는 설정이 다른 설정 소스에 설정된 경우 기본 [우선순위](https://www.nextflow.io/docs/latest/config.html)가 적용됩니다.
...
!!! tip "팁"
    ...
    자세한 내용은 Side Quest [Working with Files](../side_quests/working_with_files/index.md)를 참조하세요.

### 6.3. `nextflow config`를 사용하여 해결된 설정 확인

위에서 언급했듯이 때때로 결합하려는 프로필에서 동일한 매개변수를 다른 값으로 설정할 수 있습니다.
그리고 더 일반적으로 설정 요소를 저장할 수 있는 곳이 많고 때때로 동일한 속성이 다른 곳에서 다른 값으로 설정될 수 있습니다.

Nextflow는 충돌을 해결하기 위해 설정된 [우선순위](https://nextflow.io/docs/latest/config.html#configuration-file)를 적용하지만, 직접 결정하기 어려울 수 있습니다.
그리고 충돌이 없더라도 설정될 수 있는 모든 가능한 장소를 찾는 것은 지루할 수 있습니다.

다행히 Nextflow에는 전체 프로세스를 자동화할 수 있는 `config`라는 편리한 유틸리티 도구가 포함되어 있습니다.

`config` 도구는 현재 작업 디렉토리의 모든 내용을 탐색하고 모든 설정 파일을 수집한 다음 Nextflow가 workflow를 실행하는 데 사용할 완전히 해결된 설정을 생성합니다.
이렇게 하면 아무것도 시작하지 않고도 어떤 설정이 사용될지 알 수 있습니다.

#### 6.3.1. 기본 설정 해결

기본적으로 적용될 설정을 확인하려면 이 명령을 실행하세요.
...
이것은 여러 계층의 설정을 포함하는 복잡한 프로젝트에 특히 유용합니다.

### 핵심 정리

런타임에 최소한의 번거로움으로 사전 설정 구성을 선택하기 위해 프로필을 사용하는 방법을 알게 되었습니다.
더 일반적으로 다양한 컴퓨팅 플랫폼에 맞게 workflow 실행을 설정하고 분석의 재현성을 향상시키는 방법을 알게 되었습니다.
...
공식 Nextflow "hello" 데모 pipeline을 실행합니다:
...
자세히 알아보기: [1.1. `nextflow.config`에서 값 설정](#11-set-up-values-in-nextflowconfig)
</quiz>

<quiz>
workflow 파일과 설정 파일에서 매개변수 기본값을 설정하는 구문의 차이점은 무엇인가요?
- [ ] 동일한 구문을 사용합니다
- [x] workflow는 타입이 지정된 선언(`#!groovy param: Type = value`)을 사용하고, 설정은 할당(`#!groovy param = value`)을 사용합니다
- [ ] 설정은 타입이 지정된 선언을 사용하고, workflow는 할당을 사용합니다
- [ ] 설정 파일만 기본값을 설정할 수 있습니다
...
<quiz>
`outputDir` 설정 옵션은 무엇을 제어하나요?
- [ ] 작업 디렉토리의 위치
- [x] workflow 출력이 게시되는 기본 경로
- [ ] 로그 파일 디렉토리
- [ ] 모듈 파일의 위치

자세히 알아보기: [2.1. outputDir 디렉토리 이름 사용자 정의](#21-customize-the-outputdir-directory-name)
</quiz>

<quiz>
출력 경로 설정에서 process 이름을 동적으로 참조하는 방법은 무엇인가요?
- [ ] `#!groovy ${processName}`
- [ ] `process.name`
- [x] `#!groovy { <process>.name }`
- [ ] `@processName`

자세히 알아보기: [2.2. Process별로 출력 구성](#22-organize-outputs-by-process)
</quiz>
...
자세히 알아보기: [5.1. 리소스 활용 보고서를 생성하기 위해 workflow 실행](#51-run-the-workflow-to-generate-a-resource-utilization-report)
</quiz>

<quiz>
설정 파일에서 `cowpy`라는 특정 process에 대한 리소스 요구 사항을 어떻게 설정하나요?
...
자세히 알아보기: [5.3. 특정 process에 대한 리소스 할당 설정](#53-set-resource-allocations-for-a-specific-process)
</quiz>
...
<quiz>
Nextflow가 사용할 완전히 해결된 설정을 보여주는 명령은 무엇인가요?
- [ ] `nextflow show-config`
- [ ] `nextflow settings`
- [x] `nextflow config`
- [ ] `nextflow resolve`

자세히 알아보기: [6.3. `nextflow config`를 사용하여 해결된 설정 확인](#63-use-nextflow-config-to-see-the-resolved-configuration)
</quiz>
