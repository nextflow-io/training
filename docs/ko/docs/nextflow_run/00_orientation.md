...
기본적으로 Codespace는 모든 교육 과정의 루트에 작업 디렉토리가 설정된 상태로 열리지만, 이 과정에서는 `nextflow-run/` 디렉토리에서 작업합니다.
...
- **`data/` 아래의 `greetings.csv` 파일**은 과정의 대부분에서 사용할 입력 데이터를 포함합니다. 처음 소개할 때인 Part 2(파이프라인 실행)에서 설명됩니다.

- **`test-params.*` 파일**은 Part 3(구성)에서 사용할 구성 파일입니다. 지금은 무시해도 됩니다.

- **`solutions` 디렉토리**는 과정을 완료한 결과로 생성되는 워크플로우와 그 부속 파일(구성 및 모듈)의 최종 상태를 포함합니다.
  작업을 확인하고 문제를 해결하는 데 참조용으로 사용하도록 의도되었습니다.
...
**[Part 1: 기본 작업 실행](./01_basics.md)으로 계속하려면 이 페이지의 오른쪽 하단 모서리에 있는 화살표를 클릭하세요.**
