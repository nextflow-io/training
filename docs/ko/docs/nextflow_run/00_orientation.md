...
이 교육은 Nextflow 25.10.2 이상에서 **v2 구문 분석기가 활성화된 상태**로 설계되었습니다.
...
!!! tip "팁"
    ...
    어떤 이유로든 이 디렉토리를 벗어난 경우(예: codespace가 중지(sleep) 상태가 됨), Github Codespaces 교육 환경 내에서 실행하고 있다고 가정하면 항상 전체 경로를 사용하여 돌아올 수 있습니다:
...
- **`.nf` 파일**은 과정의 어느 부분에서 사용되는지에 따라 번호가 매겨진 워크플로우 스크립트입니다.

- **`nextflow.config` 파일**은 최소한의 환경 속성을 설정하는 구성 파일입니다.
  지금은 무시해도 됩니다.

- **`data/` 아래의 `greetings.csv` 파일**에는 과정의 대부분에서 사용할 입력 데이터가 포함되어 있습니다. Part 2(파이프라인 실행)에서 처음 소개할 때 설명됩니다.

- **`test-params.*` 파일**은 Part 3(구성)에서 사용할 구성 파일입니다. 지금은 무시해도 됩니다.

- **`solutions` 디렉토리**에는 과정을 완료하면 생성되는 워크플로우의 최종 상태와 그 부속 파일(config 및 modules)이 포함되어 있습니다.
