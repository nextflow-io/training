...
Build with nf-core 교육 과정의 네 번째 파트에서는 nf-schema 플러그인을 사용하여 파이프라인 입력과 매개변수를 검증하는 방법을 다룹니다.

??? info "이 섹션을 시작하는 방법"

    이 섹션은 [Part 3: Make an nf-core module](./03_make_module.md)을 완료하고 파이프라인의 `COWPY` process 모듈을 nf-core 표준으로 업데이트했다고 가정합니다.

    Part 3을 완료하지 않았거나 이 파트를 새로 시작하고 싶다면, `core-hello-part3` 솔루션을 시작점으로 사용할 수 있습니다.
    `nfcore-build/` 디렉토리 안에서 다음 명령을 실행하십시오:
...
이제 이러한 원칙을 실제로 적용합니다. 매개변수 검증부터 시작합니다.
...
??? info "스키마 매개변수는 어디에서 왔습니까?"
    ...
    워크플로우 스크립트의 다른 곳(`main.nf` 또는 모듈 파일)에서 선언된 매개변수는 스키마 검증기에 의해 자동으로 **인식되지 않습니다**.
...
대화형 `nf-core pipelines schema build` 도구를 사용하여 `nextflow_schema.json`에 매개변수를 추가하는 방법을 학습했고 실제로 매개변수 검증을 확인했습니다.
웹 인터페이스가 모든 JSON Schema 구문을 처리해 주므로 오류가 발생하기 쉬운 수동 JSON 편집 없이도 복잡한 매개변수 스키마를 쉽게 관리할 수 있습니다.

### 다음 단계

이제 매개변수 검증이 작동하니 입력 데이터 파일 내용에 대한 검증을 추가합니다.

---

## 2. 입력 데이터 검증 (schema_input.json)

입력 CSV 파일의 내용에 대한 검증을 추가합니다.
...
nf-core 파이프라인 템플릿은 페어드 엔드 시퀀싱 데이터를 위해 설계된 기본 `assets/schema_input.json`을 포함합니다.
...
`subworkflows/local/utils_nfcore_hello_pipeline/main.nf`를 열고 입력 채널을 생성하는 섹션을 찾으십시오(80줄 근처).

다음을 수행해야 합니다:

1. `samplesheetToList` 함수 사용 (템플릿에 이미 import됨)
2. 입력 검증 및 파싱
3. 워크플로우를 위해 인사말 문자열만 추출
...
2. **`channel.fromList(...)`**: 리스트를 Nextflow 채널로 변환
...
유효한 입력과 유효하지 않은 입력을 모두 테스트하여 검증이 작동하는지 확인합니다.
...
검증 통과는 항상 기분 좋지만, 검증이 실제로 오류를 잡아내는지 확인합니다.
