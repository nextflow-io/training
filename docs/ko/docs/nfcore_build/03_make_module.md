...
Build with nf-core 교육 과정의 세 번째 파트에서는 모듈을 이식 가능하고 유지보수 가능하게 만드는 주요 규칙을 적용하여 nf-core 모듈을 만드는 방법을 다룹니다.

nf-core 프로젝트는 Part 1에서 워크플로우에 사용했던 것과 유사하게 적절하게 구조화된 모듈 템플릿을 자동으로 생성하는 명령(`nf-core modules create`)을 제공합니다.
...
??? info "이 섹션을 시작하는 방법"
    ...
    이 섹션은 [Part 2: nf-core 모듈 사용하기](./02_use_module.md)를 완료하고 `FIND_CONCATENATE` 모듈을 파이프라인에 통합했다고 가정합니다.

    Part 2를 완료하지 않았거나 이 파트를 새롭게 시작하려는 경우, `core-hello-part2` 솔루션을 시작점으로 사용할 수 있습니다.
    `nfcore-build/` 디렉토리 내에서 다음 명령을 실행하십시오:
    ...
    이렇게 하면 `FIND_CONCATENATE` 모듈이 이미 통합된 파이프라인을 얻게 됩니다.
...
2. **`COWPY`를 메타데이터 튜플을 사용하도록 업데이트** - 워크플로우를 통해 샘플 메타데이터를 전달하기 위함입니다.
...
3. 워크플로우 본문에서 프로세스 실행 및 emit 선언 업데이트
...
프로세스 이름이 여러 단어로 구성된 경우, 예를 들어 원래 camel case로 `MyCowpyTool`이라는 프로세스가 있었다면, nf-core 규칙은 밑줄을 사용하여 분리하여 `MY_COWPY_TOOL`이 됩니다.

#### 1.1.2. 모듈 import 문 업데이트

프로세스 이름은 대소문자를 구분하므로, 프로세스 이름을 변경했으므로 `hello.nf`의 워크플로우 헤더에서 모듈 import 문을 업데이트해야 합니다:
...
import 문에서 별칭을 사용하여 프로세스 실행을 업데이트하지 않을 수도 있지만, 그렇게 하면 대문자화 규칙을 채택하는 목적이 다소 상실됩니다.

#### 1.1.3. 프로세스 실행 및 emit 선언 업데이트
...
이러한 변경 후 모든 것이 올바르게 작동하는지 테스트하기 위해 워크플로우를 실행합니다.
...
현재 버전의 `core-hello` 파이프라인에서는 아래 다이어그램의 상단에 표시된 것처럼 `FIND_CONCATENATE`의 출력 튜플에서 파일을 추출하여 `COWPY`에 전달하고 있습니다.

<figure class="excalidraw">
    --8<-- "docs/en/docs/nfcore_build/img/cowpy-inputs.svg"
</figure>

다이어그램의 하단에 표시된 것처럼 `COWPY`가 메타데이터 튜플을 직접 받아들이도록 하여 워크플로우를 통해 메타데이터가 계속 흐르도록 하는 것이 더 좋습니다.

이를 위해 다음 변경 사항을 적용해야 합니다:

1. 입력 및 출력 정의 업데이트
2. 워크플로우에서 프로세스 실행 업데이트
...
보시다시피, **주요 입력**과 **출력** 모두를 Part 2에서 소개한 `tuple val(meta), path(input_file)` 패턴을 따르는 튜플로 변경했습니다.
출력의 경우, 출력 채널에 설명적인 이름을 부여하기 위해 `emit: cowpy_output`을 추가하는 기회를 가졌습니다.

이제 프로세스가 예상하는 것을 변경했으므로, 프로세스 실행 시 제공하는 것을 업데이트해야 합니다.

#### 1.2.2. 워크플로우에서 프로세스 실행 업데이트

이 변경으로 프로세스 실행이 단순화된다는 좋은 소식이 있습니다.
이제 `FIND_CONCATENATE`의 출력과 `COWPY`의 입력이 동일한 '형태', 즉 둘 다 `tuple val(meta), path(input_file)` 구조로 구성되므로, `FIND_CONCATENATE` 프로세스의 출력에서 파일을 명시적으로 추출하는 대신 직접 연결할 수 있습니다.

`hello.nf` 워크플로우 파일(`core-hello/workflows/` 아래)을 열고 아래와 같이 `COWPY` 실행을 업데이트하십시오.
...
이제 `FIND_CONCATENATE.out.file_out`에서 직접 `COWPY`를 실행합니다.

결과적으로 더 이상 `ch_for_cowpy` 채널을 구성할 필요가 없으므로 해당 줄(및 주석 줄)을 완전히 삭제할 수 있습니다.

#### 1.2.3. 워크플로우에서 emit 블록 업데이트

`COWPY`가 이제 명명된 출력인 `cowpy_output`을 방출하므로, `hello.nf` 워크플로우의 `emit:` 블록을 이를 사용하도록 업데이트할 수 있습니다.
...
파이프라인은 성공적으로 실행되어야 하며, 이제 메타데이터가 `FIND_CONCATENATE`에서 `COWPY`로 흐릅니다.
...
결과적으로, 도구에서 설정한 기본값으로 만족하더라도 프로세스를 실행할 때마다 값을 제공해야 합니다.
...
`withName:` 구문은 이 구성을 `COWPY` 프로세스에만 할당하며, `#!groovy ext.args = { "-c ${params.character}" }`는 단순히 `character` 매개변수의 값을 포함할 문자열을 구성합니다.
...
- **파이프라인은 여전히 `params.character`를 노출합니다** - 최종 사용자는 여전히 이전과 같이 설정할 수 있습니다
- **모듈은 이제 이식 가능합니다** - 특정 매개변수 이름을 기대하지 않고 다른 파이프라인에서 재사용할 수 있습니다
- 구성이 `modules.config`에 **중앙화되어** 워크플로우 로직을 깔끔하게 유지합니다

모든 파이프라인이 모듈별 구성을 중앙화하는 장소로 `modules.config` 파일을 사용함으로써, 다른 파이프라인에서 모듈을 더 재사용 가능하게 만듭니다.

#### 1.3.3. `hello.nf` 워크플로우 업데이트

`COWPY` 모듈이 더 이상 `character` 매개변수를 입력으로 요구하지 않으므로, 워크플로우 실행을 그에 따라 업데이트해야 합니다.
...
워크플로우가 여전히 예상대로 작동하는지 테스트합니다. `ext.args` 구성이 작동하는지 확인하기 위해 다른 character를 지정합니다.
...
`ext.args` 구성이 작동했는지 출력을 확인하여 검증합니다.
...
여기서는 `meta.id`(메타맵에 포함된 식별자)를 사용하여 모듈 전체에서 출력 파일 이름을 표준화하면서도 필요한 경우 개별적으로 모듈을 설정할 수 있도록 하는 `ext.prefix`라는 Nextflow 기능을 사용하겠습니다.
...
1. **`script:` 블록에서 `#!groovy prefix = task.ext.prefix ?: "${meta.id}"` 줄을 추가했습니다.**
...
2. **명령줄에서 `#!groovy cowpy-${input_file}`을 `#!groovy ${prefix}.txt`로 대체했습니다.**
   여기에 Nextflow가 위 줄에서 결정된 `prefix` 값을 주입합니다.

3. **`output:` 블록에서 `#!groovy path("cowpy-${input_file}")`을 `#!groovy path("${prefix}.txt")`로 대체했습니다.**
...
- **설정 가능**: 필요한 경우 사용자가 기본 명명을 재정의할 수 있습니다
- **일관성**: 모든 nf-core 모듈이 이 패턴을 따릅니다
...
- **`core-hello-results`** — 명령줄에서 `--outdir`로 설정된 출력 디렉토리로, nf-core 로그와 `FIND_CONCATENATE`이 발행한 결과를 받고 있음.
...
워크플로우 설정 수준에서 출력이 발행되는 위치를 제어할 수 있어야 합니다.

"잠깐," 여러분이 말할 수 있습니다. "`FIND_CONCATENATE`은 `--outdir`로 출력을 보내고 있습니다. `publishDir` 지시문을 복사해야 할까요?"

네, 훌륭한 아이디어입니다.

단, `publishDir` 지시문이 없습니다. (계속해서 모듈 코드를 보십시오.)

nf-core 파이프라인은 개별 모듈에서 `publishDir`을 설정하는 대신 `conf/modules.config`에서 설정하여 워크플로우 수준에서 제어를 중앙화하기 때문입니다.
...
`COWPY`에서 시도하여 어떤 일이 일어나는지 확인한 다음, 작동 방식을 이해하기 위해 기본 설정 코드를 살펴보겠습니다.
...
Nextflow가 워크플로우와 모듈 이름을 기반으로 이 디렉토리 계층 구조를 생성했습니다.

!!! info "정보"

    `pipeline_info/`에 `hello_software_versions.yml`이 있는 것을 확인할 수 있습니다.
    현재는 `FIND_CONCATENATE`의 버전 정보만 포함되어 있는데, `COWPY`가 아직 버전을 보고하지 않기 때문입니다.
    섹션 1.6에서 이를 추가하는 방법을 다룹니다.

책임있는 코드는 `conf/modules.config` 파일에 있습니다.
다음은 nf-core 템플릿의 일부이며 모든 프로세스에 적용되는 기본 `publishDir` 설정입니다:
...
- **`path:`** 프로세스 이름을 기반으로 출력 디렉토리를 결정합니다.
  `task.process`에 포함된 프로세스의 전체 이름에는 워크플로우 및 모듈 가져오기 계층 구조(예: `CORE_HELLO:HELLO:FIND_CONCATENATE`)가 포함됩니다.
  `tokenize` 연산은 계층 구조를 제거하여 프로세스 이름만 얻은 다음, 밑줄 앞의 첫 부분을 가져오고(해당되는 경우) 소문자로 변환합니다.
  이것이 `FIND_CONCATENATE`의 결과가 `#!groovy ${params.outdir}/find/`에 발행되도록 결정하는 요소입니다.
- **`mode:`** 파일이 발행되는 방식(복사, 심볼릭 링크 등)을 제어합니다.
  이것은 `params.publish_dir_mode` 매개변수를 통해 설정할 수 있습니다.
...
요점은 이 시스템이 양쪽 세계의 장점을 제공한다는 것입니다: 기본적으로 일관성과 필요에 따라 설정을 사용자 정의하는 유연성.

요약하자면, 다음과 같은 이점을 얻습니다:

- **단일 진실 소스**: 모든 발행 설정은 `modules.config`에 있습니다
- **유용한 기본값**: 프로세스는 모듈별 설정 없이도 즉시 작동합니다
- **쉬운 사용자 정의**: 모듈 코드가 아닌 설정에서 발행 동작을 재정의합니다
- **이식 가능한 모듈**: 모듈은 출력 위치를 하드코딩하지 않습니다

### 1.6. topic 채널을 통한 버전 캡처

재현성은 nf-core의 핵심 원칙입니다: 모든 파이프라인 실행은 사용된 소프트웨어 버전을 정확히 기록하여 결과를 재현하고 실행 간에 비교할 수 있도록 합니다.
`pipeline_info/`의 `hello_software_versions.yml` 보고서는 nf-core 파이프라인이 이 요구 사항을 충족하는 방법입니다.

그 메커니즘은 **topic 채널**입니다: 어떤 프로세스든 명명된 topic에 데이터를 발행할 수 있고, 모든 구독자는 파이프라인의 어디에서 오든 해당 topic에 발행된 모든 것을 수신하는 Nextflow 브로드캐스트 기능입니다.
nf-core는 2026년에 모든 파이프라인에 이를 도입하고 있습니다.
모듈은 `topic:` 키워드를 사용하여 출력 블록에서 직접 버전을 튜플로 발행하고, 워크플로우는 한 번 구독하여 모두 수집합니다.

#### 1.6.1. `COWPY`에 버전 보고 추가

`cowpy.nf` 모듈 파일을 열고 아래와 같이 버전 출력 줄을 추가하십시오.
...
새 줄은 `[process_name, tool_name, version]` 튜플을 `"versions"` topic 채널에 발행합니다.
script 블록은 변경할 필요가 없습니다 — 버전은 출력 블록에 정적으로 선언됩니다.

#### 1.6.2. 파이프라인을 실행하고 버전 보고서 확인
...
`core-hello-results/pipeline_info/hello_software_versions.yml`을 열면 이제 두 모듈이 모두 표시됩니다:
...
워크플로우 측 수집 — Part 1의 플레이스홀더 워크플로우에서 보았던 `channel.topic("versions")` 블록 — 이 topic을 구독하고 이 통합 보고서를 자동으로 작성합니다.

!!! info "하위 호환성"

    워크플로우의 topic 채널 블록에 있는 `versions_file` 브랜치는 아직 `topic: versions`를 사용하도록 업데이트되지 않고 `emit: versions`와 함께 script 블록에서 `versions.yml` 파일을 작성하는 모듈을 처리하기 위해 존재합니다.
    전환 기간 동안 두 스타일이 동시에 지원됩니다.
...
- `ext.prefix`를 사용하여 설정 가능하고 표준화된 출력 파일 이름 지정;
- 일관된 결과 디렉토리 구조를 위해 중앙화된 기본 `publishDir` 지시문 채택;
- topic 채널이 모든 모듈에서 소프트웨어 버전을 자동으로 수집하는 방법 이해.

모듈 규칙에 대한 추가 자료는 [nf-core 모듈 사양](https://nf-co.re/docs/guidelines/components/modules)을 참조하십시오.
...
- **`environment.yml`**: 의존성에 대한 Conda 환경 사양
- **`tests/main.nf.test`**: 모듈이 작동하는지 검증하는 nf-test 테스트 케이스

!!! tip "테스트에 대해 더 알아보기"

    생성된 테스트 파일은 Nextflow 파이프라인 및 모듈용 테스팅 프레임워크인 nf-test를 사용합니다. 이러한 테스트를 작성하고 실행하는 방법을 알아보려면 [nf-test 사이드 퀘스트](../side_quests/nf_test/index.md)를 참조하십시오.
...
- **`label 'process_single'`**: CPU/메모리 요구 사항을 설정하기 위한 리소스 레이블
- **`when:` 블록**: `task.ext.when` 설정을 통한 조건부 실행 허용
- **`topic: versions` 출력**: 섹션 1.6에서 다룬 topic 채널을 통한 버전 보고
...
- **`script:` 블록**: 비어 있음 — 여기에 `cowpy` 명령을 추가합니다
...
Conda 환경의 경우, 모듈 코드는 `#!groovy conda "${moduleDir}/environment.yml"`을 지정하며, 이는 `environment.yml` 파일에서 설정해야 함을 의미합니다.
...
- 설정 가능한 접두사 패턴을 사용한 출력 파일 이름(와일드카드 `*` 대신 `#!groovy ${prefix}.txt`)
- 설명적인 emit 이름(일반적인 `output` 대신 `cowpy_output`)
- 템플릿의 `#!groovy eval("cowpy --version")` 대신 정적 버전 문자열(`#!groovy val("1.1.5")`). 섹션 1.6의 수동 모듈과 일치하며(`cowpy` 도구는 `--version` 플래그를 제공하지 않음)

Nextflow 언어 서버를 사용하여 구문을 검증하는 경우, script 블록에 아직 추가하지 않았기 때문에 이 단계에서 `#!groovy ${prefix}` 부분이 오류로 표시될 수 있습니다.
이제 그 부분을 살펴보겠습니다.

#### 2.3.2. script 블록

템플릿은 실제 도구 명령을 추가해야 하는 빈 script 블록을 생성합니다.
...
- `$args`와 `#!groovy ${prefix}.txt`를 모두 사용하는 실제 `cowpy` 명령을 채워 넣음

이미 `modules.config` 파일에 `COWPY` 프로세스에 대한 `ext.args` 및 `ext.prefix` 설정을 추가하지 않았다면 지금 추가해야 합니다.
...
- stub은 script 블록이 생성하는 것과 일치하는 빈 `#!groovy ${prefix}.txt` 파일을 생성합니다

이를 통해 실제 도구가 실행되기를 기다리지 않고도 워크플로우 로직과 파일 처리를 테스트할 수 있습니다.
...
이 새 버전의 `COWPY` 모듈을 시도해보기 위해 필요한 것은 `hello.nf` 워크플로우 파일의 import 문을 새 파일을 가리키도록 변경하는 것뿐입니다.
...
nf-core에 모듈을 기여하는 이점과 포함된 주요 단계 및 요구 사항에 대해 알아보겠습니다.
...
- **`ext.args`**는 설정을 통해 선택적 인수를 처리하여 모듈 인터페이스를 단순화합니다
- **`ext.prefix`**는 출력 파일 이름을 표준화합니다
- **중앙화된 발행**은 모듈에 하드코딩되는 대신 `modules.config`에서 설정된 `publishDir`를 통해 이루어집니다
...
준비가 되면 [파트 4: 입력 검증](./04_input_validation.md)으로 계속하여 파이프라인에 스키마 기반 입력 검증을 추가하는 방법을 알아보세요.
