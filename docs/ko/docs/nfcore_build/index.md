...
additional_information:
  ...
  audience_prerequisites:
    ...
    - "**과정:** [Hello Nextflow](../hello_nextflow/index.md) 및 [Use nf-core](../nfcore_use/index.md) 과정을 완료했거나 이에 상응하는 경험이 있어야 합니다."
...
실용적인 예제와 유도형 실습을 통해 nf-core 호환 모듈과 파이프라인을 사용하고 개발하는 방법, 그리고 nf-core 도구를 효과적으로 활용하는 방법을 학습합니다.

nf-core 모범 사례에 따라 파이프라인을 개발할 수 있는 기술과 자신감을 갖추게 됩니다.
...
대신, 시작하는 데 도움이 되고 nf-core가 어떻게 작동하는지 이해하는 데 필요한 필수 개념에 초점을 맞춥니다.

### 수업 계획

이 과정은 nf-core 리소스 사용의 특정 측면에 각각 초점을 맞춘 네 부분으로 나뉩니다.

| 과정 챕터                                               | 요약                                                                                                                             | 예상 소요 시간 |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| [파트 1: nf-core용 Hello 재작성](./01_rewrite_hello.md) | [Hello Nextflow](../hello_nextflow/index.md) 과정에서 생성한 간단한 워크플로우를 nf-core 템플릿 스캐폴드에 맞게 적응시킵니다     | 60분           |
| [파트 2: nf-core 모듈 사용](./02_use_module.md)         | 커뮤니티 모듈 라이브러리를 탐색하고 일반적인 생물정보학 도구를 적용하는 사전 구축되고 테스트된 모듈을 통합하는 방법을 학습합니다 | 30분           |
| [파트 3: nf-core 모듈 만들기](./03_make_module.md)      | nf-core가 정한 특정 구조, 명명 규칙 및 메타데이터 요구사항을 사용하여 자신만의 nf-core 스타일 모듈을 생성합니다                  | 30분           |
| [파트 4: 입력 검증 추가](./04_input_validation.md)      | nf-schema를 사용하여 명령줄 매개변수와 입력 데이터 파일 모두에 대한 입력 검증을 구현합니다                                       | 30분           |
