# 1부: nf-core용 Hello 재작성

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } AI 지원 번역 - [자세히 알아보기 및 개선 제안](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>

Build with nf-core 교육 과정의 첫 번째 부분에서는 [Hello Nextflow](../hello_nextflow/index.md) 초급자 과정에서 만든 파이프라인의 nf-core 호환 버전을 생성하는 방법을 보여드립니다.
...
!!! tip "팁"
    ...
    이 과정에서는 Hello Nextflow 입문 과정에서 다루지 않은 두 가지 중요한 Nextflow 메커니즘을 학습합니다: [meta maps](../side_quests/metadata/index.md)와 [workflows of workflows](../side_quests/workflows_of_workflows/index.md)입니다. 두 주제 모두 링크된 Side Quest에서 자세히 다룹니다.

    아래 지침에는 nf-core 맥락에서 이러한 메커니즘이 어떻게 사용되는지 이해하는 데 필요한 핵심 정보가 포함되어 있지만, 한 번에 많은 내용을 접하게 될 수 있습니다.
    시간이 있다면 두 Side Quest를 먼저 학습하는 것을 권장합니다(순서는 무관합니다):
...
nf-core 프로젝트는 파이프라인의 구조, 코드 구성, 설정, 문서화 방식에 대한 엄격한 가이드라인을 적용합니다.

파이프라인 생성 작업을 시작하기 전에 해당 구조와 구성 방식을 이해해야 합니다.
[시작하기](00_orientation.md) 단계에서 생성한 `pipelines` 심볼릭 링크를 사용하여 `nf-core/demo` 저장소에서 파이프라인 코드가 어떻게 구성되어 있는지 살펴보겠습니다.
...
지금은 파이프라인 코드 구성 요소(`main.nf`, `workflows`, `subworkflows`, `modules`)와 이들이 서로 어떻게 연관되는지에 집중하겠습니다.

### 1.1. nf-core 워크플로우의 모듈식 구조

표준 nf-core 파이프라인 코드 구성은 코드 재사용을 극대화하도록 설계된 모듈식 구조를 따릅니다. 이는 [Hello Nextflow](../hello_nextflow/04_hello_modules.md) 과정의 4부인 [Hello Modules](../hello_nextflow/index.md)에서 소개된 내용이지만, nf-core 방식에 맞게 약간의 추가적인 복잡성이 적용됩니다.
구체적으로, nf-core 파이프라인은 상위 workflow에서 가져오는 workflow 스크립트인 subworkflow를 적극적으로 활용합니다.

다소 추상적으로 들릴 수 있으므로, `nf-core/demo` 파이프라인에서 실제로 어떻게 사용되는지 살펴보겠습니다.

`main.nf` 파일을 열면 `workflows/demo.nf`에서 `DEMO`라는 workflow를 가져오는 것과 함께 일부 모듈 및 subworkflow를 가져오는 것을 볼 수 있습니다.

관련 코드 구성 요소 간의 관계는 다음과 같습니다:
...
!!! info "정보"
    ...
    여기서 소개하는 `nf-core/demo` 파이프라인은 비교적 단순한 편이지만, `nf-core/rnaseq`와 같은 다른 nf-core 파이프라인은 실제 분석에 관여하는 subworkflow를 활용합니다.

이제 이러한 구성 요소를 자세히 살펴보겠습니다.

### 1.2. 진입점 스크립트: `main.nf`

`main.nf` 스크립트는 `nextflow run nf-core/demo`를 실행할 때 Nextflow가 시작하는 진입점입니다.
즉, `nextflow run nf-core/demo`를 실행하면 Nextflow가 자동으로 `main.nf` 스크립트를 찾아 실행합니다.
이는 nf-core 파이프라인뿐만 아니라 이 관례적인 명명 및 구조를 따르는 모든 Nextflow 파이프라인에 적용됩니다.

진입점 스크립트를 사용하면 실제 분석 스크립트가 실행되기 전후에 표준화된 '관리' subworkflow를 쉽게 실행할 수 있습니다.
실제 분석 workflow와 모듈을 살펴본 후 이에 대해 설명하겠습니다.

### 1.3. 분석 스크립트: `workflows/demo.nf`

`workflows/demo.nf` workflow는 파이프라인의 핵심 로직이 저장되는 곳입니다.
일반적인 Nextflow workflow와 유사하게 구성되어 있지만, 상위 workflow에서 호출되도록 설계되어 있어 몇 가지 추가 기능이 필요합니다.
이 과정의 다음 부분에서 Hello Nextflow의 간단한 Hello 파이프라인을 nf-core 호환 형태로 변환할 때 관련 차이점을 다루겠습니다.
...
!!! info "정보"
    ...
    nf-core 웹사이트에서 사용 가능한 [nf-core subworkflows](https://nf-co.re/subworkflows/)를 탐색하여 몇 가지 예시를 확인할 수 있습니다.

    분석 스크립트가 subworkflow를 사용하는 경우, 해당 subworkflow는 `subworkflows/` 디렉토리 아래에 저장됩니다.

### 1.4. 모듈

모듈은 [Hello Nextflow 교육 과정 4부](../hello_nextflow/04_hello_modules.md)에서 설명한 것처럼 프로세스 코드가 위치하는 곳입니다.

nf-core 프로젝트에서 모듈은 출처와 내용을 모두 반영하는 다단계 내포 구조로 구성됩니다.
최상위 수준에서 모듈은 `nf-core` 또는 `local`(nf-core 프로젝트에 속하지 않음)로 구분되며, 이후 래핑하는 도구의 이름을 딴 디렉토리에 배치됩니다.
도구가 툴킷(여러 도구를 포함하는 패키지)에 속하는 경우 툴킷 이름을 딴 중간 디렉토리 수준이 있습니다.

`nf-core/demo` 파이프라인 모듈에 실제로 적용된 것을 확인할 수 있습니다:
...
`fastqc`와 `multiqc` 모듈은 `nf-core` 모듈 내 최상위 수준에 위치하는 반면, `trim` 모듈은 속한 툴킷인 `seqtk` 아래에 위치합니다.
이 경우 `local` 모듈은 없습니다.

프로세스를 설명하는 모듈 코드 파일은 항상 `main.nf`라고 불리며, 지금은 무시할 테스트 및 `.yml` 파일이 함께 제공됩니다.

진입점 workflow, 분석 workflow, 모듈을 합치면 파이프라인의 '핵심' 부분을 실행하기에 충분합니다.
그러나 관리 subworkflow도 있으므로 이제 살펴보겠습니다.

### 1.5. 관리 subworkflow

모듈과 마찬가지로 subworkflow는 `local`과 `nf-core` 디렉토리로 구분되며, 각 subworkflow는 자체 `main.nf` 스크립트, 테스트 및 `.yml` 파일이 있는 내포된 디렉토리 구조를 가집니다.
...
위에서 언급했듯이, `nf-core/demo` 파이프라인에는 분석 특화 subworkflow가 포함되어 있지 않으므로, 여기서 볼 수 있는 모든 subworkflow는 이름의 `utils_` 접두사로 표시된 소위 '관리' 또는 '유틸리티' workflow입니다.
이러한 subworkflow는 콘솔 출력에서 멋진 nf-core 헤더를 생성하는 것을 비롯한 다양한 부속 기능을 담당합니다.

!!! tip "팁"

    명명 패턴 외에도, 이러한 subworkflow가 실제 분석 관련 기능을 수행하지 않는다는 또 다른 표시는 프로세스를 전혀 호출하지 않는다는 것입니다.

이것으로 `nf-core/demo` 파이프라인을 구성하는 핵심 코드 구성 요소에 대한 개요가 완료됩니다.

### 핵심 정리

nf-core 파이프라인의 모듈식 구조에 대한 높은 수준의 이해를 갖추게 되었습니다.

### 다음 단계

nf-core 도구를 사용하여 파이프라인 스캐폴드를 생성합니다.

---

## 2. 새 파이프라인 프로젝트 생성

살펴본 것처럼, nf-core 파이프라인은 많은 부속 파일과 함께 표준화된 구조를 따릅니다.
이 모든 것을 처음부터 만드는 것은 매우 지루할 것이므로, nf-core 커뮤니티는 프로세스를 시작하기 위해 템플릿에서 이를 수행하는 도구를 개발했습니다.

### 2.1. 템플릿 기반 파이프라인 생성 도구 실행
...
TUI가 완료되면 도구가 파이프라인을 생성하고, 컨테이너 설정을 생성했으며, 새 파이프라인의 로컬 git 저장소를 초기화했다고 보고합니다:
...
정말 많은 파일입니다!
아직 조금 헷갈리더라도 걱정하지 마세요. 중요한 부분들을 곧 살펴보고, 이후 과정에서 단계적으로 다루겠습니다.

전반적으로 `nf-core/demo` 파이프라인에서 관찰한 코드 구조와 유사하지만, 여기에는 `modules` 디렉토리가 없습니다.

### 2.2. 스캐폴드가 작동하는지 테스트

믿기 어렵겠지만, 실제 작업을 수행할 모듈을 아직 추가하지 않았음에도 불구하고, 파이프라인 스캐폴드는 `nf-core/demo` 파이프라인을 실행했던 것과 동일한 방식으로 test 프로필을 사용하여 실행할 수 있습니다.
...
`nf-core/demo` 파이프라인의 구조를 기억하신다면, `DEMO` workflow를 래핑하는 진입점 workflow를 포함하는 `main.nf` 파일이 있었습니다.
새로 생성한 프로젝트의 `main.nf` 파일을 열면 `workflows/hello.nf`에서 `HELLO`라는 workflow를 가져오는 것을 볼 수 있습니다.
이는 `DEMO` workflow와 직접적으로 동등하지만, 현재는 플레이스홀더에 불과합니다.

따라서 파이프라인 스캐폴드의 전체 구조는 다음과 같습니다:

<figure class="excalidraw">
--8<-- "docs/en/docs/nfcore_build/img/core-hello-initial.svg"
</figure>

`nf-core/demo` 파이프라인 구조가 떠오르실 것입니다!
유일한 실질적인 차이점은 `DEMO` workflow에는 모듈의 프로세스가 포함되어 있었다는 것입니다.
여기서 동등한 `HELLO` workflow에는 아직 프로세스가 포함되어 있지 않습니다.

좀 더 자세히 살펴보겠습니다.

### 2.4. 플레이스홀더 workflow 검사
...
??? note "`channel.topic` 블록"
    ...
    이 과정의 3부에서 작동 방식을 자세히 설명합니다.

관심 있는 workflow의 관련 로직을 해당 구조에 연결해야 합니다.

### 핵심 정리

nf-core 도구를 사용하여 파이프라인 스캐폴드를 생성하고 데모 파이프라인 구조와 비교하는 방법을 알게 되었습니다.
...
## 3. 구성 가능한 Hello Nextflow workflow 만들기
...
해당 workflow는 단독으로 실행할 수 있는 간단한 이름 없는 workflow로 작성되었습니다.

nf-core 스캐폴드에서 원래 workflow의 어떤 부분이 어디에 들어가야 하는지 명확하게 파악하기 위해, 먼저 원래 Hello workflow를 nf-core 템플릿이 요구하는 대로 상위 workflow 내에서 실행 가능한 **구성 가능한** workflow로 변환하겠습니다.
...
기본적으로 nf-core 스캐폴드의 모듈식 구조를 모방하되, 처음에는 복잡성을 줄이는 것이 목표입니다.
...
구성 가능하게 만들기 위해 다음과 같은 변경을 수행합니다:

1. workflow에 이름 지정
2. 채널 구성을 `take:`로 교체
3. workflow 작업 앞에 `main:` 추가
4. `emit:` 문 추가

필요한 변경 사항을 하나씩 살펴보겠습니다.

#### 3.1.1. workflow에 이름 지정
...
#### 3.1.3. workflow 작업 앞에 `main` 문 추가
...
## 4. 업데이트된 workflow 로직을 플레이스홀더 workflow에 맞추기
...
nf-core 프로젝트는 내포된 subworkflow를 많이 사용하므로 이 부분은 처음 접근할 때 약간 혼란스러울 수 있습니다.
...
이 코드에는 nf-core 파이프라인 템플릿에 포함된 예제 samplesheet에 매우 특정적인 일부 파싱 및 검증 단계가 포함되어 있으며, 작성 당시 이는 매우 도메인별로 특정적이어서 우리의 간단한 파이프라인 프로젝트에 적합하지 않습니다.
...
잠시 휴식을 취하세요, 힘든 작업이었습니다! 준비가 되면 [2부: nf-core 모듈 사용하기](./02_use_module.md)로 넘어가서 nf-core/modules 저장소에서 커뮤니티가 관리하는 모듈을 활용하는 방법을 배우세요.
