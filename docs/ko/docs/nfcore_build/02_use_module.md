# 파트 2: nf-core 모듈 사용하기

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } AI 지원 번역 - [자세히 알아보기 및 개선 제안](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>


Build with nf-core 교육 과정의 두 번째 파트에서는 기존 nf-core 모듈을 파이프라인에서 찾고, 설치하고, 사용하는 방법을 다룹니다.
...
이것이 어떻게 작동하는지 보여드리기 위해, `core-hello` 파이프라인에서 사용자 정의 `collectGreetings` 모듈을 nf-core/modules의 `find/concatenate` 모듈로 교체하겠습니다.

??? info "이 섹션을 시작하는 방법"

    이 과정 섹션은 [파트 1: nf-core용 Hello 재작성하기](./01_rewrite_hello.md)를 완료했고 작동하는 `core-hello` 파이프라인이 있다고 가정합니다.

    파트 1을 완료하지 않았거나 이 파트를 새로 시작하고 싶다면, `core-hello-part1` 솔루션을 시작점으로 사용할 수 있습니다.
    `nfcore-build/` 디렉토리 내에서 이 명령을 실행하세요:
...
`find_concatenate` 모듈 박스를 클릭하여 모듈 문서를 확인하세요.

모듈 페이지는 다음을 보여줍니다:

- 간단한 설명: "A module for concatenation of gzipped or uncompressed files getting around UNIX terminal argument size"
- 설치 명령: `nf-core modules install find/concatenate`
- 입력 및 출력 채널 구조
- 사용 가능한 매개변수
...
`grep` 접근 방식은 이름에 검색어가 있는 결과만 추출한다는 점을 명심하세요.
...
`workflows/hello.nf` 워크플로우의 임포트 섹션에서 `collectGreetings` 모듈에 대한 `include` 문을 `FIND_CONCATENATE`에 대한 것으로 교체합니다.
...
- **nf-core 모듈**: `'../modules/nf-core/find/concatenate/main'` (`main.nf` 참조)
- **로컬 모듈**: `'../modules/local/collectGreetings.nf'` (단일 파일 참조)

이제 모듈이 워크플로우에서 사용 가능하므로, `collectGreetings` 호출을 `FIND_CONCATENATE`를 사용하도록 교체하기만 하면 됩니다. 그렇죠?
...
이에 비해 `find/concatenate` 모듈의 인터페이스는 더 복잡합니다:
...
버전 출력을 무시하면, 주요 출력도 동등합니다(연결된 파일). 다만 FIND_CONCATENATE는 출력 파일과 함께 metamap도 방출합니다.
...
방금 FIND_CONCATENATE 모듈이 입력 튜플의 일부로 메타데이터 맵을 기대한다고 말씀드렸습니다.
...
이제 이것을 입력 파일이 metamap과 함께 튜플로 패키징되기를 기대하고 출력 튜플의 일부로도 metamap을 출력하는 `FIND_CONCATENATE` 프로세스의 컨텍스트에 넣어봅시다.
...
`FIND_CONCATENATE`에 의해 출력된 파일이 메타데이터의 일부인 식별자를 기반으로 이름이 지정될 것이라고 말씀드린 것을 기억하시나요?
이것이 관련 코드입니다:
...
검토한 내용을 바탕으로 `find/concatenate` 모듈을 활용하기 위해 파이프라인에 수행해야 할 주요 변경 사항은 다음과 같습니다:

- 배치 이름을 포함하는 metamap 생성;
- metamap을 연결할 입력 파일 세트(`convertToUpper`에서 나오는)와 함께 튜플로 패키징;
- `collectGreetings()`에서 `FIND_CONCATENATE`로 호출 전환;
- `cowpy`에 전달하기 전에 `FIND_CONCATENATE` 프로세스에서 생성된 튜플에서 출력 파일 추출.
...
## 3. FIND_CONCATENATE를 `hello.nf` 워크플로우에 통합하기
...
먼저, `FIND_CONCATENATE`에 대한 메타데이터 맵을 생성해야 하며, nf-core 모듈이 최소한 `id` 필드를 포함하는 metamap을 요구한다는 점을 명심해야 합니다.
...
`convertToUpper` 호출 후에 이 줄들을 추가하고 `collectGreetings` 호출을 제거합니다:
...
- `#!groovy .map { files -> tuple(cat_meta, files) }`는 `FIND_CONCATENATE`가 기대하는 형식인 `[metadata, files]`의 튜플을 생성합니다

이것이 `FIND_CONCATENATE`에 대한 입력 튜플을 설정하기 위해 수행해야 하는 전부입니다.

### 3.3. FIND_CONCATENATE 모듈 호출하기

이제 새로 생성된 채널에 대해 `FIND_CONCATENATE`를 호출합니다:
...
그러나 `FIND_CONCATENATE` 프로세스는 출력 파일 외에 metamap을 포함하는 튜플을 생성합니다.

`cowpy`가 아직 메타데이터 튜플을 받아들이지 않으므로(과정의 다음 파트에서 수정하겠습니다), `FIND_CONCATENATE`에서 생성된 튜플에서 출력 파일을 추출한 후 `cowpy`에 전달해야 합니다:
...
`#!groovy .map { meta, file -> file }` 연산은 `FIND_CONCATENATE`에서 생성된 `[metadata, file]` 튜플에서 파일을 새 채널인 `ch_for_cowpy`로 추출합니다.
...
이것으로 완료되었습니다! 이제 파이프라인의 해당 단계에 대해 사용자 정의 프로토타입 등급 코드 대신 강력한 커뮤니티 큐레이션 모듈을 사용하고 있습니다.
