...
기본 구성 가능한 workflow를 nf-core 스캐폴드에 접목하는 방법을 배웁니다.
...
=== "변경 전"
    ...
    params {
        ...
        // 입력 데이터
        // TODO nf-core: nf-core/test-datasets에서 테스트 데이터 경로 지정
...
- **`${projectDir}` 사용**: 이는 메인 workflow 스크립트가 위치한 디렉토리(파이프라인 루트)를 가리키는 Nextflow 암시적 변수입니다. 이를 사용하면 파이프라인이 어디에서 실행되든 경로가 작동합니다.
- **절대 경로**: `${projectDir}`를 사용하여 절대 경로를 생성하며, 이는 파이프라인과 함께 제공되는 테스트 데이터에 중요합니다.
- **테스트 데이터 위치**: nf-core 파이프라인은 일반적으로 작은 테스트 파일의 경우 파이프라인 저장소 내 `assets/` 디렉토리에 테스트 데이터를 저장하거나 더 큰 파일의 경우 외부 테스트 데이터 세트를 참조합니다.

그리고 이 기회에 매우 기본적인 머신(Github Codespaces의 최소 VM과 같은)에서 실행될 수 있도록 기본 리소스 제한을 조정하겠습니다:
...
보시다시피 초기화 subworkflow 덕분에 시작 부분에 일반적인 nf-core 요약이 생성되었으며, 각 모듈의 줄은 이제 전체 PIPELINE:WORKFLOW:module 이름을 표시합니다.
...
앞서 기억하시겠지만, 새로 생성된 workflow의 첫 번째 실행은 다양한 실행 보고서와 메타데이터를 포함하는 `core-hello-results/`라는 디렉토리를 생성했습니다.
...
workflow가 여전히 플레이스홀더였던 첫 번째 실행에서 얻은 것 외에 또 다른 실행 보고서 세트를 얻었습니다.
이번에는 예상대로 실행된 모든 작업을 볼 수 있습니다.
...
!!! note "참고"
    ...
    이들이 병렬로 실행되는 것을 보려면 codespace의 CPU 할당과 테스트 구성의 리소스 제한을 늘려보세요.
...
demo 파이프라인의 출력처럼 깔끔하게 정리되기를 원한다면 출력이 게시되도록 설정하는 방법을 변경해야 합니다.
이 교육 과정의 뒷부분에서 그 방법을 보여드리겠습니다.

<!-- TODO: Update this once we've updated Hello Nextflow to use workflow-level outputs -->

그리고 완성입니다! 원래 파이프라인과 동일한 결과를 얻기 위해 많은 작업처럼 보일 수 있지만, 이러한 멋진 보고서가 자동으로 생성되고, 이제 입력 검증 및 나중 섹션에서 다룰 몇 가지 깔끔한 메타데이터 처리 기능을 포함하여 nf-core의 추가 기능을 활용할 수 있는 견고한 기반을 갖추게 되었습니다.
...
그 과정에서 workflow를 구성 가능하게 만드는 방법과 커스텀 nf-core 스타일 파이프라인을 개발할 때 가장 일반적으로 조정해야 하는 nf-core 템플릿의 요소를 식별하는 방법을 배웠습니다.

### 다음 단계

휴식을 취하세요. 힘든 작업이었습니다! 준비가 되면 [3부: nf-core 모듈 사용](./03_use_module.md)으로 이동하여 nf-core/modules 저장소의 커뮤니티 유지 관리 모듈을 활용하는 방법을 배우세요.
