...
=== "전"
    ...
    ```groovy title="core-hello/modules/local/cowpy.nf (발췌)" linenums="1" hl_lines="4"
    // cowpy로 ASCII 아트 생성 (https://github.com/jeffbuttars/cowpy)
    process COWPY {
...
모듈은 `topic:` 키워드를 사용하여 출력 블록에서 직접 버전을 튜플로 발행하고, 워크플로는 한 번 구독하여 모두 수집합니다.
...
워크플로 측 수집 — Part 2의 플레이스홀더 워크플로에서 보았던 `Channel.topic("versions")` 블록 — 이 topic을 구독하고 이 통합 보고서를 자동으로 작성합니다.

!!! note "하위 호환성"

    워크플로의 topic 채널 블록에 있는 `versions_file` 브랜치는 아직 `topic: versions`를 사용하도록 업데이트되지 않고 `emit: versions`와 함께 script 블록에서 `versions.yml` 파일을 작성하는 모듈을 처리하기 위해 존재합니다.
