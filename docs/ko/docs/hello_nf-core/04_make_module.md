...
??? info "이 섹션을 시작하는 방법"
    ...
    이 섹션은 [Part 3: nf-core 모듈 사용하기](./03_use_module.md)를 완료하고 `FIND_CONCATENATE` 모듈을 파이프라인에 통합했다고 가정합니다.
...
워크플로우 측 수집 — Part 2의 플레이스홀더 워크플로우에서 보았던 `channel.topic("versions")` 블록 — 이 topic을 구독하고 이 통합 보고서를 자동으로 작성합니다.
