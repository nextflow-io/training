...
`workflows/hello.nf` 워크플로우의 임포트 섹션에서 `collectGreetings` 모듈에 대한 `include` 문을 `CAT_CAT`에 대한 것으로 교체합니다.
...
이제 모듈이 워크플로우에서 사용 가능하므로, `collectGreetings` 호출을 `CAT_CAT`를 사용하도록 교체하기만 하면 됩니다. 그렇죠?
...
- 소프트웨어 버전 추적을 위해 `versions` 토픽 채널에 게시되는 버전 튜플.
...
버전 출력을 무시하면, 주요 출력도 동등합니다(연결된 파일). 다만 CAT_CAT는 출력 파일과 함께 metamap도 방출합니다.
...
!!! note "메타데이터에 대해 자세히 알아보기"
    ...
    샘플시트에서 메타데이터를 읽고 처리를 사용자 정의하는 데 사용하는 방법을 포함하여 Nextflow 워크플로우에서 메타데이터를 사용하는 방법에 대한 포괄적인 소개는 [워크플로우의 메타데이터](../side_quests/metadata) 사이드 퀘스트를 참조하세요.

### 2.3. 변경할 사항 요약하기

검토한 내용을 바탕으로 `cat/cat` 모듈을 활용하기 위해 파이프라인에 수행해야 할 주요 변경 사항은 다음과 같습니다:
...
## 3. CAT_CAT를 `hello.nf` 워크플로우에 통합하기
...
먼저, `CAT_CAT`에 대한 메타데이터 맵을 생성해야 하며, nf-core 모듈이 최소한 `id` 필드를 포함하는 metamap을 요구한다는 점을 명심해야 합니다.
...
- `#!groovy .map { files -> tuple(cat_meta, files) }`는 `CAT_CAT`가 기대하는 형식인 `[metadata, files]`의 튜플을 생성합니다
...
`#!groovy .map { meta, file -> file }` 연산은 `CAT_CAT`에서 생성된 `[metadata, file]` 튜플에서 파일을 새 채널인 `ch_for_cowpy`로 추출합니다.
...
새로 통합된 `cat/cat` 모듈로 워크플로우가 작동하는지 테스트합니다:
