...
이 과정은 정보를 점진적으로 소개하도록 구성된 목표 지향적 연습과 함께 실습 중심으로 설계되었습니다.
...
| 과정 챕터                                               | 요약                                                                                                                            | 예상 소요 시간 |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| [Part 1: 데모 파이프라인 실행](./01_run_demo.md)        | 기존 nf-core 파이프라인을 실행하고 코드 구조를 검토하여 이러한 파이프라인이 기본 Nextflow 워크플로우와 무엇이 다른지 파악합니다 | 30분           |
| [Part 2: nf-core용 Hello 재작성](./02_rewrite_hello.md) | [Hello Nextflow](../hello_nextflow/index.md) 과정에서 생성한 간단한 워크플로우를 nf-core 템플릿 스캐폴드에 맞게 적응시킵니다    | 60분           |
| [Part 3: nf-core 모듈 사용](./03_use_module.md)         | 커뮤니티 모듈 라이브러리를 탐색하고 일반적인 생물정보학 도구를 래핑하는 사전 구축되고 테스트된 모듈을 통합하는 방법을 배웁니다  | 30분           |
| [Part 4: nf-core 모듈 만들기](./04_make_module.md)      | nf-core가 정한 특정 구조, 명명 규칙 및 메타데이터 요구사항을 사용하여 자신만의 nf-core 스타일 모듈을 생성합니다                 | 30분           |
| [Part 5: 입력 검증 추가](./05_input_validation.md)      | nf-schema를 사용하여 명령줄 매개변수와 입력 데이터 파일 모두에 대한 입력 검증을 구현합니다                                      | 30분           |
