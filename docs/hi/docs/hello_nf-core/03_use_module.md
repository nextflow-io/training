...
- एक version tuple जो software version tracking के लिए `versions` topic channel में publish किया जाता है।
...
अभी के लिए versions आउटपुट को ignore करते हुए, उनका मुख्य output भी समतुल्य है (एक concatenated फ़ाइल), सिवाय CAT_CAT output फ़ाइल के संयोजन में metamap को भी emit करता है।
...
सबसे पहले, हमें `CAT_CAT` के लिए एक metadata map बनाने की आवश्यकता है, यह ध्यान में रखते हुए कि nf-core मॉड्यूल को metamap में कम से कम एक `id` field होना आवश्यक है।
...
- `#!groovy .map { files -> tuple(cat_meta, files) }` `[metadata, files]` के format में एक tuple बनाता है जो `CAT_CAT` अपेक्षा करता है
...
`#!groovy .map { meta, file -> file }` operation `CAT_CAT` द्वारा निर्मित `[metadata, file]` tuple से file को एक नए channel, `ch_for_cowpy`, में extract करता है।
