...
इसके वर्तमान रूप में, यह हमें schema validation के लिए nf-core की built-in क्षमताओं का लाभ उठाने नहीं देगा, लेकिन हम इसे बाद में जोड़ सकते हैं।
अभी के लिए, हम इसे यथासंभव सरल रखने पर ध्यान केंद्रित कर रहे हैं ताकि हम test डेटा पर सफलतापूर्वक कुछ चला सकें।

### 4.3. Test profile को अपडेट करें

Test डेटा और पैरामीटर की बात करें तो, आइए इस pipeline के लिए test profile को अपडेट करें ताकि template में प्रदान किए गए उदाहरण samplesheet के बजाय `greetings.csv` mini-samplesheet का उपयोग किया जा सके।

`core-hello/conf` के तहत, हमें दो templated test profiles मिलते हैं: `test.config` और `test_full.config`, जो एक छोटे डेटा sample और एक full-size one का परीक्षण करने के लिए हैं।
...
हम `test.config` को हमारी `greetings.csv` फ़ाइल पर कुछ डिफ़ॉल्ट पैरामीटर के साथ चलाने के लिए सेट अप करने पर ध्यान केंद्रित करने जा रहे हैं।

#### 4.3.1. `greetings.csv` फ़ाइल को copy करें

सबसे पहले हमें `greetings.csv` फ़ाइल को अपने pipeline project में एक उपयुक्त स्थान पर copy करने की आवश्यकता है।
आमतौर पर छोटी test फ़ाइलें `assets` डायरेक्टरी में संग्रहीत की जाती हैं, इसलिए आइए फ़ाइल को अपनी working डायरेक्टरी से copy करें।

```bash
cp greetings.csv core-hello/assets/.
```

अब `greetings.csv` फ़ाइल test इनपुट के रूप में उपयोग करने के लिए तैयार है।
...
- **`${projectDir}` का उपयोग**: यह एक Nextflow implicit variable है जो उस डायरेक्टरी को इंगित करता है जहां मुख्य workflow script स्थित है (pipeline root)। इसका उपयोग करने से यह सुनिश्चित होता है कि path काम करता है चाहे pipeline कहीं से भी चलाया जाए।
- **Absolute paths**: `${projectDir}` का उपयोग करके, हम एक absolute path बनाते हैं, जो pipeline के साथ ship होने वाले test डेटा के लिए महत्वपूर्ण है।
- **Test डेटा स्थान**: nf-core pipelines आमतौर पर छोटी test फ़ाइलों के लिए pipeline repository के भीतर `assets/` डायरेक्टरी में test डेटा संग्रहीत करते हैं, या बड़ी फ़ाइलों के लिए external test datasets को संदर्भित करते हैं।

और जब हम इस पर हैं, तो आइए डिफ़ॉल्ट resource limits को कस दें ताकि यह सुनिश्चित हो सके कि यह बहुत बुनियादी machines (जैसे Github Codespaces में minimal VMs) पर चलेगा:
...
यह उन कोड संशोधनों को पूरा करता है जो हमें करने की आवश्यकता है।

### 4.4. Test profile के साथ pipeline चलाएं

यह बहुत कुछ था, लेकिन हम अंत में pipeline चलाने की कोशिश कर सकते हैं!
ध्यान दें कि हमें कमांड लाइन में `--validate_params false` जोड़ना होगा क्योंकि हमने अभी तक validation सेट अप नहीं किया है (वह बाद में आएगा)।

```bash
nextflow run core-hello --outdir core-hello-results -profile test,docker --validate_params false
```

यदि आपने सभी संशोधन सही तरीके से किए हैं, तो यह पूरा होने तक चलना चाहिए।
...
जैसा कि आप देख सकते हैं, इसने initialisation subworkflow के लिए धन्यवाद शुरुआत में typical nf-core summary तैयार किया, और प्रत्येक module के लिए लाइनें अब पूर्ण PIPELINE:WORKFLOW:module नाम दिखाती हैं।

### 4.5. Pipeline outputs खोजें

अब सवाल यह है: pipeline के outputs कहाँ हैं?
और जवाब काफी दिलचस्प है: परिणामों को देखने के लिए अब दो अलग-अलग स्थान हैं।

जैसा कि आपको पहले से याद होगा, हमारे नए बनाए गए workflow के पहले run ने `core-hello-results/` नाम की एक डायरेक्टरी तैयार की जिसमें विभिन्न execution reports और metadata शामिल थे।
...
आप देखते हैं कि हमें पहले run से मिले execution reports के अलावा एक और सेट मिला, जब workflow अभी भी सिर्फ एक placeholder था।
इस बार आप सभी tasks देखते हैं जो अपेक्षित रूप से चलाए गए थे।

![execution timeline report for the Hello pipeline](./img/execution_timeline_hello.png)

!!! note "नोट"

    एक बार फिर tasks parallel में नहीं चलाए गए क्योंकि हम Github Codespaces में एक minimalist machine पर चल रहे हैं।
    इन्हें parallel में चलते हुए देखने के लिए, अपने codespace के CPU allocation और test configuration में resource limits को बढ़ाने का प्रयास करें।

यह बहुत अच्छा है, लेकिन हमारे वास्तविक pipeline परिणाम वहां नहीं हैं!

यहाँ क्या हुआ: हमने modules को स्वयं कुछ भी नहीं बदला, इसलिए module-level `publishDir` directives द्वारा handle किए गए outputs अभी भी एक `results` डायरेक्टरी में जा रहे हैं जैसा कि मूल pipeline में निर्दिष्ट किया गया था।
...
आह, वे वहाँ हैं, मूल Hello pipeline के पहले के runs के outputs के साथ मिश्रित।

यदि हम चाहते हैं कि वे demo pipeline के outputs की तरह सुव्यवस्थित रूप से व्यवस्थित हों, तो हमें यह बदलने की आवश्यकता होगी कि हम outputs को publish करने के लिए कैसे सेट अप करते हैं।
हम आपको बाद में इस प्रशिक्षण पाठ्यक्रम में दिखाएंगे कि यह कैसे करना है।

<!-- TODO: Update this once we've updated Hello Nextflow to use workflow-level outputs -->

और यह रहा! मूल pipeline के समान परिणाम प्राप्त करने के लिए बहुत काम लग सकता है, लेकिन आपको वे सभी सुंदर reports स्वचालित रूप से generate होती हैं, और अब आपके पास nf-core की अतिरिक्त सुविधाओं का लाभ उठाने के लिए एक ठोस नींव है, जिसमें इनपुट validation और कुछ साफ metadata handling क्षमताएं शामिल हैं जिन्हें हम बाद के section में कवर करेंगे।

---

### निष्कर्ष

आप जानते हैं कि nf-core template का उपयोग करके एक नियमित Nextflow pipeline को nf-core style pipeline में कैसे convert किया जाए।
इसके हिस्से के रूप में, आपने सीखा कि एक workflow को composable कैसे बनाया जाए, और nf-core template के उन elements की पहचान कैसे की जाए जिन्हें आमतौर पर एक custom nf-core style pipeline विकसित करते समय अनुकूलित करने की आवश्यकता होती है।

### आगे क्या है?

एक break लें, यह कठिन काम था! जब आप तैयार हों, तो [Part 3: Use an nf-core module](./03_use_module.md) पर जाएं ताकि nf-core/modules repository से community-maintained modules का लाभ उठाना सीख सकें।
