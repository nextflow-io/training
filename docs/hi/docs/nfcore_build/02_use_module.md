# भाग 2: एक nf-core मॉड्यूल का उपयोग करें

<span class="ai-translation-notice">:material-information-outline:{ .ai-translation-notice-icon } AI-सहायता प्राप्त अनुवाद - [अधिक जानें और सुधार सुझाएं](https://github.com/nextflow-io/training/blob/master/TRANSLATING.md)</span>


Build with nf-core प्रशिक्षण पाठ्यक्रम के इस दूसरे भाग में, हम आपको दिखाते हैं कि अपनी pipeline में मौजूदा nf-core मॉड्यूल को कैसे खोजें, इंस्टॉल करें, और उपयोग करें।
...
यह कैसे काम करता है यह दिखाने के लिए, हम `core-hello` pipeline में nf-core/modules से `find/concatenate` मॉड्यूल के साथ कस्टम `collectGreetings` मॉड्यूल को बदलेंगे।

??? info "इस खंड से कैसे शुरू करें"

    पाठ्यक्रम का यह सेक्शन मानता है कि आपने [भाग 1: nf-core के लिए Hello को फिर से लिखें](./01_rewrite_hello.md) पूरा कर लिया है और आपके पास एक कार्यशील `core-hello` pipeline है।

    यदि आपने भाग 1 पूरा नहीं किया है या इस भाग के लिए नए सिरे से शुरू करना चाहते हैं, तो आप अपने शुरुआती बिंदु के रूप में `core-hello-part1` समाधान का उपयोग कर सकते हैं।
    `nfcore-build/` डायरेक्टरी के अंदर से यह कमांड चलाएं:
...
उनमें से, आपको `find_concatenate` नामक एक सामान्य-उद्देश्य मॉड्यूल दिखाई देना चाहिए।

!!! info "मॉड्यूल नामकरण परंपरा"
...
मॉड्यूल दस्तावेज़ देखने के लिए `find_concatenate` मॉड्यूल बॉक्स पर क्लिक करें।

मॉड्यूल पेज दिखाता है:

- एक संक्षिप्त विवरण: "A module for concatenation of gzipped or uncompressed files getting around UNIX terminal argument size"
- इंस्टॉलेशन कमांड: `nf-core modules install find/concatenate`
...
बस ध्यान रखें कि `grep` दृष्टिकोण केवल उन परिणामों को निकालेगा जिनके नाम में खोज शब्द है।
...
`INFO Reinstalling modules found in 'modules.json' but missing from directory` संदेश को आप ignore कर सकते हैं; यह nf-core/tools 4.0.2 द्वारा किसी भी मॉड्यूल के लिए emit किया जाता है जिसे आप `info` से query करते हैं, चाहे वह वास्तव में इंस्टॉल हो या नहीं, और इसका कोई प्रभाव नहीं पड़ता क्योंकि `info` कमांड कोई फ़ाइल नहीं लिखता।

### 1.4. find/concatenate मॉड्यूल इंस्टॉल करें
...
सुनिश्चित करें कि आपकी working डायरेक्टरी `core-hello` pipeline प्रोजेक्ट की root है, फिर इंस्टॉलेशन कमांड चलाएं:
...
कमांड मॉड्यूल फ़ाइलों को `modules/nf-core/find/concatenate/` में डाउनलोड करता है और इंस्टॉल किए गए मॉड्यूल को ट्रैक करने के लिए `modules.json` को अपडेट करता है।
अंत में दिखने वाले `NotADirectoryError` को आप ignore कर सकते हैं; यह इसलिए होता है क्योंकि nf-core/tools 4.0.2 अपेक्षा करता है कि हर local मॉड्यूल अपनी खुद की डायरेक्टरी (`modules/local/<name>/main.nf`) में हो, जबकि `core-hello` अभी भी इस चरण में single-file local मॉड्यूल का उपयोग करता है।
हालाँकि, `find/concatenate` मॉड्यूल सही तरीके से इंस्टॉल हो जाता है, और `modules.json` अपेक्षा के अनुसार अपडेट हो जाता है।
हम भाग 4 में `cowpy` को directory layout में convert करेंगे।

आइए जांचें कि मॉड्यूल फ़ाइलें सही जगह पर हैं:
...
आप `modules.json` की जांच करके भी इंस्टॉलेशन की पुष्टि कर सकते हैं, जो अब nf-core/modules रिपॉजिटरी के अंतर्गत `find/concatenate` को सूचीबद्ध करता है।
...
यह पुष्टि करता है कि `find/concatenate` मॉड्यूल अब आपके प्रोजेक्ट के source code का हिस्सा है।
हालाँकि, वास्तव में नए मॉड्यूल का उपयोग करने के लिए, हमें इसे अपनी pipeline में import करना होगा।

अंत में, आप अपनी pipeline में वर्तमान में track किए जा रहे मॉड्यूल की जांच करने के लिए `nf-core modules list local` कमांड का भी उपयोग कर सकते हैं।
...
यह परिणाम तालिका में `find/concatenate` को उसके repository, version SHA, message, और date के साथ दिखाता है।

### 1.5. मॉड्यूल imports को अपडेट करें

आइए `workflows/hello.nf` workflow के imports सेक्शन में `collectGreetings` मॉड्यूल के लिए `include` statement को `FIND_CONCATENATE` के लिए वाले से बदलें।
...
- **nf-core मॉड्यूल**: `'../modules/nf-core/find/concatenate/main'` (`main.nf` को संदर्भित करता है)
- **Local मॉड्यूल**: `'../modules/local/collectGreetings.nf'` (एकल फ़ाइल संदर्भ)

मॉड्यूल अब workflow के लिए उपलब्ध है, इसलिए हमें बस `collectGreetings` की call को `FIND_CONCATENATE` का उपयोग करने के लिए swap करना है। सही?
...
!!! info "जानकारी"
    ...
    FIND_CONCATENATE process में विभिन्न compression प्रकारों, फ़ाइल extensions इत्यादि की कुछ चतुर handling शामिल है जो सख्ती से हम आपको यहां दिखाने की कोशिश कर रहे हैं उससे संबंधित नहीं हैं, इसलिए हम इसमें से अधिकांश को ignore करेंगे और केवल उन हिस्सों पर ध्यान केंद्रित करेंगे जो महत्वपूर्ण हैं।
...
तुलना में, `find/concatenate` मॉड्यूल का interface अधिक जटिल है:

```groovy title="modules/nf-core/find/concatenate/main.nf (अंश)" linenums="1" hl_lines="11 14"
...
FIND_CONCATENATE मॉड्यूल एक एकल input लेता है, लेकिन वह input एक tuple है जिसमें दो चीजें हैं:
...
पूर्ण होने पर, FIND_CONCATENATE अपने outputs दो भागों में देता है:

- एक tuple जिसमें metamap और concatenated output फ़ाइल होती है, `file_out` tag के साथ emit की गई;
- तीन version tuples (प्रत्येक उपयोग किए गए tool के लिए एक) जो software version tracking के लिए `versions` topic channel में publish किए जाते हैं।
...
अभी के लिए versions आउटपुट को ignore करते हुए, उनका मुख्य output भी समतुल्य है (एक concatenated फ़ाइल), सिवाय FIND_CONCATENATE output फ़ाइल के संयोजन में metamap को भी emit करता है।
...
हमने अभी आपको बताया कि FIND_CONCATENATE मॉड्यूल अपने input tuple के हिस्से के रूप में एक metadata map की अपेक्षा करता है।
...
अब आइए इसे `FIND_CONCATENATE` process के संदर्भ में रखें, जो input फ़ाइलों को metamap के साथ tuple में package किए जाने की अपेक्षा करता है, और output tuple के हिस्से के रूप में metamap को भी आउटपुट करता है।

```groovy title="modules/nf-core/find/concatenate/main.nf (अंश)" linenums="10" hl_lines="2 5"
input:
tuple val(meta), path(files_in)
...
याद है कि कैसे हमने आपको बताया कि `FIND_CONCATENATE` द्वारा आउटपुट की गई फ़ाइल का नाम metadata का हिस्सा है एक identifier के आधार पर होगा?
यह संबंधित code है:

```groovy title="modules/nf-core/find/concatenate/main.nf (अंश)" linenums="37"
...
!!! info "Metadata के बारे में अधिक जानें"

    Nextflow workflows में metadata के साथ काम करने के लिए एक व्यापक परिचय के लिए, जिसमें samplesheets से metadata को कैसे पढ़ें और processing को customize करने के लिए इसका उपयोग कैसे करें, [workflows में Metadata](../side_quests/metadata/index.md) side quest देखें।

### 2.3. किए जाने वाले परिवर्तनों का सारांश दें

जो हमने समीक्षा की है उसके आधार पर, ये वे प्रमुख परिवर्तन हैं जो हमें `find/concatenate` मॉड्यूल का उपयोग करने के लिए अपनी pipeline में करने की आवश्यकता है:
...
- `collectGreetings()` से `FIND_CONCATENATE` में call switch करें;
- `FIND_CONCATENATE` process द्वारा निर्मित tuple से output फ़ाइल को `cowpy` में पास करने से पहले extract करें।
...
## 3. `hello.nf` workflow में FIND_CONCATENATE को integrate करें
...
सबसे पहले, हमें `FIND_CONCATENATE` के लिए एक metadata map बनाने की आवश्यकता है, यह ध्यान में रखते हुए कि nf-core मॉड्यूल को metamap में कम से कम एक `id` field होना आवश्यक है।
...
=== "बाद में"
    ...
    ```groovy title="core-hello/workflows/hello.nf" linenums="29" hl_lines="7-8"
        // अभिवादन emit करें (nf-core samplesheet परंपरा का उपयोग करने के लिए अपडेट किया गया)
...
- `#!groovy .map { files -> tuple(cat_meta, files) }` `[metadata, files]` के format में एक tuple बनाता है जो `FIND_CONCATENATE` अपेक्षा करता है

यह सब कुछ है जो हमें `FIND_CONCATENATE` के लिए input tuple को सेट अप करने के लिए करने की आवश्यकता है।

### 3.3. FIND_CONCATENATE मॉड्यूल को call करें

अब नए बनाए गए channel पर `FIND_CONCATENATE` को call करें:
...
हालाँकि, `FIND_CONCATENATE` process एक tuple उत्पन्न करता है जिसमें output फ़ाइल के अलावा metamap शामिल है।

चूंकि `cowpy` अभी तक metadata tuples स्वीकार नहीं करता है (हम इसे पाठ्यक्रम के अगले भाग में ठीक करेंगे), हमें `cowpy` को सौंपने से पहले `FIND_CONCATENATE` द्वारा निर्मित tuple से output फ़ाइल को extract करने की आवश्यकता है:
...
`#!groovy .map { meta, file -> file }` operation `FIND_CONCATENATE` द्वारा निर्मित `[metadata, file]` tuple से file को एक नए channel, `ch_for_cowpy`, में extract करता है।
...
आइए परीक्षण करें कि workflow नए integrated `find/concatenate` मॉड्यूल के साथ काम करता है:
...
ध्यान दें कि `collectGreetings` के बजाय अब process execution list में `FIND_CONCATENATE` दिखाई देता है।
