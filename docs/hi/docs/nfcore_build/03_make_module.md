...
??? info "इस खंड से कैसे शुरू करें"
    ...
    यह सेक्शन मानता है कि आपने [भाग 2: nf-core मॉड्यूल का उपयोग करें](./02_use_module.md) पूरा कर लिया है और अपनी पाइपलाइन में `FIND_CONCATENATE` मॉड्यूल को एकीकृत कर लिया है।

    यदि आपने भाग 2 पूरा नहीं किया है या इस भाग के लिए नया शुरू करना चाहते हैं, तो आप अपने शुरुआती बिंदु के रूप में `core-hello-part2` समाधान का उपयोग कर सकते हैं।
    `nfcore-build/` डायरेक्टरी के अंदर से ये कमांड चलाएं:
    ...
    यह आपको एक पाइपलाइन देता है जिसमें `FIND_CONCATENATE` मॉड्यूल पहले से एकीकृत है।
...
`core-hello` पाइपलाइन के वर्तमान संस्करण में, हम `COWPY` को पास करने के लिए `FIND_CONCATENATE` के आउटपुट टपल से फ़ाइल निकाल रहे हैं, जैसा कि नीचे दिए गए आरेख के ऊपरी भाग में दिखाया गया है।
...
जैसा कि आप देख सकते हैं, हमने **मुख्य इनपुट** और **आउटपुट** दोनों को एक टपल में बदल दिया जो इस प्रशिक्षण के भाग 2 में प्रस्तुत `tuple val(meta), path(input_file)` पैटर्न का पालन करता है।
...
अब जब `FIND_CONCATENATE` का आउटपुट और `COWPY` का इनपुट एक ही 'आकार' के हैं, यानी दोनों में `tuple val(meta), path(input_file)` संरचना है, तो हम उन्हें सीधे कनेक्ट कर सकते हैं बजाय `FIND_CONCATENATE` प्रोसेस के आउटपुट से फ़ाइल को स्पष्ट रूप से निकालने के।
...
अब हम `FIND_CONCATENATE.out.file_out` पर सीधे `COWPY` को कॉल करते हैं।
...
पाइपलाइन सफलतापूर्वक चलनी चाहिए, मेटाडेटा अब `FIND_CONCATENATE` से `COWPY` के माध्यम से प्रवाहित हो रहा है।
...
`withName:` सिंटैक्स इस कॉन्फ़िगरेशन को केवल `COWPY` प्रोसेस को असाइन करता है, और `#!groovy ext.args = { "-c ${params.character}" }` बस एक स्ट्रिंग बनाता है जिसमें `character` पैरामीटर का मान शामिल होगा।
...
1. **`script:` ब्लॉक में, हमने लाइन `#!groovy prefix = task.ext.prefix ?: "${meta.id}"` जोड़ी।**
...
2. **कमांड लाइन में, हमने `#!groovy cowpy-${input_file}` को `#!groovy ${prefix}.txt` से बदल दिया।**
   यह वह जगह है जहाँ Nextflow ऊपर की लाइन द्वारा निर्धारित `prefix` का मान इंजेक्ट करेगा।

3. **`output:` ब्लॉक में, हमने `#!groovy path("cowpy-${input_file}")` को `#!groovy path("${prefix}.txt")` से बदल दिया।**
...
results डायरेक्टरी में आउटपुट पर नज़र डालें।
...
- **`core-hello-results`** — command line पर `--outdir` के साथ सेट की गई output directory, जो nf-core logs और `FIND_CONCATENATE` द्वारा published results प्राप्त कर रही है।
...
"अरे," आप कह सकते हैं, "`FIND_CONCATENATE` अपने outputs `--outdir` में भेज रहा है। शायद हमें इसकी `publishDir` directive कॉपी करनी चाहिए?"
...
!!! info "जानकारी"
    ...
    आप `pipeline_info/` में `hello_software_versions.yml` देख सकते हैं।
    इसमें अभी केवल `FIND_CONCATENATE` से version जानकारी है, क्योंकि `COWPY` अभी अपना version रिपोर्ट नहीं करता।
    Section 1.6 में इसे जोड़ने का तरीका बताया गया है।
...
- **`path:`** process नाम के आधार पर output directory निर्धारित करता है।
  `task.process` में निहित एक process का पूरा नाम workflow और module imports की hierarchy शामिल करता है (जैसे `CORE_HELLO:HELLO:FIND_CONCATENATE`)।
  ...
  यही निर्धारित करता है कि `FIND_CONCATENATE` के results `#!groovy ${params.outdir}/find/` में publish होते हैं।
...
### 1.6. Topic channels के साथ version capture

Reproducibility nf-core का एक मूल सिद्धांत है: हर pipeline run में ठीक-ठीक रिकॉर्ड होता है कि कौन से software versions उपयोग किए गए, ताकि results को reproduce और runs के बीच compare किया जा सके।
`pipeline_info/` में `hello_software_versions.yml` रिपोर्ट इसी तरह nf-core pipelines इस आवश्यकता को पूरा करती हैं।

इसके पीछे का mechanism है **topic channels**: एक Nextflow broadcast फीचर जहाँ कोई भी process एक named topic पर data publish कर सकता है, और कोई भी subscriber उस topic पर publish की गई हर चीज़ प्राप्त करता है — चाहे वह pipeline में कहीं से भी आए।
nf-core इसे 2026 में सभी pipelines में लागू कर रहा है।
Modules `topic:` keyword का उपयोग करके output block में सीधे एक tuple के रूप में अपना version publish करते हैं, और workflow एक बार subscribe करके सभी को collect करता है।

#### 1.6.1. `COWPY` में version reporting जोड़ें

`cowpy.nf` module file खोलें और नीचे दिखाए अनुसार एक version output line जोड़ें।
...
नई लाइन `"versions"` topic channel पर एक `[process_name, tool_name, version]` tuple publish करती है।
script block में कोई बदलाव की ज़रूरत नहीं है — version को output block में statically declare किया गया है।

#### 1.6.2. Pipeline चलाएं और versions report देखें
...
`core-hello-results/pipeline_info/hello_software_versions.yml` खोलें और अब आपको दोनों modules दिखेंगे:
...
workflow-side collection — वह `channel.topic("versions")` block जो आपने भाग 1 के placeholder workflow में देखा था — topic को subscribe करता है और यह combined report स्वचालित रूप से लिखता है।

!!! info "Backwards compatibility"

    workflow के topic channel block में `versions_file` branch उन modules को handle करने के लिए मौजूद है जिन्हें अभी तक `topic: versions` उपयोग करने के लिए update नहीं किया गया है और जो अभी भी script block में `emit: versions` के साथ `versions.yml` file लिखते हैं।
    Transition के दौरान दोनों styles एक साथ supported हैं।
...
- समझें कि topic channels सभी modules से software versions को स्वचालित रूप से कैसे collect करते हैं।

Module conventions के बारे में और पढ़ने के लिए, [nf-core modules specifications](https://nf-co.re/docs/guidelines/components/modules) देखें।
...
!!! tip "Testing के बारे में और जानें"
    ...
    Generated test file nf-test का उपयोग करती है, जो Nextflow pipelines और modules के लिए एक testing framework है। इन tests को लिखना और चलाना सीखने के लिए, [nf-test side quest](../side_quests/nf_test/index.md) देखें।
...
- **`topic: versions` output**: Section 1.6 में बताए अनुसार topic channel के माध्यम से version reporting
...
- **`script:` block**: खाली है — हम यहाँ `cowpy` command जोड़ेंगे
- **`stub:` block**: हम इसे correct outputs produce करने के लिए update करेंगे
...
Conda environment के लिए, module code `#!groovy conda "${moduleDir}/environment.yml"` specify करता है जिसका अर्थ है कि इसे `environment.yml` file में configure किया जाना चाहिए।
...
- Configurable prefix pattern का उपयोग करके output filename (`#!groovy ${prefix}.txt` wildcard `*` के बजाय)
- एक descriptive emit name (`cowpy_output` generic `output` के बजाय)
- Template के `#!groovy eval("cowpy --version")` की जगह एक static version string (`#!groovy val("1.1.5")`), जो section 1.6 के manual module से मेल खाता है (`cowpy` tool `--version` flag expose नहीं करता)

यदि आप syntax validate करने के लिए Nextflow language server का उपयोग कर रहे हैं, तो `#!groovy ${prefix}` part इस stage पर error के रूप में flag किया जाएगा क्योंकि हमने इसे अभी तक script block में नहीं जोड़ा है।
अब उस पर आते हैं।

#### 2.3.2. Script block

Template एक खाली script block generate करता है जिसे आपको actual tool command से भरना होगा।
...
- Actual `cowpy` command भरें जो `$args` और `#!groovy ${prefix}.txt` दोनों का उपयोग करता है
...
- Stub एक खाली `#!groovy ${prefix}.txt` file बनाता है जो script block के output से match करता है
...
जब आप तैयार हों, तो अपने pipeline में schema-based input validation जोड़ने के लिए [भाग 4: Input validation](./04_input_validation.md) पर जारी रखें।
