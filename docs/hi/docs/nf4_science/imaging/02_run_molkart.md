...
हालांकि, यहां तुम जो Nextflow पैटर्न सीखोगे वे किसी भी nf-core पाइपलाइन या प्रोडक्शन वर्कफ़्लो पर लागू होते हैं।
...
सभी nf-core पाइपलाइनें समान संरचना और परंपराओं का पालन करती हैं, जिसका मतलब है कि एक बार जब तुम एक को चलाना सीख लेते हो, तो तुम उनमें से किसी को भी चला सकते हो।
...
!!! tip "nf-core के बारे में अधिक जानना चाहते हो?"

    nf-core पाइपलाइन विकास के गहन परिचय के लिए, [Hello nf-core](../../hello_nf-core/index.md) प्रशिक्षण पाठ्यक्रम देखो।
...
!!! note "हम स्थानीय रूप से क्यों क्लोन कर रहे हैं?"
    ...
    आमतौर पर, तुम nf-core पाइपलाइनों को सीधे GitHub से `nextflow run nf-core/molkart -r 1.2.0` का उपयोग करके चलाते हो।
    Nextflow तुम्हारे लिए स्वचालित रूप से अनुरोधित पाइपलाइन संस्करण को `$HOME/.nextflow/assets/nf-core/molkart` पर डाउनलोड करता है और वहां से चलाता है।
...
कुछ क्षणों के बाद, तुम्हें इस तरह की एक एरर दिखेगी:
...
एरर `command not found` (exit status 127) का मतलब है कि Nextflow ने `duplicate_finder.py` चलाने का प्रयास किया लेकिन इसे तुम्हारे सिस्टम पर नहीं मिला।
...
3. कंटेनर के बिना, Nextflow तुम्हारी स्थानीय मशीन पर सीधे कमांड चलाने का प्रयास करता है

**इन टूल्स को कहां से आना चाहिए?**

आइए एक **process** मॉड्यूल का निरीक्षण करें यह देखने के लिए कि यह अपनी सॉफ़्टवेयर आवश्यकताओं को कैसे घोषित करता है।

CLAHE प्रीप्रोसेसिंग मॉड्यूल खोलो:

```bash
code molkart/modules/local/clahe/main.nf
```

लाइन 5 को देखो - तुम्हें दिखेगा:

```groovy
container 'ghcr.io/schapirolabor/molkart-local:v0.0.4'
```

यह लाइन Nextflow को बताती है: "इस **process** को चलाने के लिए, Docker इमेज `ghcr.io/schapirolabor/molkart-local:v0.0.4` का उपयोग करो, जिसमें सभी आवश्यक सॉफ़्टवेयर हैं।"

प्रत्येक **process** घोषित करता है कि कौन सी कंटेनर इमेज अपने आवश्यक टूल प्रदान करती है।
हालांकि, Nextflow इन कंटेनरों का उपयोग तभी करता है जब तुम इसे बताते हो!

**समाधान: कॉन्फ़िगरेशन में Docker सक्षम करो**

### 2.2. Docker कॉन्फ़िगर करो और पाइपलाइन लॉन्च करो
...
अब समान कमांड के साथ पाइपलाइन फिर से चलाओ:
...
3. प्रत्येक **process** को उसकी निर्दिष्ट कंटेनर के अंदर चलाएगा
...
!!! Tip "कंटेनर क्यों महत्वपूर्ण हैं"
    ...
    - तुम्हें दर्जनों टूल और उनकी निर्भरताओं को मैन्युअल रूप से इंस्टॉल करने की आवश्यकता नहीं है

    Nextflow में कंटेनरों के बारे में अधिक विवरण के लिए, Hello Nextflow प्रशिक्षण से [Hello Containers](../../hello_nextflow/05_hello_containers.md) देखो।

### 2.3. निष्पादन की निगरानी करो

जैसे-जैसे पाइपलाइन चलती है, तुम्हें इस तरह का आउटपुट दिखेगा:
...
ध्यान दो कि यह आउटपुट हमारे Hello World उदाहरण से अधिक विस्तृत है क्योंकि पाइपलाइन द्वारा अपनाई गई nf-core परंपराओं के कारण:

- पाइपलाइन अपना संस्करण और लोगो दिखाती है
- कॉन्फ़िगरेशन पैरामीटर प्रदर्शित किए जाते हैं
- कई **process** समानांतर में चलते हैं (कई **process** लाइनों द्वारा संकेत किया गया)
- **Process** नामों में पूर्ण मॉड्यूल पथ शामिल है (जैसे, `NFCORE_MOLKART:MOLKART:MINDAGAP_MINDAGAP`)

### 2.4. प्रोसेस निष्पादन को समझना

executor लाइन `executor > local (22)` तुम्हें बताती है:

- **executor**: किस कंप्यूट वातावरण का उपयोग किया जा रहा है (`local` = तुम्हारी मशीन)
- **(22)**: लॉन्च किए गए कुल कार्यों की संख्या

प्रत्येक **process** लाइन दिखाती है:

- **Hash** (`[1a/2b3c4d]`): work डायरेक्टरी आइडेंटिफ़ायर (पहले की तरह)
- **Process name**: पूर्ण मॉड्यूल पथ और **process** नाम
- **Input identifier**: कोष्ठक में नमूना नाम
- **Progress**: पूर्णता का प्रतिशत और गिनती (जैसे, `1 of 1 ✔`)

### सारांश

तुम जानते हो कि टेस्ट डेटा के साथ nf-core पाइपलाइन कैसे लॉन्च करें और इसके निष्पादन आउटपुट की व्याख्या कैसे करें।

### आगे क्या है?

जानो कि परिणाम कहां मिलेंगे और उनकी व्याख्या कैसे करें।

---

## 3. आउटपुट ढूंढो और जांचो

जब पाइपलाइन सफलतापूर्वक पूर्ण होती है, तो तुम्हें एक पूर्णता संदेश और निष्पादन सारांश दिखेगा।

### 3.1. परिणाम डायरेक्टरी का पता लगाओ
...
सामग्री की सूची बनाओ:

```bash
tree results/
```

तुम्हें कई subdirectories दिखनी चाहिए:
...
### 3.2. MultiQC रिपोर्ट जांचो

MultiQC रिपोर्ट एक व्यापक HTML फ़ाइल है जो सभी पाइपलाइन स्टेप्स से गुणवत्ता मेट्रिक्स एकत्रित करती है।

फ़ाइल ब्राउज़र में रिपोर्ट खोलो और फिर VS Code में सीधे इसे रेंडर देखने के लिए "Show Preview" बटन पर क्लिक करो।
...
### 3.3. सेल-बाय-ट्रांसक्रिप्ट टेबल जांचो

सबसे महत्वपूर्ण वैज्ञानिक आउटपुट सेल-बाय-ट्रांसक्रिप्ट काउंट टेबल है।
यह तुम्हें बताता है कि प्रत्येक सेल में प्रत्येक ट्रांसक्रिप्ट कितने पाए गए।

spot2cell डायरेक्टरी पर नेविगेट करो:
...
ध्यान दो कि Nextflow कैसे समानांतर में कई सेगमेंटेशन विधियों को प्रोसेस कर सकता है, जिससे परिणामों की तुलना करना आसान हो जाता है।

### 3.4. निष्पादन रिपोर्ट देखो

Nextflow स्वचालित रूप से कई निष्पादन रिपोर्ट जेनरेट करता है।

pipeline_info डायरेक्टरी जांचो:
...
- **execution_timeline.html**: **process** निष्पादन का Gantt चार्ट
...
संसाधन उपयोग देखने के लिए निष्पादन रिपोर्ट खोलो:
...
तुम जानते हो कि पाइपलाइन आउटपुट कैसे खोजें, गुणवत्ता नियंत्रण रिपोर्ट की जांच करें, और निष्पादन मेट्रिक्स तक पहुंचें।

### आगे क्या है?

work डायरेक्टरी के बारे में जानो और Nextflow intermediate फ़ाइलों को कैसे प्रबंधित करता है।

---

## 4. work डायरेक्टरी का अन्वेषण करो
...
### 4.2. एक कार्य डायरेक्टरी का निरीक्षण करो

console आउटपुट से एक सेगमेंटेशन **process** हैश (जैसे, `[3m/4n5o6p]`) चुनो और अंदर देखो:
...
!!! Tip "सुझाव"
    ...
    यदि कोई **process** विफल होती है, तो तुम उसकी work डायरेक्टरी पर नेविगेट कर सकते हो, एरर संदेशों के लिए `.command.err` की जांच कर सकते हो, और समस्या को debug करने के लिए `.command.sh` को मैन्युअली फिर से चला भी सकते हो।

### 4.3. work डायरेक्टरी की सफाई

कई पाइपलाइन रन पर work डायरेक्टरी काफी बड़ी हो सकती है।
जैसा कि हमने भाग 1 में सीखा, तुम पुराने रन से work डायरेक्टरियों को हटाने के लिए `nextflow clean` का उपयोग कर सकते हो।

हालांकि, बड़ी intermediate फ़ाइलों वाली nf-core पाइपलाइनों के लिए, नियमित रूप से साफ करना विशेष रूप से महत्वपूर्ण है।

### सारांश

तुम समझते हो कि nf-core पाइपलाइनें अपनी work डायरेक्टरियों को कैसे व्यवस्थित करती हैं और debugging के लिए व्यक्तिगत कार्यों का निरीक्षण कैसे करें।

### आगे क्या है?

Nextflow cache के बारे में जानो और विफल पाइपलाइन रन को कैसे resume करें।

---

## 5. पाइपलाइन रन को resume करो

Nextflow की सबसे शक्तिशाली विशेषताओं में से एक विफलता के बिंदु से पाइपलाइन को resume करने की क्षमता है।

### 5.1. cache तंत्र

जब तुम `-resume` के साथ पाइपलाइन चलाते हो, तो Nextflow:
...
### 5.2. molkart के साथ resume का प्रयास करो

समान कमांड फिर से चलाओ, लेकिन `-resume` जोड़ो:
...
तुम्हें इस तरह का आउटपुट दिखना चाहिए: <!-- TODO: full output -->
...
ध्यान दो प्रत्येक **process** के लिए `cached: 2` या `cached: 1` - कुछ भी फिर से निष्पादित नहीं किया गया!
...
- तुम्हें upstream स्टेप्स को फिर से चलाए बिना downstream **process** को संशोधित करने की आवश्यकता है
- डेटा डाउनलोड के दौरान तुम्हारा नेटवर्क कनेक्शन टूट जाता है
- तुम गणना को फिर से किए बिना अतिरिक्त आउटपुट जोड़ना चाहते हो

!!! Warning "चेतावनी"

    Resume केवल तभी काम करता है जब तुमने इनपुट डेटा, पाइपलाइन कोड, या पैरामीटर को नहीं बदला हो।
    यदि तुम इनमें से किसी को भी बदलते हो, तो Nextflow सही ढंग से प्रभावित कार्यों को फिर से चलाएगा।

### सारांश

तुम जानते हो कि सफल कार्यों को दोहराए बिना पाइपलाइनों को कुशलतापूर्वक फिर से चलाने के लिए `-resume` का उपयोग कैसे करें।

### आगे क्या है?

अब जब तुम टेस्ट डेटा के साथ nf-core/molkart चला सकते हो, तो तुम अपने स्वयं के डेटासेट के लिए इसे कॉन्फ़िगर करना सीखने के लिए तैयार हो।
