...
वेरिएंट कॉलिंग एक जीनोमिक विश्लेषण विधि है जिसका उद्देश्य एक संदर्भ जीनोम के सापेक्ष जीनोम अनुक्रम में भिन्नताओं की पहचान करना है।
यहाँ हम संपूर्ण-जीनोम अनुक्रमण डेटा में छोटे जर्मलाइन वेरिएंट, _यानी_ SNPs और indels, को कॉल करने के लिए डिज़ाइन किए गए टूल्स और विधियों का उपयोग करने जा रहे हैं।

![GATK pipeline](img/gatk-pipeline.png)

एक पूर्ण वेरिएंट कॉलिंग पाइपलाइन में आमतौर पर कई चरण शामिल होते हैं, जिनमें संदर्भ के लिए मैपिंग (कभी-कभी जीनोम संरेखण के रूप में संदर्भित) और वेरिएंट फ़िल्टरिंग और प्राथमिकता शामिल है।
सरलता के लिए, इस कोर्स में हम केवल वेरिएंट कॉलिंग भाग पर ध्यान केंद्रित करने जा रहे हैं।

### विधियाँ

हम तुम्हें जर्मलाइन SNPs और indels की पहचान करने के लिए संपूर्ण-जीनोम अनुक्रमण नमूनों पर वेरिएंट कॉलिंग लागू करने के दो तरीके दिखाने जा रहे हैं।
पहले हम एक सरल **प्रति-नमूना दृष्टिकोण** से शुरू करेंगे जो प्रत्येक नमूने से स्वतंत्र रूप से वेरिएंट को कॉल करता है।
फिर हम तुम्हें एक अधिक परिष्कृत **संयुक्त कॉलिंग दृष्टिकोण** दिखाएंगे जो कई नमूनों का एक साथ विश्लेषण करता है, अधिक सटीक और जानकारीपूर्ण परिणाम उत्पन्न करता है।

इससे पहले कि हम किसी भी दृष्टिकोण के लिए कोई workflow कोड लिखना शुरू करें, हम कुछ परीक्षण डेटा पर कमांड को मैनुअल रूप से आज़माने जा रहे हैं।
...
- **तीन संपूर्ण जीनोम अनुक्रमण नमूने** जो एक परिवार त्रिकोण (माँ, पिता और बेटा) के अनुरूप हैं, जिन्हें फ़ाइल आकार छोटा रखने के लिए क्रोमोसोम 20 पर डेटा के एक छोटे से हिस्से में विभाजित किया गया है।
  यह Illumina शॉर्ट-रीड अनुक्रमण डेटा है जिसे पहले ही संदर्भ जीनोम में मैप किया जा चुका है, [BAM](https://samtools.github.io/hts-specs/SAMv1.pdf) फ़ॉर्मेट (Binary Alignment Map, SAM का एक संकुचित संस्करण, Sequence Alignment Map) में प्रदान किया गया है।
- **जीनोमिक अंतरालों की एक सूची**, यानी जीनोम पर निर्देशांक जहाँ हमारे नमूनों में वेरिएंट कॉल करने के लिए उपयुक्त डेटा है, BED फ़ॉर्मेट में प्रदान किया गया है।

### सॉफ़्टवेयर

शामिल दो मुख्य टूल्स हैं [Samtools](https://www.htslib.org/), अनुक्रम संरेखण फ़ाइलों में हेरफेर करने के लिए व्यापक रूप से उपयोग किया जाने वाला टूलकिट, और [GATK](https://gatk.broadinstitute.org/) (Genome Analysis Toolkit), Broad Institute में विकसित वेरिएंट खोज के लिए टूल्स का एक सेट।
...
!!! note
     ...
     सुनिश्चित करो कि तुम `nf4-science/genomics` डायरेक्टरी में हो ताकि जब तुम `pwd` टाइप करो तो दिखाए गए पथ का अंतिम भाग `genomics` हो।

---

## 1. प्रति-नमूना वेरिएंट कॉलिंग

प्रति-नमूना वेरिएंट कॉलिंग प्रत्येक नमूने को स्वतंत्र रूप से प्रोसेस करती है: वेरिएंट कॉलर एक समय में एक नमूने के लिए अनुक्रमण डेटा की जांच करता है और उन स्थितियों की पहचान करता है जहाँ नमूना संदर्भ से भिन्न होता है।

इस खंड में हम प्रति-नमूना वेरिएंट कॉलिंग दृष्टिकोण बनाने वाले दो कमांड का परीक्षण करते हैं: Samtools के साथ BAM फ़ाइल को इंडेक्स करना और GATK HaplotypeCaller के साथ वेरिएंट कॉल करना।
ये वे कमांड हैं जिन्हें हम इस कोर्स के भाग 2 में Nextflow workflow में लपेटेंगे।

1. [Samtools](https://www.htslib.org/) का उपयोग करके BAM इनपुट फ़ाइल के लिए एक इंडेक्स फ़ाइल जेनरेट करें
2. VCF (Variant Call Format) में प्रति-नमूना वेरिएंट कॉल जेनरेट करने के लिए इंडेक्स की गई BAM फ़ाइल पर GATK HaplotypeCaller चलाएं

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/genomics/img/hello-gatk-1.svg"
</figure>

हम केवल एक नमूने पर दो कमांड का परीक्षण करके शुरू करते हैं।

### 1.1. Samtools के साथ BAM इनपुट फ़ाइल को इंडेक्स करें

इंडेक्स फ़ाइलें बायोइन्फॉर्मेटिक्स फ़ाइल फ़ॉर्मेट की एक सामान्य विशेषता हैं; उनमें मुख्य फ़ाइल की संरचना के बारे में जानकारी होती है जो GATK जैसे टूल्स को पूरी फ़ाइल को पढ़े बिना डेटा के एक सबसेट तक पहुँचने की अनुमति देती है।
यह महत्वपूर्ण है क्योंकि ये फ़ाइलें कितनी बड़ी हो सकती हैं।

BAM फ़ाइलें अक्सर इंडेक्स के बिना प्रदान की जाती हैं, इसलिए कई विश्लेषण workflows में पहला चरण `samtools index` का उपयोग करके एक जेनरेट करना है।

हम एक Samtools कंटेनर को पुल करने जा रहे हैं, इसे इंटरैक्टिव रूप से स्पिन अप करने जा रहे हैं और BAM फ़ाइलों में से एक पर `samtools index` कमांड चलाने जा रहे हैं।
...
यदि तुमने पहले इस इमेज को डाउनलोड नहीं किया है, तो इसे पूरा होने में एक मिनट लग सकता है।
एक बार यह हो जाने के बाद, तुम्हारे पास कंटेनर इमेज की एक लोकल कॉपी है।

#### 1.1.2. Samtools कंटेनर को इंटरैक्टिव रूप से स्पिन अप करें

कंटेनर को इंटरैक्टिव रूप से चलाने के लिए, `-it` फ्लैग के साथ `docker run` का उपयोग करें।
`-v ./data:/data` विकल्प लोकल `data` डायरेक्टरी को कंटेनर में माउंट करता है ताकि टूल्स इनपुट फ़ाइलों तक पहुँच सकें।

```bash
docker run -it -v ./data:/data community.wave.seqera.io/library/samtools:1.20--b5dfbd93de237464
```

तुम्हारा प्रॉम्प्ट `(base) root@a1b2c3d4e5f6:/tmp#` जैसा कुछ बदल जाता है, जो दर्शाता है कि तुम अब कंटेनर के अंदर हो।
...
हम एक GATK कंटेनर को पुल करने जा रहे हैं, इसे इंटरैक्टिव रूप से स्पिन अप करने जा रहे हैं और हमने अभी इंडेक्स की गई BAM फ़ाइल पर `gatk HaplotypeCaller` कमांड चलाने जा रहे हैं।
...
??? success "कमांड आउटपुट"
    ...
    कुछ लेयर्स `Already exists` दिखाती हैं क्योंकि वे Samtools कंटेनर इमेज के साथ साझा की जाती हैं जिसे हमने पहले पुल किया था।
...
यह पहले पुल की तुलना में तेज़ होना चाहिए क्योंकि दोनों कंटेनर इमेज अपनी अधिकांश लेयर्स साझा करती हैं।

#### 1.2.2. GATK कंटेनर को इंटरैक्टिव रूप से स्पिन अप करें

GATK कंटेनर को डेटा डायरेक्टरी माउंट के साथ इंटरैक्टिव रूप से स्पिन अप करें, जैसा हमने Samtools के लिए किया था।
...
हमें BAM इनपुट फ़ाइल (`-I`) के साथ-साथ संदर्भ जीनोम (`-R`), आउटपुट फ़ाइल के लिए एक नाम (`-O`) और विश्लेषण करने के लिए जीनोमिक अंतरालों की एक सूची (`-L`) प्रदान करने की आवश्यकता है।

हालाँकि, हमें इंडेक्स फ़ाइल के पथ को निर्दिष्ट करने की आवश्यकता नहीं है; टूल स्वचालित रूप से स्थापित नामकरण और सह-स्थान सम्मेलन के आधार पर इसे उसी डायरेक्टरी में खोजेगा।
...
??? success "कमांड आउटपुट"
    ...
    टूल वर्बोज़ लॉगिंग आउटपुट उत्पन्न करता है। हाइलाइट की गई लाइनें सफल समापन की पुष्टि करती हैं।
...
आउटपुट फ़ाइल `reads_mother.vcf` कंटेनर में तुम्हारी वर्किंग डायरेक्टरी के अंदर बनाई गई है, इसलिए तुम्हें यह VS Code फ़ाइल एक्सप्लोरर में तब तक नहीं दिखेगी जब तक तुम आउटपुट फ़ाइल पथ नहीं बदलते।
हालाँकि, यह एक छोटी परीक्षण फ़ाइल है, इसलिए तुम इसे खोलने और सामग्री देखने के लिए `cat` कर सकते हो।
यदि तुम फ़ाइल की शुरुआत तक स्क्रॉल करते हो, तो तुम्हें मेटाडेटा की कई लाइनों से बना एक हेडर मिलेगा, जिसके बाद वेरिएंट कॉल की एक सूची होगी, प्रति लाइन एक।
...
प्रत्येक लाइन नमूने के अनुक्रमण डेटा में पहचाने गए एक संभावित वेरिएंट का वर्णन करती है। VCF फ़ॉर्मेट की व्याख्या करने के लिए मार्गदर्शन के लिए, [यह सहायक लेख](https://www.ebi.ac.uk/training/online/courses/human-genetic-variation-introduction/variant-identification-and-analysis/understanding-vcf-format/) देखें।

आउटपुट VCF फ़ाइल के साथ `reads_mother.vcf.idx` नामक एक इंडेक्स फ़ाइल है जो GATK द्वारा स्वचालित रूप से बनाई गई थी।
इसका वही कार्य है जो BAM इंडेक्स फ़ाइल का है, टूल्स को पूरी फ़ाइल लोड किए बिना डेटा के सबसेट को खोजने और पुनर्प्राप्त करने की अनुमति देना।
...
तुम्हारा प्रॉम्प्ट सामान्य पर वापस होना चाहिए।
यह प्रति-नमूना वेरिएंट कॉलिंग परीक्षण समाप्त करता है।

---

## 2. एक कोहोर्ट पर संयुक्त कॉलिंग

हमने अभी जो वेरिएंट कॉलिंग दृष्टिकोण उपयोग किया वह प्रति नमूना वेरिएंट कॉल जेनरेट करता है।
यह प्रत्येक नमूने से अलगाव में वेरिएंट को देखने के लिए ठीक है, लेकिन यह सीमित जानकारी देता है।
यह देखना अक्सर अधिक दिलचस्प होता है कि वेरिएंट कॉल कई नमूनों में कैसे भिन्न होते हैं।
GATK इस उद्देश्य के लिए संयुक्त वेरिएंट कॉलिंग नामक एक वैकल्पिक विधि प्रदान करता है।

संयुक्त वेरिएंट कॉलिंग में प्रत्येक नमूने के लिए GVCF (Genomic VCF के लिए) नामक एक विशेष प्रकार का वेरिएंट आउटपुट जेनरेट करना, फिर सभी नमूनों से GVCF डेटा को संयोजित करना और 'संयुक्त जीनोटाइपिंग' सांख्यिकीय विश्लेषण चलाना शामिल है।

![Joint analysis](img/joint-calling.png)

एक नमूने के GVCF के बारे में जो विशेष है वह यह है कि इसमें जीनोम के लक्षित क्षेत्र में सभी स्थितियों के बारे में अनुक्रम डेटा सांख्यिकी को सारांशित करने वाले रिकॉर्ड होते हैं, न कि केवल उन स्थितियों के जहाँ प्रोग्राम को भिन्नता का सबूत मिला।
यह संयुक्त जीनोटाइपिंग गणना के लिए महत्वपूर्ण है ([आगे पढ़ें](https://gatk.broadinstitute.org/hc/en-us/articles/360035890431-The-logic-of-joint-calling-for-germline-short-variants))।
...
GVCFs को संयोजित करना GATK GenomicsDBImport के साथ किया जाता है, जो प्रति-नमूना कॉल को एक डेटा स्टोर (डेटाबेस के समान) में संयोजित करता है।
वास्तविक 'संयुक्त जीनोटाइपिंग' विश्लेषण फिर GATK GenotypeGVCFs के साथ किया जाता है।

यहाँ हम GVCFs जेनरेट करने और संयुक्त जीनोटाइपिंग चलाने के लिए आवश्यक कमांड का परीक्षण करते हैं।
ये वे कमांड हैं जिन्हें हम इस कोर्स के भाग 3 में Nextflow workflow में लपेटेंगे।
...
3. सभी GVCFs को एकत्र करें और उन्हें GenomicsDB डेटा स्टोर में संयोजित करें
4. कोहोर्ट-स्तर VCF उत्पन्न करने के लिए संयुक्त GVCF डेटा स्टोर पर संयुक्त जीनोटाइपिंग चलाएं

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/genomics/img/hello-gatk-2.svg"
</figure>

हमें अब इन सभी कमांड का परीक्षण करने की आवश्यकता है, तीनों BAM फ़ाइलों को इंडेक्स करने से शुरू करते हुए।

### 2.1. तीनों नमूनों के लिए BAM फ़ाइलों को इंडेक्स करें

ऊपर पहले खंड में, हमने केवल एक BAM फ़ाइल को इंडेक्स किया।
अब हमें तीनों नमूनों को इंडेक्स करने की आवश्यकता है ताकि GATK HaplotypeCaller उन्हें प्रोसेस कर सके।

#### 2.1.1. Samtools कंटेनर को इंटरैक्टिव रूप से स्पिन अप करें
...
तुम्हारा प्रॉम्प्ट बदल जाता है जो दर्शाता है कि तुम कंटेनर के अंदर हो, डेटा डायरेक्टरी पहले की तरह माउंट है।

#### 2.1.2. तीनों नमूनों पर इंडेक्सिंग कमांड चलाएं
...
### 2.2. तीनों नमूनों के लिए GVCFs जेनरेट करें

संयुक्त जीनोटाइपिंग चरण चलाने के लिए, हमें तीनों नमूनों के लिए GVCFs की आवश्यकता है।

#### 2.2.1. GATK कंटेनर को इंटरैक्टिव रूप से स्पिन अप करें
...
तुम्हारा प्रॉम्प्ट बदल जाता है जो दर्शाता है कि तुम GATK कंटेनर के अंदर हो।
...
यह तकनीकी रूप से एक आवश्यकता नहीं है, लेकिन यह एक दृढ़ता से अनुशंसित सम्मेलन है।
...
यह कंटेनर में वर्तमान वर्किंग डायरेक्टरी में GVCF आउटपुट फ़ाइल `reads_mother.g.vcf` बनाता है।

यदि तुम सामग्री देखने के लिए इसे `cat` करते हो, तो तुम देखोगे कि यह खंड 1 में हमने जेनरेट किए गए समकक्ष VCF की तुलना में बहुत लंबा है। तुम फ़ाइल की शुरुआत तक स्क्रॉल भी नहीं कर सकते, और अधिकांश लाइनें VCF में हमने जो देखा उससे काफी अलग दिखती हैं।
...
ये गैर-वेरिएंट क्षेत्रों का प्रतिनिधित्व करते हैं जहाँ वेरिएंट कॉलर को भिन्नता का कोई सबूत नहीं मिला, इसलिए इसने भिन्नता की अनुपस्थिति में अपने विश्वास के स्तर का वर्णन करने वाले कुछ सांख्यिकी कैप्चर किए।
यह दो बहुत अलग मामलों के बीच अंतर करना संभव बनाता है: (1) अच्छी गुणवत्ता वाला डेटा है जो दिखाता है कि नमूना होमोज़ाइगस-संदर्भ है, और (2) किसी भी तरह से निर्धारण करने के लिए पर्याप्त अच्छा डेटा उपलब्ध नहीं है।

एक GVCF में, आमतौर पर ऐसी बहुत सारी गैर-वेरिएंट लाइनें होती हैं, जिनके बीच कम संख्या में वेरिएंट रिकॉर्ड छिड़के होते हैं।
एक वास्तविक वेरिएंट कॉल खोजने के लिए फ़ाइल की केवल पहली 176 लाइनों को लोड करने के लिए GVCF पर `head -176` चलाने का प्रयास करें।
...
दूसरी लाइन फ़ाइल में पहला वेरिएंट रिकॉर्ड दिखाती है, जो VCF फ़ाइल में पहले वेरिएंट के अनुरूप है जिसे हमने पहले देखा था।

जैसे मूल VCF था, आउटपुट GVCF फ़ाइल भी एक इंडेक्स फ़ाइल के साथ है, जिसे `reads_mother.g.vcf.idx` कहा जाता है।

#### 2.2.3. अन्य दो नमूनों पर प्रक्रिया दोहराएं

नीचे दिए गए कमांड को एक के बाद एक चलाकर शेष दो नमूनों के लिए GVCFs जेनरेट करें।
...
एक बार यह पूरा हो जाने के बाद, तुम्हारे पास तुम्हारी वर्तमान डायरेक्टरी में `.g.vcf` में समाप्त होने वाली तीन फ़ाइलें (प्रति नमूना एक) और उनकी संबंधित इंडेक्स फ़ाइलें `.g.vcf.idx` में समाप्त होनी चाहिए।
...
अब जब हमारे पास सभी GVCFs हैं, तो हम नमूनों के एक कोहोर्ट के लिए वेरिएंट कॉल जेनरेट करने के लिए संयुक्त जीनोटाइपिंग दृष्टिकोण को आज़मा सकते हैं।
यह एक दो-चरणीय विधि है जिसमें सभी GVCFs से डेटा को एक डेटा स्टोर में संयोजित करना, फिर संयुक्त-कॉल किए गए वेरिएंट के अंतिम VCF को जेनरेट करने के लिए संयुक्त जीनोटाइपिंग विश्लेषण चलाना शामिल है।

#### 2.3.1. सभी प्रति-नमूना GVCFs को संयोजित करें

यह पहला चरण एक अन्य GATK टूल का उपयोग करता है, जिसे GenomicsDBImport कहा जाता है, सभी GVCFs से डेटा को GenomicsDB डेटा स्टोर में संयोजित करने के लिए।
...
तुम इसके चारों ओर देख सकते हो लेकिन तुम जल्दी से देखोगे कि यह डेटा स्टोर फ़ॉर्मेट मनुष्यों द्वारा सीधे पढ़े जाने के लिए नहीं है।

!!! note

    GATK में ऐसे टूल्स शामिल हैं जो आवश्यकतानुसार डेटा स्टोर से वेरिएंट कॉल डेटा का निरीक्षण और निष्कर्षण करना संभव बनाते हैं।

#### 2.3.2. संयुक्त जीनोटाइपिंग विश्लेषण चलाएं
...
यह कंटेनर में वर्तमान वर्किंग डायरेक्टरी में VCF आउटपुट फ़ाइल `family_trio.vcf` बनाता है।
...
यह हमने पहले जेनरेट किए गए VCF के समान दिखता है, सिवाय इस बार हमारे पास तीनों नमूनों के लिए जीनोटाइप-स्तर की जानकारी है।
फ़ाइल में अंतिम तीन कॉलम नमूनों के लिए जीनोटाइप ब्लॉक हैं, वर्णानुक्रम में सूचीबद्ध।

यदि हम अपने परीक्षण परिवार त्रिकोण के लिए पहले वेरिएंट के लिए कॉल किए गए जीनोटाइप को देखते हैं, तो हम देखते हैं कि पिता हेटरोज़ाइगस-वेरिएंट (`0/1`) है, और माँ और बेटा दोनों होमोज़ाइगस-वेरिएंट (`1/1`) हैं।

यह अंततः वह जानकारी है जिसे हम डेटासेट से निकालना चाहते हैं!
...
तुम्हारा प्रॉम्प्ट सामान्य पर वापस होना चाहिए।
यह वेरिएंट कॉलिंग कमांड के मैनुअल परीक्षण को समाप्त करता है।

---

### सारांश

तुम जानते हो कि Samtools इंडेक्सिंग और GATK वेरिएंट कॉलिंग कमांड को उनके संबंधित कंटेनरों में कैसे परीक्षण करना है, जिसमें GVCFs जेनरेट करना और कई नमूनों पर संयुक्त जीनोटाइपिंग चलाना शामिल है।

### आगे क्या है?

सीखो कि उन्हीं कमांड को workflows में कैसे लपेटना है जो काम को निष्पादित करने के लिए कंटेनरों का उपयोग करते हैं।
