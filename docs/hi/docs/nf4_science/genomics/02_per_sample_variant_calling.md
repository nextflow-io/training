...
यह [भाग 1: Method overview](./01_method.md#1-per-sample-variant-calling) के पहले सेक्शन के चरणों को automate करता है, जहाँ तुमने इन कमांड को उनके कंटेनर में मैन्युअल रूप से चलाया था।
...
!!! warning "चेतावनी"
    ...
    CSV में फ़ाइल पथ absolute paths हैं जो तुम्हारे वातावरण से match करने चाहिए।
    यदि तुम हमारे द्वारा प्रदान किए गए प्रशिक्षण वातावरण में इसे नहीं चला रहे हो, तो तुम्हें अपने सिस्टम से match करने के लिए paths को अपडेट करने की आवश्यकता होगी।
...
हम उसी पैटर्न का उपयोग करके ऐसा कर सकते हैं जिसका हमने [Hello Nextflow के भाग 2](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file) में उपयोग किया था: फ़ाइल को parse करने के लिए [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) ऑपरेटर लागू करना, फिर प्रत्येक row से फ़ाइल पथ extract करने के लिए एक `map` operation।
...
एक चीज़ जो नई है जो तुमने Hello Nextflow कोर्स में सामना की थी उसकी तुलना में यह है कि इस CSV में एक header line है, इसलिए हम `splitCsv()` कॉल में `#!groovy header: true` जोड़ते हैं।
यह हमें `map` operation में columns को नाम से संदर्भित करने की अनुमति देता है: `#!groovy row.reads_bam` प्रत्येक row के `reads_bam` column से फ़ाइल पथ extract करता है।
...
जब तुम तरोताज़ा महसूस करो, तो [भाग 3](./03_joint_calling.md) पर जाओ, जहाँ तुम सीखोगे कि डेटा पर joint वेरिएंट कॉलिंग लागू करने के लिए इस simple प्रति-नमूना वेरिएंट कॉलिंग workflow को कैसे transform करें।
