...
इस कोर्स के लिए, हम [Babraham Institute](https://www.babraham.ac.uk/) में डॉ. साइमन एंड्रयूज और लौरा बिगिन्स द्वारा [यहाँ](https://www.bioinformatics.babraham.ac.uk/training/RNASeq_Course/Analysing%20RNA-Seq%20data%20Exercise.pdf) वर्णित विधि का पालन कर रहे हैं।

हमारा लक्ष्य एक workflow विकसित करना है जो निम्नलिखित प्रोसेसिंग चरणों को लागू करता है: बल्क RNAseq नमूने में reads पर प्रारंभिक quality control चलाना, reads से adapter sequences को ट्रिम करना, reads को reference genome के साथ align करना, और एक व्यापक quality control (QC) रिपोर्ट तैयार करना।
...
- **TRIM_GALORE:** Trim Galore का उपयोग करके adapter sequences को ट्रिम करें और ट्रिमिंग के बाद QC करें (Cutadapt और FastQC को बंडल करता है)
- **HISAT2_ALIGN:** Hisat2 का उपयोग करके reads को reference genome के साथ align करें
- **MULTIQC:** MultiQC का उपयोग करके एक व्यापक QC रिपोर्ट जेनरेट करें

हालाँकि, इससे पहले कि हम कोई भी workflow कोड लिखना शुरू करें, हम कुछ टेस्ट डेटा पर कमांड्स को मैनुअल रूप से आज़माने जा रहे हैं।
हमें जिन टूल्स की आवश्यकता है वे GitHub Codespaces वातावरण में इंस्टॉल नहीं हैं, इसलिए हम उन्हें containers के माध्यम से उपयोग करेंगे ([Hello Containers](../../hello_nextflow/05_hello_containers.md) देखें)।

!!! note "नोट"

     सुनिश्चित करें कि तुम `nf4-science/rnaseq` डायरेक्टरी में हो। जब तुम `pwd` टाइप करते हो तो दिखाए गए पथ का अंतिम भाग `rnaseq` होना चाहिए।

---

## 1. प्रारंभिक QC और adapter ट्रिमिंग

हम एक container image pull करने जा रहे हैं जिसमें `fastqc` और `trim_galore` दोनों इंस्टॉल हैं, इसे इंटरैक्टिव रूप से स्पिन करेंगे और उदाहरण डेटा फ़ाइलों में से एक पर ट्रिमिंग और QC कमांड्स चलाएंगे।

### 1.1. Container को pull करें

```bash
docker pull community.wave.seqera.io/library/trim-galore:0.6.10--1bf8ca4e1967cd18
```

यह निम्नलिखित console output देता है जब सिस्टम image डाउनलोड करता है:
...
### 1.2. Container को इंटरैक्टिव रूप से स्पिन करें
...
तुम्हारा prompt `(base) root@b645838b3314:/tmp#` जैसा कुछ बदल जाएगा, जो दर्शाता है कि तुम अब container के अंदर हो।

कमांड का `-v ./data:/data` भाग हमें container के अंदर से `data/` डायरेक्टरी की सामग्री तक पहुँचने में सक्षम करेगा।
...
चलो read डेटा पर quality control मेट्रिक्स एकत्र करने के लिए `fastqc` चलाते हैं।
...
### 1.4. `trim_galore` के साथ adapter sequences को ट्रिम करें

अब चलो `trim_galore` चलाते हैं, जो Cutadapt और FastQC को बंडल करता है, adapter sequences को ट्रिम करने और ट्रिमिंग के बाद QC मेट्रिक्स एकत्र करने के लिए।

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

`--fastqc` फ्लैग कमांड को ट्रिमिंग पूर्ण होने के बाद स्वचालित रूप से एक QC संग्रह चरण चलाने का कारण बनता है।
...
### 1.5. आउटपुट फ़ाइलों को container के बाहर filesystem में ले जाएं

जो कुछ भी container के अंदर रहता है वह भविष्य के काम के लिए दुर्गम होगा इसलिए चलो इन्हें एक नई डायरेक्टरी में ले जाते हैं।
...
## 2. Reads को reference genome के साथ align करें

हम एक container image pull करने जा रहे हैं जिसमें `hisat2` इंस्टॉल है, इसे इंटरैक्टिव रूप से स्पिन करेंगे और RNAseq डेटा को reference genome के साथ align करने के लिए alignment कमांड चलाएंगे।

### 2.1. `hisat2` container को pull करें
...
### 2.2. `hisat2` container को इंटरैक्टिव रूप से स्पिन करें

```bash
docker run -it -v ./data:/data community.wave.seqera.io/library/hisat2_samtools:5e49f68a37dc010e
```

कमांड पहले जैसी ही है, प्रासंगिक container URI के साथ स्वैप की गई है।

### 2.3. Hisat2 genome index फ़ाइलें बनाएं

Hisat2 को genome reference को एक बहुत विशिष्ट प्रारूप में प्रदान करने की आवश्यकता होती है, और यह केवल `genome.fa` FASTA फ़ाइल का उपभोग नहीं कर सकता जो हम प्रदान करते हैं, इसलिए हम प्रासंगिक संसाधन बनाने के लिए इस अवसर का लाभ उठाने जा रहे हैं।
...
हम इनका उपयोग एक क्षण में करेंगे, लेकिन पहले चलो इन genome index फ़ाइलों के साथ एक gzipped tarball जेनरेट करते हैं; हमें बाद में इनकी आवश्यकता होगी और इन्हें जेनरेट करना आमतौर पर कुछ ऐसा नहीं है जो हम workflow के हिस्से के रूप में करना चाहते हैं।

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

यह हमारे filesystem पर `data/` डायरेक्टरी में genome index फ़ाइलों वाला एक `genome_index.tar.gz` tarball स्टोर करता है, जो इस कोर्स के भाग 2 में काम आएगा।

### 2.4. `hisat2` कमांड चलाएं

अब हम alignment कमांड चला सकते हैं, जो `hisat2` के साथ alignment चरण करता है फिर आउटपुट को BAM फ़ाइल के रूप में लिखने के लिए `samtools` को पाइप करता है।

read डेटा इनपुट `/data/trimmed/ENCSR000COQ1_1_trimmed.fq.gz` फ़ाइल है जिसे हमने पिछले चरण में `trim_galore` के साथ जेनरेट किया था।
...
वास्तविक पैमाने पर इसमें बहुत अधिक समय लग सकता है।
...
### 2.5. आउटपुट फ़ाइलों को container के बाहर filesystem में ले जाएं
...
## 3. एक व्यापक QC रिपोर्ट जेनरेट करें

हम एक container image pull करने जा रहे हैं जिसमें `multiqc` इंस्टॉल है, इसे इंटरैक्टिव रूप से स्पिन करेंगे और before/after FastQC रिपोर्ट फ़ाइलों पर एक रिपोर्ट जेनरेशन कमांड चलाएंगे।

### 3.1. `multiqc` container को pull करें
...
### 3.2. `multiqc` container को इंटरैक्टिव रूप से स्पिन करें
...
MultiQC संगत QC रिपोर्ट्स के लिए डायरेक्टरियों में खोज करने में सक्षम है और जो कुछ भी मिलता है उसे एकत्रित करेगा।

यहाँ हम देखते हैं कि टूल ने हमारे द्वारा जेनरेट की गई सभी तीन QC रिपोर्ट्स पाईं: प्रारंभिक QC जो हमने `fastqc` के साथ की, ट्रिमिंग के बाद की रिपोर्ट `cutadapt` से (जो `trim_galore` के माध्यम से बनाई गई) और alignment के बाद की QC जो `hisat2` द्वारा उत्पादित की गई।
...
### 3.4. आउटपुट फ़ाइलों को container के बाहर filesystem में ले जाएं
...
तुमने सभी व्यक्तिगत कमांड्स को प्रासंगिक containers में इंटरैक्टिव रूप से परीक्षण किया है।

### आगे क्या है?

सीखो कि उन्हीं कमांड्स को एक बहु-चरणीय workflow में कैसे लपेटें जो काम को निष्पादित करने के लिए containers का उपयोग करता है।
