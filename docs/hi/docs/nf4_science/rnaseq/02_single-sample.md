...
`rnaseq.nf` में, `Pipeline parameters` section के तहत, `reads` नाम का एक पैरामीटर `Path` type के साथ declare करो।
...
`nextflow.config` में एक `profiles` block जोड़ें जिसमें एक `test` profile हो जो `reads` पैरामीटर को test FASTQ फ़ाइलों में से एक पर सेट करता है।
...
जब तुम refreshed महसूस करो, तो [भाग 3](./03_multi-sample.md) पर जाओ, जहां तुम सीखोगे कि कैसे workflow को कई नमूनों को parallel में प्रोसेस करने के लिए modify करें, सभी नमूनों के लिए सभी steps में QC reports को aggregate करें, और paired-end RNAseq data पर workflow चलाना enable करें।
