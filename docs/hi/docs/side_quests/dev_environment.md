...
- **बुनियादी वर्कफ़्लो संरचना**: प्रोसेस, वर्कफ़्लो, और वे कैसे एक साथ जुड़ते हैं, यह समझना
...
- **कॉन्फ़िगरेशन बुनियादी बातें**: पैरामीटर, प्रोसेस निर्देश, और प्रोफ़ाइल के लिए `nextflow.config` का उपयोग करना
...
!!! info "IDE फ़ीचर्स अब क्यों?"
    ...
    तुम संभवतः [Hello Nextflow](../hello_nextflow/) कोर्स के दौरान पहले से ही VS Code का उपयोग कर रहे हो, लेकिन हमने IDE फ़ीचर्स के बजाय Nextflow बुनियादी बातों को सीखने पर ध्यान केंद्रित रखा। अब जब तुम प्रोसेस, वर्कफ़्लो, चैनल, और मॉड्यूल जैसी बुनियादी Nextflow अवधारणाओं के साथ सहज हो, तो तुम उन परिष्कृत IDE फ़ीचर्स का लाभ उठाने के लिए तैयार हो जो तुम्हें अधिक कुशल डेवलपर बनाएंगे।
...
!!! note "उदाहरण फ़ाइलों के बारे में"
    ...
    - `basic_workflow.nf` एक कार्यशील बुनियादी वर्कफ़्लो है जिसे तुम चला और संशोधित कर सकते हो
...
चूंकि तुम Hello Nextflow के दौरान VS Code का उपयोग कर रहे हो, तो तुम पहले से ही बुनियादी बातों से परिचित हो। यहाँ बताया गया है कि इस सत्र के लिए अपने वर्कस्पेस को कुशलता से कैसे व्यवस्थित करें:
...
- **चैनल ऑपरेशन**: डेटा ट्रांसफ़ॉर्मेशन स्टेप
- **कॉन्फ़िगरेशन निर्देश**: प्रोसेस-विशिष्ट सेटिंग्स

यह दृश्य संगठन अमूल्य हो जाता है जब कई प्रोसेस और जटिल डेटा फ़्लो वाले जटिल वर्कफ़्लो के साथ काम कर रहे हो।
...
VS Code की ऑटो-कम्प्लीशन फ़ीचर्स संदर्भ के आधार पर उपयुक्त विकल्प सुझाकर तुम्हें तेज़ी से और कम त्रुटियों के साथ कोड लिखने में मदद करती हैं।
...
`basic_workflow.nf` फिर से खोलो और वर्कफ़्लो ब्लॉक में `channel.` टाइप करने का प्रयास करो:

![Channel auto-completion](img/autocomplete_channel.png)

तुम्हें सुझाव दिखाई देंगे:
...
चलो पहचान को क्रिया में देखने के लिए जानबूझकर एक त्रुटि बनाएं। `basic_workflow.nf` खोलो और प्रोसेस नाम को `FASTQC` से `FASTQ` (या किसी अन्य अमान्य नाम) में बदलो। VS Code तुरंत वर्कफ़्लो ब्लॉक में लाल लहरदार अंडरलाइन के साथ त्रुटि को हाइलाइट करेगा:
...
Nextflow लैंग्वेज सर्वर इन मुद्दों को Problems पैनल में हाइलाइट करता है। तुम पाइपलाइन चलाते समय सिंटैक्स त्रुटियों से बचने के लिए इन्हें जल्दी जांच सकते हो।
...
यही चीज़ प्रोसेस नाम के लिए काम करती है। `basic_workflow.nf` पर वापस जाओ और वर्कफ़्लो ब्लॉक में `FASTQC` प्रोसेस नाम पर इसे आज़माओ। यह तुम्हें सीधे प्रोसेस नाम से लिंक करता है (जो इस उदाहरण में मॉड्यूल फ़ाइल के समान है, लेकिन बहुत बड़ी फ़ाइल के बीच में हो सकता है)।
...
`complex_workflow.nf` अभी भी खुली होने के साथ, तुम VS Code के शीर्ष पर सर्च बार में `@` टाइप करके फ़ाइल में सभी सिंबल का अवलोकन प्राप्त कर सकते हो (कीबोर्ड शॉर्टकट `Ctrl/Cmd+Shift+O` है, लेकिन Codespaces में काम नहीं कर सकता)। यह सिंबल नेविगेशन पैनल खोलता है, जो वर्तमान फ़ाइल में सभी सिंबल को सूचीबद्ध करता है:
...
यह सिर्फ 'एंट्री' वर्कफ़्लो है, लेकिन तुम आगे वर्कफ़्लो `RNASEQ_PIPELINE {` के ऊपर "Preview DAG" बटन पर क्लिक करके आंतरिक वर्कफ़्लो के लिए DAG का पूर्वावलोकन भी कर सकते हो:
...
- वर्कफ़्लो कॉल लिखते समय मॉड्यूल इंटरफ़ेस की जांच करना, और पूर्वावलोकन पर्याप्त नहीं है
...
- मॉड्यूल में स्थिरता सुनिश्चित करना
...
उचित कोड फ़ॉर्मेटिंग न केवल सौंदर्यशास्त्र के लिए बल्कि पठनीयता, समझ, और जटिल वर्कफ़्लो को अपडेट करने की सुविधा को बढ़ाने के लिए भी आवश्यक है।

### 6.1. क्रिया में स्वचालित फ़ॉर्मेटिंग
...
- कुछ लाइनों को अजीब तरीके से तोड़ो: `.view {` ऑपरेटर और `Processing sample:` स्ट्रिंग के बीच एक नई लाइन जोड़ो लेकिन क्लोज़िंग पैरेंथेसिस `}` से पहले संबंधित नई लाइन न जोड़ो।
...
- अनावश्यक व्हाइटस्पेस हटाता है
- पठनीय लाइन ब्रेक बनाए रखता है

ध्यान दो कि स्वचालित फ़ॉर्मेटिंग हर कोड स्टाइल मुद्दे को हल नहीं कर सकती। Nextflow लैंग्वेज सर्वर तुम्हारे कोड को साफ रखने का लक्ष्य रखता है, लेकिन यह कुछ क्षेत्रों में तुम्हारी व्यक्तिगत प्राथमिकताओं का भी सम्मान करता है। उदाहरण के लिए, यदि तुम प्रोसेस के `script` ब्लॉक के अंदर इंडेंटेशन हटाते हो, तो फ़ॉर्मेटर इसे जैसा है वैसा ही छोड़ देगा, क्योंकि तुम जानबूझकर उस स्टाइल को पसंद कर सकते हो।
...
कोड को अनकमेंट करने के लिए फिर से **Ctrl+/** (या **Cmd+/**) का उपयोग करो।

#### अवलोकन के लिए कोड फ़ोल्डिंग

`complex_workflow.nf` में, प्रोसेस परिभाषाओं के बगल में छोटे तीर देखो। प्रोसेस को फ़ोल्ड (कोलैप्स) करने के लिए उन पर क्लिक करो:

![Code folding](img/code_folding.png)

यह तुम्हें कार्यान्वयन विवरण में खोए बिना तुम्हारे वर्कफ़्लो संरचना का उच्च-स्तरीय अवलोकन देता है।
...
#### मल्टी-लाइन चयन और संपादन

कई लाइनों को एक साथ संपादित करने के लिए, VS Code शक्तिशाली मल्टी-कर्सर क्षमताएं प्रदान करता है:

- **मल्टी-लाइन चयन**: **Ctrl+Alt** (या MacOS के लिए **Cmd+Option**) दबाए रखो और कई लाइनों का चयन करने के लिए एरो कुंजियों का उपयोग करो
- **मल्टी-लाइन इंडेंटिंग**: कई लाइनों का चयन करो और पूरे ब्लॉक को इंडेंट करने के लिए **Tab** या आउटडेंट करने के लिए **Shift+Tab** का उपयोग करो

यह विशेष रूप से इसके लिए उपयोगी है:

- पूरे प्रोसेस ब्लॉक को लगातार इंडेंट करना
- एक साथ कई लाइनों में कमेंट जोड़ना
- कई प्रोसेस में समान पैरामीटर परिभाषाओं को संपादित करना

### सारांश

तुम जटिल वर्कफ़्लो को कुशलता से व्यवस्थित करने के लिए स्वचालित फ़ॉर्मेटिंग, कमेंटिंग फ़ीचर्स, कोड फ़ोल्डिंग, ब्रैकेट मैचिंग, और मल्टी-लाइन संपादन का उपयोग करके साफ, पठनीय कोड बनाए रख सकते हो।
...
- इनलाइन diff व्यू
- कमिट और पुश क्षमताएं

git परिवर्तन देखने और सीधे एडिटर में कमिट स्टेज करने के लिए source control बटन (![Source control icon](img/source_control_icon.png)) (`Ctrl+Shift+G` या `Cmd+Shift+G` यदि तुम VSCode के साथ लोकली काम कर रहे हो) का उपयोग करके Source Control पैनल खोलो।

![Source Control Panel](img/source_control.png)

### 7.2. वर्कफ़्लो चलाना और निरीक्षण करना

चलो एक वर्कफ़्लो चलाएं और फिर परिणामों का निरीक्षण करें। इंटीग्रेटेड टर्मिनल (`Ctrl+Shift+` backtick Windows और MacOS दोनों में) में, बुनियादी वर्कफ़्लो चलाओ:
...
2. **लॉग फ़ाइलें खोलो**: उन्हें सीधे VS Code में खोलने के लिए टर्मिनल आउटपुट में लॉग फ़ाइल पथ पर क्लिक करो
3. **आउटपुट का निरीक्षण करो**: फ़ाइल एक्सप्लोरर में प्रकाशित परिणाम डायरेक्टरी ब्राउज़ करो
4. **निष्पादन रिपोर्ट देखो**: HTML रिपोर्ट को सीधे VS Code या अपने ब्राउज़र में खोलो

यह कई एप्लिकेशन के बीच स्विच करने के बजाय सब कुछ एक जगह रखता है।

### सारांश

तुम एक ही इंटरफ़ेस से अपनी पूरी विकास प्रक्रिया को प्रबंधित करने के लिए वर्जन कंट्रोल और वर्कफ़्लो निष्पादन के साथ VS Code को एकीकृत कर सकते हो।
...
1. एक साथ सभी त्रुटियों को देखने के लिए **Problems पैनल** (`Ctrl+Shift+M` या `Cmd+Shift+M`)
...
4. समान पैटर्न या मुद्दों को खोजने के लिए **प्रोजेक्ट-व्यापी सर्च**

### 8.3. रिफ़ैक्टरिंग और सुधार

1. पैटर्न खोजने के लिए **प्रोजेक्ट-व्यापी सर्च** (`Ctrl+Shift+F` या `Cmd+Shift+F`)
2. स्थिरता बनाए रखने के लिए **ऑटो-फ़ॉर्मेटिंग** (`Shift+Alt+F` या `Shift+Option+F`)
...
तुमने अब Nextflow विकास के लिए VS Code के IDE फ़ीचर्स का एक त्वरित दौरा किया है। ये टूल तुम्हें निम्नलिखित द्वारा काफी अधिक उत्पादक बनाएंगे:

- रियल-टाइम सिंटैक्स जांच के माध्यम से **त्रुटियों को कम करना**
...
इन IDE फ़ीचर्स की वास्तविक शक्ति तब उभरती है जब तुम बड़े, अधिक जटिल प्रोजेक्ट पर काम करते हो। उन्हें धीरे-धीरे अपने वर्कफ़्लो में शामिल करना शुरू करो—कुछ सत्रों के भीतर, वे दूसरी प्रकृति बन जाएंगे और तुम Nextflow विकास को कैसे देखते हो इसे बदल देंगे।

त्रुटियों को पकड़ने से पहले वे तुम्हें धीमा करें से लेकर जटिल कोडबेस को आसानी से नेविगेट करने तक, ये टूल तुम्हें अधिक आत्मविश्वासी और कुशल डेवलपर बनाएंगे।
