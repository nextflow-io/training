...
जैसा कि आप देख सकते हैं, यह **process** वर्तमान में एक इनपुट फ़ाइल (जिसमें प्रदर्शित किया जाने वाला text है) और एक `character` value लेने के लिए डिज़ाइन किया गया है, जो आमतौर पर **workflow** स्तर पर एक command-line पैरामीटर द्वारा प्रदान किया जाता है।

### 3.2. meta map फ़ील्ड को इनपुट के रूप में पास करें

जब हमने Hello Nextflow course में `cowpy` tool का उपयोग किया था, तो हमने यह निर्धारित करने के लिए एक command-line पैरामीटर का उपयोग किया था कि अंतिम image draw करने के लिए किस character का उपयोग करना है।
यह समझ में आया, क्योंकि हम pipeline के प्रति run केवल एक image generate कर रहे थे।

हालाँकि, इस tutorial में, हम प्रत्येक subject के लिए एक उपयुक्त image generate करना चाहते हैं जिसे हम process कर रहे हैं, इसलिए एक command-line पैरामीटर का उपयोग करना बहुत सीमित होगा।

अच्छी खबर: हमारे पास हमारी datasheet में एक `character` column है और इसलिए, हमारे meta map में।
आइए इसका उपयोग करें ताकि **process** को प्रत्येक entry के लिए किस character का उपयोग करना चाहिए यह सेट कर सकें।

इस उद्देश्य के लिए, हमें तीन चीजें करने की आवश्यकता होगी:

1. पिछले **process** से निकलने वाले आउटपुट **channel** को एक नाम दें ताकि हम इस पर अधिक सुविधाजनक रूप से operate कर सकें।
2. निर्धारित करें कि रुचि की जानकारी तक कैसे पहुँचें
3. दूसरे **process** में एक call जोड़ें और जानकारी को उचित रूप से feed करें।

आइए शुरू करें।

#### 3.2.1. पिछले आउटपुट **channel** को नाम दें

हमने पिछले manipulations को सीधे पहले **process**, `IDENTIFY_LANGUAGE.out` के आउटपुट **channel** पर लागू किया।
उस **channel** की सामग्री को अगले **process** में feed करने के लिए (और ऐसा करने के लिए जो स्पष्ट और पढ़ने में आसान हो) हम इसे अपना नाम देना चाहते हैं, `ch_languages`।

हम [`set`](https://www.nextflow.io/docs/latest/reference/operator.html#set) ऑपरेटर का उपयोग करके ऐसा कर सकते हैं।

मुख्य **workflow** में, `.view()` ऑपरेटर को `.set { ch_languages }` से बदलें, और एक लाइन जोड़ें जो test करे कि हम **channel** को नाम से refer कर सकते हैं।
...
हम मॉड्यूल कोड को देखने से जानते हैं कि `COWPY` **process** को एक text फ़ाइल और एक `character` value दिया जाना अपेक्षित है।
`COWPY` **process** call लिखने के लिए, हमें बस यह जानने की आवश्यकता है कि **channel** में प्रत्येक element से संबंधित फ़ाइल object और मेटाडेटा को कैसे extract करें।

जैसा कि अक्सर होता है, ऐसा करने का सबसे सरल तरीका `map` operation का उपयोग करना है।

हमारे **channel** में `[meta, file]` के रूप में structured tuples हैं, इसलिए हम `file` object को सीधे access कर सकते हैं, और हम meta map के अंदर stored `character` value को `meta.character` के रूप में refer करके access कर सकते हैं।

मुख्य **workflow** में, निम्नलिखित कोड परिवर्तन करें:
...
_आपके आउटपुट में फ़ाइल paths और character values एक अलग क्रम में आ सकते हैं।_
...
अब आइए इसे सब एक साथ रखें और वास्तव में `ch_languages` **channel** पर `COWPY` **process** को call करें।
...
आप देख सकते हैं कि हम बस दो map operations (minus the `.view()` statements) को **process** call के inputs के रूप में copy करते हैं।
बस सुनिश्चित करें कि आप उनके बीच comma न भूलें!
...
यदि आप results डायरेक्टरी में देखें, तो आपको प्रत्येक अभिवादन के ASCII art युक्त व्यक्तिगत फ़ाइलें दिखनी चाहिए जो संबंधित character द्वारा बोली गई हैं।
...
हालाँकि, जैसा कि ऊपर उल्लेख किया गया है, शामिल कुछ कोड थोड़ा clunky था, क्योंकि हमें **workflow** body के संदर्भ में रहते हुए meta data को unpack करना पड़ा।
यह approach कुछ फ़ील्ड्स को meta map से उपयोग करने के लिए ठीक काम करता है, लेकिन यदि हम बहुत अधिक उपयोग करना चाहते हैं तो खराब तरीके से scale होगा।

एक और ऑपरेटर है जिसे `multiMap()` कहा जाता है जो हमें इसे थोड़ा streamline करने की अनुमति देता है, लेकिन फिर भी यह आदर्श नहीं है।

??? info "(वैकल्पिक) `multiMap()` के साथ वैकल्पिक संस्करण"

    यदि आप सोच रहे हैं, तो हम केवल एक single `map()` operation नहीं लिख सकते थे जो `file` और `character` दोनों को आउटपुट करता है, क्योंकि वह उन्हें एक tuple के रूप में return करता।
    हमें `file` और `character` elements को **process** में अलग से feed करने के लिए दो अलग `map()` operations लिखने की आवश्यकता थी।

    तकनीकी रूप से एक single mapping operation के माध्यम से ऐसा करने का एक और तरीका है, `multiMap()` ऑपरेटर का उपयोग करते हुए, जो कई channels emit करने में सक्षम है।
    उदाहरण के लिए, आप ऊपर `COWPY` की call को निम्नलिखित कोड से बदल सकते हैं:
...
किसी भी मामले में, यह अजीब है कि हमें **workflow** स्तर पर कुछ unpacking करनी पड़ी।

यह बेहतर होगा यदि हम पूरे meta map को **process** में feed कर सकें और वहाँ पहुँचने के बाद हमें जो चाहिए वह चुन सकें।

### 3.3. पूरे meta map को पास करें और उपयोग करें

meta map का बिंदु आखिरकार सभी मेटाडेटा को एक bundle के रूप में एक साथ पास करना है।
एकमात्र कारण जिससे हम ऊपर ऐसा नहीं कर सके वह यह है कि **process** meta map को स्वीकार करने के लिए सेट नहीं है।
लेकिन चूंकि हम **process** कोड को control करते हैं, हम इसे बदल सकते हैं।

आइए `COWPY` **process** को `[meta, file]` tuple structure को स्वीकार करने के लिए modify करें जिसे हमने पहले **process** में उपयोग किया था ताकि हम **workflow** को streamline कर सकें।

इस उद्देश्य के लिए, हमें तीन चीजें करने की आवश्यकता होगी:

1. `COWPY` **process** मॉड्यूल के इनपुट definitions को modify करें
2. meta map का उपयोग करने के लिए **process** command को update करें
3. **workflow** body में **process** call को update करें

तैयार? चलो चलें!

#### 3.3.1. `COWPY` मॉड्यूल इनपुट को modify करें

`cowpy.nf` मॉड्यूल फ़ाइल में निम्नलिखित संपादन करें:
...
यह हमें `[meta, file]` tuple structure का उपयोग करने में सक्षम बनाता है जिसे हमने tutorial में पहले कवर किया था।

ध्यान दें कि हमने meta map को आउटपुट करने के लिए **process** आउटपुट definition को update नहीं किया, tutorial को streamlined रखने के लिए, लेकिन `IDENTIFY_LANGUAGE` **process** के model का पालन करते हुए स्वयं ऐसा करने के लिए स्वतंत्र महसूस करें।

#### 3.3.2. meta map फ़ील्ड का उपयोग करने के लिए command को update करें
...
हमने पहले standalone इनपुट के रूप में पास की गई `character` value के reference को meta map में held value से बदल दिया है, जिसे हम `meta.character` का उपयोग करके refer करते हैं।

अब आइए तदनुसार **process** call को update करें।

#### 3.3.3. **process** call को update करें और इसे चलाएं

**process** अब अपने इनपुट को `[meta, file]` tuple structure का उपयोग करने की उम्मीद करता है, जो कि पिछला **process** आउटपुट करता है, इसलिए हम बस पूरे `ch_languages` **channel** को `COWPY` **process** में पास कर सकते हैं।
...
आइए पिछले execution के results को delete करें और इसे चलाएं:
...
यदि आप results डायरेक्टरी में देखें, तो आपको पहले की तरह ही outputs दिखने चाहिए, _यानी_ प्रत्येक अभिवादन के ASCII art युक्त व्यक्तिगत फ़ाइलें जो संबंधित character द्वारा बोली गई हैं।
...
बेशक, यह मानता है कि आप **process** कोड को modify करने में सक्षम हैं।
कुछ मामलों में, आपको मौजूदा processes पर भरोसा करना पड़ सकता है जिन्हें आप modify करने के लिए स्वतंत्र नहीं हैं, जो आपके options को सीमित करता है।
अच्छी खबर, यदि आप [nf-core](https://nf-co.re/) project से modules का उपयोग करने की योजना बना रहे हैं, तो यह है कि nf-core modules सभी `[meta, file]` tuple structure को एक standard के रूप में उपयोग करने के लिए सेट हैं।

### 3.4. लापता आवश्यक inputs की troubleshooting

`character` value `COWPY` **process** को सफलतापूर्वक चलाने के लिए आवश्यक है।
यदि हम configuration फ़ाइल में इसके लिए एक default value सेट नहीं करते हैं, तो हमें datasheet में इसके लिए एक value प्रदान करनी चाहिए।

**यदि हम नहीं करते हैं तो क्या होता है?**
यह इस बात पर निर्भर करता है कि इनपुट datasheet में क्या है और हम **workflow** का कौन सा संस्करण चला रहे हैं।

#### 3.4.1. character column मौजूद है लेकिन खाली है

मान लीजिए हम डेटा संग्रह त्रुटि को simulate करने के लिए अपनी datasheet में entries में से एक के लिए character value को delete करते हैं:
...
**workflow** के किसी भी संस्करण के लिए जिसे हमने ऊपर उपयोग किया है, `character` key सभी entries के लिए बनाई जाएगी जब datasheet को पढ़ा जाएगा, लेकिन `sampleA` के लिए value एक empty string होगी।

यह एक त्रुटि का कारण बनेगा।
...
जब Nextflow उस sample के लिए `cowpy` command line चलाता है, तो `${meta.character}` को `cowpy` command line में एक empty string से भरा जाता है, इसलिए `cowpy` tool एक त्रुटि फेंकता है जो कहता है कि `-c` argument के लिए कोई value प्रदान नहीं की गई थी।

#### 3.4.2. character column datasheet में मौजूद नहीं है

अब मान लीजिए हम अपनी datasheet से `character` column को पूरी तरह से delete करते हैं:
...
इस मामले में `character` key बिल्कुल नहीं बनाई जाएगी जब datasheet को पढ़ा जाएगा।

##### 3.4.2.1. **workflow** स्तर पर value तक पहुँच

यदि हम कोड के उस संस्करण का उपयोग कर रहे हैं जो हमने section 3.2 में लिखा था, तो Nextflow `COWPY` **process** को call करने से पहले meta map में `character` key तक पहुँचने का प्रयास करेगा।

यह कोई elements नहीं पाएगा जो instruction से match करते हैं, इसलिए यह `COWPY` को बिल्कुल नहीं चलाएगा।
...
जहाँ तक Nextflow का संबंध है, यह **workflow** सफलतापूर्वक चला!
हालाँकि, हम जो outputs चाहते हैं उनमें से कोई भी उत्पन्न नहीं होगा।

##### 3.4.2.2. **process** स्तर पर value तक पहुँच

यदि हम section 3.3 में संस्करण का उपयोग कर रहे हैं, तो Nextflow पूरे meta map को `COWPY` **process** में पास करेगा और command चलाने का प्रयास करेगा।

यह एक त्रुटि का कारण बनेगा, लेकिन पहले मामले की तुलना में एक अलग।
...
यह इसलिए होता है क्योंकि `meta.character` मौजूद नहीं है, इसलिए इसे access करने का हमारा प्रयास `null` return करता है। परिणामस्वरूप, Nextflow शाब्दिक रूप से command-line में `null` plug करता है, जो बेशक `cowpy` tool द्वारा पहचाना नहीं जाता है।

#### 3.4.3. समाधान

**workflow** configuration के हिस्से के रूप में एक default value supply करने के अलावा, हम इसे अधिक मजबूती से संभालने के लिए दो चीजें कर सकते हैं:

1. अपने **workflow** में इनपुट validation लागू करें ताकि यह सुनिश्चित हो सके कि datasheet में सभी आवश्यक जानकारी है। आप Hello nf-core training course में [input validation का परिचय](../hello_nf-core/05_input_validation.md) पा सकते हैं। <!-- TODO (future) pending a proper Validation side quest -->

2. यदि आप सुनिश्चित करना चाहते हैं कि कोई भी जो आपके **process** मॉड्यूल का उपयोग करता है वह तुरंत आवश्यक inputs की पहचान कर सके, तो आप आवश्यक मेटाडेटा property को एक explicit इनपुट भी बना सकते हैं।

यहाँ एक उदाहरण है कि यह कैसे काम करेगा।

सबसे पहले, **process** स्तर पर, इनपुट definition को निम्नानुसार update करें:
...
फिर, **workflow** स्तर पर, मेटाडेटा से `character` property को extract करने और इसे इनपुट tuple का एक explicit component बनाने के लिए एक mapping operation का उपयोग करें:
...
इस approach का लाभ यह है कि यह explicitly दिखाता है कि `character` आवश्यक है, और **process** को अन्य contexts में redeploy करना आसान बनाता है।
...
meta map **channel** structures को साफ रखने और arbitrary **channel** structures को रोकने के लिए उत्कृष्ट है, लेकिन अनिवार्य elements के लिए जो सीधे एक **process** में referenced हैं, उन्हें explicit inputs के रूप में extract करना अधिक मजबूत और maintainable कोड बनाता है।

### सारांश

इस खंड में, आपने सीखा कि **process** के execution को customize करने के लिए मेटाडेटा का उपयोग कैसे करें, इसे **workflow** स्तर पर या **process** स्तर पर access करते हुए।

---

## पूरक अभ्यास

यदि आप **process** के अंदर से meta map जानकारी का उपयोग करने का अभ्यास करना चाहते हैं, तो meta map से अन्य जानकारी के टुकड़ों जैसे `lang` और `lang_group` का उपयोग करके outputs को नाम दिया जाता है और/या व्यवस्थित किया जाता है यह customize करने का प्रयास करें।

उदाहरण के लिए, यह परिणाम उत्पन्न करने के लिए कोड को modify करने का प्रयास करें:
...
इस side quest में, आपने Nextflow workflows में मेटाडेटा के साथ प्रभावी ढंग से काम करने का तरीका खोजा है।

मेटाडेटा को explicit और डेटा से जुड़ा रखने का यह pattern Nextflow में एक मुख्य best practice है, जो कई लाभ प्रदान करता है hardcoding फ़ाइल जानकारी पर:

- फ़ाइल मेटाडेटा पूरे **workflow** में फ़ाइलों से जुड़ा रहता है
- **Process** व्यवहार को प्रति फ़ाइल customized किया जा सकता है
- आउटपुट organization फ़ाइल मेटाडेटा को reflect कर सकता है
- pipeline execution के दौरान फ़ाइल जानकारी को expanded किया जा सकता है

अपने स्वयं के काम में इस pattern को लागू करने से आप मजबूत, maintainable bioinformatics workflows बनाने में सक्षम होंगे।

### मुख्य patterns

1.  **मेटाडेटा को पढ़ना और Structuring करना:** CSV फ़ाइलों को पढ़ना और organized मेटाडेटा मैप्स बनाना जो आपकी डेटा फ़ाइलों से जुड़े रहते हैं।
...
2.  **Workflow के दौरान मेटाडेटा को Expanding करना** आपकी pipeline आगे बढ़ने के साथ अपने मेटाडेटा में नई जानकारी जोड़ना **process** outputs जोड़कर और conditional logic के माध्यम से values derive करके।

    - **process** आउटपुट के आधार पर नई keys जोड़ना
...
3.  **Process व्यवहार को Customizing करना:** **process** के अंदर मेटाडेटा का उपयोग करना।
...
[Side Quests के menu](./index.md) पर वापस जाएं या सूची में अगले विषय पर जाने के लिए पृष्ठ के निचले दाएं कोने में बटन पर क्लिक करें।
