...
बुनियादी फ़ाइल जानकारी वाली एक साधारण datasheet (जिसे अक्सर bioinformatics में samplesheet कहा जाता है) से शुरू करते हुए, तुम सीखोगे कि कैसे:
...
ये कौशल तुम्हें अधिक मजबूत और लचीली pipelines बनाने में मदद करेंगे जो जटिल फ़ाइल संबंधों और प्रोसेसिंग आवश्यकताओं को संभाल सकते हैं।
...
यदि तुमने अभी तक ऐसा नहीं किया है, तो [Environment Setup](../envsetup/index.md) में वर्णित प्रशिक्षण वातावरण को खोलना सुनिश्चित करो।

[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/nextflow-io/training?quickstart=1&ref=master)

#### प्रोजेक्ट डायरेक्टरी में जाएं

आओ उस डायरेक्टरी में चलें जहाँ इस tutorial के लिए फ़ाइलें स्थित हैं।
...
`main.nf` फ़ाइल में workflow एक stub है जिसे तुम धीरे-धीरे एक पूर्ण रूप से कार्यशील workflow में विस्तारित करोगे।
...
यदि तुम सभी boxes को चेक कर सकते हो, तो तुम जाने के लिए तैयार हो।
...
तुम देख सकते हो कि हमने उदाहरण datasheet को फ़ाइल के रूप में लोड करने के लिए एक बुनियादी channel factory सेट अप किया है, लेकिन यह अभी तक फ़ाइल की सामग्री नहीं पढ़ेगा।
आओ इसे जोड़कर शुरू करें।
...
channel निर्माण कोड में `splitCsv()` operation जोड़ने के लिए निम्नलिखित परिवर्तन करो, साथ ही यह जांचने के लिए `view()` operation भी कि फ़ाइल की सामग्री channel में सही ढंग से लोड हो रही है।
...
ध्यान दो कि हम Nextflow को CSV फ़ाइल की पहली पंक्ति को header पंक्ति के रूप में पढ़ने के लिए `header: true` option का उपयोग कर रहे हैं।

आओ देखें कि इससे क्या निकलता है, ठीक है?
workflow चलाओ:
...
??? info "(वैकल्पिक) मैप्स के बारे में अधिक"
    ...
    यहाँ एक runnable script है जो दिखाता है कि तुम व्यवहार में एक map कैसे परिभाषित कर सकते हो और इसकी सामग्री तक कैसे पहुँच सकते हो:
    ...
    और यहाँ आउटपुट में तुम क्या देखने की उम्मीद कर सकते हो:
...
हालांकि, यदि तुम datasheet में केवल एक column जोड़ते हो, तो channel का आकार उस से मेल नहीं खाएगा जो process उम्मीद करता है, और workflow त्रुटियाँ उत्पन्न करेगा।
यह process को दूसरों के साथ साझा करना भी मुश्किल बनाता है जिनके पास थोड़ा अलग इनपुट डेटा हो सकता है, और तुम्हें variables को process में hard-code करना पड़ सकता है जो script block द्वारा आवश्यक नहीं हैं।
...
अब जब हमारा मेटाडेटा लोड हो गया है, तो आओ इसके साथ कुछ करें!
...
हम तुम्हें `IDENTIFY_LANGUAGE` नामक एक पूर्व-लिखित process मॉड्यूल प्रदान करते हैं जो `langid` tool को wrap करता है, इसलिए तुम्हें workflow block से पहले केवल एक include statement जोड़ने की आवश्यकता है।
...
तुम इसके कोड की जाँच करने के लिए मॉड्यूल फ़ाइल खोल सकते हो:
...
जैसा कि तुम देख सकते हो, इनपुट definition उसी `tuple val(meta), path(file)` structure का उपयोग करता है जिसे हमने अभी अपने इनपुट channel पर लागू किया था।
...
ध्यान दो कि हमने channel निर्माण में मूल `.view()` operation हटा दिया है।
...
!!! note "नोट"
    ...
    जैसा कि तुम पहले ही सीख चुके होगे, तुम results को उनके बीच match करने के लिए channels में items के क्रम पर भरोसा नहीं कर सकते।
    इसके बजाय, तुम्हें डेटा को सही ढंग से जोड़ने के लिए keys का उपयोग करना चाहिए, और meta maps इस उद्देश्य के लिए एक आदर्श structure प्रदान करते हैं।
...
यहाँ वे संपादन हैं जो तुम्हें workflow में करने की आवश्यकता है:
...
यदि तुम अभी तक `+` ऑपरेटर से परिचित नहीं हो, या यदि यह भ्रमित लगता है, तो नीचे दिए गए विस्तृत स्पष्टीकरण को पढ़ने में कुछ मिनट लगाओ।

??? info "`+` ऑपरेटर का उपयोग करके नए meta map का निर्माण"

    **सबसे पहले, तुम्हें जानना होगा कि हम Groovy ऑपरेटर `+` का उपयोग करके दो maps की सामग्री को merge कर सकते हैं।**
    ...
    **लेकिन अगर तुम्हें एक ऐसा फ़ील्ड जोड़ने की आवश्यकता है जो पहले से ही map का हिस्सा नहीं है तो क्या होगा?**
    ...
    **अब आओ इसे Nextflow `channel.map()` operation के संदर्भ में transpose करें।**
    ...
    उम्मीद है कि तुम देख सकते हो कि यदि हम `map1` को `meta` में बदलते हैं, तो मूल रूप से हमारे workflow में हमारे meta map में भाषा predication जोड़ने के लिए यही सब हमें चाहिए।
    ...
    यदि तुम्हें यह समझने में कठिनाई हो रही है कि `file` `map` operation में क्यों घूम रहा है, तो कल्पना करो कि `[meta + [lang: lang_id], file]` के बजाय, वह लाइन `[new_map, file]` पढ़ती है।
...
एक बार जब तुम्हें विश्वास हो जाए कि तुम समझ गए हो कि यह कोड क्या कर रहा है, तो यह देखने के लिए workflow चलाओ कि यह काम किया या नहीं:
...
अब जब हमारे पास हमारी भाषा predictions हैं, तो आओ कुछ नए groupings असाइन करने के लिए जानकारी का उपयोग करें।
...
यह classification बाद में pipeline में कहीं आसानी से उपलब्ध होना उपयोगी हो सकता है, तो आओ उस जानकारी को meta map में जोड़ें।
...
- default value `'unknown'` के साथ `lang_group` नामक एक variable परिभाषित करो।
- यदि `lang` German (`'de'`) या English (`'en'`) है, तो `lang_group` को `germanic` में बदलो।
- अन्यथा यदि `lang` French (`'fr'`), Spanish (`'es'`) और Italian (`'it'`) युक्त सूची में शामिल है, तो `lang_group` को `romance` में बदलो।

यदि तुम पहले से ही जानते हो कि Nextflow में conditional statements कैसे लिखें तो इसे स्वयं लिखने का प्रयास करो।
...
एक बार जब यह सब समझ में आ जाए, तो परिणाम देखने के लिए फिर से workflow चलाओ:
...
- **कस्टम keys बनाएँ**: तुमने अपने meta map में दो नई keys बनाईं, उन्हें `meta + [new_key:value]` के साथ मौजूदा meta map में merge करते हुए। एक process से computed value के आधार पर, और एक तुम्हारे द्वारा `map` ऑपरेटर में सेट की गई condition के आधार पर।

ये तुम्हें अपनी pipeline में आगे बढ़ते समय फ़ाइलों के साथ नए और मौजूदा मेटाडेटा को जोड़ने की अनुमति देते हैं।
भले ही तुम process के हिस्से के रूप में मेटाडेटा का उपयोग नहीं कर रहे हो, इस तरह से meta map को डेटा से जुड़ा रखने से सभी प्रासंगिक जानकारी को एक साथ रखना आसान हो जाता है।

---

## 3. process में meta map जानकारी का उपयोग करना

अब जब तुम जानते हो कि meta map कैसे बनाएं और update करें, तो हम वास्तव में मज़ेदार बिट पर आ सकते हैं: वास्तव में process में मेटाडेटा का उपयोग करना।
...
यदि तुमने Hello Nextflow course किया है, तो तुम पहले ही इस tool को action में देख चुके हो।
यदि नहीं, तो चिंता न करो; हम सब कुछ कवर करेंगे जो तुम्हें जानने की आवश्यकता है।

### 3.1. process को import करें और कोड की जाँच करें

हम तुम्हें `COWPY` नामक एक पूर्व-लिखित process मॉड्यूल प्रदान करते हैं जो `cowpy` tool को wrap करता है, इसलिए तुम्हें workflow block से पहले केवल एक include statement जोड़ने की आवश्यकता है।
...
जैसा कि तुम देख सकते हो, यह process वर्तमान में एक इनपुट फ़ाइल (प्रदर्शित करने के लिए text युक्त) और एक value लेने के लिए डिज़ाइन किया गया है जो ASCII art में draw किए जाने वाले character को निर्दिष्ट करता है, आमतौर पर workflow स्तर पर एक command-line parameter द्वारा प्रदान किया जाता है।
...
आओ इसका उपयोग करें प्रत्येक entry के लिए process द्वारा उपयोग किए जाने वाले character को set करने के लिए।

इसके लिए, हमें तीन काम करने होंगे:

1. पिछले process से आने वाले आउटपुट channel को एक नाम दो ताकि हम इसे अधिक सुविधाजनक रूप से operate कर सकें।
2. रुचि की जानकारी तक पहुँचने का तरीका निर्धारित करो
3. दूसरे process को call जोड़ो और जानकारी को उचित रूप से feed करो।
...
मुख्य workflow में, `.view()` ऑपरेटर को `.set { ch_languages }` से बदलो, और एक लाइन जोड़ो जो test करती है कि हम channel को नाम से refer कर सकते हैं।
...
ध्यान दो कि हम `.view` operations के आउटपुट को अधिक readable बनाने के लिए closures (जैसे `{ file -> "File: " + file }`) का उपयोग कर रहे हैं।
...
_फ़ाइल paths और character values तुम्हारे आउटपुट में अलग क्रम में आ सकते हैं।_
...
अब सब कुछ एक साथ रखो और वास्तव में `ch_languages` channel पर `COWPY` process को call करो।
...
तुम देखते हो कि हम बस दो map operations (`.view()` statements को हटाकर) को process call के inputs के रूप में copy करते हैं।
बस उनके बीच comma न भूलना सुनिश्चित करो!
...
यदि तुम results डायरेक्टरी में देखो, तो तुम्हें प्रत्येक अभिवादन का ASCII art वाली अलग-अलग फ़ाइलें दिखनी चाहिए, जो संबंधित character द्वारा बोली गई हैं।
...
??? info "(वैकल्पिक) `multiMap()` के साथ वैकल्पिक version"
    ...
    यदि तुम सोच रहे हो, तो हम केवल एक single `map()` operation नहीं लिख सकते थे जो `file` और `character` दोनों output करे, क्योंकि वह उन्हें tuple के रूप में return करता।
    ...
    उदाहरण के लिए, तुम ऊपर `COWPY` की call को निम्नलिखित code से बदल सकते हो:
...
आओ `COWPY` process को `[meta, file]` tuple structure स्वीकार करने के लिए modify करें जो हमने पहले process में उपयोग किया था ताकि हम workflow को streamline कर सकें।
...
ध्यान दो कि हमने meta map output करने के लिए process output definition को update नहीं किया, tutorial को streamlined रखने के लिए, लेकिन `IDENTIFY_LANGUAGE` process के model का अनुसरण करते हुए इसे स्वयं एक exercise के रूप में करने के लिए स्वतंत्र महसूस करो।
...
आओ पिछले execution के results हटाएँ और चलाएँ:
...
यदि तुम results डायरेक्टरी में देखो, तो तुम्हें पहले जैसे ही outputs दिखने चाहिए, अर्थात् प्रत्येक अभिवादन का ASCII art वाली अलग-अलग फ़ाइलें, संबंधित character द्वारा बोली गई।
...
निश्चित रूप से, यह मानता है कि तुम process code को modify करने में सक्षम हो।
कुछ मामलों में, तुम्हें मौजूदा processes पर निर्भर रहना पड़ सकता है जिन्हें तुम modify करने के लिए स्वतंत्र नहीं हो, जो तुम्हारे options को सीमित करता है।
अच्छी खबर, यदि तुम [nf-core](https://nf-co.re/) project से modules उपयोग करने की योजना बना रहे हो, तो nf-core modules सभी standard के रूप में `[meta, file]` tuple structure उपयोग करने के लिए set up हैं।
...
मान लो हम data collection error simulate करने के लिए अपनी datasheet में एक entry का character value हटा देते हैं:
...
अब मान लो हम अपनी datasheet से `character` column को पूरी तरह हटा देते हैं:
...
1. अपने workflow में input validation implement करें ताकि यह सुनिश्चित हो सके कि datasheet में सभी आवश्यक जानकारी है। तुम Hello nf-core training course में [input validation का परिचय](../hello_nf-core/05_input_validation.md) पा सकते हो।

2. यदि तुम यह सुनिश्चित करना चाहते हो कि तुम्हारे process module का उपयोग करने वाला कोई भी व्यक्ति तुरंत आवश्यक inputs की पहचान कर सके, तो तुम आवश्यक metadata property को एक explicit input भी बना सकते हो।

यहाँ एक उदाहरण है कि यह कैसे काम करेगा।

सबसे पहले, process स्तर पर, input definition को इस प्रकार update करो:
...
फिर, workflow स्तर पर, metadata से `character` property extract करने और इसे input tuple का एक explicit component बनाने के लिए mapping operation का उपयोग करो:
...
इस section में, तुमने सीखा कि कैसे मेटाडेटा का उपयोग करके process के execution को customize किया जाता है, इसे workflow स्तर पर या process स्तर पर access करके।

---

## पूरक exercise

यदि तुम process के अंदर से meta map जानकारी का उपयोग करने का अभ्यास करना चाहते हो, तो meta map से `lang` और `lang_group` जैसी अन्य जानकारी का उपयोग करके outputs का नाम और/या संगठन customize करने का प्रयास करो।

उदाहरण के लिए, इस result produce करने के लिए code को modify करने का प्रयास करो:
...
इस side quest में, तुमने Nextflow workflows में मेटाडेटा के साथ प्रभावी ढंग से काम करने का तरीका जाना।
...
अपने काम में इस pattern को लागू करने से तुम robust, maintainable bioinformatics workflows बना सकोगे।

### मुख्य patterns

1.  **मेटाडेटा पढ़ना और structuring:** CSV फ़ाइलें पढ़ना और organized मेटाडेटा maps बनाना जो तुम्हारी डेटा फ़ाइलों से जुड़ी रहती हैं।
...
[Side Quests के मेनू](./index.md) पर वापस लौटो या सूची में अगले topic पर जाने के लिए पृष्ठ के नीचे दाईं ओर बटन पर क्लिक करो।
