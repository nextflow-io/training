...
Nextflow pipelines में, मेटाडेटा का उपयोग इसके लिए किया जा सकता है:
...
इस side quest में, हम workflows में मेटाडेटा को कैसे हैंडल करें, यह जानेंगे।
...
ये कौशल तुम्हें अधिक मजबूत और लचीली pipelines बनाने में मदद करेंगे जो जटिल फ़ाइल संबंधों और प्रोसेसिंग आवश्यकताओं को संभाल सकती हैं।

### पूर्वापेक्षाएँ

इस side quest को शुरू करने से पहले, तुम्हें:

- [Hello Nextflow](../hello_nextflow/README.md) tutorial या समकक्ष beginner's course पूरा कर लेना चाहिए।
...
तुम्हें एक main workflow फ़ाइल और एक `data` directory मिलेगी जिसमें एक datasheet और कुछ डेटा फ़ाइलें हैं।
...
- `character`: एक character का नाम, जिसका उपयोग हम बाद में विभिन्न creatures बनाने के लिए करेंगे
...
हम तुम्हें `langid` नामक एक containerized language analysis tool भी प्रदान करेंगे।
...
2. फ़ाइलों को भाषा परिवार (Germanic vs Romance languages) द्वारा **समूहित** करेगी
3. प्रत्येक फ़ाइल के लिए उसकी भाषा और मेटाडेटा के आधार पर प्रोसेसिंग को **अनुकूलित** करेगी
4. outputs को भाषा समूह द्वारा **व्यवस्थित** करेगी

यह एक विशिष्ट workflow pattern का प्रतिनिधित्व करता है जहाँ फ़ाइल-विशिष्ट मेटाडेटा प्रोसेसिंग निर्णयों को संचालित करता है; बिल्कुल उस तरह की समस्या जिसे मेटाडेटा maps सुंदरता से हल करते हैं।

#### तैयारी checklist

लगता है तुम शुरू करने के लिए तैयार हो?
...
तुम देख सकते हो कि हमने example datasheet को एक फ़ाइल के रूप में लोड करने के लिए एक बुनियादी channel factory सेट किया है, लेकिन यह अभी तक फ़ाइल की contents को नहीं पढ़ेगा।
चलो इसे जोड़कर शुरू करते हैं।

### 1.1. `splitCsv` के साथ contents पढ़ना

हमें एक operator चुनना होगा जो हमारी ओर से न्यूनतम प्रयास के साथ फ़ाइल contents को उचित रूप से parse करेगा।
...
channel construction code में `splitCsv()` operation जोड़ने के लिए निम्नलिखित परिवर्तन करो, साथ ही यह जाँचने के लिए एक `view()` operation भी जोड़ो कि फ़ाइल की contents channel में सही तरीके से लोड हो रही हैं।
...
यह बहुत बढ़िया है! यह प्रत्येक फ़ाइल से विशिष्ट fields तक पहुँचना आसान बनाता है।
उदाहरण के लिए, हम `id` के साथ फ़ाइल ID या `recording` के साथ txt फ़ाइल path तक पहुँच सकते हैं।

??? info "(Optional) Maps के बारे में अधिक"

    Groovy में, वह programming language जिस पर Nextflow बनाया गया है, एक map एक key-value data structure है जो Python में dictionaries, JavaScript में objects, या Ruby में hashes के समान है।

    यहाँ एक runnable script है जो दिखाता है कि तुम व्यवहार में एक map को कैसे define कर सकते हो और इसकी contents तक कैसे पहुँच सकते हो:
...
### 1.2. `map` के साथ विशिष्ट fields चुनना

मान लो हम datasheet से `character` column तक पहुँचना और इसे print करना चाहते हैं।
हम Nextflow `map` operator का उपयोग करके हमारे channel में प्रत्येक item पर iterate कर सकते हैं और विशेष रूप से map object से `character` entry को चुन सकते हैं।
...
सफलता! हमने अपनी datasheet से प्राप्त map structure का लाभ उठाया है ताकि प्रत्येक row के लिए individual columns से values तक पहुँच सकें।

अब जब हमने सफलतापूर्वक datasheet को पढ़ लिया है और प्रत्येक row में डेटा तक पहुँच है, तो हम अपनी pipeline logic को लागू करना शुरू कर सकते हैं।
...
व्यावहारिक परिणाम यह है कि इस channel को consume करने वाली प्रत्येक process को इस structure को ध्यान में रखते हुए configure करना होगा:
...
यह तब तक ठीक है जब तक datasheet में columns की संख्या नहीं बदलती।
...
यह process को दूसरों के साथ share करना भी कठिन बनाता है जिनके पास थोड़ा अलग input data हो सकता है, और तुम्हें process में variables को hard-code करना पड़ सकता है जो script block द्वारा आवश्यक नहीं हैं।

इस समस्या से बचने के लिए, हमें channel structure को consistent रखने का एक तरीका खोजना होगा, चाहे datasheet में कितने भी columns हों।
...
`map` operation में निम्नलिखित edits करो:
...
हमने अपने channel elements को एक tuple में पुनर्गठित किया है जिसमें दो elements हैं, meta map और संबंधित file object।
...
अब, channel में प्रत्येक element में पहले metadata map और दूसरे संबंधित file object है:
...
- **datasheets को कैसे पढ़ें:** header जानकारी के साथ CSV फ़ाइलों को पढ़ने और rows को structured data में transform करने के लिए `splitCsv` का उपयोग करना
- **meta map कैसे बनाएँ:** tuple structure `[ [id:value, ...], file ]` का उपयोग करके मेटाडेटा को file data से अलग करना

---

## 2. मेटाडेटा में हेरफेर करना

अब जब हमारे पास हमारा मेटाडेटा लोड हो गया है, चलो इसके साथ कुछ करते हैं!
...
यह tool भाषाओं के एक set पर pre-trained आता है, और text का एक snippet दिए जाने पर, यह एक भाषा prediction और एक संबंधित probability score output करेगा, दोनों `stdout` पर।
...
जैसा कि तुम देख सकते हो, input definition उसी `tuple val(meta), path(file)` structure का उपयोग करती है जिसे हमने अभी अपने input channel पर लागू किया है।

output definition एक tuple के रूप में structured है जिसमें input के समान structure है, सिवाय इसके कि इसमें तीसरे element के रूप में `stdout` भी है।
यह `tuple val(meta), path(file), <output>` pattern मेटाडेटा को input data और outputs दोनों के साथ जुड़ा रखता है क्योंकि यह pipeline के माध्यम से बहता है।

ध्यान दो कि हम यहाँ Nextflow के [`stdout`](https://www.nextflow.io/docs/latest/process.html#outputs) output qualifier का उपयोग कर रहे हैं क्योंकि tool अपने output को सीधे console पर print करता है बजाय एक फ़ाइल लिखने के; और हम command line में `sed` का उपयोग probability score को हटाने, newline characters को हटाकर string को साफ करने, और केवल भाषा prediction return करने के लिए करते हैं।

### 2.2. `IDENTIFY_LANGUAGE` के लिए एक call जोड़ो

अब जब process workflow के लिए उपलब्ध है, हम data channel पर इसे चलाने के लिए `IDENTIFY_LANGUAGE` process के लिए एक call जोड़ सकते हैं।
...
ध्यान दो कि हमने channel construction में original `.view()` operation को हटा दिया है।
...
बढ़िया! अब हमारे पास एक prediction है कि प्रत्येक character कौन सी भाषा बोलता है।

और जैसा कि पहले बताया गया है, हमने output में input फ़ाइल और meta map भी शामिल किया है, जिसका अर्थ है कि दोनों नई जानकारी के साथ जुड़े रहते हैं जो हमने अभी उत्पन्न की है।
यह अगले step में उपयोगी साबित होगा।

!!! note

    अधिक सामान्य रूप से, results के साथ meta map को जुड़ा रखने का यह pattern उन संबंधित results को associate करना आसान बनाता है जो समान identifiers साझा करते हैं।
    ...
    इसके बजाय, तुम्हें डेटा को सही तरीके से associate करने के लिए keys का उपयोग करना होगा, और meta maps इस उद्देश्य के लिए एक आदर्श structure प्रदान करते हैं।

    हम [Splitting & Grouping](./splitting_and_grouping.md) side quest में इस use case का विस्तार से पता लगाते हैं।

### 2.3. process outputs के साथ मेटाडेटा को augment करना

यह देखते हुए कि हमने अभी जो results उत्पन्न किए हैं वे स्वयं फ़ाइलों की contents के बारे में मेटाडेटा का एक रूप हैं, उन्हें हमारे meta map में जोड़ना उपयोगी होगा।
...
इसलिए इसके बजाय, हम मौजूदा meta map की contents के साथ एक नया meta map बनाएँगे और नई जानकारी रखने वाली एक नई `lang: lang_id` key-value pair जोड़ेंगे, `+` operator (एक Groovy feature) का उपयोग करके।
और हम इसे पुराने map को नए से replace करने के लिए [`map`](https://www.nextflow.io/docs/latest/operator.html#map) operation के साथ combine करेंगे।

यहाँ वे edits हैं जो तुम्हें workflow में करने होंगे:
...
यदि तुम अभी तक `+` operator से परिचित नहीं हो, या यदि यह confusing लगता है, तो नीचे दिए गए विस्तृत explanation को पढ़ने में कुछ मिनट लगाओ।

??? info "`+` operator का उपयोग करके नए meta map का निर्माण"

    **सबसे पहले, तुम्हें यह जानना होगा कि हम Groovy operator `+` का उपयोग करके दो maps की contents को merge कर सकते हैं।**
    ...
    **लेकिन क्या होगा यदि तुम्हें एक field जोड़ना है जो पहले से map का हिस्सा नहीं है?**

    मान लो तुम फिर से `map1` से शुरू करते हो, लेकिन भाषा prediction अपने map में नहीं है (कोई `map2` नहीं है)।
    इसके बजाय, यह `lang_id` नामक एक variable में है, और तुम जानते हो कि तुम इसके value (`'fr'`) को key `lang` के साथ store करना चाहते हो।
    ...
    यहाँ, `[lang: new_info]` on the fly एक नया unnamed map बनाता है, और `map1 + ` `map1` को नए unnamed map के साथ merge करता है, पहले जैसी ही `new_map` contents उत्पन्न करता है।
    ...
    **अब चलो इसे Nextflow `channel.map()` operation के संदर्भ में transpose करते हैं।**
    ...
    output एक single unnamed map है जिसमें हमारे ऊपर के example में `new_map` के समान contents हैं।
    तो हमने effectively transform किया है:
    ...
    इससे यह अधिक स्पष्ट होना चाहिए कि हम बस `file` को tuple में दूसरी position में अपनी original जगह पर छोड़ रहे हैं। हमने बस `new_info` value ली है और इसे पहली position में map में fold किया है।

    **और यह हमें वापस `tuple val(meta), path(file)` channel structure पर लाता है!**

एक बार जब तुम्हें विश्वास हो जाए कि तुम समझ गए हो कि यह code क्या कर रहा है, तो देखने के लिए workflow चलाओ कि क्या यह काम किया:
...
हमने process के output को `meta, file, lang_id` से सुंदरता से पुनर्गठित किया है ताकि `lang_id` अब meta map में keys में से एक हो, और channel के tuples फिर से `meta, file` model में fit हों।
...
हमारे example data में, हमारे characters द्वारा उपयोग की जाने वाली भाषाओं को germanic languages (English, German) और romance language (French, Spanish, Italian) में grouped किया जा सकता है।
यह classification pipeline में बाद में कहीं आसानी से उपलब्ध होना उपयोगी हो सकता है, तो चलो उस जानकारी को meta map में जोड़ते हैं।

और, अच्छी खबर, यह फिर से एक case है जो `map` operator का उपयोग करने के लिए पूरी तरह से उपयुक्त है!

विशेष रूप से, हम `lang_group` नामक एक variable define करने जा रहे हैं, प्रत्येक piece of data के लिए `lang_group` को कौन सा value assign करना है यह निर्धारित करने के लिए कुछ सरल conditional logic का उपयोग करेंगे।
...
!!! tip
    ...
    तुम map operation के भीतर `meta.lang` के साथ `lang` के value तक पहुँच सकते हो।
...
- हम `def lang_group = "unknown"` का उपयोग करके `lang_group` variable को default value `unknown` के साथ बनाते हैं।
- हम conditional logic के लिए `if {} else if {}` structure का उपयोग करते हैं, दो germanic languages के लिए alternative `.equals()` tests के साथ, और तीन romance languages के लिए एक list में existence के लिए एक test के साथ।
...
एक बार जब यह सब समझ में आ जाए, तो result देखने के लिए workflow को फिर से चलाओ:
...
जैसा कि तुम देख सकते हो, channel elements अपनी `[meta, file]` structure बनाए रखते हैं, लेकिन meta map में अब यह नया classification शामिल है।

### सारांश

इस section में, तुमने सीखा है कि कैसे:

- **output channels पर input मेटाडेटा apply करना**: इस तरह से मेटाडेटा copy करने से हमें बाद में मेटाडेटा content के आधार पर results को associate करने की अनुमति मिलती है।
- **custom keys बनाना**: तुमने अपने meta map में दो नई keys बनाईं, उन्हें `meta + [new_key:value]` के साथ मौजूदा meta map में merge किया। एक process से computed value के आधार पर, और एक `map` operator में तुमने set की गई condition के आधार पर।
...
भले ही तुम किसी process के हिस्से के रूप में मेटाडेटा का उपयोग नहीं कर रहे हो, इस तरह डेटा के साथ meta map को जुड़ा रखना सभी प्रासंगिक जानकारी को एक साथ रखना आसान बनाता है।
...
??? info "`cowpy` क्या करता है?"
    ...
    `cowpy` एक command-line tool है जो मज़ेदार तरीके से arbitrary text inputs प्रदर्शित करने के लिए ASCII art generate करता है।
...
यदि तुमने Hello Nextflow course के माध्यम से काम किया है, तो तुमने पहले ही इस tool को action में देखा है।
...
जैसा कि तुम देख सकते हो, यह process वर्तमान में एक input फ़ाइल (प्रदर्शित किए जाने वाले text वाली) और एक value लेने के लिए designed है जो character को specify करती है जिसे ASCII art में draw किया जाना चाहिए, आमतौर पर workflow level पर एक command-line parameter द्वारा प्रदान किया जाता है।

### 3.2. एक meta map field को input के रूप में pass करना

जब हमने Hello Nextflow course में `cowpy` tool का उपयोग किया, तो हमने यह निर्धारित करने के लिए एक command-line parameter का उपयोग किया कि final image draw करने के लिए किस character का उपयोग करना है।
यह समझ में आया, क्योंकि हम pipeline के प्रति run केवल एक image generate कर रहे थे।

हालाँकि, इस tutorial में, हम प्रत्येक subject के लिए एक उपयुक्त image generate करना चाहते हैं जिसे हम process कर रहे हैं, इसलिए एक command-line parameter का उपयोग करना बहुत सीमित होगा।

अच्छी खबर: हमारे datasheet में एक `character` column है और इसलिए, हमारे meta map में।
चलो इसका उपयोग करके character set करते हैं जिसे process को प्रत्येक entry के लिए उपयोग करना चाहिए।

इस उद्देश्य के लिए, हमें तीन चीजें करनी होंगी:

1. पिछली process से निकलने वाले output channel को एक नाम दें ताकि हम इस पर अधिक सुविधाजनक रूप से operate कर सकें।
2. निर्धारित करें कि interest की जानकारी तक कैसे पहुँचें
3. दूसरी process के लिए एक call जोड़ें और जानकारी को उचित रूप से feed करें।

चलो शुरू करते हैं।

#### 3.2.1. पिछले output channel को नाम दें

हमने पिछले manipulations को सीधे पहली process के output channel, `IDENTIFY_LANGUAGE.out` पर apply किया।
अगली process को channel की contents को feed करने के लिए (और ऐसा करने के लिए जो स्पष्ट और पढ़ने में आसान हो) हम इसे अपना नाम देना चाहते हैं, `ch_languages`।

हम [`set`](https://www.nextflow.io/docs/latest/reference/operator.html#set) operator का उपयोग करके ऐसा कर सकते हैं।

main workflow में, `.view()` operator को `.set { ch_languages }` से replace करो, और एक line जोड़ो जो test करे कि हम channel को नाम से refer कर सकते हैं।
...
हम module code को देखने से जानते हैं कि `COWPY` process को एक text फ़ाइल और एक `character` value दिया जाना अपेक्षित है।
`COWPY` process call लिखने के लिए, हमें बस यह जानना होगा कि channel में प्रत्येक element से संबंधित file object और मेटाडेटा को कैसे extract करें।
...
_तुम्हारे output में file paths और character values एक अलग order में आ सकते हैं।_

यह confirm करता है कि हम channel में प्रत्येक element के लिए file और character तक पहुँचने में सक्षम हैं।
...
यदि तुम results directory में देखते हो, तो तुम्हें individual फ़ाइलें दिखनी चाहिए जिनमें प्रत्येक greeting की ASCII art है जो संबंधित character द्वारा बोली गई है।

??? abstract "Directory और example file contents"
...
हालाँकि, जैसा कि ऊपर बताया गया है, शामिल कुछ code थोड़ा clunky था, क्योंकि हमें workflow body के संदर्भ में रहते हुए meta data को unpack करना पड़ा।
यह approach meta map से fields की एक छोटी संख्या का उपयोग करने के लिए ठीक काम करता है, लेकिन यदि हम बहुत अधिक उपयोग करना चाहते हैं तो खराब तरीके से scale होगा।

एक और operator है जिसे `multiMap()` कहा जाता है जो हमें इसे थोड़ा streamline करने की अनुमति देता है, लेकिन फिर भी यह आदर्श नहीं है।

??? info "(Optional) `multiMap()` के साथ alternative version"

    यदि तुम सोच रहे हो, तो हम बस एक single `map()` operation नहीं लिख सकते थे जो `file` और `character` दोनों को output करता है, क्योंकि यह उन्हें एक tuple के रूप में return करता।
    ...
    उदाहरण के लिए, तुम ऊपर `COWPY` के call को निम्नलिखित code से replace कर सकते हो:
    ...
    यह बिल्कुल वही result produce करता है।

किसी भी case में, यह awkward है कि हमें workflow level पर कुछ unpacking करना पड़ता है।

यह बेहतर होगा यदि हम पूरे meta map को process में feed कर सकें और वहाँ पहुँचने के बाद हमें जो चाहिए उसे चुन सकें।
...
एकमात्र कारण जो हम ऊपर ऐसा नहीं कर सके वह यह है कि process एक meta map को accept करने के लिए set up नहीं है।
लेकिन चूंकि हम process code को control करते हैं, हम इसे बदल सकते हैं।

चलो `COWPY` process को `[meta, file]` tuple structure को accept करने के लिए modify करते हैं जिसे हमने पहली process में उपयोग किया था ताकि हम workflow को streamline कर सकें।

इस उद्देश्य के लिए, हमें तीन चीजें करनी होंगी:

1. `COWPY` process module के input definitions को modify करना
2. meta map का उपयोग करने के लिए process command को update करना
3. workflow body में process call को update करना

तैयार? चलो चलते हैं!

#### 3.3.1. `COWPY` module input को modify करो

`cowpy.nf` module फ़ाइल में निम्नलिखित edits करो:
...
यह हमें `[meta, file]` tuple structure का उपयोग करने में सक्षम बनाता है जिसे हमने tutorial में पहले cover किया था।

ध्यान दो कि हमने meta map को output करने के लिए process output definition को update नहीं किया, tutorial को streamlined रखने के लिए, लेकिन `IDENTIFY_LANGUAGE` process के model का पालन करते हुए इसे स्वयं एक exercise के रूप में करने के लिए स्वतंत्र महसूस करो।

#### 3.3.2. meta map field का उपयोग करने के लिए command को update करो

अब पूरा meta map process के अंदर उपलब्ध है, इसलिए हम command block के अंदर से सीधे इसमें निहित जानकारी को refer कर सकते हैं।
...
हमने पहले standalone input के रूप में pass किए गए `character` value के reference को meta map में held value से replace कर दिया है, जिसे हम `meta.character` का उपयोग करके refer करते हैं।

अब चलो तदनुसार process call को update करते हैं।

#### 3.3.3. process call को update करो और इसे चलाओ

process अब अपने input को `[meta, file]` tuple structure का उपयोग करने की अपेक्षा करती है, जो कि पिछली process outputs है, इसलिए हम बस पूरे `ch_languages` channel को `COWPY` process को pass कर सकते हैं।

main workflow में निम्नलिखित edits करो:
...
यदि तुम results directory में देखते हो, तो तुम्हें पहले जैसे ही outputs दिखने चाहिए, _i.e._ individual फ़ाइलें जिनमें प्रत्येक greeting की ASCII art है जो संबंधित character द्वारा बोली गई है।
...
कुछ cases में, तुम्हें मौजूदा processes पर भरोसा करना पड़ सकता है जिन्हें तुम modify करने के लिए स्वतंत्र नहीं हो, जो तुम्हारे options को सीमित करता है।
अच्छी खबर, यदि तुम [nf-core](https://nf-co.re/) project से modules का उपयोग करने की योजना बना रहे हो, तो यह है कि nf-core modules सभी `[meta, file]` tuple structure को एक standard के रूप में उपयोग करने के लिए set up हैं।

### 3.4. missing required inputs की troubleshooting

`character` value `COWPY` process को सफलतापूर्वक चलाने के लिए आवश्यक है।
यदि हम configuration फ़ाइल में इसके लिए एक default value set नहीं करते हैं, तो हमें datasheet में इसके लिए एक value प्रदान करना चाहिए।
...
मान लो हम data collection error को simulate करने के लिए अपनी datasheet में entries में से एक के लिए character value को delete करते हैं:
...
हमने ऊपर उपयोग किए गए workflow के किसी भी version के लिए, जब datasheet को पढ़ा जाता है तो सभी entries के लिए `character` key बनाई जाएगी, लेकिन `sampleA` के लिए value एक empty string होगी।
...
इस case में जब datasheet को पढ़ा जाता है तो `character` key बिल्कुल नहीं बनाई जाएगी।
...
यह instruction से मेल खाने वाले कोई elements नहीं पाएगा, इसलिए यह `COWPY` को बिल्कुल नहीं चलाएगा।
...
जहाँ तक Nextflow का संबंध है, यह workflow सफलतापूर्वक चली!
हालाँकि, हम जो outputs चाहते हैं उनमें से कोई भी produce नहीं होगा।
...
workflow configuration के हिस्से के रूप में एक default value supply करने के अलावा, हम इसे अधिक robustly handle करने के लिए दो चीजें कर सकते हैं:

1. अपने workflow में input validation implement करो ताकि यह सुनिश्चित हो सके कि datasheet में सभी आवश्यक जानकारी है। तुम Hello nf-core training course में [input validation का परिचय](../hello_nf-core/05_input_validation.md) पा सकते हो। <!-- TODO (future) pending a proper Validation side quest -->

2. यदि तुम यह सुनिश्चित करना चाहते हो कि कोई भी जो तुम्हारे process module का उपयोग करता है वह तुरंत required inputs की पहचान कर सके, तो तुम required metadata property को एक explicit input भी बना सकते हो।

यहाँ एक example है कि यह कैसे काम करेगा।
...
फिर, workflow level पर, metadata से `character` property को extract करने और इसे input tuple का एक explicit component बनाने के लिए एक mapping operation का उपयोग करो:
...
meta map channel structures को clean रखने और arbitrary channel structures को रोकने के लिए excellent है, लेकिन mandatory elements के लिए जो सीधे एक process में referenced हैं, उन्हें explicit inputs के रूप में extract करना अधिक robust और maintainable code बनाता है।
...
यदि तुम एक process के अंदर से meta map जानकारी का उपयोग करने का अभ्यास करना चाहते हो, तो meta map से अन्य जानकारी के pieces जैसे `lang` और `lang_group` का उपयोग करके outputs को कैसे नाम दिया जाता है और/या व्यवस्थित किया जाता है, इसे customize करने का प्रयास करो।

उदाहरण के लिए, यह result produce करने के लिए code को modify करने का प्रयास करो:
...
इस side quest में, तुमने Nextflow workflows में मेटाडेटा के साथ effectively काम करने का पता लगाया है।

मेटाडेटा को explicit और डेटा के साथ attached रखने का यह pattern Nextflow में एक core best practice है, जो hardcoding file जानकारी पर कई फायदे प्रदान करता है:
...
अपने काम में इस pattern को apply करने से तुम्हें robust, maintainable bioinformatics workflows बनाने में सक्षम बनाया जाएगा।

### मुख्य patterns

1.  **मेटाडेटा को पढ़ना और संरचित करना:** CSV फ़ाइलों को पढ़ना और organized metadata maps बनाना जो तुम्हारी data फ़ाइलों के साथ जुड़े रहते हैं।
...
2.  **Workflow के दौरान मेटाडेटा का विस्तार करना** process outputs जोड़कर और conditional logic के माध्यम से values derive करके अपनी pipeline progress करते समय अपने मेटाडेटा में नई जानकारी जोड़ना।
...
[Side Quests के menu](./index.md) पर वापस जाओ या list में अगले topic पर जाने के लिए page के निचले दाएँ कोने में button पर click करो।
