...
तुम देख सकते हो कि process दूसरी इनपुट फ़ाइल के लिए `null` मान के साथ `fastp` चलाने की कोशिश कर रहा है, जिससे यह fail हो रहा है। ऐसा इसलिए है क्योंकि हमारे डेटासेट में single-end reads हैं, लेकिन process paired-end reads (एक बार में दो इनपुट फ़ाइलें) की अपेक्षा करने के लिए hardcoded है।

`FASTP` process `script:` ब्लॉक में conditional logic जोड़कर इसे ठीक करो। एक if/else statement read फ़ाइलों की संख्या की जांच करता है और उसके अनुसार command को समायोजित करता है।
...
अब workflow single-end और paired-end reads दोनों को gracefully handle कर सकता है। Conditional logic इनपुट फ़ाइलों की संख्या की जांच करती है और `fastp` के लिए उचित command बनाती है। आओ देखें कि यह काम करता है या नहीं:
...
अच्छा लग रहा है! यदि हम उन वास्तविक commands की जांच करें जो चलाई गई थीं (अपने task hash के लिए customize करो):

```console title="निष्पादित commands की जांच करो"
...
Process script blocks में scripting का उपयोग करने के ये पैटर्न अत्यंत शक्तिशाली हैं और कई परिदृश्यों में लागू किए जा सकते हैं - variable input types को handle करने से लेकर file collections से complex command-line arguments बनाने तक, जो तुम्हारे processes को वास्तविक दुनिया के डेटा की विविध आवश्यकताओं के लिए सचमुच अनुकूलनीय बनाते हैं।

### 2.3. Variable Interpolation: Nextflow और Shell Variables

Process scripts Nextflow variables, shell variables, और command substitutions को मिलाते हैं, प्रत्येक अलग interpolation syntax के साथ। गलत syntax का उपयोग करने से errors होती हैं। आओ इन्हें एक processing report बनाने वाले process के साथ explore करें।

`modules/generate_report.nf` module फ़ाइल पर एक नज़र डालो:
...
यह process sample ID और filename के साथ एक सरल report लिखता है। अब इसे चलाओ यह देखने के लिए कि जब हमें विभिन्न प्रकार के variables को mix करने की आवश्यकता होती है तो क्या होता है।
...
अब workflow चलाओ और `results/reports/` में generated reports की जांच करो। उनमें प्रत्येक sample के बारे में बुनियादी जानकारी होनी चाहिए।
...
लेकिन क्या होगा यदि हम processing कब और कहाँ हुई इसके बारे में जानकारी जोड़ना चाहते हैं? आओ report में current user, hostname, और date शामिल करने के लिए **shell** variables और कुछ command substitution का उपयोग करके process को संशोधित करें:
...
यदि तुम इसे चलाते हो, तो तुम्हें एक error दिखेगा - Nextflow `${USER}` को एक Nextflow variable के रूप में interpret करने की कोशिश करता है जो मौजूद नहीं है।
...
Shell variables और command substitutions को backslash (`\`) के साथ escape करके इसे ठीक करो:
...
ये string processing और generation पैटर्न वास्तविक bioinformatics workflows में तुम्हें मिलने वाले विविध फ़ाइल formats और नामकरण सम्मेलनों को handle करने के लिए आवश्यक हैं।

---

## 3. पुन: प्रयोज्य फ़ंक्शन बनाना

Channel operators या process definitions में inline जटिल workflow logic पठनीयता और maintainability को कम करता है। **Functions** तुम्हें इस logic को named, reusable components में extract करने देते हैं।

हमारा map operation लंबा और जटिल हो गया है। आओ इसे `def` keyword का उपयोग करके एक reusable function में extract करें।

यह दिखाने के लिए कि यह हमारे मौजूदा workflow के साथ कैसा दिखता है, नीचे दिए गए modification को करो, `separateMetadata` नामक एक reusable function परिभाषित करने के लिए `def` का उपयोग करते हुए:
...
- **Function scope**: Script level पर परिभाषित functions तुम्हारे पूरे Nextflow workflow में accessible हैं
- **Return values**: Functions automatically अंतिम expression return करते हैं, या explicit `return` का उपयोग करो
...
अब तक हमने processes के `script` block में scripting का उपयोग किया है। लेकिन **closures** (Section 1.1 में पेश किए गए) process directives में भी अविश्वसनीय रूप से उपयोगी हैं, विशेष रूप से dynamic resource allocation के लिए। आओ अपने FASTP process में resource directives जोड़ें जो sample विशेषताओं के आधार पर adapt होते हैं।

### 4.1. Sample-specific resource allocation

वर्तमान में, हमारा FASTP process default resources का उपयोग करता है। आओ इसे smarter बनाएं high-depth samples के लिए अधिक CPUs allocate करके। Dynamic `cpus` directive और एक static `memory` directive शामिल करने के लिए `modules/fastp.nf` को edit करो:
...
`-ansi-log false` option के साथ workflow को फिर से चलाओ ताकि task hashes देखना आसान हो।
...
तुम किसी भी दिए गए task के लिए CPU allocation देखने के लिए चलाए गए वास्तविक `docker` command की जांच कर सकते हो:

```console title="docker command की जांच करो"
cat work/48/6db0c9e9d8aa65e4bb4936cd3bd59e/.command.run | grep "docker run"
```

तुम्हें कुछ इस तरह दिखना चाहिए:

```bash title="docker command"
    docker run -i --cpu-shares 4096 --memory 2048m -e "NXF_TASK_WORKDIR" -v /workspaces/training/side-quests/essential_scripting_patterns:/workspaces/training/side-quests/essential_scripting_patterns -w "$NXF_TASK_WORKDIR" --name $NXF_BOXID community.wave.seqera.io/library/fastp:0.24.0--62c97b06e8447690 /bin/bash -ue /workspaces/training/side-quests/essential_scripting_patterns/work/48/6db0c9e9d8aa65e4bb4936cd3bd59e/.command.sh
```

इस उदाहरण में हमने एक ऐसा उदाहरण चुना है जिसने 2 CPUs (`--cpu-shares 2048`) का अनुरोध किया, क्योंकि यह एक high-depth sample था, लेकिन तुम्हें sample depth के आधार पर विभिन्न CPU allocations दिखनी चाहिए। अन्य tasks के लिए भी यह आज़माओ।

### 4.2. Retry strategies

एक और शक्तिशाली पैटर्न retry strategies के लिए `task.attempt` का उपयोग करना है। यह दिखाने के लिए कि यह क्यों उपयोगी है, हम FASTP को memory allocation को उसकी ज़रूरत से कम करके शुरू करेंगे। `modules/fastp.nf` में `memory` directive को `1.GB` में बदलो:
...
यह वास्तविक workflows में एक बहुत सामान्य परिदृश्य है - कभी-कभी तुम नहीं जानते कि किसी task को कितनी memory की आवश्यकता होगी जब तक तुम इसे चला नहीं लेते।

हमारे workflow को अधिक robust बनाने के लिए, हम एक retry strategy implement कर सकते हैं जो प्रत्येक attempt पर memory allocation बढ़ाती है, फिर से एक Groovy closure का उपयोग करते हुए। Base memory को `task.attempt` से multiply करने के लिए `memory` directive को संशोधित करो, और `errorStrategy 'retry'` और `maxRetries 2` directives जोड़ो:
...
Closures के साथ dynamic directives तुम्हें अनुमति देते हैं:

- इनपुट विशेषताओं के आधार पर resources allocate करो
- बढ़ते resources के साथ automatic retry strategies implement करो
- कई factors (metadata, attempt number, priorities) को combine करो
- जटिल resource calculations के लिए conditional logic का उपयोग करो
...
उदाहरण के लिए, मान लो कि हमारे sequencing samples को केवल तभी FASTP से trim करने की आवश्यकता है जब वे एक निश्चित threshold से ऊपर coverage वाले human samples हों। Mouse samples या low-coverage samples को इसके बजाय Trimgalore के साथ चलाना चाहिए (यह एक काल्पनिक उदाहरण है, लेकिन यह बात को illustrate करता है)।

हमने `modules/trimgalore.nf` में एक सरल Trimgalore process प्रदान किया है, यदि तुम चाहो तो देखो, लेकिन इस exercise के लिए details महत्वपूर्ण नहीं हैं। मुख्य बात यह है कि हम samples को उनके metadata के आधार पर route करना चाहते हैं।

`modules/trimgalore.nf` से नए module को include करो:
...
... और फिर अपने `main.nf` workflow को samples को उनके metadata के आधार पर branch करने और उन्हें उचित trimming process के माध्यम से route करने के लिए संशोधित करो, इस तरह:
...
Workflow execution को control करने का एक और शक्तिशाली पैटर्न `.filter()` operator है, जो यह निर्धारित करने के लिए एक closure का उपयोग करता है कि कौन से items pipeline में आगे बढ़ने चाहिए। Filter closure के अंदर, तुम **boolean expressions** लिखोगे जो तय करते हैं कि कौन से items pass होते हैं।
...
इसका मतलब है कि `meta.id` अकेले (explicit `!= null` के बिना) जांच करता है कि ID मौजूद है और empty नहीं है। आओ इसका उपयोग उन samples को filter out करने के लिए करें जो हमारी quality requirements को पूरा नहीं करते।
...
=== "बाद में"
    ...
    ```groovy title="main.nf" linenums="28" hl_lines="5-11"
        ...
        // अमान्य या low-quality samples को filter out करो
...
इस खंड में, तुमने Nextflow operators जैसे `.branch{}` और `.filter{}` के closure interfaces का उपयोग करके workflow execution को control करने के लिए conditional logic का उपयोग करना सीखा, संक्षिप्त conditional expressions लिखने के लिए truthiness का लाभ उठाते हुए।

हमारी pipeline अब बुद्धिमानी से samples को उचित processes के माध्यम से route करती है, लेकिन production workflows को अमान्य data को gracefully handle करने की आवश्यकता है। आओ हमारे workflow को missing या null values के विरुद्ध robust बनाएं।

---

## 6. Safe Navigation और Elvis Operators

हमारा `separateMetadata` function वर्तमान में मानता है कि सभी CSV fields मौजूद और valid हैं। लेकिन अपूर्ण data के साथ क्या होता है? आओ पता करें।

### 6.1. समस्या: ऐसी Properties को Access करना जो मौजूद नहीं हैं

मान लो कि हम optional sequencing run information के लिए support जोड़ना चाहते हैं। कुछ labs में, samples में sequencing run ID या batch number के लिए एक अतिरिक्त field हो सकता है, लेकिन हमारे वर्तमान CSV में यह column नहीं है। आओ इसे किसी भी तरह access करने का प्रयास करें।
...
अब जब हम safe navigation का उपयोग कर रहे हैं, तो उस field के बिना samples के लिए `run_id` `null` होगा। आओ एक default value प्रदान करने और इसे हमारे `sample_meta` map में जोड़ने के लिए Elvis operator का उपयोग करें:
...
इन operators का उपयोग functions, operator closures (`.map{}`, `.filter{}`), process scripts, और config files में consistently करो। वे वास्तविक डेटा handle करते समय crashes को रोकते हैं।
...
कभी-कभी तुम्हें input parameters अमान्य होने पर workflow को तुरंत रोकने की आवश्यकता होती है। Nextflow में, तुम validation logic implement करने के लिए `error()` और `log.warn` जैसे built-in functions, साथ ही `if` statements और boolean logic जैसे standard programming constructs का उपयोग कर सकते हो। आओ अपने workflow में validation जोड़ें।

अपने workflow block से पहले एक validation function बनाओ, इसे workflow से call करो, और CSV file path के लिए एक parameter का उपयोग करने के लिए channel creation को बदलो। यदि parameter missing है या file मौजूद नहीं है, तो एक स्पष्ट message के साथ execution रोकने के लिए `error()` call करो।
...
=== "बाद में"
    ...
    def validateInputs() {
        // जांचो कि input parameter प्रदान किया गया है
        if (!params.input) {
            error("Input CSV file path not provided. Please specify --input <file.csv>")
        }

        // जांचो कि CSV file मौजूद है
...
अब CSV file के बिना चलाने का प्रयास करो:
...
अब एक non-existent file के साथ चलाओ:
...
तुम `separateMetadata` function के अंदर भी validation जोड़ सकते हो। आओ non-fatal `log.warn` का उपयोग करके low sequencing depth वाले samples के लिए warnings issue करें, लेकिन workflow को जारी रहने दें:
...
- **Early validation**: Helpful errors के साथ तेज़ी से fail करने के लिए processing से पहले inputs की जांच करो
...
अब तक, हम अपने workflow scripts और process definitions में code लिख रहे थे। लेकिन एक और महत्वपूर्ण feature है जो तुम्हें पता होनी चाहिए: workflow event handlers।

Event handlers closures हैं जो तुम्हारे workflow के lifecycle में विशिष्ट बिंदुओं पर चलते हैं। वे logging, notifications, या cleanup operations जोड़ने के लिए बिल्कुल सही हैं। ये handlers तुम्हारे workflow script में तुम्हारी workflow definition के साथ परिभाषित किए जाने चाहिए।

### 8.1. `onComplete` Handler

सबसे अधिक उपयोग किया जाने वाला event handler `onComplete` है, जो तुम्हारे workflow के समाप्त होने पर (चाहे यह सफल हो या fail) चलता है। आओ हमारे pipeline results को summarize करने के लिए एक जोड़ें।

अपने `main.nf` file में, अपनी workflow definition के अंदर event handler जोड़ो:
...
यह closure workflow पूरा होने पर चलता है। अंदर, तुम्हारे पास `workflow` object तक access है जो execution के बारे में उपयोगी properties प्रदान करता है।

अपना workflow चलाओ और तुम्हें अंत में यह summary दिखाई देगी!
...
Conditional logic जोड़कर इसे और उपयोगी बनाओ:
...
`onComplete` के अलावा, एक और event handler है जिसका तुम उपयोग कर सकते हो: `onError`, जो केवल workflow fail होने पर चलता है:
...
- **Event handler closures**: तुम्हारे workflow script में closures जो lifecycle के विभिन्न बिंदुओं पर चलते हैं
...
Event handlers दिखाते हैं कि तुम अपने workflow scripts में sophisticated logging और notification capabilities जोड़ने के लिए Nextflow भाषा की पूरी शक्ति का उपयोग कैसे कर सकते हो।
...
इस side quest में, तुमने एक व्यापक sample processing pipeline बनाई जो बुनियादी metadata handling से एक sophisticated, production-ready workflow तक विकसित हुई।
...
- **स्पष्ट कोड**: Dataflow बनाम scripting को समझना तुम्हें अधिक organized workflows लिखने में मदद करता है
- **Robust handling**: Safe navigation और Elvis operators workflows को missing data के लिए resilient बनाते हैं
- **Flexible processing**: Conditional logic तुम्हारे workflows को विभिन्न sample types को उचित रूप से process करने देती है
...
तुमने जो भी चुनौती हल की और पैटर्न सीखा, वह Nextflow workflows को scale करते समय developers को आने वाली वास्तविक समस्याओं को reflect करता है।

इन patterns को अपने काम में लागू करना तुम्हें robust, production-ready workflows बनाने में सक्षम करेगा।

### मुख्य पैटर्न

1.  **Dataflow बनाम Scripting:** तुमने dataflow operations (channel orchestration) और scripting (data manipulate करने वाला code) के बीच अंतर करना सीखा, जिसमें Channel बनाम List पर `collect` जैसे विभिन्न types पर operations के बीच महत्वपूर्ण अंतर शामिल हैं।
...
2.  **Advanced String Processing**: तुमने file names parsing के लिए regular expressions, processes में dynamic script generation, और variable interpolation (Nextflow बनाम Bash बनाम Shell) में महारत हासिल की।
...
3.  **Reusable Functions बनाना**: तुमने जटिल logic को named functions में extract करना सीखा जिन्हें channel operators से call किया जा सकता है, workflows को अधिक readable और maintainable बनाते हुए।

    - एक named function परिभाषित करो
    ...
    - Workflow में named function को call करो
...
4.  **Closures के साथ Dynamic Resource Directives**: तुमने input विशेषताओं के आधार पर adaptive resource allocation के लिए process directives में closures का उपयोग करना explore किया।
...
5.  **Conditional Logic और Process Control**: तुमने `.branch()` और `.filter()` operators का उपयोग करके intelligent routing जोड़ा, संक्षिप्त conditional expressions के लिए truthiness का लाभ उठाते हुए।

    - विभिन्न workflow branches के माध्यम से data route करने के लिए `.branch()` का उपयोग करो
    ...
    - 'Truthiness' के साथ data subset करने के लिए `filter()` का उपयोग करो
...
6.  **Safe Navigation और Elvis Operators**: तुमने null-safe property access के लिए `?.` और default values प्रदान करने के लिए `?:` का उपयोग करके pipeline को missing data के विरुद्ध robust बनाया।

    ```groovy
    def id = data?.sample?.id ?: 'unknown'
    ```

7.  **error() और log.warn के साथ Validation**: तुमने inputs को जल्दी validate करना और स्पष्ट error messages के साथ तेज़ी से fail करना सीखा।
...
जब तुम्हें अधिक advanced features explore करने की आवश्यकता हो तो इन resources की जांच अवश्य करो।

तुम अपने skills का अभ्यास और विस्तार करके लाभ उठाओगे:

- Dataflow और scripting के बीच उचित separation के साथ साफ़ workflows लिखो
...
[Side Quests के मेनू](./index.md) पर वापस जाओ या सूची में अगले विषय पर जाने के लिए पेज के नीचे दाईं ओर बटन पर क्लिक करो।
