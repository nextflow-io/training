...
Nextflow डेटा के साथ लचीले ढंग से काम करने के लिए शक्तिशाली उपकरण प्रदान करता है। एक प्रमुख क्षमता है डेटा को विभिन्न स्ट्रीम में विभाजित करना और फिर संबंधित आइटम को वापस एक साथ समूहित करना। यह बायोइन्फॉर्मेटिक्स वर्कफ़्लो में विशेष रूप से मूल्यवान है जहाँ तुम्हें विश्लेषण के लिए परिणामों को संयोजित करने से पहले विभिन्न प्रकार के नमूनों को अलग-अलग प्रोसेस करने की आवश्यकता होती है।

इसे मेल को सॉर्ट करने की तरह समझो: तुम गंतव्य के आधार पर पत्रों को अलग करते हो, प्रत्येक ढेर को अलग तरीके से प्रोसेस करते हो, फिर एक ही व्यक्ति के पास जाने वाली वस्तुओं को फिर से जोड़ते हो। Nextflow वैज्ञानिक डेटा के साथ ऐसा करने के लिए विशेष ऑपरेटर का उपयोग करता है। इस दृष्टिकोण को डिस्ट्रिब्यूटेड कंप्यूटिंग और बायोइन्फॉर्मेटिक्स वर्कफ़्लो में **scatter/gather** पैटर्न के रूप में भी जाना जाता है।

Nextflow का channel सिस्टम इस लचीलेपन का केंद्र है। Channel तुम्हारे वर्कफ़्लो के विभिन्न भागों को जोड़ते हैं, जिससे डेटा तुम्हारे विश्लेषण के माध्यम से प्रवाहित हो सकता है। तुम एक ही डेटा स्रोत से कई channel बना सकते हो, प्रत्येक channel को अलग तरीके से प्रोसेस कर सकते हो, और फिर आवश्यकता पड़ने पर channel को वापस एक साथ मर्ज कर सकते हो। यह दृष्टिकोण तुम्हें ऐसे वर्कफ़्लो डिज़ाइन करने देता है जो स्वाभाविक रूप से जटिल बायोइन्फॉर्मेटिक्स विश्लेषणों के शाखाओं और अभिसरण पथों को दर्शाते हैं।

### सीखने के लक्ष्य

इस side quest में, तुम Nextflow के channel ऑपरेटर का उपयोग करके डेटा को विभाजित और समूहित करना सीखोगे।
...
इस side quest के अंत तक, तुम निम्नलिखित तकनीकों का उपयोग करके डेटा स्ट्रीम को प्रभावी ढंग से अलग और संयोजित करने में सक्षम होगे:
...
ये कौशल तुम्हें ऐसे वर्कफ़्लो बनाने में मदद करेंगे जो कई इनपुट फ़ाइलों और विभिन्न प्रकार के डेटा को कुशलता से संभाल सकते हैं, जबकि स्वच्छ, रखरखाव योग्य कोड संरचना बनाए रखते हैं।
...
- बुनियादी Nextflow अवधारणाओं और तंत्रों (processes, channels, operators, फ़ाइलों के साथ काम करना, meta data) का उपयोग करने में सहज हो
...
यदि तुम कैंसर विश्लेषण से परिचित नहीं हो, तो बस जान लो कि यह एक प्रायोगिक मॉडल से मेल खाता है जो विपरीत विश्लेषण करने के लिए युग्मित tumor/normal नमूनों का उपयोग करता है।

विशेष रूप से रोगी A के लिए, हमारे पास तकनीकी प्रतिकृतियों (repeats) के दो सेट हैं।

!!! note "नोट"

    यदि तुम इस प्रायोगिक डिज़ाइन से परिचित नहीं हो तो चिंता न करो, यह इस ट्यूटोरियल को समझने के लिए महत्वपूर्ण नहीं है।
...
यह एक सामान्य बायोइन्फॉर्मेटिक्स पैटर्न का प्रतिनिधित्व करता है जहाँ तुम्हें स्वतंत्र प्रोसेसिंग के लिए डेटा को विभाजित करने की आवश्यकता होती है, फिर तुलनात्मक विश्लेषण के लिए संबंधित आइटम को फिर से संयोजित करना होता है।
...
चलो `splitCsv` के साथ नमूना डेटा को पढ़कर और इसे meta map पैटर्न में व्यवस्थित करके शुरुआत करते हैं। `main.nf` में, तुम देखोगे कि हमने पहले से ही वर्कफ़्लो शुरू किया है।
...
यदि तुमने [Metadata in workflows](./metadata.md) side quest पूरा किया है, तो तुम इस पैटर्न को पहचानोगे। हम CSV को पढ़ने के लिए `splitCsv` का उपयोग करेंगे और तुरंत डेटा को meta map के साथ संरचित करेंगे ताकि मेटाडेटा को फ़ाइल पथों से अलग किया जा सके।
...
यह `splitCsv` ऑपरेशन (headers के साथ CSV पढ़ना) और `map` ऑपरेशन (डेटा को `[meta, file]` tuples के रूप में संरचित करना) को एक चरण में जोड़ता है। उस परिवर्तन को लागू करो और pipeline चलाओ:
...
हम एक शर्त के आधार पर डेटा को फ़िल्टर करने के लिए [`filter` ऑपरेटर](https://www.nextflow.io/docs/latest/operator.html#filter) का उपयोग कर सकते हैं। मान लो कि हम केवल normal नमूनों को प्रोसेस करना चाहते हैं। हम `type` फ़ील्ड के आधार पर डेटा को फ़िल्टर करके ऐसा कर सकते हैं। चलो इसे `view` ऑपरेटर से पहले डालते हैं।
...
फ़िल्टर किए गए परिणाम देखने के लिए वर्कफ़्लो को फिर से चलाओ:
...
वर्तमान में हम CSV से सीधे बनाए गए channel पर फ़िल्टर लागू कर रहे हैं, लेकिन हम इसे एक से अधिक तरीकों से फ़िल्टर करना चाहते हैं, तो चलो normal नमूनों के लिए एक अलग फ़िल्टर किया गया channel बनाने के लिए तर्क को फिर से लिखते हैं:
...
Nextflow में channel को संयोजित करने के कई तरीके शामिल हैं, लेकिन इस मामले में सबसे उपयुक्त ऑपरेटर [`join`](https://www.nextflow.io/docs/latest/operator.html#join) है। यदि तुम SQL से परिचित हो, तो यह `JOIN` ऑपरेशन की तरह कार्य करता है, जहाँ हम join करने के लिए key और join के प्रकार को निर्दिष्ट करते हैं।
...
यदि तुम्हारे पास अभी भी console आउटपुट उपलब्ध नहीं है, तो चलो हमारी डेटा संरचना की जांच करने और देखने के लिए pipeline चलाते हैं कि हमें `id` फ़ील्ड पर join करने के लिए इसे कैसे संशोधित करने की आवश्यकता है।
...
यह सूक्ष्म हो सकता है, लेकिन तुम्हें प्रत्येक tuple में पहला तत्व `id` फ़ील्ड होना चाहिए।
...
यह बताना थोड़ा मुश्किल है क्योंकि यह इतना चौड़ा है, लेकिन तुम्हें देखने में सक्षम होना चाहिए कि नमूनों को `id` फ़ील्ड द्वारा joined किया गया है। प्रत्येक tuple में अब यह प्रारूप है:
...
!!! warning "चेतावनी"
    ...
    `join` ऑपरेटर किसी भी बेमेल tuple को त्याग देगा। इस उदाहरण में, हमने यह सुनिश्चित किया कि सभी नमूने tumor और normal के लिए मेल खाते थे, लेकिन यदि यह सच नहीं है तो तुम्हें बेमेल tuple रखने के लिए पैरामीटर `remainder: true` का उपयोग करना होगा। अधिक विवरण के लिए [दस्तावेज़ीकरण](https://www.nextflow.io/docs/latest/operator.html#join) देखो।

तो अब तुम जानते हो कि tuple में किसी फ़ील्ड को अलग करने के लिए `map` का उपयोग कैसे करें, और पहले फ़ील्ड के आधार पर tuple को संयोजित करने के लिए `join` का उपयोग कैसे करें।
...
चलो एक नई joining key बनाने से शुरू करते हैं। हम पहले की तरह [`map` ऑपरेटर](https://www.nextflow.io/docs/latest/operator.html#map) का उपयोग करके ऐसा कर सकते हैं ताकि एक नया tuple बनाया जा सके जिसमें `id` और `repeat` फ़ील्ड पहले तत्व के रूप में हों।
...
अब हमें join होता दिखना चाहिए लेकिन `id` और `repeat` दोनों फ़ील्ड का उपयोग करते हुए। वर्कफ़्लो चलाओ:
...
ध्यान दो कि हमारे पास प्रत्येक joined परिणाम के पहले तत्व के रूप में दो तत्वों (`id` और `repeat` फ़ील्ड) का एक tuple है। यह दर्शाता है कि जटिल आइटम को joining key के रूप में कैसे उपयोग किया जा सकता है, जो एक ही स्थितियों से नमूनों के बीच काफी जटिल मिलान को सक्षम बनाता है।

यदि तुम विभिन्न keys पर join करने के अधिक तरीकों का पता लगाना चाहते हो, तो अतिरिक्त विकल्पों और उदाहरणों के लिए [join ऑपरेटर दस्तावेज़ीकरण](https://www.nextflow.io/docs/latest/operator.html#join) देखो।
...
ध्यान दो कि हम फ़ाइल पथ को `file()` का उपयोग करके एक Path object में भी convert करते हैं ताकि इस channel को प्राप्त करने वाली कोई भी process फ़ाइल को सही ढंग से handle कर सके (अधिक जानकारी के लिए [Working with files](./working_with_files.md) देखो)।

चलो अपने वर्कफ़्लो में closure को implement करते हैं:
...
यह जांचने के लिए कि सबकुछ अभी भी काम कर रहा है, वर्कफ़्लो को एक बार और चलाओ:
...
चूँकि `id` और `repeat` फ़ील्ड grouping key में उपलब्ध हैं, चलो डुप्लीकेशन से बचने के लिए प्रत्येक channel आइटम के बाकी हिस्सों से उन्हें हटाते हैं। हम केवल `type` फ़ील्ड के साथ एक नया map बनाने के लिए `subMap` method का उपयोग कर सकते हैं। यह दृष्टिकोण हमें हमारे डेटा संरचना में अतिरेक को समाप्त करते हुए सभी आवश्यक जानकारी बनाए रखने की अनुमति देता है।
...
यह देखने के लिए वर्कफ़्लो चलाओ कि यह कैसा दिखता है:
...
अब तुम्हारे पास एक वर्कफ़्लो है जो samplesheet को विभाजित कर सकता है, normal और tumor नमूनों को फ़िल्टर कर सकता है, उन्हें sample ID और replicate number के आधार पर जोड़ सकता है, फिर परिणाम प्रिंट कर सकता है।

यह बायोइन्फॉर्मेटिक्स वर्कफ़्लो में एक सामान्य पैटर्न है जहाँ तुम्हें स्वतंत्र रूप से प्रोसेस करने के बाद नमूनों या अन्य प्रकार के डेटा को मिलाने की आवश्यकता होती है, इसलिए यह एक उपयोगी कौशल है। अगले भाग में, हम एक नमूने को कई बार दोहराने पर नज़र डालेंगे।
...
चलो intervals का एक channel बनाकर शुरू करते हैं। चीजों को सरल रखने के लिए, हम केवल 3 intervals का उपयोग करेंगे जिन्हें हम मैन्युअल रूप से परिभाषित करेंगे। एक वास्तविक वर्कफ़्लो में, तुम इन्हें किसी फ़ाइल इनपुट से पढ़ सकते हो या बहुत सी interval फ़ाइलों के साथ एक channel बना सकते हो।
...
याद रखो, हम प्रत्येक नमूने को प्रत्येक interval के लिए दोहराना चाहते हैं। इसे कभी-कभी नमूनों और intervals का Cartesian product कहा जाता है। हम [`combine` ऑपरेटर](https://www.nextflow.io/docs/latest/operator.html#combine) का उपयोग करके इसे प्राप्त कर सकते हैं। यह channel 1 से प्रत्येक आइटम लेगा और इसे channel 2 में प्रत्येक आइटम के लिए दोहराएगा। चलो अपने वर्कफ़्लो में combine ऑपरेटर जोड़ते हैं:
...
अब चलो इसे चलाते हैं और देखते हैं क्या होता है:
...
हम अपने sample data को व्यवस्थित और पुनर्गठित करने के लिए `map` ऑपरेटर का उपयोग कर सकते हैं ताकि इसे समझना आसान हो। चलो intervals string को पहले तत्व में joining map में ले जाते हैं।
...
चलो इसे फिर से चलाते हैं और channel contents की जांच करते हैं:
...
!!! note "नोट"
    ...
    हम यहाँ `replicate` फ़ील्ड को हटा रहे हैं। ऐसा इसलिए है क्योंकि हमें इसकी आगे की downstream प्रोसेसिंग के लिए आवश्यकता नहीं है। इस ट्यूटोरियल को पूरा करने के बाद, देखो कि क्या तुम इसे बाद के grouping को प्रभावित किए बिना शामिल कर सकते हो!
...
ध्यान दो कि हमारे डेटा की संरचना बदल गई है और प्रत्येक channel तत्व के भीतर फ़ाइलें अब `[patientA_rep1_normal.bam, patientA_rep2_normal.bam]` जैसे tuples में हैं। ऐसा इसलिए है क्योंकि जब हम `groupTuple` का उपयोग करते हैं, तो Nextflow एक group के प्रत्येक नमूने के लिए अलग-अलग फ़ाइलों को संयोजित करता है। downstream डेटा को संभालने का प्रयास करते समय इसे याद रखना महत्वपूर्ण है।

!!! note "नोट"

    [`transpose`](https://www.nextflow.io/docs/latest/reference/operator.html#transpose) groupTuple का विपरीत है। यह channel में आइटम को अनपैक और flatten करता है। `transpose` जोड़कर ऊपर किए गए grouping को पूर्ववत करने का प्रयास करो!
...
इस side quest में, तुमने channels का उपयोग करके डेटा को विभाजित और समूहित करना सीखा।

Pipeline के माध्यम से बहते डेटा को संशोधित करके, तुम loops या while statements का उपयोग किए बिना एक scalable pipeline बना सकते हो, जो अधिक पारंपरिक दृष्टिकोणों की तुलना में कई फायदे प्रदान करता है:
...
इन channel operations में महारत हासिल करने से तुम लचीली, scalable pipelines बना सकोगे जो loops या iterative programming का सहारा लिए बिना जटिल डेटा संबंधों को संभालती हैं, जिससे Nextflow स्वचालित रूप से execution को optimize और स्वतंत्र operations को parallelize कर सके।
...
[Side Quests के मेनू](./index.md) पर लौटो या सूची में अगले विषय पर जाने के लिए पृष्ठ के नीचे दाईं ओर बटन पर क्लिक करो।
