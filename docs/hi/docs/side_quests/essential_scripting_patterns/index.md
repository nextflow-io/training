...
Closure `#!groovy { row -> return row }` एक parameter `row` लेता है (कोई भी नाम हो सकता है: `item`, `sample`, आदि)।
...
!!! note "नोट"
    ...
    **Maps और Metadata**: Maps Nextflow में metadata के साथ काम करने के लिए fundamental हैं। metadata maps के साथ काम करने की अधिक विस्तृत व्याख्या के लिए, [Working with metadata](../metadata/index.md) side quest देखो।
...
!!! tip "सुझाव: Spread बनाम Collect कब Use करें"
    ...
    - **Collect use करो** transformations या complex logic के लिए: `#!groovy samples.collect { it.id.toUpperCase() }`, `#!groovy samples.collect { [it.id, it.quality > 40] }`
...
अगर तुम यह run करते हो, तो तुम्हें एक error दिखेगी - Nextflow `#!groovy ${USER}` को एक Nextflow variable के रूप में interpret करने की कोशिश करता है जो exist नहीं करता।
...
- **Variable interpolation**: यह समझना कि Nextflow strings कब interpret करता है बनाम shell कब करता है
  - `#!groovy ${var}` - Nextflow variables (workflow compile time पर Nextflow द्वारा interpolated)
  - `#!groovy \${var}` - Shell environment variables (escaped, runtime पर bash को pass)
  - `#!groovy \$(cmd)` - Shell command substitution (escaped, runtime पर bash द्वारा execute)
...
Closure `#!groovy { meta.depth > 40000000 ? 2 : 1 }` **ternary operator** (Section 1.1 में cover किया गया) का उपयोग करता है और प्रत्येक task के लिए evaluate किया जाता है, जिससे per-sample resource allocation possible होती है। High-depth samples (>40M reads) को 2 CPUs मिलते हैं, जबकि बाकी को 1 CPU मिलता है।
...
इस case में तीनों samples filter को satisfy करते हैं, इसलिए हर sample पाइपलाइन में आगे जाता है।
एक stricter threshold low-depth samples को exclude करेगा और run होने वाले tasks की संख्या कम करेगा।
...
[Side Quests के menu](../index.md) पर वापस जाओ या list में अगले topic पर जाने के लिए page के bottom right में button click करो।
