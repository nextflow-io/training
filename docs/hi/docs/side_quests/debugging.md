...
workflow {
    ...
    // एरर: 2 channels पास कर रहे हैं लेकिन process केवल 1 की अपेक्षा करता है
...
workflow {
    ...
    .view { "Channel content: $it" }  // Debug: मूल channel सामग्री दिखाएं
    .map { tuple -> tuple[0] }        // Transform: पहला तत्व निकालें
    .view { "After mapping: $it" }    // Debug: परिवर्तित channel सामग्री दिखाएं
...
=== "बाद में"
    ...
    process PROCESS_FILES {
        ...
        output:
        path "${sample_name}_output.txt"  // ठीक किया: स्क्रिप्ट आउटपुट से मिलान करें
...
process PROCESS_FILES {
    ...
    time '1 ms'  // एरर: अवास्तविक समय सीमा
...
=== "बाद में"
    ...
    process PROCESS_FILES {
        ...
        time '100 s'  // ठीक किया: वास्तविक समय सीमा
...
process PROCESS_FILES {
    debug true  // रियल-टाइम आउटपुट सक्षम करें
...
अब व्यवस्थित डिबगिंग दृष्टिकोण को अभ्यास में लाने का समय है। वर्कफ़्लो `buggy_workflow.nf` में कई सामान्य एरर हैं जो वास्तविक दुनिया के विकास में आपको मिलने वाली समस्याओं के परकारों का प्रतिनिधित्व करती हैं।
    ...
    **आपके पास उपलब्ध डिबगिंग टूल्स:**
    ...
    ??? solution "समाधान"
        ...
        **एरर 2: सिंटैक्स एरर - गुम Closing Brace**
        ...
        """
        // एरर: processFiles process के लिए closing brace गुम है
        ```
        ...
        **एरर 4: अपरिभाषित वेरिएबल एरर**
        ```groovy linenums="87"
        heavy_ch = heavyProcess(sample_ids)  // एरर: sample_ids अपरिभाषित
        ```
        ...
        **एरर 7: Bash वेरिएबल Escaping एरर**
        ```groovy linenums="48"
        echo "Heavy computation $i for ${sample_id}"  // एरर: $i escaped नहीं है
        ```
        ...
        **एरर 9: आउटपुट फ़ाइल नाम बेमेल**
        ```groovy linenums="49"
        done > ${sample_id}.txt  // एरर: गलत फ़ाइलनाम, आउटपुट घोषणा से मेल खाना चाहिए
        ```
        ...
        **एरर 10: आउटपुट फ़ाइल नाम बेमेल**
        ```groovy linenums="88"
        file_ch = channel.fromPath("*.txt") // एरर: pwd से इनपुट लेने का प्रयास कर रहे हैं न कि process से
        handleFiles(file_ch)
        ```
