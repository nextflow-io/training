...
=== "बाद में"
    ...
    process PROCESS_FILES {
        ...
        """
        prefix="${sample_name}_output"
        echo "Processing ${sample_name}" > \${prefix}.txt  # ठीक किया: डॉलर चिह्न को एस्केप किया
        """
    }
...
एरर मैसेज में square brackets यहाँ सुराग प्रदान करते हैं - process tuple को एक single value के रूप में treat कर रही है, जो हम नहीं चाहते। आइए `bad_channel_shape.nf` की जांच करें:
...
आप देख सकते हैं कि हम tuples से बना एक channel generate कर रहे हैं: `['sample1', 'file1.txt']`, लेकिन process एक single value की अपेक्षा करती है, `val sample_name`। निष्पादित कमांड दिखाता है कि process `[sample3, file3.txt]_output.txt` नाम की फ़ाइल बनाने की कोशिश कर रही है, जो इच्छित आउटपुट नहीं है।

#### कोड को ठीक करें

इसे ठीक करने के लिए, यदि process को दोनों इनपुट की आवश्यकता है तो हम process को tuple स्वीकार करने के लिए समायोजित कर सकते हैं:
...
=== "विकल्प 1: Process में tuple स्वीकार करें"
    ...
    === "बाद में"
        ...
        process PROCESS_FILES {
            input:
                tuple val(sample_name), val(file_name)  // ठीक किया: tuple स्वीकार करें
...
=== "विकल्प 2: पहला तत्व निकालें"
    ...
    === "बाद में"
        ...
        workflow {
            ...
            PROCESS_FILES(input_ch.map { it[0] })  // ठीक किया: पहला तत्व निकालें
        }
        ```
...
समाधानों में से एक चुनें और वर्कफ़्लो को फिर से चलाएं:
...
Channels के लिए सबसे शक्तिशाली डिबगिंग टूल `.view()` operator है। `.view()` के साथ, आप डिबगिंग में मदद के लिए सभी चरणों में अपने channels के आकार को समझ सकते हैं।
...
workflow {
    ...
    .view { "Channel content: $it" }  // डिबग: मूल channel सामग्री दिखाएं
    .map { tuple -> tuple[0] }        // रूपांतरण: पहला तत्व निकालें
    .view { "After mapping: $it" }    // डिबग: रूपांतरित channel सामग्री दिखाएं
...
भविष्य में channel सामग्री को समझने के लिए `.view()` ऑपरेशन का अत्यधिक उपयोग करने से बचाने के लिए, मदद के लिए कुछ टिप्पणियां जोड़ना उचित है:
...
यह आपके वर्कफ़्लो की जटिलता बढ़ने और channel संरचना अधिक अपारदर्शी होने पर अधिक महत्वपूर्ण हो जाएगा।
...
कई channel संरचना एरर मान्य Nextflow सिंटैक्स के साथ बनाई जा सकती हैं। आप डेटा फ़्लो को समझकर, निरीक्षण के लिए `.view()` operators का उपयोग करके, और square brackets जैसे एरर मैसेज पैटर्न को पहचानकर channel संरचना एरर को डिबग कर सकते हैं जो अप्रत्याशित tuple संरचनाओं को इंगित करते हैं।
...
Processes से संबंधित अधिकांश एरर जो आपको मिलेंगी वे कमांड बनाने में की गई गलतियों से संबंधित होंगी, या अंतर्निहित सॉफ़्टवेयर से संबंधित समस्याओं से। उस ने कहा, ऊपर channel समस्याओं के समान, आप process परिभाषा में गलतियां कर सकते हैं जो सिंटैक्स एरर के रूप में योग्य नहीं हैं, लेकिन जो रन टाइम पर एरर का कारण बनेंगी।

### 3.1. गुम आउटपुट फ़ाइलें

Processes लिखते समय एक सामान्य एरर कुछ ऐसा करना है जो process की अपेक्षा और जो उत्पन्न होता है के बीच बेमेल उत्पन्न करता है।
...
एरर मैसेज इंगित करता है कि process को `sample3.txt` नाम की आउटपुट फ़ाइल उत्पन्न करने की उम्मीद थी, लेकिन स्क्रिप्ट वास्तव में `sample3_output.txt` बनाती है। आइए `missing_output.nf` में process परिभाषा की जांच करें:
...
process PROCESS_FILES {
    ...
    output:
    path "${sample_name}.txt"  // अपेक्षा: sample3.txt
...
आपको `output:` ब्लॉक में आउटपुट फ़ाइल नाम और स्क्रिप्ट में उपयोग किए गए के बीच बेमेल देखना चाहिए। यह बेमेल process को विफल करने का कारण बनता है। यदि आपको इस प्रकार की एरर मिलती है, तो वापस जाएं और जांचें कि आपकी process परिभाषा और आपके आउटपुट ब्लॉक के बीच आउटपुट मेल खाते हैं।

यदि समस्या अभी भी स्पष्ट नहीं है, तो वास्तविक आउटपुट फ़ाइलों की पहचान करने के लिए work directory की जांच करें:
...
इस उदाहरण के लिए यह हमें हाइलाइट करेगा कि एक `_output` suffix आउटपुट फ़ाइल नाम में शामिल किया जा रहा है, हमारी `output:` परिभाषा के विपरीत।
...
=== "बाद में"
    ...
    process PROCESS_FILES {
        ...
        output:
        path "${sample_name}_output.txt"  // ठीक किया: स्क्रिप्ट आउटपुट से मेल खाएं
...
एक अन्य वर्ग की एरर सॉफ़्टवेयर प्रावधान में गलतियों के कारण होती है। `missing_software.nf` एक सिंटैक्टिकली मान्य वर्कफ़्लो है, लेकिन यह कुछ बाहरी सॉफ़्टवेयर पर निर्भर करता है जो `cowpy` कमांड प्रदान करता है जिसका यह उपयोग करता है।
...
Process को हमारे द्वारा निर्दिष्ट कमांड तक पहुंच नहीं है। कभी-कभी यह इसलिए होता है क्योंकि एक स्क्रिप्ट workflow `bin` directory में मौजूद है, लेकिन executable नहीं बनाई गई है। अन्य समय यह इसलिए होता है क्योंकि सॉफ़्टवेयर container या environment में इंस्टॉल नहीं है जहां वर्कफ़्लो चल रहा है।

#### कोड की जांच करें

उस `127` exit code पर ध्यान दें - यह आपको बिल्कुल समस्या बताता है। आइए `missing_software.nf` की जांच करें:
...
हम यहाँ थोड़े धोखेबाज रहे हैं, और वास्तव में कोड में कुछ भी गलत नहीं है। हमें बस process को इस तरह से चलाने के लिए आवश्यक कॉन्फ़िगरेशन निर्दिष्ट करने की आवश्यकता है कि इसे प्रश्न में कमांड तक पहुंच हो। इस मामले में process में एक container परिभाषा है, इसलिए हमें बस Docker सक्षम के साथ वर्कफ़्लो चलाने की आवश्यकता है।

#### पाइपलाइन चलाएं

हमने आपके लिए `nextflow.config` में एक Docker profile सेट अप किया है, इसलिए आप वर्कफ़्लो को इसके साथ चला सकते हैं:
...
!!! note
    ...
    Nextflow containers का उपयोग कैसे करता है इसके बारे में अधिक जानने के लिए, [Hello Nextflow](../hello_nextflow/05_hello_containers.md) देखें

### 3.3. खराब संसाधन कॉन्फ़िगरेशन

उत्पादन उपयोग में, आप अपनी processes पर संसाधनों को कॉन्फ़िगर करेंगे। उदाहरण के लिए `memory` आपकी process के लिए उपलब्ध अधिकतम मेमोरी को परिभाषित करता है, और यदि process उससे अधिक हो जाती है, तो आपका scheduler आमतौर पर process को kill कर देगा और `137` का exit code लौटाएगा। हम यहाँ इसे प्रदर्शित नहीं कर सकते क्योंकि हम `local` executor का उपयोग कर रहे हैं, लेकिन हम `time` के साथ कुछ समान दिखा सकते हैं।

#### पाइपलाइन चलाएं

`bad_resources.nf` में 1 millisecond के समय की अवास्तविक सीमा के साथ process कॉन्फ़िगरेशन है:
...
process PROCESS_FILES {
    ...
    time '1 ms'  // ERROR: अवास्तविक समय सीमा
    ...
    script:
    """
    sleep 1  // 1 सेकंड लेता है, लेकिन समय सीमा 1ms है
    cowpy ${sample_name} > ${sample_name}_output.txt
    """
}
```

हम जानते हैं कि process एक सेकंड से अधिक समय लेगी (हमने यह सुनिश्चित करने के लिए वहाँ एक sleep जोड़ा है), लेकिन process 1 millisecond के बाद timeout होने के लिए सेट है। कोई अपने कॉन्फ़िगरेशन के साथ थोड़ा अवास्तविक रहा है!

#### कोड को ठीक करें

समय सीमा को एक यथार्थवादी मान तक बढ़ाएं:
...
=== "बाद में"
    ...
    process PROCESS_FILES {
        ...
        time '100 s'  // ठीक किया: यथार्थवादी समय सीमा
...
यदि आप सुनिश्चित करते हैं कि आप अपने एरर मैसेज पढ़ते हैं तो इस तरह की विफलताएं आपको बहुत लंबे समय तक परेशान नहीं करनी चाहिए। लेकिन सुनिश्चित करें कि आप उन कमांड की संसाधन आवश्यकताओं को समझते हैं जो आप चला रहे हैं ताकि आप अपने संसाधन निर्देशों को उचित रूप से कॉन्फ़िगर कर सकें।

### 3.4. Process डिबगिंग तकनीकें

जब processes विफल होती हैं या अप्रत्याशित रूप से व्यवहार करती हैं, तो आपको यह जांचने के लिए व्यवस्थित तकनीकों की आवश्यकता होती है कि क्या गलत हुआ। Work directory में वह सभी जानकारी होती है जो आपको process निष्पादन को डिबग करने के लिए चाहिए।

#### Work Directory निरीक्षण का उपयोग

Processes के लिए सबसे शक्तिशाली डिबगिंग टूल work directory की जांच करना है। जब एक process विफल होती है, तो Nextflow उस विशिष्ट process निष्पादन के लिए एक work directory बनाता है जिसमें वे सभी फ़ाइलें होती हैं जो यह समझने के लिए आवश्यक हैं कि क्या हुआ।

#### पाइपलाइन चलाएं

आइए work directory निरीक्षण को प्रदर्शित करने के लिए पहले के `missing_output.nf` उदाहरण का उपयोग करें (यदि आवश्यक हो तो आउटपुट नामकरण बेमेल को फिर से उत्पन्न करें):
...
#### Work directory की जांच करें

जब आपको यह एरर मिलती है, तो work directory में सभी डिबगिंग जानकारी होती है। एरर मैसेज से work directory path खोजें और इसकी सामग्री की जांच करें:

```bash
# एरर मैसेज से work directory खोजें
...
- **गुम quotes**: स्पेस वाले वेरिएबल को उचित quoting की आवश्यकता होती है
- **गलत फ़ाइल पथ**: इनपुट फ़ाइलें जो मौजूद नहीं हैं या गलत स्थानों में हैं
- **गलत वेरिएबल नाम**: वेरिएबल संदर्भों में टाइपो
- **गुम environment सेटअप**: कमांड जो विशिष्ट environments पर निर्भर करते हैं
...
- **Resource errors**: मेमोरी/समय सीमा पार हो गई

##### Standard आउटपुट की जांच करें

`.command.out` फ़ाइल दिखाती है कि आपके कमांड ने क्या उत्पन्न किया:

```bash
# standard आउटपुट देखें
...
- **डिबग जानकारी**: आपकी स्क्रिप्ट से कोई diagnostic आउटपुट

##### Exit Code की जांच करें

`.exitcode` फ़ाइल में process के लिए exit code होता है:
...
- **Exit code 127**: Command not found - सॉफ़्टवेयर इंस्टॉलेशन की जांच करें
- **Exit code 137**: Process killed - मेमोरी/समय सीमा की जांच करें

##### फ़ाइल अस्तित्व की जांच करें

जब processes गुम आउटपुट फ़ाइलों के कारण विफल होती हैं, तो जांचें कि वास्तव में कौन सी फ़ाइलें बनाई गईं:

```bash
# work directory में सभी फ़ाइलों की सूची बनाएं
...
- **Path समस्याएं**: गलत directories में बनाई गई फ़ाइलें
...
Process डिबगिंग के लिए यह समझने के लिए work directories की जांच करने की आवश्यकता होती है कि क्या गलत हुआ। मुख्य फ़ाइलों में `.command.sh` (निष्पादित स्क्रिप्ट), `.command.err` (एरर मैसेज), और `.command.out` (standard आउटपुट) शामिल हैं। 127 (command not found) और 137 (process killed) जैसे exit codes विफलता के प्रकार के बारे में तत्काल diagnostic सुराग प्रदान करते हैं।

### आगे क्या है?

Nextflow के बिल्ट-इन डिबगिंग टूल्स और troubleshooting के व्यवस्थित दृष्टिकोण के बारे में जानें।

---

## 4. बिल्ट-इन डिबगिंग टूल्स और उन्नत तकनीकें

Nextflow वर्कफ़्लो निष्पादन को डिबग और विश्लेषण करने के लिए कई शक्तिशाली बिल्ट-इन टूल्स प्रदान करता है। ये टूल्स आपको यह समझने में मदद करते हैं कि क्या गलत हुआ, यह कहाँ गलत हुआ, और इसे कुशलता से कैसे ठीक किया जाए।

### 4.1. Real-time Process आउटपुट

कभी-कभी आपको यह देखने की आवश्यकता होती है कि चल रही processes के अंदर क्या हो रहा है। आप real-time process आउटपुट सक्षम कर सकते हैं, जो आपको बिल्कुल दिखाता है कि प्रत्येक task निष्पादित होते समय क्या कर रहा है।

#### पाइपलाइन चलाएं

हमारे पहले के उदाहरणों से `bad_channel_shape_viewed.nf` ने `.view()` का उपयोग करके channel सामग्री को प्रिंट किया, लेकिन हम process के भीतर से ही वेरिएबल को echo करने के लिए `debug` directive का भी उपयोग कर सकते हैं, जिसे हम `bad_channel_shape_viewed_debug.nf` में प्रदर्शित करते हैं। वर्कफ़्लो चलाएं:
...
आइए `bad_channel_shape_viewed_debug.nf` की जांच करें यह देखने के लिए कि `debug` directive कैसे काम करता है:

```groovy title="bad_channel_shape_viewed_debug.nf" linenums="3" hl_lines="2"
process PROCESS_FILES {
    debug true  // real-time आउटपुट सक्षम करें
...
`debug` directive एक process के environment को समझने का एक त्वरित और सुविधाजनक तरीका हो सकता है।

### 4.2. Preview Mode

कभी-कभी आप कोई भी processes चलने से पहले समस्याओं को पकड़ना चाहते हैं। Nextflow इस प्रकार की proactive डिबगिंग के लिए एक flag प्रदान करता है: `-preview`।

#### पाइपलाइन चलाएं

Preview mode आपको कमांड निष्पादित किए बिना वर्कफ़्लो लॉजिक का परीक्षण करने देता है। यह आपके वर्कफ़्लो की संरचना की जांच करने और यह सुनिश्चित करने के लिए काफी उपयोगी हो सकता है कि processes सही ढंग से जुड़े हुए हैं बिना कोई वास्तविक कमांड चलाए।

!!! note

    यदि आपने पहले `bad_syntax.nf` को ठीक किया है, तो इस कमांड को चलाने से पहले स्क्रिप्ट ब्लॉक के बाद closing brace को हटाकर सिंटैक्स एरर को फिर से पेश करें।
...
Preview mode विशेष रूप से कोई भी processes चलाए बिना सिंटैक्स एरर को जल्दी पकड़ने के लिए उपयोगी है। यह निष्पादन से पहले वर्कफ़्लो संरचना और process कनेक्शन को मान्य करता है।

### 4.3. लॉजिक परीक्षण के लिए Stub Running

कभी-कभी एरर को डिबग करना मुश्किल होता है क्योंकि कमांड बहुत लंबे समय तक चलते हैं, विशेष सॉफ़्टवेयर की आवश्यकता होती है, या जटिल कारणों से विफल होते हैं। Stub running आपको वास्तविक कमांड निष्पादित किए बिना वर्कफ़्लो लॉजिक का परीक्षण करने देता है।

#### पाइपलाइन चलाएं

जब आप एक Nextflow process विकसित कर रहे हों, तो आप 'dummy' कमांड को परिभाषित करने के लिए `stub` directive का उपयोग कर सकते हैं जो वास्तविक कमांड चलाए बिना सही रूप के आउटपुट उत्पन्न करते हैं। यह दृष्टिकोण विशेष रूप से मूल्यवान है जब आप यह सत्यापित करना चाहते हैं कि आपका वर्कफ़्लो लॉजिक वास्तविक सॉफ़्टवेयर की जटिलताओं से निपटने से पहले सही है।

उदाहरण के लिए, पहले से हमारे `missing_software.nf` को याद रखें? वह जहां हमारे पास गुम सॉफ़्टवेयर था जिसने वर्कफ़्लो को चलने से रोका जब तक हमने `-profile docker` नहीं जोड़ा? `missing_software_with_stub.nf` एक बहुत समान वर्कफ़्लो है। यदि हम इसे उसी तरह चलाते हैं, तो हम वही एरर उत्पन्न करेंगे:
...
हालांकि, यह वर्कफ़्लो `-stub-run` के साथ चलाने पर एरर उत्पन्न नहीं करेगा, यहां तक कि `docker` profile के बिना भी:
...
`missing_software.nf` के सापेक्ष, इस process में एक `stub:` directive है जो `script:` में निर्दिष्ट के बजाय उपयोग किए जाने वाले कमांड को निर्दिष्ट करता है, इस घटना में कि Nextflow को stub mode में चलाया जाता है।

हम यहाँ जो `touch` कमांड उपयोग कर रहे हैं वह किसी भी सॉफ़्टवेयर या उपयुक्त इनपुट पर निर्भर नहीं करता है, और सभी स्थितियों में चलेगा, जिससे हम process internals के बारे में चिंता किए बिना वर्कफ़्लो लॉजिक को डिबग कर सकते हैं।

**Stub running डिबग करने में मदद करता है:**

- Channel संरचना और डेटा फ़्लो
...
अब जब आपने व्यक्तिगत डिबगिंग तकनीकें सीख ली हैं - trace फ़ाइलों और work directories से लेकर preview mode, stub running, और resource monitoring तक - आइए उन्हें एक व्यवस्थित पद्धति में बांधें। एक संरचित दृष्टिकोण होने से आपको जटिल एरर से अभिभूत होने से रोकता है और सुनिश्चित करता है कि आप महत्वपूर्ण सुराग नहीं चूकते।

यह पद्धति हमने कवर किए गए सभी टूल्स को एक कुशल वर्कफ़्लो में जोड़ती है:

**चार-चरण डिबगिंग विधि:**

**चरण 1: सिंटैक्स एरर समाधान (5 मिनट)**

1. VSCode या आपके IDE में लाल अंडरलाइन की जांच करें
2. सिंटैक्स समस्याओं की पहचान करने के लिए `nextflow run workflow.nf -preview` चलाएं
3. सभी सिंटैक्स एरर को ठीक करें (गुम braces, trailing commas, आदि)
4. आगे बढ़ने से पहले सुनिश्चित करें कि वर्कफ़्लो सफलतापूर्वक parse होता है

**चरण 2: त्वरित मूल्यांकन (5 मिनट)**

1. रनटाइम एरर मैसेज को ध्यान से पढ़ें
2. जांचें कि यह रनटाइम, लॉजिक, या संसाधन एरर है या नहीं
3. बुनियादी वर्कफ़्लो लॉजिक का परीक्षण करने के लिए preview mode का उपयोग करें

**चरण 3: विस्तृत जांच (15-30 मिनट)**

1. विफल task की work directory खोजें
2. लॉग फ़ाइलों की जांच करें
3. Channels का निरीक्षण करने के लिए `.view()` operators जोड़ें
4. निष्पादन के बिना वर्कफ़्लो लॉजिक का परीक्षण करने के लिए `-stub-run` का उपयोग करें

**चरण 4: ठीक करें और मान्य करें (15 मिनट)**

1. न्यूनतम लक्षित फिक्स करें
...
!!! tip "कुशल डिबगिंग के लिए Resume का उपयोग"
    ...
    एक बार जब आपने एक समस्या की पहचान कर ली है, तो आपको अपने वर्कफ़्लो के सफल भागों को फिर से चलाने में समय बर्बाद किए बिना अपने फिक्स का परीक्षण करने के लिए एक कुशल तरीके की आवश्यकता है। Nextflow की `-resume` functionality डिबगिंग के लिए अमूल्य है।

    यदि आपने [Hello Nextflow](../hello_nextflow/) के माध्यम से काम किया है तो आपको `-resume` का सामना हुआ होगा, और यह महत्वपूर्ण है कि आप अपनी समस्या process से पहले की processes के चलने की प्रतीक्षा करते समय खुद को बचाने के लिए डिबगिंग करते समय इसका अच्छा उपयोग करें।

    **Resume डिबगिंग रणनीति:**

    1. विफलता तक वर्कफ़्लो चलाएं
    2. विफल task के लिए work directory की जांच करें
    3. विशिष्ट समस्या को ठीक करें
    4. केवल फिक्स का परीक्षण करने के लिए resume करें
    5. वर्कफ़्लो पूर्ण होने तक दोहराएं

#### डिबगिंग कॉन्फ़िगरेशन Profile
...
profiles {
    debug {
        process {
            ...
            // डिबगिंग के लिए रूढ़िवादी संसाधन
            maxForks = 1
            memory = '2.GB'
            cpus = 1
        }
...
फिर आप इस profile को सक्षम करके पाइपलाइन चला सकते हैं:

```bash
nextflow run workflow.nf -profile debug
```

यह profile real-time आउटपुट सक्षम करता है, work directories को संरक्षित करता है, और आसान डिबगिंग के लिए समानांतरता को सीमित करता है।

### 4.5. व्यावहारिक डिबगिंग अभ्यास

अब व्यवस्थित डिबगिंग दृष्टिकोण को व्यवहार में लाने का समय है। वर्कफ़्लो `buggy_workflow.nf` में कई सामान्य एरर हैं जो उन प्रकार की समस्याओं का प्रतिनिधित्व करते हैं जिनका सामना आप वास्तविक दुनिया के विकास में करेंगे।

!!! exercise

    `buggy_workflow.nf` में सभी एरर की पहचान करने और ठीक करने के लिए व्यवस्थित डिबगिंग दृष्टिकोण का उपयोग करें। यह वर्कफ़्लो CSV फ़ाइल से sample डेटा को प्रोसेस करने का प्रयास करता है लेकिन इसमें कई जानबूझकर बग हैं जो सामान्य डिबगिंग परिदृश्यों का प्रतिनिधित्व करते हैं।
    ...
    ??? failure "कमांड आउटपुट"
        ...
        यह गूढ़ एरर `params{}` ब्लॉक में लाइन 11-12 के आसपास एक parsing समसमस्या को इंगित करती है। v2 parser संरचनात्मक समस्याओं को जल्दी पकड़ता है।

    आपने जो चार-चरण डिबगिंग विधि सीखी है उसे लागू करें:

    **चरण 1: सिंटैक्स एरर समाधान**
    - VSCode या आपके IDE में लाल अंडरलाइन की जांच करें
    - सिंटैक्स समस्याओं की पहचान करने के लिए `nextflow run workflow.nf -preview` चलाएं
    - सभी सिंटैक्स एरर को ठीक करें (गुम braces, trailing commas, आदि)
    - आगे बढ़ने से पहले सुनिश्चित करें कि वर्कफ़्लो सफलतापूर्वक parse होता है

    **चरण 2: त्वरित मूल्यांकन**
    - रनटाइम एरर मैसेज को ध्यान से पढ़ें
    - पहचानें कि एरर रनटाइम, लॉजिक, या संसाधन-संबंधित हैं या नहीं
    - बुनियादी वर्कफ़्लो लॉजिक का परीक्षण करने के लिए `-preview` mode का उपयोग करें

    **चरण 3: विस्तृत जांच**
    - विफल tasks के लिए work directories की जांच करें
    - Channels का निरीक्षण करने के लिए `.view()` operators जोड़ें
    - Work directories में लॉग फ़ाइलों की जांच करें
    - निष्पादन के बिना वर्कफ़्लो लॉजिक का परीक्षण करने के लिए `-stub-run` का उपयोग करें

    **चरण 4: ठीक करें और मान्य करें**
    - लक्षित फिक्स करें
    - फिक्स का कुशलता से परीक्षण करने के लिए `-resume` का उपयोग करें
    - पूर्ण वर्कफ़्लो निष्पादन सत्यापित करें

    **आपके निपटान में डिबगिंग टूल्स:**
    ```bash
    ...
    ??? solution
        `buggy_workflow.nf` में 9 या 10 अलग-अलग एरर हैं (आप कैसे गिनते हैं इस पर निर्भर करते हुए) जो सभी प्रमुख डिबगिंग श्रेणियों को कवर करते हैं। यहाँ प्रत्येक एरर और इसे कैसे ठीक करें का एक व्यवस्थित विवरण है

        आइए उन सिंटैक्स एरर से शुरू करें:
        ...
        **एरर 2: सिंटैक्स एरर - गुम Closing Brace**
        ...
        """
        // ERROR: processFiles process के लिए closing brace गुम है
        ```
        ...
        **एरर 4: अपरिभाषित वेरिएबल एरर**
        ```groovy linenums="87"
        heavy_ch = heavyProcess(sample_ids)  // ERROR: sample_ids अपरिभाषित है
        ```
        **फिक्स:** सही channel का उपयोग करें और sample IDs निकालें
        ...
        इस बिंदु पर वर्कफ़्लो चलेगा, लेकिन हमें अभी भी एरर मिल रही होंगी (जैसे `processFiles` में `Path value cannot be null`), खराब channel संरचना के कारण।

        **एरर 5: Channel संरचना एरर - गलत Map आउटपुट**
        ```groovy linenums="83"
        .map { row -> row.sample_id }  // ERROR: processFiles tuple की अपेक्षा करता है
        ```
        **फिक्स:** tuple संरचना लौटाएं जो processFiles अपेक्षा करता है
        ```groovy linenums="83"
        .map { row -> [row.sample_id, file(row.fastq_path)] }
        ```

        लेकिन यह ऊपर `heavyProcess()` चलाने के लिए हमारे फिक्स को तोड़ देगा, इसलिए हमें केवल sample IDs को उस process में पास करने के लिए एक map का उपयोग करने की आवश्यकता होगी:

        **एरर 6: heavyProcess के लिए खराब channel संरचना**
        ```groovy linenums="87"
        heavy_ch = heavyProcess(input_ch)  // ERROR: input_ch में अब प्रति emission 2 तत्व हैं- heavyProcess को केवल 1 (पहला) की आवश्यकता है
        ```
        **फिक्स:** सही channel का उपयोग करें और sample IDs निकालें
        ```groovy linenums="87"
        ...
        अब हम थोड़ा आगे बढ़ते हैं लेकिन `No such variable: i` के बारे में एक एरर प्राप्त करते हैं, क्योंकि हमने एक Bash वेरिएबल को escape नहीं किया।

        **एरर 7: Bash वेरिएबल Escaping एरर**
        ```groovy linenums="48"
        echo "Heavy computation $i for ${sample_id}"  // ERROR: $i escaped नहीं है
        ```
        **फिक्स:** bash वेरिएबल को escape करें
        ...
        अब हमें `Process exceeded running time limit (1ms)` मिलता है, इसलिए हम संबंधित process के लिए run time limit को ठीक करते हैं:

        **एरर 8: संसाधन कॉन्फ़िगरेशन एरर**
        ```groovy linenums="36"
        time '1 ms'  // ERROR: अवास्तविक समय सीमा
        ```
        **फिक्स:** यथार्थवादी समय सीमा तक बढ़ाएं
        ```groovy linenums="36"
        time '100 s'
        ```
        ...
        **एरर 9: आउटपुट फ़ाइल नाम बेमेल**
        ```groovy linenums="49"
        done > ${sample_id}.txt  // ERROR: गलत फ़ाइलनाम, आउटपुट घोषणा से मेल खाना चाहिए
        ```
        ...
        **एरर 10: आउटपुट फ़ाइल नाम बेमेल**
        ```groovy linenums="88"
        file_ch = channel.fromPath("*.txt") // एरर: pwd से इनपुट लेने का प्रयास कर रहे हैं बजाय एक process के
        handleFiles(file_ch)
        ```
        ...
        */
        ...
        params{
            // गुम validation के साथ parameters
            input: Path = 'data/sample_data.csv'
            output: String = 'results'
        }

        /*
        * इनपुट/आउटपुट बेमेल के साथ process
        */
        ...
        /*
        * संसाधन समस्याओं के साथ process
        */
        ...
        /*
        * फ़ाइल हैंडलिंग समस्याओं के साथ process
        */
        process handleFiles {
        ...
        /*
        * channel समस्याओं के साथ मुख्य वर्कफ़्लो
        */
        workflow {

            // गलत उपयोग के साथ channel
            input_ch = channel
...
- **सिंटैक्स एरर**: गुम braces, trailing commas, अपरिभाषित वेरिएबल
...
1. **एरर मैसेज को ध्यान से पढ़ें** - वे अक्सर सीधे समस्या की ओर इशारा करते हैं
2. **व्यवस्थित दृष्टिकोण का उपयोग करें** - एक समय में एक एरर को ठीक करें और `-resume` के साथ परीक्षण करें
3. **डेटा फ़्लो को समझें** - channel संरचना एरर अक्सर सबसे सूक्ष्म होती हैं
4. **Work directories की जांच करें** - जब processes विफल होती हैं, तो लॉग आपको बिल्कुल बताते हैं कि क्या गलत हुआ
...
अपने स्वयं के काम में इन तकनीकों को लागू करने से आपको अपने कंप्यूटर से लड़ने में कम समय बिताने, समस्याओं को तेजी से हल करने और भविष्य की समस्याओं से खुद को बचाने में सक्षम बनाया जाएगा।

### मुख्य पैटर्न

**1. सिंटैक्स एरर की पहचान और फिक्स कैसे करें**:

- Nextflow एरर मैसेज की व्याख्या करना और समस्याओं का पता लगाना
- सामान्य सिंटैक्स एरर: गुम braces, गलत keywords, अपरिभाषित वेरिएबल
- Nextflow (Groovy) और Bash वेरिएबल के बीच अंतर करना
- जल्दी एरर पहचान के लिए VS Code एक्सटेंशन फीचर्स का उपयोग करना

```groovy
// गुम brace - IDE में लाल अंडरलाइन देखें
process FOO {
    script:
    """
    echo "hello"
...
// अपरिभाषित वेरिएबल - Bash वेरिएबल के लिए backslash से escape करें
echo "${undefined_var}"      // Nextflow वेरिएबल (यदि परिभाषित नहीं है तो एरर)
echo "\${bash_var}"          // Bash वेरिएबल (escaped)
```

**2. Channel संरचना समस्याओं को कैसे डिबग करें**:
...
- आउटपुट में square brackets जैसे एरर पैटर्न को पहचानना जो अप्रत्याशित tuple संरचनाओं को इंगित करते हैं
...
**3. Process निष्पादन समस्याओं को कैसे troubleshoot करें**:

- गुम आउटपुट फ़ाइल एरर का निदान करना
- Exit codes को समझना (गुम सॉफ़्टवेयर के लिए 127, मेमोरी समस्याओं के लिए 137)
- Work directories और कमांड फ़ाइलों की जांच करना
- संसाधनों को उचित रूप से कॉन्फ़िगर करना

```bash
# वास्तव में क्या निष्पादित किया गया था जांचें
...
# Exit code 137 = killed (मेमोरी/समय सीमा)
```

**4. Nextflow के बिल्ट-इन डिबगिंग टूल्स का उपयोग कैसे करें**:

- Preview mode और real-time डिबगिंग का लाभ उठाना
- लॉजिक परीक्षण के लिए stub running को लागू करना
...
!!! tip "त्वरित डिबगिंग संदर्भ"
    ...
    **Process विफलताएं?** → Work directory फ़ाइलों की जांच करें:

    - `.command.sh` - निष्पादित स्क्रिप्ट
    - `.command.err` - एरर मैसेज
    - `.exitcode` - exit status (127 = command not found, 137 = killed)

    **रहस्यमय व्यवहार?** → वर्कफ़्लो लॉजिक का परीक्षण करने के लिए `-stub-run` के साथ चलाएं

    **फिक्स किए?** → परीक्षण समय बचाने के लिए `-resume` का उपयोग करें: `nextflow run workflow.nf -resume`

---

### अतिरिक्त संसाधन

- [Nextflow troubleshooting guide](https://www.nextflow.io/docs/latest/troubleshooting.html): आधिकारिक troubleshooting डॉक्यूमेंटेशन
- [Nextflow channels को समझना](https://www.nextflow.io/docs/latest/channel.html): Channel प्रकारों और व्यवहार में गहरी डुबकी
...
- [nf-test](https://www.nf-test.com/): Nextflow पाइपलाइनों के लिए परीक्षण फ्रेमवर्क
- [Nextflow Slack community](https://www.nextflow.io/slack-invite.html): समुदाय से मदद प्राप्त करें

उत्पादन वर्कफ़्लो के लिए, विचार करें:

- बड़े पैमाने पर निगरानी और डिबगिंग के लिए [Seqera Platform](https://seqera.io/platform/) सेट अप करना
- पुनरुत्पादनीय सॉफ़्टवेयर environments के लिए [Wave containers](https://seqera.io/wave/) का उपयोग करना

**याद रखें:** प्रभावी डिबगिंग एक कौशल है जो अभ्यास के साथ सुधरता है। व्यवस्थित पद्धति और व्यापक टूलकिट जो आपने यहाँ प्राप्त की है, आपके पूरे Nextflow विकास यात्रा में आपकी अच्छी सेवा करेगी।

---

## आगे क्या है?

[Side Quests के मेनू](./index.md) पर वापस जाएं या सूची में अगले विषय पर जाने के लिए पृष्ठ के निचले दाएं कोने में बटन पर क्लिक करें।
