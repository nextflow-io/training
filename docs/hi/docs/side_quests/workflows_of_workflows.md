...
जब तुम एक pipeline विकसित कर रहे होते हो, तो तुम अक्सर खुद को विभिन्न डेटा प्रकारों या विश्लेषण चरणों के लिए समान प्रोसेस अनुक्रम बनाते हुए पाते हो। तुम इन प्रोसेस अनुक्रमों को कॉपी और पेस्ट करते हुए समाप्त हो सकते हो, जिससे डुप्लिकेट कोड बनता है जिसे बनाए रखना कठिन होता है; या तुम एक विशाल workflow बना सकते हो जो समझना और संशोधित करना मुश्किल है।
...
इस side quest के अंत तक, तुम निम्न करने में सक्षम होगे:
...
ये कौशल तुम्हें स्वच्छ, maintainable कोड संरचना बनाए रखते हुए जटिल pipelines बनाने में मदद करेंगे।
...
चलो उस डायरेक्टरी में चलते हैं जहां इस ट्यूटोरियल के लिए फ़ाइलें स्थित हैं।

```bash
cd side-quests/workflows_of_workflows
```

तुम VSCode को इस डायरेक्टरी पर ध्यान केंद्रित करने के लिए सेट कर सकते हो:
...
तुम्हें एक `modules` डायरेक्टरी मिलेगी जिसमें कई प्रोसेस परिभाषाएं हैं जो 'Hello Nextflow' में तुमने जो सीखा था उस पर आधारित हैं:
...
तुम्हारी चुनौती इन मॉड्यूल को दो अलग workflows में असेंबल करना है जिन्हें हम फिर एक मुख्य workflow में संयोजित करेंगे:
...
चलो एक workflow बनाकर शुरू करते हैं जो नामों को validate करता है और timestamped अभिवादन उत्पन्न करता है।
...
यह कोड `workflows/greeting.nf` में जोड़ो:
...
यह एक पूर्ण workflow है, जिसकी संरचना 'Hello Nextflow' ट्यूटोरियल में तुमने देखे गए workflows के समान है, जिसे हम स्वतंत्र रूप से परीक्षण कर सकते हैं। चलो अब इसे आज़माते हैं:
...
Composable workflows में 'Hello Nextflow' ट्यूटोरियल में तुमने देखे गए workflows से कुछ अंतर हैं:
...
चलो greeting workflow को इस संरचना से मेल खाने के लिए अपडेट करते हैं। कोड को निम्नलिखित में बदलो:
...
तुम देख सकते हो कि workflow को अब नाम दिया गया है और इसमें `take:` और `emit:` block है, और ये वे कनेक्शन हैं जिनका उपयोग हम उच्च स्तर के workflow को संयोजित करने के लिए करेंगे।
Workflow सामग्री भी `main:` block के अंदर रखी गई है। यह भी नोट करो कि हमने `names_ch` इनपुट channel घोषणा को हटा दिया है, क्योंकि इसे अब workflow को एक argument के रूप में पास किया जाता है।

चलो यह देखने के लिए workflow को फिर से परीक्षण करते हैं कि क्या यह अपेक्षा के अनुसार काम करता है:
...
यह तुम्हें एक अन्य नई अवधारणा के बारे में बताता है, एक 'entry workflow'। Entry workflow वह workflow है जो तब कॉल किया जाता है जब तुम एक Nextflow script चलाते हो। डिफ़ॉल्ट रूप से, Nextflow एक unnamed workflow को entry workflow के रूप में उपयोग करेगा, जब मौजूद हो, और यह वही है जो तुम अब तक कर रहे थे, workflow blocks इस तरह शुरू करते हुए:
...
नोट करो कि इस फ़ाइल में हमारी workflow entry un-named है, और ऐसा इसलिए है क्योंकि हम इसे entry workflow के रूप में उपयोग करने जा रहे हैं।
...
इस खंड में, तुमने कई महत्वपूर्ण अवधारणाएं सीखी हैं:
...
अब तुम्हारे पास एक काम करने वाला greeting workflow है जो:
...
यह modular दृष्टिकोण तुम्हें greeting workflow को स्वतंत्र रूप से परीक्षण करने या बड़े pipelines में एक component के रूप में उपयोग करने की अनुमति देता है।

---

## 2. Transform Workflow जोड़ें

अब चलो एक workflow बनाते हैं जो अभिवादन पर text transformations लागू करता है।
...
यह कोड `workflows/transform.nf` में जोड़ो:
...
हम यहां composable सिंटैक्स की व्याख्या नहीं दोहराएंगे, लेकिन ध्यान दो कि named workflow फिर से `take:` और `emit:` block के साथ घोषित किया गया है, और workflow सामग्री `main:` block के अंदर रखी गई है।

### 2.3. मुख्य workflow को अपडेट करें

दोनों workflows का उपयोग करने के लिए `main.nf` को अपडेट करो:
...
यदि तुम उन reversed फ़ाइलों में से एक पर नज़र डालते हो, तो तुम देखोगे कि यह अभिवादन का uppercase संस्करण उलटा है:
...
अब तुम्हारे पास एक पूर्ण pipeline होना चाहिए जो:
...
- Entry points को तुम्हारे pipeline के विभिन्न भागों को आवश्यकतानुसार चलाने के लिए कॉन्फ़िगर किया जा सकता है

_यह ध्यान रखना महत्वपूर्ण है कि workflows को कॉल करना थोड़ा processes को कॉल करने जैसा है, लेकिन यह वास्तव में एक ही चीज नहीं है। उदाहरण के लिए, तुम N आकार के channel के साथ इसे कॉल करके एक workflow को N बार नहीं चला सकते - तुम्हें workflow को N आकार के channel को पास करने और आंतरिक रूप से iterate करने की आवश्यकता होगी।_

अपने स्वयं के काम में इन तकनीकों को लागू करने से तुम अधिक परिष्कृत Nextflow pipelines बनाने में सक्षम होगे जो maintainable और scalable रहते हुए जटिल bioinformatics कार्यों को संभाल सकते हैं।
...
3.  **Entry points**: Nextflow को यह जानने के लिए एक unnamed entry workflow की आवश्यकता होती है कि निष्पादन कहां से शुरू करना है। यह entry workflow तुम्हारे named workflows को कॉल करता है।
...
[Side Quests के मेनू](./index.md) पर वापस जाओ या सूची में अगले विषय पर जाने के लिए पृष्ठ के निचले दाहिने कोने में बटन पर क्लिक करो।
