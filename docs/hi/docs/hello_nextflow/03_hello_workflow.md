...
यह familiar लगना चाहिए, सिवाय इसके कि अब हम explicitly दिखा रहे हैं कि process के outputs एक channel में package हैं, बिल्कुल वैसे ही जैसे inputs थे।
हम एक minute में उस output channel का अच्छा उपयोग करने जा रहे हैं।
...
Conveniently, Nextflow automatically process का output एक channel में package करता है, जैसा warmup section के diagram में दिखाया गया है।
हम process के output channel को `<process>.out` के रूप में refer कर सकते हैं।
...
यह दिखाता है कि तुम output settings को बहुत granular level पर control कर सकते हो, प्रत्येक individual output के लिए।
Paths या publish mode को किसी एक process के लिए बदलकर देखो कि क्या होता है।
...
Course में बाद में, तुम सीखोगे कि इन settings को multiple outputs के लिए structured तरीके से कैसे configure करें।
...
यह convenient है! लेकिन यह worth है कि second process के किसी call की work directory के अंदर भी देखें।
...
Notice करें कि दो `*-output` files हैं: first process का output साथ ही second का output भी।

First process का output वहाँ है क्योंकि Nextflow ने इसे वहाँ **stage** किया ताकि execution के लिए जो भी चाहिए वह same subdirectory में हो।

हालांकि, यह वास्तव में एक symbolic link है जो first process call की subdirectory में original file को point करता है।
By default, जब single machine पर run कर रहे हों जैसा हम यहाँ कर रहे हैं, Nextflow input और intermediate files को stage करने के लिए copies के बजाय symbolic links use करता है।

अब, आगे बढ़ने से पहले, इस बारे में सोचो कि हमने सिर्फ `sayHello` के output को `convertToUpper` के input से connect किया और दोनों processes series में run हो सकीं।
Nextflow ने individual input और output files handle करने और उन्हें दो commands के बीच pass करने का कठिन काम हमारे लिए किया।

यह एक कारण है कि Nextflow channels इतने powerful हैं: वे workflow steps को एक साथ connect करने में involved busywork का ध्यान रखते हैं।
...
हम इसे लिखना शुरू कर सकते हैं based on what we've seen before।
...
यह वह है जो हम confidence के साथ लिख सकते हैं based on what you've learned so far।
...
हमें `convertToUpper()` process के सभी calls से greetings collect करने की जरूरत है।
हम क्या जानते हैं कि हम workflow के previous step से क्या प्राप्त कर सकते हैं?

`convertToUpper()` द्वारा output किया गया channel उन individual files के paths contain करेगा जिनमें uppercased greetings हैं।
यह एक input slot के बराबर है; चलो simplicity के लिए इसे `input_files` कहते हैं।
...
Notice करें कि हम `path` prefix use करते हैं भले ही हम expect करते हैं कि इसमें multiple files हों।

#### 2.2.3. Concatenation command compose करें

यहाँ चीजें थोड़ी tricky हो सकती हैं, क्योंकि हमें arbitrary number of input files handle करने में सक्षम होना चाहिए।
Specifically, हम command को up front नहीं लिख सकते, इसलिए हमें Nextflow को बताना होगा कि runtime पर इसे कैसे compose करें based on what inputs flow into the process।

दूसरे शब्दों में, यदि हमारे पास एक input channel है जिसमें element `[file1.txt, file2.txt, file3.txt]` है, तो हमें Nextflow को इसे `cat file1.txt file2.txt file3.txt` में turn करना चाहिए।

Fortunately, Nextflow हमारे लिए यह करने में quite happy है यदि हम simply script command में `cat ${input_files}` लिखें।
...
Theory में यह किसी भी arbitrary number of input files handle कर सकता है।

!!! tip

    कुछ command-line tools प्रत्येक input file के लिए एक argument (जैसे `-input`) provide करने की require करते हैं।
    उस case में, हमें command compose करने के लिए थोड़ा extra काम करना होगा।
    तुम [Nextflow for Genomics](../../nf4_science/genomics/) training course में इसका example देख सकते हो।

### 2.3. Workflow में collection step add करें

अब हमें uppercasing step के output पर collection process call करना चाहिए।
...
यह successfully runs, including third step।

हालांकि, last line पर `collectGreetings()` के calls की number देखो।
हम केवल एक की expectation कर रहे थे, लेकिन तीन हैं।

अब final output file की contents देखो।
...
हमें कुछ ऐसा करना होगा जो Nextflow को explicitly बताए कि हम चाहते हैं कि third step `convertToUpper()` द्वारा output किए गए channel के सभी elements पर run हो।

### 2.4. Greetings को single input में collect करने के लिए operator use करें

हाँ, एक बार फिर हमारी problem का answer एक operator है।

Specifically, हम aptly-named [`collect()`](https://nextflow.io/docs/latest/reference/operator.html#collect) operator use करेंगे।

#### 2.4.1. `collect()` operator add करें

इस बार यह थोड़ा different दिखेगा क्योंकि हम channel factory के context में operator add नहीं कर रहे हैं; हम इसे एक output channel में add कर रहे हैं।
...
चलो channel contents के before और after states visualize करने के लिए कुछ `view()` statements भी include करते हैं।
...
`view()` statements कहीं भी जा सकते हैं जहाँ तुम चाहो; हमने उन्हें readability के लिए call के ठीक बाद रखा है।
...
यह successfully runs, हालांकि log output इससे थोड़ा messier दिख सकता है (हमने readability के लिए इसे clean किया)।
...
- एक single `After collect:` statement: तीनों file paths अब एक single element में packaged हैं।
...
Finally, तुम output file की contents देख सकते हो यह satisfy करने के लिए कि सब कुछ correctly काम किया।
...
इस बार हमारे final output file में तीनों greetings हैं। Success!

!!! note

    यदि तुम इसे `-resume` के बिना कई बार run करो, तो तुम देखोगे कि greetings का order एक run से दूसरे run में बदलता है।
    यह दिखाता है कि जिस order में elements process calls के through flow होते हैं वह consistent होने की guarantee नहीं है।

#### 2.4.4. Readability के लिए `view()` statements remove करें

अगले section पर move करने से पहले, हम recommend करते हैं कि तुम console output को clutter करने से बचने के लिए `view()` statements delete कर दो।
...
- Workflow में एक command-line parameter add करें (`--batch`) और इसे collector process को pass करें
...
अच्छी खबर: हम process definition में जितने चाहें input variables declare कर सकते हैं।
चलो इसे `batch_name` कहते हैं।
...
तुम अपने processes को जितने चाहें inputs expect करने के लिए set up कर सकते हो।
अभी, ये सभी required inputs के रूप में set up हैं; तुम्हें workflow के काम करने के लिए एक value provide करनी _होगी_।
...
हम variable को output file name में उसी तरह insert कर सकते हैं जैसे हमने पहले dynamic file names compose किए हैं।
...
अब हमें `batch_name` के लिए value supply करने और process call को feed करने का एक तरीका चाहिए।

#### 3.2.1. Parameter set up करने के लिए `params` use करें

तुम पहले से जानते हो कि CLI parameters declare करने के लिए `params` system कैसे use करें।
चलो एक `batch` parameter declare करने के लिए इसका उपयोग करते हैं (एक default value के साथ क्योंकि हम lazy हैं)।
...
बिल्कुल वैसे ही जैसे हमने `--input` के लिए demonstrate किया, तुम command line पर `--batch` के साथ value specify करके उस default value को override कर सकते हो।

#### 3.2.2. Process को `batch` parameter pass करें

Process को parameter की value provide करने के लिए, हमें इसे process call में add करना होगा।
...
तुम देखते हो कि process को multiple inputs provide करने के लिए, तुम simply उन्हें call parentheses में list करते हो, commas से separated।

!!! warning

    तुम्हें process को inputs उसी EXACT SAME ORDER में provide करने होंगे जैसे वे process के input definition block में listed हैं।

### 3.3. Workflow चलाएं

चलो command line पर batch name के साथ इसे run करने का प्रयास करते हैं।
...
यह successfully runs और desired output produce करता है:
...
अब, जब तक हम parameter को appropriately specify करें, inputs के दूसरे batches पर subsequent runs previous results को clobber नहीं करेंगे।
...
अभी तक हम ऐसे processes use कर रहे थे जिन्होंने प्रत्येक केवल एक output produce किया।
हम उनके respective outputs को `<process>.out` syntax use करके बहुत conveniently access कर पाए, जिसे हमने output को next process में pass करने के context में (e.g. `convertToUpper(sayHello.out)`) और `publish:` section के context में (e.g. `first_output = sayHello.out`) दोनों में use किया।

जब एक process एक से अधिक produce करता है तो क्या होता है?
...
Multiple outputs separate channels में packaged होंगे।
...
Demonstration purposes के लिए, मान लो हम दिए गए batch of inputs के लिए collect किए जा रहे greetings की count करना और इसे एक file में report करना चाहते हैं।

### 4.1. Greetings count और output करने के लिए process modify करें

इसके लिए process definition में दो key changes की require होगी: हमें greetings count करने और report file write करने का एक तरीका चाहिए, फिर हमें process के `output` block में उस report file को add करना होगा।

#### 4.1.1. Collected greetings की संख्या count करें

Conveniently, Nextflow हमें process definition के `script:` block में arbitrary code add करने देता है, जो इस तरह की चीजें करने के लिए बहुत handy आता है।

इसका मतलब है कि हम `input_files` array में files की संख्या प्राप्त करने के लिए Nextflow के built-in `size()` function use कर सकते हैं, और result को `echo` command के साथ file में write कर सकते हैं।
...
#### 4.1.2. Report file emit करें और outputs को name दें

Principle में हमें बस report file को `output:` block में add करना होगा।

हालांकि, जब हम यहाँ हैं, हम अपने output declarations में कुछ `emit:` tags भी add करने जा रहे हैं। ये हमें positional indices use करने के बजाय outputs को name द्वारा select करने में enable करेंगे।
...
`emit:` tags optional हैं, और हम केवल एक output में tag add कर सकते थे।
लेकिन जैसा saying जाती है, दोनों क्यों नहीं?

!!! tip

    यदि तुम `emit:` use करके process के outputs को name नहीं करते, तो भी तुम उन्हें उनके respective (zero-based) index use करके individually access कर सकते हो।
    उदाहरण के लिए, तुम first output प्राप्त करने के लिए `<process>.out[0]` use करोगे, second output के लिए `<process>.out[1]`, और so on।

    हम outputs name करना prefer करते हैं क्योंकि otherwise, error से wrong index grab करना बहुत आसान है, विशेष रूप से जब process बहुत सारे outputs produce करती है।

### 4.2. Workflow outputs update करें

अब जब `collectGreetings` process से दो outputs आ रहे हैं, तो `collectGreetings.out` output दो channels contain करता है:
...
हमें तदनुसार workflow outputs update करने की जरूरत है।
...
जब हम Part 5 (Containers) में अपनी pipeline में एक और step add करेंगे, तो हम easily `collectGreetings.out.outfile` को refer कर पाएंगे और इसे new process को hand कर पाएंगे (spoiler: new process को `cowpy` कहा जाता है)।
...
हमें `collected` output definition update करने की जरूरत नहीं है क्योंकि वह name नहीं बदला है।
हमें बस new output add करना होगा।

### 4.3. Workflow चलाएं

चलो greetings के current batch के साथ इसे run करने का प्रयास करते हैं।
...
इसे open करो verify करने के लिए कि workflow ने correctly greetings की count report की जो process किए गए थे।
...
CSV में और greetings add करने और test करने के लिए free feel करो कि क्या होता है।

### सीख

तुम जानते हो कि process को multiple named outputs कैसे emit करवाना और उन्हें workflow level पर appropriately कैसे handle करना।
...
- [ ] जब तुम multiple processes में data split करना चाहते हो
