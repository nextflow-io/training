...
4. एक process से निकलने वाले कई outputs को हैंडल करना

प्रदर्शन के लिए, हम भाग 1 और 2 के domain-agnostic Hello World उदाहरण पर निर्माण जारी रखेंगे।
...
यह परिचित लगना चाहिए, सिवाय इसके कि अब हम स्पष्ट रूप से दिखा रहे हैं कि process के outputs को एक channel में पैकेज किया जाता है, ठीक वैसे ही जैसे inputs थे।
...
इस तरह के एक सरल मामले के लिए (एक output से एक input), दो processes को जोड़ने के लिए हमें बस इतना ही करना है!
...
कोर्स में बाद में, तुम सीखोगे कि इन सेटिंग्स को structured तरीके से कई outputs के लिए कैसे configure किया जाए।
...
Console output में अब एक अतिरिक्त लाइन है जो नए process से मेल खाती है जिसे हमने अभी जोड़ा है।
...
अब, आगे बढ़ने से पहले, सोचो कि हमने बस `sayHello` के output को `convertToUpper` के input से जोड़ा और दोनों processes को series में चलाया जा सकता था।
Nextflow ने व्यक्तिगत input और output फ़ाइलों को हैंडल करने और उन्हें दो कमांड्स के बीच पास करने का कठिन काम हमारे लिए किया।
...
यह वह है जो हम अब तक सीखे हुए के आधार पर आत्मविश्वास के साथ लिख सकते हैं।
...
यहाँ चीजें थोड़ी मुश्किल हो सकती हैं, क्योंकि हमें input फ़ाइलों की एक arbitrary संख्या को हैंडल करने में सक्षम होना चाहिए।
...
सिद्धांत रूप में यह किसी भी arbitrary संख्या की input फ़ाइलों को हैंडल करना चाहिए।
...
हालाँकि, अंतिम लाइन पर `collectGreetings()` के लिए calls की संख्या देखो।
...
यह सफलतापूर्वक चलता है, हालाँकि log output इससे थोड़ा अधिक messy लग सकता है (हमने इसे readability के लिए साफ किया)।
...
अंत में, तुम output फ़ाइल की सामग्री पर एक नज़र डाल सकते हो यह संतुष्ट करने के लिए कि सब कुछ सही ढंग से काम किया।
...
अगले सेक्शन में जाने से पहले, हम अनुशंसा करते हैं कि तुम console output को cluttering से बचाने के लिए `view()` statements को delete कर दो।
...
हम अंतिम output फ़ाइल को कुछ विशिष्ट नाम देने में सक्षम होना चाहते हैं ताकि greetings के बाद के batches को प्रोसेस करते समय अंतिम परिणामों को overwrite न किया जाए।
...
!!! warning "चेतावनी"
    ...
    तुम्हें process को inputs को उसी EXACT SAME ORDER में प्रदान करना होगा जैसे वे process के input परिभाषा ब्लॉक में listed हैं।
...
सीखो कि कई outputs को कैसे emit किया जाए और उन्हें सुविधाजनक तरीके से कैसे हैंडल किया जाए।
...
क्या होता है जब एक process एक से अधिक उत्पन्न करता है?
हम कई outputs को कैसे हैंडल करते हैं?
...
प्रदर्शन उद्देश्यों के लिए, मान लें कि हम inputs के एक दिए गए batch के लिए एकत्र की जा रही greetings की संख्या को count करना चाहते हैं और इसे एक फ़ाइल में रिपोर्ट करना चाहते हैं।

### 4.1. Greetings की संख्या को count और output करने के लिए process को संशोधित करें

इसके लिए process परिभाषा में दो प्रमुख परिवर्तनों की आवश्यकता होगी: हमें greetings को count करने और एक report फ़ाइल लिखने का एक तरीका चाहिए, फिर हमें उस report फ़ाइल को process के `output` ब्लॉक में जोड़ना होगा।

#### 4.1.1. एकत्र की गई greetings की संख्या count करें
...
सिद्धांत रूप में हमें बस report फ़ाइल को `output:` ब्लॉक में जोड़ना है।
...
`workflow block` में, निम्नलिखित कोड परिवर्तन करें:
...
इसे खोलो यह सत्यापित करने के लिए कि workflow ने सही ढंग से प्रोसेस की गई greetings की count की रिपोर्ट की।
...
तुम जानते हो कि एक process को कई named outputs कैसे emit करवाया जाए और workflow स्तर पर उन्हें उचित रूप से कैसे हैंडल किया जाए।
...
- [ ] जब तुम items को parallel में प्रोसेस करना चाहते हो
