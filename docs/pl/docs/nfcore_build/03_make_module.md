# Część 3: Tworzenie modułu nf-core
...
??? info "Jak zacząć od tej sekcji"
    ...
    Ta sekcja zakłada, że ukończyłeś [Część 2: Użycie modułu nf-core](./02_use_module.md) i zintegrowałeś moduł `FIND_CONCATENATE` ze Swoim pipeline'em.
    ...
    Uruchom te polecenia z wnętrza katalogu `nfcore-build/`:
    ...
    To daje Ci pipeline z już zintegrowanym modułem `FIND_CONCATENATE`.
...
W obecnej wersji pipeline `core-hello` wyodrębniamy plik z krotki wyjściowej `FIND_CONCATENATE`, aby przekazać go do `COWPY`, jak pokazano w górnej części diagramu poniżej.
...
Jak widzisz, zmieniliśmy zarówno **główne wejście**, jak i **wyjście** na krotkę zgodną ze wzorcem `tuple val(meta), path(input_file)` wprowadzonym w Części 2 tego szkolenia.
...
Teraz, gdy wyjście `FIND_CONCATENATE` i wejście `COWPY` mają tę samą 'kształt', tzn. oba składają się ze struktury `tuple val(meta), path(input_file)`, możemy po prostu połączyć je bezpośrednio zamiast konieczności jawnego wyodrębniania pliku z wyjścia procesu `FIND_CONCATENATE`.
...
Teraz wywołujemy `COWPY` bezpośrednio na `FIND_CONCATENATE.out.file_out`.
...
Pipeline powinien działać pomyślnie, a metadane teraz przepływają z `FIND_CONCATENATE` przez `COWPY`.
...
Składnia `withName:` przypisuje tę konfigurację tylko do procesu `COWPY`, a `#!groovy ext.args = { "-c ${params.character}" }` po prostu tworzy ciąg, który będzie zawierał wartość parametru `character`.
...
- **Komponent jest teraz przenośny** — może być ponownie użyty w innych pipeline'ach bez oczekiwania konkretnej nazwy parametru
- Konfiguracja jest **scentralizowana** w `modules.config`, utrzymując logikę workflow'u czystą

Używając pliku `modules.config` jako miejsca, w którym wszystkie pipeline'y centralizują konfigurację dla poszczególnych komponentów, sprawiamy, że nasze moduły są bardziej wielokrotnego użytku w różnych środowiskach.
...
1. **W bloku `script:` dodaliśmy linię `#!groovy prefix = task.ext.prefix ?: "${meta.id}"`.**
...
2. **W linii poleceń zastąpiliśmy `#!groovy cowpy-${input_file}` przez `#!groovy ${prefix}.txt`.**
   To tutaj Nextflow wstawi wartość `prefix` określoną przez powyższą linię.

3. **W bloku `output:` zastąpiliśmy `#!groovy path("cowpy-${input_file}")` przez `#!groovy path("${prefix}.txt")`.**
...
- **`core-hello-results`** — Ścieżka wyjściowa ustawiona za pomocą `--outdir` w linii poleceń, która otrzymywała logi nf-core i wyniki publikowane przez `FIND_CONCATENATE`.
...
Posiadanie indywidualnych modułów podejmujących decyzje o publikowaniu wyraźnie nie jest właściwym podejściem, szczególnie w świecie, w którym ten sam moduł może być używany w wielu różnych pipeline'ach przez ludzi, którzy mają różne potrzeby lub preferencje.
Chcemy mieć możliwość kontrolowania, gdzie publikowane są wyjścia na poziomie konfiguracji workflow.

„Hej," możesz powiedzieć, „`FIND_CONCATENATE` wysyła swoje wyjścia do `--outdir`. Może powinniśmy skopiować jego dyrektywę `publishDir`?"
...
!!! info "Info"
    ...
    Zawiera on obecnie tylko informacje o wersji z `FIND_CONCATENATE`, ponieważ `COWPY` jeszcze nie raportuje swojej wersji.
...
- **`path:`** Określa katalog wyjściowy na podstawie nazwy procesu.
  Pełna nazwa procesu zawarta w `task.process` obejmuje hierarchię importów workflow'u i modułów (takich jak `CORE_HELLO:HELLO:FIND_CONCATENATE`).
  ...
  To właśnie określa, że wyniki `FIND_CONCATENATE` są publikowane w `#!groovy ${params.outdir}/find/`.
...
To powiedziawszy, możesz zdecydować, że chcesz zorganizować Swoje wyjścia inaczej, a dobrą wiadomością jest to, że łatwo to zrobić.
...
Kolekcja po stronie workflow'u — blok `channel.topic("versions")`, który widziałeś w zastępczym workflow'ie w Części 1 — subskrybuje temat i automatycznie zapisuje ten zbiorczy raport.

!!! info "Zgodność wsteczna"
...
!!! tip "Wskazówka"
    ...
    Wygenerowany plik testowy używa nf-test, frameworka testowego dla pipeline'ów i modułów Nextflow. Aby dowiedzieć się, jak pisać i uruchamiać te testy, zobacz [zadanie dodatkowe nf-test](../side_quests/nf_test/index.md).
...
Dla środowiska Conda kod modułu określa `#!groovy conda "${moduleDir}/environment.yml"`, co oznacza, że powinno być skonfigurowane w pliku `environment.yml`.
...
=== "Przed"
    ...
    channels:
      ...
      - bioconda
    dependencies:
      # TODO nf-core: Wymień wymagane pakiety Conda.
      #               Oprogramowanie MUSI być przypięte do kanału (np. "bioconda"), wersji (np. "1.10").
      #               Dla Conda, build (np. "h9402c20_2") MUSI być WYKLUCZONY, aby umożliwić instalację na różnych systemach operacyjnych.
      - "YOUR-TOOL-HERE"
    ```
...
- Nazwę pliku wyjściowego używając konfigurowalnego wzorca prefiksu (`#!groovy ${prefix}.txt` zamiast wieloznacznika `*`)
- Opisową nazwę emit (`cowpy_output` zamiast ogólnego `output`)
- Statyczny ciąg wersji (`#!groovy val("1.1.5")`) w miejsce szablonowego `#!groovy eval("cowpy --version")`, zgodnie z ręcznym modułem z sekcji 1.6 (narzędzie `cowpy` nie udostępnia flagi `--version`)

Jeśli używasz serwera językowego Nextflow do walidacji składni, część `#!groovy ${prefix}` zostanie oznaczona jako błąd na tym etapie, ponieważ jeszcze nie dodaliśmy jej do bloku script.
...
- Wypełnij rzeczywistym poleceniem `cowpy`, które używa zarówno `$args`, jak i `#!groovy ${prefix}.txt`
...
- Stub tworzy pusty plik `#!groovy ${prefix}.txt` pasujący do tego, co produkuje blok script
...
Gdy będziesz gotowy, kontynuuj do [Części 4: Walidacja wejścia](./04_input_validation.md), aby dowiedzieć się, jak dodać walidację wejścia opartą na schemacie do swojego pipeline'a.
