...
Nextflow to język programowania działający na Wirtualnej Maszynie Javy. Choć Nextflow jest zbudowany na [Groovy](http://groovy-lang.org/) i dzieli z nim znaczną część składni, Nextflow to coś więcej niż tylko „Groovy z rozszerzeniami" — jest to samodzielny język z w pełni określoną [składnią](https://nextflow.io/docs/latest/reference/syntax.html) i [biblioteką standardową](https://nextflow.io/docs/latest/reference/stdlib.html).

Możesz napisać sporo kodu w Nextflow, nie wychodząc poza podstawową składnię zmiennych, map i list. Większość tutoriali Nextflow koncentruje się na orkiestracji workflow'ów (kanały, procesy i przepływ danych), i możesz zajść zaskakująco daleko, mając tylko tę wiedzę.

Jednak gdy musisz manipulować danymi, parsować złożone nazwy plików, implementować logikę warunkową lub budować solidne workflow'y produkcyjne, pomocne jest myślenie o dwóch odrębnych aspektach Twojego kodu: **przepływ danych** (kanały, operatory, procesy i workflow'y) oraz **skryptowanie** (kod wewnątrz domknięć, funkcji i skryptów procesów). Choć to rozróżnienie jest nieco arbitralne — to wszystko jest kod Nextflow — dostarcza użytecznego modelu myślowego do zrozumienia, kiedy orkiestrujesz swój pipeline, a kiedy manipulujesz danymi. Opanowanie obu aspektów znacząco poprawia Twoją zdolność do pisania przejrzystych, łatwych w utrzymaniu workflow'ów.
...
- **Obsługa zdarzeń oparta na konfiguracji:** Używanie obsługi zdarzeń workflow'a do logowania, powiadomień i zarządzania cyklem życia
...
Blok `workflow` definiuje strukturę naszego pipeline'u, podczas gdy `channel.fromPath()` tworzy kanał ze ścieżki pliku. Operator `.splitCsv()` przetwarza plik CSV i konwertuje każdy wiersz na strukturę danych typu mapa.
...
#### 1.1.3. Tworzenie Struktury Danych Typu Mapa
...
Używamy metod manipulacji napisami takich jak `.toLowerCase()` i `.replaceAll()` do czyszczenia naszych danych oraz metod konwersji typów takich jak `.toInteger()` i `.toDouble()` do konwersji danych tekstowych z CSV na odpowiednie typy numeryczne.
...
Dodajmy linię, która tworzy uproszczoną wersję naszych metadanych zawierającą tylko pola identyfikacyjne:
...
!!! note
    ...
    **Mapy i Metadane**: Mapy są fundamentalne dla pracy z metadanymi w Nextflow. Aby uzyskać bardziej szczegółowe wyjaśnienie pracy z mapami metadanych, zobacz side quest [Praca z metadanymi](./metadata.md).

Nasz workflow demonstruje podstawowy wzorzec: **operacje przepływu danych** (`workflow`, `channel.fromPath()`, `.splitCsv()`, `.map()`, `.view()`) orkiestrują, jak dane przemieszczają się przez pipeline, podczas gdy **skryptowanie** (mapy `[key: value]`, metody napisów, konwersje typów, operatory trójargumentowe) wewnątrz domknięcia `.map()` obsługuje przekształcanie poszczególnych elementów danych.
...
Tym razem NIE zmieniliśmy struktury danych, nadal mamy 3 elementy na liście, ale przekształciliśmy każdy element używając metody `collect` Listy, aby wyprodukować nową listę ze zmodyfikowanymi wartościami. Jest to podobne do użycia operatora `map` na kanale, ale działa na strukturze danych Lista zamiast na kanale.
...
| `\.fastq(?:\.gz)?$` | Rozszerzenie pliku `.fastq` lub `.fastq.gz` | Nieprzechwycone (?: to grupa nieprzechwytująca) |
...
Bloki skryptów procesów to zasadniczo wieloliniowe napisy, które są przekazywane do powłoki. Możesz używać **logiki warunkowej** (if/else, operatory trójargumentowe) do dynamicznego generowania różnych napisów skryptowych na podstawie charakterystyk danych wejściowych. Jest to niezbędne do obsługi różnorodnych typów danych wejściowych — takich jak odczyty single-end vs paired-end — bez duplikowania definicji procesów.
...
Ale co, jeśli chcemy dodać informacje o tym, kiedy i gdzie nastąpiło przetwarzanie? Zmodyfikujmy proces, aby używał zmiennych **powłoki** i trochę podstawienia poleceń, aby uwzględnić bieżącego użytkownika, nazwę hosta i datę w raporcie:
...
Na przykład, udajmy, że nasze próbki sekwencjonowania muszą być przycinane za pomocą FASTP tylko wtedy, gdy są to próbki ludzkie z pokryciem powyżej pewnego progu. Próbki mysie lub próbki o niskim pokryciu powinny być uruchamiane z Trimgalore zamiast tego (to wymyślony przykład, ale ilustruje punkt).

Dostarczyliśmy prosty proces Trimgalore w `modules/trimgalore.nf`, możesz spojrzeć, jeśli chcesz, ale szczegóły nie są ważne dla tego ćwiczenia. Kluczowym punktem jest to, że chcemy kierować próbki na podstawie ich metadanych.
...
Doskonale! Teraz wszystkie próbki mają pole `run` z ich faktycznym ID przebiegu (wielkimi literami) lub wartością domyślną 'UNSPECIFIED'. Kombinacja `?.` i `?:` zapewnia zarówno bezpieczeństwo (brak awarii), jak i sensowne wartości domyślne.
...
Czasami musisz natychmiast zatrzymać workflow, jeśli parametry wejściowe są nieprawidłowe. W Nextflow możesz używać wbudowanych funkcji takich jak `error()` i `log.warn`, a także standardowych konstrukcji programistycznych, takich jak instrukcje `if` i logika logiczna, do implementacji logiki walidacji. Dodajmy walidację do naszego workflow'a.
...
Możesz również dodać walidację wewnątrz funkcji `separateMetadata`. Użyjmy niekrytycznego `log.warn` do wydawania ostrzeżeń dla próbek o niskiej głębokości sekwencjonowania, ale nadal pozwalając workflow'owi kontynuować:
...
## 8. Obsługa Zdarzeń Workflow'a

Do tej pory pisaliśmy kod w naszych skryptach workflow'a i definicjach procesów. Ale jest jeszcze jedna ważna funkcja, o której powinieneś wiedzieć: obsługa zdarzeń workflow'a.

Obsługa zdarzeń to domknięcia, które uruchamiają się w określonych punktach cyklu życia Twojego workflow'a. Są idealne do dodawania logowania, powiadomień lub operacji czyszczenia. Te obsługi powinny być zdefiniowane w Twoim skrypcie workflow'a obok definicji workflow'a.

### 8.1. Obsługa `onComplete`

Najczęściej używaną obsługą zdarzeń jest `onComplete`, która uruchamia się, gdy Twój workflow się kończy (niezależnie od tego, czy zakończył się sukcesem, czy niepowodzeniem). Dodajmy jedną, aby podsumować wyniki naszego pipeline'u.

Dodaj obsługę zdarzeń do pliku `main.nf`, wewnątrz definicji workflow'a:
...
Oprócz `onComplete`, jest jeszcze jedna obsługa zdarzeń, której możesz użyć: `onError`, która uruchamia się tylko wtedy, gdy workflow zawiedzie:
...
Możesz używać wielu obsług razem w swoim skrypcie workflow'a:
...
- **Domknięcia obsługi zdarzeń**: Domknięcia w Twoim skrypcie workflow'a, które uruchamiają się w różnych punktach cyklu życia
- **Obsługa `onComplete`**: Do podsumowań wykonania i raportowania wyników
- **Obsługa `onError`**: Do obsługi błędów i logowania niepowodzeń
- **Właściwości obiektu workflow**: Dostęp do `workflow.success`, `workflow.duration`, `workflow.errorMessage`, itp.

Obsługa zdarzeń pokazuje, jak możesz używać pełnej mocy języka Nextflow w swoich skryptach workflow'a, aby dodać zaawansowane możliwości logowania i powiadamiania.
...
- **Adaptacyjne zasoby**: Dynamiczne dyrektywy optymalizują użycie zasobów na podstawie charakterystyk danych wejściowych
...
2.  **Zaawansowane Przetwarzanie Napisów**: Opanowałeś wyrażenia regularne do parsowania nazw plików, dynamiczne generowanie skryptów w procesach oraz interpolację zmiennych (Nextflow vs Bash vs Powłoka).
    ...
    - Kolekcja plików do argumentów polecenia (w bloku skryptu proce```groovy
...
3.  **Tworzenie Funkcji Wielokrotnego Użytku**: Nauczyłeś się wyodrębniać złożoną logikę do nazwanych funkcji, które można wywoływać z operatorów kanałów, czyniąc workflow'y bardziej czytelnymi i łatwiejszymi w utrzymaniu.
...
4.  **Dynamiczne Dyrektywy Zasobów z Domknięciami**: Zbadałeś używanie domknięć w dyrektywach procesów do adaptacyjnej alokacji zasobów na podstawie charakterystyk danych wejściowych.
...
5.  **Logika Warunkowa i Kontrola Procesów**: Dodałeś inteligentne kierowanie używając operatorów `.branch()` i `.filter()`, wykorzystując prawdziwość do zwięzłych wyrażeń warunkowych.
    ...
    - Ewaluacja logiczna z Groovy Truth
...
7.  **Walidacja za pomocą error() i log.warn**: Nauczyłeś się walidować dane wejściowe wcześnie i szybko zawodzić z jasnymi komunikatami błędów.
...
8.  **Obsługa Zdarzeń Konfiguracji**: Nauczyłeś się używać obsługi zdarzeń workflow'a (`onComplete` i `onError`) do logowania, powiadamiania i zarządzania cyklem życia.
...
Upewnij się, że sprawdzisz te zasoby, gdy będziesz potrzebować zbadać bardziej zaawansowane funkcje.
...
- Implementować zarządzanie cyklem życia workflow'a za pomocą obsługi zdarzeń
