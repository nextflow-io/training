...
Każdy plik ma własne dodatkowe informacje: czym jest, skąd pochodzi i co go wyróżnia.
...
W tym side queście zbadasz sposoby obsługi metadanych w workflow.
Zaczynając od prostej tabeli danych (często nazywanej samplesheet w bioinformatyce) zawierającej podstawowe informacje o plikach, nauczysz się:
...
Te umiejętności pomogą Ci tworzyć bardziej solidne i elastyczne pipeline'y potrafiące obsługiwać złożone relacje między plikami i wymagania przetwarzania.
...
Workflow w pliku `main.nf` to szablon, który stopniowo rozwiniesz w pełni funkcjonujący workflow.
...
Możesz zobaczyć, że skonfigurowaliśmy podstawową fabrykę kanałów wczytującą przykładową tabelę danych jako plik, ale to jeszcze nie odczyta zawartości pliku.
...
Zauważ, że używamy opcji `header: true`, aby powiedzieć Nextflow'owi, aby odczytał pierwszy wiersz pliku CSV jako wiersz nagłówka.
...
Możemy użyć operatora `map` Nextflow, aby iterować po każdym elemencie w naszym kanale i specyficznie wybrać wpis `character` z obiektu mapy.
...
Teraz, gdy pomyślnie odczytaliśmy tabelę danych i mamy dostęp do danych w każdym wierszu, możemy zacząć implementować logikę naszego pipeline'u.
...
Praktyczną konsekwencją jest to, że każdy proces konsumujący ten kanał musiałby być skonfigurowany z tą strukturą na myśli:
...
Utrudnia to również dzielenie się procesem z innymi, którzy mogą mieć nieco inne dane wejściowe, i możesz skończyć na hardcodowaniu zmiennych do procesu, które nie są potrzebne przez blok skryptu.
...
To umożliwia nam pisanie procesów konsumujących kanał bez konieczności hardcodowania elementów metadanych w specyfikacji wejściowej:
...
??? info "Tworzenie nowej mapy meta przy użyciu operatora `+`"
    ...
    **Teraz przetransponujmy to w kontekst operacji `channel.map()` Nextflow.**
...
Jedynym powodem, dla którego nie mogliśmy tego zrobić powyżej, jest to, że proces nie jest skonfigurowany tak, aby przyjmować mapę meta.
Ale ponieważ kontrolujemy kod procesu, możemy to zmienić.

Zmodyfikujmy proces `COWPY`, aby przyjmował strukturę krotki `[meta, file]`, której użyliśmy w pierwszym procesie, abyśmy mogli usprawnić workflow.
...
Zauważ, że nie zaktualizowaliśmy definicji wyjściowej procesu, aby wyprowadzać mapę meta, aby utrzymać kurs uproszczonym, ale możesz zrobić to sam jako ćwiczenie, podążając za modelem procesu `IDENTIFY_LANGUAGE`.
...
Zastąpiliśmy odwołanie do wartości `character` wcześniej przekazywanej jako samodzielne wejście wartością przechowywaną w mapie meta, do której odwołujemy się za pomocą `meta.character`.

Teraz zaktualizujmy wywołanie procesu odpowiednio.

#### 3.3.3. Zaktualizuj wywołanie procesu i uruchom go

Proces oczekuje teraz, że jego wejście będzie używać struktury krotki `[meta, file]`, która jest tym, co wyprowadza poprzedni proces, więc możemy po prostu przekazać cały kanał `ch_languages` do procesu `COWPY`.
...
Jeśli spojrzysz w katalog results, powinieneś zobaczyć takie same wyjścia jak wcześniej, _tj._ poszczególne pliki zawierające sztukę ASCII każdego powitania wymówionego przez odpowiednią postać.
...
Więc to produkuje takie same wyniki jak wcześniej z prostszym kodem.

Oczywiście zakłada to, że jesteś w stanie modyfikować kod procesu.
W niektórych przypadkach możesz musieć polegać na istniejących procesach, których nie masz swobody modyfikować, co ogranicza Twoje opcje.
Dobra wiadomość, jeśli planujesz używać modułów z projektu [nf-core](https://nf-co.re/), jest taka, że moduły nf-core są wszystkie skonfigurowane tak, aby używać struktury krotki `[meta, file]` jako standardu.
...
To zależy od tego, co zawiera tabela danych wejściowych i której wersji workflow używamy.

#### 3.4.1. Kolumna character istnieje, ale jest pusta

Powiedzmy, że usuniemy wartość character dla jednego z wpisów w naszej tabeli danych, aby zasymulować błąd zbierania danych:
...
Dla obu wersji workflow, których użyliśmy powyżej, klucz `character` zostanie utworzony dla wszystkich wpisów podczas odczytu tabeli danych, ale dla `sampleA` wartością będzie pusty ciąg.
...
Gdy Nextflow uruchamia polecenie `cowpy` dla tej próbki, `${meta.character}` jest wypełnione pustym ciągiem w wierszu poleceń `cowpy`, więc narzędzie `cowpy` zgłasza błąd mówiący, że nie podano wartości dla argumentu `-c`.

#### 3.4.2. Kolumna character nie istnieje w tabeli danych

Teraz powiedzmy, że całkowicie usuniemy kolumnę `character` z naszej tabeli danych:
...
W tym przypadku klucz `character` nie zostanie w ogóle utworzony podczas odczytu tabeli danych.
...
To spowoduje błąd, ale inny niż w pierwszym przypadku.
...
Dzieje się tak, ponieważ `meta.character` nie istnieje, więc nasza próba uzyskania do niego dostępu zwraca `null`. W rezultacie Nextflow dosłownie wstawia `null` do wiersza poleceń, co oczywiście nie jest rozpoznawane przez narzędzie `cowpy`.

#### 3.4.3. Rozwiązania

Oprócz dostarczenia wartości domyślnej jako części konfiguracji workflow, możemy zrobić dwie rzeczy, aby obsłużyć to w bardziej solidny sposób:

1. Zaimplementować walidację wejść do swojego workflow, aby upewnić się, że tabela danych zawiera wszystkie wymagane informacje. Możesz znaleźć [wprowadzenie do walidacji wejść](../hello_nf-core/05_input_validation.md) w kursie Hello nf-core.

2. Jeśli chcesz się upewnić, że każdy, kto używa Twojego modułu procesu, może natychmiast zidentyfikować wymagane wejścia, możesz również uczynić wymaganą właściwość metadanych jawnym wejściem.

Oto przykład, jak by to działało.

Po pierwsze, na poziomie procesu zaktualizuj definicję wejściową w następujący sposób:
...
To podejście ma tę zaletę, że jawnie pokazuje, że `character` jest wymagane i ułatwia ponowne wdrażanie procesu w innych kontekstach.
...
Mapa meta jest doskonała do utrzymywania struktur kanałów czystych i zapobiegania arbitralnym strukturom kanałów, ale dla obowiązkowych elementów, do których bezpośrednio się odwołuje proces, wyodrębnianie ich jako jawnych wejść tworzy bardziej solidny i łatwy w utrzymaniu kod.

### Podsumowanie

W tej sekcji nauczyłeś się wykorzystywać metadane do dostosowywania wykonania procesu, uzyskując do nich dostęp albo na poziomie workflow, albo na poziomie procesu.

---

## Ćwiczenie uzupełniające

Jeśli chciałbyś poćwiczyć używanie informacji z mapy meta wewnątrz procesu, spróbuj użyć innych informacji z mapy meta, takich jak `lang` i `lang_group`, aby dostosować sposób nazywania i/lub organizacji wyjść.
...
W tym side queście zbadałeś, jak skutecznie pracować z metadanymi w workflow Nextflow.

Ten wzorzec utrzymywania metadanych jawnych i dołączonych do danych jest podstawową najlepszą praktyką w Nextflow, oferującą kilka zalet w porównaniu z hardcodowaniem informacji o plikach:
...
Stosowanie tego wzorca we własnej pracy umożliwi Ci budowanie solidnych, łatwych w utrzymaniu workflow bioinformatycznych.
...
2.  **Rozszerzanie metadanych podczas workflow** Dodawanie nowych informacji do Twoich metadanych w miarę postępu pipeline'u poprzez dodawanie wyjść procesów i wyprowadzanie wartości przez logikę warunkową.
    ...
    - Dodawanie nowych kluczy przy użyciu klauzuli warunkowej
