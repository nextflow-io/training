...
Dla obrazu kontenera `quote` możesz użyć tego, który zbudowałeś samodzielnie w poprzednim dodatkowym ćwiczeniu, lub tego, który pobrałeś z Seqera Containers.
...
### 1.2. Zmodyfikuj swój pipeline Nextflow'a, aby mógł działać w trybach `quote` i `sayHello`.
...
Oto przykład użycia instrukcji `if` w workflow'ie Nextflow'a:
...
Wiesz już, jak używać kontenerów w Nextflow'ie do uruchamiania procesów oraz jak budować logikę rozgałęzień w swoich pipeline'ach!

### Co dalej?

Świętuj, zrób sobie przerwę na rozciąganie i napij się wody!

Kiedy będziesz gotowy, przejdź do Części 3 tej serii szkoleniowej, aby nauczyć się, jak zastosować to, czego się do tej pory nauczyłeś, do bardziej realistycznego przypadku analizy danych.
