...
Nextflow dostarcza potężne narzędzia do efektywnej obsługi plików, pomagając Ci organizować i przetwarzać dane przy minimalnej ilości kodu.
...
Pod koniec tego zadania pobocznego będziesz w stanie:

- Tworzyć obiekty Path z ciągów znaków ścieżek plików przy użyciu metody `file()` Nextflow'a
...
- Integrować operacje na plikach z procesami Nextflow'a przy odpowiedniej obsłudze wejść
- Organizować wyjścia procesów przy użyciu struktur katalogów opartych na metadanych

Te umiejętności pomogą Ci budować workflow'y, które mogą obsługiwać różne rodzaje wejść plików z dużą elastycznością.
...
- Czuć się komfortowo z podstawowymi koncepcjami i mechanizmami Nextflow'a (procesy, kanały, operatory)
...
Ten katalog zawiera dane sekwencjonowania sparowanego od trzech pacjentów (A, B, C).
...
1. **Załaduje** pliki wejściowe przy użyciu metod obsługi plików Nextflow'a
...
??? info "Co to jest `.class`?"
    ...
    W Nextflow'ie `.class` mówi nam, z jakim typem obiektu mamy do czynienia. To jak pytanie "co to za rzecz?", aby dowiedzieć się, czy jest to ciąg znaków, liczba, plik czy coś innego.
...
To tylko wyjście tekstowe; Nextflow jeszcze nic specjalnego z tym nie zrobił.
Potwierdziliśmy również, że jeśli chodzi o Nextflow'a, jest to tylko ciąg znaków (klasy `java.lang.String`).
Ma to sens, ponieważ jeszcze nie powiedzieliśmy Nextflow'owi, że odpowiada to plikowi.
...
??? info "Co robi `debug true`?"
    ...
    Aby uzyskać więcej informacji na temat debugowania procesów Nextflow'a, zobacz zadanie poboczne [Debugowanie workflow'ów Nextflow'a](debugging.md).
...
To często sprawia problemy nowicjuszom w Nextflow'ie, więc poświęćmy kilka minut na przyjrzenie się, co się dzieje, gdy zrobisz to źle.
...
To pokazuje wiele szczegółów na temat błędu, ponieważ proces jest ustawiony na wypisywanie informacji debugowania, jak wspomniano powyżej.
...
!!! note
    ...
    Upewnij się, że cofniesz i naprawisz oba celowe błędy przed przejściem do następnej sekcji.
...
Jeśli robisz to dobrze, nigdy nie powinieneś musieć zmieniać logiki swojego workflow'a, aby dostosować pliki pochodzące z różnych lokalizacji.
...
Kluczową zaletą tego systemu jest to, że umożliwia nam przełączanie się między środowiskami bez zmiany jakiejkolwiek logiki pipeline'u.
...
!!! warning
    ...
    To zadziała tylko wtedy, gdy masz aktywne połączenie internetowe.
...
Nie musisz pamiętać tych klas; wspominamy o tym tylko po to, aby zademonstrować, że Nextflow identyfikuje i obsługuje różne lokalizacje odpowiednio.
...
??? abstract "Zawartość katalogu roboczego"
    ...
    Jeśli hash procesu wynosił `8a/2ab7ca`, możesz zbadać katalog roboczy:
...
!!! note
    ...
    Jedynym ważnym wyjątkiem od tej zasady jest to, że nie możesz używać wzorców glob ani ścieżek katalogów z HTTPS, ponieważ HTTPS nie może wyświetlać wielu plików, więc musisz określić dokładne adresy URL plików.
...
- Dane zdalne są dostępne przy użyciu URI (HTTP, FTP, S3, Azure, Google Cloud)
...
Do tej pory pracowaliśmy z pojedynczym plikiem na raz, ale w Nextflow'ie zazwyczaj będziemy chcieli utworzyć kanał wejściowy z wieloma plikami wejściowymi do przetworzenia.
...
W tym miejscu pojawia się [`channel.fromPath()`](https://www.nextflow.io/docs/latest/reference/channel.html#frompath): wygodna fabryka kanałów, która łączy wszystkie funkcje, których potrzebujemy, aby wygenerować kanał z jednego lub więcej statycznych ciągów znaków plików, a także wzorców glob.
...
!!! note
    ...
    Jak wspomniano wcześniej, Nextflow obsługuje globowanie do zarządzania plikami wejściowymi i wyjściowymi w większości przypadków, z wyjątkiem ścieżek plików HTTPS, ponieważ HTTPS nie może wyświetlać wielu plików.
...
Jak widzisz, mamy teraz dwa obiekty Path w naszym kanale, co pokazuje, że Nextflow wykonał rozszerzenie nazw plików poprawnie i załadował oraz przetworzył oba pliki zgodnie z oczekiwaniami.
...
To duże "jeśli", oczywiście, i powinieneś być bardzo ostrożny, gdy polegasz na strukturze nazw plików; ale rzeczywistość jest taka, że to podejście jest bardzo szeroko stosowane, więc przyjrzyjmy się, jak to się robi w Nextflow'ie.
...
Na szczęście te komponenty są po prostu oddzielone podkreśleniami w oryginalnej nazwie pliku, więc możemy zastosować powszechną metodę Nextflow'a zwaną `tokenize()`, która jest idealna do tego zadania.
...
To świetnie!
Rozbiliśmy informacje o pacjencie z pojedynczego ciągu znaków na listę ciągów znaków.
...
Nextflow dostarcza wyspecjalizowaną fabrykę kanałów do pracy ze sparowanymi plikami, zwaną `channel.fromFilePairs()`, która automatycznie grupuje pliki na podstawie wspólnego wzorca nazewnictwa. Pozwala to na ściślejsze powiązanie sparowanych plików przy mniejszym wysiłku.
...
Jest to podobne do wzorca glob, którego użyliśmy wcześniej, z wyjątkiem tego, że konkretnie wylicza podciągi (albo `1`, albo `2` pojawiające się zaraz po R), które identyfikują dwóch członków pary.
...
O nie, tym razem uruchomienie nie powiodło się!
...
Wszystkie manipulacje metadanymi, które robiliśmy, nie wpłynęły faktycznie na zawartość kanału.
...
Hurra, tym razem workflow'a się powiódł!
...
Teraz połączmy to wszystko w prostym procesie, aby wzmocnić, jak używać operacji na plikach wewnątrz procesu Nextflow'a.
...
Moglibyśmy sobie wyobrazić, że robi to dopasowanie sekwencji lub wywołanie wariantów lub jakikolwiek inny krok, który ma sens dla tego typu danych.
...
W głównym workflow'ie zastąp operator `.view()` przez `.set { ch_samples }` i dodaj linię testującą, że możemy odwoływać się do kanału po nazwie.
...
Oczywiście to przetwarza tylko pojedynczą parę plików dla pojedynczego pacjenta, co nie jest dokładnie tym rodzajem wysokiej przepustowości, na którą liczysz z Nextflow'em.
...
Tutaj pokazujemy opcję użycia dodatkowych poziomów katalogów, aby uwzględnić typy próbek i repliki, ale możesz poeksperymentować z robieniem tego na poziomie nazwy pliku.
...
I oto są, wszystkie nasze metadane, starannie zorganizowane. To sukces!
...
Ten wzorzec utrzymywania metadanych w sposób jawny i dołączony do danych (zamiast kodowania w nazwach plików) jest podstawową najlepszą praktyką w Nextflow'ie, która umożliwia budowanie solidnych, łatwych w utrzymaniu workflow'ów analizy.
Możesz dowiedzieć się więcej na ten temat w zadaniu pobocznym [Metadane i mapy meta](./metadata.md).
...
W tym zadaniu pobocznym nauczyłeś się, jak pracować z plikami w Nextflow'ie, od podstawowych operacji po bardziej zaawansowane techniki obsługi kolekcji plików.
...
6.  **Używanie operacji na plikach w procesach:** Zintegrowaliśmy operacje na plikach z procesami Nextflow'a przy odpowiedniej obsłudze wejść, używając `publishDir` do organizowania wyjść na podstawie metadanych.
...
- [Dokumentacja Nextflow'a: Praca z plikami](https://www.nextflow.io/docs/latest/working-with-files.html)
