...
Nextflow zapewnia potężne narzędzia do efektywnej obsługi plików, pomagając Ci organizować i przetwarzać dane przy minimalnym nakładzie kodu.
...
Kluczowe zmiany tutaj to:

- **Przypisanie destrukturyzujące**: `def (patient, replicate, type, readNum) = ...` wyodrębnia wartości z tokenizacji do nazwanych zmiennych w jednej linii
- **Składnia literału mapy**: `[id: patient, replicate: ...]` tworzy mapę, gdzie każdy klucz (jak `id`) jest powiązany z wartością (jak `patient`)
- **Zagnieżdżona struktura**: Zewnętrzna lista `[..., myFile]` łączy mapę metadanych z oryginalnym obiektem pliku

Uprościliśmy również kilka ciągów metadanych używając metody zastępowania ciągów zwanej `replace()` do usunięcia niektórych niepotrzebnych znaków (_np._ `replicate.replace('rep', '')` aby zachować tylko liczbę z ID replikatów).
...
Teraz metadane są czytelnie oznaczone (_np._ `[id:patientA, replicate:1, type:normal, readNum:2]`) więc jest o wiele łatwiej powiedzieć, co jest czym.

Będzie też o wiele łatwiej faktycznie wykorzystać elementy metadanych w workflow i sprawi, że nasz kod będzie łatwiejszy do odczytania i bardziej łatwy w utrzymaniu.
...
- Operacja `.map()` transformuje elementy kanału zachowując strukturę
...
Wiele projektów eksperymentalnych produkuje sparowane pliki danych, które korzystają z bycia obsługiwanymi w wyraźnie sparowany sposób.
...
Nextflow zapewnia wyspecjalizowaną fabrykę kanałów do pracy z takimi sparowanymi plikami o nazwie `channel.fromFilePairs()`, która automatycznie grupuje pliki na podstawie wspólnego wzorca nazewnictwa. To pozwala Ci powiązać sparowane pliki ściślej przy mniejszym wysiłku.

Zamierzamy zmodyfikować nasz workflow, aby z tego skorzystać.
Zajmie to dwa kroki:
...
Aby użyć `channel.fromFilePairs`, musimy określić wzorzec, którego Nextflow powinien używać do identyfikacji dwóch członków w parze.
...
To jest podobne do wzorca glob, którego użyliśmy wcześniej, z wyjątkiem tego, że szczegółowo wymienia podciągi (albo `1` albo `2` przychodzące zaraz po R), które identyfikują dwóch członków pary.
...
Przełączyliśmy fabrykę kanałów i dostosowaliśmy wzorzec dopasowywania plików, i przy okazji, zakomentowaliśmy operację map.
...
Wszystkie manipulacje metadanych, które robiliśmy, faktycznie nie wpłynęły na zawartość kanału.
...
To świetnie, ponieważ Nextflow wykonał ciężką pracę wyodrębniania nazwy pacjenta przez zbadanie wspólnego prefiksu i użycie go jako identyfikatora pacjenta.

Jednak to psuje nasz obecny workflow.
Jeśli chcielibyśmy nadal uruchamiać `COUNT_LINES` w ten sam sposób bez zmiany procesu, musielibyśmy zastosować operację mapowania, aby wyodrębnić ścieżki plików.
Ale nie zamierzamy tego robić, ponieważ naszym ostatecznym celem jest użycie innego procesu, `ANALYZE_READS`, który obsługuje pary plików odpowiednio.
...
Możesz również zakomentować lub usunąć instrukcję include `COUNT_LINES`, ale nie będzie to miało żadnego funkcjonalnego efektu.
...
Hurra, tym razem workflow się udało!

Jednak nadal musimy wyodrębnić resztę metadanych z pola `id`.
...
Mamy już dostęp do rzeczywistego identyfikatora pacjenta w ciągu, którego `fromFilePairs()` użył jako identyfikatora, więc możemy użyć tego do wyodrębnienia metadanych bez pobierania `simpleName` z obiektu Path, jak robiliśmy wcześniej.
...
Tym razem map zaczyna od `id, files` zamiast tylko `myFile`, a `tokenize()` jest zastosowane do `id` zamiast do `myFile.simpleName`.

Zauważ również, że porzuciliśmy `readNum` z linii `tokenize()`; wszelkie podciągi, których nie nazwiemy specjalnie (zaczynając od lewej) zostaną cicho porzucone.
...
I oto jest: mamy mapę metadanych (`[id:patientA, replicate:1, type:normal]`) w pierwszej pozycji krotki wyjściowej, po której następuje krotka sparowanych plików, jak zamierzono.

Oczywiście, to wybierze i przetworzy tylko tę konkretną parę plików.
...
- To upraszcza obsługę odczytów paired-end w Twoim pipeline
- Sparowane pliki mogą być grupowane jako krotki `[id, [file1, file2]]`
- Wyodrębnianie metadanych może być wykonane z ID sparowanego pliku zamiast z pojedynczych plików

---

## 6. Używanie operacji na plikach w procesach

Teraz połączmy to wszystko razem w prostym procesie, aby wzmocnić, jak używać operacji na plikach wewnątrz procesu Nextflow.

Udostępniamy Ci wstępnie napisany moduł procesu o nazwie `ANALYZE_READS`, który przyjmuje krotkę metadanych i parę plików wejściowych i je analizuje.
Moglibyśmy sobie wyobrazić, że to robi dopasowanie sekwencji, wykrywanie wariantów lub jakikolwiek inny krok, który ma sens dla tego typu danych.
...
!!! note
    ...
    Składnia closure odkłada ewaluację do momentu, gdy proces faktycznie się uruchomi.

!!! note

    Nazywamy naszą mapę metadanych `meta` przez konwencję.
    Aby głębiej zagłębić się w mapy meta, zobacz side quest [Metadata and meta maps](./metadata.md).
...
1. Nadać nazwę przekształconemu kanałowi
2. Dodać wywołanie procesu

#### 6.2.1. Nazwij przekształcony kanał wejściowy

Wcześniej zastosowaliśmy manipulacje mapowaniem bezpośrednio do kanału wejściowego.
...
Ten proces jest skonfigurowany do publikowania swoich wyjść w katalogu `results`, więc spójrz tam.
...
Oczywiście, to jest tylko przetwarzanie pojedynczej pary plików dla pojedynczego pacjenta, co nie jest dokładnie rodzajem wysokiej przepustowości, której się spodziewasz po Nextflow.
...
Dlatego jeśli chcemy uwzględnić wszystkich pacjentów, możemy po prostu zmodyfikować ciąg wejściowy, aby uwzględnić więcej pacjentów, jak wspomniano wcześniej.

Udajmy, że chcemy być jak najbardziej zachłanni.
Wprowadź następujące edycje w workflow:
...
Może nie.
Jeśli przyjrzysz się bliżej, mamy problem: mamy dwa replikaty dla patientA, ale tylko jeden plik wyjściowy!
Nadpisujemy plik wyjściowy za każdym razem.

### 6.4. Uczyń opublikowane pliki unikalnymi

Ponieważ mamy dostęp do metadanych pacjenta, możemy ich użyć, aby uczynić opublikowane pliki unikalnymi poprzez włączenie różnicujących metadanych, albo w strukturze katalogów, albo w samych nazwach plików.
...
Tutaj pokazujemy opcję użycia dodatkowych poziomów katalogów, aby uwzględnić typy próbek i replikaty, ale możesz poeksperymentować z robieniem tego na poziomie nazwy pliku również.

Teraz uruchom pipeline jeszcze raz, ale upewnij się, że najpierw usuniesz katalog wyników, aby dać sobie czystą przestrzeń roboczą:
...
Jest dużo więcej, co możesz zrobić, gdy masz swoje metadane załadowane do mapy w ten sposób:

1. Utworzyć zorganizowane katalogi wyjściowe oparte na atrybutach pacjenta
...
Ten wzorzec utrzymywania metadanych w sposób wyraźny i przyłączonych do danych (zamiast kodowanych w nazwach plików) jest podstawową najlepszą praktyką w Nextflow, która umożliwia budowanie solidnych, łatwych w utrzymaniu workflow analizy.
...
- To podejście tworzy łatwe w utrzymaniu workflow z wyraźnym pochodzeniem danych
...
- Struktura workflow oddziela tworzenie kanałów od wykonania procesu
...
Zastosowanie tych technik w Twojej własnej pracy umożliwi Ci budowanie bardziej wydajnych i łatwych w utrzymaniu workflow, szczególnie podczas pracy z dużą liczbą plików ze złożonymi konwencjami nazewnictwa.
...
2. **Używanie plików zdalnych**: Nauczyliśmy się, jak przejrzyście przełączać między plikami lokalnymi i zdalnymi używając URI, demonstrując zdolność Nextflow do obsługi plików z różnych źródeł bez zmiany logiki workflow.
...
3. **Ładowanie plików za pomocą fabryki kanałów `fromPath()`:** Utworzyliśmy kanały ze wzorców plików za pomocą `channel.fromPath()` i przeglądaliśmy ich atrybuty plików, włączając typy obiektów.
...
4. **Wyodrębnianie metadanych pacjenta z nazw plików:** Użyliśmy `tokenize()` i `replace()` do wyodrębnienia i ustrukturyzowania metadanych z nazw plików, konwertując je do zorganizowanych map.
...
6. **Używanie operacji na plikach w procesach:** Zintegrowaliśmy operacje na plikach w procesach Nextflow z prawidłową obsługą wejścia, używając `publishDir` do organizowania wyjść na podstawie metadanych.

   - Skojarzyliśmy mapę meta z wejściami procesu
   ...
   - Zorganizowaliśmy wyjścia na podstawie metadanych
