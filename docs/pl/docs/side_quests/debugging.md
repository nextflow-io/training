...
Kolejna klasa błędów występuje z powodu błędów w dostarczaniu oprogramowania. `missing_software.nf` to składniowo poprawny workflow, ale zależy od pewnego zewnętrznego oprogramowania, aby dostarczyć polecenia `cowpy`, którego używa.
...
Proces nie ma dostępu do polecenia, które określamy. Czasami dzieje się tak, ponieważ skrypt jest obecny w katalogu `bin` workflow'a, ale nie został oznaczony jako wykonywalny. Innym razem dzieje się tak, ponieważ oprogramowanie nie jest zainstalowane w kontenerze lub środowisku, w którym uruchomiony jest workflow.

#### Sprawdź kod

Zwróć uwagę na kod wyjścia `127` - mówi Ci dokładnie, jaki jest problem. Przeanalizujmy `missing_software.nf`:
...
W użyciu produkcyjnym będziesz konfigurować zasoby w swoich procesach. Na przykład `memory` definiuje maksymalną ilość pamięci dostępną dla Twojego procesu, a jeśli proces ją przekroczy, Twój scheduler zwykle zabije proces i zwróci kod wyjścia `137`. Nie możemy tego zademonstrować tutaj, ponieważ używamy executora `local`, ale możemy pokazać coś podobnego z `time`.
...
process PROCESS_FILES {
    ...
    time '1 ms'  // BŁĄD: Nierealistyczne ograniczenie czasu
    ...
    script:
    """
    sleep 1  // Trwa 1 sekundę, ale ograniczenie czasu to 1ms
    cowpy ${sample_name} > ${sample_name}_output.txt
    """
}
```

Wiemy, że proces zajmie więcej niż sekundę (dodaliśmy tam uśpienie, aby się upewnić), ale proces jest ustawiony, aby upłynął czas po 1 milisekundzie. Ktoś był trochę nierealistyczny ze swoją konfiguracją!

#### Napraw kod

Zwiększ ograniczenie czasu do realistycznej wartości:
...
=== "Po"
    ...
    process PROCESS_FILES {
        ...
        time '100 s'  // Naprawiono: Realistyczne ograniczenie czasu
...
Jeśli upewnisz się, że czytasz swoje komunikaty błędów, takie awarie jak ta nie powinny Cię za długo zagmatwać. Ale upewnij się, że rozumiesz wymagania dotyczące zasobów poleceń, które uruchamiasz, abyś mógł odpowiednio skonfigurować swoje dyrektywy zasobów.

### 3.4. Techniki debugowania procesów

Gdy procesy zawiodą lub zachowują się nieoczekiwanie, potrzebujesz systematycznych technik do zbadania, co poszło nie tak. Katalog roboczy zawiera wszystkie informacje potrzebne do debugowania wykonania procesu.
...
Użyjmy przykładu `missing_output.nf` z wcześniejszego okresu, aby zademonstrować inspekcję katalogu roboczego (ponownie wygeneruj niedopasowanie nazewnictwa wyjścia, jeśli potrzebujesz):
...
Możesz następnie zbadać kluczowe pliki:

##### Sprawdź skrypt poleceń
...
Typowe problemy do poszukiwania:

- **Brakujące cudzysłowy**: Zmienne zawierające spacje potrzebują odpowiedniego cytowania
- **Złe ścieżki plików**: Pliki wejściowe, które nie istnieją lub znajdują się w złych lokalizacjach
- **Nieprawidłowe nazwy zmiennych**: Literówki w odwołaniach zmiennych
...
- **Oczekiwane wyjście**: Czy polecenie wygenerowało właściwe wyniki
...
Gdy procesy zawiodą z powodu brakujących plików wyjściowych, sprawdź, jakie pliki zostały faktycznie utworzone:
...
- **Niedopasowania nazewnictwa plików**: Pliki wyjściowe o innych nazwach niż oczekiwano
...
Debugowanie procesów wymaga badania katalogów roboczych, aby zrozumieć, co poszło nie tak. Kluczowe pliki obejmują `.command.sh` (wykonany skrypt), `.command.err` (komunikaty błędów) i `.command.out` (standardowe wyjście). Kody wyjścia takie jak 127 (polecenie nie znalezione) i 137 (proces zabity) dostarczają natychmiastowych wskazówek diagnostycznych o typie awarii.
...
Nextflow zapewnia kilka potężnych wbudowanych narzędzi do debugowania i analizowania wykonania workflow'a. Te narzędzia pomagają Ci zrozumieć, co poszło nie tak, gdzie poszło nie tak i jak to efektywnie naprawić.

### 4.1. Wyjście procesu w czasie rzeczywistym

Czasami musisz zobaczyć, co dzieje się wewnątrz uruchomionych procesów. Możesz włączyć wyjście procesu w czasie rzeczywistym, które pokazuje Ci dokładnie, co robi każde zadanie podczas wykonywania.

#### Uruchom pipeline

`bad_channel_shape_viewed.nf` z naszych wcześniejszych przykładów wypisywał zawartość kanału używając `.view()`, ale możemy również użyć dyrektywy `debug` do wyświetlania zmiennych z wnętrza samego procesu, co demonstrujemy w `bad_channel_shape_viewed_debug.nf`. Uruchom workflow:
...
Czasami chcesz wychwycić problemy, zanim jakiekolwiek procesy zostaną uruchomione. Nextflow zapewnia flagę dla tego rodzaju proaktywnego debugowania: `-preview`.

#### Uruchom pipeline

Tryb podglądu pozwala testować logikę workflow bez wykonywania poleceń. Może to być całkiem użyteczne do szybkiego sprawdzenia struktury Twojego workflow'a i upewnienia się, że procesy są poprawnie połączone bez uruchamiania jakichkolwiek rzeczywistych poleceń.

!!! note

    Jeśli naprawiłeś `bad_syntax.nf` wcześniej, przywróć błąd składni, usuwając nawias zamykający po bloku `script` przed uruchomieniem tego polecenia.
...
Tryb podglądu jest szczególnie użyteczny do wczesnego wychwytywania błędów składni bez uruchamiania jakichkolwiek procesów. Waliduje strukturę workflow'a i połączenia procesów przed wykonaniem.

### 4.3. Uruchamianie zaślepek do testowania logiki

Czasami błędy są trudne do debugowania, ponieważ polecenia trwają zbyt długo, wymagają specjalnego oprogramowania lub zawodzą z złożonych powodów. Uruchamianie zaślepek pozwala testować logikę workflow bez wykonywania rzeczywistych poleceń.

#### Uruchom pipeline

Gdy rozwijasz proces Nextflow, możesz użyć dyrektywy `stub`, aby zdefiniować 'fikcyjne' polecenia, które generują wyjścia o poprawnej formie bez uruchamiania prawdziwego polecenia. To podejście jest szczególnie wartościowe, gdy chcesz zweryfikować, że logika Twojego workflow'a jest poprawna przed zajęciem się złożonościami rzeczywistego oprogramowania.

Na przykład, pamiętasz nasz `missing_software.nf` z wcześniejszych okresów? Ten, gdzie mieliśmy brakujące oprogramowanie, które uniemożliwiało uruchomienie workflow'a, dopóki nie dodaliśmy `-profile docker`? `missing_software_with_stub.nf` to bardzo podobny workflow. Jeśli uruchomimy go w ten sam sposób, wygenerujemy ten sam błąd:
...
W stosunku do `missing_software.nf`, ten proces ma dyrektywę `stub:` określającą polecenie, które ma być używane zamiast tego określonego w `script:`, w przypadku gdy Nextflow jest uruchamiany w trybie zaślepki.

Polecenie `touch`, którego tutaj używamy, nie zależy od żadnego oprogramowania lub odpowiednich wejść i będzie działać we wszystkich sytuacjach, pozwalając nam debugować logikę workflow bez martwienia się o wewnętrzne procesy.
...
- Logikę workflow bez zależności od oprogramowania

### 4.4. Systematyczne podejście do debugowania

Teraz, gdy nauczyłeś się indywidualnych technik debugowania - od plików śledzenia i katalogów roboczych po tryb podglądu, uruchamianie zaślepek i monitorowanie zasobów - powiążmy je razem w systematyczną metodologię. Posiadanie ustrukturyzowanego podejścia zapobiega przytłoczeniu przez złożone błędy i zapewnia, że nie przeoczysz ważnych wskazówek.

Ta metodologia łączy wszystkie narzędzia, które omówiliśmy, w efektywny workflow:

**Metoda czterofazowego debugowania:**
...
1. Przeczytaj uważnie komunikaty błędów w czasie wykonania
2. Sprawdź, czy jest to błąd w czasie wykonania, logiczny czy zasobów
3. Użyj trybu podglądu do testowania podstawowej logiki workflow

**Faza 3: Szczegółowe badanie (15-30 minut)**

1. Znajdź katalog roboczy zawiedzonego zadania
...
3. Zweryfikuj pełne wykonanie workflow

!!! tip "Używanie Resume do efektywnego debugowania"

    Gdy już zidentyfikujesz problem, potrzebujesz efektywnego sposobu na przetestowanie swoich poprawek bez marnowania czasu na ponowne uruchamianie udanych części Twojego workflow'a. Funkcjonalność `-resume` Nextflow jest nieoceniona dla debugowania.

    Napotkałeś `-resume`, jeśli pracowałeś nad [Hello Nextflow](../hello_nextflow/), i jest ważne, abyś z niego dobrze korzystał podczas debugowania, aby zaoszczędzić sobie oczekiwania, podczas gdy procesy przed Twoim problemowym procesem są uruchamiane.

    **Strategia debugowania z wznowieniem:**

    1. Uruchom workflow do awarii
    2. Zbadaj katalog roboczy dla zawiedzonego zadania
    3. Napraw konkretny problem
    4. Wznów, aby przetestować tylko poprawkę
    5. Powtórz, aż workflow zakończy się

#### Profil konfiguracji debugowania

Aby uczynić to systematyczne podejście jeszcze bardziej efektywnym, możesz utworzyć dedykowaną konfigurację debugowania, która automatycznie włącza wszystkie narzędzia, których potrzebujesz:
...
profiles {
    debug {
        process {
            ...
            // Konserwatywne zasoby dla debugowania
            maxForks = 1
            memory = '2.GB'
            cpus = 1
        }
...
Teraz czas wprowadzić systematyczne podejście do debugowania w praktyce. Workflow `buggy_workflow.nf` zawiera kilka typowych błędów, które reprezentują typy problemów, z którymi spotkasz się w rzeczywistym rozwoju.

!!! exercise

    Użyj systematycznego podejścia do debugowania, aby zidentyfikować i naprawić wszystkie błędy w `buggy_workflow.nf`. Ten workflow próbuje przetwarzać dane próbek z pliku CSV, ale zawiera wiele zamierzonych bugów reprezentujących typowe scenariusze debugowania.
    ...
    ??? failure "Wyjście polecenia"
        ...
        Ten zagadkowy błąd wskazuje na problem parsowania wokół linii 11-12 w bloku `params{}`. Parser v2 wychwytuje problemy strukturalne wcześnie.
    ...
    **Faza 2: Szybka ocena**
    - Przeczytaj uważnie komunikaty błędów w czasie wykonania
    ...
    **Faza 3: Szczegółowe badanie**
    - Zbadaj katalogi robocze dla zawiedzionych zadań
    ...
    **Faza 4: Naprawa i walidacja**
    ...
    - Zweryfikuj pełne wykonanie workflow

    **Narzędzia debugowania do Twojej dyspozycji:**
    ```bash
    # Tryb podglądu dla sprawdzania składni
    ...
    # Wznowienie po poprawkach
    nextflow run buggy_workflow.nf -resume
    ```

    ??? solution
        `buggy_workflow.nf` zawiera 9 lub 10 odrębnych błędów (w zależności od tego, jak liczysz), obejmujących wszystkie główne kategorie debugowania. Oto systematyczne zestawienie każdego błędu i jak go naprawić
        ...
        **Błąd 2: Błąd składni - brakujący nawias zamykający**
        ...
        ```
        **Naprawa:** Dodaj brakujący nawias zamykający
        ```groovy linenums="29"
        """
        ...
        **Błąd 3: Błąd nazwy zmiennej**
        ...
        ```
        **Naprawa:** Użyj poprawnej nazwy zmiennej wejściowej
        ```groovy linenums="26"
        echo "Processing: ${sample_id}"
        cat ${input_file} > ${sample_id}_result.txt
        ```
        ...
        **Błąd 4: Błąd niezdefiniowanej zmiennej**
        ...
        ```
        **Naprawa:** Użyj poprawnego kanału i wydobądź identyfikatory próbek
        ```groovy linenums="87"
        heavy_ch = heavyProcess(input_ch)
        ```

        W tym momencie workflow uruchomi się, ale wciąż będziemy otrzymywać błędy (np. `Path value cannot be null` w `processFiles`), spowodowane złą strukturą kanału.

        **Błąd 5: Błąd struktury kanału - złe wyjście mapy**
        ```groovy linenums="83"
        .map { row -> row.sample_id }  // BŁĄD: processFiles oczekuje krotki
        ```
        **Naprawa:** Zwróć strukturę krotki, której oczekuje processFiles
        ```groovy linenums="83"
        .map { row -> [row.sample_id, file(row.fastq_path)] }
        ```

        Ale to zepsuje naszą linię do uruchamiania `heavyProcess()` powyżej, więc będziemy musieli użyć mapy do przekazania tylko identyfikatorów próbek do tego procesu:
        ...
        **Naprawa:** Użyj poprawnego kanału i wydobądź identyfikatory próbek
        ...
        Teraz dostajemy się trochę dalej, ale otrzymujemy błąd o `No such variable: i`, ponieważ nie zmieniliśmy znaczenia zmiennej Bash.

        **Błąd 7: Błąd zmiany znaczenia zmiennej Bash**
        ```groovy linenums="48"
        echo "Heavy computation $i for ${sample_id}"  // BŁĄD: $i nie zmienił znaczenia
        ```
        **Naprawa:** Zmień znaczenie zmiennej bash
        ```groovy linenums="48"
        ...
        **Błąd 8: Błąd konfiguracji zasobów**
        ...
        time '1 ms'  // BŁĄD: Nierealistyczne ograniczenie czasu
        ```
        **Naprawa:** Zwiększ do realistycznego limitu czasu
        ```groovy linenums="36"
        time '100 s'
        ```
        ...
        **Błąd 9: Niedopasowanie nazwy pliku wyjściowego**
        ...
        ```
        **Naprawa:** Dopasuj deklarację wyjścia
        ```groovy linenums="49"
        ...
        **Błąd 10: Niedopasowanie nazwy pliku wyjściowego**
        ...
        handleFiles(file_ch)
        ```
        **Naprawa:** Weź wyjście z poprzedniego procesu
        ```groovy linenums="88"
        ...
        * Ten workflow zawiera kilka zamierzonych bugów do celów edukacyjnych
        */
        ...
        */
        ...
        /*
        * Proces z problemami z zasobami
        */
...
1. **Czytaj uważnie komunikaty błędów** - często wskazują bezpośrednio na problem
...
4. **Sprawdzaj katalogi robocze** - gdy procesy zawiodą, logi mówią Ci dokładnie, co poszło nie tak
...
Stosowanie tych technik w swojej pracy umożliwi Ci spędzenie mniej czasu na walce z komputerem, szybsze rozwiązywanie problemów i ochronę się przed przyszłymi problemami.
...
// Niezdefiniowana zmienna - zmień znaczenie ukośnikiem wstecznym dla zmiennych Bash
...
!!! tip "Szybka instrukcja debugowania"
    ...
    **Tajemnicze zachowanie?** → Uruchom z `-stub-run` do testowania logiki workflow

    **Wprowadzono poprawki?** → Użyj `-resume` do oszczędzenia czasu testowania: `nextflow run workflow.nf -resume`
...
- [Zrozumienie kanałów Nextflow](https://www.nextflow.io/docs/latest/channel.html): Głębokie zanurzenie się w typy kanałów i zachowanie
- [Instrukcja dyrektyw procesów](https://www.nextflow.io/docs/latest/process.html#directives): Wszystkie dostępne opcje konfiguracji procesów
...
- Ustawienie [Seqera Platform](https://seqera.io/platform/) do monitorowania i debugowania na dużą skalę
- Używanie [Wave containers](https://seqera.io/wave/) dla odtwarzalnych środowisk oprogramowania

**Pamiętaj:** Efektywne debugowanie to umiejętność, która poprawia się z praktyką. Systematyczna metodologia i kompleksowy zestaw narzędzi, które tutaj nabyłeś, będą Ci dobrze służyły przez całą Twoją drogę rozwoju Nextflow.
