...
- **Filtrowanie danych**: Jak filtrować dane za pomocą `filter`
- **Dzielenie danych**: Jak dzielić dane na różne kanały na podstawie warunku
- **Wyświetlanie danych**: Jak używać `view` do wydrukowania danych i oznaczania wyjścia z różnych kanałów
...
Mamy teraz każdą próbkę powtórzoną na wszystkich interwałach genomowych, tworząc wiele niezależnych jednostek analizy, które można przetwarzać równolegle. Ale co, jeśli chcemy połączyć powiązane próbki z powrotem? W następnej sekcji nauczymy się, jak grupować próbki dzielące wspólne atrybuty.
...
- **Przygotowania do przetwarzania równoległego**: Jak ustawić dane do rozproszonej analizy

## 5. Agregowanie próbek za pomocą `groupTuple`

W poprzednich sekcjach nauczyliśmy się, jak dzielić dane z pliku wejściowego i filtrować według określonych pól (w naszym przypadku próbki normalne i nowotworowe). Ale to obejmuje tylko jeden typ łączenia. Co, jeśli chcemy grupować próbki według określonego atrybutu? Na przykład, zamiast łączyć dopasowane pary normalny-nowotworowy, możemy chcieć przetwarzać wszystkie próbki z "sampleA" razem niezależnie od ich typu. Ten wzorzec jest powszechny w workflow'ach bioinformatycznych, gdzie możesz chcieć przetwarzać powiązane próbki oddzielnie dla efektywności przed porównaniem lub połączeniem wyników na końcu.

Nextflow zawiera wbudowane metody do tego, główną, którą się przyjrzymy, jest `groupTuple`.

Zacznijmy od grupowania wszystkich naszych próbek, które mają te same pola `id` i `interval`, co byłoby typowe dla analizy, w której chcielibyśmy grupować replikaty techniczne, ale utrzymywać znacząco różne próbki rozdzielone.

Aby to zrobić, powinniśmy wyodrębnić nasze zmienne grupujące, abyśmy mogli używać ich w izolacji.
...
Możemy ponownie wykorzystać metodę `subMap` z wcześniej, aby wyizolować nasze pola `id` i `interval` z mapy. Jak wcześniej, użyjemy operatora `map` do zastosowania metody `subMap` do pierwszego elementu krotki dla każdej próbki.
...
Możemy zobaczyć, że udało nam się wyizolować pola `id` i `interval`, ale jeszcze nie zgrupować próbek.

!!! note

    Odrzucamy tutaj pole `replicate`. Dzieje się tak, ponieważ nie potrzebujemy go do dalszego przetwarzania downstream. Po ukończeniu tego tutorialu, sprawdź, czy możesz je uwzględnić bez wpływu na późniejsze grupowanie!

Teraz zgrupujmy próbki według tego nowego elementu grupującego, używając [operatora `groupTuple`](https://www.nextflow.io/docs/latest/operator.html#grouptuple).
...
Zauważ, że nasza struktura danych zmieniła się, a w obrębie każdego elementu kanału pliki są teraz zawarte w krotkach jak `[patientA_rep1_normal.bam, patientA_rep2_normal.bam]`. Dzieje się tak, ponieważ gdy używamy `groupTuple`, Nextflow łączy pojedyncze pliki dla każdej próbki grupy. Jest to ważne do zapamiętania podczas obsługi danych downstream.
...
- **Izolowania kluczy grupujących**: Jak używać `subMap` do wyodrębnienia określonych pól do grupowania
...
W tej misji pobocznej nauczyłeś się, jak dzielić i grupować dane za pomocą kanałów.

Poprzez modyfikowanie danych w miarę przepływu przez pipeline, możesz skonstruować skalowalny pipeline bez używania pętli lub instrukcji while, oferując kilka przewag nad bardziej tradycyjnymi podejściami:
...
- Możemy być tak złożeni lub prości, jak to wymagane
...
Opanowanie tych operacji kanałów umożliwi Ci budowanie elastycznych, skalowalnych pipeline'ów, które obsługują złożone relacje danych bez uciekania się do pętli lub programowania iteracyjnego, pozwalając Nextflow optymalizować wykonanie i równoleglić niezależne operacje automatycznie.

### Kluczowe wzorce

1.  **Tworzenie ustrukturyzowanych danych wejściowych:** Zaczynając od pliku CSV z mapami meta (budując na wzorcach z [Metadane w workflow'ach](./metadata.md))
...
5.  **Agregowanie według kluczy grupujących:** Użyliśmy `groupTuple` do grupowania według pierwszego elementu w każdej krotce, zbierając próbki dzielące pola `id` i `interval` i łącząc replikaty techniczne.

    ```groovy
    channel.groupTuple()
    ```

6.  **Optymalizowanie struktury danych:** Użyliśmy `subMap` do wyodrębnienia określonych pól i utworzyliśmy nazwane domknięcie do uczynienia transformacji wielokrotnego użytku.
