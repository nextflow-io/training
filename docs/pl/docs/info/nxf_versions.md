...
Od wersji 3.0 portalu szkoleniowego wszystkie nasze kursy opierają się na wydaniu Nextflow w wersji 25.10.2, chyba że na stronie indeksu kursu podano inaczej (z wyjątkiem przestarzałych lub zarchiwizowanych materiałów, które mogą nie zawierać informacji o wersji).

Ponieważ kursy używają teraz typowanych wejść na poziomie workflow'a oraz dyrektyw wyjściowych na poziomie workflow'a, wymagają użycia parsera składni V2.
...
Możesz uzyskać dostęp do starszych wersji materiałów szkoleniowych, które działają z Nextflow'em **przed wersją 25.10.2**, poprzez menu rozwijane na górze każdej strony, które pokazuje numerowaną wersję materiałów szkoleniowych.
...
Parser v2 jest bardziej rygorystyczny i umożliwia nowe funkcje języka, takie jak statyczne typowanie (typowane wejścia i wyjścia) oraz dyrektywy wyjściowe na poziomie workflow'a.
...
Podsumowując: DSL2 to język, który piszesz, a wariant parsera składni określa, jak ściśle ten język jest interpretowany i jakie zaawansowane funkcje są dostępne.
...
W zależności od generacji kodu Nextflow, który zamierzasz migrować, możesz być w stanie wykonać większość pracy za pomocą lintera Nextflow przy użyciu polecenia `nextflow lint -format`.
