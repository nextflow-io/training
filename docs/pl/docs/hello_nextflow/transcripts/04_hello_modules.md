...
Ten rozdział nazywa się Hello Modules i będziemy mówić o tym, jak modularyzować kod Nextflow. Zamierzamy wziąć nasz skrypt workflow'u i podzielić go na osobne pliki.
...
Klasycznym przykładem jest repozytorium modułów nf-core, które zawiera tysiące różnych gotowych do użycia narzędzi w modułach, które możesz zainstalować i użyć w swoim workflow.

Nextflow może również pracować z sub workflow'ami, które są jak moduły, ale mają wiele procesów. To wykracza poza zakres tego szkolenia, ale działa to w zasadzie w ten sam sposób.
...
Generalnie dobrym pomysłem jest umieszczenie modułów w podkatalogu w repozytorium Twojego pipeline'u, po prostu aby zachować porządek. Możesz nazwać to jak chcesz, ale zgodnie z konwencją zazwyczaj nazywamy to modules.

Więc chodźmy do terminala i zróbmy mkdir modules. Możesz zobaczyć, jak pojawia się na pasku bocznym VS Code tutaj.
...
Dobrze, teraz wezmę kod modułu z workflow'u. Wezmę również hash bang tutaj i najpierw go skopiuję, aby było jasne, że to plik Nextflow. A potem wezmę ten proces i wytnę. Więc usunę go z mojego głównego skryptu workflow'u i wkleję do tego nowego modułu.
...
Więc przenieśliśmy nasz kod, ale nie zmieniliśmy niczego w sposobie wykonywania workflow'u i jest całkowicie niezmieniony. Jedyna różnica polega na tym, że mamy teraz czystszy kod, łatwiejszy w utrzymaniu i łatwiejszy do udostępnienia innym.
...
W następnym rozdziale zmienimy trochę tempo i przestanimy myśleć tak dużo o składni pisania kodu Nextflow, a pomyślimy trochę o tym, jak używamy oprogramowania w samych procesach. Dołącz do nas w części piątej Hello Containers.
