...
Przejdziemy przez podstawy Nextflow'a, wyjaśniając jak pisać i uruchamiać pipeline'y oraz jak je konfigurować.

Zbudujesz własny prosty wieloetapowy pipeline. Omówimy terminologię taką jak operatory i fabryki kanałów, a pod koniec kursu będziesz gotowy, aby rozpocząć budowanie własnych pipeline'ów bioinformatycznych.
...
Wszystkie materiały szkoleniowe dla kursów Nextflow'a znajdują się na training.nextflow.io. Możesz przejść do nich w Twojej przeglądarce internetowej. Uruchom ją teraz i możemy się rozejrzeć.

Będę korzystał z wersji 2.1.1. Wprowadzamy co jakiś czas niewielkie aktualizacje i poprawki, więc nie martw się, jeśli będzie trochę inaczej. Jeśli jednak materiały zbyt bardzo się rozjechały, zawsze możesz użyć tego selektora wersji na górze, aby wybrać dokładną wersję materiałów, którą będę omawiał.

Jeśli wolisz jasny motyw, możesz zmienić motyw strony tutaj.

Zobacz tłumaczenia tutaj, chociaż w momencie nagrywania tylko angielski zawiera te nowe materiały.

Zobacz także cały kod źródłowy strony szkoleniowej i wszystko, z czym będziemy pracować, na GitHub.
...
Klikam go, przenosi mnie do tej sekcji. Widzimy instrukcje dotyczące lokalnego programowania — jeśli chcesz użyć Twojego własnego laptopa z Twoją własną kopią VS Code i własnymi instalacjami oprogramowania. Albo możesz skorzystać z tego, czego oczekujemy od większości ludzi, czyli użycia czegoś, co nazywa się GitHub Codespaces.
...
Aby utworzyć codespace ze wszystkim, czego potrzebujemy, poszukaj przycisków w materiałach dokumentacji, które mówią "Open in GitHub Codespaces". Kliknę to teraz, otwieram to w nowej karcie. Pokazano mi tę stronę internetową. Widzisz, że jest to wstępnie skonfigurowane z nextflow-io training.

Mogę po prostu kliknąć create new codespace. Ale tak naprawdę zalecamy użycie nieco większej maszyny do szkolenia Nextflow'a z czterema procesorami zamiast dwóch. Możesz zmienić, której wersji materiałów używa. Więc to domyślnie ustawia się na 2.1.1, ponieważ to jest wersja dokumentacji, z której poszedłem za linkiem. Ale mogę też ustawić to na konkretną gałąź repozytorium, jeśli chcę.
...
Musisz czekać w ten sposób tylko za pierwszym razem, gdy tworzysz codespace. Jeśli przejdziesz do github.com/codespaces tutaj, zobaczysz wszystkie różne Codespaces, które masz otwarte. Oto ten, który właśnie utworzyłem. Następnym razem możesz przejść tutaj i wybrać poprzedni codespace, aby od razu do niego wrócić. To jest znacznie, znacznie szybszy proces rozgrzania tego istniejącego środowiska. To również zachowa wszystkie zmiany, które wprowadzałeś w VS Code i w plikach, więc nie stracisz postępu, jeśli wyjdziesz i wrócisz.
...
Na górze tutaj jest główne menu. Eksplorator plików jest tym, który będziemy mieć najczęściej tutaj. Możesz kliknąć prawym przyciskiem myszy dowolny z tych plików i zrobić normalne rzeczy, których oczekujesz. Może być konieczne kliknięcie przez niektóre ostrzeżenia takie jak to, gdzie wytnij, kopiuj i możesz również pobrać na Twoją lokalną maszynę.
...
Mamy również plik nextflow.config, który ma tylko podstawowe ustawienia konfiguracji do uruchamiania Nextflow'a w tym środowisku, którymi nie musisz się martwić w tym momencie. Plik greetings.csv, którego będziemy używać do przetwarzania danych, który zostanie wprowadzony w następnej części tego kursu, oraz plik test-params.json, który będzie używany w części szóstej i możesz go zignorować na razie.

Te pliki Nextflow'a to tylko początek każdego ćwiczenia. Jeśli chcesz zobaczyć, jak powinny wyglądać, gdy są skończone, możesz przejść do katalogu solutions i tam są odpowiedzi dla każdej części kursu szkoleniowego, więc możesz zobaczyć działającą wersję tego, do czego dążysz.
