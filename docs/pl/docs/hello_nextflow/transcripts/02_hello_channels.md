...
!!!note "Uwaga"
    ...
    Numery sekcji w transkrypcji służą wyłącznie orientacji i mogą nie obejmować wszystkich numerów sekcji z materiałów.

## Powitanie

Cześć, witamy w drugiej części Hello Nextflow.

Ten rozdział nosi tytuł Hello Channels. Porozmawiamy o fundamentalnej części Nextflow'a.

Kanały to elementy łączące różne kroki w Twoim pipeline. To właśnie przez nie Twoje dane i logika przepływają w workflow.
...
Wszystkie materiały są tutaj spisane, więc możesz podążać we własnym tempie i nadrobić wszystko, co mogłeś przegapić.
...
W tym rozdziale będziemy edytować inny plik. Nazywa się Hello Channels, więc możesz go znaleźć w pasku bocznym, kliknij dwukrotnie, aby otworzyć.

Jeśli właśnie przyszedłeś z rozdziału pierwszego, ten plik będzie Ci bardzo znajomy. Punkt wyjścia tutaj to w zasadzie miejsce, w którym zakończyliśmy rozdział pierwszy: nasz proces o nazwie sayHello, nasze wejście, wyjście, nasz publishDir i nasz params.greeting, oraz nasz prosty workflow.
...
W niektórych pipeline Nextflow'a możesz zobaczyć, że ludzie używają specjalnej niejawnej zmiennej o nazwie "$it". Tak jak to. To jest specjalna zmienna w kodzie Nextflow'a, która jest skrótem, więc nie musisz robić małej definicji zmiennej. Jednak z czasem myślimy, że to nie jest bardzo jasne dla ludzi, którzy są nowi w Nextflow, i teraz zniechęcamy do używania "$it".
...
Teraz spróbujemy to zrobić, używając pliku z serią wejść zamiast tego. To jest bardzo powszechny sposób pisania pipeline Nextflow'a przy użyciu arkusza próbek lub CSV z metadanymi.
...
Więc operator map jest niezwykle elastyczny i bardzo potężny do modyfikowania kanałów w locie. Wstawmy kolejną instrukcję view, aby zobaczyć, co robi w naszym wykonaniu. Może adjudicat tę linię i przenieść ją w dół. I after map.
...
Dobra, to prawdopodobnie jeden z najważniejszych rozdziałów całego szkolenia. Gdy zrozumiesz kanały, fabryki kanałów i operatory, zaczniesz włączać się w siłę Nextflow'a i to, co czyni go wyjątkowym jako język programowania. Ta funkcjonalność pozwala Nextflow'owi zrównoleglać wszystkie Twoje workflow'y dla Ciebie i generować niezwykle złożoną logikę workflow z bardzo czystą składnią i modelem przepływu danych push. To może być na początku trochę dziwna koncepcja, ale gdy już przyzwyczaisz się do pisania kodu w ten sposób, szybko poczuje się to naturalne i zanim się zorientujesz, będziesz pisać fantastyczne workflow'y.
