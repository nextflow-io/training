...
Uruchomimy pierwszy pipeline Nextflow'a, zobaczymy, co Nextflow robi, gdzie działa, jakie pliki tworzy i jaki jest cel tych plików.
...
Proces musi mieć sekcję `script`, a zawarta tutaj jest fragmentu bash w wielowierszowym ciągu znaków, który jest częścią kodu faktycznie wykonywaną w środowisku obliczeniowym.

Mamy również tutaj instrukcję `output`, która mówi Nextflow'owi, jakie pliki mają zostać utworzone przez skrypt. Zauważ, że `output` tutaj ma słowo kluczowe `path`, które mówi Nextflow'owi, że to jest plik, a nie wartość czy ciąg znaków.

W bloku `script` jest to po prostu zwykła instrukcja bash i jest dokładnie taka sama jak to, co napisaliśmy w terminalu. Wykonujemy echo hello world do pliku o nazwie output.txt. Ten output.txt jest następnie przechwytywany przez definicję `output`. Definicja `output` tak naprawdę nic nie robi. Tylko mówi Nextflow'owi, czego się spodziewać, a gdyby ten plik nie został utworzony, Nextflow zgłosiłby błąd.
...
Najważniejszą częścią jest jednak to, że mówi nam, które kroki zostały uruchomione w pipeline'ie. Możesz zobaczyć, że nasz proces o nazwie sayHello został uruchomiony i został uruchomiony raz i został ukończony w stu procentach.
...
Krótka uwaga tutaj. Jest nowa składnia Nextflow'a, która nadchodzi, nazywana definicjami wyjścia workflow'a, która ostatecznie zastąpi `publishDir`. Daje nam to sposób na zdefiniowanie wszystkich wyjść z workflow'a na poziomie pipeline'u w dół w bloku `workflow`. Jest to opisane w dokumentacji Nextflow'a, jeśli chcesz to wypróbować. Ale na razie `publishDir` będzie jeszcze przez jakiś czas, więc nadal mamy to w szkoleniu na rok 2025.
...
Może budujesz iteracyjnie swój workflow i dodajesz kroki i chcesz, aby pierwsze kroki po prostu użyły wersji z cache'a. Lub może coś poszło nie tak w Twoim systemie obliczeniowym w połowie workflow'a i chcesz, aby kontynuował od miejsca, w którym przerwał, ale pominął kroki, które już wykonał.
...
Dobra. Ale jeśli rozwijasz iteracyjnie, zbudujesz wiele tych plików workflow'a. To może być problem, jeśli możesz mieć mało miejsca.

Nextflow może nam pomóc wyczyścić te katalogi work za pomocą kilku poleceń pomocniczych. Jeśli zrobię "nextflow log". To da mi listę wszystkich różnych uruchomień workflow'a, które wykonałem w tym katalogu, i mają tutaj nazwy uruchomień. Możesz zobaczyć ten gloomy quick, który był pierwszym, który uruchomiliśmy, a następnie te dwa nowe.
...
Zacznijmy od tego. Zrobimy trzy rzeczy. Dodamy nowe wejście do procesu. Powiemy skryptowi procesu, jak użyć tego wejścia, a następnie podłączymy to w workflow'ie, abyśmy mogli używać tego dynamicznie z flagą linii poleceń podczas uruchamiania Nextflow'a.
...
Więc jeśli wrócę do mojego pliku hello-world.nf, mogę przejść do skryptu tuż nad workflow'em, wpisać "params.greeting" i zdefiniować to jak każdą inną zmienną. Więc umieśćmy tutaj ciąg znaków i powiedzmy "Holà mundo!"
