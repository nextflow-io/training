...
W praktyce to podejście ma poważne ograniczenia; mianowicie działa tylko dla bardzo prostych przypadków, gdy chcemy uruchomić proces tylko raz, na pojedynczej wartości.
W większości realistycznych przypadków użycia workflow'u chcemy przetwarzać wiele wartości (dane eksperymentalne dla wielu próbek, na przykład), więc potrzebujemy bardziej wyrafinowanego sposobu obsługi wejść.
...
Możesz pamiętać, że zakodowaliśmy na sztywno nazwę pliku wyjściowego dla procesu `sayHello`, więc wszystkie trzy wywołania utworzyły plik o nazwie `output.txt`.
...
Chcemy móc porównać, jak zawartość wygląda przed i po zastosowaniu operatora `flatten()`, więc dodamy drugą instrukcję I dodamy trochę kodu, aby były wyraźniej oznaczone w wyjściu.
