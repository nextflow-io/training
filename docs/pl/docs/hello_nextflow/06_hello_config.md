...
Ta sekcja zbada, jak skonfigurować i zarządzać konfiguracją Twojego pipeline'u Nextflow, abyś mógł dostosować jego zachowanie, zaadaptować go do różnych środowisk i zoptymalizować wykorzystanie zasobów _bez modyfikacji choćby jednej linii samego kodu workflow_.
...
Kiedy kontrolujemy sposób organizacji naszych 'publikowanych' wyjść, mamy dwa odrębne priorytety:
...
Zacznijmy od jego dostosowania, używając opcji CLI `-output-dir`.
...
Jest to zalecany sposób kontrolowania ścieżki głównej, gdzie publikowane są wyjścia.
...
Przedrostek `hello_config/` był zakodowany na sztywno we wcześniejszych rozdziałach, ale skoro teraz uczymy się elastycznego konfigurowania ścieżek wyjściowych, możemy usunąć to zakodowanie.
...
Oprócz zmiany katalogu wyjściowego przez CLI, możemy również ustawić niestandardową wartość domyślną w pliku konfiguracyjnym używając `outputDir`.
Pozwala to ustawić ścieżkę katalogu dynamicznie - nie tylko używając statycznych ciągów.
...
Możemy również ustawić deklaracje wyjściowych `path` dynamicznie, dla każdego wyjścia osobno.
...
Możemy pójść dalej i komponować bardziej złożone ścieżki podkatalogów.
...
!!! tip "Wskazówka"
    ...
    Dołączenie `params.batch` w `path` bloku wyjścia, zamiast w konfiguracji `outputDir`, oznacza, że nie zostanie nadpisane przez `-output-dir` w CLI.
...
Ustawienie `workflow.output.mode` w pliku konfiguracyjnym wystarczy do nadpisania tego, co jest ustawione w pliku workflow, ale usuńmy niepotrzebny kod tak czy inaczej.
...
Żeby było jasne, nie _zastępujemy_ dyrektywy `container`, _dodajemy_ alternatywną opcję.
