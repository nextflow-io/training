...
Poznasz sposób, w jaki Nextflow umożliwia:
...
Tym razem wprowadzimy następujące zmiany do naszego workflow'a, aby lepiej odzwierciedlał sposób tworzenia rzeczywistych workflow'ów:

1. Dodamy drugi krok, który przekształca powitanie na wielkie litery.
2. Dodamy trzeci krok, który zbiera wszystkie przekształcone powitania i zapisuje je do jednego pliku.
3. Dodamy parametr do nazwania końcowego pliku wyjściowego i przekażemy go jako drugie wejście do kroku zbierającego.
4. Sprawimy, że krok zbierający będzie również raportował prostą statystykę dotyczącą przetworzonych danych.
...
Jest on równoważny skryptowi powstałemu w wyniku ukończenia części 2 tego kursu, z tym że usunęliśmy instrukcje `view()` i zmieniliśmy miejsce docelowe wyjścia:
...
Powinien wyglądać znajomo, z tym że teraz jawnie pokazujemy, że wyjścia procesu są pakowane w kanał, podobnie jak wejścia.
...
Dodamy krok przekształcający każde powitanie na wielkie litery.
...
- Zdefiniować polecenie, którego użyjemy do przekształcenia na wielkie litery.
- Napisać nowy proces opakowujący polecenie przekształcające.
- Wywołać nowy proces w bloku workflow i skonfigurować go tak, aby przyjmował wyjście procesu `sayHello()` jako wejście.

### 1.1. Zdefiniuj polecenie przekształcające i przetestuj je w terminalu

Do przekształcenia powitań na wielkie litery użyjemy klasycznego narzędzia UNIX o nazwie `tr` (od 'text replacement'), z następującą składnią:

```bash title="Składnia"
tr '[a-z]' '[A-Z]'
```

To bardzo naiwne jednolinijkowe polecenie zamiany tekstu, które nie uwzględnia liter akcentowanych, więc na przykład 'Holà' stanie się 'HOLà', ale wystarczająco dobrze posłuży do zademonstrowania koncepcji Nextflow, a to się liczy.

Aby je przetestować, możemy uruchomić polecenie `echo 'Hello World'` i przekierować jego wyjście do polecenia `tr`:
...
### 1.2. Napisz krok przekształcający jako proces Nextflow
...
/*
 * Użyj narzędzia do zamiany tekstu, aby przekształcić powitanie na wielkie litery
 */
...
To jeszcze nie działa, ponieważ nie określiliśmy, co powinno być wejściem do procesu `convertToUpper()`.
...
To pokazuje, że możesz kontrolować ustawienia wyjścia na bardzo szczegółowym poziomie, dla każdego pojedynczego wyjścia.
...
W wyjściu konsoli pojawia się teraz dodatkowa linia odpowiadająca nowemu procesowi, który właśnie dodaliśmy.
...
Naucz się zbierać wyjścia z przetwarzanych wsadowo wywołań procesów i przekazywać je do pojedynczego procesu.

---

## 2. Dodaj trzeci krok do zbierania wszystkich powitań

Gdy używamy procesu do zastosowania przekształcenia do każdego z elementów w kanale, jak robimy to tutaj z wieloma powitaniami, czasami chcemy zebrać elementy z kanału wyjściowego tego procesu i przekazać je do innego procesu, który wykonuje jakąś analizę lub sumowanie.

Aby to zademonstrować, dodamy nowy krok do naszego pipeline'u, który zbierze wszystkie powitania zapisane wielkimi literami wyprodukowane przez proces `convertToUpper` i zapisze je do jednego pliku.
...
### 2.1. Zdefiniuj polecenie zbierające i przetestuj je w terminalu

Krok zbierający, który chcemy dodać do naszego workflow'a, użyje polecenia `cat` do połączenia wielu powitań zapisanych wielkimi literami w jeden plik.
...
### 2.2. Utwórz nowy proces do wykonania kroku zbierającego
...
Zauważ, że używamy przedrostka `path`, mimo że spodziewamy się, że będzie to zawierać wiele plików.
...
Konkretnie, nie możemy napisać polecenia z góry, więc musimy powiedzieć Nextflow'owi, jak je skomponować w czasie wykonania na podstawie tego, jakie wejścia wpłyną do procesu.
...
Teoretycznie powinno to obsłużyć dowolną liczbę plików wejściowych.
...
### 2.3. Dodaj krok zbierający do workflow'a

Teraz powinniśmy po prostu wywołać proces zbierający na wyjściu kroku przekształcającego.
...
O nie. Krok zbierający został uruchomiony indywidualnie dla każdego powitania, co NIE jest tym, czego chcieliśmy.

<figure class="excalidraw">
--8<-- "docs/en/docs/hello_nextflow/img/hello-collect-no-operator.svg"
</figure>

Musimy coś zrobić, aby jawnie powiedzieć Nextflow'owi, że chcemy, aby ten trzeci krok został uruchomiony na wszystkich elementach w kanale wyjściowym z `convertToUpper()`.
...
=== "Po"
    ...
    ```groovy title="hello-workflow.nf" linenums="73" hl_lines="4-6"
        ...
        convertToUpper.out.view { contents -> "Przed collect: $contents" }
        convertToUpper.out.collect().view { contents -> "Po collect: $contents" }
    }
    ```
...
Instrukcje `view()` mogą znajdować się w dowolnym miejscu; umieściliśmy je zaraz po wywołaniu dla czytelności.
...
??? success "Wyjście polecenia"
    ...
    Przed collect: /workspaces/training/hello-nextflow/work/b3/d52708edba8b864024589285cb3445/UPPER-Bonjour-output.txt
    Przed collect: /workspaces/training/hello-nextflow/work/99/79394f549e3040dfc2440f69ede1fc/UPPER-Hello-output.txt
    Przed collect: /workspaces/training/hello-nextflow/work/aa/56bfe7cf00239dc5badc1d04b60ac4/UPPER-Holà-output.txt
...
- Trzy instrukcje `Przed collect:`, po jednej dla każdego powitania: w tym momencie ścieżki plików są pojedynczymi elementami w kanale.
...
To w zasadzie odwrotna operacja od punktu 2.4.2.

### Podsumowanie

Wiesz, jak zbierać wyjścia z partii wywołań procesów i przekazywać je do wspólnego kroku analizy lub sumowania.
...
Podobnie jak zademonstrowano dla `--input`, możesz nadpisać tę wartość domyślną, określając wartość za pomocą `--batch` w wierszu poleceń.
...
Możemy albo nadać tym kanałom wyjściowym nazwy, co ułatwia późniejsze odwoływanie się do nich indywidualnie, albo możemy odwoływać się do nich za pomocą indeksu.
...
=== "Po"
    ...
    ```groovy title="hello-workflow.nf" linenums="55" hl_lines="2 5"
        ...
        """
        cat ${input_files} > 'COLLECTED-${batch_name}-output.txt'
        echo 'Było ${count_greetings} powitań w tej partii.' > '${batch_name}-report.txt'
        """
    ```
...
Ale jak mówi powiedzenie, dlaczego nie oba?

!!! tip

    Jeśli nie nazwiesz wyjść procesu używając `emit:`, nadal możesz uzyskać do nich dostęp indywidualnie, używając ich odpowiedniego indeksu (liczenie od zera).
...
Jak widzisz, odwoływanie się do konkretnych wyjść procesów jest teraz trywialne.
Gdy w części 5 (Kontenery) dodamy jeszcze jeden krok do naszego pipeline'u, będziemy mogli łatwo odwołać się do `collectGreetings.out.outfile` i przekazać go do nowego procesu (spoiler: nowy proces nazywa się `cowpy`).
...
Ogólniej, rozumiesz kluczowe zasady związane z łączeniem procesów w typowe wzorce.

### Co dalej?

Zrób sobie ekstra długą przerwę, zasłużyłeś na to.

Gdy będziesz gotowy, przejdź do [**Części 4: Hello Modules**](./04_hello_modules.md), aby nauczyć się modularyzować swój kod dla lepszej łatwości utrzymania i efektywności kodu.
...
- [ ] Losowa kolejność dla wykonania równoległego
...
Dowiedz się więcej: [2.4. Użyj operatora do zebrania powitań w jedno wejście](#24-uzyj-operatora-do-zebrania-powitan-w-jedno-wejscie)
</quiz>
...
Dowiedz się więcej: [3. Przekaż więcej niż jedno wejście do procesu](#3-przekaz-dodatkowe-parametry-do-procesu)
</quiz>
