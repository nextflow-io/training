...
Powinien wyglądać znajomo, z tą różnicą, że teraz wyraźnie pokazujemy, iż wyjścia procesu są pakowane w kanał, podobnie jak wejścia.
...
Wygodnie jest to, że Nextflow automatycznie pakuje wyjście procesu w kanał, jak pokazano na diagramie w sekcji rozgrzewki.
Możemy odnieść się do kanału wyjściowego procesu jako `<proces>.out`.
...
Zauważ, że używamy przedrostka `path`, mimo że oczekujemy, iż będzie to zawierało wiele plików.
...
!!! tip "Wskazówka"
    ...
    Przykład tego możesz zobaczyć w kursie [Nextflow for Genomics](../../nf4_science/genomics/).
...
Mogliśmy uzyskać dostęp do ich wyjść bardzo wygodnie za pomocą składni `<proces>.out`, której używaliśmy zarówno w kontekście przekazywania wyjścia do następnego procesu (np. `convertToUpper(sayHello.out)`), jak i w kontekście sekcji `publish:` (np. `first_output = sayHello.out`).
