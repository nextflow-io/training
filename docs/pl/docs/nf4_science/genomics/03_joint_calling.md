...
W Części 2 zbudowałeś pipeline wywoływania wariantów dla pojedynczych próbek, który przetwarzał dane każdej próbki niezależnie.
Teraz rozszerzymy go, aby zaimplementować wspólne wywoływanie wariantów, jak omówiono w [Części 1](01_method.md).
...
??? info "Jak rozpocząć od tej sekcji"
    ...
    Ta sekcja kursu zakłada, że ukończyłeś [Część 2: Wywoływanie wariantów dla pojedynczych próbek](./02_per_sample_variant_calling.md) i masz działający pipeline `genomics.nf`.
    ...
    To da Ci kompletny workflow wywoływania wariantów dla pojedynczych próbek.
...
1. **Zmodyfikować krok wywoływania wariantów dla pojedynczych próbek, aby generował GVCF.**
   Obejmuje to aktualizację poleceń i wyjść procesu.
2. **Dodać krok wspólnego genotypowania, który łączy i genotypuje pliki GVCF z pojedynczych próbek.**
   Wprowadza to operator `collect()`, domknięcia Groovy do konstruowania linii poleceń oraz procesy z wieloma poleceniami.
...
Pipeline z Części 2 generuje pliki VCF, ale wspólne wywoływanie wymaga plików GVCF.
Musimy włączyć tryb wywoływania wariantów GVCF i zaktualizować rozszerzenie pliku wyjściowego.

Przypomnij sobie polecenie wywoływania wariantów GVCF z [Części 1](01_method.md):
...
### 1.1. Poinformować HaplotypeCaller, aby emitował GVCF i zaktualizować rozszerzenie wyjściowe
...
### 1.2. Zaktualizować rozszerzenie pliku wyjściowego w bloku wyjść procesu
...
### 1.3. Zaktualizować cele publikowania dla nowych wyjść GVCF
...
### 1.4. Zaktualizować blok wyjściowy dla nowej struktury katalogów
...
## 2. Dodać krok wspólnego genotypowania

Teraz musimy zebrać pliki GVCF z pojedynczych próbek, połączyć je w magazyn danych GenomicsDB i uruchomić wspólne genotypowanie, aby wygenerować plik VCF na poziomie kohorty.
Jak omówiono w [Części 1](01_method.md), jest to operacja dwóch narzędzi: GenomicsDBImport łączy pliki GVCF, a następnie GenotypeGVCFs generuje końcowe wywołania wariantów.
...
Pierwsze polecenie przyjmuje pliki GVCF z pojedynczych próbek i plik interwałów, a następnie generuje magazyn danych GenomicsDB.
...
#### 2.1.1. Dodać parametr `cohort_name`
...
#### 2.1.2. Zebrać wyjścia HaplotypeCaller z wszystkich próbek
...
Możemy zebrać pliki GVCF i ich pliki indeksów osobno (w przeciwieństwie do trzymania ich razem w krotkach), ponieważ Nextflow przygotuje wszystkie pliki wejściowe razem do wykonania, więc pliki indeksów będą obecne obok plików GVCF.

!!! tip

    Możesz użyć operatora `view()`, aby sprawdzić zawartość kanałów przed i po zastosowaniu operatorów kanału.

### 2.2. Napisać proces wspólnego genotypowania i wywołać go w workflow'ie
...
#### 2.2.1. Skonstruować ciąg znaków, aby nadać każdemu GVCF argument `-V`
...
Co ważne, musimy skonstruować ten ciąg dynamicznie z dowolnych plików znajdujących się w zebranym kanale.
Nextflow (poprzez Groovy) zapewnia zwięzły sposób, aby to zrobić:
...
#### 2.2.2. Wypełnić moduł dla procesu wspólnego genotypowania
...
GenomicsDBImport łączy pliki GVCF z pojedynczych próbek w magazyn danych, a następnie GenotypeGVCFs odczytuje ten magazyn danych i generuje końcowy plik VCF na poziomie kohorty.
...
Aby użyć go w workflow'ie, musisz zaimportować moduł i dodać wywołanie procesu.

#### 2.2.3. Zaimportować moduł
...
### 2.3. Skonfigurować obsługę wyjść

Musimy opublikować wyjścia wspólnego VCF.
Dodaj cele publikowania i wpisy bloku wyjściowego dla wyników wspólnego genotypowania.

#### 2.3.1. Dodać cele publikowania dla wspólnego VCF
...
#### 2.3.2. Dodać wpisy bloku wyjściowego dla wspólnego VCF

Dodaj wpisy dla plików wspólnego VCF.
Umieścimy je w katalogu głównym katalogu wyników, ponieważ jest to końcowe wyjście.
...
Końcowy plik wyjściowy, `family_trio.joint.vcf` (i jego indeks), znajdują się w katalogu wyników.
...
!!! note
    ...
    Pamiętaj, że pliki danych, które Ci przekazaliśmy, obejmują tylko niewielką część chromosomu 20.
...
Wiesz również, jak konstruować linię poleceń używając domknięć Groovy oraz jak uruchamiać wiele poleceń w jednym procesie.

### Co dalej?

Świętuj swój sukces i weź zasłużoną przerwę.

W następnej części tego kursu nauczysz się, jak uruchomić gotowy do produkcji pipeline wywoływania wariantów z nf-core i porównać go z pipeline'em, który zbudowałeś ręcznie.
