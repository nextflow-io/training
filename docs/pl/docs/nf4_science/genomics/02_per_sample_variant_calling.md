...
To automatyzuje kroki z pierwszej sekcji [Części 1: Przegląd metody](./01_method.md#1-per-sample-variant-calling), gdzie uruchamiałeś te polecenia ręcznie w kontenerach.
...
!!! warning "Ostrzeżenie"
    ...
    Ścieżki plików w pliku CSV są ścieżkami bezwzględnymi, które muszą pasować do Twojego środowiska.
    Jeśli nie uruchamiasz tego w środowisku szkoleniowym, które dostarczamy, będziesz musiał zaktualizować ścieżki, aby pasowały do Twojego systemu.
...
Możemy to zrobić, używając tego samego wzorca, którego użyliśmy w [Części 2 Hello Nextflow](../../hello_nextflow/02_hello_channels.md#42-use-the-splitcsv-operator-to-parse-the-file): stosując operator [`splitCsv()`](https://nextflow.io/docs/latest/reference/operator.html#splitcsv) do parsowania pliku, a następnie operację `map`, aby wyodrębnić ścieżkę pliku z każdego wiersza.
...
Jedną rzeczą, która jest nowa w porównaniu z tym, co napotkałeś w kursie Hello Nextflow, jest to, że ten plik CSV ma linię nagłówka, więc dodajemy `#!groovy header: true` do wywołania `splitCsv()`.
To pozwala nam odwoływać się do kolumn po nazwie w operacji `map`: `#!groovy row.reads_bam` wyodrębnia ścieżkę pliku z kolumny `reads_bam` każdego wiersza.
...
Gdy poczujesz się odświeżony, przejdź do [Części 3](./03_joint_calling.md), gdzie nauczysz się, jak przekształcić ten prosty workflow wywoływania wariantów dla poszczególnych próbek, aby zastosować wspólne wywoływanie wariantów do danych.
