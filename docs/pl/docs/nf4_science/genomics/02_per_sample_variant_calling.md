...
Po ukończeniu tego proces jest kompletny.
Aby użyć go w workflow, musisz zaimportować moduł i dodać wywołanie procesu.

#### 1.2.2. Dołącz moduł

W `genomics.nf` dodaj instrukcję `include`, aby udostępnić proces dla workflow'a:
...
Jak wcześniej, dostarczamy wartości domyślne przez profil testowy, a nie bezpośrednio.
...
Musimy wymienić je jawnie, aby Nextflow umieścił je w katalogu roboczym w czasie wykonywania; w przeciwnym razie GATK zgłosi błąd dotyczący brakujących plików.
...
Powinieneś rozpoznać składnię `*.out` z serii szkoleń Hello Nextflow; mówimy Nextflow'owi, aby wziął kanał wyjściowy z `SAMTOOLS_INDEX` i podłączył go do wywołania procesu `GATK_HAPLOTYPECALLER`.
...
Wiesz, jak stworzyć dwukrokowy modularny workflow'a, który wykonuje rzeczywistą pracę analityczną i jest w stanie radzić sobie z osobliwościami formatów plików genomicznych, takimi jak pliki pomocnicze.
...
Ponownie, może się to udać lub nie. Oto jak wygląda wyjście dwóch wywołań `.view()` dla nieudanego uruchomienia:
...
Ale nie ma gwarancji, że tak będzie, dlatego czasami (choć nie zawsze) niewłaściwe pliki są łączone w pary w drugim kroku.
...
Teraz musimy zaktualizować workflow'a, aby odzwierciedlał nową strukturę krotki w wywołaniu procesu i celach publikacji.
...
Może to być tak proste, jak plik tekstowy wymieniający jedną ścieżkę pliku na linię i nic więcej, lub plik może zawierać dodatkowe metadane, w którym to przypadku często nazywany jest arkuszem próbek.
...
!!! note
    ...
    Jeśli nie używasz dostarczonego środowiska Codespaces, może być konieczne dostosowanie ścieżek plików do Twojej lokalnej konfiguracji.
...
Jednak używając bardziej wszechstronnego operatora `splitCsv` (uzupełnionego przez `map`), możemy przygotować nasz workflow'a na przyszłość na wypadek, gdybyśmy zdecydowali się dodać metadane do pliku zawierającego ścieżki plików.
...
Uruchom workflow'a jeszcze raz. Powinno to dać taki sam wynik jak wcześniej, prawda?
...
Wiesz, jak stworzyć wielokrokowy modularny workflow'a do indeksowania pliku BAM i zastosowania wywoływania wariantów dla poszczególnych próbek przy użyciu GATK.
