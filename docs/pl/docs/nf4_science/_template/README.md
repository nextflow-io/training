...
| Symbol zastępczy       | Opis                                  | Przykład (Genomics)  |
| ---------------------- | ------------------------------------- | -------------------- |
| `{PRIMARY_INPUT_TYPE}` | Podstawowy typ pliku wejściowego      | BAM file             |
| `{PRIMARY_PARAM_NAME}` | Nazwa parametru Nextflow'a            | reads_bam            |
| `{TEST_INPUT_PATH}`    | Ścieżka do wejścia testowego w data/  | bam/reads_mother.bam |
...
| `{TYPEFORM_ID}`         | ID osadzenia Typeform dla ankiety              |
