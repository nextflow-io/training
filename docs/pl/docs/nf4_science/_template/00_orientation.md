...
To środowisko szkoleniowe zawiera wszystkie niezbędne oprogramowanie, kod i dane do pracy z kursem, więc nie musisz niczego instalować samodzielnie.
...
Domyślnie codespace otwiera się z katalogiem roboczym ustawionym w katalogu głównym wszystkich kursów szkoleniowych, ale w tym kursie będziemy pracować w katalogu `nf4-science/{DOMAIN_DIR}/`.
...
W całym kursie używamy wyniku polecenia `tree` do przedstawienia struktury katalogów i zawartości w czytelnej formie, czasami z niewielkimi modyfikacjami dla przejrzystości.
...
Używamy takich zwijanych sekcji do wyświetlania oczekiwanego wyniku poleceń, a także zawartości katalogów i plików w zwięzły sposób.

- **Plik `{DOMAIN_DIR}.nf`** to skrypt workflow'a, który będziesz budować w trakcie kursu.
...
- **Katalog `solutions`** zawiera ukończone pliki modułów i rozwiązania specyficzne dla poszczególnych części, które mogą służyć jako punkt wyjścia do następnej części.
...
**Aby przejść do [Część 1: Przegląd metody](./01_method.md), kliknij strzałkę w prawym dolnym rogu tej strony.**
