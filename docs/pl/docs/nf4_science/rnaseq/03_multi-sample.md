# Część 3: Implementacja dla wielu próbek z odczytami sparowanymi

W tej ostatniej części kursu podniesiemy nasz prosty workflow na wyższy poziom, przekształcając go w potężne narzędzie do automatyzacji przetwarzania wsadowego dowolnej liczby próbek.
Przy okazji przełączymy go również na obsługę danych sparowanych, które są częstsze w nowszych badaniach.

Zrobimy to w trzech etapach:

1. Sprawimy, że workflow będzie akceptował wiele próbek wejściowych i zrównolegli wykonanie
2. Dodamy generowanie kompleksowego raportu QC
3. Przełączymy się na dane RNAseq ze sparowanymi odczytami
...
### 1.1. Zmienimy główne wejście na plik CSV ze ścieżkami plików zamiast pojedynczego pliku

W katalogu `data/` udostępniamy plik CSV zawierający identyfikatory próbek i ścieżki do plików FASTQ.
Ten plik CSV zawiera wiersz nagłówka.
...
Zmieńmy nazwę głównego parametru wejściowego na `input_csv` i ustawmy domyślną wartość na ścieżkę do pliku `single-end.csv`.
...
### 1.2. Zaktualizujemy fabrykę kanału wejściowego, aby obsługiwała CSV jako wejście

Chcemy załadować zawartość pliku do kanału zamiast samej ścieżki do pliku, więc używamy operatora `.splitCsv()` do parsowania formatu CSV, a następnie operatora `.map()`, aby pobrać konkretną informację, której potrzebujemy (ścieżkę do pliku FASTQ).
...
### 1.3. Uruchomimy workflow, aby sprawdzić, czy działa
...
Tym razem widzimy, że każdy krok jest wykonywany 6 razy, dla każdego z 6 plików danych, które podaliśmy.

To wszystko, czego potrzeba, aby workflow działał na wielu plikach!
Nextflow obsługuje za nas całą paralelizację.

---

## 2. Zagregujemy metryki QC wstępnego przetwarzania w jeden raport MultiQC

Wszystko to generuje wiele raportów QC i nie chcemy przeszukiwać poszczególnych raportów.
To idealny moment, aby dodać krok agregacji raportu MultiQC!

### 2.1. Utworzymy moduł dla procesu agregacji QC
...
### 2.2. Zaimportujemy moduł do pliku workflow'a
...
### 2.3. Dodamy parametr `report_id` i nadamy mu sensowną wartość domyślną
...
### 2.4. Wywołamy proces na wyjściach poprzednich kroków
...
Gdybyśmy mieli cztery procesy o nazwach A, B, C i D, każdy z prostym kanałem `.out`, składnia wyglądałaby tak: `A.out.mix( B.out, C.out, D.out )`. Jak widzisz, stosujesz go do pierwszego z kanałów, które chcesz połączyć (nie ma znaczenia którego) i po prostu dodajesz wszystkie pozostałe, oddzielone przecinkami, w nawiasach, które następują.

W przypadku naszego workflow'a mamy następujące wyjścia do zagregowania:
...
Więc przykład składni staje się następujący:
...
W kontekście pełnego bloku `workflow` wygląda to tak:
...
### 2.5. Uruchomimy workflow, aby sprawdzić, czy działa
...
Wyjścia znajdziesz w katalogu `results/trimming`, jak określono w procesie `TRIM_GALORE` przez dyrektywę `publishDir`.
...
## 3. Umożliwimy przetwarzanie danych RNAseq ze sparowanymi odczytami

Obecnie nasz workflow może obsługiwać tylko dane RNAseq z pojedynczymi odczytami.
Coraz częściej spotyka się dane RNAseq ze sparowanymi odczytami, więc chcemy móc je obsługiwać.

Stworzenie workflow'a całkowicie niezależnego od typu danych wymagałoby użycia nieco bardziej zaawansowanych funkcji języka Nextflow'a, więc nie zrobimy tego tutaj, ale możemy stworzyć wersję do przetwarzania sparowanych odczytów, aby zademonstrować, co należy dostosować.

### 3.1. Utworzymy kopię workflow'a o nazwie `rnaseq_pe.nf`

```bash
cp rnaseq.nf rnaseq_pe.nf
```

### 3.2. Zmodyfikujemy domyślną wartość `input_csv`, aby wskazywała na dane sparowane
...
Zmieńmy domyślną wartość `input_csv` na ścieżkę do pliku `paired-end.csv`.
...
### 3.3. Zaktualizujemy fabrykę kanału

Musimy teraz powiedzieć operatorowi `.map()`, aby pobrał obie ścieżki do plików FASTQ.
...
### 3.4. Utworzymy wersję procesu FASTQC dla sparowanych odczytów
...
- Zastąp `${reads.simpleName}` symbolem wieloznacznym (`*`), aby uniknąć konieczności indywidualnej obsługi plików wyjściowych.
...
Technicznie rzecz biorąc, to uogólnia proces `FASTQC` w sposób, który umożliwia mu obsługę zarówno danych RNAseq z pojedynczymi, jak i sparowanymi odczytami.

Na koniec zaktualizuj instrukcję importu modułu, aby używała wersji dla sparowanych odczytów.

```groovy title="rnaseq_pe.nf" linenums="4"
include { FASTQC } from './modules/fastqc_pe.nf'
```

### 3.5. Utworzymy wersję procesu TRIM_GALORE dla sparowanych odczytów
...
- Zaktualizuj deklaracje wyjść, aby odzwierciedlały dodane pliki i różne konwencje nazewnictwa, używając symboli wieloznacznych, aby uniknąć konieczności wymieniania wszystkiego.
...
### 3.6. Zaktualizujemy wywołanie procesu MULTIQC, aby oczekiwał dwóch raportów z TRIM_GALORE
...
Skoro już jesteśmy przy MultiQC, zaktualizujmy również domyślną wartość parametru `report_id` z `"all_single-end"` na `"all_paired-end"`.
...
### 3.7. Utworzymy wersję procesu HISAT2_ALIGN dla sparowanych odczytów
...
- Zastąp wszystkie wystąpienia `${reads.simpleName}` przez `${read1.simpleName}` w poleceniu w bloku `script`, a także w deklaracjach wyjść.
...
### 3.8. Uruchomimy workflow, aby sprawdzić, czy działa

Nie używamy `-resume`, ponieważ to nie zostałoby zbuforowane, a do przetworzenia jest dwa razy więcej danych niż wcześniej, ale powinno się to zakończyć w mniej niż minutę.
...
I to wszystko! Teraz mamy dwie nieco rozbieżne wersje naszego workflow'a, jedną dla danych z pojedynczymi odczytami i jedną dla danych sparowanych.
Następnym logicznym krokiem byłoby sprawienie, aby workflow akceptował dowolny typ danych w locie, co wykracza poza zakres tego kursu, ale możemy się tym zająć w kontynuacji.

---

### Podsumowanie

Wiesz, jak dostosować workflow dla pojedynczej próbki, aby zrównoleglić przetwarzanie wielu próbek, wygenerować kompleksowy raport QC i w razie potrzeby dostosować workflow do używania danych ze sparowanymi odczytami.

### Co dalej?

Gratulacje, ukończyłeś mini-kurs Nextflow For RNAseq! Świętuj swój sukces i weź zasłużoną przerwę!

Następnie prosimy Cię o wypełnienie bardzo krótkiej ankiety dotyczącej Twoich doświadczeń z tym kursem szkoleniowym, a następnie przekierujemy Cię na stronę z linkami do dalszych materiałów szkoleniowych i pomocnych odnośników.
