...
Istnieje wiele prawidłowych metod przetwarzania i analizy danych z sekwencjonowania RNA (bulk RNAseq).
...
Naszym celem jest stworzenie workflow'a, który implementuje następujące etapy przetwarzania: przeprowadzenie wstępnej kontroli jakości odczytów w próbce bulk RNAseq, przycięcie sekwencji adapterów z odczytów, dopasowanie odczytów do genomu referencyjnego oraz wygenerowanie kompleksowego raportu kontroli jakości (QC).

<figure class="excalidraw">
--8<-- "docs/en/docs/nf4_science/rnaseq/img/preprocess.svg"
</figure>

- **FASTQC:** Przeprowadzenie kontroli jakości danych odczytów przed przycięciem przy użyciu FastQC
- **TRIM_GALORE:** Przycięcie sekwencji adapterów i przeprowadzenie kontroli jakości po przycięciu przy użyciu Trim Galore (łączy Cutadapt i FastQC)
- **HISAT2_ALIGN:** Dopasowanie odczytów do genomu referencyjnego przy użyciu Hisat2
- **MULTIQC:** Wygenerowanie kompleksowego raportu kontroli jakości przy użyciu MultiQC

Zanim jednak zaczniemy pisać jakikolwiek kod workflow'a, przetestujemy polecenia ręcznie na danych testowych.
Narzędzia, których potrzebujemy, nie są zainstalowane w środowisku GitHub Codespaces, więc użyjemy ich poprzez kontenery (zobacz [Hello Containers](../../hello_nextflow/05_hello_containers.md)).
...
## 1. Wstępna kontrola jakości i przycięcie adapterów

Pobierzemy obraz kontenera, który ma zainstalowane zarówno `fastqc`, jak i `trim_galore`, uruchomimy go interaktywnie i wykonamy polecenia przycięcia oraz kontroli jakości na jednym z przykładowych plików danych.
...
Otrzymasz następujące wyjście w konsoli, gdy system pobiera obraz:
...
Twój wiersz poleceń zmieni się na coś w rodzaju `(base) root@b645838b3314:/tmp#`, co oznacza, że jesteś teraz wewnątrz kontenera.

Część polecenia `-v ./data:/data` umożliwi nam dostęp do zawartości katalogu `data/` z wnętrza kontenera.
...
Polecenie powinno wykonać się bardzo szybko.
...
### 1.4. Przycięcie sekwencji adapterów przy użyciu `trim_galore`

Teraz uruchommy `trim_galore`, który łączy Cutadapt i FastQC, aby przyciąć sekwencje adapterów i zebrać metryki kontroli jakości po przycięciu.

```bash
trim_galore --fastqc /data/reads/ENCSR000COQ1_1.fastq.gz
```

Flaga `--fastqc` powoduje, że polecenie automatycznie uruchamia etap zbierania danych kontroli jakości po zakończeniu przycięcia.

_Wyjście jest bardzo obszerne, więc poniżej znajduje się skrócona wersja._
...
Hisat2 wymaga, aby genom referencyjny był dostarczony w bardzo specyficznym formacie i nie może po prostu wykorzystać pliku FASTA `genome.fa`, który udostępniamy, więc skorzystamy z tej okazji, aby utworzyć odpowiednie zasoby.

```bash
hisat2-build /data/genome.fa genome_index
```

Wyjście jest bardzo obszerne, więc poniżej znajduje się skrócona wersja:
...
Tworzy to wiele plików indeksu genomu, które znajdziesz w katalogu roboczym.
...
Użyjemy ich za chwilę, ale najpierw wygenerujmy skompresowane archiwum tar z tymi plikami indeksu genomu; będziemy ich potrzebować później, a generowanie ich nie jest zazwyczaj czymś, co chcemy robić w ramach workflow'a.

```bash
tar -czvf /data/genome_index.tar.gz genome_index.*
```

Zapisuje to archiwum tar `genome_index.tar.gz` zawierające pliki indeksu genomu w katalogu `data/` w naszym systemie plików, co przyda się w części 2 tego szkolenia.

### 2.4. Uruchomienie polecenia `hisat2`

Teraz możemy uruchomić polecenie dopasowania, które wykonuje etap dopasowania przy użyciu `hisat2`, a następnie przekazuje wyjście do `samtools`, aby zapisać wynik jako plik BAM.

Danymi wejściowymi odczytów jest plik `/data/trimmed/ENCSR000COQ1_1_trimmed.fq.gz`, który wygenerowaliśmy przy użyciu `trim_galore` w poprzednim kroku.
...
Wykonuje się niemal natychmiast, ponieważ jest to bardzo mały plik testowy.
W rzeczywistej skali mogłoby to potrwać znacznie dłużej.
...
## 3. Wygenerowanie kompleksowego raportu kontroli jakości

Pobierzemy obraz kontenera, który ma zainstalowany `multiqc`, uruchomimy go interaktywnie i wykonamy polecenie generowania raportu na plikach raportów FastQC przed i po przycięciu.
...
MultiQC potrafi przeszukiwać katalogi w poszukiwaniu kompatybilnych raportów kontroli jakości i agreguje wszystko, co znajdzie.

Tutaj widzimy, że narzędzie znalazło wszystkie trzy raporty kontroli jakości, które wygenerowaliśmy: wstępną kontrolę jakości wykonaną przy użyciu `fastqc`, raport po przycięciu z `cutadapt` (wykonany przez `trim_galore`) oraz kontrolę jakości po dopasowaniu wygenerowaną przez `hisat2`.

Pliki wyjściowe znajdują się ponownie w katalogu roboczym:
...
Dowiedz się, jak opakować te same polecenia w wieloetapowy workflow, który wykorzystuje kontenery do wykonywania pracy.
