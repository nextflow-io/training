...
Wyniki znajdziesz w katalogu `results/fastqc`, jak określono w procesie `FASTQC` przez dyrektywę `publishDir`.
...
Wyniki znajdziesz w katalogu `results/trimming`, jak określono w procesie `TRIM_GALORE` przez dyrektywę `publishDir`.
...
Wyniki znajdziesz w katalogu `results/align`, jak określono w procesie `HISAT2_ALIGN` przez dyrektywę `publishDir`.
