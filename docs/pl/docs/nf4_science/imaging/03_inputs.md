# Część 3: Organizacja danych wejściowych

W Części 2 uruchomiliśmy molkart z wieloma parametrami w wierszu poleceń.
Teraz poznamy dwa lepsze podejścia do zarządzania danymi wejściowymi: **pliki parametrów** i **arkusze próbek**.

## 1. Używanie plików parametrów

### 1.1. Problem z długimi wierszami poleceń

Przypomnij sobie nasze polecenie z Części 2:
...
Co jeśli będziesz musiał uruchomić tę samą analizę ponownie za miesiąc?
Co jeśli współpracownik będzie chciał użyć dokładnie Twoich ustawień?
...
To wszystko! Plik parametrów dokumentuje Twoją dokładną konfigurację i ułatwia ponowne uruchomienie lub udostępnienie.
...
Powyższa linia zmienia `segmentation_method` na `stardist`, a nazwę `--outdir` na `stardist_results` zamiast parametrów z pliku `params.yaml`.
Dodatkowo możesz zauważyć, że flaga `-resume` pozwoliła nam ponownie wykorzystać wyniki wstępnego przetwarzania z poprzedniego uruchomienia, oszczędzając czas.
Możesz użyć tego wzorca, aby szybko przetestować różne warianty pipeline'u.
...
- **URL-e**: Nextflow pobiera automatycznie (jak pokazano powyżej)
...
!!! warning "Ostrzeżenie"
    ...
    Zauważ, że ścieżki w arkuszu próbek są względne do miejsca, w którym **uruchamiasz** Nextflow'a, a nie do miejsca, w którym znajduje się arkusz próbek.

Na koniec uruchommy nf-core/molkart jeszcze raz z arkuszem próbek zawierającym lokalne ścieżki plików:

`nextflow run ./molkart -params-file params.yaml -resume`

Jak widzisz, Nextflow wykonuje to uruchomienie podobnie jak wtedy, gdy pliki były pobierane z Github. To jedna z wspaniałych cech Nextflow'a — prawidłowo przygotowuje dane dla Ciebie, niezależnie od tego, gdzie się znajdują.

### Podsumowanie

Arkusze próbek organizują zestawy danych z wieloma próbkami w sposób, który pozwala jawnie zdefiniować metadane wraz ze ścieżkami plików.
