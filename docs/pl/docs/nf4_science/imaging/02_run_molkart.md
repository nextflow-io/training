...
Wzorce Nextflow'a, których się tutaj nauczysz, mają jednak zastosowanie do każdego pipeline'u nf-core i każdego workflow'u produkcyjnego.
...
Wszystkie pipeline'y nf-core przestrzegają tej samej struktury i konwencji, co oznacza, że nauczywszy się uruchamiać jeden, możesz uruchomić każdy z nich.
...
!!! tip "Chcesz dowiedzieć się więcej o nf-core?"
    ...
    Obejmuje on to, jak tworzyć i dostosowywać pipeline'y nf-core od podstaw.
...
3. **Przypisanie punktów**: Przypisywanie punktów transkryptu do zsegmentowanych komórek
...
!!! note "Dlaczego klonujemy lokalnie?"
    ...
    Dla celów tego szkolenia klonujemy jednak pipeline do innego katalogu lokalnego, abyśmy mogli łatwiej sprawdzić kod.
...
Ta linia informuje Nextflow'a: "Aby uruchomić ten proces, użyj obrazu Docker `ghcr.io/schapirolabor/molkart-local:v0.0.4`, który zawiera całe wymagane oprogramowanie."

Każdy proces deklaruje, który obraz kontenera dostarcza wymagane narzędzia.
Nextflow używa jednak tych kontenerów tylko jeśli mu powiesz!
