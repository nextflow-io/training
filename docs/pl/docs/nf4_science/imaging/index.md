...
Uczy podstawowych koncepcji Nextflow związanych z uruchamianiem, organizowaniem i konfigurowaniem workflow'ów przy użyciu [nf-core/molkart](https://nf-co.re/molkart), pipeline'u do przetwarzania danych transkryptomiki przestrzennej Molecular Cartography.
...
Pracując nad tym kursem, nauczysz się stosować podstawowe koncepcje i narzędzia Nextflow do uruchamiania pipeline'ów analizy obrazowania.

Pod koniec tego warsztatu będziesz w stanie:

- Uruchomić workflow Nextflow'a lokalnie i monitorować jego wykonywanie
...
Ten kurs zakłada minimalną znajomość następujących zagadnień:

- Doświadczenie z wierszem poleceń
- Podstawowa znajomość formatów plików obrazowych (obrazy TIFF, dane tabelaryczne)

Wymagania techniczne i konfigurację środowiska znajdziesz w mini-kursie [Environment Setup](../../envsetup/).
