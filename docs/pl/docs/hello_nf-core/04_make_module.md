...
!!! warning "Katalog roboczy"
    ...
    Upewnij się, że jesteś w katalogu `core-hello` (głównym katalogu Twojego pipeline'u) dla wszystkich edycji plików i wykonywania poleceń w tej sekcji.
...
2. Poprawić instrukcję importu modułu w nagłówku workflow'u
3. Dostosować wywołanie procesu i deklarację emit w treści workflow'u
...
#### 1.1.4. Uruchomienie pipeline'u w celu przetestowania
...
W obecnej wersji pipeline'u `core-hello` wyodrębniamy plik z krotki wyjściowej `CAT_CAT`, aby przekazać go do `COWPY`, jak pokazano w górnej części diagramu poniżej.
...
2. Poprawić wywołanie procesu w workflow'ie
3. Dostosować blok emit w workflow'ie
...
Otwórz plik workflow'u `hello.nf` (w `core-hello/workflows/`) i zaktualizuj wywołanie `COWPY`, jak pokazano poniżej.
...
Ponieważ `COWPY` teraz emituje nazwane wyjście `cowpy_output`, możemy zaktualizować blok `emit:` workflow'u `hello.nf`, aby z niego korzystał.
...
#### 1.2.4. Uruchomienie pipeline'u w celu przetestowania
...
Zauważ użycie nawiasów klamrowych, które mówią Nextflow'owi, aby ocenił wartość parametru w czasie wykonywania.
...
#### 1.3.3. Aktualizacja workflow'u `hello.nf`

Ponieważ moduł `COWPY` nie wymaga już parametru `character` jako wejścia, musimy odpowiednio zaktualizować wywołanie workflow'u.
...
Kod workflow'u jest teraz czystszy: nie musimy przekazywać `params.character` bezpośrednio do procesu.
...
#### 1.3.4. Uruchomienie pipeline'u w celu przetestowania
...
Zauważ, że ponownie używamy nawiasów klamrowych, tym razem aby powiedzieć Nextflow'owi, aby ocenił wartość `meta.id` w czasie wykonywania.
...
#### 1.4.3. Uruchomienie pipeline'u w celu przetestowania
...
Możesz zmienić konfigurację `ext.prefix` w `conf/modules.config`, aby upewnić się, że możesz zmienić wzorzec nazewnictwa bez konieczności wprowadzania jakichkolwiek zmian w module lub kodzie workflow'u.
...
Chcemy mieć możliwość kontrolowania, gdzie publikowane są wyjścia na poziomie konfiguracji workflow'u.
...
#### 1.5.2. Uruchomienie pipeline'u w celu przetestowania
...
W kontekście Nextflow blok [stub](https://www.nextflow.io/docs/latest/process.html#stub) pozwala zdefiniować lekki, fikcyjny skrypt używany do szybkiego prototypowania i testowania logiki pipeline'u bez wykonywania rzeczywistego polecenia.
...
To pozwala testować logikę workflow'u i obsługę plików bez czekania na uruchomienie rzeczywistego narzędzia.
...
### 2.4. Wymiana nowego modułu `COWPY` i uruchomienie pipeline'u

Wszystko, co musimy zrobić, aby wypróbować tę nową wersję modułu `COWPY`, to przełączyć instrukcję importu w pliku workflow'u `hello.nf`, aby wskazywała na nowy plik.
