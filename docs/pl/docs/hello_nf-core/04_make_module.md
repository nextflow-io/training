...
Projekt nf-core udostępnia polecenie (`nf-core modules create`), które automatycznie generuje prawidłowo ustrukturyzowane szablony modułów, podobnie jak to, czego użyliśmy dla workflow w Części 2.
Jednak w celach edukacyjnych zaczniemy od wykonania tego ręcznie: przekształcenia lokalnego modułu `cowpy` w Twoim pipeline `core-hello` w moduł w stylu nf-core krok po kroku.
...
??? info "Jak rozpocząć od tej sekcji"
    ...
    Ta sekcja zakłada, że ukończyłeś [Część 3: Użycie modułu nf-core](./03_use_module.md) i zintegrowałeś moduł `CAT_CAT` ze swoim pipeline.
...
W tej sekcji zastosujemy konwencje nf-core do lokalnego modułu `cowpy` w Twoim pipeline `core-hello`, przekształcając go w moduł zgodny ze standardami społeczności nf-core.
...
Nazwy procesów są wrażliwe na wielkość liter, więc teraz, gdy zmieniliśmy nazwę procesu, musimy odpowiednio zaktualizować instrukcję importu modułu w nagłówku workflow pliku `hello.nf`:
...
Teraz zaktualizujmy dwa odwołania do procesu w bloku workflow pliku `hello.nf`:
...
#### 1.1.4. Uruchomienie pipeline w celu przetestowania
...
#### 1.2.4. Uruchomienie pipeline w celu przetestowania
...
Miejmy nadzieję, że możesz sobie wyobrazić, że wszystkie moduły w pipeline mają swoje `ext.args` określone w tym pliku, z następującymi korzyściami:
...
- Konfiguracja jest **scentralizowana** w `modules.config`, utrzymując logikę workflow czystą
...
#### 1.3.4. Uruchomienie pipeline w celu przetestowania
...
- **Brak zmian w workflow**: Dodawanie lub zmiana opcji narzędzia nie wymaga aktualizacji kodu workflow
...
(Nie są potrzebne zmiany w workflow.)
...
#### 1.4.3. Uruchomienie pipeline w celu przetestowania
...
To dlatego, że pipeline nf-core centralizują kontrolę na poziomie workflow, konfigurując `publishDir` w `conf/modules.config` zamiast w poszczególnych modułach.
...
#### 1.5.2. Uruchomienie pipeline w celu przetestowania
...
Widać, że Nextflow utworzył tę hierarchię katalogów na podstawie nazw workflow i modułu.
...
- **`path:`** Określa katalog wyjściowy na podstawie nazwy procesu.
  Pełna nazwa procesu zawarta w `task.process` obejmuje hierarchię importów workflow i modułów (takich jak `CORE_HELLO:HELLO:CAT_CAT`).
...
Wyjście wygląda jeszcze lepiej, gdy wszystkie moduły w pipeline przyjmą tę konwencję, więc śmiało usuń dyrektywy `publishDir` z innych modułów w swoim pipeline.
