...
W czwartej części kursu szkoleniowego Hello nf-core pokażemy, jak utworzyć moduł nf-core, stosując kluczowe konwencje czyniące takie komponenty przenośnymi i łatwymi w utrzymaniu.
...
Jednak w celach edukacyjnych zaczniemy od wykonania tego ręcznie: przekształcimy lokalny moduł `cowpy` w Twoim pipeline'ie `core-hello` w moduł w stylu nf-core krok po kroku.
...
Moglibyśmy użyć aliasu w instrukcji importu, aby uniknąć konieczności aktualizacji wywołań procesu, ale nieco zniweczyłoby to sens przyjęcia konwencji pisania wielkimi literami.
...
Świetnie, to działa! Teraz przejdźmy do wprowadzania bardziej znaczących zmian.
...
Dowiedz się, jak używać wbudowanych narzędzi nf-core opartych na szablonach do tworzenia modułów w łatwy sposób.
...
### 2.1. Wygenerowanie szkieletu modułu na podstawie szablonu

Podobnie jak istnieje dla tworzenia pipeline, projekt nf-core udostępnia narzędzia do generowania poprawnie ustrukturyzowanych modułów na podstawie szablonu, z wszystkimi tymi wzorcami wbudowanymi od początku.
...
Utwórzmy nową wersję modułu `COWPY` z minimalnym szablonem, uruchamiając to polecenie:
...
- **Informacje o autorze**: Twoja nazwa użytkownika GitHub dla atrybucji
...
- **Wymóg metadanych**: Czy moduł potrzebuje informacji specyficznych dla próbki poprzez mapę `meta` (zazwyczaj tak dla modułów przetwarzania danych).

Narzędzie obsługuje złożoność znajdowania informacji o pakiecie i konfigurowania struktury, pozwalając Ci skupić się na implementacji specyficznej logiki narzędzia.

#### 2.1.2. Zbadanie szkieletu modułu
...
!!! tip "Dowiedz się więcej o testowaniu"
    ...
    Wygenerowany plik testowy używa nf-test, frameworka testowego dla pipeline i modułów Nextflow. Aby dowiedzieć się, jak pisać i uruchamiać te testy, zobacz [zadanie poboczne nf-test](../side_quests/nf-test.md).

Wygenerowany `main.nf` zawiera wszystkie wzorce, których się właśnie nauczyłeś, plus niektóre dodatkowe funkcje:
...
- **`when:` blok**: Umożliwia warunkowe wykonanie poprzez konfigurację `task.ext.when`
...
- **Blok `stub:`**: Szablon, który zaktualizujemy, aby produkował prawidłowe wyjścia
- **Kontener i środowisko**: Symbole zastępcze, które wypełnimy informacjami o pakiecie

Następne sekcje przeprowadzą przez ukończenie tych dostosowań.
...
Dla kontenera możesz użyć [Seqera Containers](https://seqera.io/containers/) do automatycznego budowania kontenera z dowolnego pakietu Conda, w tym pakietów conda-forge.
W tym przypadku używamy tego samego wcześniej zbudowanego kontenera co wcześniej.
...
Narzędzie do tworzenia modułu ostrzegło nas, że nie mogło znaleźć pakietu `cowpy` w Bioconda (głównym kanale dla narzędzi bioinformatycznych).
...
W przypadku wysłania do nf-core musielibyśmy bardziej ściśle przestrzegać ustawień domyślnych, ale dla naszego własnego użytku możemy uprościć kod w ten sposób.

!!! tip "Pakiety Bioconda vs conda-forge"

    - **Pakiety Bioconda**: Automatycznie budowane są BioContainers, dostarczające gotowe do użycia kontenery
...
Teraz zaktualizujmy elementy kodu specyficzne dla tego, co robi proces `COWPY`: wejścia i wyjścia oraz blok skryptu.
...
Jeśli używasz serwera języka Nextflow do walidacji składni, część `${prefix}` zostanie oznaczona jako błąd na tym etapie, ponieważ nie dodaliśmy jej jeszcze do bloku skryptu.
Przejdźmy do tego teraz.

#### 2.3.2. Blok skryptu

Szablon zapewnia symbol zastępczy komentarza w bloku skryptu, w którym powinieneś dodać właściwe polecenie narzędzia.
...
- Zmień `def prefix` na samo `prefix` (bez `def`), aby uczynić go dostępnym w bloku wyjścia
- Zastąp komentarz właściwym poleceniem `cowpy`, które używa zarówno `$args`, jak i `${prefix}.txt`

Zauważ, że gdybyśmy nie wykonali już pracy nad dodaniem konfiguracji `ext.args` i `ext.prefix` dla procesu `COWPY` do pliku `modules.config`, musielibyśmy to teraz zrobić.

#### 2.3.3. Implementacja bloku stub

W kontekście Nextflow blok [stub](https://www.nextflow.io/docs/latest/process.html#stub) pozwala zdefiniować lekki, fikcyjny skrypt używany do szybkiego prototypowania i testowania logiki pipeline'u bez wykonywania właściwego polecenia.

Nie martw się zbytnio, jeśli wydaje się to tajemnicze; włączamy to dla kompletności, ale możesz również po prostu usunąć sekcję stub, jeśli nie chcesz się tym zajmować, ponieważ jest całkowicie opcjonalna.
...
- Zmień `def prefix` na samo `prefix`, aby dopasować blok skryptu
- Usuń linię `echo $args` (która była tylko kodem symbolu zastępczego szablonu)
- Stub tworzy pusty plik `${prefix}.txt` pasujący do tego, co produkuje blok skryptu

To pozwala testować logikę workflow'u i obsługę plików bez czekania na faktyczne uruchomienie narzędzia.
...
### 2.4. Zamiana na nowy moduł `COWPY` i uruchomienie pipeline'u
...
Dowiedz się, jakie są korzyści z wnoszenia modułów do nf-core oraz jakie są główne kroki i wymagania.
...
Wnoszenie Twoich modułów do nf-core:
...
### 3.2. Lista kontrolna współpracownika
...
2. Sforkuj repozytorium [nf-core/modules](https://github.com/nf-core/modules)
3. Użyj `nf-core modules create` do wygenerowania szablonu
...
Szczegółowe instrukcje znajdziesz w [tutorialu komponentów nf-core](https://nf-co.re/docs/tutorials/nf-core_components/components).

### 3.3. Zasoby

- **Tutorial komponentów**: [Kompletny przewodnik po tworzeniu i wnoszeniu modułów](https://nf-co.re/docs/tutorials/nf-core_components/components)
...
- **Scentralizowane publikowanie** poprzez `publishDir` skonfigurowany w `modules.config` zamiast zakodowany na stałe w modułach

Przekształcając `COWPY` krok po kroku, rozwinąłeś głębokie zrozumienie tych wzorców, co czyni Cię przygotowanym do pracy z, debugowania i tworzenia modułów nf-core.
W praktyce będziesz używać `nf-core modules create` do generowania poprawnie ustrukturyzowanych modułów z tymi wzorcami wbudowanymi od początku.

Na koniec nauczyłeś się, jak wnosić moduły do społeczności nf-core, udostępniając narzędzia badaczom na całym świecie, jednocześnie korzystając z ciągłego utrzymania społeczności.

### Co dalej?

Gdy będziesz gotowy, kontynuuj do [Części 5: Walidacja wejścia](./05_input_validation.md), aby nauczyć się, jak dodać walidację wejścia opartą na schemacie do Twojego pipeline'u.
