...
Teraz, gdy poznałeś wzorce modułów nf-core stosując je ręcznie, spójrzmy, jak tworzyłbyś moduły w praktyce.

### 2.1. Wygenerowanie szkieletu modułu z szablonu

Podobnie jak dla tworzenia pipeline, projekt nf-core udostępnia narzędzia do generowania prawidłowo ustrukturyzowanych modułów na podstawie szablonu, ze wszystkimi wzorcami wbudowanymi od samego początku.

#### 2.1.1. Uruchomienie polecenia tworzenia modułu

Polecenie `nf-core modules create` generuje szablon modułu, który już od początku stosuje wszystkie konwencje, których się nauczyłeś.
...
Zostaniesz poproszony o kilka opcji konfiguracji:

- **Informacje o autorze**: Twoja nazwa użytkownika GitHub do przypisania autorstwa
- **Etykieta zasobów**: Predefiniowany zestaw wymagań obliczeniowych.
...
- **Wymaganie metadanych**: Czy moduł potrzebuje informacji specyficznych dla próbki poprzez `meta` map (zazwyczaj tak dla modułów przetwarzania danych).
...
#### 2.1.2. Przejrzenie szkieletu modułu
...
!!! tip "Dowiedz się więcej o testowaniu"
    ...
    Wygenerowany plik testowy używa nf-test, frameworku testowego dla pipeline i modułów Nextflow. Aby dowiedzieć się, jak pisać i uruchamiać te testy, zobacz [side quest nf-test](../side_quests/nf-test.md).

Wygenerowany plik `main.nf` zawiera wszystkie wzorce, których właśnie się nauczyłeś, plus kilka dodatkowych funkcji:
...
process COWPY {
    ...
    """
    // Dodaj tutaj swoje polecenie narzędzia
...
- **`label 'process_single'`**: Etykieta zasobów do konfiguracji wymagań CPU/pamięci
- **`when:` blok**: Umożliwia warunkowe wykonywanie poprzez konfigurację `task.ext.when`
...
W tym przypadku używamy tego samego prekonstruowanego kontenera co wcześniej.

Domyślny kod oferuje przełączanie między Docker a Singularity, ale uprascimy tę linię i po prostu określimy kontener Docker, który otrzymaliśmy z Seqera Containers powyżej.
...
Dla środowiska Conda kod modułu określa `conda "${moduleDir}/environment.yml"`, co oznacza, że powinno być ono skonfigurowane w pliku `environment.yml`.

Narzędzie tworzenia modułu ostrzegło nas, że nie mogło znaleźć pakietu `cowpy` w Bioconda (głównym kanale dla narzędzi bioinformatycznych).
Jednak `cowpy` jest dostępny w conda-forge, więc możesz uzupełnić plik `environment.yml` w ten sposób:
...
=== "Przed zmianami"
    ...
    channels:
      ...
      - bioconda
    dependencies:
      # TODO nf-core: Wypisz wymagane pakiety Conda.
      #               Oprogramowanie MUSI być przypięte do kanału (np. "bioconda"), wersji (np. "1.10").
      #               Dla Condy kompilacja (np. "h9402c20_2") MUSI być WYKLUCZONA, aby wspierać instalację na różnych systemach operacyjnych.
      - "YOUR-TOOL-HERE"
    ```

Do przesłania do nf-core musielibyśmy bardziej ściśle trzymać się wartości domyślnych, ale dla naszego własnego użytku możemy uprościć kod w ten sposób.

!!! tip "Pakiety Bioconda vs conda-forge"

    - **Pakiety Bioconda**: Automatycznie otrzymują zbudowane BioContainers, dostarczając gotowe do użycia kontenery
    - **Pakiety conda-forge**: Mogą używać Seqera Containers do budowania kontenerów na żądanie z przepisu Conda

    Większość narzędzi bioinformatycznych jest w Bioconda, ale dla narzędzi conda-forge Seqera Containers zapewnia łatwe rozwiązanie dla konteneryzacji.

### 2.3. Wstawienie logiki `COWPY`
...
Patrząc wstecz na nasz ręczny moduł `COWPY` z sekcji 1, możemy go użyć jako przewodnika.
...
- Nazwę pliku wyjściowego używając konfigurowalnego wzorca prefixu (`${prefix}.txt` zamiast wieloznacznego `*`)
- Opisową nazwę emit (`cowpy_output` zamiast ogólnego `output`)

Jeśli używasz serwera językowego Nextflow do walidacji składni, część `${prefix}` zostanie oflagowana jako błąd na tym etapie, ponieważ jeszcze nie dodaliśmy jej do bloku skryptu.
Przejdźmy do tego teraz.

#### 2.3.2. Blok skryptu

Szablon zawiera symbol zastępczy komentarza w bloku skryptu, gdzie powinieneś dodać faktyczne polecenie narzędzia.
...
W kontekście Nextflow blok [stub](https://www.nextflow.io/docs/latest/process.html#stub) pozwala zdefiniować lekki, fikcyjny skrypt używany do szybkiego prototypowania i testowania logiki pipeline bez wykonywania faktycznego polecenia.

Nie martw się zbytnio, jeśli to wydaje się tajemnicze; dołączamy to dla kompletności, ale możesz też po prostu usunąć sekcję stub, jeśli nie chcesz się tym zajmować, ponieważ jest całkowicie opcjonalna.
...
- Zmień `def prefix` na po prostu `prefix`, aby dopasować blok skryptu
...
To pozwala testować logikę workflow i obsługę plików bez czekania na faktyczne uruchomienie narzędzia.

Po ukończeniu konfiguracji środowiska (sekcja 2.2), wejść/wyjść (sekcja 2.3.1), bloku skryptu (sekcja 2.3.2) i bloku stub (sekcja 2.3.3), moduł jest gotowy do przetestowania!

### 2.4. Zamiana nowego modułu `COWPY` i uruchomienie pipeline
...
Daje to takie same wyniki jak wcześniej.
...
- **Tutorial komponentów**: [Kompletny przewodnik tworzenia i wnoszenia modułów](https://nf-co.re/docs/tutorials/nf-core_components/components)
...
- **`ext.args`** upraszcza interfejsy modułów poprzez obsługę opcjonalnych argumentów przez konfigurację
- **`ext.prefix`** standaryzuje nazewnictwo plików wyjściowych
- **Scentralizowane publikowanie** poprzez `publishDir` skonfigurowany w `modules.config` zamiast zakodowanego na stałe w modułach

Przekształcając `COWPY` krok po kroku, rozwinąłeś głębokie zrozumienie tych wzorców, czyniąc Cię wyposażonym do pracy z, debugowania i tworzenia modułów nf-core.
W praktyce będziesz używać `nf-core modules create` do generowania prawidłowo ustrukturyzowanych modułów z tymi wzorcami wbudowanymi od samego początku.
...
Gdy będziesz gotowy, przejdź do [Części 5: Walidacja wejścia](./05_input_validation.md), aby dowiedzieć się, jak dodać walidację wejścia opartą na schemacie do Twojego pipeline.
