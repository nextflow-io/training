...
- **Scope the mandatory verification gate to fixes that change lesson meaning** - a new or reworded explanation, a behavior claim, pedagogical framing, or where an explanation is placed. A fix that's mechanically checkable (a `grep` for a banned pattern returns zero matches, an `hl_lines` count is arithmetically correct, a hash-consistency check passes) is just as reliably confirmed by the coordinator rerunning that exact check directly, and doesn't need a second subagent. Reserve the fresh-subagent gate below for fixes where a diff that merely looks plausible could still be wrong in a way no single rerun would surface.
- **For those fixes, verify independently before the fix reaches Phase 4's PR workflow.** The subagent that found a discrepancy and drafted the fix is not the one that confirms the fix is right - it has already committed to its own diagnosis. Spawn a second, fresh subagent with no memory of the walkthrough's reasoning, but give it enough to actually test the diagnosis rather than just judge the diff plausible: the lesson section text, the proposed diff, the original discrepancy, the observed command output or file contents that exposed it, and instructions to rerun or reproduce the smallest relevant check. Its report must state a verdict, the evidence it checked, and any remaining uncertainty - not a bare pass/fail. Only bring a fix into Phase 4 once this independent check passes. See [Phase 4](#phase-4-propose-fixes-and-create-pr-if-issues-found).
- **This gate is separate from the fixer's own sanity check.** The subagent applying a fix should still confirm its own change works (rerun the pipeline, recheck any `hl_lines` it touched) before handing off, but that self-check is necessary, not sufficient. It doesn't substitute for the independent gate above, which exists because a subagent re-checking its own diagnosis shares the same blind spot that produced it.
- **Check a fix for completeness, not just correctness.** A fix that resolves the specific instance cited in a discrepancy report can still leave the rest of the same problem in place - for example, a style fix applied only to the lines a review happened to quote, while the same pattern recurs elsewhere in the file. Before passing a fix, re-derive the full set of instances (grep the file for the pattern the fix targets, or the equivalent) and confirm nothing of the same kind remains unfixed.
