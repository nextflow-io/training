...
NOTE: Make sure that your modules.nf file is the one containing the `emit` on the `CONVETTOUPPER` process. Alternatively, you could remove the definition in `CONVERTTOUPPER.out.view{ it }`.
