...
2. *Input* defines the expected input file/s, path/s, value/s.

3. *Output* defines the expected output file/s, path/s, value/s.
...
To run the above script, you need to point to a valid input file, e.g.:

  nextflow run scripts/gzip.nf --file2compress /workspace/nf-training-public/nf-training/dsl2/data/prot/BB11001.fa

To try the other compressor, change the second flag:

  nextflow run scripts/gzip.nf --file2compress /workspace/nf-training-public/nf-training/dsl2/data/prot/BB11001.fa --compress bzip2
