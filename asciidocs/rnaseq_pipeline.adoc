...
Plus, add the input channel definition and the workflow scope:

[source,nextflow,linenums]
----
in_channel=Channel.from(params.transcriptome)
...
This process takes the transcriptome params file as `input` and activates this in the input channel (in_channel), then calls the index process (using the `salmon` tool) on this input channel, to create the salmon indexed transcriptome `salmon_index`. This is passed to the `index_ch` `channel`.
...
`script4.nf` adds the `quantification` process and new workflow scope line.
