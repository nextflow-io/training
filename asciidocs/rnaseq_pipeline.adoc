...
This process takes a path as `input` that we label `transcriptome`, then we create the transcriptome index by using the `salmon` tool (in the later `script` section). The `output` is specified as a path to a output file called `salmon_index`.
...
The execution will fail because we have not included a workflow. So far we have only paramaterised the inputs and defined a single process defintion, yet have not called the process in a workflow.

Add the following to the end of your script2.nf, or open scrip2b.nf:
