...
Where the standard output shows the Nextflow version executed, the script name and the version names, 
followed by the executor used (in our case local).
Then, you can see that the first process is executed once (1), and the second
twice (2). Followed by the printed result string.
