...
* *4*: Intitalise a Channel `greeting_ch` which contains the value of `params.greeting`.

* *6*: Befin the process definition for the `splitLetters` process.

Our first process is "splitLetters". In this code block, we specify an input, expected output and the script to execute (shown between pair of the three double quote marks).
