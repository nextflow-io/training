...
Next, two processes are called. The first splits the input string into chunks 
of 6 characters ("Hello " and "world!") and saves these words to two files 
(chunk_aa and chunk_ab). 
The second receives the output files of the first process (chunk_*) and 
splits the two files into separate items (using .flatten), then the two 
words are separately converted to uppercase letters. 
The resulting two strings are emitted on the `result` channel and the 
final output is printed using the `view` operator (appended onto the channel name).
