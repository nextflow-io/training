...
This script first defines a parameter called "greeting", 
...
containing chunks of 6 characters ("Hello " and "world!"). 
The second receives the output of the first process (chunk_*) and splits the two files into separate items (using .flatten). These two words are then converted to uppercase letters in the second process. 
The resulting two strings are emitted on the `result` channel and the final output is printed using the `view` operator (appended onto the channel name).
...
[warm up] executor > local
executor >  local (2)
[19/c2f873] process > splitWords     [100%] 1 of 1, cached: 1 ✔
[a7/a410d3] process > convertToUpper [100%] 2 of 2 ✔
olleH
!dlrow
----

You will see that the execution of the process `splitWords` is
...
[warm up] executor > local
executor >  local (4)
[48/e8315b] process > splitWords   [100%] 1 of 1 ✔
