...
of 6 characters ("Hello " and "world!") and saves these words to two files 
(chunk_aa and chunk_ab) into the `letters_ch`. 
...
words are separately converted to uppercase letters. The resulting two strings are emitted from the standant output (stdout) into the `result_ch` channel.
