...
Then use your favorite editor (e.g. `vim` or `nano`) to create a file named `Dockerfile` and copy the
following content:
...
to use instead of having to specify it as a command-line argument every time.
...
An easier way to create a Singularity container without requiring sudo permission and
