...
=== Create a conda environment with Docker

Another way to build conda environments is through a Dockerfile. 

This saves having to build an environment each time you want to use one (as outlined in previous sections).

To do this you require a Dockerfile, such as the one in this public repo: https://github.com/nextflow-io/rnaseq-nf/blob/master/docker/Dockerfile-mamba[rnaseq-nf]:
...
Try to run the RNA-Seq experiment from earlier (script2.nf), but start by building your own conda `Dockerfile`. 
...
TIP: "my-image" could be any name you choose. Just choose something memorable!
