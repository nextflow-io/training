...
Contrary to other registries that will pull the latest image when no tag (version) is provided, you must specify a tag when pulling BioContainers (after the semi colon `:` in the image name). Check the tags within the registry and pick the one that better suits your needs.
