...
=== Using Docker to pull a conda environment

Another way to operate with conda is by specifying it with a Dockerfile. This saves having to do build the conda environment within your terminal each time you want to use it (see the steps in previous section).


To do this you can simply add the following lines to your Dockerfile.

See the following example from https://github.com/nextflow-io/rnaseq-nf[rnaseq-nf]:

```
FROM continuumio/miniconda:4.7.12  // <1>
MAINTAINER Your Name <You email address>  // <2>

RUN apt-get -y install ttf-dejavu   // <3>

COPY conda.yml .                    // <4>
RUN \                               // <5>
  conda env update -n root -f conda.yml \   // <6>
  && conda clean -a                  // <7>

```

<1> Choice of parent image from which you build the Docker container.
<2> Write your information here.
<3> This installs a dependecy "ttf-dejavu".
<4> This instructs the container to find the conda.yml required in cwd.
<5> RUN the following code block.
<6> Tell conda to update the environment using the conda.yml file.
<7> Removes unused conda packages and caches.

Also, you will need to make sure you have the conda.yml file in the current directory too:
...
Try to run the RNA-Seq run you did in the third section of this document, but start by building your own conda dockerfile. 

IMPORTANT: Building a Docker container can take up to ten minutes, so please head to the next section while this runs.

.For the brief overview of step to take, click here:
[%collapsible]
====
1. Make a Dockerfile in the current directory (with above code).
...
```
process.container = 'yourid/image_name_chosen'
```
