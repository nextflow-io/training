...
So far, we've seen how to install packages with the conda and micromamba tools, both locally and within containers. With BioContainers, you don't need to create your own container image for the tools you want, and you don't need either to use conda or micromamba to install the packages. It already provides you with a Docker image containing what you want installed. Let's say you want a Docker image with fastqc installed. You'd have to create a Dockerfile installing the fastqc package with conda, for example, like below:

```
conda install -c conda-forge -c bioconda fastqc==0.11.5
```

Then build your container image, and push it to Docker Hub so that others can use. Or.. you could just get the image from BioContainers :)
...
You can check the registry for the packages you want in https://biocontainers.pro/registry[BioContainers official website]. Differently from other registries that will pull the latest image when no tag is provided, you must mention a tag for BioContainers (the part after the : in the image name). Check it over there and pick the tag that better suits your needs.

[discrete]
=== Exercise

If you remember well, script2.nf contains a single process to create an index with the salmon tool. Because we do not have salmon installed locally in this machine, we had to either run it with -with-conda or -with-docker. Your task now is to run it again with Docker, but without having to create your own Docker container image. Instead, use the BioContainers image for salmon 1.7.0.
...
Change the process blocks in script5.nf to make them automatically use BioContainers for salmon when it's the case, or fastqc otherwise. A suggestion is to temporarily comment the line `docker.enabled = true` in your `nextflow.config` file to make sure the processes are using the BioContainers that you set, and not the container image we have been using in this training. When the process blocks are changed, you should be able to run them with:

```
nextflow run script5.nf -with-docker
```
