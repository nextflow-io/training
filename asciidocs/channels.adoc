...
When you run it, it prints only 2, as you can see below:.
...
Besides, in many situations Nextflow will implicitly convert variablers to value channels. When you run a
process with a parameter (`params.example`), the content is just a string, for example, but when the process
will receive it as a value channel.
