...
process foo {
  input:
  tuple val(pair), file(reads)
  
  output:
  tuple val(pair), file('*.bam'), emit: bam_ch
...
process bar {
  ...
  output:
  tuple val(pair), file('*.bai'), emit: bai_ch
...
process gather {
  input:
  tuple val(pair), file(bam)
  tuple val(pair), file(bai)
...
process gather {
  input:
  tuple val(pair), file(bam), file(bai)
