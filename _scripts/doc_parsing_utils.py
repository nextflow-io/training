"""
Utilities for extracting and validating document elements.

Adapted from FastAPI's doc_parsing_utils.py for Nextflow training.
"""
...
def replace_header_permalinks(
    ...
    for header_no in range(len(header_permalinks)):
        ...
        if permalink:
            ...
            modified_text[line_no] = f"{hashes} {title}"
...
def replace_markdown_links(
    ...
    for i, link_info in enumerate(links):
        ...
        # Replace with original URL but translated text
...
def replace_html_links(
    ...
    for link_index, link in enumerate(links):
        ...
        # Replace with original attributes but translated text
...
def get_code_block_lang(line: str) -> str:
    """Extract language from code block fence line."""
...
def extract_multiline_code_blocks(text: list[str]) -> list[MultilineCodeBlockInfo]:
    """Extract all multiline code blocks from text."""
...
def _split_hash_comment(line: str) -> tuple[str, Union[str, None]]:
    """Split a line into code and hash-style comment."""
...
def _split_slashes_comment(line: str) -> tuple[str, Union[str, None]]:
    """Split a line into code and slash-style comment."""
...
def replace_multiline_code_block(
    ...
    Raises ValueError if the blocks are not compatible.
    """
...
def check_translation(
    ...
    """
    Check and optionally fix a translation against the English source.
    ...
    fixed_doc_lines = replace_markdown_links(
        doc_lines, doc_markdown_links, en_markdown_links
    )
    if auto_fix and (fixed_doc_lines != doc_lines):
    ...
    fixed_doc_lines = replace_html_links(doc_lines, doc_html_links, en_html_links)
    if auto_fix and (fixed_doc_lines != doc_lines):
